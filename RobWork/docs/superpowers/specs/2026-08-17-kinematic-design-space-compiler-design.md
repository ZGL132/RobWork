...
第一阶段采用面向昂贵、非连续、带碰撞和硬约束的黑箱问题的无梯度混合优化。算法只读取 `CompiledDesignSpace.independentVariables`；派生变量按依赖顺序计算，禁用变量固定为名义值。

### 10.1 统一设计向量

连续变量在算法内部优先使用归一化坐标 `u_i in [0, 1]`，再映射到实际范围：`x_i = minimum_i + u_i * (maximum_i - minimum_i)`。整数变量按 `step` 生成并去重，离散变量从 `discreteOptions` 选择，派生变量不进入向量。

### 10.2 三种初始采样策略

`Random` 对每个独立变量做范围内随机采样，适合快速探索、高维和不规则空间；随机种子必须保存。

`Grid` 对每维切分固定数量的点并遍历组合，适合低维参数研究和敏感性验证。若有 `d` 个变量、每维 `k` 个点，组合数为 `k^d`，必须受候选数量上限保护。

`LHS`（Latin Hypercube Sampling）将每个连续变量分成候选数量个分层区间，每个区间采样一次并打乱组合，保证全范围覆盖。LHS 是 Hybrid 的默认初始采样方式。

### 10.3 Hybrid 六阶段流程

```text
Baseline Verified
  -> Initial Pool (LHS/Random/Grid)
  -> Quick Evaluation
  -> Feasibility + Score + Diversity Elite Selection
  -> Verified Elite Evaluation
  -> Local Perturbation / Local Sweeps
  -> Final Verified
  -> Sensitivity Analysis
```

1. **基线评估**：原始设计先执行 Verified，作为评估器有效性、改善幅度和报告比较基准。
2. **生成初始候选池**：只生成独立变量，随后执行范围、参数化模式、依赖关系和派生值检查。
3. **Quick 快速评估**：使用低密度工作空间采样、快速 IK 和快速碰撞筛选，淘汰编译失败、模型非法、明显不可达或明显碰撞的候选。
4. **精英选择**：可行候选优先；可行候选按目标分排序；不可行候选按硬约束总违反量排序；得分接近时保留设计空间距离较大的候选。
5. **Verified 精确评估**：对精英执行高密度采样、完整 IK、碰撞检查、可操作度、关节裕度和全部目标/约束评估，结果覆盖 Quick 结果。
6. **局部精修和最终复核**：围绕 Verified 精英生成局部扰动或多轮扫掠，最后对领跑候选执行 `finalVerificationCount` 次 Verified 重评估。

最终最佳方案必须满足 `feasible == true`、状态为 Verified、编译器/评估器版本匹配，并在可行候选中总目标分最高。

### 10.4 局部精修

局部扰动使用每个变量自己的单位步长和范围：长度通常采用毫米级步长，角度采用小角度步长，基座位置采用毫米级步长，离散变量只访问相邻选项或保持不变。初始局部半径可取变量范围的 `10% ~ 15%`。

推荐的多轮局部扫掠为：以当前中心生成邻域、评估邻域、选择最好可行邻居作为新中心，直到无改善、达到 `maxLocalSweeps`、达到局部评估预算或收到取消请求。

### 10.5 约束处理

约束在三个时点处理：生成前检查范围/步长/依赖，编译时检查拓扑/单位轴/几何/适配器，评估后检查可达率、碰撞、工作空间、关节裕度和结构尺寸。排序采用可行性优先；硬约束违反候选保留为 `Infeasible` 并记录证据，软约束进入惩罚项。

### 10.6 缓存、确定性和停止条件

缓存键包含基线模型指纹、编译后的设计向量、编译器版本、评估器版本和评估配置版本。相同输入必须得到相同指标和诊断；并行评估按稳定候选索引合并。随机种子、候选索引、阶段版本和缓存命中信息写入运行诊断。

第一阶段支持 `candidateCount`、`eliteCount`、`localEliteCount`、`maxLocalSweeps`、`finalVerificationCount` 和 `randomSeed`。后续可增加最大评估次数、时间预算、连续无改善轮数和收敛容差。所有阶段必须响应暂停、取消和全局评估错误。

### 10.7 后续混合变量扩展

本阶段不引入新的进化算法，但 `DesignVector` 协议必须允许未来采用“外层电机/减速器离散组合搜索 + 内层连续运动学/动力学优化 + 局部精修和完整约束复核”。算法可以替换，但设计空间编译、候选生命周期、约束优先级、缓存和评估阶段接口保持稳定。
