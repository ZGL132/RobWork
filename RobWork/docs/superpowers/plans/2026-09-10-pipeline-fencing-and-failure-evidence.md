...
- [x] **Step 3: 最小实现 mutex 临界区与失败记录 schema**
...
在 `validate-docs.ps1` 加入文本断言：PIPE 必须包含 Windows 命名 mutex、`lastFailureRecord` 与“阻塞等待返回后 renew”；ACC 不得再含把证据分支写为 `acc/<taskId>` 的操作指令。
...
Run: `pwsh -File RobWork/scripts/industrialrobot/validate-docs.ps1`
