...
if(USE_gtest)
    ...
    if(GTEST_FOUND)
        # FindGTest implementations differ: RobWork's module guarantees the
        # singular variable, while package-config modules commonly provide the
        # plural form. Normalize both before exporting RobWorkBuildConfig.
        if(NOT GTEST_INCLUDE_DIRS AND GTEST_INCLUDE_DIR)
            set(GTEST_INCLUDE_DIRS "${GTEST_INCLUDE_DIR}")
        endif()
        if(GTEST_INCLUDE_DIRS)
            set(_rw_gtest_include_dirs "")
            foreach(_rw_gtest_include_dir IN LISTS GTEST_INCLUDE_DIRS)
                file(TO_CMAKE_PATH "${_rw_gtest_include_dir}" _rw_gtest_include_dir_normalized)
                list(APPEND _rw_gtest_include_dirs "${_rw_gtest_include_dir_normalized}")
            endforeach()
            set(GTEST_INCLUDE_DIRS "${_rw_gtest_include_dirs}")
        endif()
