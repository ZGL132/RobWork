...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.19（O-41 所有者裁决落地——§9.5 新增 PRJ-LOCK-HELD 用户级诊断记录 subject 必填规范〔subject=项目对象 ProjectId——ERR-01 用户级 subject 边界，diagnostics 工厂对空 subject 抛 subject-missing〕＋§5 码清单/factory.open 行同格标注；diagnostics.md §8.2 "subject=∅"格同步对齐＝F-290 根因消解；纯设计文档修订零代码变更，详见 §15.4 v0.19；v0.18（PRJ-T16 文档与门禁同步——全文与实现零偏差汇总核对〔§3.1 组成表与磁盘实测对齐：README 指向行"§9"过期陈述更正为 §12；ResourceRefs/PackageService/Upgrade 三头与 StoreTypes 的 StorePath 类型为"阶段 B 落位/零消费者不预建"的已登记未落位项，非偏差；§3.4 布局图后补磁盘现状注记——增量文件随 v0.5～v0.17 各任务登记落位〕＋§11 实现状态逐组登记（PRJ-TX-1～14，标题"未实现未运行"过期陈述同步更正）＋§15.3 P-PR-1～8 状态逐条同步（P-PR-4/P-PR-7 确认消账——对端冻结答复与交叉核对逐项一致；P-PR-5/P-PR-6 裁决申请提交 traceability/wp04-t16-adjudication-requests.md——O-09 处置联动，不私裁）＋README 复核登记；详见 §15.4 v0.18；v0.17＝PRJ-T15 契约测试与崩溃注入落地登记——§11 表 PRJ-TX-1～14 用例体收口：F8 真进程五挡（§7.6——TestProcessRunner kill 于 T1～T6 边界后重启恢复，RecoveryReport 逐项＋字节不变，AT-11/13）＋PRJ-TX-7 存储上下文层三场景（TestProcessRunner 重述——第二实例只读+PID/存活不接管 D-03/崩溃并发接管仅一胜者 D-02）＋PRJ-TX-8 真进程半边（在途归档强杀——D-13 无 manifest＝不完整）＋PRJ-TX-2 观测点诊断字段完整（testkit ContractCheck.checkDiagnosticRecord 机器校验）＋PRJ-TX-13 只读全入口遍历（§11 预期"错误码集合＝设计清单"）＋PRJ-TX-11 前置未就绪显式登记（另存为/包实体＝阶段 B WP-04-T17/T18，GTEST_SKIP 不计通过）＋testkit 消费接入（FaultInterceptor 经 src/win32/IFileOps 接缝 D-10 形态/TempDir/DeterministicEnv/IRD_TEST_INFO/IRD_EXPECT_*/TestProcessRunner/EventWatch/RecordListener——T-1 允许形态仅测试目标，产品目标零 testkit 边由 LinkageContractTest.NoTestkitEdge 双侧钉住；两测试目标自有 main 安装 TestRecordListener——ird-test-report.json 与 gtest XML 并存）；v0.16＝PRJ-T14 归档端口落位登记——公共头 `ArchivePort.hpp`〔ArchiveRequest/ArchiveItem/ArchiveBatch/ArchiveStatus/ArchiveEndReason/IResultArchivePort/ArchiveSessionRef——实现口径①～⑦见 §15.4〕＋`src/ArchiveServiceImpl.{hpp,cpp}`〔§5.6 begin/batch/finalize/abandon、§10.1 两段式发布与幂等冲突、§9.6/§9.7 引用持有与失权防护〕＋`ProjectStore.hpp` 增挂 `archive()` 访问器〔五端口访问器全部挂载完成〕＋`Codec` 增量 `dump(RunManifest)`＋`DiagRecords.hpp` 增量 makeArchiveConflict；v0.15＝PRJ-T13 撤销/重做服务；v0.14＝PRJ-T12 草稿服务；v0.13＝PRJ-T11 确认放行流；v0.12＝PRJ-T10 命令服务；v0.11＝PRJ-T09 查询端口；v0.10＝PRJ-T08 打开与恢复；v0.9＝PRJ-T07 事务引擎；v0.8＝PRJ-T06 修订/分支/元数据；v0.7＝PRJ-T05 对象库；v0.6＝PRJ-T04 格式契约与编码；v0.5＝PRJ-T03 写锁与路径；v0.4＝PRJ-T02 存储原语；v0.3＝PRJ-T01 构建落位；v0.2＝全链一致性审计消账；v0.1＝首版草案） |
| 日期 | 2026-09-09 |
...
| `README.md` | 既有保留位说明（不参与编译；任务卡指向已由 §9 更正为本文 §12——2026-09-10 版修正，PRJ-T01 复核、PRJ-T16 再复核零偏差） | — |
...
> 磁盘现状登记（2026-09-17，PRJ-T16 实测——与上图增量对照，均随 §15.4 v0.5～v0.17 各任务登记落位，非偏差）：`src/` 另有 `Project.cpp`（PRJ-T01 静态库锚点翻译单元）、`win32/IFileOps.hpp`/`win32/ILockOps.hpp`（PRJ-T02/T03 故障注入接缝——D-10 生产窄接口/测试侧 fake 形态）、`ConfirmationFlow.{hpp,cpp}`（PRJ-T11 确认放行流）、`DiagRecords.hpp`（PRJ-T10 PRJ-\* 诊断记录共享工厂）；查询实现落位名为 `QueryPortImpl.{hpp,cpp}`。公共头侧 `ResourceRefs.hpp`/`PackageService.hpp`/`Upgrade.hpp` 为 §3.1 表内标注的阶段 B 落位项（零消费者不预建——NFR-MNT-04 同款纪律），`StoreTypes.hpp` 的 StorePath 类型零消费者暂不落位（v0.10 登记）。
...
project 侧稳定诊断码清单（**码值分配权威＝diagnostics StableCodeRegistry——已收编全量 10 项〔diagnostics.md §4.6，2026-09-10〕，与下列清单一致（P-PR-6 码值部分消账；sink 名称/归属统一仍按 P-PR-6 裁决）**）：`PRJ-LOCK-HELD`（只读打开且含持有 PID；用户级记录 subject=项目对象 ProjectId 必填——O-41 裁决，见 §9.5）、`PRJ-STORE-CORRUPT`、`PRJ-RECOVERY-IGNORED-UNCOMMITTED`、`PRJ-RECOVERY-ORPHAN-DRAFT`、`PRJ-RECOVERY-DANGLING-OBJECTS`、`PRJ-SCHEMA-FUTURE`、`PRJ-FORMAT-LEGACY`、`PRJ-STALE-REVISION-REJECTED`、`PRJ-ARCHIVE-CONFLICT`、`PRJ-WRITE-AUTHORITY-LOST`。
...
| --- | --- | --- | --- | --- |
| `factory.open` | 进程内同规范路径未见 writable 打开 | 返回存储上下文或稳定错误；Writable 请求失败降级 ReadOnly 并携带 `PRJ-LOCK-HELD`（PM-07 不阻塞等待；subject=项目对象 ProjectId——§9.5 O-41） | 线程安全（内部互斥）；读校验期间持读句柄 | workflow 打开向导第②③⑤步调用 |
...
**PRJ-LOCK-HELD 用户级诊断记录字段规范（O-41 所有者裁决，2026-09-22——findings F-290 根因消解）**：本码为**用户级**（进用户目录，category=PermissionOrLock／severity=Warning，码表权威＝diagnostics.md §4.6），按 ERR-01"每条用户级稳定诊断以 subjectObjectId 绑定对象"的边界强制，**subject 必填＝项目对象标识（ProjectId）**——锁竞争场景（§9.9 实例 2 只读打开）manifest 已读出项目身份，ProjectId 可得；若连 manifest 都不可读则走 PRJ-STORE-CORRUPT 族，不在本码语义内。记录其余字段不变：paramSchema{pid,host}（持有者 PID 与主机名——§9.9 图中"PRJ-LOCK-HELD(pid)"的 (pid) 指 paramSchema 参数而非 subject）、非比较型。**为什么必须补**：本单元自身路径经 IDiagnosticsSink 适配器直接构造 core::DiagnosticRecord（PRJ-T03 落位口径），未过 diagnostics IDiagnosticFactory 校验面，缺 subject 此前不被本单元察觉；首个经工厂消费的下游（WP-10-T15 ProjectDiagnosticsBridge）即触发契约异常 subject-missing（Factory.hpp 校验面）——发射侧补 subject 后记录全量合法，PM-07 双写者场景恢复"用户可见『可写：否』＋真实原因"的正常降级链。落地登记：DTB §4 O-41（已裁决）；实现侧随发射代码修正任务消账（findings F-290 余留项）。
...
## 11. 验证方案及故障注入矩阵（用例体已随 PRJ-T15 按本表落地——逐组实现状态见表后登记〔2026-09-17 PRJ-T16 同步〕；执行留痕 traceability/builds/wp04-t15/）
...
**实现状态登记（PRJ-TX-1～14；2026-09-17 PRJ-T16 同步——用例体随 PRJ-T15 落地，执行留痕 `traceability/builds/wp04-t15/`，两模式一致：单元 254/255 通过＋1 显式跳过、契约 28/28；本登记为设计矩阵的状态映射，不替代验收记录 `traceability/acceptance/`，未执行者不标注通过）：**

| 组 | 实现载体（具名测试） | 状态 |
| --- | --- | --- |
| PRJ-TX-1 并发与过期基线 | CommandServiceTest 并发串行组＋CommandContractTest（双线程并发恰序提交／过期基线 Rejected(stale-revision) 零修订） | 已执行通过（v0.12 落地，v0.15/v0.17 复验零回归） |
| PRJ-TX-2 断言与确认 | CommandServiceTest 确认四路（Path1～Path4）＋BindingVerify 逐成员扰动；Tx2DiagnosticRecordContractTest（观测点收口——testkit checkDiagnosticRecord 机器校验诊断字段完整） | 已执行通过 |
| PRJ-TX-3 双编译 | CommandContractTest 桩三路（WC 失败／DWC 失败／双成功——任一失败零修订零发布、.staging/tmp 清理） | 已执行通过 |
| PRJ-TX-4 事务边界（F1～F8） | TxEngineTest F1～F7 逐边界注入（进程内——含磁盘满/重命名拒绝/清理失败面，旧版本字节不变）＋ProcessCrashContractTest F8 真进程五挡（T1～T6 停靠→kill→重启恢复，RecoveryReport 逐项＋字节两档断言） | 已执行通过（真进程组连跑 3 轮稳定） |
| PRJ-TX-5 分支与撤销/重做 | RevisionIndexTest 走查七步＋闭包/防回退断言；UndoRedoServiceTest undo/redo 新修订、空历史稳定提示、跨分支栈隔离 | 已执行通过 |
| PRJ-TX-6 草稿 | DraftServiceTest（round-trip 零修订／截断→.bak 恢复＋损坏诊断／.new 残留处置／放弃幂等） | 已执行通过 |
| PRJ-TX-7 锁 | StoreLockTest/PathCanonicalTest/LockContractTest（原语层，PRJ-T03——含真双进程只读降级＋PID／卡顿不接管／崩溃接管恰一胜者）＋ProcessLockContractTest（存储上下文层，PRJ-T15——两级对照不互相替代） | 已执行通过 |
| PRJ-TX-8 UI 关闭归档未完 | ArchiveContractTest 进程内模拟（存活至 finalize→一次性回调→事件先于关闭回调全序；跨项目事件不写入）＋ProcessCrashContractTest 真进程半边（在途会话随宿主强杀——批次残留、manifest 不在、listRuns 空） | 已执行通过 |
| PRJ-TX-9 迟到写与误认 | ProjectStoreTest PRJ-TX-9 落位面（Closed/Draining 迟到写拒绝＋稳定诊断、闭包外修订不可见、激活前失败不影响当前项目）＋Tx11Tx13SuiteTest（恢复报告 uncommittedRevisions＝F5 后重开浏览面） | 已执行通过 |
| PRJ-TX-10 磁盘与清理 | TxEngineTest F 系注入（与 TX-4 共用故障矩阵——F2 磁盘满→Failed(disk-full)＋残留仅诊断等）＋ObjectStoreTest 磁盘满/写拒绝/读残余映射 | 已执行通过 |
| PRJ-TX-11 复制一致性 | Tx11CopyConsistency.StageBPrerequisiteNotReady_SkipRegistered——**阻塞登记**：另存为/包导入导出实体＝阶段 B WP-04-T17/T18（§13.1/§5.8），GTEST_SKIP 显式跳过不计通过（AGENTS §4.2） | 阻塞（前置实体未就绪，非缺陷）；用例体语义随实体落位任务执行留痕（O-09 处置条款联动——裁决申请见 `traceability/wp04-t16-adjudication-requests.md`） |
| PRJ-TX-12 元数据不变量 | RevisionIndexTest 闭包双通道（CreateBranch 修订可达）／谱系成环注入／分支表回退注入（发布边界 branch-metadata-regression 拒绝） | 已执行通过 |
| PRJ-TX-13 只读全入口 | Tx11Tx13SuiteTest.TraversesEveryWriteEntry_AllRejectedWithStableDiag（错误码集合＝设计清单＋零磁盘副作用＋读轨对照） | 已执行通过 |
| PRJ-TX-14 构建红线 | BuildRedLineTest（零 Qt/无私有头出 include/CMake 无业务单元边）＋LinkageContractTest（产品目标零 testkit 边——双侧钉住；测试目标消费面白名单登记） | 已执行通过（ird_gates 命中集合与 wp04-t15 基线逐项一致，新增命中均有契约明文依据并转登 F-173/WP-01-T03） |
...
| # | 问题 | 依据 | 影响 | 建议 | 需要裁决者 |
| --- | --- | --- | --- | --- | --- |
| P-PR-1 | core.md v0.1（Draft）契约可能随冻结调整 | core.md 文档头状态行 | 本文 §3.2 消费清单相关任务返工 | core 冻结时出 diff，project 按影响面增量修订并留痕；不私改 core 语义。**〔PRJ-T16 状态同步 2026-09-17〕仍未决（冻结未发生）**：core.md 实测 v0.11（Draft-Structured，未冻结）；PRJ-T01～T15 全部落位登记的处置＝消费 core v0.1 公共契约基线、零 core 修改（BuildRedLineTest/LinkageContractTest 自动覆盖＋各任务类型同一性与 round-trip 用例钉住）——冻结 diff 后按影响面增量修订路径不变 | core 详设所有者 |
| P-PR-2 | ARCHITECTURE v0.11 状态 Draft 待评审 | 架构文档头 | 评审结论可能要求同步（依赖表/端口/锁机制表述） | 评审后按影响面增量修订。**〔PRJ-T16 状态同步 2026-09-17〕仍未决（评审未完成）**：ARCHITECTURE 实测 v0.12（"全链一致性审计事实修正"，仍 Draft 待评审）；其 v0.11→v0.12 增量未触及本文锚点（同 diagnostics.md P-DIAG-2 实测口径）；评审结论产出后按影响面增量修订 | 架构所有者 |
| P-PR-3 | ARCH §7.1/S1 走查将"前置断言"画在 project 命令服务框内，而断言逻辑（物性/限位/策略）属业务与策略 | ARCH §7.1 图示 vs §3.5 依赖边（project 仅依赖 core,diagnostics）与 N-1 边界 | 若按字面读法 project 须实现业务断言——违反分层 | 本文解释：命令服务**编排**断言阶段，判定由注入处理器＋④端口执行（§6.5/§6.6）；请架构侧确认该读法（不改依赖方向）。**〔PRJ-T16 状态同步 2026-09-17〕裁决中（架构所有者）**：本文解释已随 PRJ-T10 兑现（命令服务零业务数值，断言判定全在注入处理器——测试处理器走查验证），O-08 登记维持，等架构评审确认该读法 | 架构所有者 |
| P-PR-4 | 归档端口调用线程与 RunRegistry 对接细节未定（execution 详设未产出） | ARCH §4.2"调度线程……写 results/"与本文 writer 互斥（§9.8） | 契约测试线程模型需二次对齐 | execution 详设起草时按 §5.6/§9.8 冻结调用线程与重复投递上限。**〔PRJ-T16 状态同步 2026-09-17〕已消账（对端已冻结、两侧一致，本文确认）**：execution.md §9.5 冻结答复（其 P-EX-5 明言"请 project 侧确认消账"）与本文逐项核对一致——调度线程调用＋经本文 §9.8 writer 互斥串行化／begin 白名单前置校验（§5.6）／writeBatch≠finalize＝唯一完整标志（D-13）／abandon 全路径无永久等待（§9.7）／StoreError 码消费（§4.4.8 封闭集）；RunRegistry 登记项映射、完成事件重复投递上限、强杀 abandon 三时机由 execution §9.5 单侧冻结承载，本文零增量需求（PRJ-T14/T15 实现与契约测试按同语义落地——§15.4 v0.16/v0.17）；对端 P-EX-5 消账由其卡修订承接，本文不私改对端 | execution 详设所有者 |
| P-PR-5 | ARCH §3.5 依赖表未登记 project→io 边，而任务约束与 PM-05/CON-03 需要 project 消费 io 能力 | ARCH §3.5 表（表外同层边＝构建失败） | 阶段 B 包导入导出/固化的编译期依赖不合法 | 建议架构修订补登 project→io（接口依赖，无环）；备选：project 定义 IPackageCodec/IResourceReader、L5 注入 io 实现（零编译边）。**〔PRJ-T16 状态同步 2026-09-17〕裁决申请已提交（O-09 处置联动——不私裁）**：申请与依据见 `traceability/wp04-t16-adjudication-requests.md`；io 侧 P-IO-1 承接答复已按注入式设计且接口不因裁决结果变化（io.md §3.1/§8 依赖表）；ARCH §3.5 实测仍无 project→io 边；阶段 A 零 io 依赖不受影响；阻塞面＝WP-04-T18 编译形态（WP-04-T17/T18 届时按 DOC-T10 契约 O-09 处置条款回填：kept planned＋blocked 原因或注入式先行显式登记） | 架构所有者 |
| P-PR-6 | PRJ-\* 稳定诊断码值与 IDiagnosticsSink 形态待 diagnostics 冻结 | diagnostics 详设未产出；ARCH §3.5 project→diagnostics 边已登记 | 码表冲突或适配器返工 | diagnostics.md 起草时收编 §5.0 建议清单；sink 接口以其详设为准。**〔PRJ-T16 状态同步 2026-09-17〕码值部分已消账（v0.2——diagnostics.md §4.6 收编全量 10 项）；sink 名称/归属统一与链接形态裁决申请已提交**：申请与依据见 `traceability/wp04-t16-adjudication-requests.md`（diagnostics.md §9.7 IDiagnosticSink＋DiagnosticsSinkImpl 兼容形态已交付——P-DIAG-5，report/reportDev 与本文 §5.0 签名逐字一致；本文维持注入式先行、零 diagnostics 链接边〔LinkageContractTest 钉住〕；消账路径＝裁决后本文增量修订收编统一接口与链接形态，不私改对端；execution P-EX-8 同案由其卡联动） | diagnostics 详设所有者 |
| P-PR-7 | ICommandInteraction（ui Marshal/取消）与 IModelCompilePort（runtime）最小契约需对端确认 | 本文 §5.3.3/§5.3.6 为单侧冻结 | 对端详设若另立形态需本文增量修订 | ui/runtime 详设起草时以本文契约为起点交叉核对。**〔PRJ-T16 状态同步 2026-09-17〕已消账（两侧交叉核对完成，本文确认）**：ui 半区——ui.md §9.2/§16.3 D-4 交叉核对通过（Marshal＋阻塞桥、占槽零资源、无超时自动确认、关闭主动取消均可满足）；runtime 半区——runtime.md §10.2 承接答复"兼容"（其 P-RT-8），两处差异按同步命令路径语义确认无增量需求：①CompileResult 无快照句柄——命令路径仅验证可编译性（MDL-06 门禁，产物瞬态；PRJ-T10 S5 只消费 ok＋diagnostics）；②无取消令牌——同步执行（ARCH §4.2 命令执行线程），取消走命令中止（§5.3.4 关闭取消确认等待；编译在确认复核之后）；runtime 侧 P-RT-8 消账由其卡修订承接，本文不私改对端 | ui/runtime 详设所有者 |
| P-PR-8 | 分支 label 创建时一次写入、不可改名是否满足 PM-11"当前方案"显示需要 | PM 明确不做"分支改名"；PM-11 标题栏需显示方案名 | 若需可编辑方案名则走需求变更 | 维持"创建时 label＋不可改名"；P-PR-8 登记请需求侧确认口径。**〔PRJ-T16 状态同步 2026-09-17〕裁决中（需求所有者——O-19 登记维持）**：本文维持既有设计（PRJ-T06 落地双重防线——derive 无改名路径＋发布边界 INV-M3 不可变条目拒绝漂移）；若需求侧确认需可编辑方案名则走需求变更，本文按结论增量修订 | 需求所有者 |
| P-PR-9 | §4.4.4 commandType 冻结语法 `^[a-z0-9-]{3,64}`（不含点）与 §6.5 内置命令族示例 `project.create-branch`（含点）矛盾 | 本文 §4.4.4 表 vs §6.5 原文（PRJ-T04 实施期发现，2026-09-15） | PRJ-T10 落位内置元数据命令族时注册 token 被字段校验拒绝，或需扩语法 | 二选一由 project 详设所有者裁决：①扩语法为 `^[a-z0-9.-]{3,64}`（修订 §4.4.4 表）；②内置命令族改用不含点 token（修订 §6.5 示例）。PRJ-T04 按 §4.4.4 字段级契约执行（round-trip/拒绝语义不受影响），正向测试样本不含点，不在代码中私自裁决 | project 详设所有者 |
...
| v0.5 | 2026-09-15 | PRJ-T03（≙WP-04-T03）写锁与路径落位登记：`src/win32/StoreLock.{hpp,cpp}`＋`PathCanonical.{hpp,cpp}`＋`ILockOps.hpp`（锁面生产窄接口接缝，IFileOps 先例 D-10 形态；故障点 `project/store-lock/*`）＋公共头 `StoreTypes.hpp` **增量落位**（§9.2 LockHolderRecord＋§5.0 IDiagnosticsSink——§3.1 其余登记类型随 PRJ-T04/T08 增补，不预建）；§9.1～§9.6 全部兑现（D-02 独占句柄内核原子裁决／构造即获取＋写定宽 PID 记录／析构即失权／心跳线程原地重写纯诊断〔SA-17 结构性落实：心跳线程无权限状态写通路〕／§9.2 撕裂容忍读＋只读共享不破坏互斥／§9.4 永不删除重建＋接管残留恢复诊断／§9.5 原始码透传映射归 T08／§9.6 失权三道防线：①状态机门卫 requireWriteAuthority〔全部写入口统一防线〕②GetHandleInformation 权威探测③三码 I/O 分类锁存 LostWrite）；**P-PR-6 处置按契约 acceptance 4 落地**：锁诊断经 IDiagnosticsSink 适配器产出 core::DiagnosticRecord（PRJ-LOCK-HELD／PRJ-WRITE-AUTHORITY-LOST 逐字码值；注入式先行——本单元首个 core 头消费点 StoreTypes.hpp→DiagData.hpp，构建图零 diagnostics 链接边），sink 名称/归属统一仍归 P-PR-6/P-EX-8 裁决不消账。实现口径登记三项（§9 未定义判据，DTB §5.4）：①锁记录＝三行定宽文本 113 字节（pid 零填充/host 空白填充/hb 定宽 ISO 毫秒）——§9.2"固定宽度记录"落值；②框架全局 `_WIN32_WINNT=0x0501`（RWS_DEFINITIONS，SA-02 不可动全局）下 SetFilePointerEx/GetFinalPathNameByHandleW 需按 TU 提升 SDK 基线至 0x0601（声明门控宏，无 ABI 影响，先于各 TU 首个 windows.h）；③初始心跳时间戳由调用方时钟供给（LockSelfRecord.initialHeartbeatUtc——诊断字段非权限判据，交由调用方支持测试构造陈旧心跳验证 D-03；心跳线程启动后以系统时钟重写）。测试＝StoreLockTest 13＋PathCanonicalTest 7＋LockContractTest 5（真双进程：只读降级＋PID／卡顿不接管——心跳 2 h 陈旧＋冻结实据／TerminateProcess 后双实例并发接管 Exit {0,3} 恰一胜者／崩溃-接管全链文件身份不变＝永不删除重建／跨拼写多路径拒绝），两模式 48/48 零回归（PRJ-T01/T02 既有 18 用例全绿）；TestProcessRunner 形态归 PRJ-T15 按 testkit §6.5/§10.2 节奏重述。本文契约语义零变更。证据：traceability/builds/wp04-t03/ |
| v0.6 | 2026-09-15 | PRJ-T04（≙WP-04-T04）格式契约与编码落位登记：公共头 `PersistenceFormat.hpp` **增量落位**（§4.2/§4.4 字段级契约六类型——ProjectStaticIdentity/HeadRecord/RevisionManifest/ProjectMetadataRecord/CommandRecord/DraftDocument；身份字段一律 core 规范文本〔§3.2 消费清单：Identity.hpp/Digest.hpp，using 声明显式引入，P-PR-1 基线零 core 修改〕；DraftDocument 落位说明：§3.1 组成表列于 DraftService.hpp 行，但 §4.4 开头"以下类型在 PersistenceFormat.hpp 冻结"＋任务契约 acceptance 1 明确在本头冻结，PRJ-T12 复用不重定义）＋`StoreTypes.hpp` 增量落位 §5.0 StoreError/StoreErrorCode（21 稳定码封闭集，§4.4.8 token 一一对应）＋`src/Codec.{hpp,cpp}`（§3.4 布局私有实现头——R-2；§4.8 canonical 编解码 `parse(dump(x))==x` 全类型、§8.11 版本判定拒绝、CR-02 唯一摘要入口 contentVersionOf）。§4.8/§8.11 全部兑现（ASCII 纯输出＋固定字段序＝表列序＋无浮点＋可选空值省略；主版本破坏性拒绝 FormatLegacy/SchemaFuture＋升级指引数据三键 document/supported/upgrade〔PM-06 数据侧，升级器本体归阶段 B〕＋次版本追加可选字段容忍）。实现口径登记三项（§4/§8.11 未定义判据，DTB §5.4）：①schemaVersion 编码＝主版本×10000＋次版本（当前 1.0＝10000）；②token 类字段（objectTypeToken/moduleId/externalRefId/state/时间戳等）落值口径＝非空/可打印 ASCII/≤128，路径类字段（absolutePath）独立口径＝非空/无控制字符/≤4096——防失控字段不收词表；③无版本字段的类型（RevisionManifest/CommandRecord——§4.4.2/§4.4.4 表无 schemaVersion/formatId）版本语义由容器判定，Codec 按当前支持版本严格解析。**CR-02 处置按契约 acceptance 4 落地**：contentVersionOf()＝本单元唯一哈希入口（core::ContentDigester——D-10 对收到的字节计算），dump/parse 对全部 digest 类字段零计算只透传（"不私设第二哈希路径"以非真实摘要 round-trip 反证用例钉住）。**新登记待裁决 P-PR-9**（§15.3）：§4.4.4 commandType 冻结语法与 §6.5 内置命令族含点示例矛盾——PRJ-T10 落位前需裁决，PRJ-T04 按字段级契约执行。测试＝PersistenceFormatTest 43 用例（round-trip 9/非法拒绝 20/版本兼容 5/externalRefs 3/摘要边界 5/core 消费基线 1），两模式 81/81 零回归（PRJ-T01/T02/T03 既有 38 用例＋contract_test 10 用例全绿）；ird_gates 命中集合与 wp04-t03 逐项一致（零新增命中）；validate-task PASS。证据：traceability/builds/wp04-t04/ |
| v0.7 | 2026-09-16 | PRJ-T05（≙WP-04-T05）对象库落位登记：`src/ObjectStore.{hpp,cpp}`（§3.4 布局私有实现头——R-2；§4.6 五段语义实现载体）。**编址**：objects/<oid 规范文本>/<cv 64hex 无 tag>——目录名/文件名派生与读回解析全走 core 公共 API（toCanonical/tryFromCanonical），本地零第二格式化。**只增发布**：publishObject＝writeThrough 持久性闸门→publishNew（PRJ-T02 原语）；目标已存在→读回 size＋摘要校验一致＝共享（磁盘零写入＋mtime 不变）/不一致＝store-corrupt；EXISTS→消失竞态有界重试 2 次。**读取校验**：首次加载 size＋SHA-256 双半边校验（size 半边＝读得字节数与目录项比对——部分读/截断检出）；通过后记入已校验集，同上下文免复检（校验结论生命周期＝存储上下文——负载逐出不清除已校验结论，用例双向钉住）；tryObject 缺失＝空 optional 不抛、object 缺失＝store-corrupt（§5.2 错误表）、篡改一律 store-corrupt。**LRU 缓存**：默认 256 MiB 可配（§4.6 原文）、键＝(oid,cv)、读路径填充（惰性加载——发布不预热，保证发布后首读必过校验）、单条目超预算不缓存；cacheStats/cacheLruOrder 开发观测面；读写并发安全（互斥内部实现）。**引用存在性**：scanObjects 对照引用集产出四桶——missingReferenced〔有引用无对象，调用方映射 store-corrupt〕/corruptReferenced〔全量档校验失败〕/danglingOnDisk〔悬挂对象＝开发诊断 reportDev，只读不删——GC 不做〕/malformedEntries〔命名不合规〕；报告确定性排序（枚举序不进结果）。实现口径登记三项（§4.6 未定义判据，DTB §5.4）：①错误映射＝磁盘满→disk-full、拒绝访问→access-denied、其余写路径失败→write-rejected（§7.1 第 2/4 步口径）、读路径不可读残余→store-corrupt（无法证明完整性按数据侧拒绝）；②发布侧共享校验强制实算（不信任历史校验结论——判定对象是"磁盘既有字节"）；③悬挂/不合规发现以返回报告＋reportDev 双通道交付，用户级文案仍归 diagnostics（P-PR-6 链路不动）。**CR-02 处置按契约 acceptance 4 落地**：ObjectStore 零哈希实现——cv 一律经 codec::contentVersionOf（core::ContentDigester）；**P-PR-1 处置**：消费面＝core 公共契约 Identity.hpp/Digest.hpp，零 core 修改。实现期缺陷留证：初版发布成功预热读取缓存，被 TryObject_TamperedContent/TamperedSize 真实失败暴露违背"首次加载执行校验"（发布后首读命中缓存跳过校验）——移除预热后全绿（§4.6 惰性加载语义以用例钉住）。测试＝ObjectStoreTest 28 用例（publish 8/read 7/immutability 1/cache 4/scan 6/core 契约 2），两模式 109/109 零回归（既有 81 用例＋contract_test 10 用例全绿）；ird_gates 命中集合与 wp04-t03/wp04-t04 逐项一致（零新增命中）；validate-task PASS。证据：traceability/builds/wp04-t05/ |
| v0.8 | 2026-09-16 | PRJ-T06（≙WP-04-T06）修订/分支/元数据落位登记：`src/RevisionIndex.{hpp,cpp}`（§3.4 布局私有实现头——R-2；§4.3/§4.5 语义模型载体；**纯进程内、零磁盘 I/O**——清单/元数据经 registerRevision/registerMetadata 由调用方喂入〔深拷贝、注册后不可变＝PA-2 模型层〕，文件读写归 PRJ-T07 TxEngine）。**提交闭包**（§4.5 伪码迭代化）：通道一 parent 链＋通道二元数据谱系 supersedes→committedBy，visit_meta 将 committedBy 修订**作为修订继续访问**（§4.5.1 结论③"M4→M3(r3)→r3→r0"行走形态——CreateBranch 修订 r1 无子修订、非 tip，仅此步保证可达）；访问集保证终止（合流去重；环由发布边界 INV-M2 前置拒绝，闭包不重复判定），悬挂引用（parent/metadataRef/supersedes/committedBy 未注册）＝store-corrupt 不得静默截断；结果确定性排序（revisions 按 seq,id；metadata 按规范文本——NFR-COR-02）。**发布边界校验** validateMetadataPublish（§4.5"发布期校验"唯一实现；统一稳定码 branch-metadata-regression＝§4.4.8"防御性，§4.5 不变量"）：INV-M1a branchId 唯一／INV-M1b tip 与新增分支 base 指向已注册修订／committedBy 绑定本次新修订／无新版本拒绝（candidateRef＝authoritativeRef 同编址）／D-5 链（candidate.supersedes 恰为权威引用键）／INV-M3 超集（既有分支不丢失）＋不可变条目（label〔P-PR-8 一次写入〕/base/createdAt 逐字相等）＋tip 不回退（变更须恰为新修订＋新修订所属分支＋旧 tip 为其真祖先——沿 parent 链判定）／INV-M2 谱系无环（访问集含候选键种子——自环/间接环均拒）＋版本单调（committedBy seq 向权威方向严格递减）。时序契约：先 registerRevision(新修订) 后校验（祖先/seq 判定前提）；喂入失真（权威未注册/内容不符/候选键已注册）＝invalid_argument fail-fast。**D-5 机制化** deriveNextMetadata：权威版本深拷贝为唯一底本＋显式增量声明制（TipUpdate/addedBranches）——无任何"从父修订元数据重建"入口，禁止路径由 ParentMetadataRebuildRejected 反证用例钉住。**O-15 落位**（§6.1）：allocateRevisionSeq＝水位+1 只进不退，S6 决策点后调用（失败不消费）；adoptHeadSeq 按 HEAD.revisionSeq 断点恢复（D-06，只升不降）——崩溃残留序号跨会话可重合但不同在闭包，committed 单调不受影响（§4.5.1 结论④）。**branchTip** 为静态纯函数并把"从哪份元数据读"显式留给调用方——权威纪律（只读 HEAD 引用版本、禁经旧修订元数据重建）由走查步骤 6/7 用例钉住。**P-PR-1/P-PR-8 处置按契约 acceptance 4 落地**（消费基线零 core 修改；label 双重防线：derive 无改名路径＋发布边界拒绝漂移）。线程：读路径互斥并发安全、写路径调用方串行化（§6.1 命令执行槽）——§4.7 模型层同款。实现口径登记两项（§4.5 未定义判据，DTB §5.4）：①闭包对"带环损坏存储"按访问集截断不死循环（环防线在发布边界；残缺闭包由调用方覆盖率核对暴露）；②谱系版本单调判据＝committedBy 修订 seq 严格递减（每次提交恰一修订＋至多一新元数据版本——§6.1 S6 推论）。卡头文档版本行同步至 v0.8（补齐 v0.5～v0.7 未更新漂移），本文契约语义零变更。测试＝RevisionIndexTest 24 用例（Walkthrough 2/Closure 4/PublishValidation 10/SeqAllocation 1/Derive 3/Registration 1/CoreContract 3），两模式 133/133 零回归（既有 109 用例＋contract_test 10 用例全绿）；ird_gates 命中集合与 wp04-t05 基线逐项一致（零新增命中）；validate-task PASS。证据：traceability/builds/wp04-t06/ |
| v0.10 | 2026-09-16 | PRJ-T08（≙WP-04-T08）打开与恢复落位登记：公共头 `ProjectStore.hpp` **增量落位**（§5.1 生命周期与身份子集——ProjectStore〔writable/lockInfo/projectId/schema/canonicalPath/requestClose/closed/subscribeClose〕＋ProjectStoreFactory〔open/createNew〕＋ICloseObserver；**实现口径登记（DTB §5.4）**：§5.1 原文的五个端口访问器 query/commands/drafts/archive/undoRedo 随 PRJ-T09/T10/T12/T13/T14 增量挂载——各端口实现类未存在，先落访问器只能 stub，与 §3.1"随任务节奏增量落位、不预建无消费者接口（NFR-MNT-04）"同一纪律）＋`StoreTypes.hpp` 增量 §5.1 打开协议类型（OpenMode/ObjectCacheBudget/SchemaInfo/LockInfo/RecoveryReport/OpenStoreRequest/OpenStoreResult；§3.1 点名的 StorePath 不落位——零消费者，出现真实消费者时随其任务登记）。`src/ProjectStoreImpl.{hpp,cpp}`（私有实现头——R-2）：**打开协议②③⑤**（②project.json 版本判定〔FormatLegacy/SchemaFuture＋升级指引三键 detail，PM-06 原文件不动〕→锁半步〔D-02 内核裁决/HeldByOther 降级只读＋PRJ-LOCK-HELD/进程内重复 Writable 打开直接拒绝——§9.3 进程内注册表/显式 ReadOnly 读持有者记录〕→③HEAD 跨文件一致性〔projectId/schemaVersion 不一致＝store-corrupt——§4.1 行〕＋闭包装载〔两段式：TxEngine::scanForRecovery 预检＋跳过 uncommittedRevisions 搬运——**闭包外修订不注册进会话索引**＝§7.3/PRJ-TX-9 可见性纪律的结构前提，T09 查询端口消费同一索引〕→⑤孤儿草稿扫描）＋**createNew**（PM-01 存储侧：同卷组装区完整组装→逐顶层条目 rename 就位→失败清理不留半成品；身份分配经 core generate〔P-PR-1〕；对象发布经 ObjectStore 校验通道）＋**生命周期**（§9.6① 状态机门卫〔Draining/Closed→ContextClosed＋PRJ-WRITE-AUTHORITY-LOST；只读→LockHeldByOther＋PRJ-LOCK-HELD；失权→WriteRejected〕＋§9.7 Draining 排空〔在途引用 shared_ptr 票据 RAII〕＋一次性关闭回调〔析构静默终局不回调〕）＋内部写通道 executeCommit（writer 互斥＋TxEngine::commit＋权威元数据读回前进）。**实现口径登记七项**（详见 src/ProjectStoreImpl.cpp 文件头：open 步骤顺序/锁 OsError §9.5 映射/createNew 组装区/两段式装载/孤儿草稿保守判定/草稿 flush 钩子随 T12/析构终局）。**P-PR-1 处置按契约 acceptance 4 落地**：身份生成与 DiagnosticRecord 全部消费 core 公共契约头（Identity.hpp/DiagData.hpp v0.1 基线），零 core 修改；**P-PR-6 处置按契约 acceptance 4 落地**：诊断码 PRJ-FORMAT-LEGACY/PRJ-SCHEMA-FUTURE/PRJ-STORE-CORRUPT/PRJ-RECOVERY-IGNORED-UNCOMMITTED/PRJ-RECOVERY-ORPHAN-DRAFT/PRJ-LOCK-HELD/PRJ-WRITE-AUTHORITY-LOST 全部取自 diagnostics.md §4.6 收编清单，经 §5.0 IDiagnosticsSink 注入（打开期 CollectingSink 双通道——RecoveryReport.diagnostics 快照与 sink 上报同源同序，报告数据面不依赖装配注入），零 diagnostics 链接边（LinkageContractTest 零改动维持）。测试＝ProjectStoreTest 18 用例（acceptance 1＝PRJ-TX-9 落位面 4 用例——Closed/Draining 迟到写拒绝＋稳定诊断、闭包外修订不可见、激活前失败不影响当前项目〔完成条件具名用例〕；acceptance 2＝旧格式/未来版本拒绝＋升级指引＋恢复报告逐字段 4 用例；acceptance 3＝生命周期/只读打开/进程内互斥 6 用例；acceptance 4＝P-PR-1/P-PR-6 自证 2 用例＋边界 2 用例），两模式 169/169＋contract_test 10/10 零回归（PRJ-T01～T07 既有 151 用例全绿）；validate-task PASS。写入口代表口径：全写入口 archive.begin/submit/draft.save 随 T10/T12/T14 经同一门卫挂载，PRJ-TX-9 全量用例归 PRJ-T15 复验。本文契约语义零变更。证据：traceability/builds/wp04-t08/ |
| v0.9 | 2026-09-16 | PRJ-T07（≙WP-04-T07）事务引擎落位登记：`src/TxEngine.{hpp,cpp}`（§3.4 布局私有实现头——R-2；§7 全节实现载体）。**七步协议**（§7.1 逐步表逐项）：第 1 步准备＝tx-id（进程内单调＋随机后缀，§4.1 口径）＋`.staging/<tx-id>/`（含 tmp 子目录——D-09 临时产物落点随事务建立）＋序号分配（O-15：S6 决策点＝进入本事务后，失败不回收——跨会话 adoptHeadSeq 断点恢复同语义）；**内存段先行**（顺序即原子性，D-09 前置精神）：计划字段/tipUpdate 一致性校验→deriveNextMetadata（D-5 唯一入口）→registerRevision→validateMetadataPublish（INV-M1/M2/M3）→registerMetadata（"切换权威"——闭包可达前提；validate 契约要求 candidateRef 未注册故必须在校验后）——全部失败零磁盘副作用。第 2 步暂存＝全部内容写 `.staging/<tx-id>/`（write-through＋flush 闸门，§7.2 行 1；失败残留设计行为——恢复扫描处置）；第 3 步验证＝重读 size＋SHA-256 全量（含对象字节→ContentVersion 名一致性）不符＝StoreCorrupt 定位到文件＋开发诊断；第 4 步发布＝只增就位（对象先行〔元数据对象→域对象〕→command.json→manifest.json 最后——"清单在而对象缺"中间态不可达），目标已存在经 ObjectStore 读回共享判定（一致＝共享/不一致＝store-corrupt），部分失败已就位项悬挂不回滚；第 5 步＝head.new 暂存→MoveFileExW(REPLACE_EXISTING|WRITE_THROUGH) 唯一提交点→重读 HEAD 自校验 manifestDigest（失败＝StoreCorrupt 硬错误，D-18 不回滚，恢复④兜底）；第 6 步＝事件 FIFO（RevisionCommitted→DependencyInvalidated）失败重试一次＋开发诊断（每败即诊断）不回滚（D-18）；第 7 步＝清理失败仅开发诊断。**恢复扫描**（§7.4①④⑤＋§7.3）：①.staging 残留忽略不删＋PRJ-RECOVERY-IGNORED-UNCOMMITTED（每 tx 目录一条明细清单）＋.staging/tmp 启动清理（§4.1 两行不同处置的对照）；④HEAD 闭包完整性（装载磁盘清单入临时索引→committedClosure→闭包内全部对象引用 size＋SHA-256 全量档）失败＝headIntegrityVerified=false＋PRJ-STORE-CORRUPT 定位到文件；§7.3 识别＝磁盘修订目录集＼闭包集＝uncommittedRevisions（确定性排序 NFR-COR-02）；⑤悬挂对象只读计数不删。**损坏分流纪律**（实现口径登记，DTB §5.4）：清单/元数据缺失或损坏仅对闭包成员构成 store-corrupt，闭包外残缺（如 F4"有目录无清单"）＝未提交残留——装载期失败先挂账、闭包计算成功后统一归类。**对象发布不经 ObjectStore::publishObject**（实现口径登记）：§7.1 第 2 步要求暂存落事务目录＋§7.6 要求注入经 IFileOps 接缝，而 publishObject 绑定对象库级暂存与真实 Win32FileOps——引擎自做暂存写＋publishNew，共享判定复用 ObjectStore 读回校验通道；publishObject 保留给非事务发布路径。**修订身份随 CommitPlan 带入**（实现口径登记）：分配点＝命令服务 S6 计划装配（单一分配点），tipUpdate.newTip 一致性提前校验（invalid_argument），validateMetadataPublish 二道防线；序号由引擎分配。**元数据对象类型 token＝"ProjectMetadata"**（kMetadataTypeToken，随清单持久化一经发布不变）。**IFileOps 接缝增补目录两方法** createDirectories/removeTree（faultpoint::kCreateDirectories/kRemoveTree——§7.6 表头"注入经 IFileOps 接缝"对 F1/F7 目录级边界的承载；§4.1 白名单路径消费侧校验不变；AtomicFileTest fake 同步补全转发，既有用例零语义变化）。**P-PR-1 处置按契约 acceptance 4 落地**：清单摘要/对象 cv 全经 contentVersionOf（core ContentDigester——CR-02 唯一入口，测试以 core 直算三方比对钉住）；事件消费 core::IDomainEventBus（v0.1 §4.9/§5.8）；恢复诊断码 PRJ-RECOVERY-IGNORED-UNCOMMITTED/PRJ-STORE-CORRUPT（diagnostics.md §4.6 收编清单，不私造码）；core.md 仍 Draft-Structured，冻结 diff 后按影响面增量同步。测试＝TxEngineTest 18 用例（acceptance 1＝F1～F7 逐边界注入＋内存段零磁盘副作用；acceptance 2＝F8 四边界进程内模拟〔T1/T2-3/T4 旧态字节不变、T5-6 新态字节不变、T7 控制腿——进程级 kill 归 PRJ-T15〕；acceptance 3＝闭包外识别/查询不可见＋.staging/tmp 清理；acceptance 4＝core 摘要直算比对），两模式 151/151 零回归（既有 133 用例＋contract_test 10 用例全绿）；ird_gates 命中集合与 wp04-t06 基线逐项一致（零新增命中）；validate-task PASS。实现期缺陷留证：初版 8 用例失败全部为测试侧期望错误（osError 5 误期望 write-rejected——映射表拒绝访问→access-denied 系 PRJ-T05 口径①正确行为；共享 .staging/tmp 误以为随事务创建——§4.1 该目录归非事务操作），引擎零缺陷修改。测试口径登记两项（DTB §5.4）：①F3"写后篡改"注入挂 closeHandle 而非 flush（暂存句柄 share mode 0 独占——句柄存活期二次打开共享冲突）；②"字节不变"断言两档（严格全等＝零发布边界；expectCommittedUnchanged＝既有文件不变＋新增仅限悬挂残留——§7.1 第 4 步"已就位项保留"）。卡头文档版本行同步至 v0.9，本文契约语义零变更。证据：traceability/builds/wp04-t07/ |
| v0.11 | 2026-09-16 | PRJ-T09（≙WP-04-T09）查询端口落位登记：公共头 `QueryPort.hpp` **增量落位**（§3.1 组成表行——IProjectQueryPort 十一方法＋视图值类型 RevisionView/ProjectMetadataView/BranchTip/DraftInfo/RunInfo＋InverseRef；§5.2 原文签名逐条锚定）＋`ProjectStore.hpp` 增挂 `query()` 访问器（§5.1 原文形态——②端口含只读实例，v0.10 登记的增量挂载承诺兑现）＋`PersistenceFormat.hpp` 增量 **RunManifest/RunManifestItem 读取类型**（§4.4.7 字段级契约；首消费者＝listRuns，dump 随 PRJ-T14 写路径——PRJ-T04 六类型“随消费者落位”同一纪律）＋`src/Codec.{hpp,cpp}` 增量 parseRunManifest（读侧严格解析，容器判定口径同 RevisionManifest/CommandRecord）。`src/QueryPortImpl.{hpp,cpp}`（私有实现头——R-2）：**视图/闭包约束/缓存/隔离**（§12 行四要素）——修订视图自会话索引组装（metadataRef 完整引用形态防御校验＋command.json 摘要/逆命令读取）；head 重读磁盘 HEAD＋tryRevision/revision 仅会话索引可达（装载纪律＋writer 互斥一致性窗口双防线——“已注册未切 HEAD”的提交中修订不可见，§4.5.1 结论④/A-6）；currentMetadata/branchTips/branchHistory 自 HEAD 引用权威版本（INV-M3 收口）；metadataAt 返回历史快照（R2 浏览，非权威——注释钉住）；对象读取直通 ObjectStore（惰性＋首读校验＋LRU——§4.6）；listDrafts 扫当前有效草稿（.new/.bak 残留不列——§8.4；stale 自权威 tip 判定）；listRuns 仅 finalize（D-13——manifest 原子发布“在即完整”，按修订过滤）；runDir 规范编址 results/<run-id>。**实现口径登记五项**（DTB §5.4，详见 QueryPort.hpp/QueryPortImpl 文件头）：①tryObject noexcept 边界＝一切失败 nullopt＋reportDev（§5.2 签名原文与“损坏＝抛”二分的异常面冲突，noexcept 契约优先、损坏不静默；强语义走 object()）；②hasUnresolvedPayload 判据注入（CommandKnownFn 可空函数——本阶段注册表未装配〔PRJ-T10 产物〕字段按 §5.2 默认 false，“无法判定”≠“已知未知”；T10 注入后真判定）；③branchHistory 未知分支＝invalid_argument（封闭集无码不私扩——§5.2 表“未知分支抛”）；④清单扫描容错＝单条目损坏跳过＋reportDev、迭代中断整表 StoreCorrupt（不交付半表）；⑤ProjectMetadataView＝{ObjectRefPair ref; ProjectMetadataRecord record}（§5.2 未给字段级定义——复用 §4.4.3 冻结类型不重定义；InverseRef＝InverseCommand using 别名同口径）。**线程与生命周期**（§4.7/§9.8）：全方法并发只读安全——抛轨方法在 writer 互斥内取“HEAD/权威快照＋首级索引读”、锁外组装只读不可变数据（PA-2）；对象/清单面不取 writer 锁（对象不可变/草稿与 manifest 原子替换或原子发布）；Draining 不拒查询（PM-03 关闭对话框呈现数据源），仅 Closed 拒绝（context-closed——tryObject 以 nullopt 表达＋开发诊断）。**P-PR-1 处置按契约 acceptance 4 落地**：视图携带的全部身份类型（RevisionId/BranchId/ObjectId/ContentVersion/RunId/TaskIdentity）与 core 公共契约类型**同一**（QueryPortTest 编译期 static_assert 五断言＋运行期规范文本往返——零 core 修改、零本地第二身份格式化；core 冻结 diff 后按消费点增量同步）。测试＝QueryPortTest 14 用例（acceptance 1＝ConcurrentReadOnly 4 线程×40 轮×全方法＋主线程并发提交〔head 单调/字节恒等/链完整；gtest 断言不进工作线程——互斥消息表汇合〕＋ViewIsValueCopy；acceptance 2＝闭包外端口级不可见〔对照 ProjectStoreTest 索引级断言升级〕＋try 轨不抛＋context-closed 全方法拒绝＋只读实例查询〔PM-07〕；acceptance 3＝修订/元数据/分支/草稿/结果六类视图逐字段＋ObjectCacheBudget 预算传导与 LRU 逐出〔具名验收点〕；acceptance 4＝Ppr1 类型同一性），两模式 183/183＋contract_test 10/10 零回归（PRJ-T01～T08 既有 169 用例全绿）；validate-task PASS。**构建红线误报修复登记**：BuildRedLineTest NoQtInclude 的 `#include "Q` 前缀启发式误中单元卡 §3.1 点名头 QueryPort.hpp/QueryPortImpl.hpp——自有 Q 头显式白名单排除（Qt 形态判定不变），随本任务留痕。本文契约语义零变更。证据：traceability/builds/wp04-t09/ |
| v0.12 | 2026-09-16 | PRJ-T10（≙WP-04-T10）命令服务落位登记（**部分交付——内置元数据命令族 blocked by P-PR-9**）：公共头 `CommandService.hpp` **增量落位**（§3.1 组成表行——CommandEnvelope/CommandStatus/CommandResult/ObjectWrite/MetadataChange/CommandPlan/PrepareOutcome/HandlerContext/ICommandHandler/ICommandInteraction/IModelCompilePort/CompileRequest/CompileResult〔§5.3.6 三字段冻结签名——P-PR-7 单侧冻结不私改；§6.6“携带专用临时目录”与冻结签名的冲突按冻结面执行，临时落点 D-09 由命令服务编排兑现——实现口径⑤〕/HandlerRegistry/ProjectCommandService）＋`ProjectStore.hpp` 增挂 `commands()` 访问器＋`src/CommandServiceImpl.{hpp,cpp}`（§12 行产物——**S1 形式校验**〔type→payload 版本→branch 存在→writable；拒绝码逐条 §6.3〕/**S2 基线解析与过期拒绝**〔expectedRevision 缺省＝tip；失配→Rejected(stale-revision)＋PRJ-STALE-REVISION-REJECTED 双通道——diagrec::makeStaleRevision〕/**串行槽**〔§6.1 每上下文一命令执行槽——submit 全程互斥＋§9.7 在途票据〕/**S3 处理器 prepare**〔HandlerContext：objectId 分配/query 只读/编译端口/交互回调——P-PR-3 处置：命令服务零业务数值，断言判定全在注入处理器〕/**S4 确认决策映射**〔非交互＝confirmations-unresolved/拒绝＝confirmations-rejected/失效与抛出＝interaction-lost/确认＝凭据回填 finding；绑定复核与 command.json 留痕归 PRJ-T11——§12 分工〕/**S5 双编译编排**〔§6.6：编译先于 S6、任一半边失败→Failed(compile-failed)＋诊断透传＋无修订无暂存；命令结束清空 .staging/tmp〕/**S6 CommitPlan 装配**〔修订身份单一分配点——修订/对象/分支三 generate；CreateBranch 型＝无 tipUpdate＋addedBranches base/tip＝源分支 tip（§4.5.1 走查步骤 1/3），其余 tipUpdate{活动分支→新修订}；逆命令三元组持久化 §6.9〕）＋`src/DiagRecords.hpp`（PRJ-* 诊断记录共享工厂——T08 原文件局部两工厂收敛＋stale 新增，防码值/文案漂移）＋**HandlerRegistry**（§5.3.5——重复/空 token 注册＝invalid_argument 边界拒绝；find 并发安全）＋**QueryPort 判据注入兑现**（T09 预留的 CommandKnownFn 接注册表查询＋project-init bootstrap 豁免——根修订不误报 unresolved，§6.3 末行真判定）。**P-PR-9 阻断如实登记**：§6.5 内置元数据命令族（project.create-branch 等含点示例）与 §4.4.4 冻结语法冲突的裁决权在所有者（§15.3 原文“不在代码中私自裁决”）——本任务不落位该命令族实体，机制面（注册表/提交/持久化/查询判据）与建支**语义机制验证**（无点测试处理器 test-create-branch 走 CommandPlan.metadataChange 声明面，走查步骤 1 断言全绿）就绪；裁决后随增量任务落位处理器实体与注册。**P-PR-6 处置按契约 acceptance 5 落地**：PRJ-STALE-REVISION-REJECTED/PRJ-LOCK-HELD/PRJ-WRITE-AUTHORITY-LOST 逐字码值（diagnostics.md §4.6 收编清单）经 §5.0 IDiagnosticsSink 注入产出，零 diagnostics 链接边；**P-PR-1 处置**：事件消费 core Events.hpp v0.1 基线（TxEngine 第 6 步既有面——D-18 端到端用例经工厂装配上下文验证发布失败不回滚）。**实现口径登记六项**（DTB §5.4，详见 CommandService.hpp/CommandServiceImpl 文件头）：①S1 未知分支→invalid-payload（封闭集不私扩）；②not-writable 携带门卫稳定码 error＋执行中 ContextClosed→Aborted(context-closing)；③建支型/常规型两种提交形态（§4.5.1 背书）；④逆命令载荷版本＝处理器当前受理版本；⑤.staging/tmp 清理编排（D-09）；⑥装配违约 fail-fast 两处。**缺陷修复登记（T08 遗留）**：openLocked 的 CollectingSink 为工厂栈局部，TxEngine/ObjectStore/StoreLock 长寿命部件绑定其指针——open 返回即悬空，打开后引擎级开发诊断（D-18 重试诊断等）踩悬空指针；修复＝shared_ptr 堆分配＋ProjectStoreImpl 持有 m_collectorLifetime 生命周期锚（声明序最先＝析构最后）。测试＝CommandServiceTest 23＋CommandContractTest 5（PRJ-TX-1 双线程并发串行/过期基线、PRJ-TX-3 桩三路——契约面按 §3.3 归 contract_test），两模式 206/206＋15/15（契约目标 6 连跑稳定）；ird_gates 实跑命中 9 项＝7×SUB＋1×T1＋1×R4，与 wp04-t08/t09 基线逐项一致（零新增——R4 为 T08 既有工具版本串字面量，base 核对非本任务引入）；validate-task PASS。证据：traceability/builds/wp04-t10/ |
| v0.13 | 2026-09-16 | PRJ-T11（≙WP-04-T11）确认放行流落位登记（§5.3.3/§5.3.4/§6.7——T10 交付的 S4 决策映射之上的数据半区与留痕半区；§12 行"绑定复核与 command.json 留痕归 PRJ-T11"的分工兑现）：`src/ConfirmationFlow.{hpp,cpp}`（私有实现头——R-2）：**绑定四元组设施**〔findingDigest（SHA-256 over DiagnosticRecord project 侧 canonical 编码——编码表 magic "IRD-PRJ-FINDING-DIGEST-V1"＋tag＋u64le 长度前缀框架，双精度 IEEE-754 位模式大端直编无浮点文本化；CR-02 唯一算法路径＝core::ContentDigester）/policyContentId（策略语境投影——code＋比较三要素＋cause，magic "IRD-PRJ-POLICY-CONTEXT-V1"；**实现口径②**：project 无④策略端口通道〔P-PR-3——判定与阈值读取全在域处理器〕，finding 内嵌比较语境即 project 可见面的已解析策略内容〔CON-06〕，真实域处理器落位后经计划声明面增量切换）/commandDigest（SHA-256 over payloadCanonical）/baseRevisionId（S2 解析基线）〕＋**freezeConfirmation 冻结**（§4.4.4 ConfirmationRecord 五字段——凭据 confirmedAtUtc 经 win32::formatIsoMilli 转 ISO-8601 带毫秒文本〔PRJ-T11 增量：StoreLock 抽出共享格式化主体，行为零变化——单元内唯一时间格式来源〕）＋**verifyBinding 复核**（四元组序逐成员定性——首个不符成员即返回 BindingVerdict）＋**IConfirmationProbe/ProductionConfirmationProbe**（实现口径⑤——D-10 生产窄接口/测试侧 fake，IFileOps/ILockOps 先例：§6.7 失配分支生产不可达〔"输入变化不可能——槽内无并发写"明文〕，冻结不经探针直调生产真值、探针只在复核点被咨询；CommandServiceImpl::setConfirmationProbe 装配通道＝setCompilePort 同款）。`CommandServiceImpl` 编排接入：**S4 冻结＋确认提交时复核**（回调返回后、放行前——diagnostics.md §5.5 要点⑦前半）/**S5 编译前复核**（IModelCompilePort 调用前——要点⑦后半，确认不豁免编译 MDL-06④ 顺序面）/**S6 留痕落位**（confirmations[] 入 commitPlan.command——§4.4.4 空表缺省省略）。**实现口径登记五项**（DTB §5.4，详见 ConfirmationFlow.hpp/.cpp 文件头）：①findingDigest 编码表；②policyContentId 阶段 A 来源；③复核时机落点（两处——非编译命令仅确认提交时）；④失配处置＝Rejected(confirmations-unresolved)（§6.7"按未确认处置"＋diagnostics.md §5.3 同语），失配维度经 reportDev 定位（§4.6 收编清单无确认流用户码，不私造——CR-08）；⑤探针接缝。**P-PR-7 处置按契约 acceptance 4 落地**：ICommandInteraction/IModelCompilePort 签名零改动（git diff 为证——§5.3.3 单侧冻结面不私改；§5.3.3 行为契约〔命令执行线程同步调用/三态映射〕由具名用例钉住）；**P-PR-6 处置按契约 acceptance 4 落地**：确认流诊断仅 reportDev 开发通道经 §5.0 IDiagnosticsSink 注入产出，零 diagnostics 链接边（LinkageContractTest 零改动维持）；**core.md §13.2 交接项（确认留痕编码）**：编码由本单元 §4.4.4/ConfirmationFlow 实现——五字段磁盘契约随 PRJ-T04 已冻结，本轮兑现其唯一生产写入点。测试＝CommandServiceTest 新增 13 用例（acceptance 1＝PRJ-TX-2 四路 Path1～Path4＋BindingVerify 逐成员扰动〔四路均无修订＋无新 command.json＋诊断字段完整 ERR-01 逐字段——testkit ContractCheck 谓词消费随 PRJ-T15 按 T-1 口径登记〕；acceptance 2＝占槽/零事务资源/关闭取消〔requestClose 非阻塞入 Draining＋回调 ui 拆除形态→Aborted(interaction-lost)＋票据释放排空收尾——无永久等待〕/用户取消/回调同步于提交线程；acceptance 3＝confirmations 入 command.json 逐字段 round-trip＋空省略＋UX-03 比较三要素齐备与文案边界〔project 确认流零用户级诊断〕；acceptance 4＝横跨上列——零 Widgets〔BuildRedLineTest 源码扫描自动覆盖新增文件〕），两模式 219/219＋contract_test 15/15 零回归（既有 206＋15 全绿）；ird_gates 实跑命中 9 项＝7×SUB＋1×T1＋1×R4，与 wp04-t08/t09/t10 基线逐项一致（零新增）；validate-task PASS。实现期缺陷留证两项：①appendField 双重载对字面量歧义（收敛 string_view 单入口）；②测试侧扰动探针初版误设前 N 次真值（冻结不经探针设计下扰动须自首次咨询生效）。本文契约语义零变更。证据：traceability/builds/wp04-t11/ |
| v0.14 | 2026-09-16 | PRJ-T12（≙WP-04-T12）草稿服务落位登记（§5.4/§8.1～§8.6——PM-04"保存与应用分离"的存储侧全量：落盘不产生修订、应用归命令端口、基线冲突拒绝并保留）：公共头 `DraftService.hpp` **增量落位**（§3.1 组成表行——DraftService 五方法〔save/tryLoad/summarize/discard/list〕＋DraftProjection/DraftProjectionItem〔§8.5〕＋SaveResult〔§5.4 原文〕＋DiscardResult；DraftDocument 复用 PersistenceFormat §4.4.5 冻结类型不重定义）＋`ProjectStore.hpp` 增挂 `drafts()` 访问器〔§5.1 五端口访问器的 T12 位〕＋`src/DraftServiceImpl.{hpp,cpp}`（§12 行产物——**落盘协议**〔§5.4 原文序列：writeThrough(.new 持久性闸门)→replaceFile(current→.bak 轮换，REPLACE_EXISTING——.bak 恒为上一版，D-12)→replaceFile(.new→current 原子就位)→失败清理 .new；writer 互斥内执行 §9.8〕/**写门卫三道**〔与 executeCommit 同序同码走**返回值轨**——ContextClosed/LockHeldByOther/WriteRejected，save/discard 不抛环境/状态错误＝ui 定时器安全；§9.7 在途票据持于保存全程〕/**tryLoad 恢复入口**〔§8.4：.new 残留丢弃→current 解析＋归属三元组校验→draft-corrupt 诊断〔sink＋diags 双通道〕→.bak 回退〔旧草稿保留〕→.bak 亦损追加诊断 nullopt〕/**summarize 投影**〔§8.5：权威 tip 锁内快照 INV-M3→锁外扫盘→moduleId 字典序 NFR-COR-02〕/**discard 幂等目标态**〔current/.bak 删净或本来没有＝ok〕/**list**〔§8.4"打开时 listDrafts＋tryLoad"的恢复面入口——与查询端口 listDrafts 语义同源各自落位〕）＋`DiagRecords.hpp` 增量 **makeDraftCorrupt**（PRJ-RECOVERY-ORPHAN-DRAFT 的单文件形态——diagnostics.md §8.2 映射行"draft-corrupt→该码，附加上下文＝模块/分支"；唯一装配点纪律）。**实现口径登记四项**（DTB §5.4，详见 DraftService.hpp/DraftServiceImpl.hpp 文件头）：①discard 返回 DiscardResult（§5.4 代码块 SaveResult 与 §3.1/任务契约双锚点不一致——按双锚点落位＋删除明细）；②discard 额外清理 .new（放弃的完备语义——用户明确不要的数据不因崩溃残留复活）；③投影 present 位阶段 A＝磁盘事实投影（损坏文件条目 present=false——阶段 B 域模块注册面就位后扩展，ui 两源拼接 §8.5 口径不变）；④save 对未知分支不校验（§5.4 前置仅"上下文 Active"——孤儿兜底在打开协议⑤步）。**P-PR-1 处置按契约 acceptance 4 落地**：草稿负载中 SourcedValue/ValueProvenance 来源标记**透传不解释**（§3.2 消费清单——payload 经 Codec 仅 UTF-8 健全性检查，round-trip 逐字节一致具名用例＋磁盘直读双证）；消费基线＝core.md v0.1（Identity/DiagData 既有契约面），**零 core 修改**（BuildRedLineTest 源码扫描＋LinkageContractTest 唯一单元边自动覆盖新增文件），core 冻结出 diff 后按影响面增量同步（§15.3 既有口径）。模块 token 白名单 A-Za-z0-9_-（1～64——§4.1"注册的模块 token"的路径安全面，PersistenceFormat"注册表校验归 DraftService"落位；违约 fail-fast 不静默净化）。测试＝DraftServiceTest 14 用例（acceptance 1＝PRJ-TX-6 四用例〔round-trip 不产生修订/截断→重开 .bak 恢复＋损坏诊断/.new 残留丢弃 current·.bak 保留/DiscardResult 幂等〕；acceptance 2＝应用恰一修订＋StaleRevisionRejected 拒绝并保留〔apply-retained 读回〕＋多模块投影〔排序/stale/present/损坏面〕；acceptance 3＝§8.1 覆盖写 vs 只增＋§8.6 只读拒绝写可读＋Closed 拒绝＋60 s 周期边界〔PM-04-S1 归 R2/WP-04-T20 不提前实现——autosave/manual 同一入口〕；acceptance 4＝透传自证＋白名单/跨项目 fail-fast 面〕，两模式 233/233＋contract_test 15/15 零回归（既有 219＋15 全绿）；ird_gates 实跑命中 9 项＝7×SUB＋1×T1＋1×R4，与 wp04-t08～t11 基线逐项一致（零新增——R4 为 T08 既有工具版本串字面量，base 核对非本任务引入）；validate-task PASS。实现期缺陷留证两项（自测暴露修复，终态全绿）：①AtomicFile 门面方法名误记 replaceExisting（实为 replaceFile——编译期捕获）；②测试初版关闭后经旧上下文构造文档（查询面拒绝干扰被测语义——构造前置到关闭前）。本文契约语义零变更（§4.4.5/§5.4/§8.x 原文全部照语义实现，四项实现口径为文档内不一致的最小收敛登记）。证据：traceability/builds/wp04-t12/ |
| v0.15 | 2026-09-16 | PRJ-T13（≙WP-04-T13）撤销/重做服务落位登记（§5.5/§6.9——PM-18「撤销/重做＝新修订的提交机制」全量：历史不改写〔PA-2〕、空历史稳定提示、跨分支/过期基线/草稿边界）：公共头 `UndoRedo.hpp` **增量落位**（§3.1 组成表行——UndoRedoStatus〔canUndo/canRedo＋undoSummary/redoSummary 提示数据＋blockedReason 机器可读边界说明〕＋InverseRecord〔会话 redo 栈条目：撤销修订身份＋被撤销命令摘要＋原始命令三元组拷贝〕＋UndoRedoService 三方法〔status noexcept／undo／redo〕）＋`src/UndoRedoServiceImpl.{hpp,cpp}`（§12 行产物——**逆命令提交**：undo envelope 的 type/payload 恒取自当前 tip 修订的 inverse 记录、expectedRevision＝tip〔§5.5 原文〕，经命令端口 submit 走 S1～S7 全流程＝恰一新修订；**会话栈**：redo 栈按分支隔离、仅内存不持久化〔D-11〕，条目在 undo 提交后从撤销修订视图的 inverse 拷贝（对称声明面——§6.9 可逆性声明制）；**边界语义**：空历史稳定提示数据不抛错、前置违约 invalid_argument fail-fast、查询失败映射 Aborted(context-closing)/Failed〔与命令端口同表〕）＋`ProjectStore.hpp` 增挂 `undoRedo()` 访问器〔§5.1 五端口访问器的 T13 位；文件头增量落位说明同步更新——archive() 归 T14〕。**实现口径登记三项**（DTB §5.4，详见 UndoRedo.hpp/UndoRedoServiceImpl.hpp 文件头）：①「新命令清空 redo」（§6.9 行 4）＝惰性同步——status/undo/redo 进入时读权威分支表比对上次观察 tip，不一致且栈非空即清空＋一次性 stale 说明（§6.9 行 6→blockedReason=StaleRevisionRejected，复用 §4.4.8 现有码；旗标读即消费；status 轮询面不产用户级诊断——真拒绝由命令端口诊断链路承载）；②redo 条目数据源＝撤销修订 inverse 的时点拷贝〔PA-2 下与重读等价；取拷贝使 D-11 结构自明〕，非对称族不入栈＋reportDev 观察面（CR-08 不私造码）；③§5.5 undo 字面语义的推论＝撤销后再次 undo 即「撤销该撤销」（tip 的 inverse＝原始命令）——撤销深度由处理器 inverse 声明链决定，project 不推断可逆性；重启后 canUndo 由 tip 推导即该机制的极端情形。零自有写通道：undo/redo 一律经 commands() 唯一写路径（§6.5——逆命令由业务处理器产出，project 只拷贝三元组）。**P-PR-1 处置按契约 acceptance 4 落地**：服务面消费身份类型＝core.md v0.1 公共契约类型（编译期 static_assert＋运行期 toCanonical/tryFromCanonical 往返用例钉住——零本地第二身份形态）；零 core 修改（diff 无 core/ 文件；BuildRedLineTest＋LinkageContractTest 自动覆盖新增文件）。测试＝UndoRedoServiceTest 9 用例（acceptance 1＝PRJ-TX-5 undo/redo 部分两用例〔恰一新修订/seq 递增/目录只增/清单原序只增/视图逐字段不变——AT-29＋内容面重放佐证〕；acceptance 2＝五用例〔空历史稳定提示＋前置违约 fail-fast／跨分支提交后再回栈隔离＋undoSummary 数据源核验／重开 redo 丢失·canUndo 由 tip 推导·L5 重注册边界／未注册逆 token→unknown-command 透传零修订＋处理器就位后成功〔§6.5 注册协议〕／新命令清空 redo＋一次性 stale〕；acceptance 3＝草稿边界〔落盘零修订＋撤销后草稿原样保留——REQ-11 分界存储侧形态〕；acceptance 4＝P-PR-1 自证），两模式 242/242＋contract_test 15/15 零回归（既有 233＋15 全绿）；ird_gates 实跑命中 9 项＝7×SUB＋1×T1＋1×R4，与 wp04-t08～t12 基线逐项一致（零新增——R4 为 T08 既有工具版本串字面量，base 核对非本任务引入）；validate-task PASS。实现期缺陷留证两项（自测暴露修复，终态全绿）：①status() 一次性旗标消费误用 const 引用（C3490 编译期捕获）；②测试初版两处期望错误（B 分支 undoSummary 误期望为空——其自身 tip 本可逆；重开用例 redo 断言次序——对称族下重开后的首次 undo 会建立新会话 redo 条目）。本文契约语义零变更（§5.5/§6.9 原文全部照语义实现，三项实现口径为文档未定义判据的最小收敛登记）。证据：traceability/builds/wp04-t13/ |
| v0.16 | 2026-09-16 | PRJ-T14（≙WP-04-T14）归档端口落位登记（§5.6/§10.1/§10.2——TASK-03/PM-13 迟到结果归属的存储侧全量：绑定原修订不因 HEAD 拒绝〔A1/A8〕、manifest 原子发布＝完整判据〔D-13/CON-04〕、重投递幂等与冲突拒绝〔D-14〕、begin/abandon 全路径终结〔§10.1 两段式，无永久等待〕）：公共头 `ArchivePort.hpp` **增量落位**（§3.1 组成表行——ArchiveRequest〔五元组＋登记 runDir＋runKind/evaluationKey 透传〕/ArchiveItem/ArchiveBatch/ArchiveStatus/ArchiveEndReason 四值词表/IResultArchivePort 四方法〔§5.6 原文签名〕/ArchiveSessionRef〔值语义不透明句柄——类型擦除形态，实现细节不出公共头，R-2〕；RunManifest 复用 PersistenceFormat §4.4.7 冻结类型不重定义）＋`src/ArchiveServiceImpl.{hpp,cpp}`（§12 行产物——**begin**：§5.6 校验清单逐项〔五元组有效/项目归属 AT-10/Active 门卫三道/runDir 白名单＋run 绑定/manifest 存在性身份比对〕＋运行目录创建（§4.1"何时创建＝归档 begin 时"）＋会话登记（同 run 并发重复会话拒绝）；**writeBatch**：只增发布（writeThrough 持久性闸门→publishNew——§7.2 行 1/2，"存在的批次文件必完整"由 rename 原子性成立）＋批次级重投递（摘要一致跳过/不一致 PRJ-ARCHIVE-CONFLICT）；**finalize**：manifestDigest 计算回填→D-14 摘要比对（一致幂等不重写不重复发事件/不一致冲突拒绝）→磁盘核验把守发布路径（O-12 完整性发布义务——不发布与磁盘不符的"完整"声明）→write-through＋原子替换＝D-13 提交点→ResultArchived 事件→票据释放（§10.2 流程序）；**abandon**：全路径责任终结＋批次残留保留为"未完成"（CON-04）＋幂等）＋`ProjectStore.hpp` 增挂 `archive()` 访问器〔§5.1 五端口访问器全部挂载完成〕＋`Codec` 增量 `dump(RunManifest)`（§4.4.7 表列序写侧编码——manifestDigest 透传不重算，CR-02）＋自由函数 `computeManifestDigestHex`（digest 置空后 canonical 字节经 contentVersionOf——端口与测试复算同一函数）。**会话生命周期**（acceptance 3"闭包双通道归 project"的机制化）：begin 起持宿主在途票据（§9.7 引用持有通道——requestClose 排空等待覆盖在途归档，PRJ-TX-8 存活前提），每笔写经 StoreLock::requireWriteAuthority 探测（§9.6 失权防护通道）；宿主析构先 finishClose 置 Closed、m_archive 成员（声明序最末＝成员逆序析构中最先销毁）强制终结全部在途会话并锁外释放票据（删除器观测 Closed 不触发排空回调——析构静默终局口径）。**实现口径登记七项**（DTB §5.4，详见 ArchivePort.hpp/ArchiveServiceImpl.hpp 文件头）：①begin 的 manifest 存在性比对＝身份比对（请求不携摘要——语义等价的幂等判据前置到身份面，内容级 D-14 由 finalize 承载；同一 run 目录绑定一份登记事实，身份不符含同 runId 不同 attempt＝ArchiveConflict）；②runDir 白名单判据＝weakly_canonical 相等（白名单内＋绑定 run-id 单判据——A8 一致即通过、不一致即拒绝、不静默重定向）；③manifestDigest 被摘要对象＝除自身外全部字段的 canonical 编码（digest 置空后 dump 字节经 contentVersionOf；编码归 project §4.8，finalize 由端口计算回填、调用方该字段不参与——finalizedAtUtc 由调用方供给，代填会使重投递摘要必不一致与 D-14 矛盾）；④finalize 磁盘核验仅把守发布路径、D-14 比对先行（manifest 在＝已完整，重投递只与已发布事实比对——否则"声明了未写入文件"的分歧重投递会被误判 store-corrupt 而非 archive-conflict；实现期自测暴露后修正的顺序语义，留证）；⑤暂存后缀 ".ird-part" 与 manifest.json 为保留名（relPath 校验拒绝——防暂存名/正式名碰撞与完整性发布冲突；末分量另拒 Windows 设备名）；⑥幂等 finalize 命中不重复发布 ResultArchived（进程内总线恰一次——core Events.hpp；重复投递由 execution at-least-once 承接，存储侧不放大）；⑦已终结句柄 writeBatch/finalize＝fail-fast、abandon 幂等 no-op、空句柄一律 fail-fast（调用方契约错误不走返回值轨）。并发面：锁序＝宿主 lifecycle→writer→sessions（不反向）；票据释放一律锁外（§9.7 排空触发点不在锁内发生）；事件在终结态下发布、票据最后释放（§10.2 流程序）。**P-PR-4 处置按契约 acceptance 4 落地**：调用线程单侧冻结——任意线程进入、全部变更性文件操作经宿主 writer 互斥串行（§5.6/§9.8；契约目标双线程并发用例行为级自证）；RunRegistry 对接细节（登记项映射/重复投递上限/强杀 abandon 时机）不预设不私改对端契约——execution 详设（WP-08-T01 P-PR-4 冻结点）定稿后二次对齐。**P-PR-1 处置按契约 acceptance 4 落地**：ResultArchived 事件经 core IDomainEventBus（Events.hpp v0.1 基线 DomainEvent::make 工厂——失败重试一次＋开发诊断不影响归档完成，D-18 同源）；零 core 修改（diff 无 core/ 文件；BuildRedLineTest＋LinkageContractTest 自动覆盖新增文件）；core 冻结 diff 后按影响面增量同步（§15.3 既有口径）。测试＝ArchiveServiceTest 11 用例（acceptance 2＝F9/D-14 四用例〔完整判据三通道＋round-trip＋摘要自洽/重投递幂等不重写＋冲突拒绝＋诊断/批次级两粒度/磁盘核验三类不符→StoreCorrupt 且不发布假完整〕；acceptance 3＝四用例〔SessionRef 生命周期——requestClose 在途计数＋失效句柄 fail-fast＋abandon 幂等；Abandon 全路径终结——失败不终结、abandon 排空归零关闭＋一次性回调＋A-4＋残留未完成；relPath 白名单全拒绝矩阵＋零副作用＋空批；begin 拒绝矩阵——跨项目 AT-10＋attempt0＋runKind 空＋白名单外〕；acceptance 4 关联＝三用例〔TASK-03 绑定原修订 HEAD 前进照常归档＋r0 查询面可见；只读 begin→LockHeldByOther＋PRJ-LOCK-HELD；Draining 拒绝新 begin 而既有会话照常〕）＋ArchiveContractTest 3 用例（契约目标——§3.3 归档协作面：PRJ-TX-8 进程内模拟两用例〔存活至 finalize 后才 Closed＋一次性回调＋事件先于关闭回调全序戳；跨项目事件不写入三通道〕＋P-PR-4 并发自证〔双线程双会话全成功＋requestClose 同步完成；线程内不做 gtest 断言——QueryPortTest 汇合纪律〕），两模式 253/253＋contract_test 18/18 零回归（既有 242＋15 全绿）；ird_gates 实跑命中 9 项＝7×SUB＋1×T1＋1×R4，与 wp04-t08～t13 基线逐项一致（零新增——R4 为 T08 既有工具版本串字面量，base 核对非本任务引入）；validate-task PASS。实现期缺陷留证三项（自测暴露修复，终态全绿）：①finalize 初版磁盘核验先于 D-14 比对（分歧重投递被误判 StoreCorrupt——顺序语义修正，即实现口径④）；②测试初版两处笔误（单元素 ArchiveBatch 聚合初始化缺内层花括号；SessionRef 用例漏写 writeBatch 致核验失败）；③resolveActiveSession 初版占位实现（类型擦除句柄经别名 shared_ptr 底层指针反查注册表后定稿）。本文契约语义零变更（§5.6/§10.1/§10.2 原文全部照语义实现，七项实现口径为文档未定义判据的最小收敛登记）。证据：traceability/builds/wp04-t14/ |
| v0.17 | 2026-09-17 | PRJ-T15（≙WP-04-T15）契约测试与崩溃注入落地登记（§11 全表 PRJ-TX-1～14 用例体收口＋§12 行产物——testkit §6.4/§6.5/§10.1 消费兑现；零产品源码变更，全部改动位于 test/* 与 CMake 测试目标登记）。**真进程崩溃注入（acceptance 3）**：`test/Tx15Child.{hpp,cpp}`（contract_test 可执行文件自再执行的子进程半边——main 拦截 `--ird-wp04-t15-child` 分派，宽字符环境变量传参；停靠型 IFileOps fake 经 **testkit FaultInterceptor 接缝**消费 faultpoint 冻结 ID，D-10 形态）＋`test/ProcessCrashContractTest.cpp`：F8 参数化五挡（T1 暂存目录已建/T2T3 暂存验证完成/T4 发布完成 HEAD 未切/T5 提交点后〔post 停靠〕/T6 事件已发布清理未跑）——父进程 EventWatch.awaitFile 等停靠标记→TestProcessRunner.kill（outcome.Killed）→重启打开（死亡子进程遗留写锁的 D-02 接管路径一并实证）→RecoveryReport 逐项（完整性 true/暂存残留恰 1/PRJ-RECOVERY-IGNORED-UNCOMMITTED/无 PRJ-STORE-CORRUPT）＋字节不变（未提交挡旧状态全等、已提交挡 r0 文件逐字节不变＋HEAD/清单/命令新状态在盘）＋查询面 tip 判定——§7.6 F8/AT-11/AT-13 真进程载体；PRJ-TX-8 真进程半边（在途归档会话随宿主强杀——批次残留/manifest 不在/listRuns 空/恢复报告干净，D-13 反例面）。**写锁存储上下文层重述**：`test/ProcessLockContractTest.cpp` 三场景（第二实例降级只读＋holder PID 提示〔PM-07/§9.2〕；持锁者存活停靠期间不接管〔D-03〕＋强杀后接管且锁记录重写为胜利者〔§9.1〕；强杀后双竞争者并发恰一胜者——胜利者持锁停靠保证重叠窗口〔winner=Killed/loser=Exited 形态互证，D-02〕）——与 PRJ-T03 LockContractTest 的 StoreLock 原语层互为两级对照不互相替代。**PRJ-TX-2 观测点收口**：`test/Tx2DiagnosticRecordContractTest.cpp`——四路拒绝产出的 core::DiagnosticRecord 逐条过 testkit checkDiagnosticRecord 机器校验（allowTransient=true——测试域记录无主体；code 句法/必填串/比较型三要素 checkComparativeFields(Length)/NotApplicable 纪律）＋IRD_EXPECT_IDENTICAL 稳定码匹配；与 CommandConfirmTx2 组（单元面决策映射）零重复。**PRJ-TX-13 具名用例体**：`test/Tx11Tx13SuiteTest.cpp`——只读上下文遍历阶段 A 全部写入口（submit/undo→Rejected(NotWritable)＋LockHeldByOther＋PRJ-LOCK-HELD；redo→只读会话栈恒空 D-11 前置违约 invalid_argument〔不私造稳定码 CR-08〕；save/discard→返回值轨＋PRJ-LOCK-HELD；archive.begin→异常轨＋PRJ-LOCK-HELD）＋零磁盘副作用＋读轨对照面——错误码集合＝设计清单。**PRJ-TX-11 阻塞登记**：另存为/包导入导出实体＝阶段 B WP-04-T17/T18（§13.1/§5.8），GTEST_SKIP 如实标注前置未就绪（§7.2 计 skipped 不计通过——AGENTS §4.2），用例体语义随实体落位任务执行留痕。**testkit 消费接入（T-1 允许形态）**：两测试目标链接 sdurws_ird_testkit（产品目标零 testkit 边不变）——TempDir（四个新夹具）/DeterministicEnv·IRD_TEST_INFO·IRD_EXPECT_*（AssertMacros 展开于消费方 TU）/TestProcessRunner·EventWatch/ContractCheck；`test/TestMainReport.cpp`（_test 目标自有 main，GTest::gtest_main→GTest::gtest 换轨）＋LockContractTest main 增量——双 main 安装 TestRecordListener（§7.2/§7.3，ird-test-report.json 与 gtest XML 并存，--ird_report 证据落盘）。**LinkageContractTest 白名单增量修订**（按其 v0 原文预告路径）：UnitEdgeOnlyCore 白名单 +sdurws_ird_testkit（仅测试目标消费面）；NoTestkitEdge 升级双侧钉住（产品目标链接块零 testkit＝T-1 红线＋两测试目标链接块显式登记消费＝acceptance 2 用例化在案，防漂移）。**P-PR-7/P-PR-4/P-PR-1 处置按契约 acceptance 4 落地**：双编译 IModelCompilePort 桩（§5.3.6 单侧冻结签名零改动）；归档线程模型进程内模拟＋真进程半边不预设 RunRegistry 对接细节；用例构造的 core 数据随 v0.1 基线消费、零 core 文件改动。测试＝contract_test 28/28（既有 18 零回归＋新增 10：F8×5＋TX-7×3＋TX-8×1＋TX-2×1；真进程组连跑 3 轮稳定）＋unit_test 254/255＋1 skipped（既有 253 零回归＋TX-13 过＋TX-11 显式跳过），双模式一致；ird_gates 命中 11＝T14 基线 9＋2×SUB（两测试目标→testkit，DTB §4.5 既有例外行『各 _test/_contract_test 经 testkit/policy 替身消费』同型实例；T-1 产品面零命中；实例行增补归 WP-01-T03，随验收请求转呈）；validate-task PASS。本文契约语义零变更（§11 表为用例体实现，无设计变更；TX-13 错误码集合按 §9.6/§5.1/§5.4/§5.6 既有门卫语义锚定）。证据：traceability/builds/wp04-t15/ |
| v0.18 | 2026-09-17 | PRJ-T16（≙WP-04-T16）文档与门禁同步登记（纯文档任务——双模式构建按 DTB §5.2 DoD 第 1 条免除；零代码变更，diff 无 project/ 源码与 CMake 文件；validate-docs 通过，留痕 traceability/builds/wp04-t16/）。**零偏差汇总核对（acceptance 1）**：§15.4 v0.3～v0.17 逐任务登记与 §12 表逐行对账可查；§3.1 组成表/§3.3 目标/§3.4 布局与磁盘实测对齐——阶段 A 公共头 8 个全部落地（StoreTypes/ProjectStore/QueryPort/CommandService/UndoRedo/DraftService/ArchivePort/PersistenceFormat），ResourceRefs/PackageService/Upgrade 三头为 §3.1 表内标注的阶段 B 落位项、StorePath 类型零消费者不预建（v0.10 登记）——均为设计内未落位项而非偏差；§3.1 README 行"已指向本文 §9"过期陈述更正为 §12（2026-09-10 已修正事实的同步）；§3.4 布局图后补磁盘现状注记（增量文件 Project.cpp/IFileOps/ILockOps/ConfirmationFlow/DiagRecords/QueryPortImpl）；公共头 README 复核——指向 units/project.md §12 零偏差，追加 PRJ-T16 核对登记段。**§11 实现状态登记（acceptance 2 前半）**：标题"设计；未实现未运行"过期陈述更正＋表后新增逐组状态映射——PRJ-TX-1～10/12～14 已执行通过（具名载体，留痕 traceability/builds/wp04-t15/）；PRJ-TX-11 阻塞登记＝另存为/包实体阶段 B 未就绪 GTEST_SKIP 不计通过（AGENTS §4.2）。**§15.3 状态逐条同步（acceptance 2 后半）**：P-PR-1/2 仍未决（core.md 实测 v0.11 未冻结／ARCHITECTURE 实测 v0.12 待评审）；P-PR-3/8 裁决中（O-08/O-19 维持）；P-PR-4 **已消账**（execution.md §9.5 冻结答复与本文 §5.6/§9.8/§10.1/D-13/§4.4.8 逐项一致——本文确认零增量需求，对端 P-EX-5 消账由其卡承接）；P-PR-7 **已消账**（ui.md §9.2/D-4 交叉核对通过＋runtime.md §10.2 承接答复"兼容"——快照句柄/取消令牌两处差异按同步命令路径语义确认无增量需求，对端 P-RT-8 消账由其卡承接）；P-PR-5/P-PR-6 **裁决申请已提交（acceptance 3）**：`traceability/wp04-t16-adjudication-requests.md`——P-PR-5 经 O-09 处置联动（project→io 边补登或注入形态确认，二选一由架构所有者裁决；WP-04-T17/T18 届时按 DOC-T10 契约 O-09 处置条款回填 kept planned＋blocked 原因或注入式先行显式登记）；P-PR-6 sink 形态二选一（diagnostics 详设所有者＋架构所有者；码值部分已消账不重开）。两申请均为流程动作，未私裁、未改对端契约（diff 无 ARCHITECTURE/io/diagnostics/execution 文件）。本文契约语义零变更（§3.1 一处事实性过期陈述更正＋§11 标题/§3.4/§15.3 状态标注，无设计条目语义修改）。证据：traceability/wp04-t16-adjudication-requests.md、traceability/builds/wp04-t16/ |
| v0.19 | 2026-09-22 | O-41 所有者裁决落地（纯文档增量修订——零代码变更）：§9.5 新增 PRJ-LOCK-HELD 用户级诊断记录字段规范（subject=项目对象 ProjectId 必填——ERR-01 用户级 subject 边界；paramSchema{pid,host} 与非比较型不变）＋§5 码清单与 factory.open 行同格标注＋diagnostics.md §8.2 映射表该码"subject=∅（项目级）"格同步对齐（units/diagnostics.md v0.13——该格与 diagnostics 工厂 subject-missing 校验面相悖，即 findings F-290 的跨单元契约缺口根因）。背景：WP-10-T15 补验收 B-1 根因链三探针实证——project 经 IDiagnosticsSink 直构 core::DiagnosticRecord 未过工厂校验面，首个经工厂消费的下游（UI-T15 ProjectDiagnosticsBridge）触发 subject-missing 契约异常；裁决方向＝发射侧补齐（方案①），驳回"diagnostics 豁免"方案②（豁免将掏空 ERR-01 完整性强制）。余留：project 发射侧代码补 subject 的实现修正任务（登记于 DTB O-41／F-290，编译入队随任务合入消账） |
