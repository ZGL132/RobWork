...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.16（RPT-T13 文档与门禁同步——README 指向核对零偏差＋§14.3 P-RPT-1~9 状态逐条同步〔P-RPT-1/2/3 裁决申请三件提交 traceability/wp12-t13-adjudication-requests.md，P-RPT-6/8 消账维持〕＋§12.1/§12.2 交接状态表刷新＋RPT-T01~T12 零偏差登记，详见 §14.4 v0.16）；v0.15（RPT-T12 实现落位登记——§9.7 ModelSummary schema 逐字段冻结＋IModelSummaryProvider 签名冻结〔P-RPT-2〕＋§3.3 ITaskStatusSource 最小接口与 TaskStatusProjection 投影族；两契约头 header-only 落地，§3.1 组成表增列 TaskStatusSource.hpp 行〔公共头 12→13〕；落位偏差四项登记 §14.4 v0.15；schema 冻结留痕〔runtime §13.2 交接凭据〕见 traceability/builds/wp12-t12/schema-freeze-note.md）；v0.14（RPT-T11 实现落地登记——四具名替身正本＋RPT-T03~T10 局部夹具收敛＋RP-* 契约套件 §10.1 矩阵 32 组统一用例体＋testkit 报告设施接入〔T-1 允许形态，双自有 main〕＋替身边界声明正本 test/README，详见 §14.4 v0.14）；v0.13（RPT-T10 实现落位登记——§7.6/§9.6 证据包组装 Bundle.hpp/.cpp 落地＋IArchiveWriter 注入契约与 FakeArchiveWriter 随任务交付（条目编址/totalDigest 规范域/reproduction.json 形状/放弃路径 RAII 四项实现口径登记 §14.4 v0.13）；v0.12＝RPT-T09 实现落位登记——§7.3/§7.4/§9.5/§9.6 导出与归档协调 Export.hpp/.cpp＋Archive.hpp/.cpp 落地＋IReportArtifactSink 契约与 Fake 随任务交付（落位偏差六项登记 §14.4 v0.12）；v0.11＝RPT-T08 实现落位登记〔文档头版本行同步遗漏 v0.11 停留 v0.10，本次补登〕；v0.10＝RPT-T07 实现落位登记——§8.5/§9.4 一致性检查器 Consistency.hpp/.cpp 落地＋CSV 回读 io reader 注入缝（P-RPT-1）；v0.9＝RPT-T06 实现落位登记——§8.1~§8.4/§9.3 渲染器 Render.hpp/.cpp 落地＋§9.5 io 注入契约族暂载（偏差登记）；v0.8＝RPT-T05 实现落位登记、v0.7＝RPT-T04 实现落位登记、v0.6＝RPT-T03 实现落位登记、v0.5＝RPT-T02 实现落位登记、v0.4＝RPT-T01 构建落位登记、v0.3＝RPT 编译批次评审驱动的两处任务号漂移校正、v0.2＝全链一致性审计消账、v0.1＝首版草案） |
| 日期 | 2026-09-20 |
...
| 构建落位 | `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/reporting/`（**已随 RPT-T01 落位〔2026-09-19〕**：目标 `sdurws_ird_reporting` 升级 STATIC〔C++17、PUBLIC 链 core＋evidence＋diagnostics＋project——ARCH §3.5 四条登记编译链接边，§3.4 原文全链；P-RPT-1 处置零 io 编译边〔IReportIoFactory 注入〕、零 runtime/execution 编译边；零 Qt 含 Core——D-01；src/ 空起步锚点翻译单元 `Reporting.cpp`——STATIC 库最小翻译单元先例形态，CORE-T01/POL-T01/DIAG-T02/PRJ-T01/EX-T01 同款，无接口预建〕，别名 `RWS::ird::reporting`；测试目标 `sdurws_ird_reporting_test`/`sdurws_ird_reporting_contract_test` 已按 DTB §5.5 注册〔gtest vcpkg 接入；落位期最小构建冒烟用例，替身与 RP-\* 用例体随 RPT-T11〕；配置期红线守卫就位。公共头保留位 `include/sdurws/ird/reporting/README.md` 指向本文 **§11**〔任务拆分〕——卡 v0.1~v0.3 此处与 §3.1 所载"指向 §9"与磁盘不符，RPT-T01 复核更正（PRJ-T01 家族同型偏差），核对结论见 traceability/builds/wp12-t01/；"不参与编译/不表示已实现"声明保留；§3.1 契约头 `Errors.hpp`/`Identity.hpp` 已随 RPT-T02 落地〔2026-09-19——§3.5 错误契约＋§4.1/§7.3 身份类型＋RPT-\* 码清单按收编表引用登记；`core::ContentIdentity`/`ContentDigester` 与 diagnostics `StableCodeRegistry` 只读为首轮上游头消费（P-RPT-9 基线＝core.md v0.1 Draft）；证据 traceability/builds/wp12-t02/〕；§3.1 契约头 `ReportModel.hpp` 已随 RPT-T03 落地〔2026-09-19——§4 全节数据模型（§4.2 ReviewReport 不可变值对象 make() 唯一生产者＋§4.3 引用类型全族＋§4.5 ReviewMetadata 族）＋§4.2 非法实例字段校验原语 firstFieldViolation＋§4.4 ReportCodec Data/Full 双编码（src/ReportCodec.hpp/.cpp 私有实现头——R-2 不出 include/）；上游头消费扩面＝core 身份/词表族＋evidence 包络类型族〔EV-T06/EV-T07 done 交付面〕＋diagnostics 词表（P-RPT-9 基线不变＝各卡 v0.1 Draft）；SectionStatus/RenderHint/条目冻结视图族暂载 ReportModel.hpp 的偏差登记见 §14.4 v0.6；证据 traceability/builds/wp12-t03/〕；§3.1 契约头 `Sections.hpp`/`SectionProvider.hpp` 已随 RPT-T04 落地〔2026-09-19——§5.1 词表 14 项冻结（5 组 constexpr 数组＋级别归属/框架域归属；词表权威单点在本头——ReportLevelRule 注入值 frozenReportLevelRule，ReportModel 侧零复制）＋LevelConflict/ScopeInsufficient 判定原语＋§5 默认选择规则＋RPT-SCOPE-INSUFFICIENT 诊断挂接（P-RPT-5 保守处置）＋未注册域章节缺项表达；§9.2 SectionRequest/SectionContent（entries 消费 ReportModel 冻结视图 SectionEntryView——v0.6 偏差登记的兑现，零重复定义）＋IReportSectionProvider 纯投影接口＋SectionRegistry（注册边界五类拒绝/运行期 find 并发只读/字典序稳定；header-only——§11 产物列形态）＋firstSectionContentViolation 结构自检原语；上游头消费扩面＝evidence ResultEnvelope（SectionRequest.results——P-RPT-9 基线不变＝各卡 v0.1 Draft）；证据 traceability/builds/wp12-t04/〕；§7.1/§7.2/§9.1 构建器契约头 `Builder.hpp` 已随 RPT-T05 落地〔2026-09-20——§7.1 会话状态机（七态主链＋Failed/Canceled 终态不可变＋RAII 清理守卫）＋§9.1 IReviewReportBuilder/ReportBuildRequest/ReportBuildOutcome＋§9 章首 ReportCancelToken/ReportProgress（零 io 类型——P-RPT-1）＋IReportResultSource 注入源（tryEnvelope/currentnessOf 原文两方法＋eligibilityOf/tryReproduction 落位增量——登记于 §14.4 v0.8）；上游头消费扩面＝evidence Currentness/Verdict（EV-T08 done 交付面）＋project QueryPort（PRJ-T09 done 交付面——仅 head/tryRevision/listRuns 三只读方法，零写路径结构保证）；证据 traceability/builds/wp12-t05/〕；§8.1~§8.4/§9.3 渲染器契约头 `Render.hpp` 已随 RPT-T06 落地〔2026-09-20——§8.4 FieldCell/FieldMatrix＋extractFieldMatrix 单次投影（值文本规范形＝core tryConvert 唯一换算＋to_chars 最短往返；四态呈现词/Invalid 保留原文 NFR-COR-03；曲线类 ResultBinding 引用文本；§6.4 限定语固定推导序去重——呈现词表收口归 RPT-T08）＋§9.3 ReportRenderFormat/RenderOptions/RenderArtifact/RenderOutcome/IReportRenderer＋三实现 HtmlReportRenderer/JsonReportRenderer/CsvReportRenderer（§8.2① 资格声明前置断言门＋§4.3.3 脱敏双保险经注入 diagnostics::IRedactionService——C-3 登记边；P-RPT-6 零 PDF 词位/字段）＋§9.5 io 注入契约族 IReportIoFactory/IReportCsvWriter/IReportJsonWriter/IReportOutputTarget/ReplacePolicy/ReportCsvCell/ReportJsonDom **暂载本头**——§9.5 原文列于 Export.hpp（RPT-T09），渲染器先于导出服务消费该族（§11 RPT-T06 行"注入 io 工厂"），RPT-T09 消费本头类型不重复定义，v0.6 同型偏差先例，登记 §14.4 v0.9；P-RPT-1 公共头零 io 类型由契约测试源码扫描常驻自证；上游头消费扩面＝diagnostics Redaction（IRedactionService——C-3）；证据 traceability/builds/wp12-t06/〕；§8.5/§9.4 一致性检查器契约头 `Consistency.hpp` 已随 RPT-T07 落地〔2026-09-20——§9.4 ConsistencyInput/FieldMismatch/ConsistencyResult〔**增补 notes 注记承载位**——§9.4 原文形状无注记位而"artifacts<2 平凡通过＋注记"须有承载，偏差登记 §14.4 v0.10〕＋mismatch 维度词表（value/unit/status/qualifier/ref/missing＋parse-failed 错误轨词）＋CSV 回读注入缝 IReportCsvReader/IReportCsvReaderFactory（io ICsvReader 的 reporting 侧投影——公共头零 io 类型 P-RPT-1，裁决补边后直连签名零改动）＋IReportConsistencyChecker 接口与 FieldConsistencyChecker 实现（§8.5 逐字段回读比对——HTML data-field 严格文法扫描/JSON 严格解析〔ird-report-json/1 回读面，顶层键消费 kJsonTopLevelKeys 冻结集 NFR-DEP-04 读取侧〕/CSV 经注入 reader 按列名映射零字节解析〔NFR-SEC-03 转义唯一实现归 io〕；纯函数不短路；维度覆盖与失败语义边界登记 §14.4 v0.10）；私有共享头 src/RenderText.hpp（工程判定 token↔中文显示名单表——RPT-T06 正向映射抽取等价重构，R-2 不出 include/）；上游头消费扩面＝零（reporting 自有类型闭环）；证据 traceability/builds/wp12-t07/〕；§7.3/§7.4/§9.5/§9.6 导出与归档协调契约头 `Export.hpp`/`Archive.hpp` 已随 RPT-T09 落地〔2026-09-20——`Archive.hpp`：§9.6 IReportArtifactSink 四方法逐字（begin/writeArtifact/finalize/abandon——acceptance 5"逐字一致"面）＋ReportArtifactBeginRequest/ArtifactWrite/ArtifactStatus（六值）/ReportEndReason/ArtifactSessionRef（值句柄）＋§7.3 ReportArtifactManifest 全字段与 computeManifestDigest（幂等判定键 D-02：contentIdentity＋工件集 (relPath,format,sha256,sizeBytes,rendererVersion,templateVersion) canonical SHA-256，core ContentDigester 唯一算法；排除 ReportId/身份链/生成发布时刻——不以目标路径）＋IReportArchiveCoordinator/ReportArchiveCoordinator（幂等预检零汇集座调用→begin→write×N→finalize，任一非 Ok→abandon；listPublished 仅 Finalized）＋IReportPublishedIndex 只读清单注入缝；`Export.hpp`：§9.5 ExportDestination/ReportExportRequest/ReportExportResult/ExportedFile/IReportExportService＋ReportExportService（§8.5 编排：解析→矩阵单次→逐格式渲染＋二次渲染字节比对→一致性→发布/写出；§7.4 四轨——项目内幂等/冲突差异定位〔身份/版本维度〕、外部同字节幂等无操作〔mtime/哈希不变〕、NeverOverwrite 拒绝/OverwriteAtomic 原子替换〔提交失败先前输出完整保留〕、DiskFull abandon＋比较型诊断可重试、ReadOnlyStore 两路径诊断〔PM-07〕、逐格式/逐工件取消检查点〔UX-03 全空形态〕）＋IReportResolver 解析注入缝＋工件相对命名（report.html/report-data.json/report.csv——JSON 让位 manifest 的 report.json）；测试面＝test/ArchiveExportTest.cpp 30 条＋contract_test/ArchiveCrossUnitContractTest.cpp 13 条（sink 契约 fake 投影——§3.4"与 project 工件汇集座协作随 RPT-T09 落地"兑现）＋FakeArtifactSink 随任务交付（§10 三项故障可注入——冲突/磁盘满/清理失败；具名替身收口归 RPT-T11）；落位偏差六项登记 §14.4 v0.12；上游头消费扩面＝core Digest/Identity/DiagData/Provenance/Units（P-RPT-9 基线不变＝core.md v0.1 Draft）；证据 traceability/builds/wp12-t09/〕，其余契约头随 RPT-T10+。见单元 `reporting/CMakeLists.txt` 与上级 `industrialrobot/CMakeLists.txt`） |
...
reporting 由 13 个公共头模块＋1 个编译实现组成（实现文件随 §11 任务落地，头为契约权威；13＝12＋1——RPT-T12 落位增列 `TaskStatusSource.hpp`，§14.4 v0.15 登记）：
...
| `TaskStatusSource.hpp` | ITaskStatusSource、TaskStatusProjection/TaskProgressProjection/TaskTerminationKind（execution 任务状态只读投影——§3.3 注入形态；RPT-T12 落位增列，落位偏差登记 §14.4 v0.15） | §3.3、§9.9 |
| `README.md` | 既有保留位说明（不参与编译；指向本文 §11〔任务拆分〕——v0.4 RPT-T01 复核更正：原"已指向本文 §9，实测一致"的 v0.1 记载与磁盘不符，PRJ-T01 家族同型偏差，核对结论见 traceability/builds/wp12-t01/；RPT-T13 复核〔2026-09-20〕指向核对零偏差——本行现文、README 现文与 §11 现文三方一致，README 实现状态段同步至 13 契约头落位现状，"不参与编译/不表示已实现"声明保留，核对结论见 traceability/builds/wp12-t13/README-audit.md） | — |
...
| 来源 | 义务 | 本文落点 | 状态 |
| --- | --- | --- | --- |
| project.md §13.2 reporting 行 | 冻结**报告工件引用结构与幂等追加规则** | §7.3（ReportArtifactManifest/sources.json）＋§7.4＋§9.6（IReportArtifactSink） | 已冻结（project 侧实现随其阶段 B 任务对齐，P-RPT-4 关联）；本文侧 sink 契约已落地（Archive.hpp——RPT-T09，含 IReportPublishedIndex 只读清单缝） |
| runtime.md §13.2 | 冻结 **RPT-01 输入章节摘要 schema** | §9.7 ModelSummary/IModelSummaryProvider | 已冻结并落地（ModelSummary.hpp——RPT-T12；traceability/builds/wp12-t12/schema-freeze-note.md 交接凭据；L5 适配；P-RPT-2 裁决申请已提交——wp12-t13-adjudication-requests.md 申请二） |
| ui.md P-UI-9 | 冻结**预览接口形状**（裁决者=reporting 详设所有者） | §9.8（ReviewReport 只读值＋renderPreview＋JumpTarget） | 已裁决（ui 侧消费即可消账） |
| evidence.md §13 B·reporting 行 | 消费 FormalPass/ReviewRecord、追溯链、复现要素 | §6.2/§4.3.1/§7.6 | 已消费（RPT-T05 构建器经 IReportResultSource 注入隔离交付——v0.8；envelope 归档工件的解码入口仍待 evidence/execution 提供——下方交接，R-3/P-RPT-9） |
| io.md §10.10/§13.2 | 消费 CsvWriter/JsonWriter/AtomicWriter | §3.3/§8.3/§9.5（注入形态，P-RPT-1） | 契约已定并落地（IReportIoFactory 族暂载 Render.hpp/Export.hpp——RPT-T06~T10 交付；编译边裁决申请已提交——wp12-t13-adjudication-requests.md 申请一，与 P-IO-1/P-PR-5 合并裁决建议） |
...
| --- | --- | --- |
| project | IReportArtifactSink 契约（D-13/D-14 报告语义：只增/manifest 原子发布/幂等/冲突）；ReportArtifactManifest＋sources.json 结构——已随 RPT-T09 落地（Archive.hpp） | reports/ 写入端口实现（§4.1 reports 行既定行为）；随包导出勾选的引用完整性扫描消费 sources.json |
| evidence | 消费口径：报告读取 envelope 的**归档解码入口**（results/<run-id>/ envelope 工件→ResultEnvelope 只读值——建议其提供 decode 入口或经 execution 通道 DTO）；CurrentnessResult 的报告侧取值形态——reporting 侧已按 IReportResultSource 注入隔离交付（RPT-T05——v0.8），解码入口仍待提供 | EnvelopeCodec/解码入口；CurrentnessIndex 值快照查询形态（P-RPT-9 关联） |
| io | IArchiveWriter 契约（证据包 ZIP 适配——其 ZipChannel 之上的薄适配）；reporting 的 JsonProfile 注册（`ird-report-json/1`）——IArchiveWriter 已随 RPT-T10 落地（Bundle.hpp）；`ird-report-json/1` schema 已冻结（Render.hpp kJsonTopLevelKeys 17 键——RPT-T06） | ZIP 通道适配或通用 ZIP 写出接口（P-RPT-1/7 合并考虑）；profile 注册接纳 |
| runtime | ModelSummary schema（§9.7——冻结凭据）——已随 RPT-T12 落地冻结（ModelSummary.hpp＋schema-freeze-note.md） | L5 适配器（IRuntimeModelView→IModelSummaryProvider）；快照释放时的归档身份元数据供给 |
| ui | 预览契约（§9.8）；`report.export` 目标服务签名（§9.5）——服务签名已随 RPT-T09 落地（Export.hpp IReportExportService） | 预览宿主实现；签署表单（ReviewMetadataSeed 采集）；命令注册联调 |
...
| # | 问题 | 依据 | 影响 | 建议 | 需要裁决者 |
| --- | --- | --- | --- | --- | --- |
| P-RPT-1 | ARCH §3.5 未登记 reporting→io 边，而 RPT-02/AT-22 需要 reporting 消费 io canonical 写出设施 | ARCH §3.5（表外边＝构建失败）；io.md P-IO-1/IO-D02（注入先行、双形态兼容）；io.md §10.10 已把 reporting 列为消费方 | 编译形态（注入 vs 直连） | **本文已免依赖**（IReportIoFactory 注入先行，接口契约与裁决无关）；建议与 P-IO-1/P-PR-5 合并裁决：补登 reporting→io（接口依赖，无环——io 不依赖 reporting）。**〔RPT-T13 状态同步 2026-09-20〕裁决申请已提交**（traceability/wp12-t13-adjudication-requests.md 申请一——三案同批裁决建议随附回填条款）；实现侧注入形态已全量落地且零阻塞（IReportIoFactory 族——v0.9，IReportCsvReaderFactory——v0.10；公共头零 io 类型契约测试扫描常驻自证）；裁决前各任务注入形态维持不变，申请≠裁决（不私改 ARCHITECTURE.md——RPT-T13 契约 forbiddenFiles 红线） | 架构所有者（P-IO-1 同案） |
| P-RPT-2 | ARCH §3.5 未登记 reporting→runtime 边；runtime §10.12 已按"reporting 调用只读摘要"设计且其 §13.2 要求本文冻结摘要 schema | ARCH §3.5；runtime.md §10.12/§13.2 | 编译形态 | 维持注入式（IModelSummaryProvider，§9.7 已冻结 schema）；建议架构确认注入为正式形态或补边。**〔RPT-T13 状态同步 2026-09-20〕裁决申请已提交**（traceability/wp12-t13-adjudication-requests.md 申请二）；schema 已冻结落地（ModelSummary.hpp——RPT-T12；trySummary 签名 static_assert 钉住；traceability/builds/wp12-t12/schema-freeze-note.md＝runtime §13.2 交接凭据）；裁决前签名维持冻结不变 | 架构所有者＋runtime 详设所有者 |
| P-RPT-3 | ReportId（`rpt-` tag）为 reporting 层类型，core 未定义 | core.md §4.1 六类型无 ReportId；diagnostics P-DIAG-4（FindingId `fnd-` 同案） | 跨单元身份类型分散 | reporting 自建并遵循 core Id128 约定；建议 core 收编时一并处理 `fnd-`/`rpt-`。**〔RPT-T13 状态同步 2026-09-20〕裁决申请已提交**（traceability/wp12-t13-adjudication-requests.md 申请三——与 P-DIAG-4 同案收编）；core.md 实测 v0.11（Draft-Structured）仍未收编（Id128 冻结 7 tag：obj/prj/brn/rev/run/evt/att——无 `fnd-`/`rpt-`）；reporting 自建 `rpt-` 已落地且句法与 core Id128 严格一致（RPT-T02——v0.5，契约用例钉住） | core 详设所有者 |
| P-RPT-4 | project 侧 reports/ 写入端口（IReportArtifactSink）为本文单侧冻结，project.md 未定义对应公共接口 | project.md §4.1 reports 行（行为契约）＋§13.2（交接预留） | project 实现时对齐 | 以本文 §9.6 契约为起点；project 详设修订时交叉核对（D-13/D-14 语义一致性）。**〔RPT-T13 状态同步 2026-09-20〕维持 open（对端公共接口未定义）**：本文侧 IReportArtifactSink 契约已单侧冻结并落地（Archive.hpp——RPT-T09：§9.6 四方法＋computeManifestDigest＋IReportPublishedIndex 只读清单缝，偏差登记 v0.12）；project.md 实测 v0.18、其 §13.2 交接行在册（reports/ 写入端口随其阶段 B 任务对齐）——交叉核对以本文 §9.6 为起点不变 | project 详设所有者 |
| P-RPT-5 | C 级全部追加章节缺正式结果时的处置（拒绝＋显式降级建议）为本文保守设计，上游未明文 | RPT-01-C/§16 验收要点未覆盖该边界 | 边界场景验收解释 | 维持保守（宁可拒绝不可虚级）；如需"全缺仍生成 C 骨架"语义，走需求变更。**〔RPT-T13 状态同步 2026-09-20〕仍 open（需求所有者未裁决）**：保守处置已逐字落位——RPT-SCOPE-INSUFFICIENT 诊断 recommendedAction 固定"显式确认＋B 级重建＋非静默降级"契约词（RPT-T04——v0.7）＋构建器 ScopeInsufficient 拒绝路径（RPT-T05——v0.8）；裁决前行为可预测且保守 | 需求所有者 |
| P-RPT-6 | **O-25 裁决：渲染层是否预留 PDF 接口** | REQUIREMENTS 附录 B PDF 行（未立项、不设需求条目、**不留接口桩**——待同步消费者＝WP-12 卡）；dtb §4 O-25 | 渲染器架构 | **裁决：不留 PDF 接口桩**——IReportRenderer 是格式无关抽象（新格式＝新实现，天然可扩展，非 PDF 专用预留）；不建 PDF 专属字段/配置/空页面（附录 B 裁决＋TRJ-08"不建占位"同精神）；未来 PDF 走需求变更。**〔RPT-T13 状态同步 2026-09-20〕已裁决维持**：实现侧零 PDF 词位/字段自证（RPT-T06 渲染器交付——v0.9）；DTB §4 O-25 行实测"已关闭（设计级）" | 本文裁决（O-25 消账；需求侧知悉） |
| P-RPT-7 | 证据包 ZIP 封装的实现通道：io 未暴露通用 ZIP 写出公共接口（ZipChannel 为其 src 内部） | io.md §3.1/§7；RPT-03 | RPT-T10 实现 | IArchiveWriter 注入（io 薄适配）；建议 io 在 P-IO-1 裁决时一并考虑暴露通用 ZIP 写出接口或提供适配器。**〔RPT-T13 状态同步 2026-09-20〕仍 open（io 详设所有者）**：本文侧 IArchiveWriter 注入契约已冻结落地（Bundle.hpp——RPT-T10，FakeArchiveWriter 承载测试；零第二套 ZIP 实现源码扫描常驻自证——v0.13）；io.md 实测 v0.9（Package.hpp 为 .rwpack 专属通道），通用 ZIP 写出公共接口仍未暴露——"随 P-IO-1 一并考虑"建议维持 | io 详设所有者 |
| P-RPT-8 | RPT-\* 稳定诊断码建议清单收编 | diagnostics.md §4.5（码值权威）；§3.5 建议 | 码表冲突 | **已收编（2026-09-10）**：diagnostics.md v0.2 §4.5 补登 RPT 前缀、§4.6 收编全量 8 项——码值部分消账；RPT-T02 注册时按收编表登记即可（『码清单登记』完成条件归 RPT-T02 行，§11——v0.3 校正）。**〔RPT-T13 状态同步 2026-09-20〕已消账维持**：RPT-T02 注册已按收编表落地（Errors.hpp diagcodes 8 常量＋全集数组——v0.5），与 diagnostics StableCodeRegistry 一致性由契约测试逐码断言常驻自证 | diagnostics 详设所有者（已处置码值部分） |
| P-RPT-9 | 本文消费的 core/evidence 契约以其 v0.1（Draft）为基线，冻结版可能调整（含 envelope 解码入口缺位——R-3） | 各卡文档头状态行 | RPT-T02 起返工 | 冻结 diff 清单后按影响面增量修订；envelope 解码入口列入 evidence/execution 交接催办。**〔RPT-T13 状态同步 2026-09-20〕仍 open（冻结未发生）**：core.md 实测 v0.11（Draft-Structured）、evidence.md 实测 v1.3（Draft-Structured）、ARCHITECTURE 实测 v0.12（Draft 待评审）——v0.1 签名基线维持；本文消费面由契约测试 static_assert/类型恒等断言常驻钉住（v0.5~v0.15 各登记）；envelope 解码入口缺位（R-3）仍待 evidence/execution 提供（IReportResultSource 注入隔离已落地——v0.8，本文不自建解码器） | core/evidence 详设所有者＋本文所有者 |
...
| v0.5 | 2026-09-19 | RPT-T02（错误与身份类型）实现落位登记：§3.1 组成表 `Errors.hpp`/`Identity.hpp` 两契约头落地（实现＝src/Errors.cpp＋src/Identity.cpp）——§3.5 原文 16 值 ReportErrorCode token 全表＋ReportError（what() 前缀 "reporting/<域>:" 强制拼装）＋§4.1 ReportId（`rpt-<32hex>` Id128 形自建实现——P-RPT-3 处置：不调用 core 头内 detail 非公共契约层、零 core 修改）＋§4.2 ReportLevel（level-b/level-c）＋§7.3 PublishedReportRecord 六字段（contentIdentity/manifestDigest 以 core::ContentIdentity 承载——SHA-256 经 core ContentDigester 唯一算法、字节等值比较）；RPT-\* 稳定诊断码清单按收编表引用登记（diagcodes 8 常量＋全集数组——P-RPT-8，与 diagnostics 内置码表一致性由契约测试对 StableCodeRegistry 逐码 find/ownerUnit/severity 断言常驻自证）；首个上游头消费＝core/Digest.hpp＋diagnostics/DiagCodes.hpp（C-1/C-3 消费行，P-RPT-9 基线 core.md v0.1 Draft——冻结出 diff 后按影响面增量同步）；try\* 非抛出双轨约定（§1.4 异常行）在头文件契约注释声明并具名落位（ReportId::tryFromCanonical/tryLevelFromToken）。本文契约语义零变更（实现与 §3.5/§4.1/§7.3 原文逐字对齐）。证据：traceability/builds/wp12-t02/ |
| v0.6 | 2026-09-19 | RPT-T03（报告模型与编码，≙WP-12-T03 模型部分）实现落位登记：§3.1 组成表 `ReportModel.hpp` 契约头落地（实现＝src/ReportModel.cpp＋src/ReportCodec.cpp，私有头 src/ReportCodec.hpp 不出 include/——R-2）——§4.2 ReviewReport 不可变值对象（成员私有＋只读访问器＋无 setter＋私有构造仅 make() 可达；"冻结是类型系统事实"的类型级兑现）＋make() 唯一生产者（字段校验→§4.4 ReportCodec 身份计算→冻结构造——dataIdentity/contentIdentity 调用方不可申报，§4.2 原文）＋§4.2 非法实例字段校验原语 firstFieldViolation（检查序＝字段表行序；22 反例族：身份保留值/resultRefs 空/Preview SourceMissing/B 级携 C 章节 LevelConflict/C 级零 C 章节 ScopeInsufficient/NaN DataInvalid/引用越界 EvidenceRefInvalid/当前性与签署 presence 纪律等——端到端构建拒绝路径随 RPT-T05 构建器收口，本任务字段校验形态交付）＋§4.3 引用类型全族（ResultRefSnapshot/EvidenceRefEntry/DiagRefEntry/ReviewReportSection＋§4.5 ReviewMetadata 族）＋§4.4 ReportCodec Data/Full 双编码（magic IRDRPTD1/IRDRPT1＋编码器版本号入编码＋字段按名序/章节按 order＋长度前缀＋大端＋presence 字节＋UTF-8 禁 NUL＋浮点 IEEE754 位模式、NaN/±Inf 编码入口拒绝〔evidence D-06 同源〕＋纯函数同输入同字节——NFR-COR-02；往返 parse(encode(x))==x 单测锁定）；上游头消费扩面＝core 身份/词表族（Identity/Evaluation/Provenance/Units/DiagData）＋evidence 包络类型族（Evaluator/Envelope/Snapshot/Evidence/Currentness/Verdict——EV-T06/EV-T07 done 交付面）＋diagnostics DiagCodes 词表（P-RPT-9 基线不变＝各卡 v0.1 Draft；O-13/O-24 消费侧零重定义由契约测试 static_assert 常驻自证）。**落位偏差登记（DTB §5.4，零契约语义变更）**：§3.1 组成表原列 SectionStatus 归 Sections.hpp（RPT-T04）、SectionEntry/ResultBinding/EvidenceBinding 等归 SectionProvider.hpp（RPT-T04），但 §4.3.4 ReviewReportSection 以值字段引用这些类型且 RPT-T03 先于 RPT-T04 落位（§11 依赖列）——SectionStatus/RenderHint 两枚举与条目冻结视图族（SectionEntryView/FieldValue/ResultBinding/EvidenceBinding/JumpTarget/MissingItemView/EligibilityNote）暂载 ReportModel.hpp，RPT-T04 的 Sections.hpp/SectionProvider.hpp 消费本头类型不重复定义；级别×章节组合判定的 C 专属词表以 ReportLevelRule 注入（词表权威归 RPT-T04，不在 ReportModel 复制 §5.1 清单）。证据：traceability/builds/wp12-t03/ |
| v0.7 | 2026-09-19 | RPT-T04（章节契约与注册表，卡内行——WP-12-T03 章节模型与 WP-12-T04 渲染链共享前置）实现落位登记：§3.1 组成表 `Sections.hpp`/`SectionProvider.hpp` 两契约头落地（实现＝src/Sections.cpp；SectionProvider.hpp header-only——§11 产物列"SectionProvider.hpp（接口＋注册表）"无 .cpp）——`Sections.hpp`：§5.1 词表 14 项以 5 组 inline constexpr 数组冻结（kAllSectionIds 词表序＝表行号，即 ReviewReportSection::order 冻结依据；kFrameworkSectionIds 6 项/kDomainSectionIds 8 项/kBLevelSectionIds 8 项/kCExclusiveSectionIds 6 项——级别归属与所有者归属逐项锁定）＋词表查找/级别归属原语（isValidSectionId/trySectionOrder/trySectionMinimumLevel/isFrameworkSectionId/isDomainSectionId）＋frozenReportLevelRule（ReportLevelRule 注入值——v0.6 登记的"词表权威归 RPT-T04"兑现：ReportModel 侧零复制，RPT-T05 构建器以本值注入 ReviewReport::make）＋sectionLevelConflict 判定原语（B∧C 专属章节→LevelConflict——§4.6 行 2；端到端拒绝随 RPT-T05）＋domainSectionsInScope（B 四项/C 八项——§5.2 范围图）＋unregisteredSectionMissingItem（"章节不可用（提供方未注册）"缺项文案冻结——§5.1 词表规则/TRJ-08 同精神）＋SectionSelection（§3.1 组成表载荷——§9.1 sectionOverrides 条目类型）＋defaultSelectionForStatus（§5 默认选择规则——NoFormalResult 默认不选，§16 验收要点契约承载）＋checkSectionScope（ScopeInsufficient 判定原语——C 级全部 C 章节缺正式结果〔含空集情形与 §5.2 零 C 章节选中同判〕→scopeInsufficient＋缺项明细；B 级恒不判）＋makeScopeInsufficientDiagnostic（RPT-SCOPE-INSUFFICIENT 诊断挂接——recommendedAction 固定"显式确认＋B 级重建＋非静默降级"契约词，**P-RPT-5 保守处置逐字落位不放宽**）。`SectionProvider.hpp`：§9.2 SectionRequest/SectionContent 逐字段落地（SectionRequest.results＝evidence::ResultEnvelope 值向量——P-RPT-9 基线＝各卡 v0.1 Draft，类型恒等由契约测试 static_assert 常驻自证；SectionContent.entries 元素类型＝ReportModel 冻结视图 SectionEntryView——v0.6 偏差登记的消费面兑现，零重复定义）＋IReportSectionProvider 纯投影接口（前置/后置/错误注释行逐条：同请求同输出、不写项目不派发任务、请求外数据读取＝评审约束）＋SectionRegistry（注册边界五类拒绝全部 ReportError(Usage) fail-fast：空指针/词表外/框架章节不可覆盖〔§5.1"不注册、不可覆盖"〕/minimumLevel 与词表级别不一致双向〔B 级注册 C 专属章节＝注册拒绝——§9.2 前置行〕/重复 sectionId；运行期 find 并发只读＋registeredSections 字典序稳定——§9.2 维度表逐行）＋firstSectionContentViolation/isValidEntryKey（§5.3 presence 纪律＋entryKey 词形 [a-z0-9.-]{2,63} 的上下文无关结构自检原语——RPT-T05 构建器复用；绑定越界等需请求上下文面随 RPT-T05 收口）。本任务以局部夹具（ScriptedSectionProvider）交付词表/级别/注册边界用例（§11 验证方式"RP-SCOPE-1~4〔替身〕"的判定原语面；RP-SCOPE-1~4 具名替身用例体随 RPT-T11 契约套件收口——卡行 T11 产物列原文）。CMakeLists 仅增列源文件，目标名/别名/四条链接边/编译选项零改动。本文契约语义零变更（实现与 §5/§9.2 原文逐字对齐）。证据：traceability/builds/wp12-t04/ |
| v0.8 | 2026-09-20 | RPT-T05（构建器，≙WP-12-T03 构造部分）实现落位登记：§3.1 组成表契约头家族新增 `Builder.hpp`（实现＝src/Builder.cpp）——§7.1 生成主流程与会话状态机（ReportBuildStage 九值＝主链七态 Requested→Resolving→Building→Rendering→Verifying→Publishing→Completed＋Failed/Canceled 终态；ReportBuildSession 严格逐步推进/终态不可变/任一非终态可转 Failed/取消→Canceled〔UX-03 正常取消非错误〕/RAII 析构守卫——非终态未交接析构转 Canceled 收尾；状态词 token() 稳定 kebab 字面）＋§9.1 IReviewReportBuilder/ReportBuildRequest/ReportBuildOutcome 逐字段落地（outcome 三形态冻结：成功＝report 唯一非空、失败＝error 在场＋诊断随交付〔NFR-COR-03 不静默〕、取消＝三字段全空〔UX-03〕）＋§9 章首 ReportCancelToken/ReportProgress/IReportProgressSink（公共头零 io 类型纪律——P-RPT-1，L5 桥接同形类型）＋IReportResultSource 注入源（§9.1 原文 tryEnvelope/currentnessOf 两方法＋**落位增量两方法**：eligibilityOf/tryReproduction——§7.2 步③资格纯检查需要 VerdictInput/AnalysisSnapshot/注册表投影等重事实面，reporting 无此注入面且禁止自算〔§6.2"不自算"〕，按 §3.3 注入模式把纯检查调用承载于注入源，L5/evidence 侧适配器持有事实面；复现块读取为 §4.2 reproduction 必填字段的归档数据源——两增量均为 §7.2 步③的注入承载，契约语义零变更；envelope 解码本身仍归 evidence/execution 适配〔R-3/§12.2 交接催办〕，本单元不自建解码器、不私建编译边）。构建编排＝§7.2 步①~⑦：①锚定 tryRevision 恰一次〔缺失→SourceMissing〕＋listRuns 恰一次〔仅 finalize 口径——未 finalize→SourceMissing〕＋head 恰一次〔当前性判定上下文锚——HEAD 前进/迟到结果〔AT-10〕不入已锚定视图，§7.1 锚定纪律〕；③逐结果 envelope〔解码失败→SourceMissing〕→Preview 拒绝〔D-16/§4.6 行 5——SourceMissing〕→任务三元组/快照一致性校验〔SourceAmbiguous——§7.2 步⑤〕→资格纯检查冻结进 ResultRefSnapshot.eligibility〔每报告重查、不缓存跨报告复用——§6.2〕→当前性投影快照冻结〔status=nullopt 原样＋RPT-CURRENTNESS-UNEVALUABLE 诊断＋unevaluableNote 必填——**不默认 Current**，P-EV-4/§6.3〕；⑥章节组装＝域章节按词表序投影〔未注册＝缺项非空壳〕＋提供方结果子集按 requiredEvaluationKeys 过滤＋结构自检〔firstSectionContentViolation 消费——DataInvalid〕＋绑定校验四查〔结果绑定越界/非 Completed 结果入结论条目〔TASK-02/§4.6 行 6——DataInvalid〕/证据引用 itemId·状态·摘要·工况范围四面对包络清单〔§4.6 行 7——EvidenceRefInvalid〕〕＋章节级资格 AND 聚合与 Superseded 当前性必填〔§4.3.4〕；级别×章节范围判定两路同码面〔checkSectionScope 内容路＋零 C 章节选中选择路——ScopeInsufficient＋RPT-SCOPE-INSUFFICIENT 降级建议诊断，P-RPT-5 保守处置非静默降级〕；⑦冻结＝ReportCodec-Data→dataIdentity 先于字段组装〔评审演化种子一致性校验——不采信调用方申报〕→ReviewReport::make 唯一生产者〔frozenReportLevelRule 注入——级别×章节判定权威单点在 Sections〕；**冻结时刻＝build() 返回**〔§7.1 逐项明确表〕；零项目写＝结构保证〔本类不持有任何 project 写端口引用，查询端口仅 head/tryRevision/listRuns 三读方法——§7.1 逐项明确表"生成失败是否修改原项目＝否"〕。测试面：test/BuilderTest.cpp 二十三条（会话状态机五条/正例三条/反例矩阵七条/引用不可解析与零项目写三条/资格当前性快照三条/取消两条/请求面与评审演化两条——契约 acceptance 1~5 构建器层逐条对应）＋contract_test/BuilderCrossUnitContractTest.cpp 三条（注入面类型恒等 static_assert〔evidence::CurrentnessResult/ResultEnvelope/EligibilityCheck——EV-T08 交付面；IProjectQueryPort const 引用——PRJ-T09 交付面〕＋RPT-* 呈现码 StableCodeRegistry 一致性＋解码隔离自证——acceptance 6）；替身边界声明在案〔Preview 包络聚合初始化为受控越界样本——evidence make() 本就拒绝、被测对象是 reporting 第二道闸〕；RP-MDL-2/RP-SCOPE-1~4/RP-CUR-3/RP-STATE-1~3 具名用例体随 RPT-T11 收口〔卡行 T11 产物列原文〕。CMakeLists 仅增列源文件与两测试文件，目标名/别名/四条链接边/编译选项零改动。本文契约语义零变更（实现与 §7.1/§7.2/§9.1 原文对齐；两注入增量按本行登记——DTB §5.4 口径）。证据：traceability/builds/wp12-t05/ |
| v0.9 | 2026-09-20 | RPT-T06（渲染器三格式，≙WP-12-T04）实现落位登记：§3.1 组成表契约头家族新增 `Render.hpp`（实现＝src/Render.cpp）——§8.4 字段矩阵（FieldCell〔fieldKey 层级稳定键 "<sectionId>.<entryKey>.<field.key>"／valueRepr 文本规范形／unit/status/qualifier/resultRef/evidenceRef/caseScope/currentness；**entryKey 为呈现承载增列**——CSV item_label 列数据源，条目无独立显示名，§8.4 形状未列出，本行登记 DTB §5.4〕＋FieldMatrix＝vector<FieldCell>＋extractFieldMatrix 单次投影：遍历序＝章节 order＋条目 entryKey 稳定序＋字段存储序，值文本规范形＝显示单位换算**经 core tryConvert 唯一入口**（KIN-12 纯投影；SI 基线单位表＝core R1 注册表子集逐项 find 校验，缺位即 DataInvalid 不静默——P-RPT-9 同步面）后 std::to_chars 最短往返、四态呈现词（NotApplicable→"不适用"/NotProvided→"未提供"——§8.3 值列规则）、Invalid 保留原文（NFR-COR-03——脱敏不适用于本态）、曲线类复杂结构＝ResultBinding 引用文本 "run-<hex>#<fieldPath>"（不复制不丢弃——RP-CONS-4）；字段显示单位与 §4.2 unitPreference 冻结集同量纲冲突＝DataInvalid（冻结入身份的执行侧一致性闸）；§6.4 限定语固定推导序去重〔estimated/screening-only/historical-superseded/data-insufficient/not-applicable/outcome 三态——external-validation-incomplete 与 downgraded-reference-value 的条目级关联键未在 §4 报告模型冻结，不虚构关联（ERR-01），数据面随框架章节数源任务收口——本行登记〕）＋§9.3 渲染器契约（ReportRenderFormat/RenderOptions〔模板版本随渲染器不可调用方选〕/RenderArtifact〔digest＝SHA-256 经 core ContentDigester 唯一算法〕/RenderOutcome〔成功/失败两形态无取消——§9.3 取消行〕/IReportRenderer＋三实现 HtmlReportRenderer/JsonReportRenderer/CsvReportRenderer——服务级无状态并发安全、零 I/O 字节返回内存；HTML5 静态零外部依赖零脚本/样式内联/LF 无 BOM/data-field 机器可提取锚/折叠追溯区块〔§8.1"折叠呈现"——details 元素〕/正文缩略形＋完整身份规范文本限追溯区块附录〔UX-02〕；资格声明 §8.2① 前置断言门＝DataInvalid 硬门槛＋②不可行结论独立措辞；TemplateVersion＝sectionModelVersion 兼容检查〔make 产出的合法报告恒兼容——防御面〕；JSON＝顶层 schemaVersion "ird-report-json/1"＋SourcedValue 四态显式 token＋限定语数组字段＋kJsonTopLevelKeys 17 键冻结集与 firstUnknownJsonTopLevelKey 装配自检〔NFR-DEP-04 执行侧——产出物结构上不可携带未知字段，读取侧拒绝随 RPT-T07 回读消费同一冻结集〕＋canonical 写出经注入 io IJsonWriter〔SA-12 不建第二序列化器〕＋数值以文本规范形进 JSON 字符串〔三格式逐字符一致 AT-22〕；CSV＝主表 11 列冻结序＋诊断附表（comparison 三要素展开为实际/期望值＋单位四列）＋覆盖附表〔**caseId/status/runId 列序冻结、零数据行**——§4 模型 CaseCoverageSummary 为五态计数聚合、逐工况矩阵未入模型，不伪造逐工况状态（ERR-01），数据面随模型扩展填充——本行登记〕；单一 CSV 工件＝三份完整 CSV 文档以空行分隔顺序拼接、各携方言标识行〔emitDialectMarker=true 请求、字面归 io——表边界定位约定，本行登记〕）。**§9.5 io 注入契约族落位偏差**（IReportIoFactory/IReportCsvWriter/IReportJsonWriter/IReportOutputTarget/ReplacePolicy/ReportCsvCell/ReportJsonDom＋IReportOutputTarget 增 write 字节通道方法——§9.5 原文列于 Export.hpp〔RPT-T09 产物〕，但 §11 RPT-T06 行"注入 io 工厂"＝渲染器先于导出服务消费该族：CSV 转义/方言行/JSON canonical 行为全归 io 适配实现、工厂零行为（NFR-MNT-04 非转发包装器——隔离 P-RPT-1 未登记编译边＋约束报告只用 io canonical 路径），故暂载 Render.hpp，RPT-T09 Export.hpp 消费本头类型不重复定义——SectionStatus 暂载 ReportModel.hpp 的 v0.6 同型偏差先例；IReportOutputTarget::write 为"签名为实现建议"口径下的字节通道形状补全——io IAtomicFileWriter 写入方法在 io 侧，投影面无字节通道则 writer 无从落字节；公共头零 io 类型纪律不变〔P-RPT-1——接口只出现 reporting/core 类型〕，契约测试源码扫描常驻自证）。诊断脱敏双保险（§4.3.3/NFR-SEC-07）：注入 diagnostics::IRedactionService（C-3 登记边）以 User 档对机器产源自由文本〔诊断定位名/缺项原因/不适用原因/资格说明/评审元数据人读字段/生成者/版本串〕渲染前再过滤——**不含**值文本（Invalid 保留原文是 NFR-COR-03 硬约束）、词表 token 与规范身份文本（UX-02 允许域：追溯区块/附录/JSON 机器镜像）。测试面：test/RenderTest.cpp 十六条（FieldMatrixTest 六条＋HtmlRenderTest 九条＋RenderFormatTokenTest 一条——acceptance 1~5 单元内面对逐条对应；脱敏用例消费真实 RedactionService 非替身）＋contract_test/RenderCrossUnitContractTest.cpp 七条（值单源毒化矩阵三格式验证〔D-07/AT-22 结构性前提——三格式输出均携带毒化标记零重算〕＋JSON/CSV 格式契约字节面＋工厂会话记账＋公共头零 io include 源码扫描＋digest 类型恒等 static_assert——acceptance 4/5/6）；io fake 边界声明在案〔FakeCsvWriter/FakeJsonWriter＝io canonical 最小确定性替身——P-RPT-1 裁决补边后直连签名零改动；io 真实 canonical 正确性归 io 单元验证矩阵（SA-12）〕；RP-STATE-1~3 具名用例体随 RPT-T08 措辞冻结收口〔§8.2① 前置断言门即其渲染侧承载位——契约 note 两任务共享验证面互引〕。CMakeLists 仅增列源文件与两测试文件，目标名/别名/四条链接边/编译选项零改动。本文契约语义零变更（实现与 §8/§9.3 原文对齐；io 族落位偏差/entryKey 增列/覆盖附表零行/限定语两词推导面四项按本行登记——DTB §5.4 口径）。证据：traceability/builds/wp12-t06/ |
| v0.10 | 2026-09-20 | RPT-T07（一致性检查器，≙WP-12-T04——DTB §2.13 同行双产物之二）实现落位登记：§3.1 组成表契约头家族新增 `Consistency.hpp`（实现＝src/Consistency.cpp；私有共享头 src/RenderText.hpp 不出 include/——R-2）——§9.4 契约值类型逐字落地（ConsistencyInput〔report/matrix/artifacts 借用指针；artifacts 2~3 格式互异，<2 平凡通过〕/FieldMismatch〔定位四元组 format/fieldKey/expected/actual＋dimension〕/ConsistencyResult/IReportConsistencyChecker＋FieldConsistencyChecker）＋mismatch 维度词表（kMismatchDim* 六词＋parse-failed 错误轨词＋kMismatchAbsentMark 缺席标记）。**三项按本行登记（DTB §5.4 口径）**：①**ConsistencyResult 增补 notes 成员**（vector<string>——§9.4 原文形状无注记位，而 §9.4 ConsistencyInput 注释与契约 acceptance 4"artifacts<2 返回平凡通过＋**注记**"须有承载位；其余成员与原文逐字一致）；②**比对维度覆盖冻结**（acceptance 3 判定面）：value/unit/qualifier 三格式全比对（qualifier 任一格式丢失＝mismatch——§6.4"跳过限定语维度"禁止的执行面）；status＝CSV 原生 token 列＋HTML 中文显示名经共享映射表反查 token 后比对（src/RenderText.hpp 单表双向——RPT-T06 正向映射自 Render.cpp 抽取为共享私有头的**等价重构**，正向/反向同表防措辞冻结调整后双表失步产生假不一致）、JSON 字段镜像（ird-report-json/1 成员集冻结于 RPT-T06）不携带 field 对齐 status——该维度在 JSON 无回读值可比，不比对〔"携带却不查"与"该格式结构上不输出"不同质，非 §9.4 非法调用行的跳过限定语同款〕；ref＝仅 CSV 字段对齐携带 result_ref/evidence_ref 列（HTML/JSON 引用在追溯区块/顶层镜像非字段对齐）——仅 CSV 比对 resultRef/evidenceRef 双面；missing＝三格式全比对（矩阵驱动成员检查）；③**提取器失败语义边界裁定**：parse-failed（抛 ConsistencyMismatch，detail 含 dimension=parse-failed）＝结构层损坏〔JSON 语法错/CSV reader 报失败/HTML 标签与行列结构截断/fieldKey 重复锚定〕，mismatch（入列不短路）＝内容层差异〔字段缺席/值失配〕——工件整体缺条目表时矩阵驱动比对把全部字段判 missing（"提取器只对它进入的结构负责"）。三格式回读提取（§8.5 步④）：HTML＝data-field 严格文法扫描器（htmlUnescape＝htmlEscape 精确逆——具名实体在前 &amp; 收尾防二次解码；行缺 data-entry/双锚/单元格与行未闭合＝parse-failed；data-field 锚整格删除＝内容层 missing，fieldKey 以行内序位单元格重建）；JSON＝严格解析器（RFC 8259 子集：重复键/尾随内容/前导零/孤立小数点拒绝、\uXXXX UTF-8 编码代理对成对校验——reporting 自有 schema 的回读面非第二序列化器〔SA-12 不涉写出〕；顶层键消费 kJsonTopLevelKeys 冻结集拒绝外来未知字段——NFR-DEP-04 读取侧，RPT-T06 执行侧的对偶面）；CSV＝经注入 IReportCsvReaderFactory 取 reader 回读后**按列名映射**（Render.cpp csvMainColumns 冻结列序；多值列分号拆分空 token＝转义层损坏）——本实现**零 CSV 字节解析**，不私建第二套 CSV 解析器（NFR-SEC-03 转义唯一实现归 io/SA-12；P-RPT-1/P-IO-1 合并裁决补边后直连、签名零改动——acceptance 5 处置）。纯函数纪律（§9.4 维度表）：check() 同输入同结论/零副作用/不修改任何输入（全部 const 只读）/并发安全（工厂每次产出独立 reader）；调用方违约 Usage fail-fast（空 report/matrix、工件空指针、格式重复、工件 sourceReportIdentity 与报告 contentIdentity 不符、报告未冻结——同源前置的可廉价验证面；矩阵↔报告按 §9.4 前置信任）；回读本身失败→ConsistencyMismatch 错误轨（含工厂适配缺位——不静默当一致）。测试面：test/ConsistencyTest.cpp 十四条（平凡通过二〔0/1 工件＋notes 断言〕/Usage 四/HTML＋手写 JSON 镜像一致基线一〔fieldCount 断言〕/HTML 坏样本五〔删 data-field 属性/删整格→missing、单位改→unit、状态显示名改→status〔反查 token〕、删限定语 span→qualifier〕/HTML 截断 parse-failed 一/纯函数一——第二格式以手写 JSON 镜像夹具承载，schema 逐成员对齐 ird-report-json/1 冻结面，字节是测试数据非 io 协作）＋contract_test/ConsistencyCrossUnitContractTest.cpp 十三条（三格式一致＋reader 工厂记账一/CSV 改值一/JSON 删限定语一/不短路全量二坏样本一/Csv status·ref 列坏样本二/CSV 转义 roundtrip 一〔=、+、-、@、' 前缀/中文/引号/换行样例集——文件层转义形自证＋结构化层恒原文自证＋回读逐字符一致；空值列与"不适用"区分——§8.3〕/parse-failed 四〔JSON 截断/无标识行 CSV/引号结构破坏/工厂适配缺位〕/三格式纯函数一/公共头零 io include 源码扫描一——acceptance 1~5 逐条对应）；io fake 边界声明在案〔FakeCsvWriter/FakeCsvReader＝io §5.3 前缀转义可逆对＋RFC4180＋CRLF＋方言行分帧的最小确定性替身——P-RPT-1 裁决补边后直连签名零改动；roundtrip 用例验证报告链注入缝与消费路径，io 真实转义正确性归 io 验证矩阵 IO-V01/V02〕。CMakeLists 仅增列源文件与两测试文件，目标名/别名/四条链接边/编译选项零改动；Render.cpp 正向映射改经 RenderText.hpp 共享表（等价重构，调用点拼写不变）。本文契约语义零变更（实现与 §8.5/§9.4 原文对齐；notes 增补/维度覆盖/失败语义边界三项按本行登记）。证据：traceability/builds/wp12-t07/ |
| v0.11 | 2026-09-20 | RPT-T08（措辞冻结渲染规则——限定语映射/两类声明前置断言，卡内行≙WP-12-T06）实现落位登记（渲染器内规则层，涉及文件 Render.cpp——契约 note 原文口径）：①**§8.2①硬编码纪律规则层**＝`validateDeclarationConsistency`（三格式渲染装配前统一执行；DataInvalid——"渲染了再说"路径不存在）五路前置断言：⓵非 Completed 结果永不携带任何声明资格（RP-STATE-1/TASK-02 表 3）；⓶formalPass 资格 ⇒ Verified∧Completed∧Feasible（RPT-05 五条件的报告可见面——Quick 永不满足 FormalPass，表 1 模式效力）；⓷reviewRecord 资格 ⇒ Completed∧EngineeringInfeasible（§6.2 行 2）；⓸章节资格声明 ⇔ 所引结果资格 AND 聚合（§8.2①"条目所属结果"复核——v0.8 构建器聚合不变量的渲染侧防线）；⓹覆盖漏验 ⇒ 任何章节不得声明 formalPass（EVI-02/§8.1 表 2②呈现口径）——断言不重算资格（§2.1 C-2/PA-1），只核对已冻结数据内部一致性（与单位冻结集闸同性质防御面）；②**措辞冻结收口**（两声明独立——RPT-05/RV-07）：不可行声明对齐 §8.2②原文引号形"经验证的不可行结论（正式评审记录）"；否定声明由"未达正式通过（资格不成立）"调整为"不构成正式结论（资格不成立）"（§6.2"不构成结论"词族——RP-STATE-1"HTML 文本扫描'正式通过'计数=0"机械口径要求否定表述不含声明短语，同位置同资格语义）；追溯附录资格列名"正式通过资格/评审记录资格"调整为"formalPass 资格/reviewRecord 资格"（附录为 UX-02 追溯域＋扫描口径——声明短语唯一合法出源＝资格成立时的声明行）；状态呈现钉住"已取消/失败/已中断——不构成结论"（§6.2 行 5）；③**§6.4 限定语词表锁定**（acceptance 3——逐 token 触发数据源与呈现义务）：单元格级 8 token（estimated/data-insufficient/screening-only/historical-superseded/not-applicable＋outcome 三态——§6.4 触发数据源列中已入 §4 报告模型者）具名用例锁定三格式结构性在场（HTML data-qualifier 标记/CSV qualifier 列/JSON qualifier 数组——任何格式省略＝ConsistencyMismatch，T07 检查器维度覆盖不变）；**§8.2④模板纪律自检**＝`assertWordingFreezeTemplateDiscipline`（§6.4 词表全 10 token 呈现文案缺位或模板版本低于限定语强制下限 v1＝TemplateVersion 拒绝——"模板不得移除限定语"的机械化，防御性不变量与 MatrixIndex 重复键拒绝同性质）；**报告级 2 token 报告侧承接**：external-validation-incomplete＝外部验证边界章 Recorded 态限定语（HTML data-qualifier＋JSON externalResourceSummary[].qualifier 数组——§6.4 呈现义务"外部验证边界章节＋相关条目标注"的章节半区；条目级关联键未入模型，v0.9 收口安排不变）；downgraded-reference-value＝覆盖摘要块限定语（**P-EV-5 报告侧承接**——数据源＝diagRefs 携带 EVI-REGION-COVERAGE-DOWNGRADED，evidence `kDiagRegionCoverageDowngraded` 常量同源不私抄字面；HTML data-qualifier＋JSON coverageSummary.qualifier 数组；不得弱化为普通标注、不得省略；CSV 面＝数据面——诊断附表携带该码，列序冻结不动；条目级关联键收口安排 v0.9 不变）。**登记偏差（DTB §5.4）**：JSON 镜像 coverageSummary 与 externalResourceSummary 条目增补 qualifier 数组成员（顶层 kJsonTopLevelKeys 17 键冻结集不变——嵌套呈现面，§6.4 结构性输出在报告级块的承接位）；④**§6.5 RP-COV-1 覆盖呈现**：coverageIncomplete 呈现侧判定（未执行/执行无效/执行失败任一非零，或已执行＋不适用≠必验总数）＋覆盖完备性结论三态（全覆盖→"全部启用必验工况已覆盖"；漏验→漏验清单规模计数＋"整体数据不足（缺项呈现）"——逐工况漏验名单数据未入模型，v0.9 登记不变，不伪造逐工况状态；totalRequired==0→"无启用必验工况"——P-EV-7 呈现侧如实表达，判定语义归 evidence）；包络合并标注＝条目 caseScope 跨多工况的结构触发，标注"包络合并（呈现方式）"逐条目一次（§6.5/DYN-07 呈现方式声明——不替代逐工况条目、不参与覆盖核算；人读 HTML 呈现面，JSON/CSV 数据面由 caseScope 忠实承载，不增设第二标注通道）。测试面：test/RenderTest.cpp 新增 12 用例（WordingFreezeTest 八——RP-STATE-1 具名两例〔断言①六路反例＋状态呈现扫描计数=0〕/RP-STATE-2 具名/§8.2②不可行声明与证明呈现/断言④双反例/断言②Quick 反例＋screening-only 三格式强制/§6.4 词表 8 项三格式逐 token/外部边界限定语；CoveragePresentationTest 四——全覆盖结论/漏验呈现＋断言⑤/包络合并标注/P-EV-5 降级限定语强制呈现与零伪造）＋io 写出替身最小形态（FakeCsvWriter/FakeJsonWriter/MinimalIoFactory——与契约测试同款边界声明，夹具自持）＋FormalPassWordingGate 夹具改为资格一致形态（章节声明与所引结果 AND 聚合一致——断言④同口径；两声明并存改为"各随其结果、不同章节"的诚实可达形态，T06 门语义断言不变）；v0.9 登记的"RP-STATE-1~3 具名用例体随 RPT-T08 收口"兑现。双模式构建零错误；测试两模式 120＋38 全绿；ird_gates 双树直跑命中集归一化 diff 为空（GATES-HIT-SET-IDENTICAL——gates-hit-set-diff.txt）。本文契约语义零变更（实现与 §6.2/§6.4/§6.5/§8.1/§8.2 原文对齐；否定声明与附录列名措辞调整、JSON qualifier 嵌套成员、包络合并结构触发等项按本行登记）。证据：traceability/builds/wp12-t08/ |
| v0.12 | 2026-09-20 | RPT-T09（导出与归档协调——Export/Archive＋IReportArtifactSink，卡内行≙WP-12-T03『幂等部分』/T04）实现落位登记：§3.1 组成表契约头家族新增 `Archive.hpp`（实现＝src/Archive.cpp）与 `Export.hpp`（实现＝src/Export.cpp）。`Archive.hpp`：§9.6 IReportArtifactSink 四方法**逐字**（begin/writeArtifact/finalize/abandon——acceptance 5"四方语义与 §9.6 逐字一致"面；begin 前置环境错误〔ReadOnlyStore/目录占用〕走异常轨〔§9.9"StoreError 透传→ReportError"投影〕、write/finalize 操作结果经 ArtifactStatus 六值、abandon 不抛＝责任终结）＋ReportArtifactBeginRequest/ArtifactWrite/ArtifactSessionRef（值句柄——跨单元值边界，非指针）＋ReportEndReason 三值（§9.6 abandon 注释原文）＋§7.3 ReportArtifactManifest 全字段（formatId "ird-report"/schemaVersion/身份链/数据源锚/双层内容身份/生成信息/artifacts[] 五列/sourceRuns/finalizedAtUtc/manifestDigest）＋computeManifestDigest（**幂等判定键 D-02**：contentIdentity＋工件集 (relPath,format,sha256,sizeBytes,rendererVersion,templateVersion) 按 relPath 字节序稳定排序的 canonical 编码〔magic "IRDRPTM1"＋编码器版本＋长度前缀＋大端——ReportCodec §4.4 同口径〕SHA-256，core ContentDigester 唯一算法；**排除** reportId/supersedes/reportVersion/数据源锚〔不以 ReportId〕、生成与发布时刻〔D-04 同源——同输入重建摘要一致是幂等前提〕、formatId/schemaVersion〔格式常量〕；空工件集/重复 relPath＝Usage fail-fast）＋IReportArchiveCoordinator/ReportArchiveCoordinator（**编排顺序 begin→write×N→finalize、任一非 Ok→abandon**——§9.6 编排行；幂等预检经只读索引先于 begin：同 reportId 已 Finalized 且摘要一致＝IdempotentHit **零汇集座调用**〔零重写最强形态——acceptance 1"sink 调用记录"〕、不一致＝ArchiveConflict＋差异定位诊断〔contentIdentity 异＝"身份维度"、同而摘要异＝"版本/工件集维度"——D-02 判定键因子推演〕；finalize 处 IdempotentHit/Conflict 为汇集座兜底复核〔§9.6 finalize 注释——并发/索引滞后防御〕；DiskFull→abandon(Failed)＋比较型诊断〔§7.4 表〕；取消→abandon(Canceled)→Canceled〔UX-03〕；listPublished 仅 Finalized）＋IReportPublishedIndex 只读清单注入缝。`Export.hpp`：§9.5 ExportDestination/ReportExportRequest/ReportExportResult/ExportedFile/IReportExportService **逐字段**＋ReportExportService（§8.5 编排步①~⑤：解析〔IReportResolver 注入〕→矩阵单次提取→逐格式渲染＋**二次渲染字节比对**〔§8.5 步③——不一致＝RenderFailed 渲染层非确定〕→一致性检查〔FieldConsistencyChecker 注入构造，不一致＝ConsistencyMismatch＋RPT-CONSISTENCY-MISMATCH 诊断、零写出——"一致性不通过不出工件"〕→发布/写出；**§7.4 四轨**：项目内经 ReportArchiveCoordinator（manifest 由服务自报告值装配——身份/生成信息/工件摘要列/sourceRuns 去重保序）；外部路径＝目标存在读字节比对〔只读探测——偏差登记⑤〕：同字节＝幂等成功无操作〔不调 makeAtomicTarget——mtime/哈希不变，acceptance 1〕、异字节 NeverOverwrite＝ExportFailed 拒绝＋RPT-EXPORT-FAILED 诊断〔cause 引 io 族码 IO-PACK-TARGET-EXISTS——码值权威归 io/diagnostics，cause 文本引用非注册〕、OverwriteAtomic＝makeAtomicTarget 原子替换〔commit 失败→abort 清理＋ExportFailed、先前输出完整保留〕；ReadOnlyStore→两路径诊断〔PM-07：项目内拒绝≠导出被禁〕；逐格式渲染后/逐工件写前取消检查点→五成员全空〔UX-03 形态〕；formats ≥1 必含 Html 无重复〔§9.5 非法调用行〕；withEvidenceBundle==true＝Usage 显式拒绝〔证据包组装归 RPT-T10——能力到位前不静默忽略，RPT-T10 落地后改组装编排〕）＋工件相对命名常量（report.html/report-data.json/report.csv＋kManifestRelPath "report.json"——§7.3 与 §8.3 文件名重叠的让位决策）。**落位偏差登记（DTB §5.4，六项）**：①IReportPublishedIndex 只读清单接口（§9.6 sink 四方法无读取通道而 §7.4 幂等行要求"返回既有 PublishedReportRecord"、§9.6 协调器要求 listPublished 仅 Finalized——既有记录〔含原 publishedAtUtc〕的诚实读取通道；与 sink 成对注入、project 侧实现，非第二写路径）；②IReportResolver 报告值解析注入缝（§9.5 前置"reportId 可解析〔本会话构建或自已发布清单加载〕"的承载）；③取消结果形态＝ReportExportResult 五成员全空（§9.5 结果结构无取消位——构建器 outcome UX-03 全空形态同款：成功必有 published 或 files 在场，全空即取消信号）；④withEvidenceBundle 处理＝Usage 显式拒绝（RPT-T10 边界——见上）；⑤外部幂等判定的存在性/字节探测经只读文件读（std::filesystem/ifstream——P-RPT-1 纪律约束 io 单元 canonical **写出**设施的编译边，读探测不属编码/转义行为〔SA-12 不涉〕；全部写入仍经注入 makeAtomicTarget 原子协议，零写路径旁路）；⑥DiskFull 比较型诊断的可用侧＝NotProvided 四态（汇集座四方法契约无可用空间通道——所需侧数值在位〔失败工件字节数〕，可用侧精确值随 project 实现对齐补充——P-RPT-4 关联）。另登记：RPT-DISK-FULL/RPT-READONLY-STORE 两诊断码引用自 §7.4 表原文、**登记状态＝待收编**（不在 P-RPT-8 已收编 8 项内；码值权威归 diagnostics StableCodeRegistry，收编前呈现链路按"占位＋开发诊断"处理——diagnostics §4.5；本文档头版本行 v0.11 停留 v0.10 的同步遗漏本版补登）。CMakeLists 仅增列源文件（src/Archive.cpp、src/Export.cpp）与两测试文件，目标名/别名/四条链接边/编译选项零改动。测试面：test/ArchiveExportTest.cpp 30 条（ManifestDigestTest 七〔D-02 判定键五性〕＋CoordinatorTest 十〔幂等零调用/冲突双维度/磁盘满可重试/取消双检查点/只读两路径/清理失败零影响/Usage 矩阵〕＋ExportServiceTest 十三〔非法调用矩阵/外部成功/外部幂等 mtime 哈希不变/NeverOverwrite 保留/OverwriteAtomic 成功与失败保留/项目内磁盘满重试/manifest 同源装配/服务级幂等/只读两路径/三取消点〕——acceptance 1~4 逐条对应）＋contract_test/ArchiveCrossUnitContractTest.cpp 13 条（枚举声明序/值句柄/命名常量/摘要类型恒等 static_assert、公共头零 io include 行扫描、sink 契约 fake 投影六条〔D-13 原子发布入清单/D-14 幂等零重写/冲突不覆盖/只增幂等/abandon 终结/只读异常轨〕、状态↔错误码转译面——acceptance 5"与 project 工件汇集座〔fake〕的协作随 RPT-T09 落地于 contract_test"兑现，§3.4 目标分工行）；FakeArtifactSink（test/FakeArtifactSink.hpp）随任务交付——§10 替身清单三项故障可注入（冲突〔seedPublished 摘要预置/强制注入口〕/磁盘满〔diskFullAtWrite〕/清理失败〔cleanupFailure 计数〕）＋调用日志观测；替身边界声明在案（仅验证 reporting 侧契约语义、不构成 project 持久化正确性证明——具名替身收口归 RPT-T11，卡行 T11 产物列原文）。双模式构建零错误；测试两模式 150＋51 全绿；ird_gates 双树直跑命中集归一化 diff 为空（GATES-HIT-SET-IDENTICAL——gates-hit-set-diff.txt）。本文契约语义零变更（实现与 §7.3/§7.4/§9.5/§9.6 原文对齐；六项落位偏差按本行登记）。证据：traceability/builds/wp12-t09/ |
| v0.13 | 2026-09-20 | RPT-T10（证据包组装——Bundle＋IArchiveWriter，卡内行≙WP-12-T05）实现落位登记：§3.1 组成表契约头家族 `Bundle.hpp` 落地（实现＝src/Bundle.cpp）。`Bundle.hpp`：§9.6 IArchiveWriter **三方法逐字**（open〔同卷临时区〕/addEntry〔entryName 纯相对正斜杠——SP-2 口径〕/finish〔返回 core::Digest256 容器级摘要——装配器不消费；签名注释"原子替换到目标；异常/失败→清理临时、目标不变"〕；reporting 定义、io 实现——ZipChannel 之上薄适配，P-RPT-7；测试经 FakeArchiveWriter 注入，reporting 不私建第二套 ZIP 实现〔NFR-DEP-03 第三方唯一渠道经 vcpkg 归 io〕）＋**放弃路径契约澄清**（§9.6 三方法无 abandon——调用方不 finish 即废弃会话〔协作取消/上游失败〕的清理义务由实现析构承担〔RAII〕，ReportBuildSession 析构守卫同型先例，签名零改动——本行登记）＋§7.6 EvidenceBundleRequest 三字段逐字（runs **空向量＝缺省取报告 resultRefs**——原文"默认＝报告 resultRefs"；非空须 ⊆ resultRefs 无重复〔越界＝Usage——包与身份锚脱钩即违约〕；target 空＝Usage）＋BundleStatus/EvidenceBundleOutcome 三形态（取消＝全空〔UX-03〕；失败＝error 在场——BundleIncomplete detail 全量列出）＋**IEvidenceBundleSource 归档事实注入缝**（§7.6 输入②"归档工件清单（project runDir 逐字节读取授权）"＋快照冻结凭据的投影承载：tryEnvelopeDigest〔归档 envelope 工件摘要——RunManifest 事实投影，reporting 不自算摘要〕/tryCaseSetDigest〔RequiredCaseSet::requiredCaseSetId 投影——§4.1.3 冻结凭据〕/tryConfigurationRefs〔快照 configurationRefs 投影——种子/线程数/容差的值在配置 canonical 字节内，域 schema 归域〔N-5/N-9〕，bundle 只承载 configKindToken＋contentIdentity 引用；nullopt＝事实缺位计缺失、空集＝合法无配置〕；§3.3 注入模式，L5 以 project RunManifest＋evidence 快照事实适配）＋IEvidenceBundleAssembler/EvidenceBundleAssembler（§7.6"组装〔IEvidenceBundleAssembler，reporting 所有〕"：请求校验 Usage fail-fast→**收集阶段零 IO**〔先全量收集再写——要素缺失→BundleIncomplete **writer 从未 open**＝不产出半包零临时，缺失清单"<要素>@<runId>"全量列出不短路；复现块 productVersion/evidenceContractVersion 必填字段逐字段完整性核对〕→逐条目写出〔JSON 经注入 IReportIoFactory canonical 序列化——SA-12 不自建序列化器；写前取消检查点逐条目〕→totalDigest〔magic "IRDRPTB1"＋编码器版本＋条目数＋逐条目（名＋条目 SHA-256）大端编码，core ContentDigester 唯一算法；**排除 bundle.json 自身**——generatedAtUtc 不入内容摘要〔D-04〕，同输入同摘要 NFR-COR-02〕→finish 原子替换；Usage 抛出 fail-fast、环境错误转译 Failed——Archive.cpp 同款）。**实现口径登记（DTB §5.4，零契约语义变更）**：①条目编址＝§7.6 目录形态的 ZIP 条目化（snapshot/<runId 规范文本>/snapshot.json〔refs-only 四列：snapshotId/sliceId/inputBaselineId/caseSetDigest〕、results/<runId>/result.json〔envelopeDigest＋证据清单 itemId/status/digest〔Satisfied 携摘要——presence 纪律的序列化对偶〕＋复现要素 ReproductionBlock 逐字段〕、reproduction.json、bundle.json〔六字段 schemaVersion "ird-evidence-bundle/1"/reportId/contentIdentity/sourceRuns/generatedAtUtc/totalDigest——§7.6 大括号原文逐字段〕）；②reproduction.json 形状＝逐运行 {versions＋configurations}——版本列逐字段原样承载 evidence 值、不聚合不改写〔"数值来自 evidence 复现块与配置 canonical，不自算"——种子/线程数/容差在配置 canonical 内由引用承载，消费方按快照物化后按域 schema 读值〕；③evidence::EvidenceItemStatus 五值的包内 JSON 词面映射 bundleItemStatusToken（satisfied/missing/invalid/unverified/not-applicable——格式序列化词面非第二词表〔O-13：枚举类型恒等由契约测试 static_assert 钉死〕；全枚举 switch 无 default 防漏）；④withEvidenceBundle 导出编排接线不在本任务面（§11 涉及文件列"同名文件"——Export.hpp 偏差④的显式 Usage 边界维持，装配经 §7.6 EvidenceBundleRequest 独立入口；接线编排随集成任务收口）。测试面：test/BundleTest.cpp 十四条（清单完整六字段/摘要确定性 D-04/缺省 runs/缺失拒绝零临时/全量列出/复现块逐字段/空集与缺位分界/取消三检查点〔入轨/收集/写出〕/写出失败两路/失败重试/Usage 五路矩阵——acceptance 1~3 与 RP-BUN-1 观测点〔bundle.json 内容/条目枚举/终态〕逐条对应）＋contract_test/BundleCrossUnitContractTest.cpp 七条（§9.6 三签名 static_assert＋P-RPT-9 类型恒等/状态词面稳定互异/格式独立标识/SP-2 拒绝五形态/会话纪律/放弃路径 RAII/条目编址相对性与项目镜像零要素＋**P-RPT-7 源码扫描零第二套 ZIP 实现常驻自证**——acceptance 4/5）；FakeArchiveWriter（test/FakeArchiveWriter.hpp）随任务交付——真实临时文件承载生命周期（open 建 <target>.ird-fake-partial／addEntry SP-2 校验只增／finish 发布／open、addEntry、finish 三类故障注入各自清理临时目标不变／析构 RAII 放弃路径）；替身边界声明在案（不构成 ZIP 容器格式正确性证明——容器格式/压缩/原子替换归 io 验证矩阵；具名替身收口归 RPT-T11，卡行 T11 产物列原文）。CMakeLists 仅增列源文件与两测试文件，目标名/别名/四条链接边/编译选项零改动。本文契约语义零变更（实现与 §7.6/§9.6 原文对齐；放弃路径澄清/条目编址/totalDigest 规范域/reproduction.json 形状/状态词面四项按本行登记）。证据：traceability/builds/wp12-t10/ |
| v0.14 | 2026-09-20 | RPT-T11（替身与契约套件——四具名替身＋RP-* 用例体全量收口，卡内行≙WP-12-T09；任务契约 tasks/foundation/RPT-T11.json）实现落地登记：**四具名替身正本落位**（§10 可控替身清单）——`test/ScriptedResultSource.hpp`（新建：脚本面 envelope 经 evidence ResultEnvelope::make 构造——合法组合-only、§10 替身边界声明原文；调用计数观测面承接 §6.2 不缓存重查断言）＋`test/ScriptedSectionProvider.hpp`（新建：§5.3 四态内容脚本化，§9.2 纯投影/值拷贝请求契约承载）＋FakeArtifactSink/FakeArchiveWriter 升格具名正本（RPT-T09/T10 交付，头注补登记）；**局部夹具收敛**（acceptance 2『随 RPT-T11 收口』承诺兑现）——RPT-T04 SectionProviderTest/RPT-T05 BuilderTest 局部两替身、RPT-T10 BundleTest 局部 FakeResultSource、RPT-T05 contract_test 局部 MinimalResultSource 全部替换为具名正本，BuilderTest 身份铺位值对齐具名替身 scriptedTask/scriptedCid（构建器锚定一致性核对面）；**RP-\* 契约套件**＝`test/ReportingScenario.hpp`＋`test/ReportingContractSuiteTest.cpp` §10.1 矩阵 32 组统一用例体（RP-MDL-1~3/RP-SCOPE-1~4/RP-STATE-1~4/RP-CUR-1~3/RP-COV-1/RP-TRACE-1/RP-CONS-1~4/RP-CONC-1/RP-CANC-1/RP-IDEM-1/2/RP-CONF-1/RP-DISK-1/RP-RO-1/RP-SAN-1/RP-REVW-1/2/RP-BUN-1/RP-GATE-1；阶段 A 范围收敛点如实登记于套件文件头——框架章节端到端随 RPT-T12/T14/T15、coverageSummary 聚合语义未冻结零值分支如实断言、RP-ROUND-1 阶段 C 不在本任务矩阵）；**testkit 设施接入（T-1 允许形态）**——双自有 main（test/contract_test 各 TestMainReport.cpp）安装 RecordListener→ird-test-report.json 与 gtest XML 并存（testkit §7.2/§7.3；EX-T09 先例）；IRD_TEST_INFO/IRD_EXPECT_IDENTICAL/TempDir/DeterministicEnv/ContractCheck 消费；CMake 两测试目标链接 sdurws_ird_testkit＋GTest::gtest（产品目标零 testkit 不变——LinkageContractTest::NoTestkitEdge 收窄重述＋UnitEdgesFourRegistered 白名单显式增量修订，EX-T09 同款节奏）；**替身边界声明正本**＝`test/README.md`（RP-STATE-4 文档留痕位）＋四具名替身头声明＋RP_STATE_4 用例机器核对；**P-RPT-9/O-13 处置**——替身消费 evidence/core 契约维持各卡 v0.1 Draft 基线（冻结 diff 后按影响面同步），EvidenceItemStatus 五值各有其位不合并断言锁定（RP_STATE_2 静态断言＋五态互异＋呈现面锁定）。本文档契约语义零变更。证据：traceability/builds/wp12-t11/ |
| v0.15 | 2026-09-20 | RPT-T12（模型摘要与任务投影接入——ModelSummary/ITaskStatusSource，卡内行；任务契约 tasks/foundation/RPT-T12.json，interUnit＝产出两件跨单元注入契约）实现落位登记：§3.1 组成表契约头家族新增 `ModelSummary.hpp` 与 `TaskStatusSource.hpp`（两契约头 **header-only**——§11 产物列仅列头文件，SectionProvider.hpp v0.7 同款先例；公共头计数 12→13 按本行登记）。`ModelSummary.hpp`：§9.7 schema **逐字段冻结落地**（revision/snapshotIdentity/modelIdentity/nameMapIdentity/policyContentIdentity〔core::ContentIdentity 四块——类型恒等 static_assert 常驻自证，P-RPT-9 基线＝core.md v0.1 Draft〕＋jointCount/allRevolute 链型摘要＋installPresetToken 四词呈现 token＋Capabilities 八布尔〔内嵌结构，字段序即 §9.7 声明序〕＋resources{resourceId,state,digest}/configurations{configKindToken,contentIdentity}）＋IModelSummaryProvider（trySummary 签名 std::optional\<ModelSummary\>(core::RevisionId) const **冻结**——P-RPT-2 处置：架构所有者确认注入为正式形态或补边裁决前签名不变，契约测试 static_assert 常驻自证；后置三支＝在案全字段/快照已释放→snapshotIdentity 非空＋其余可空＋注记〔runtime §10.12 口径〕/闭包外→nullopt 不伪造）＋资源状态投影词表 ResourceStateKind{Solidified,Recorded}（runtime ResourceState 值投影——同名一一映射）＋头内 L5 适配建议（IRuntimeModelView→IModelSummaryProvider 装配要点——适配代码归 L5，本任务不建 runtime 编译边）。`TaskStatusSource.hpp`：ITaskStatusSource **最小接口**（恰一个 tryStatus(core::TaskIdentity) const 只读查询——无控制面/无事件订阅〔§3.3 事件仅预览刷新提示不入报告内容〕/无批量枚举；§9.9 引用边界"任务状态引用=TaskIdentity"逐字落位）＋TaskStatusProjection（state＝core::TaskState 九态**直传零第二词表**＋progress 可选投影＋termination 仅终态非空）＋TaskProgressProjection（percent/phaseToken/batchesDone/batchesTotal 四字段对齐 execution ProgressReport 值语义）＋TaskTerminationKind 五值投影词表（canceled/failed/completed/interrupted 与 core::TaskState 四终态 token 逐字一致＋force-terminated 单列——"不伪装为普通失败"呈现承载）＋头内 L5 适配建议（tryTask/progress→TaskStatusProjection 映射）。**落位偏差登记（DTB §5.4，四项，零契约语义变更）**：①ITaskStatusSource 独立成头 `TaskStatusSource.hpp`（§3.1 组成表未列该头而 §3.3/§11 卡行产物列要求交付——对端不同〔runtime vs execution〕不与 ModelSummary.hpp 混载，组成表增列一行＋计数 12→13）；②ModelSummary 增补 provenanceNote 注记承载位（§9.7 结构体原文无注记位而后置行"＋注记（不伪造）"须有 schema 承载位——ConsistencyResult.notes v0.10 同型先例；参与值相等比较）；③ResourceStateKind/TaskTerminationKind 两投影词表（§9.7/§3.3 值形态的对端词表零 runtime/execution 类型承载——同名一一映射由 L5 适配器落实；core::TaskState 直传不设转词）；④TaskStatusProjection 不投影 archivePhase（§3.3"任务终态/『已中断』呈现数据源"的最小面——归档辅轴非任务终态呈现面；Completed＋归档失败的报告路径经 envelope 缺失→SourceMissing 自洽，不依赖任务投影）。**验收对照**：acceptance 1＝schema 全量落地＋冻结留痕 traceability/builds/wp12-t12/schema-freeze-note.md（§9.7 逐字段核对结论——runtime §13.2 交接凭据）；acceptance 2＝test/ModelSummaryTest.cpp 六用例（全字段往返/词表 token/呈现字段零判定通道/同修订同投影/快照已释放注记/闭包外 nullopt）；acceptance 3＝TaskStatusSource.hpp＋test/TaskStatusSourceTest.cpp 六用例＋contract_test 公共头零 runtime/execution/io include 与命名空间限定名扫描（SA-10 表外边红线常驻自证）；acceptance 4＝trySummary 签名冻结 static_assert＋注入形态维持（零 runtime 编译边）；acceptance 5＝接口面仅 trySummary（installPresetToken 零判定通道）＋禁项注释逐字（不触发编译/不读 WC/DWC/权威模式呈现不经本接口）。CMakeLists 仅增列两测试文件与一契约测试文件，目标名/别名/四条链接边/编译选项零改动；两替身（ScriptedModelSummaryProvider/ScriptedTaskStatusSource）为局部夹具登记形态（替身边界声明在案——不构成 runtime/execution 行为证明，真实适配归 L5 装配）。本文契约语义零变更（实现与 §9.7/§3.3 原文对齐；四项落位偏差按本行登记）。证据：traceability/builds/wp12-t12/ |
| v0.16 | 2026-09-20 | RPT-T13（文档与门禁同步——README 核对/待裁决状态更新/裁决申请提交，卡内治理行；任务契约 tasks/foundation/RPT-T13.json；**纯文档任务零代码变更**——reporting/src、include/*.hpp、CMakeLists、test 零触碰，契约 outputs＝doc-update＋traceability-update）：①**RPT-T01~T12 零偏差登记（acceptance 4）**——§3.1 组成表与磁盘逐项复核：include/ 14 文件＝13 契约头＋README.md 与组成表逐行对应（13＝RPT-T02~T12 累计落地：2＋1＋2＋1＋1＋1＋1＋2＋1＋1，其中 SectionProvider.hpp/ModelSummary.hpp/TaskStatusSource.hpp 三头 header-only——各卡产物列形态）；src/ 12 翻译单元（Reporting.cpp 锚点＋Errors＋Identity＋ReportModel＋ReportCodec＋Sections＋Builder＋Render＋Consistency＋Archive＋Export＋Bundle——累加 1＋2＋2＋1＋1＋1＋1＋2＋1 与 v0.4~v0.13 各登记一致）＋2 私有头（ReportCodec.hpp——v0.6、RenderText.hpp——v0.10 等价重构抽取，均 R-2 不出 include/）；test/ 14 测试 .cpp＋5 替身/夹具头（FakeArtifactSink/FakeArchiveWriter/ReportingScenario/ScriptedResultSource/ScriptedSectionProvider）＋README.md、contract_test/ 10 契约测试 .cpp＋TestMainReport.cpp——与 §11 产物列及 §14.4 v0.4~v0.15 各登记逐项对应；**偏差登记面核查**＝v0.6（SectionStatus/RenderHint/冻结视图族暂载 ReportModel.hpp）、v0.8（IReportResultSource 两方法增量）、v0.9（io 注入契约族暂载 Render.hpp＋entryKey 增列等）、v0.10（notes 增补/维度覆盖/失败语义边界三项）、v0.12（六项）、v0.13（四项）、v0.15（四项）均已按 DTB §5.4 就地登记——RPT-T01~T12 实现与卡行逐任务**零未登记偏差**；双模式复证实跑零回归：集成模式 sdurws_ird_reporting_test 208/208＋contract_test 61/61、冒烟模式同 208/208＋61/61（四份 ird-test-report＋gtest XML——wp12-t12 验收基线 208＋61 逐数一致，零新增用例——纯文档任务）；ird_gates 双树直跑命中集与 wp12-t12 验收基线全等零新增（gates-hit-set-diff.txt——GATES-HIT-SET-IDENTICAL）。②**README 指向核对零偏差（acceptance 1）**——`include/sdurws/ird/reporting/README.md` 指向维持 §11：本行现文（"指向本文 §11〔任务拆分〕——v0.4 复核更正"）、README 现文与 §11 现文（该节真实存在且含 RPT-T13 治理行）三方一致，RPT-T01 acceptance 4 核对结论（前置事实）复审通过——无回退、无第二指向、无悬空；README 实现状态段同步至 13 契约头落位现状（取代 2026-09-19 段"契约头随 RPT-T02+ 落地"前瞻句），"不参与编译/不表示已实现"声明保留；§1.2"构建骨架"行（2026-09-10 实测快照表）"引用本文 §9（实测一致）"为编写时历史记载——卡头注记已声明快照口径，现行口径以构建落位行＋本行 v0.4/v0.16 更正为准，快照表不改写；核对证据 traceability/builds/wp12-t13/README-audit.md。③**§14.3 状态更新（acceptance 2）**——P-RPT-1~9 逐条附〔RPT-T13 状态同步 2026-09-20〕：**P-RPT-1/2/3 裁决申请三件提交**（traceability/wp12-t13-adjudication-requests.md——申请一 P-RPT-1 建议与 P-IO-1/P-PR-5 三案同批裁决〔补登 reporting→io 或确认注入形态，随附 O-09 型回填条款〕；申请二 P-RPT-2 确认 IModelSummaryProvider 注入为正式形态或补边〔schema-freeze-note.md＝runtime §13.2 交接凭据〕；申请三 P-RPT-3 core 收编 `fnd-`/`rpt-` tag〔与 P-DIAG-4 同案〕；裁决者栏按本节原文——架构所有者/架构所有者＋runtime 详设所有者/core 详设所有者；**申请≠裁决**：不私改 ARCHITECTURE.md/core 公共头〔forbiddenFiles 红线〕，裁决前各任务注入形态维持不变）；P-RPT-4/5/7/9 维持 open 并登记实现侧落位事实与上游实测（project.md 实测 v0.18、io.md v0.9、core.md v0.11、evidence.md v1.3、ARCHITECTURE v0.12——均未冻结未评审）；P-RPT-6（O-25 消账）/P-RPT-8（码值收编 2026-09-10）如实维持；九项裁决归属均不变更、不私裁（沿 CORE-T10/EV-T12/RT-T13/DIAG-T11/PRJ-T16 文档任务纪律）。④**§12.1/§12.2 交接状态表同步刷新（acceptance 4）**——§12.1 project/runtime/evidence/io 四行状态列按落地事实刷新（sink 契约已落地/ModelSummary 已冻结并落地＋裁决申请/evidence 消费经 IReportResultSource 注入隔离/io 注入契约族已落地＋裁决申请）；§12.2 project/evidence/io/runtime/ui 五行"从 reporting 接收"列补落地锚点（Archive.hpp/IReportResultSource〔RPT-T05〕/Bundle.hpp＋`ird-report-json/1` 冻结/ModelSummary.hpp/Export.hpp）；§3.1 README 行补 RPT-T13 复核结论。⑤**卡头同步**——文档版本行 v0.15→v0.16；文末"本文档结束"行版本残留 v0.6 更正为 v0.16（卡内措辞 slips——非语义变更，DTB §5.4 登记不静默）。验证：validate-docs.ps1（契约 verify 命令）PASS、validate-task.ps1 对 RPT-T01~T13 十三份契约三查零失败、ird_gates 双树直跑命中集全等、双模式构建零 error 与测试复证留痕见 traceability/builds/wp12-t13/ |
...
**本文档结束**（reporting 单元详细设计 v0.16，状态 `Draft`；对应任务 WP-12-T01。）
