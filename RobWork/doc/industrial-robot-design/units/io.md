...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.8（IO-T05 落位同步；v0.7＝IO-T04 落位同步＋JSON-SYNTAX 码追加；v0.6＝P-IO-3 选型冻结登记；v0.5＝IO-T03 落位同步；v0.4＝IO-T02 落位同步；v0.3＝IO-T01 落位同步；v0.2＝全链一致性审计消账；v0.1＝首版草案） |
| 日期 | 2026-09-17 |
...
**本文档结束**（io 单元详细设计 v0.8，状态 `Draft`；构建落位进度：骨架随 IO-T01、SafePath/BudgetGuard 随 IO-T02、CSV 读写器随 IO-T03、JSON 读写器＋ZIP 通道随 IO-T04、资源导入服务与外部源检测随 IO-T05 落位完成；P-IO-3 选型冻结随 v0.6 登记。）
