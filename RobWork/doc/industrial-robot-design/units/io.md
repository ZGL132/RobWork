...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.9（IO-T06 落位同步；v0.8＝IO-T05 落位同步；v0.7＝IO-T04 落位同步＋JSON-SYNTAX 码追加；v0.6＝P-IO-3 选型冻结登记；v0.5＝IO-T03 落位同步；v0.4＝IO-T02 落位同步；v0.3＝IO-T01 落位同步；v0.2＝全链一致性审计消账；v0.1＝首版草案） |
| 日期 | 2026-09-17 |
...
| io 构建骨架 | **已落位**（IO-T01≙WP-11-T02，2026-09-17 实测）：`industrialrobot/io/CMakeLists.txt`（新建，`sdurws_ird_io` STATIC、C++17、PUBLIC 链 `sdurws_ird_core`＋`sdurws_ird_diagnostics`——§3.3 两条登记边；L1 文件/XML 目标按 P-IO-3 冻结后由 IO-T04 登记）；公共头 `IoFwd.hpp`/`IoError.hpp`（§3.1 表前两行，类型契约头）；`src/Io.cpp` 锚点翻译单元（零接口实现预建）。**SafePath/BudgetGuard 已落位**（IO-T02≙WP-11-T03，2026-09-17 实测，见 §15.5 v0.4）：`SafePath.hpp/.cpp`、`Budget.hpp/.cpp`、`IoDiagnostics.hpp/.cpp`（§3.1 表对应行）、`sdurws_ird_io_test` 随 IO-T02 以真实用例登记（契约 verify 目标；`_contract_test` 仍随 IO-T06/T07）；`src/Io.cpp` 仍为锚点翻译单元。**CSV 读写器已落位**（IO-T03≙WP-11-T04，2026-09-17 实测，见 §15.5 v0.5）：`Csv.hpp/.cpp`（§3.1 表对应行，内容清单按实现刷新——方言标识＋可逆编码唯一转义/还原实现）、`IoDiagnostics` 增补逐行列定位助手 `makeRowColError`（IO-T02 头注"随首个消费者 CSV 通道落位"的兑现）、`sdurws_ird_io_test` 增列 `test/CsvTest.cpp`（20 用例）。**JSON 读写器＋ZIP 通道已落位**（IO-T04≙WP-11-T05，2026-09-17 实测，见 §15.5 v0.7）：`Json.hpp/.cpp`、`ZipChannel.hpp/.cpp`（§3.1 表对应行）；L1 目标链接登记落位（libzip＋expat）。**资源导入服务与外部源检测已落位**（IO-T05≙WP-11-T06，2026-09-17 实测，见 §15.5 v0.8）：`ResourceIo.hpp/.cpp`（§3.1 表对应行——IResourceReader/IResourceSnapshotter/IProjectBytesSource/IExternalRefSource/IRuntimeResourceAdapter；expat 消费点）、`IoError.hpp` 表尾追加 `IO-FORMAT-XML-SYNTAX`（第 59 值）、`sdurws_ird_io_test` 增列 `test/ResourceIoTest.cpp`/`test/ResourceSolidifyTest.cpp`（IoRes/IoSolidify 组 27 用例）。**包导入导出 io 侧设施＋固化中转已落位**（IO-T06≙WP-11-T05/T07，2026-09-17 实测，见 §15.5 v0.9）：`Package.hpp/.cpp`（§3.1 表对应行——PackFormat/ISnapshotFileSource/PackageImportOptions·Session·Report/IPackageImporter/PackageExportOptions·Report/IPackageExporter/PackageIoFacilities；libzip 写侧消费点）、`TempArea.hpp/.cpp`（§3.1 表对应行）、`AtomicFile.hpp/.cpp`（§3.1 表对应行）、`ZipEntryInfo` 表尾增补条目属性两字段、`sdurws_ird_io_contract_test` 随本任务以真实用例登记（§3.3——contract_test/PackageContractTest.cpp 8 用例）、`sdurws_ird_io_test` 增列 `test/PackageTest.cpp`（IoPack/IoTemp/IoAtomic 组 14 用例） | 原 INTERFACE 占位骨架（仅 README.md 占位）已按 §3.3/§12 IO-T01 行升级为真实库；P-IO-3 候选可用性验证留痕见 `traceability/io-pio3-dependency-probe.md`（2026-09-17）；IO-T02 实施留痕见 `traceability/builds/wp11-t02/`（2026-09-17）；IO-T03 实施留痕见 `traceability/builds/wp11-t03/`（2026-09-17）；IO-T04 实施留痕见 `traceability/builds/wp11-t04/`（2026-09-17）；IO-T05 实施留痕见 `traceability/builds/wp11-t05/`（2026-09-17）；IO-T06 实施留痕见 `traceability/builds/wp11-t06/`（2026-09-17） |
...
| `Csv.hpp` | `CsvEol`、`CsvEncoding`、`CsvDialect`（含 `rwDefault`）、转义原语 `escapeCsvText`/`unescapeCsvText`/`renderDialectMarker`（唯一转义/还原/标识渲染——§5.1/§5.3）、`CsvCell`、`CsvArityPolicy`、`CsvBlankPolicy`、`CsvReadOptions`、`CsvRowView`、`CsvRowError`、`CsvParseReport`、`RawTable`、`CsvOutputTarget`、`CsvWriteOptions`、`ICsvReader`、`ICsvWriter`、工厂 `makeCsvReader`/`makeCsvWriter`（v0.5 随 IO-T03 落位按实现刷新——§15.5 v0.5） | §5、§9.3/§9.4 |
| `Json.hpp` | `JsonSourceSpan`、`JsonValue`/`JsonMember`/`JsonDocument`（受限 DOM——对象键保文件出现序、每值位置区间）、`JsonParseReport`、`JsonValueType`/`JsonProperty`/`JsonShape`/`JsonUnknownKeyPolicy`/`JsonProfile`、`JsonProfileRegistry`（C-6 注册协议）、`JsonReadOptions`/`JsonWriteOptions`/`JsonOutputTarget`、`IStructuredDataReader`、`IJsonWriter`、canonical 工具 `canonicalizeJson`/`digestCanonicalJson`、工厂 `makeStructuredDataReader`/`makeJsonWriter`（v0.7 随 IO-T04 落位按实现刷新） | §5.9、§9.5 |
| `ZipChannel.hpp` | `ZipEntryInfo`、`ZipManifestEntry`、`ZipEntryVerifyReport`、`IZipChannel`（条目枚举/逐字节还原/manifest 哈希校验）、工厂 `openZipChannel`（v0.7 随 IO-T04 落位登记——§3.4 布局 `ZipChannel.cpp` 的同名公共头，文件名等价增补见 §15.5 v0.7；实现细节 libzip 不出公共面） | §7.1、§9.5 |
...
| 库目标 | `sdurws_ird_io`（骨架 INTERFACE 占位 → IO-T01 升级 STATIC），别名 `RWS::ird::io`；链接 `sdurws_ird_core`＋`sdurws_ird_diagnostics`＋L1 文件/XML 目标（**v0.7 随 IO-T04 登记落地：`libzip::zip`（1.11.4）＋`expat::expat`（2.8.3）经 vcpkg x64-windows 经典模式 PRIVATE 链接——P-IO-3 冻结裁决（§15.3/governance-log §1.9）；libzip 消费＝ZipChannel.cpp，expat 消费＝ResourceIo.cpp（IO-T05），链接面按 §15.3 R-IO-1"登记随 IO-T04"先行落位**）；ird_gates LIB 词表扩登（libzip::zip/expat::expat）随 F-191 归门禁所有者处置；**零 Qt、零 Widgets** |
| 测试目标 | `sdurws_ird_io_test`（单元/黄金；随 IO-T02 以真实用例登记、随 IO-T03~T06 增列）、`sdurws_ird_io_contract_test`（契约/故障注入；**已随 IO-T06 以真实用例登记**——contract_test/PackageContractTest.cpp，§3.4 布局 contract_test/ 目录）——googletest 经 vcpkg `find_package(GTest CONFIG REQUIRED)`（development-task-breakdown §5.5），不预建空目标 |
...
| 格式-JSON | `IO-FORMAT-JSON-ENCODING/-DUPKEY/-NUMBER/-VERSION-MISSING/-VERSION-TYPE/-VERSION-FUTURE/-VERSION-LEGACY/-UNKNOWN/-REQUIRED/-TYPE/-RANGE`；`IO-FORMAT-JSON-SYNTAX`（v0.7 表尾追加——非数值类语法违例：截断文档/缺失分隔符/非法记号/悬空代理对等；数值字面量语法非法仍归 `-NUMBER`，NaN/Infinity 具名记号归 `-NUMBER`） |
| 格式-包 | `IO-FORMAT-PACK-ZIP/-ENCRYPTED/-ENTRY/-MANIFEST` |
| 格式-XML/网格 | `IO-FORMAT-XML-CYCLE`；`IO-FORMAT-XML-SYNTAX`（v0.8 表尾追加——§6.1 XML 良构检查的解析器级失败〔标签不闭合/属性语法错等〕，定位行列；循环仍归 `-CYCLE`，互不混用）；`IO-FORMAT-MESH-UNKNOWN` |
...
§11.2 覆盖任务清单 28 项：CSV roundtrip（V01~03）、BOM/编码/引号/换行（V04）、列异常（V05）、JSON schema/未知/未来（V07）、NaN/超长/深度（V08）、穿越（V09）、UNC/大小写/多路径（V10）、symlink/junction（V11）、炸弹（V12）、重复条目（V13）、哈希（V14）、循环（V15）、复制期变化（V17）、导出并发写（V18/V32）、导入取消（V19）、磁盘不足（V20）、清理失败（V21）、失败不建目标（V22/V23）、只读（V24）、权限（V25）、缺失（V26）、摘要稳定（V27）、重关联新修订（V28）、引用完整性（V29）、脱敏（V30）、后台不阻塞（V31）、预算超限（V06/V16）。

**覆盖核对登记（IO-T07，2026-09-18 实测执行）**：上列 28 项逐项对应用例编号可查如下（gtest 全名；结果＝双模式〔集成＋独立冒烟〕实跑一致；留痕 `traceability/builds/wp11-t07/`——gtest XML＋ird-test-report.json〔用例级明细经 testkit TestRecordListener，需求/AT 追溯字段经 IRD_TEST_INFO 逐用例登记〕＋任务级汇总报告）。无跳过用例；任何未执行用例不得标注通过（§11.1 通过判定行）。

| §11.4 项 | V 行 | 用例编号（gtest 全名；同 V 行多用例＝分解断言面） | 结果 |
| --- | --- | --- | --- |
| CSV roundtrip | V01 | IoCsvTest.RoundtripBasicQuotesDelimsNewlineChineseEmpty | passed×2 模式 |
| 同上 | V02 | IoCsvTest.RoundtripPrefixedEscapeOnlyAtFileLayer | passed×2 模式 |
| 同上 | V03 | IoCsvTest.UnmarkedNoRewriteKeepsAllFieldsVerbatim | passed×2 模式 |
| BOM/编码/引号/换行 | V04 | IoCsvTest.BomStrippingAndUtf16Decoding；IoCsvTest.EolToleranceAndQuotedNewlineNesting；IoCsvTest.NonUtf8WithoutBomStableReject | passed×2 模式 |
| 列异常 | V05 | IoCsvTest.DuplicateHeaderColumnsRejectedWithLocation；IoCsvTest.ArityPoliciesDefaultRejectAndExplicitPadTrim；IoCsvTest.BlankRowPoliciesSkipCountAndReject | passed×2 模式 |
| 预算超限（CSV 行维） | V06 | BudgetTest.StreamBudgetRowsOverBudgetTriggersIoSecBudgetRowsWithBoundedMemory；IoCsvTest.RowBudgetChargedThroughReaderChannel | passed×2 模式 |
| JSON schema/未知/未来 | V07 | IoJsonTest.SchemaUnknownKeyRejectedWithLocation；IoJsonTest.SchemaVersion999FutureRejectWithUpgradeData；IoJsonTest.DuplicateKeyRejectedWithLocationAndNestingScoping；IoJsonTest.RequiredMissingLocatedAndNoDefaultInjection | passed×2 模式 |
| NaN/超长/深度 | V08 | IoJsonTest.NanInfLiteralsRejectedAsNumber；IoJsonTest.NumberOverflowRejectedUnderflowAccepted；IoJsonTest.DeepNestingTriggersDepthBudget；IoJsonTest.OversizedStringTriggersStringBudget | passed×2 模式 |
| 穿越 | V09 | IoSecTest.PathTraversalP4IllegalTableRejectedWithExactCodes；IoSecTest.PathTraversalP5IllegalTableRejectedWithExactCodes（合法行放行＝PathTraversalP4/P5LegalRows 两用例） | passed×2 模式 |
| UNC/大小写/多路径 | V10 | IoSecTest.SameProjectAltPathsEquivalentKeysAndSessionMutex | passed×2 模式 |
| symlink/junction | V11 | IoSecTest.SymlinkJunctionEscapeRejectedWithIoSecSymlink | passed×2 模式 |
| 炸弹 | V12 | IoPackTest.ZipBombRatioAbortsAndCleansStaging；IoPackTest.ZipBombOverExpandAbortsOnBudgetExpand | passed×2 模式 |
| 重复条目 | V13 | IoPackTest.DuplicateEntriesRejectedBeforeExtraction；IoPackTest.CaseDifferingEntryPairRejectedAsDuplicate | passed×2 模式 |
| 哈希 | V14 | IoPackTest.TamperedPayloadHashMismatchLocatedPerEntry；IoZipTest.ManifestHashVerifyOkMismatchLocatedAndRefIncomplete（容器层支撑） | passed×2 模式 |
| 循环 | V15 | IoResTest.CyclicIncludeReportsCycleWithPathList；IoResTest.SelfIncludeReportsCycle；IoResTest.IncludeChainBeyondDepthTriggersIncludeBudget | passed×2 模式 |
| 网格预算 | V16 | IoResTest.BinaryStlDeclaredFacesLyingRejectedBeforeBodyRead；IoResTest.AsciiStlRealOversizeTriggersMeshBudgetMidRead | passed×2 模式 |
| 复制期变化 | V17 | IoSolidifyTest.SourceChangedDuringCopyDetectedAndStagingCleaned | passed×2 模式 |
| 导出并发写 | V18 | IoPackContractTest.ExportConcurrentWriteRereadsStableVersion；IoPackContractTest.ExportConcurrentWritePersistentFailureLeavesNoTarget | passed×2 模式 |
| 导入取消 | V19 | IoPackTest.ImportCancelMidExtractCleansAndEmitsNoDiagnostic；IoPackTest.ImportCancelAtBeginPreCancelledTokenLeavesNoArtifacts（F-199 消账——begin 入口预取消） | passed×2 模式 |
| 磁盘不足 | V20 | IoPackContractTest.DiskFullReportsRequiredAvailableAndCleansUp | passed×2 模式 |
| 清理失败 | V21 | IoPackContractTest.CleanupFailedListsResiduesAndRetrySucceeds | passed×2 模式 |
| 失败不建目标 | V22 | IoPackContractTest.FailureScenariosNeverCreateTargetDirectory（V12~V14/V19~V21 全失败分支聚合负断言） | passed×2 模式 |
| 同上（导出面） | V23 | IoPackContractTest.ExportCommitFailureLeavesNoTargetFile；IoPackContractTest.ExportFailurePreservesPreviousCompletePack | passed×2 模式 |
| 只读 | V24 | IoSolidifyTest.ReadonlyTargetWritesMapToReadonlyCode | passed×2 模式 |
| 权限 | V25 | IoResTest.AclDeniedReadMapsToAccessDeniedNotNotFound | passed×2 模式 |
| 缺失 | V26 | IoResTest.DependencyTreeMissingLeafReportsMissingList；IoSolidifyTest.ProbeReportsMissingForDeletedExternalFile | passed×2 模式 |
| 摘要稳定 | V27 | IoResTest.SnapshotDigestStableAcross100Reads；IoResTest.RenameDoesNotChangeDigestPathIsNotIdentity | passed×2 模式 |
| 重关联新修订 | V28 | IoSolidifyTest.ReassociateNewRevisionProjectCommandFakeSequence（O-09 对端＝project 命令 fake） | passed×2 模式 |
| 引用完整性 | V29 | IoPackTest.ManifestReferencingMissingArchiveEntryRejected；IoPackTest.MissingRequiredMirrorEntryRejected | passed×2 模式 |
| 脱敏 | V30 | IoDiagSanitization.UserTierLocatesWithoutFullPathOrRawSnippetDevKeepsSnippet（O-09 对端＝diagnostics sink fake） | passed×2 模式 |
| 后台不阻塞 | V31 | IoPackContractTest.BackgroundImportKeepsUiThreadResponsive（对端＝驱动线程 fake） | passed×2 模式 |
| 已提交对暂存 | V32 | IoPackContractTest.CommittedVsStagedOnlySnapshotEntriesPacked | passed×2 模式 |

P-IO-2 专用固化用例（§8.6 五条裁决——acceptance 侧处置）：IoSolidifyTest.AdapterResourceBytesLifecycleFiveRulingsPinned（第 1 条最低/实际保证、第 4 条 Recorded 独立缓冲与不缓存＋Solidified 同指针缓存；第 2/5 条＝纪律/契约登记注释口径，第 3 条＝AdapterConcurrentRecordedReadsAreSafe 专用并发用例）。用例跳过：无（本套件零 GTEST_SKIP）。用例总数：sdurws_ird_io_test 120＋sdurws_ird_io_contract_test 9（组名族 IoSec/IoCsv/IoJson/IoRes/IoPack 前缀系——IoZip/IoSolidify/IoTemp/IoAtomic/IoPackContract/IoDiag 为其分解面）。
...
| v0.7 | 2026-09-17 | IO-T04（≙WP-11-T05）落位同步：①JSON 读写器实现落位——`Json.hpp/.cpp`（§3.1 表 Json.hpp 行内容清单按实现刷新：`JsonSourceSpan`/`JsonValue`·`JsonMember`·`JsonDocument`〔受限 DOM——对象键保文件出现序、每值位置区间〕/`JsonParseReport`/`JsonValueType`·`JsonProperty`·`JsonShape`·`JsonUnknownKeyPolicy`·`JsonProfile`/`JsonProfileRegistry`〔C-6 注册协议——装配期注册、io 执行校验〕/`JsonReadOptions`·`JsonWriteOptions`·`JsonOutputTarget`/`IStructuredDataReader`/`IJsonWriter`/canonical 工具 `canonicalizeJson`·`digestCanonicalJson`〔摘要经 core ContentDigester——SA-12 唯一算法〕/工厂）；②ZIP 通道实现落位——`ZipChannel.hpp/.cpp`（§3.1 新增 ZipChannel.hpp 行＝§3.4 布局同名任务文件的公共头——文件名等价增补按 DTB §5.4 在此登记；`IZipChannel` 三方法＋`openZipChannel`，§7.1 容器层约束——加密拒绝/压缩方法白名单 STORED·DEFLATE/manifest 条目 SHA-256 校验〔IO-PACK-HASH-MISMATCH 定位、V14 前置能力〕，九步导入协议语义仍归 IO-T06）；③§9.12 JSON 族表尾追加 `IO-FORMAT-JSON-SYNTAX`（IoErrorCode 第 58 值——§5.9.1/§5.9.4"语法层拒绝"语义在非数值类语法违例上的码面缺口补登，数值字面量语法非法仍归 `-NUMBER`；IoError.hpp 枚举尾追加＋ioCodeDescriptors 同步注册，IoDiagnosticsTest 双清单机械比对同步）；④§3.3 库目标行——L1 文件/XML 目标链接登记落地（`libzip::zip`＋`expat::expat` PRIVATE，P-IO-3 冻结裁决的执行面）；⑤§9.5 等价调整两项登记（§9.0 条款覆盖）：parse/parseBytes 增补尾随参数 `BudgetScopeId budgetScope = {}`（§9.3 CSV 同款——调用方收紧规格传导读取会话）与写出目标 `IOutputTarget&&` → 实现 `JsonOutputTarget&&`（io 自有承载类型，§4.6 原子写出同 CSV 口径）。设计正文语义零变更。双模式构建零错误＋测试 75/75 双模式通过（IoJsonTest 26 用例＝V07/V08＋canonical/摘要＋版本门＋注册表契约；IoZipTest 9 用例＝逐字节还原/哈希校验/加密·压缩方法·CRC 拒绝/预算·炸弹预检/取消——留痕 `traceability/builds/wp11-t04/`）；ird_gates 14 处命中＝基线 11 处＋F-191 登记的 3 处 LIB 词表待扩（libzip::zip/expat::expat——依赖本体已随 P-IO-3 冻结裁决登记，词表扩登归门禁所有者）。 |
| v0.8 | 2026-09-17 | IO-T05（≙WP-11-T06）落位同步：①资源导入服务与外部源检测实现落位——`ResourceIo.hpp/.cpp`（§3.1 表 ResourceIo.hpp 行按实现刷新：`ResourceSnapshot`·`ResourceContentId`〔§8.2 逐字——digest 为内容身份、路径不作身份，mtime＝NT FILETIME 预筛〕/`ResourceKind`〔阶段 A 格式族，含 `GenericXml`＝§6.4 R2 预留通道的文件层形态〕/`ResourceStreamHandle`〔opaque PIMPL RAII 流式句柄〕/`ResourceOpenSpec`〔P-1/P-5/P-6〕/`ResourceDependencyTree`〔节点/边/稳定序——折叠相对键字典序〕/`ExternalRefProbe` 事实三态（`ExternalRefState`·`ExternalRefRecord`·`ProbeDetail`）/`SolidifyStagingResult`/`IResourceReader`/`IResourceSnapshotter`/`IProjectBytesSource`·`IExternalRefSource`·`IRuntimeResourceAdapter`〔§9.7 注入面〕＋工厂 `makeResourceReader`·`makeResourceSnapshotter`·`makeRuntimeResourceAdapter`；expat 消费点＝ResourceIo.cpp〔§3.3 登记行兑现〕）。实现要点：读取通道按 §4.4 防护流程②→⑦（SafePath 规范化→SingleFileBytes 预检先拒后开→Win32 打开〔P-5/P-6 带 FILE_FLAG_OPEN_REPARSE_POINT＋SP-6 final-path 复核，比较面归一 \\?\ 前缀/分隔符/折叠；P-1 跟随链接按实体路径记录〕→流式每块 TotalBytes＋取消合并检查点→core ContentDigester SHA-256）；网格规模护栏（§6.3：二进制 STL 声明面读前预检＋声明跨度补账＋实际大小反推复核、ASCII STL/OBJ 流式计数读中中止；识别失败不阻断字节资源快照——格式族门禁归消费通道）；依赖树显式栈 DFS＋出口标记（环＝回边指向进行中节点，与 DAG 共享正确分界；IncludeDepth 先比后展开；缺失叶汇总 IO-RES-MISSING 缺失清单；include 越界 ESCAPE；XML 良构 SYNTAX）；OS 错误 §4.2.5 四分类（写方向以 READONLY 属性判别；读方向不降级 NOT-FOUND）；固化执行（§8.4 步骤 1~6 io 执行侧：budgetBytes 取小的收紧子 scope、前后双快照、CREATE_ALWAYS 中转复制〔预删除刻意不做——MSVC filesystem::remove 静默清只读属性，写护栏在打开面，V24 依据〕、变化检出→清理中转→IO-RES-CHANGED 比较型诊断、成功副本保留待 project 入库——objects 零写入）；runtime 资源读取桥（§9.7 路由＋§8.6 五条裁决执行：Solidified 缓存命中同指针/Recorded 每次重读＋重算绝不缓存/缓冲稳定区至析构/并发只读安全/值拷贝兼容——P-RT-6·P-IO-2 处置，P-IO-1 注入面零 project/runtime 编译依赖）。②§9.12 XML/网格族表尾追加 `IO-FORMAT-XML-SYNTAX`（IoErrorCode 第 59 值——§6.1 XML 良构检查的解析器级失败码面缺口补登，IO-T04 同款先例；IoError.hpp 枚举尾追加＋ioCodeDescriptors 同步注册 57 条＋IoDiagnosticsTest 双清单机械比对同步）。③§9.6/§9.8 等价调整三项登记（§9.0 条款覆盖）：§9.6 snapshot/dependencyTree 与 §9.8 solidifyToStaging 增补尾随参数 `BudgetScopeId budgetScope = {}`（§9.3 CSV 同款——多阶段共享 ledger）；§9.8 probe 返回类型由裸 `ExternalRefState` 改为 `IoResult<ExternalRefState>`（资源事实三态走值轨道、探测过程取消/预算环境失败走错误轨道——事实与状态分轨，防取消误报为 Missing，§2.5 正交）；§9.6 dependencyTree `importRoot` 参数语义澄清＝入口文档（P-1），include 管辖根＝其父目录（§6.2"用户选择的文件所在目录为基解析"直接落点，不改调用形态）；solidifyToStaging `budgetBytes=0` 语义＝未声明调用方预算（按产品默认生效）。设计正文（§2~§14）语义零变更。双模式构建零错误＋测试 102/102 双模式通过（IoResTest 17 用例＝V15/V16/V25/V26/V27＋基础契约；IoSolidifyTest 10 用例＝V17/V24/probe 三态/三段边界/§8.6 五条——含 4 线程×8 并发读取与 Recorded 不缓存的真实失败能力用例；既有 75 用例零回归——留痕 `traceability/builds/wp11-t05/`）；ird_gates 14 处命中＝wp11-t04 基线全等（11 处登记例外＋F-191 的 3 处 LIB 词表待扩，零新增命中——留痕 gates-ird-gates-target.log）。 |
| v0.9 | 2026-09-17 | IO-T06（≙WP-11-T05/T07）落位同步：①包导入导出 io 侧设施与固化中转实现落位——`Package.hpp/.cpp`（§3.1 表 Package.hpp 行按实现刷新：`PackFormat`〔§7.1 契约常量单点——formatId/schemaVersion/manifest 版本/必备条目〕、`PackFileEntry`·`ISnapshotFileSource`〔§7.2 一致数据视图注入面——io 定义、project 实现并注入〕、`PackageImportOptions/Session/Report/State`〔§7.3 状态机机器可判形态——Staging/Extracting/Verified/Canceled/Failed/CleanupFailed/Cleaned；报告含条目级哈希结论＋结构化诊断＋账本快照观测面〕、`IPackageImporter`〔begin＝步骤①②／verifyThrough＝③~⑦／cleanup＝⑨，**无任何 rename/发布 API**——§7.7 责任切分、§2.1 N-7〕、`PackageExportOptions/Report`、`IPackageExporter`〔§7.2 六步：枚举→暂存→canonical 元数据→压缩→自检→原子替换〕、`PackageIoFacilities`＋工厂 makePackageImporter/makePackageExporter〔§9.11 落位前直接装配形态——测试经 facilities 注入 fake 适配层〕）、`TempArea.hpp/.cpp`（§3.1 表 TempArea.hpp 行：`TempAreaRole/Spec`〔PackImport/PackExport/Solidify 三角色命名前缀——§7.5 创建行〕、`TempAreaSession`〔值语义 pimpl〕、`ITempAreaManager`——等价键互斥（EquivKeyMutex 消费）＋io-session.json 标记的崩溃残留回收（仅清带标记残留——"绝不误删用户目录"）＋幂等清理与残留清单报告（§7.6 CleanupFailed 可重试））、`AtomicFile.hpp/.cpp`（§3.1 表 AtomicFile.hpp 行：`ReplacePolicy/AtomicTarget/IAtomicFileWriter`——§4.6 协议：同目录 `<target>.<8hex>.tmp` 暂存→FlushFileBuffers→ReplaceFileW/MoveFileExW 原子替换，commit 前目标不变）。②libzip 写侧消费点增登（§3.3——读侧消费点仍归 ZipChannel.cpp；导出建归档经 zip_source_win32w_create（Windows 宽路径可写源）＋zip_open_from_source(ZIP_CREATE)，条目文件名窄 UTF-8 经 libzip CP_UTF8→UTF-16 无损转换；filep_create 系只读 ops 不可用于创建——实测发现后更正）。③测试目标落位——`sdurws_ird_io_contract_test` 随本任务以真实用例登记（§3.3；contract_test/PackageContractTest.cpp，IoPackContract 组 8 用例＝V18/V20/V21/V23/V31/V32 故障注入——§11.1 fake 适配层经 PackageIoFacilities 注入，不依赖真实磁盘满/删除失败）；`sdurws_ird_io_test` 增列 `test/PackageTest.cpp`（IoPack/IoTemp/IoAtomic 组 14 用例＝导出导入 roundtrip＋V12 比例/超展开＋V13 同名与仅大小写异＋V14 条目级定位＋V19 各阶段取消＋V29 幽灵引用/必备缺失＋临时区/原子写出单元契约；全部失败样例带"目标零写入＋临时区清理"结构性断言——acceptance 4）。④`ZipEntryInfo` 表尾增补 `attributeHostSystem`/`externalAttributes` 两字段（§7.4 步骤③ SP-4 symlink 属性**展开前预检**的数据源——容器层此前不暴露条目属性；原始透传、解释权在导入器）。⑤私有头 `src/IoPlatform.hpp` 增登（UTF-8↔UTF-16/§4.2.5 四分类/display 呈现/8hex 会话标签的单一实现点——TempArea/AtomicFile/Package 消费，ResourceIo.cpp 本地副本收编为 using 声明——NFR-MNT-04）。⑥等价调整与增补登记（§9.0 条款覆盖，五项）：§9.9 verifyThrough 增补尾随参数 cancel/progress（九步"逐步可取消"需令牌在③~⑤检查点生效——begin 同形态对称）；§9.9 PackageExportOptions 增补 sourceProjectId/headRevisionId/createdAtUtcIso8601/createdWithToolVersion 四字段（§7.1 rwpack.json 契约元数据的来源——快照源接口仅 path+size、io 不得扫描源项目目录，由 project PackageService 传入；空 createdAtUtc＝取当前 UTC——IO-D11 记录位）；§9.10 ITempAreaManager 增补 availableBytes(session)（§7.3④"磁盘可用空间检查"的注入面——真实实现＝fs::space(root).available，V20 fake 覆写；提示性探测，写入失败 IO-PACK-DISK-FULL 兜底仍为防线之二）；AtomicTarget 增补 write(std::string_view)（§9.10 协议"[写入]"步骤的通道——分块驱动，大包内存有界）与 adopt/impl pimpl 通道（TempAreaSession/PackageImportSession 同款——管理器/导入器实现装配底层会话的唯一入口，m_impl 保持私有）；§7.2②"按快照重读一次"落位＝单文件 read 失败重试恰一次（稳定版本语义/V18）。设计正文（§2~§14）语义零变更。双模式构建零错误＋测试 124/124 双模式通过（sdurws_ird_io_test 116＝既有 102 零回归＋IoPack/IoTemp/IoAtomic 14 新增；sdurws_ird_io_contract_test 8＝IoPackContract 8——V12~V14/V18~V23/V29/V31/V32 对应面全绿——留痕 `traceability/builds/wp11-t06/`）；ird_gates 15 处命中＝wp11-t05 基线 14＋1 处新增（sdurws_ird_io_contract_test→libzip::zip——F-191 同类，已自登记 F-197；F-194 同步消账——ResourceIo.cpp 显式补 `<mutex>`）；knownPitfalls 四项处置：P-IO-1/O-09＝ISnapshotFileSource/发布入口以 project fake 注入（公共头零 project/runtime 类型与头依赖——不私裁补边，裁决申请维持登记在途）；P-IO-2＝IRuntimeResourceAdapter 按 §8.6 五条裁决实现（IO-T05 已落位——IoSolidifyTest 既有用例承载，本任务零新码）；P-IO-7＝§7.8 目录包文件名/清单 schema 未预写（本任务仅承载 .rwpack 协议——selection 卡注册归阶段 C）。 |
| v1.0 | 2026-09-18 | IO-T07（≙WP-11-T07）落位同步：契约测试套件整备与留痕——**§11.2 全矩阵 32 行可执行且结果逐条登记**。①§11.4 覆盖核对登记落位（本卡 §11.4"28 项逐项对应用例编号可查"）：覆盖核对表 32 行（V01~V32 全行→gtest 用例全名→双模式实跑结果），28 项逐项可查；V22/V28/V30 缺口补齐（V28＝IoSolidifyTest.ReassociateNewRevisionProjectCommandFakeSequence——project 命令 fake 记录调用序列＋io 不产生修订负断言；V30＝IoDiagSanitization.UserTierLocatesWithoutFullPathOrRawSnippetDevKeepsSnippet——diagnostics sink fake 双级行槽＋真实 RedactionService 脱敏〔snippet 为登记的开发级参数，Tier-U 不渲染〕；V22＝IoPackContractTest.FailureScenariosNeverCreateTargetDirectory——V12~V14/V19~V21 全失败分支聚合"目标目录从未创建"负断言）；P-IO-2 专用固化用例＝IoSolidifyTest.AdapterResourceBytesLifecycleFiveRulingsPinned（§8.6 五条逐条断言面——第 1 条最低/实际保证、第 4 条 Recorded 独立缓冲＋不缓存＋Solidified 同指针缓存；第 3 条既有并发用例承载，第 2/5 条纪律/契约注释登记）。②testkit 报告设施接入（PRJ-T15 同款先例——T-1 允许形态：仅测试目标链 sdurws_ird_testkit，产品目标零 testkit 边）：双自有 main（test/TestMainReport.cpp＋contract_test/ContractTestMain.cpp）安装 TestRecordListener，ird-test-report.json 用例级明细与 gtest XML 并存（testkit §7.2/§7.3）；§11.2 全部 V 行用例以 IRD_TEST_INFO 携带需求/AT 追溯字段（AT-02→V01~V05/V30、AT-20→V12~V14/V17~V23/V28、AT-34→V31 迁注落位）。③F-199/F-198 消账：IoPackTest.ImportCancelAtBeginPreCancelledTokenLeavesNoArtifacts（V19 覆盖深度——begin 入口预取消，无会话产物/临时区零残留/目标零写入）；writeBytesToFile 与 AtomicTarget::write 前置契约注释钉死（≤SingleFileBytes 硬上限 2 GiB——F-198 处置②口径）。④设计正文（§2~§14）语义零变更；§11.4 状态行由"全部为设计，未执行"更新为实测执行登记（本条即登记依据）。双模式构建零错误＋测试 129/129 双模式一致通过（sdurws_ird_io_test 120＝既有 116 零回归＋V28/V30/P-IO-2/F-199 四用例；sdurws_ird_io_contract_test 9＝既有 8 零回归＋V22 聚合用例——留痕 traceability/builds/wp11-t07/：gtest XML×4＋ird-test-report 用例级明细×4＋任务级汇总 ird-test-report.json＋双模式构建/测试/验证/门禁日志）；ird_gates 17 处命中＝wp11-t06 基线 15＋2 处新增（io 两测试目标→sdurws_ird_testkit 的 SUB 命中——任务契约明文的 testkit 消费链接边，DTB §4.5 既有例外行同型实例，已自登记 F-202，例外实例/词表扩登归门禁所有者）；knownPitfalls 处置：P-IO-2（本卡 §15.3 已裁决——§8.6 五条以专用用例固化）、O-09（V28/V30/V31 对端以 project 命令 fake/diagnostics sink fake/驱动线程 fake 承载——io 零 project/runtime 编译依赖不补边，裁决申请维持 IO-T06/PRJ-T16 联动在途）。 |

---

**本文档结束**（io 单元详细设计 v1.0，状态 `Draft`；构建落位进度：骨架随 IO-T01、SafePath/BudgetGuard 随 IO-T02、CSV 读写器随 IO-T03、JSON 读写器＋ZIP 通道随 IO-T04、资源导入服务与外部源检测随 IO-T05、包导入导出 io 侧设施＋固化中转随 IO-T06、契约测试套件整备与留痕随 IO-T07 落位完成——阶段 A 六任务全闭环；P-IO-3 选型冻结随 v0.6 登记。）
