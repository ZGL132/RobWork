...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v1.1（EV-T10 评估器接口与注册表实现落位登记；v1.0＝EV-T09 缓存/检查点兼容判定实现落位登记；v0.9＝EV-T08 当前性纯投影实现落位登记；v0.8＝EV-T07 结果包络实现落位登记；v0.7＝EV-T06 五级汇总判定实现落位登记；v0.6＝EV-T05 证据与 Profile 承载实现落位登记；v0.5＝EV-T04 切片与身份实现落位登记；v0.4＝EV-T03 快照实现落位登记＋卡头版本链补全——F-057 消账；v0.3＝EV-T02 错误与依赖类型落位；v0.2＝FOUNDATION-CR-01 契约审查修正；v0.1＝首版草案；各版变更记录见 §15.4） |
| 日期 | 2026-09-12 |
