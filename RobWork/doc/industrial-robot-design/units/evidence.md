...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v1.3（EV-T12 文档与门禁同步——§15.3 九项状态实测同步＋P-EV-6 就地消账〔runtime P-RT-9 对账〕＋README 零偏差登记；v1.2＝EV-T11 测试替身与契约套件实现落位登记；v1.1＝EV-T10 评估器接口与注册表实现落位登记；v1.0＝EV-T09 缓存/检查点兼容判定实现落位登记；v0.9＝EV-T08 当前性纯投影实现落位登记；v0.8＝EV-T07 结果包络实现落位登记；v0.7＝EV-T06 五级汇总判定实现落位登记；v0.6＝EV-T05 证据与 Profile 承载实现落位登记；v0.5＝EV-T04 切片与身份实现落位登记；v0.4＝EV-T03 快照实现落位登记＋卡头版本链补全——F-057 消账；v0.3＝EV-T02 错误与依赖类型落位；v0.2＝FOUNDATION-CR-01 契约审查修正；v0.1＝首版草案；各版变更记录见 §15.4） |
| 日期 | 2026-09-13 |
...
测试目标 `sdurws_ird_evidence_test`（单元内）与 `sdurws_ird_evidence_contract_test`（跨单元）。**阶段 A 矩阵用例已全部实现并通过并留痕**（EV-T02～T11 逐任务交付；用例→文件落位映射见 `evidence/test/README.md` §3，验证留痕见各任务 `traceability/builds/wp05-tnn/`）。测试设施：testkit 断言/夹具（ContractCheck 谓词消费本文类型——envelope 访问器特化即 testkit §10.2 交接的承接，EV-T11 交付于 `_contract_test`）；**可控评估器替身**（`ScriptedEvaluator`：按脚本返回预设证据/证明/搜索未果/取消抛出，注册于测试内 registry——EV-T11 交付，`evidence/test/EvidenceTestDoubles.hpp`）——替身只验证 evidence 契约与汇总逻辑，**其输出不构成任何业务算法正确性证明**（任务约束§八；自审项 A-9；替身边界声明全文见 `evidence/test/README.md` §1）。
...
| # | 问题 | 依据 | 影响 | 建议 | 需要裁决者 |
| --- | --- | --- | --- | --- | --- |
| P-EV-1 | 本文消费的 core 契约以其 v0.1（Draft）为基线；冻结版可能调整签名/字段（含 P-D-1 词表位置、P-AR-3 证据等级两项 core 侧待裁决的 evidence 侧确认——本文 §3.4 已按"词表归 core、等级＝模式效力"回复） | core.md 文档头状态行、其 §10.2/§10.3 | EV-T02 起返工 | core 冻结时出 diff 清单，本文按影响面增量修订并留痕。**〔EV-T12 状态同步 2026-09-13〕仍 open（冻结未发生）**：core.md 实测 v0.11（Draft-Structured，未冻结）；其 §15.3 P-D-1/P-AR-3 已于 2026-09-11 经 CORE-T10 五卡交叉核对**关闭**（本文 §3.4 回复被引为消账证据）——内嵌两个 core 侧子项消账，主项（冻结 diff 增量同步）维持；本文消费面由 evidence 两测试目标 219 例两模式实跑钉住无漂移（builds/wp05-t12/） | core 详设所有者＋本文所有者 |
| P-EV-2 | ARCHITECTURE v0.11 Draft 待评审；本文以其为基线 | ARCH 文档头 | 评审结论可能要求同步（尤其 §7.6/§7.2③） | 评审后按影响面增量修订。**〔EV-T12 状态同步 2026-09-13〕仍 open（评审未完成）**：ARCHITECTURE 实测 v0.12（"全链一致性审计事实修正"，仍 Draft 待评审）——v0.11→v0.12 属事实修正，本文 §3/§7/§9 锚点经本任务核对零返工面；评审结论产出后按影响面增量修订 | 架构所有者 |
| P-EV-3 | 跨域汇总顺序：任务级不可行证明（域 A，③）与他域证据缺失（域 B，④）并存时，本文按表 2 字面顺序（③ 先于 ④）输出整体 EngineeringInfeasible；"替代豁免仅限因不可行无法生成的成功产物"（C6）在跨域场景的边界（B 的缺失若与不可行无关）未明文 | §8.1 表 2 ③④、C6 | 整体判定的验收解释可能分歧 | 维持字面顺序（证明为任务级作用域，先命中先出）；如需"B 类无关缺失阻断整体"语义，走需求变更。**〔EV-T12 状态同步 2026-09-13〕仍 open（需求所有者未裁决，governance-log 实测 open，O-13 同源）**：实现侧已按字面顺序落地——aggregateVerdict ③命中即输出整体判定、④/⑤不再评估（EV-T06 落位登记 §15.4 v0.7，VerdictPitfalls 用例钉住），裁决前行为可预测且保守 | 需求所有者 |
| P-EV-4 | 当前性"不可判定"（依赖无法解析/资源缺失/跨上下文）的呈现：上游仅定义 Current/Superseded 两态，本文以 status=空＋诊断表达、不新增持久第三状态；UX-10"结果过期"与 PM-11"是否过期"的 UI 呈现口径需与之对齐 | CON-02/05、UX-10、PM-11 | ui 投影措辞与状态呈现 | 维持两持久态＋不可判定诊断；ui 详设产出时引用本条定呈现。**〔EV-T12 状态同步 2026-09-13〕仍 open（需求侧确认缺位）**：ui 详设已产出（units/ui.md）并以 **P-UI-2** 承接本条——建议呈现＝归入 results-stale＋原因区显示"当前性无法判定（依赖缺失/上下文变化）"，不显示为 Current、不显示为通过（与本文"无默认 Current"规则一致）；其冻结以需求侧确认为前提，本卡计算面维持不变 | ui 详设所有者＋需求侧确认 |
| P-EV-5 | 区域覆盖率参考值呈现：KIN-04"数据不足样本存在时覆盖率数值仍可计算（作参考值报告）但结论整体降级"——参考值与正式结论的分离呈现归 reporting/ui；本文仅承载 downgraded 标记 | KIN-04（R8） | 报告措辞（RPT-05 限定语） | reporting 详设引用本条；参考值必须携带"降级中"限定语。**〔EV-T12 状态同步 2026-09-13〕对端承接已落卡（裁决宣告归 reporting 所有者，本卡不代翻）**：reporting 详设已产出（units/reporting.md）并消费本条——§6.4 downgraded-reference-value 诊断（RegionCoverageEvidence.downgraded==true，KIN-04）＋§14 追踪 RP-COV-1，参考值措辞"降级中——不可作为正式覆盖率结论"；本卡仅承载 downgraded 标记的分工不变 | reporting 详设所有者 |
...
| v0.3 | 2026-09-12 | EV-T02 实现落位登记（DTB §5.4，错误与依赖类型＝`Errors.hpp/.cpp`＋`Dependency.hpp`，按 §12 EV-T02 行交付；验证留痕 traceability/builds/wp05-t02/）：①**EvidenceErrorCode 全表 9 值**（§3.1 组成表＋各抛错点原文承载——§3.5 缺位仍按 F-020 由所有者裁决）：7 值建议码与 §13 清单逐字对应，2 值为设计抛错点明文（§4.1.5④b snapshot-integrity、§4.2.3① 声明闭包拒绝）但 §13 未列建议码的**补登建议** EVI-SNAPSHOT-INTEGRITY/EVI-DECLARATION-INVALID（P-PR-6 同模式——建议值，码值权威归 StableCodeRegistry，登记 F-056 转 evidence 详设所有者＋diagnostics 所有者于注册时裁决）；token 命名取抛错点原文（snapshot-integrity/duplicate-evaluator）＋其余按 §13 建议码 kebab 派生，"evidence/" 前缀与 what() 消息约定同 runtime §3.4 先例；枚举/issue 码一经交付只允许表尾追加。②**Dependency.hpp（header-only，契约产物列无 .cpp）**：§4.2.1 七类 Kind 载荷值类型＋DependencyKey 语法闸门 `[a-z][a-z0-9.-]{2,63}`；实现侧 Kind 稳定名表 7 值（object/configuration/policy/name-map/sample-set/upstream-result/environment，kebab 化——canonical 编码单一权威序＝枚举声明序，EV-T04 SliceCodec 消费）；域 token（conditionToken/objectTypeToken/configKindToken/Environment 名值）只施加"非空＋无 NUL"编码安全下限（§5.2），不套用键字符集（词表归域，N-5）。③**校验器分轨**：声明闭包校验器（§4.2.3①——referencedKeys ⊆ 同 descriptor 已声明键 ∪ 快照事实键；事实键由注册方参数注入，evidence 不拥有其词表；另按声明语法收紧三条实现口径——同集键唯一拒绝、Conditional 必带条件/Required 必不带、引用键非空集）为非抛出纯函数＋require* fail-fast 原语（注册拒绝归 EvaluatorRegistry〔EV-T10〕消费）；条目语法校验器（§4.2.1 载荷行＋§4.2.2 (kind,key) 字典序稳定存储＋applied/notAppliedReason 配对 presence 语义）只交付非抛出轨——冻结拒绝的抛错时机与码面归 SliceBuilder（EV-T04）任务卡登记（§2.1 try* 分轨先例）。④测试组名 EV-ERR/EV-DEP 为实现侧组名（§11 矩阵未设 EV-T02 专项组，EV-BUILD 同例）；单元测试 22/22 两模式通过并留痕（EV-ERR-1～3＋EV-DEP-1～15，含 token 漂移探针失败能力自证） |
| v0.4 | 2026-09-12 | EV-T03 实现落位登记（DTB §5.4，快照＝`Snapshot.hpp/.cpp`——builder/校验/SnapshotCodec refs-only＋materialized，按 §12 EV-T03 行交付；验证留痕 traceability/builds/wp05-t03/）：①**值类型面**（§4.1.2 字段表全量承载）：ObjectRefEntry/ConfigEntry/PolicyRef/NameMapRef/CaseEntry＋RequiredCaseSet/SamplingPlanRef/ExternalResourceState（Recorded｜Solidified 两态＋presence 语义）/ReproductionBlock/AnalysisSnapshot（无 setter，冻结后按只读对待——builder 为唯一合法生产者）＋CaseId 语义别名（＝core::ObjectId，§4.1.3 原文）；§3.3 注入接口 IObjectBytesSource/IRevisionClosureSource 按原文签名落地（适配器归 L5/请求方）。②**SnapshotBuilder 冻结协议**（§4.1.5①～④a）：setter 纯累加、build() 单一确定性校验路径（校验顺序固定——同坏组装必报同一首错）；防混入＝全部 (oid,cv) 逐条经 IRevisionClosureSource 对锚定修订校验（混入反例→EvidenceError(SnapshotIncomplete)）；CON-06 策略/名称映射身份非空、工况 caseId 唯一、外部资源状态机 Recorded→Solidified 单向、复现块完整性（presence 语义）全部在 build() 即时验证。③**SnapshotCodec**（§5.2 规则表落点）：magic `IRDSNAP1`＋codec 版本＋形态字节；定宽大端/长度前缀/presence 字节/字符串禁 NUL；refs-only 与 materialized 双形态**同身份**（D-01——身份恒取 refs-only，§4.1.5⑤）；④b 载荷级校验（重算 SHA-256 比对 contentVersion）在物化（encodeMaterialized）与接收（parseMaterialized）双端执行，不符→EvidenceError(SnapshotIntegrity)；结构非法（截断/坏 magic/形态互混/版本不符/尾随字节）→SnapshotIncomplete。④**实现口径 I-1～I-7 登记**（实现细节精确化，语义与设计一致）：I-1 revisionSeq 不入编码与 snapshotId（§4.1.2"不参与内容身份语义判断"原文的机械落实——parse 恢复 0，往返等值断言排除该字段）；I-2 ObjectRefEntry.digest＝对象载荷字节 SHA-256 裸字节（与 contentVersion 同源——core §4.2），builder 校验二者一致（冗余承载的防错闸门）；I-3 builder 冻结时集合类成员规范化排序（closure/configurationRefs/caseSet/externalResources/samplingPlans，codecVersions 保序）＋同集重复键拒绝（objectId/configKindToken/resourceId/regionObjectId——EV-T02 声明键唯一同源口径）；I-4 requiredCaseSetId＝SHA-256 over "IRDCASE1"＋caseId 序条目规范串（"entries 规范编码摘要"的字节级定义，builder/parse 双端同函数）；I-5 编码含形态字节，snapshotId 恒按 form=0 计算；I-6 parse 结构非法→SnapshotIncomplete（带偏移）、载荷摘要不符→SnapshotIntegrity（④b 原文码面），双端④b；I-7 caseSet.entries 允许空集（P-EV-7 保守处置在汇总判定层，组装层不拒）、enabled/mandatory 字面承载不解释（O-14/P-EV-9 保守字面——契约 acceptance 3 编入）。⑤**范围边界**：§4.1.2"validateForMode(Verified) 的 Recorded 外部资源阻断"锚定 §6.2，其入口归证据清单任务 EV-T05，本任务未实现该函数（§4.1.2 原文指向关系不变）。⑥测试：新增 21 用例（EV-ID 3＋EV-SNAP 10＋EV-CASET 4＋EV-SNAPC 4，组名为实现侧组名——§11 矩阵 EV-T03 专项＝EV-ID-1/3＋混入反例，其余对照 §12 行"全部快照不变量用例"），43/43 两模式通过并留痕；防混入校验探针（短路注入→3 用例变红→恢复复绿）失败能力自证。⑦F-057 消账：卡头"文档版本/日期"随本次修订推进至 v0.4/2026-09-12 并补版本链；validate-docs 增加"卡头版本行＝§15.4 末行"机检维度的建议仍归治理脚本所有者（开放建议，非本卡辖域） |
| v0.5 | 2026-09-12 | EV-T04 实现落位登记（DTB §5.4，切片与身份＝`Slice.hpp/.cpp`——InputSlice/SliceBuilder/SliceCodec/双层身份 D-04，按 §12 EV-T04 行交付；验证留痕 traceability/builds/wp05-t04/）：①**InputSlice 值类型面**（§4.2.2 字段表全量承载）：evaluationKey/evaluatorContractVersion（进 sliceId——CON-04）＋entries（(kind,key) 字典序规范化存储，≥1）＋snapshotId（取自 build() 入参快照，非申报值）＋sliceId/inputBaselineId（builder 计算的双层身份）；§3.1 组成表概念名 SliceId/InputBaselineId 以语义别名落地（＝core::ContentIdentity——§4.2.2 原文类型，CaseId 同款先例，互不转换）；**实现口径 D-5**：evaluationKey 暂以 std::string 承载（isValidEvaluationKey 词形闸门＝§8.2 EvaluationKey 原文语法 `[a-z][a-z0-9-]{1,63}`——类型化归 Evaluator.hpp〔EV-T10〕，落地后按需替换）。②**SliceBuilder 冻结协议**（§4.2.3②）：setEvaluation/addEntry/setConsumesCanonicalModel 纯累加、build(snapshot) 单一确定性校验路径（校验顺序固定——评估键语法→快照冻结性→条目集排序→条目语法→Object 快照子集→CR-05 值形态→CR-05 必填；同坏组装必报同一首错）；**实现口径 D-4**：冻结时对 entries 规范化排序（"未排序"不是拒绝面，同内容任意添加序同身份），(kind,key) 重复/载荷非法/applied 配对矛盾仍拒绝；Object 条目对来源快照 objectClosure 做 (oid,cv) 精确子集校验（§4.2.2 防漏声明错配）；冻结拒绝码面＝EvidenceErrorCode **表尾追加 SliceIncomplete**（9→10 值，token evidence/slice-incomplete，建议码 EVI-SLICE-INCOMPLETE 补登——P-PR-6 同模式；ErrorsTest 全表锚定 9→10 值同步）。③**SliceCodec 双形态与双层身份**（§5.1/§5.2＋CR-02）：magic `IRDSLCE1`＋codec 版本＋形态字节（0=full〔sliceId 摘要对象、往返载体〕/1=baseline-projection〔inputBaselineId 摘要对象、非往返载体——parse 拒绝〕）；**实现口径 D-2**：双身份是编码的导出值不入编码载荷，摘要唯一经 core::ContentDigester（CR-02，单测以手工重算钉住）；canonical 浮点原语 canonicalF64/parseCanonicalF64（IEEE754 位模式大端 8 字节、NaN/±Inf 编码入口拒绝＋接收端非有限位型拒绝——runtime RT-Codec f64 双端纪律同款；§5.2 数值行在 evidence 侧的唯一实现点，EV-ID-2 的浮点面承载）。④**实现口径 D-1（基准投影参与集机械口径）**：进入＝全部 Object＋SampleSet＋NameMap＋Environment 基准类保留 token（CR-05 双条目，isBaselineEnvironmentToken 封闭白名单）＋快照身份块；排除＝Configuration（求解类——KIN-13/D-04）/Policy（§4.2.2"仅模型/需求/工况集/冻结样本集条目参与"＋§5.1 参与清单未列——策略属消费机制，经 sliceId 失效不构成输入基准差异）/UpstreamResult（同上未列）/Environment 非基准版本（§5.1 排除语）。Object 的"task/region/case-set/model 类角色"（§5.1 正向括注）机械化为**全量进入**——evidence 无域词表（N-5/N-9）按 key 前缀猜类即把域知识搬进 evidence；§5.1 排除语仅点名"求解类 Configuration 与 Environment 非基准版本"，Object 不在排除面；全量进入使碰撞几何/负载/目录等模型侧对象变化同样被 EVI-02 拦截（保守方向——宁可多算不可错复用）。⑤**CR-05 落地**：保留 token runtime.model-identity/runtime.robwork-baseline 常量＋isBaselineEnvironmentToken 封闭白名单；必填校验＝setConsumesCanonicalModel(true) 时双条目各恰一条（谁必填由组装方按评估器声明置位，evidence 不域判——与 compilerContractVersion 必填口径并列）；**实现口径 D-3 值形态闸门**（保留 token 条目恒久生效，不论必填开关）：model-identity 值须可被 core::ContentIdentity::fromCanonical 解析（cid-<64hex>——CR-05 裁决第 1 点）、baseline 值须非空无空白（RobWorkBaselineVersion 契约——RT-T09 断言 4 同款，空白判定显式六字符表无 locale 依赖）；与 RT-T09 共享样例的联合断言＝值传递全链逐字节透明（供给值→条目→编码→parse 零重编码——evidence 不重算 CanonicalModel 的切片侧等价断言面）＋CR-05 裁决第 2/3 点身份行为（改基线串→双身份均变；改求解配置→仅 sliceId 变）。⑥测试：新增 13 用例（EV-ID-1/2＋浮点位模式/NaN 拒绝＋EV-CR02 排除面＋EV-CR05 保留条目＋EV-SLICE 冻结拒绝＋EV-SLICEC 结构非法——组名为实现侧组名，§11 矩阵 EV-T04 专项＝"canonical 往返＋浮点位模式＋NaN 拒绝"对照 §12 行），56/56 两模式通过并留痕；CR-02 排除面探针（Policy 吸入投影注入→Cr02 用例变红→逐字还原复绿）失败能力自证；冒烟全量 394/394 零回归（core 61＋evidence 56＋policy 4＋runtime 176＋testkit 97）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注）零新增 |
| v0.6 | 2026-09-12 | EV-T05 实现落位登记（DTB §5.4，证据与 Profile 承载＝`Evidence.hpp/.cpp`——状态/清单/Profile/证明/搜索未果/覆盖证据校验，按 §12 EV-T05 行交付；验证留痕 traceability/builds/wp05-t05/）：①**值类型面**（§6.1～§6.3/§6.6 承载）：EvidenceItemStatus 五值（P-EV-8 实现承载词表）/EvidenceItem（presence 纪律：Satisfied 必带产物摘要、Invalid 必带原因诊断、NotApplicable 必带原因——ERR-01）/EvidenceManifest（snapshotId/sliceId/Profile 三元组绑定面）/EvidenceItemClass＋EvidenceProfileItem＋RequiredEvidenceProfile（**D-14：evidence 只内建 commonRequiredItems 三条通用必需项〔itemId 稳定键 common.snapshot-identity/common.case-coverage-matrix/common.mode-evidence-grade〕，五域明细行零复制——域按需求表 4 自行实例化注册**）/ProofCategory＋DeterministicInfeasibilityProof＋MandatoryStateDescriptor＋CollisionPair/SearchExhaustedRecord（FilteredSolution＋三值过滤原因）/CaseExecutionStatus＋CaseCoverageMatrix＋RegionCoverageEvidence。②**validateProof 字段级校验**（§6.3 五组校验清单全量承载，非抛出纯函数、全部问题一次列出、检查序固定）：类别三类词表→类别条件字段（AnalyticBound 界限/Contradiction 矛盾/MandatoryState 三要素＋碰撞对象对）→产生者注册与契约版本→快照/切片绑定→coverageClaim 句法；**实现口径 I-1～I-7**：I-1 Applicability 复用 ApplicabilityCondition 词形（无条件＝nullopt 即 Always）；I-2 stateKind 以域登记 token 承载（词表开放——evidence 只做非空＋无 NUL 下限，不私裁封闭枚举，O-13 保守字面）；I-3 CollisionPair 须双方非零、A≠B、判定为肯定（否定判定对不能支撑碰撞主张——不伪造）；I-4 coverageClaim 规范 token＝§6.3 原文短语逐字（kCoverageClaimAllAlternatives/kCoverageClaimMandatoryState 常量，按类别核对）；I-5 validateProof 第 4 参 expectedSliceId（"sliceId 与被汇总结果一致"的检查载体，必填不给默认——校验不可静默跳过）；I-6 IProducerRegistryView 最小只读注入端口（§3.3 注入先例——产生者注册检查面，EvaluatorRegistry〔EV-T10〕落地后适配，零跨单元编译依赖）；I-7 producer 以 std::string 承载（D-5 同款词形先行，类型化归 EV-T10）。③**Profile 注册期校验与内容身份**（§6.1 注册行＋§9.5 语法面——四组规则：item 语法/itemId 词形"<域>.<项>"两段 kebab〔I-8 实现口径〕/条件词形/替代标志与 itemClass 一致性＋Common 禁止域登记；与评估器声明的闭包交叉校验归 EV-T10）：computeProfileContentIdentity 对 canonical 身份投影编码（magic "IRDPRF1"＋codec 版本＋逐字段长度前缀大端＋required/suggested 各按 itemId 字典序规范化——**I-9**：非往返载体，SliceCodec baseline-projection 同款纪律；摘要唯一经 core::ContentDigester——CR-02；total 函数不做语义校验，垃圾入给出确定身份出、由注册门禁拦截）。④**证据清单校验与完备性核对**（§6.2）：validateEvidenceItems（presence 纪律）/validateEvidenceManifestBinding（caseScope ⊆ 快照必验集、subject ∈ 对象闭包——"防错误工况/对象引用"原文，EV-COV-2 同源）/checkEvidenceCompleteness（**I-10**：必需项 Satisfied＝满足、NotApplicable＝**不计缺失**〔EV-VER-7 判定面〕、Missing/Invalid/Unverified 全量列出不短路；建议项单独标注不阻断；通用必需项不在域核对——隐式附加归②级门禁 EV-T06）。⑤**Verified 模式前置校验**（§6.2，EV-T03 v0.4 ⑤ 预登记入口的兑现）：validateSnapshotForMode——Verified 强制外部资源全部 Solidified（CON-03/PM-01，逐条带下标）＋复现块版本要素完整（各模式通用面，对手工构造快照复核）；Quick/Preview 不强制固化。⑥**覆盖证据校验**（§6.6 字面——契约 acceptance 4 O-14 保守字面：分母＝快照冻结态必验工况集〔enabled∧mandatory 冻结标记，evidence 不解释标记，schema 权威归 requirements 卡 WP-14-T01〕）：validateCaseCoverageMatrix（与计划核对/重复拒绝/必验集外引用/NotApplicable 必填原因/漏验逐工况点名〔EV-COV-1 校验面〕；**I-11**：矩阵非法与覆盖不完备两面分离——EV-COV-2/EV-COV-1 判定面各自观测；**I-12**：CaseCoverageEntry 增补 notApplicableReason 载体——§6.6 约束行"必填原因"要求载体，字段表未列、约束行为权威）；validateRegionCoverageEvidence（样本集锚对快照冻结采样计划/分母完整性溢出安全求和/零样本暴露〔覆盖率不定义——裁定归 EV-T06〕/downgraded 单向强制——dataInsufficient>0 ⇒ true）。⑦测试：新增 27 用例（EV-PROOF 12＋EV-ITEM 1＋EV-COMPLETENESS 1＋EV-MANIFEST 1＋EV-PROFILE 3＋EV-COVM 4＋EV-COVR 2＋EV-MODE 2＋EV-SEARCH 1——组名为实现侧组名，§11 矩阵 EV-T05 专项＝EV-VER-6/7＋EV-COV-2/3＋validateProof 逐字段反例；EV-VER-1~5/8 与 EV-COV-1/4 用例体随 EV-T06/EV-T11 落地——§12 依赖序），83/83 两模式通过并留痕；validateProof 探针（CollisionPairsMissing 检查短路注入→变红→逐字恢复复绿，PROBE 零残留）失败能力自证；冒烟全量 421/421 零回归（core 61＋evidence 83＋policy 4＋runtime 176＋testkit 97）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注）零新增 |
| v0.7 | 2026-09-12 | EV-T06 实现落位登记（DTB §5.4，五级汇总判定＝`Verdict.hpp/.cpp`——决策表/VerdictTrace/资格/基准检查，按 §12 EV-T06 行交付；验证留痕 traceability/builds/wp05-t06/）：①**值类型面**（§6.4.1/§6.5/§7.2 承载）：ReadinessSummary/SnapshotGateResult（①②级调用方装配面——REQ-06 就绪结论与快照身份门禁）/DomainVerdictInputs＋DomainVerdictViolation（⑤级域判定输入——evidence 只汇总定级与透传，PA-1）/VerdictInput（伪代码字段逐字承载＋**I-2 增补三字段**：mode〔②级"模式与证据等级标识"组成＋§7.2 资格条件〕/regionCoverages〔§6.4.3 工作区域级汇总裁定输入——EV-COV-3〕/searchRecord〔§6.3 末透传入参〕）/VerdictLevel 六值（值即表行号）/VerdictLevelRecord/VerdictTrace（固定六槽完整决策路径——NFR-COR-04）/MissingItem {itemId,reason}（**I-3**：itemId 可定位稳定指称、reason 人读中文，机器判别走 hitLevel＋诊断码面）/VerdictResult（伪代码五成员逐字承载）/ComparisonBaseline＋BaselineDifferenceDimension 五值＋BaselineConsistencyResult＋EligibilityCheck。②**五级决策表 aggregateVerdict**（§6.4.1 字面顺序，自上而下首个命中生效；**knownPitfalls P-EV-3 处置**＝字面顺序：③命中即输出 EngineeringInfeasible、④/⑤不评估〔B 类无关缺失不阻断③——证明为任务级作用域〕）：⓪ outcome≠Completed→NotApplicable（执行轴与工程判定轴正交——TASK-02）；①输入非法→DataInsufficient（input-invalid 全量）；②通用证据门禁＝**固定检查序全量收集**（快照身份门禁→Verified 前置固化 validateSnapshotForMode〔I-2：CON-03 汇总器内执行〕→Profile 注册核对〔findProfile 精确 (profileId,version)＋内容身份三元组一致〕→清单绑定 validateEvidenceManifestBinding→覆盖矩阵 validateCaseCoverageMatrix→区域证据形状；任一命中即本级命中，缺失全量列出——acceptance 2；C6 证明不豁免门禁）；③证明只信 validateProof（**D-09/acceptance 3**：存在性≠有效性，expectedSliceId＝manifest.sliceId；③附加义务＝substitutable 成功产物类项按"因不可行而不适用"出具说明诊断，清单标记义务在评估器侧——**I-6**）；④数据不足判定五条件全量收集（gaps／证明无效〔Invalid 语义——EV-VER-6 反例半边〕／searchRecord 在场〔§6.3 末硬规则：附记录透传〕／区域零样本与降级〔**I-5 分流**：形状非法归②级、零样本与 downgradedRequired 归④级——EV-COV-3"整体 DataInsufficient"〕／**P-EV-7 空必验集保守处置**＝无 enabled∧mandatory 工况→④级 DataInsufficient＋"无启用必验工况"诊断，绝不输出正式通过；全不适用场景由 §6.6 覆盖判据字面在②级拦截不另设路径——**I-6**）；⑤Must 违例→EngineeringInfeasible／Should 违例→Feasible＋警告（verdictInputs 透传进诊断——EV-VER-8）。③**实现口径 I-1（签名落位）**：aggregateVerdict(input, snapshot, producers, profiles)——伪代码两注册表参数以最小只读投影承载（IProducerRegistryView 复用 EV-T05＋新增 **IProfileRegistryView**，EV-T10 注册表直接适配；快照为显式参数＝覆盖矩阵分母/清单与证明绑定/模式前置校验的事实面）；分母恒取快照冻结态 RequiredCaseSet（**acceptance 4/O-14 保守字面**——enabled∧mandatory 冻结标记字面消费，schema 权威归 requirements 卡 WP-14-T01）。④**资格检查**（§7.2 表两行逐条件——**I-7**）：checkFormalPassEligibility（五条件）/checkReviewRecordEligibility（五条件，"携带有效证明"必须字段级校验通过）；unmetConditions 稳定 token 词表冻结于函数注释。⑤**基准检查**（§6.5——**I-8**）：checkComparisonBaselinesConsistent 差异维度五值按枚举序去重输出；sampleSetIds 按序比较（快照冻结采样计划序）；空集/单条平凡通过。⑥**汇总诊断建议码**（**I-9**，P-PR-6/F-056 同模式——建议值，码值权威归 diagnostics StableCodeRegistry）：复用 §13 清单四值（EVI-SNAPSHOT-INCOMPLETE/EVI-CASE-COVERAGE-MISSING/EVI-EVIDENCE-MISSING/EVI-PROOF-INVALID）＋补登八值（EVI-OUTCOME-NOT-COMPLETED/EVI-INPUT-NOT-READY/EVI-MANIFEST-BINDING-INVALID/EVI-PROFILE-UNRESOLVED/EVI-INFEASIBILITY-SUBSTITUTION/EVI-SEARCH-EXHAUSTED/EVI-REGION-COVERAGE-DOWNGRADED/EVI-MUST-VIOLATION/EVI-SHOULD-VIOLATION——kDiag* 常量承载，登记转 diagnostics 所有者注册时裁决）。⑦测试：新增 24 用例（VerdictDecisionTable 2＋VerdictGate 1＋VerdictSearchExhausted 1＋VerdictConfigCollision 2＋VerdictPathCollision 1＋VerdictMandatoryProof 2＋VerdictNotApplicableItem 1＋VerdictMustShould 2＋VerdictCoverage 1＋VerdictBaseline 2＋VerdictMissingCompleteness 2＋VerdictD9 1＋VerdictPitfalls 2＋VerdictRegionCoverage 1＋VerdictEligibility 2＋VerdictDeterminism 1——组名为实现侧组名，§11 矩阵 EV-T06 专项＝EV-VER-1~8＋EV-COV-1/4 对照 §12 行；EV-VER-6 字段级校验半边与 EV-COV-2/3 校验器半边已随 EV-T05 落地——§12 依赖序），107/107 两模式通过并留痕；覆盖门禁探针（②级矩阵检查短路注入→EV-COV-1/acceptance 2 等 3 用例变红→逐字恢复复绿，PROBE 零残留）失败能力自证；冒烟全量 445/445 零回归（core 61＋evidence 107＋policy 4＋runtime 176＋testkit 97）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注）零新增 |
| v0.8 | 2026-09-12 | EV-T07 实现落位登记（DTB §5.4，结果包络＝`Envelope.hpp/.cpp`——Draft/make/validateCombination，按 §12 EV-T07 行交付；验证留痕 traceability/builds/wp05-t07/）：①**值类型面**（§7.1 注释逐字承载，未定名类型按注释原文最小成形）：DomainPayloadDraft/DomainPayload（正式结论载荷——摘要由 make 计算，非申报值）/PartialDataRef（reusable 恒 false——构造边界强制，EV-ENV-2 包络侧）/CaseScope/EvidenceProfileRef（profileId+version+contentIdentity 三元组绑定面）/ProducerProcess＋ProducerInfo（设计注释点名两字段，省略号字段待后续任务按需增补）/ResultEnvelopeDraft/ResultEnvelope。②**构造边界 make()**（SA-13）：唯一入口、非法组合抛 EvidenceError(EnvelopeIllegalCombination)（detail 逐条含字段名——EV-ENV-1"Draft 校验失败消息含字段"）；**EnvelopeIssueCode 21 值**＝表 3 逐格区分（Errors.hpp 该码注释登记的"逐条区分在 EV-T07 落地"兑现——异常轨仍单一稳定码、逐格区分由 issue 码＋消息承载）；检查序固定：Preview 无条件边界（表 3 行 4/表 1——排首位）→结构面（task 五元组/评估键词形/三身份非零/产出者版本/载荷 token/reusable 标记）→Completed 行（行 2 NotApplicable 拒绝＋行 1 证据清单与工况标识必含/清单项 presence 纪律复用 validateEvidenceItems/DataInsufficient 空缺失清单拒绝/Feasible 带缺失拒绝/EngineeringInfeasible 凭据）→非 Completed 行（状态必须 NotApplicable＋payload 空＋无 Satisfied 判定声明＋无 missingItems——行 3 及其反面）；**实现口径 E-1**："构造后不可变"由 make 唯一生产者纪律承载（公共成员无 setter，InputSlice 同款——不用 const 成员以保全值语义）。③**validateCombination 两段式**（§7.2"make/validateCombination 纯函数"＋"接纳侧 execution 复用同一校验器"）：单参上下文无关面＝make 的拒绝依据；三参快照绑定面＝caseScope ⊆ 快照冻结必验工况集（§7.1 caseScope 注释——越界 id 逐条点名）＋EngineeringInfeasible 且凭据为证明时 validateProof 字段级核对（表 3 行 1"有效证明（validateProof 通过）"——**D-09 存在性≠有效性**；expectedSliceId＝包络 sliceId）＋清单对快照绑定复核（§6.2——汇总段已挡一次、接纳段复检＝纵深防御）。④**实现口径 E-2～E-5**：E-2 payload 摘要＝make() 经 core::ContentDigester 对 canonicalBytes 重算（CR-02 摘要算法唯一；Draft 携带无摘要组装形态 DomainPayloadDraft——申报摘要可伪造，重算值才构成完整性凭据）；E-3 EngineeringInfeasible 凭据两段式＝在场（上下文无关面：证明在场或 Must 违例记录在场）＋有效（快照绑定面：validateProof 通过）；E-4 Must 违例记录识别面＝复用 EV-T06 kDiagMustViolation 码面（CR-01 同源纪律不私造 token，码值权威归 diagnostics）；E-5 清单—包络绑定一致性（manifest.snapshotId/sliceId 必须等于包络同名绑定——§6.2 记录面，防证据张冠李戴）；E-6 EvaluationKey/producer 暂以 std::string 承载（D-5 同款词形闸门 isValidEvaluationKey——类型化归 Evaluator.hpp/EV-T10）；ProducerInfo 词形闸门＝非空无 NUL 编码安全下限（O-13）。⑤**CR-01 acceptance 2**：新注册跨单元契约测试目标 `sdurws_ird_evidence_contract_test`（§11 测试目标分工——`_test` 单元内/`_contract_test` 跨单元——的首次落地；CMake 与 _test 同构）——EvidenceVocabularyContract 三防线：token 字面量与 core::toToken 输出逐一相等（11 值＋*FromToken 往返＋包络承载枚举回流钉住）/evidence 产品源码零词表本地定义扫描（include/＋src/）/Envelope.hpp include 面断言＋6 组 static_assert 钉死包络三轴字段类型＝core 词表类型。⑥测试：新增 31 用例（EnvelopeTest 26：EnvelopeMatrix 21＋EnvelopeSnapshotBound 3＋EnvelopeDeterminism 2；EnvelopeContractTest 5：EvidenceVocabularyContract 5——组名为实现侧组名，§11 矩阵 EV-T07 专项＝EV-ENV-1/2 对照 §12 行；EV-ENV-2 的 judgeCacheHit 判定面归 §8.2/EV-T09——§12 依赖序，本任务交付其包络侧数据面前提：partialData.reusable 恒 false 由构造边界强制），133/133＋5/5 两模式通过并留痕；探针失败能力自证（Preview 检查 'false &&' 短路注入→2 用例变红→逐字恢复复绿，PROBE 零残留）；冒烟全量 476/476 零回归（core 61＋evidence 133＋evidence_contract 5＋policy 4＋runtime 176＋testkit 97）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注，与 wp05-t06 基线 diff 逐项一致）零新增 |
| v0.9 | 2026-09-12 | EV-T08 实现落位登记（DTB §5.4，当前性纯投影＝`Currentness.hpp/.cpp`——computeCurrentness/CurrentnessIndex，按 §12 EV-T08 行交付；验证留痕 traceability/builds/wp05-t08/）：①**类型面**（§8.1 输入/输出原文承载）：ResultRef（结果引用身份摘要五要素——task/evaluationKey/contractVersion/sliceId/inputBaselineId；fromEnvelope 投影＝"envelope 或其身份摘要"的承接）、CurrentnessTarget（目标上下文——projectId/branchId/scheme?/evaluationKey/当前配置内容身份）、ICurrentnessFactSource（依赖重建事实源——声明→当前条目，nullopt＝不可解析；§3.3 值传递＋最小注入接口同款模式，适配器归 L5/请求方）、CurrentnessSources（重建源集合——目标快照＋事实源＋当前契约版本＋声明集＋CR-05 开关＋结果切片可选注入）、InvalidationKind/InvalidationReason、CurrentnessStatus 两持久态＋UnevaluableCause（cross-context/unresolved-dependency——§8.1 大括号标注的承载）＋CurrentnessResult 四字段（status: optional——**不可判定为计算形态非第三持久态**，P-EV-4；呈现口径归 ui/P-UI-2，本单元计算面不阻断）。②**computeCurrentness 五步算法**：步骤 1 上下文匹配（project∧branch——TASK-03/S7 拦截）→ 步骤 2 契约匹配（短路 Superseded{EvaluatorContractChanged}）→ 步骤 3 目标切片重建（逐声明解析——缺失全量列出诊断不在首错短路；任一无法解析→不可判定{unresolved-dependency}**不得默认 Current**；SliceBuilder 拒绝转译为不可判定＋原细节透出；事实源条目与声明键/Kind 不符＝实现违约 fail-fast）→ 步骤 4 比较 → 步骤 5 输出（evaluatedAgainst＝target 原样快照）。③**实现口径 I-1～I-8 登记**（实现细节精确化，语义与设计一致）：I-1 结果引用以身份摘要形态承载；I-2 "注册表当前 descriptor"在 EV-T10 落地前以值传递注入（currentContractVersion＋declarations＋consumesCanonicalModel——snapshotFactKeys 注入同款先例）；I-3 目标快照由请求方按 §10.1①~③同流程组装注入，目标条目校验与身份计算唯一经 SliceBuilder/SliceCodec（CR-02），**失效判定按条目级对比执行**（§5.1 身份纪律原文"失效判定用条目级 ContentVersion/内容身份对比，从不用修订号代替"——sliceId 全值编码含快照锚〔SliceCodec full 的 [32] snapshotId，EV-T04 布局〕，全值判定会让 HEAD 前进直接进入判定、违反 §8.1 步骤 4"HEAD 前进本身不出现在比较里"）；I-4 resultSlice 可选注入——在场输出条目级失效原因（设计主形态/KIN-13 提示数据源），缺席降级 sliceId 全值比较（跨修订恒 Superseded 保守方向）；I-5 InvalidationKind 词表＝步骤 4 列举值＋EvaluatorContractChanged（步骤 2 承载）＋EnvironmentChanged/SliceContentChanged（"…"两处保守扩展——Environment 变化与身份级兜底）；I-6 scheme 无 core Id 类型（六 Id128 无 SchemeId），以可选字符串承载且仅入 evaluatedAgainst 记录面；I-7 评估键并入上下文匹配（规则表行 1 四要素——跨评估键查询＝不可判定{cross-context}，诊断区分失配维度）；I-8 判据分层：sliceId 全值捷径→条目级对比主判据（Current ⇔ 条目集逐条相等）→身份摘要降级。④**CurrentnessIndex 会话态投影缓存**（§8.1 规则表行 3）：键＝(runId, targetContextId)、互斥量保护（事件线程×查询线程）、store 拒绝空串上下文标识/保留值 runId（CacheIncompatible fail-fast）、invalidateRun 按运行粒度失效（事件不携带数据 core D-09——保守正确）、判定本身不写缓存（EV-CUR-3"currentness 仅存在于投影索引"的结构性保证）；**纯投影承诺**：全部入参 const 引用、归档包络/快照/切片零修改（acceptance 2 的断言面）。⑤**诊断建议码**：kDiagCurrentnessCrossContext/kDiagCurrentnessUnresolvedDependency（EVI-* 建议值——码值权威归 StableCodeRegistry，P-PR-6 同模式；reporting 侧 RPT-CURRENTNESS-UNEVALUABLE 为其呈现面消费码）。⑥测试：新增 18 用例（EvidenceCurrentness 组——§11 矩阵 EV-T08 专项＝EV-CUR-1~4 对照 §12 行）：EV-CUR-1（无关 HEAD 前进仍 Current/被消费对象变化 Superseded＋旧→新 cv detail/新会话同内容）＋EV-CUR-2（缺失不默认 Current/多键全量列出/重冻结失败转译/事实源违约 fail-fast）＋EV-CUR-3（判定前后归档包络逐字段不变＋投影索引拷贝语义）＋EV-CUR-4（跨项目/跨分支/评估键失配）＋AT-05 载体（预算复评 Superseded＋ConfigurationChanged＋同快照双切片钉 inputBaseline 排除面——sliceId 变而基准不变＝复评不冒充原运行）＋契约短路（事实源零调用观测）＋条件翻转/条目消失＋身份摘要降级＋Index 行为＋重建源缺位 fail-fast；151/151＋5/5 两模式通过并留痕（既有 133＋5 零修改回归）；探针失败能力自证（unresolved 分支注入 'out.status = Current' 一行→2 用例变红→逐字恢复复绿，PROBE 零残留）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注，与 wp05-t07 基线计数逐项一致）零新增 |
| v1.0 | 2026-09-12 | EV-T09 实现落位登记（DTB §5.4，缓存/检查点兼容判定＝`Compatibility.hpp/.cpp`——judgeCacheHit/judgeCheckpointCompatibility 两套独立判定接口，按 §12 EV-T09 行交付；验证留痕 traceability/builds/wp05-t09/）：①**类型面**（§8.2 原文冻结代码逐字承载）：CachedResultSummary（被复用结果七字段摘要——mode/outcome/manifestFinalized/sliceId/evaluatorContractVersion/profileContentIdentity/inputBaselineId?；"从缓存条目/envelope 提取（不载入全载荷）"——判定只消费身份面与完成面，不读全载荷）、CacheHitQuery（请求身份面四要素——requestedMode/requestSliceId/requestContractVersion/requestProfileIdentity；两判定共用请求载体）、CacheMissReason 六值（§8.2 reasons 注释行 kebab 词表逐字——mode-mismatch/slice-mismatch/contract-mismatch/profile-mismatch/outcome-not-completed/manifest-incomplete；枚举序＝检查序，一经交付只允许表尾追加——EvidenceErrorCode 同款纪律）、CacheHitResult（verdict 三档嵌套枚举 FullHit/DiagnosticOnly/Incompatible＋reasons 全量清单）、CheckpointSummary（检查点登记头五字段——evaluatorKey/sliceId/evaluatorContractVersion/checkpointFormatVersion/integrityVerified；无 outcome/模式/Profile 面——**可恢复性与任务成败独立的类型级保证**，EV-CPA-3）、CheckpointCompatibilityResult（Resumeable/Incompatible 两档＋reasons 稳定 token 串〔std::vector<std::string>——§8.2 原文；检查点拒绝原因经 execution 通道跨进程透出〔D-15 错误码化的上游输入〕，词表五值冻结于契约头注释：evaluator-key-invalid/slice-mismatch/contract-mismatch/checkpoint-format-incompatible/integrity-not-verified〕）。②**judgeCacheHit 三档判定**（§8.2 判定式逐字）：FullHit ⇔ 模式相等∧sliceId 相等∧契约版本相等∧Profile 身份相等∧outcome==Completed∧manifestFinalized（策略/种子/线程/配置/样本/环境版本已含于 sliceId——D-01/D-11 身份要素完备性是 FullHit 的前提，一处相等覆盖全部输入要素）；DiagnosticOnly ⇔ 身份面四项全对齐∧(outcome≠Completed∨未 finalize)——部分/失败/取消结果**可读作诊断不得正式命中**（CON-04；EV-ENV-2 判定面兑现——EV-T07 v0.8 已交付其包络侧数据面前提 partialData.reusable 恒 false）；Incompatible＝其余一切（Quick 缓存对 Verified 请求与反方向**双向均 Incompatible**——D-13 模式是请求效力属性不升降级）。③**judgeCheckpointCompatibility 五条件**（§8.2 Resumeable 判定式逐字）：evaluatorKey 相等∧sliceId 相等∧契约版本相等∧checkpoint 格式版本兼容∧integrityVerified；**实现口径 C-1～C-5 登记**（实现细节精确化，语义与设计一致）：C-1 未满足条件**全量收集不首错短路**（reasons 按检查序＝枚举声明序——同失配必得同一清单，NFR-COR-02；verdict 裁决＝身份面任一失配→Incompatible／身份面全对齐且有结果面失配→DiagnosticOnly／全对齐→FullHit）；C-2 CheckpointSummary.evaluatorKey 暂以 std::string 承载（D-5 同款词形闸门 isValidEvaluationKey——InputSlice/Envelope 先例，类型化归 Evaluator.hpp〔EV-T10〕）＋"evaluatorKey 相等"的承载＝评估键进入切片身份（Slice.hpp"进 sliceId——CON-04"，sliceId 相等⇒键相等，请求侧无需另带键字段），显式字段做词形校验（词形非法＝损坏/伪造检查点头→evaluator-key-invalid 保守拒绝）；C-3 "checkpoint 格式版本兼容"＝落在本构建支持**闭区间**〔kCheckpointFormatVersionMin=1, kCheckpointFormatVersionMax=1〕（§8.2 标题括注存储归 execution/project——区间常量随其存储格式演进推进并留痕；区间外一律 Incompatible，读不懂的格式不得恢复）；C-4 requestedMode 与 requestProfileIdentity 不参与检查点判定（Resumeable 五条件无此二项——检查点恢复的是**运行进度**而非正式证据，续跑完成的结果仍要过包络构造边界与 judgeCacheHit，模式效力与 Profile 绑定在结果面把关）；C-5 判定函数为**非抛出纯函数**（§2.1 try 轨分轨——可恢复查询路径；Errors.hpp CacheIncompatible 注释"判定输出供调用方转译"：调用方对 Incompatible 按需转译 EvidenceErrorCode::CacheIncompatible／建议码 EVI-CACHE-INCOMPATIBLE，码值权威归 StableCodeRegistry——PA-1 本单元不注册）。④**四类复用严格区分**（§8.2 首行"四类复用严格区分"任务约束§五.9 的承载）：正式缓存命中（FullHit）/诊断性读取（DiagnosticOnly）/不兼容拒绝（Incompatible，含 Quick↔Verified 双向）/检查点恢复（Resumeable）——两套独立判定接口互不冒充（§15.5 自审行"是否对缓存与检查点采用了同一套错误门禁"→两套接口）。⑤P-EV-1 处置：core.md v0.1（Draft 未冻结）契约按卡内签名消费（EvaluationMode/TaskOutcome——core Evaluation.hpp；ContentIdentity——core Digest.hpp 字节精确等值），冻结时按 diff 增量同步；本任务未发现基线漂移。⑥测试：新增 22 用例（CompatibilityCacheHit 13＋CompatibilityCheckpoint 9——组名为实现侧组名，§11 矩阵 EV-T09 专项＝EV-CPA-1~3 对照 §12 行；EV-ENV-2 判定面在本任务兑现）：EV-CPA-1（Quick↔Verified 双向拒绝＋同身份同模式 FullHit 正例半边）＋EV-CPA-2（契约失配两判定面＋token 精确断言）＋EV-CPA-3（Failed 检查点仍 Resumeable＋无 outcome 字段类型级独立＋请求模式不参与＋integrity 未验证反面）＋acceptance 2（Canceled/Failed/Interrupted→DiagnosticOnly 非 FullHit＋reasons 含 outcome-not-completed；未封账反例；身份面＋结果面混合不降档；六项全失配全量收集按序）＋检查点五条件逐项与格式区间边界（0/1/2）＋两判定确定性；173/173＋5/5 两模式通过并留痕（既有 151＋5 零修改回归）；冒烟全量 516/516 零回归（core 61＋evidence 173＋evidence_contract 5＋policy 4＋runtime 176＋testkit 97）；探针失败能力自证（judgeCacheHit 模式检查短路注入 'if (false && …)' 一行→EV-CPA-1 两用例等 4 用例变红→逐字恢复＋touch 强制重编译复绿〔F-068 mtime 陷阱规避〕，PROBE 零残留）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注，与 wp05-t08 基线命中区块逐项一致）零新增 |
| v1.1 | 2026-09-12 | EV-T10 实现落位登记（DTB §5.4，评估器接口与注册表＝`Evaluator.hpp/.cpp`——descriptor/接口/两注册表/manifest，按 §12 EV-T10 行交付；验证留痕 traceability/builds/wp05-t10/）：①**类型与接口面**（§9.2/§9.3 原文承载）：ThreadSafety 三值词表（SingleThread/ConcurrentReadOnly/FullyThreadSafe）＋isKnownThreadSafety 位型防御面（诚构造不可能非法——检查是注册门禁对反序列化损坏/伪造 descriptor 的位型完整性防御）；EvaluationKey（**D-5 收敛**：语义别名＝std::string——§3.1 概念名落地，词形闸门维持 isValidEvaluationKey、捕获点固定在注册期与编码期；**不做强包装类型**的理由＝§9.4 明文把"key 语法"列为注册期校验项，词形非法值若构造期即不可存在将架空该拒绝面）；EvaluatorDescriptor 七字段（§9.2 字段表全量——**P-EX-7 处置**：不含任何执行期能力字段〔暂停/检查点粒度/强制终止代价〕，该面归 execution 侧注册表扩展声明承载，文件头登记处置方向、两侧均不私改对端口径；测试以结构化绑定钉死恰七成员与成员类型——字段面漂移即编译失败）；EvaluationRequest/EvaluationOutput（§9.3 逐字——envelope 由调用侧经 aggregateVerdict＋ResultEnvelope::make 组装，评估器只产出素材）；IEvaluationContext（取消查询/进度上报/物化对象读取——evidence **零实现**，宿主〔execution/主进程〕注入）；IEngineeringEvaluator/IEvaluatorFactory（§9.3 实现契约：同切片同环境等价输出/纯计算不派发不写项目/可重入性＝threadSafety/长评估周期性查询取消）。②**EvaluatorRegistry**（§9.4 行为冻结表逐行）：注册期单线程约定＋运行期 find/manifest 并发只读安全（std::shared_mutex——注册排他/查询共享/create 工厂调用锁外，锁序唯一无死锁面）；重复键注册拒绝、不覆盖、不静默（EvidenceError〔EvaluatorDuplicate〕——同键不同 contractVersion 同拒〔版本冲突行〕，换版本＝装配清单变更无运行期热替换）；未知键 find/create 返回 nullptr（查询非抛）；注册期校验＝requireValidEvaluatorRegistration（**检查序固定**：key 词形→契约版本>0→模式集非空无重复〔"子集"语义〕→threadSafety 合法→contentIdentity 保留值→Profile 解析→Profile 条件交叉校验→声明闭包；"任一失败即拒绝并指明字段"由 RegistrationIssue 字段名聚合承载——同坏注册必报同一首错，NFR-COR-02）；**码面分工**（EV-T02 预登记"注册拒绝归 EvaluatorRegistry 消费"契约兑现）：声明闭包问题→DeclarationInvalid、其余→EvaluatorDescriptorInvalid（表尾新增），两类并存按检查序首类优先；manifest()＝按 key 字典序 {key, contractVersion, profileIdentity} 清单及摘要（**IRDRGM1** 非往返身份投影编码——magic＋codec 版本＋条目数＋逐条目长度前缀/大端/32 字节身份；CON-06/AT-19 主/worker 比对面，同注册集恒同摘要）；**实现口径 R-1～R-4** 登记（R-1 create 未知键 nullptr 查询轨——装配不一致由 execution 摘要比对在派发前拦截；R-2 注册检查序＝空 factory→重复键→注册期校验→登记；R-3 descriptor.profile 复用 EvidenceProfileRef 三元组结构但只消费声明面 {profileId, version}——contentIdentity 域不可申报〔§9.5〕、申报非零即拒绝、manifest 恒取注册表权威值；R-4 Profile 注册检查序＝重复键→语法校验→身份覆盖计算→入库）。③**EvidenceProfileRegistry**（§9.5）：重复 (profileId, version) 拒绝（EvidenceError〔ProfileDuplicate〕——同域多版本共存，版本是键的一部分）；validateEvidenceProfile 语法面（EvidenceError〔ProfileInvalid〕——detail 逐条含 itemId）；contentIdentity 注册时由 evidence 计算、域申报值一律覆盖（"域不可申报"）；条件闭包经注册顺序解耦（Profile 注册查 referencedKeys 语法＝上半场、评估器注册做 referencedKeys ⊆ 声明键 ∪ 快照事实键交叉校验＝下半场）；**视图适配兑现**（EV-T05 I-6/EV-T06 I-1 预登记"EV-T10 注册表直接适配"）：EvaluatorRegistry 实现 IProducerRegistryView、EvidenceProfileRegistry 实现 IProfileRegistryView——注册表即视图本体，无需另建适配器。④**错误码表尾追加三值**（10→13 值）：EvaluatorDescriptorInvalid/ProfileDuplicate/ProfileInvalid（token evidence/evaluator-descriptor-invalid、evidence/profile-duplicate、evidence/profile-invalid——§9.4/§9.5 注册边界语义 kebab 派生；建议码 EVI-EVALUATOR-DESCRIPTOR-INVALID/EVI-PROFILE-DUPLICATE/EVI-PROFILE-INVALID 补登——P-PR-6/F-056 同模式，码值权威归 diagnostics StableCodeRegistry，登记转 diagnostics 所有者注册时裁决）；ErrorsTest 全表锚定同步 13 值（表尾序号 12 钉死）。⑤**D-12 调度分离**（acceptance 2）：两注册表与调用约定零调度设施（无调度器/队列/取消通道/进度设施——IEvaluationContext 只是接口、evidence 零实现）；NoSchedulerFacilitiesInEvaluatorSources 源码扫描钉死（Scheduler/TaskQueue/RunRegistry/std::queue/priority_queue 与执行能力字段词面零命中——P-EX-7 双保险）。⑥测试：新增 18 用例（EvidenceRegistrationBoundary 5＋EvidenceRegistrationManifest 3＋EvidenceRegistryViewAdapters 2＋EvidenceProfileRegistryBehavior 4＋EvidenceDescriptorFieldFace 1＋EvidenceEvaluationContract 2＋EvidenceRegistryNoScheduler 1——组名为实现侧组名，§11 矩阵 EV-T10 专项＝EV-REG-1/2 对照 §12 行；EV-REG-3 替身边界声明在测试文件头前置登记——完整 ScriptedEvaluator 契约替身与 EV-* 全量用例体归 EV-T11，§12 依赖序），191/191＋5/5 两模式通过并留痕（既有 173＋5 零修改回归）；探针失败能力自证（registerEvaluator 重复键检查短路注入→EV-REG-1 主例变红〔probe-red-run exit 1〕→逐字恢复＋touch 强制重编译复绿〔F-068 mtime 陷阱规避〕，PROBE 零残留）；冒烟全量 534/534 零回归（core 61＋evidence 191＋evidence_contract 5＋policy 4＋runtime 176＋testkit 97）；ird_gates 3 处命中均为既有（F-019 两项＋F-051 一项——按 findings.json 权威归属标注，与 wp05-t09 基线逐项一致）零新增 |
| v1.2 | 2026-09-12 | EV-T11 实现落位登记（DTB §5.4，测试替身与契约套件＝evidence/test/*＋testdata/golden/ev-slice-fixture/，按 §12 EV-T11 行交付；验证留痕 traceability/builds/wp05-t11/）：①**ScriptedEvaluator 规范替身**（§11 测试设施行承载，`evidence/test/EvidenceTestDoubles.hpp`——testdoubles 命名空间，runtime 先例同款）：脚本步骤回放（预设产出/EvidenceError 域自选抛出/取消抛出三轨——§9.3"抛 EvidenceError 或返回诊断，域自选"＋"观测到取消后自主选择退出路径"）；周期性取消查询＋进度上报＋对象读取三通道演练（ScriptedEvaluationContext 替身承载 IEvaluationContext——evidence 零实现的宿主面）；ScriptedEvaluatorFactory 经**真实** EvaluatorRegistry 注册（§9.4/§9.5 注册期校验全过——§11"注册于测试内 registry"的字面兑现）；descriptor＝makeScriptedKinDescriptor（key"kin-batch-ik"@7、两 Required 声明、profile 声明引用 contentIdentity 置保留值〔§9.5/R-3〕、模式集 {Quick,Verified}、SingleThread 与替身单线程行为一致）。②**EV-* 用例体端口路径补齐**（EvaluatorPortSuiteTest——与 EV-T06 VerdictTest 决策表直接装配互补的"经评估器端口"组合面）：EV-VER-1~5/8＋EV-VER-6 双例经 ScriptedEvaluator 产出→调用侧装配→aggregateVerdict（素材透传承接链：proof/searchRecord/verdictInputs/evidence 逐项不丢不变形）；§9.3 上下文契约演练（取消抛出经端口传播——D-15 不捕获域异常；对象读取/进度上报可观测）；EV-INV 显示单位子例（§5.3 KIN-12 行——显示单位切换→无任何 reason，补齐矩阵行观测点）；每用例 IRD_TEST_INFO 需求/AT 追溯（§11 登记要求）。③**EV-REG-3 替身边界声明两面**：文书面＝evidence/test/README.md（§1 全文声明＋§2 替身清单＋§3 §11 矩阵→测试文件落位映射全表）；机检面＝ScriptedDoubleBoundary 两用例（产品源码 src/＋include/ 替身符号零命中扫描＋README 声明在案断言——替身数据不冒充真实证据的机制面）。④**envelope 访问器特化**（testkit §10.2 evidence 行交接承接，EnvelopeAccessorContractTest 于 `_contract_test`）：五访问器面（outcome/engineeringStatus/hasEvidenceList/hasMissingItemsList/hasFormalConclusion）把 ResultEnvelope 喂入 testkit checkEnvelopeCombination 泛型谓词；合法组合双过、DataInsufficient 空缺失清单/Canceled×Feasible 双拒对账，Completed 空证据清单格如实登记两面分工（谓词面筛查 vs 产品构造边界绑定结构＋汇总④级完备性兜底）；特化定义在测试侧——产品库不引入 testkit 依赖（§3.2 红线）。⑤**切片契约夹具数据集**（testdata/golden/ev-slice-fixture/1.0.0，contract-fixture 类按 testkit manifest schema 登记）：inputs/slice.json＝切片语义描述（Object/Configuration〔内嵌 IEEE754 0.1 位模式〕/Policy 条件未适用三形态＋空必验集快照）；expected 由 generate/make_ev_slice_fixture.mjs 按 §5.2/编码布局规则**独立重实现**产出（snapshotId→sliceId→inputBaselineId 三身份＋双形态编码 hex）——与产品 C++ 编码器双实现互证（SliceFixtureContractTest：三身份/双形态编码逐字节对照＋parse(expected)==重建切片〔EV-ID-1 数据集载体〕＋重建确定性＋配置载荷自洽）；lint 通过（GoldenDataset::load 全链＋testdata_lint 工具全量扫描 3 数据集零违规）并随 CI 跑通（`_contract_test` 两模式）；随附容差档案 ev-slice（零宽容差条目＝字节级等值的档案化表达——EV-ID-2 反例面）。⑥CMake：evidence/CMakeLists.txt 两测试目标增列新用例文件；_test/_contract_test 链接 sdurws_ird_testkit（testkit §10.2 evidence 行消费面——测试侧依赖，产品目标链接面仍仅 core，红线守卫配置期断言通过）。⑦测试：`_test` 205/205＋`_contract_test` 14/14 两模式通过并留痕（既有 191＋5 零修改回归；新增 15＋9）；失败能力自证＝产品源码注入替身符号→ScriptedDoubleBoundary 主例如期变红→逐字恢复＋touch 强制重编译复绿（F-068 规避），PROBE 零残留；lint 工具 3 数据集零违规。⑧ird_gates 5 处命中＝3 既有（F-019 两项＋F-051 一项）＋**2 新增**（evidence _test/_contract_test→testkit 直链——F-051 同型：白名单 T-1 段明文允许而引擎 SUB 形态一对 →testkit 边未豁免；契约测试消费面＝本任务 §10.2 交接交付物本体、_test 链接＝§11 IRD_TEST_INFO 登记要求——已登记 **F-072**，处置归 WP-01 构建所有者） |

| v1.3 | 2026-09-13 | EV-T12 文档与门禁同步落位登记（DTB §5.4；验证留痕 traceability/builds/wp05-t12/）：①**§15.3 九项状态实测同步**——P-EV-1（core.md 现 v0.11 未冻结；其 P-D-1/P-AR-3 已于 CORE-T10 关闭，本文 §3.4 回复被引为消账证据）、P-EV-2（ARCHITECTURE 现 v0.12 事实修正，本文锚点零返工面）、P-EV-3/P-EV-7（维持 open，实现侧保守字面已落地——EV-T06 落位）、P-EV-4（ui.md 产出，P-UI-2 承接待需求侧确认）、P-EV-5（reporting.md 产出，downgraded-reference-value＋RP-COV-1 承接落卡）、P-EV-8（reporting/ui 按五值实现词表消费）、P-EV-9（requirements 详设未产出维持 open）；②**P-EV-6 就地消账**（runtime.md P-RT-9 同源请求对账关闭）——project.md 磁盘复核完整（§1～§15 全在、实测 1386 行、`__PART3__` 零命中），残余接口联合复核按 phase-one-readiness §2 维持；③**README 指向核对零偏差**——include README 同步至 EV-T11 落位现状（十模块逐头比对、过期前瞻句消除，证据 traceability/builds/wp05-t12/README-audit.md）；④**UT-BUILD 并入 CI 建议登记 WP-01**（traceability/wp05-t12-gate-registrations.md——沿 CORE-T10/RT-T13 先例：既有承载如实消账＋CI 模板单目标正则缺口登记，本卡不代行 scripts/ci 辖域改动）；⑤**本文与实现零偏差登记**＝v0.3~v1.2 各落位登记在案（实现口径 I/E/R/C 系全部随落位登记，无未登记偏差）＋双模式实跑 205＋14 全绿复证（§15.4 v1.2 口径逐数一致） |
