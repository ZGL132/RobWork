...
| v1.15 | 2026-09-26 | **WP-24-T03 首版装配落位登记**（owner 指示 2026-09-26"每完成一个插件进行装配"提前启动——原排期阶段 E；分支 wp24-t03 待验收；首版范围＝ui 侧装配契约落地＋建模插件真宿主挂位，收口余量见④）：①**装配契约头落位（P-MDL-8 消账）**——IPluginUiRegistrar.hpp〔§10.9 六形状＋createPluginUiRegistrar 工厂＋实现 src/PluginUiRegistrar.cpp：白名单→重复→描述符→Ok 校验序、失败隔离 §11.3、白名单构造期固定〕＋IPluginUiModule.hpp〔§11.2 三方法冻结签名；project::CommandEnvelope/IWorkbenchShell 前向声明——ui 头零 project include 纪律＋IWorkbenchShell→UiPorts→AboutDialog 包含链断环〕；PluginAssemblyReport 自 AboutDialog.hpp 迁移（v1.2 预注兑现，形状零变化源/二进制兼容）。②**宿主插件装配序列**——ui/plugin/DomainAssembly.*（白名单八 token＋建模 registrar 登记报告＋面板工厂消费＋命令提交出口绑定〔宿主状态栏瞬态消息"已受理域命令 …（域执行面随装配收口任务接线）"——首版可见反馈面，不虚构执行成功〕）；ui_plugin 链接 sdurws_ird_modeling_plugin（ui→modeling 装配边——DTB §4.5 同款 SUB；消费面仅 modeling 装配门面 assembly/…/ModelingPluginAssembly.hpp〔modeling 侧新增，plugin 目标 PUBLIC 包含面〕）；建模 Dock 入宿主 Left 区（ird_modeling_dock）；UiText 增键族"域命令标题"10 键（cmd.modeling.*.title 中文工程用语——resolveText 唯一出口，面板按钮键名→中文）。③**建模侧切换**——ModelingUiModule 同形→继承 ui::IPluginUiModule（三方法 override，签名/语义零变化）；新增装配 API（bindCommandSubmit/bindTextResolver/seedTemplateSession/createPanel）＋面板 setCommandTitleResolver 呈现缝。④**首版范围诚实边界（收口余量）**——域命令入 content 注册表（§7.2 冲突规则）／只读投影入 StageStatusModel 汇聚／中央区 CentralAreaHost 挂位〔首版＝宿主侧 Left Dock 承载〕／AssertionSuite 真判定注入（policy 评估器＋runtime 名称上下文）／ModuleSessionState 与 UiSessionController 事件全同步／**确认交互桥接线**——随装配收口任务（阶段 E）兑现。**T03b-2 已落位（2026-09-26 同日增量）**：draft.apply 域侧挂钩——WorkbenchContentDeps 增 `applyDraftHandler` 覆写（同 new/open/save/close 五覆写模式）＋宿主 orchestrateApplyDraft（权威分支表首条 tip 锚定会话→模块 buildDraftCommand→`ProjectCommandService::submit` 非交互提交→回执投影回写 onCommandResult→Committed 后会话锚前移＋根身份回填〔objectRefs 按 robot-design token 过滤〕）；StorePortAdapter 增 projectStore() 访问器（装配层特权面）。非交互提交边界：Confirmable 集存在＝Rejected(confirmations-unresolved) 如实呈现（不虚构放行），交互桥随收口尾增量。⑤**验证**——集成 ui/modeling 五测试目标全绿（154/154＋22/22＋265/265＋17/17）；真宿主 GUI 冒烟＝--rwsplugin 直载、建模 Dock 呈现＋十条命令按钮中文工程用语（UiText 生效实证）＋无错误弹窗，截图 traceability/builds/wp24-t03-first/modeling-in-host-smoke-02.png；ird_gates 引擎直跑基线..branch 归一化比对＝净新增恰 1 处逻辑命中（SUB：ui->modeling——ui→project 同族装配边，登记册回填归 WP-01-T03）。需求语义零变化（详见 §3.3 两头落位行＋§10.9/§11 落位登记） |
