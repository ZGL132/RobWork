...
ui/include/sdurws/ird/ui/          命名空间 sdurws::ird::ui
  ...
  UiPorts.hpp           C-3/4/5/7/8/10/11/12 最小端口接口（O-31 裁决注入面：会话/命令/草稿/查询/任务/当前性/策略摘要/名称解析/交互回调；接口形状随 UI-T03+ 各消费任务在其 §10 对应节实现冻结并增量修订登记，L5 适配；C-4 命令网关 IUiCommandGateway 随 UI-T06 冻结；C-12 阶段门控 IUiStageGate＋域就绪源 IUiDomainReadinessSource 随 UI-T09 冻结——P-UI-6 单侧冻结）
  UiProjections.hpp     值投影承载（StatusWordProjection〔§6.3〕及会话/元数据/任务/当前性等投影——§3.2 冻结基准的 ui 侧载体；C-4 CommandEnvelopeProjection/CommandResultProjection 随 UI-T06 增量）
  ...
  UiText.hpp              文案键→本地化文本解析（UX-02 工程用语唯一出口；UI-T09 落位：resolveText 键解析＋位置参数替换纪律＋formatQuantityText 数值带单位〔core displayValueIn 唯一换算入口〕＋notApplicableText 占位＋ensureNoInternalIdentity 哈希形态守卫＋registeredTextKeys 键盘点面＋内建中文过渡文案表——键冻结值过渡，§16.7 v1.1）
```
...
- **文案键（TextKey）**：`cmd.<id>.title`、`stage.<id>.title`、`state.<token>.label`、`diag.<code-lower>.title/detail`（diagnostics 码表键约定）等；键在本文与各码表冻结，**值（中英文资源）归 ui 文案资源文件**（diagnostics.md P-DIAG-9 交接）。`UiText::resolve(TextKey, params)` 是唯一出口；界面禁止出现哈希、Schema 版本号、内部插件名（UX-02）。（UiText 已随 UI-T09 落位——`UiText.hpp`：内建中文过渡表承载值源〔键不变，迁资源文件只换值源〕＋参数替换纪律＋数值带单位/不适用占位＋哈希形态守卫；`statusWordTransitionalLabel` 等三过渡函数值源已切换至 UiText、键与值逐字不变。§16.7 v1.1。）
