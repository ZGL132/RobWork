...
| --- | --- |
| 文档代号 | UNIT-UI |
| 文档版本 | v1.0（WP-10-T08 阶段 B 落位登记：参数表与表单公共件——FormEditCommon.hpp 公共契约头〔ParamEditModel 编辑会话模型＋IFormEditOutlet 编辑出口＋KIN-12 显示投影纯函数＋advancedPanelDefaultFields〕＋ParamTablePanel.cpp 参数表面板、§12.3 UI-FRM-1 用例行、§14.1 承接行更新、§16.7 v1.0 行；v0.9＝WP-10-T07 阶段 A 落位登记、v0.8＝WP-10-T06 落位登记、v0.7＝WP-10-T05 阶段 A 落位登记、v0.6＝WP-10-T04 落位登记、v0.5＝WP-10-T03 落位登记、v0.4＝O-31 裁决落地、v0.3＝WP-10-T02 落位登记、v0.2＝全链一致性审计消账、v0.1＝首版草案） |
| 日期 | 2026-09-21 |
...
ui/include/sdurws/ird/ui/          命名空间 sdurws::ird::ui
  UiTypes.hpp            命令设施基础词表（CommandId/TextKey/MenuPath 别名——§7.1；UI-T06 落位，其余条目随归属任务增量）
  UiPorts.hpp           C-3/4/5/7/8/10/11 最小端口接口（O-31 裁决注入面：会话/命令/草稿/查询/任务/当前性/策略摘要/名称解析/交互回调；接口形状随 UI-T03+ 各消费任务在其 §10 对应节实现冻结并增量修订登记，L5 适配；C-4 命令网关 IUiCommandGateway 随 UI-T06 冻结）
