...
| --- | --- |
| 文档代号 | UNIT-UI |
| 文档版本 | v1.15（WP-24-T03 首版装配落位登记——IPluginUiRegistrar/IPluginUiModule 两头落位〔P-MDL-8 消账〕＋宿主插件域装配序列＋建模真宿主挂位＋UiText 域命令键族＋PluginAssemblyReport 迁移；见 §3.3/§10.9/§11/§16.7 v1.15 行）；v1.14-r2（WP-10-T18 返工登记〔验收 attempt 1 fail——阻断 B-1/B-2，记录 traceability/acceptance/UI-T18-20260924.md @ acc/UI-T18/1；attempts.fix=1〕：B-1 范围越界修复〔仓库根 traceability/ 3 件 r2 代 XML 删除〕＋B-2 宿主 GUI 冒烟补录执行〔--rwsplugin 直载两轮装载：多 Dock 拓扑＋宿主状态栏唯一＋PM-11 投影＋瞬态消息实拍＋PM-14 隐藏重载保持；截图 9 帧＋双轮 console/dev 日志＋驱动工具入库 host-smoke-r4-README.md；F1 瞬态未确证等诚实缺口如实登记〕＋建议级 F-332/F-333/F-334 顺带消账〔§16.7 v1.14 行口径勘误＋三文件头随 UI-T18 增量＋钳制注释表意对齐＋死包含删除〕＋返工后全量重验 -r4 全新捕获〔双模式零错误＋154/22/35＋gates 63↔63 差集 0＋validate-task PASS〕——详见 §13 UI-T18 落位登记注增补 v1.14-r2＋§16.7 v1.14-r2 行）；v1.14（WP-10-T18 落位登记：Dock 拓扑拆分＋状态行迁移宿主状态栏〔O-43 ③承接〕——IWorkbenchContent 契约面变更〔statusBarWidget() 出口移除→双状态观测钩子：PM-11 永久文本＋瞬态消息〕＋插件多 Dock 拓扑〔主 Dock＝命令条＋项目导航；属性/任务同级 Dock 装载自证拍入宿主〕＋harness 自有状态栏等价迁移〔呈现行为逐项等价〕——详见 §10.1 v1.14 增量注＋§13 UI-T18 行＋落位登记注＋§16.7 v1.14 行；宿主 GUI 冒烟未执行如实登记，随验收段补录）；v1.13-r4（WP-10-T17 宿主 GUI 冒烟执行登记〔所有者人工操作 2026-09-24 14:32~14:44〕：新建项目→保存草稿→关闭取消/确认→重开被 S2 候选验证拒绝全序执行；磁盘落盘＋dev 日志 `draining entered` 留痕＋锁 pid/心跳佐证；对话框/状态行过程视觉帧未捕获〔连拍错过＋所有者裁决"不用"不重走〕如实登记，B-1 裁定权交验收段 attempt 2——详见 §13 UI-T17 落位登记注增补 v1.13-r4）；v1.13-r3（WP-10-T17 返工登记〔验收 attempt 1 fail——阻断唯一项 B-1〕：F-326 两文件头注释对齐修复〔纯注释零行为〕＋F-327 延期裁决〔零 CMake/链接面变更红线〕＋F-326 后全量重验 -r3 全新捕获〔GUI 34/35 中 1 失败＝桌面剪贴板被锁的环境干扰，三重证据如实登记〕＋宿主 GUI 冒烟闭环取证工具就位、所有者裁决稍后指定时间〔仍未执行如实登记〕——详见 §13 UI-T17 落位登记注增补 v1.13-r3）；v1.13-r2（WP-10-T17 落位登记增补——证据归位更正〔根目录→doc 规范路径〕＋ird_gates detached worktree 基线全量归一化比对〔94=94 双向差集 0〕＋validate-task PASS＋宿主 GUI 冒烟所有者裁决随验收段补录〔自动化取证面移交〕——详见 §13 UI-T17 落位登记注增补）；v1.13（WP-10-T17 落位登记：宿主菜单融合＋会话入口处理器补全——WorkbenchContentDeps 覆写面扩展落位＋插件侧真实编排（关闭统一确认对话框/Draining 轮询驱动/DraftController 保存链路）＋宿主 File/Tools/视图菜单融合——详见 §13 UI-T17 落位登记注＋§16.7 v1.13 行；宿主 GUI 冒烟未执行如实登记，随验收段补录）；v1.12（UI-T17 立项登记——宿主菜单融合＋会话入口处理器补全（O-43 裁决承接，DTB v0.19 §4.2/§2.11 同日登记）：§13 新增 UI-T17 行＋立项登记注＋§16.7 v1.12 行；契约面增量（WorkbenchContentDeps 覆写面扩展至 draft.save/workbench.closeProject）随实现落位登记）；v1.11（WP-10-T16 返工登记：宿主装载呈现缺陷修复——框架 loadPlugin 对话框路径硬编码 visible=false＋addPlugin 尾段 restoreState 隐藏状态 blob 未登记 Dock（两步均在插件 initialize 之后，SA-02 框架零修改），插件侧零等待单发重申嵌入式呈现（reassertEmbeddedPresentation）＋G1 装载门控增第二判据＋宿主交互证据全量重录（G1~G4 客观门控全过）＋装载环境前置登记（PATH 需求清单＋装载门控判据——S-3/F-320）＋B-2 失实复跑载体删除更正——详见 §13 UI-T16 落位登记注⑤勘误＋UI-T16 返工登记注＋§16.7 v1.11 行）；v1.10（WP-10-T16 落位登记：工作台宿主插件——`sdurws_ird_ui_plugin` 集成树专属目标（RobWorkStudioPlugin 派生薄插件、零计算逻辑、AUTOMOC 仅该目标显式开启）＋壳拆「内容装配层（WorkbenchContent.hpp 公共契约面）＋顶层窗口宿主层（WorkbenchShell 收敛为 chrome）」两层（harness 与插件共用同一内容装配面，行为回归零变化）＋嵌入式宿主形态的布局记忆旗标半区（LayoutMemory::loadFlags/storeFlags）＋会话入口处理器覆写面（§11.5）＋三维让位页（O-38 裁决③）——详见 §10.1 v1.10 增量注＋§3.3 增列＋§13 UI-T16 落位登记注＋§16.7 v1.10 行；v1.9（UI-T16 立项登记：工作台宿主插件——`sdurws_ird_ui_plugin` 开发期 RobWorkStudio 集成入口＋壳内容装配层/顶层窗口宿主层拆分授权＋宿主模式 RWStudioView3D 共存最小接入（O-38 裁决承接）——详见 §13 UI-T16 行＋UI-T16 立项登记注＋§16.7 v1.9 行；v1.8（WP-10-T15 返工登记：诊断桥对端记录拒绝显式纪律＋打开编排兜底错误页——ProjectDiagnosticsBridge::report 的 create 拒绝两分组处置〔装配缺陷 fail-fast 上抛／对端契约缺口 Dev 具名上报后协议继续〕＋HarnessMain 打开编排 try/catch 兜底〔exit 5 可观测错误页〕＋v0.1 诚实边界增补〔PRJ-* 项目级事件条目未入目录〕＋PM-07 双可写意图实例复测——详见 §13 UI-T15 落位登记注勘误＋UI-T15 返工登记注＋§16.7 v1.8 行；v1.7（WP-10-T15 落位登记：工作台验证 harness——`sdurws_ird_ui_app` 开发期 L5 装配雏形〔app/PortAdapters 端口适配器族＋HarnessMain 装配序列〕＋ui/CMakeLists 目标登记〔_app 后缀首次启用〕＋§13 UI-T15 行＋落位登记注＋§16.7 v1.7 行；v1.6＝WP-10-T14 落位登记：ui 契约测试套件——详见 §16.7 尾注与本节 §13 UI-T14 行；v1.5（WP-10-T13 落位登记：诊断/确认/任务呈现模型与 CommandInteractionBridge——IDiagnosticPresentationModel.hpp 诊断呈现模型〔§9.1/§10.8：查询/原因链/动作映射/三要素同显/恢复横幅 PM-15/Tier-U 面板/脱敏转发〕＋ITaskPresentationModel.hpp 任务呈现模型〔§9.4/§9.5/§10.7：清单双区＋进度三规则＋能力启用＋强杀独立确认＋归档子标＋当前性徽标〕＋CommandInteractionBridge.hpp 确认交互桥〔§9.2/§9.3：Marshal＋阻塞等待＋isAlive 探针＋principal 缓存〕＋C-6/C-8/C-9 呈现面端口首消费冻结〔IUiCommandInteraction/IUiTaskPresentationPort/IUiFindingQueryPort/IUiUserLogSource/IUiRedactionPolicyPort〕＋任务投影族〔TaskProgressProjection/TaskViewProjection/UiTaskAck/UserLogEntry〕＋UiText 键族⑧＋§13 UI-T13 落位登记注＋§16.7 v1.5 行；v1.4＝WP-10-T12 落位登记：DraftController＋草稿写半区端口——IDraftController.hpp 草稿控制器〔§8 保存/应用分离＋未保存标记＋定时保存＋打开恢复＋Stale 冲突＋两栈〕＋IUiDraftStorePort（C-5 写半区）＋草稿投影族（DraftDocumentProjection 等＋StaleRevisionDetailProjection 随 CommandResultProjection.staleDetail）＋DraftRowProjection 增列＋UiSessionController.reportSessionDirty 生产者接线点＋§10.5 实现冻结登记注＋§13 UI-T12 落位登记注＋§16.7 v1.4 行；v1.3＝WP-10-T11 落位登记：UiSessionController＋关闭/切换对话框机制——UiSessionController.hpp 会话状态机编程入口〔§5.2 七态＋INV-SES-1~4＋§5.6 四级防线，T_force/T_force2 按 §5.6 默认保守值装配期可配——P-UI-8〕＋C-3/5/8 会话端口首消费冻结〔IUiStoreFactoryPort/IUiProjectStorePort/IUiDraftQueryPort/IUiSessionTaskPort＋SessionPortBundle〕＋会话投影族〔打开结果/锁持有者/只读横幅/草稿行/任务行/统一确认对话框数据与决议〕＋UiTypes 会话词表〔UiSessionState/UiOpenMode/UiCloseIntent〕＋§13 UI-T11 落地登记注＋§16.7 v1.3 行；v1.2＝WP-10-T10 落位登记：帮助入口与关于对话框——AboutDialog.hpp 呈现契约〔静态白名单∩装配报告清单＋冻结版本基线投影＋手册入口〕＋IUiAboutDataSource 端口＋ShellWiring.aboutSource 增量＋§10.1/§11.4/§12.3 落地登记注＋§16.7 v1.2 行；v1.1＝WP-10-T09 落位登记：阶段状态投影与工程用语——IStageNavigationModel.hpp 阶段导航投影＋UiTypes 阶段值类型/§6.5 汇聚投影＋UiText.hpp 文案体系＋C-12 阶段门控/域就绪源端口〔P-UI-6 单侧冻结〕＋§3.3/§3.5/§6.4/§6.5 落位登记注＋§16.7 v1.1 行；v1.0＝WP-10-T08 阶段 B 落位登记：参数表与表单公共件——FormEditCommon.hpp 公共契约头〔ParamEditModel 编辑会话模型＋IFormEditOutlet 编辑出口＋KIN-12 显示投影纯函数＋advancedPanelDefaultFields〕＋ParamTablePanel.cpp 参数表面板、§12.3 UI-FRM-1 用例行、§14.1 承接行更新、§16.7 v1.0 行；v0.9＝WP-10-T07 阶段 A 落位登记、v0.8＝WP-10-T06 落位登记、v0.7＝WP-10-T05 阶段 A 落位登记、v0.6＝WP-10-T04 落位登记、v0.5＝WP-10-T03 落位登记、v0.4＝O-31 裁决落地、v0.3＝WP-10-T02 落位登记、v0.2＝全链一致性审计消账、v0.1＝首版草案） |
| 日期 | 2026-09-24 |
...
ui/include/sdurws/ird/ui/          命名空间 sdurws::ird::ui
  ...
  WorkbenchContent.hpp   §10.1 v1.10 工作台内容装配面（UI-T16 落位：壳拆「内容装配层＋顶层窗口宿主层」——WorkbenchHostKind 宿主形态词表〔顶层窗口＝harness／嵌入式 Dock＝宿主插件〕＋WorkbenchContentDeps 注入包〔ShellWiring 转发＋窗口几何/位形承载钩子＋会话入口处理器覆写〕＋IWorkbenchContent 两段装配〔build/activate〕＋五区内容 Widget 出口＋可见性目标登记＋上下文投影/命令/最近项目转发＋观察钩子；工厂 createWorkbenchContent——harness 与宿主插件共用的同一装配面，O-38 裁决②）
  ...
  IPluginUiRegistrar.hpp §10.9 插件界面注册端口（WP-24-T03 首版装配落位：PanelRegistration/PluginCapabilities/PluginUiDescriptor/RegistrationOutcome 四形状＋PluginAssemblyReport〔自 AboutDialog.hpp 迁移——v1.2 预注兑现，NFR-MNT-03 单点〕＋IPluginUiRegistrar 三方法＋createPluginUiRegistrar 工厂〔白名单构造期固定——§11.1〕＋实现 src/PluginUiRegistrar.cpp〔校验序：白名单→重复→描述符→Ok；失败隔离 §11.3〕；CommandDescriptor/IPluginUiModule 前向声明断环〔ICommandRegistry→IWorkbenchShell→UiPorts→AboutDialog 既有链经本头回引即 C2065——ui/src 实现与装配 TU 自含完整类型〕，§16.7 v1.15）
  IPluginUiModule.hpp   §11.2 插件界面模块接口（WP-24-T03 首版装配落位：onShellReady/readonlyProjections/buildDraftCommand 三方法冻结签名；project::CommandEnvelope 前向声明——ui 头零 project include〔NoCrossUnitInclude_O31_UI_BUILD〕；IWorkbenchShell 前向声明断环；P-MDL-8 消账载体——modeling ModelingUiModule 同形→继承切换，§16.7 v1.15）
...
>
> **v1.10 增量冻结（WP-10-T16；壳拆「内容装配层＋顶层窗口宿主层」——O-38 裁决②的契约面落位，下述与实现文件逐字一致）**：
> ①**两层拆分**：IWorkbenchShell 门面（本节签名零变化）收敛为**顶层窗口宿主层**——只承载顶层 QMainWindow 的窗口 chrome（§4.1 六菜单、五区 Dock 包裹、中央区/状态行安放、出厂几何/位形快照、resize 折叠转发）；一切界面语义（五区内容/命令设施/布局记忆编排/上下文投影/最近项目）移入**内容装配层**，公共契约面＝`WorkbenchContent.hpp`（IWorkbenchContent＋createWorkbenchContent——实现类封闭库内，R-2 同 createWorkbenchShell 惯例）。门面方法逐条转发内容装配层，§10.1 契约表（前置/后置/错误/线程/所有权）零变化——观测方零迁移；
> ②**两段装配时序**：内容装配层拆 build()（内容构建＋§7.1 命令设施装配）→宿主层安放 chrome（Dock 包裹/可见性目标登记/观察钩子）→activate()（布局记忆装载＋PM-10 初始上下文＋写线程＋快捷键 attach＋命令面板）三步——原 initialize 装配序的等价重排（步骤内容逐行对应，harness 行为回归零变化为验收硬项，UI-WB-1/2/3＋PM-10/PM-11/PM-14 既有 GUI 套件全量回归通过）；几何/位形承载钩子（save/restoreWindowGeometry/State）由顶层宿主提供＝布局记忆全量五键（UI-T03 以来逐字节一致）；
> ③**嵌入式 Dock 宿主形态**（WorkbenchHostKind::EmbeddedDock——sdurws_ird_ui_plugin）：宿主控件＝插件 Dock 体（栅格安放五区＋状态行），**零顶层 QMainWindow**（O-38 裁决②反模式禁令）；布局记忆只承载三区可见性键（同组同键跨形态共享用户级设置——PM-14；几何/位形属框架主窗口不读写，LayoutMemory 增 loadFlags/storeFlags 旗标半区；损坏处置沿 §4.5 同纪律：回退默认＋UI-LAYOUT-RESTORE-FAILED（Dev）＋损坏段整段丢弃）；§4.4 尺寸折叠随形态关闭（该规则量测顶层窗口尺寸，Dock 面板量测对象不同＝形态误配；用户手动开关三区不受影响）；
> ④**会话入口处理器覆写**（§11.5"触发时机编排归装配层"的注入面）：WorkbenchContentDeps 增可选 newProjectHandler/openProjectHandler——宿主插件注入真实打开编排（文件对话框→UiSessionController）后，§7.1 壳入口五处（首页/菜单/顶栏/面板/快捷键）同走真实协议；覆写只发生在装配期登记之前，描述符/谓词/登记序/门控零变化（§7.2 静态白名单无运行期改写）；未注入＝§7.1 阶段 A 占位说明处理器（harness 形态不变）；
> ⑤**三维让位页**（O-38 裁决③"让位形态"）：嵌入式宿主中央区页 1＝让位页（ird_view3d_host_yield——声明三维视图由宿主主窗口承载，框架 RWStudioView3D 与工作台 Dock 共存；纯 QLabel 零交互控件——§11.4 不虚构能力），顶层窗口宿主维持 UI-T05 阶段 A 占位面板形态不变；完整三维交互仍归 WP-10-T05 阶段 B；
> ⑥**观察钩子**：setCommandStateObserver（§7.6 三处一致禁用的宿主菜单半区同步）＋setTitleTextObserver（PM-11"标题栏与状态栏"双面的标题半区——顶层宿主转发 setWindowTitle；嵌入式宿主可不注册）。
>
> **v1.14 增量注（WP-10-T18；Dock 拓扑拆分＋状态行迁移宿主状态栏——O-43 裁决③的契约面落位，下述与实现文件逐字一致）**：
> ⑦**状态出口契约面变更**：IWorkbenchContent 移除 `statusBarWidget()`（QStatusBar\* 出口）——内容装配层不再持有任何状态栏 Widget；代之以双状态观测钩子 `setStatusTextObserver`（PM-11 永久文本投影——formatProjectStatusText 唯一权威现取现投，与 setTitleTextObserver 同源同格式）＋`setStatusMessageObserver`（瞬态消息投影——原 QStatusBar::showMessage 语义原样移交，文本＋超时 ms 零加工）。顶层宿主（WorkbenchShell）自有 QStatusBar 承载（永久位 QLabel＋showMessage 转发——呈现行为与原"内容装配层持有同一 QStatusBar"逐项等价）；嵌入式宿主（插件）直投 getRobWorkStudio()->statusBar()；未注册观察者＝无投影（无宿主测试场景，同 setTitleTextObserver 纪律）。调用面收敛：内容层 6 处瞬态消息点统一走 showStatusFeedback 私有路由（判空收口点唯一）；
> ⑧**多 Dock 拓扑**（O-43 ③"单一工作台 Dock 五区栅格拆分"）：插件本体 Dock＝主 Dock（Left 停靠区，命令条＋项目导航纵排）＋"IRD 属性与诊断"Dock（Right 区＝右栏内容）＋"IRD 任务和状态"Dock（Bottom 区＝底部内容）——右/底两 Dock 由插件创建（父对象＝插件）、装载呈现自证拍 addDockWidget 入宿主主窗口（插件本体在 addPlugin 尾段才入主窗口，彼时宿主窗口方可寻址；同受宿主装载语义支配——restoreState 对状态 blob 未登记 Dock 按隐藏的根因链与 v1.11 主 Dock 同链，故一并重显＋尺寸初值：右＝宿主宽约 1/5 钳制 [300,480] px、底＝宿主高约 1/4 钳制 [190,340] px，此后归用户拖拽）；重显后按内容装配面模型位重施右/底区域旗标（PM-14——用户跨会话隐藏不被装载重显夺回，可经宿主"视图"菜单重新开启；主 Dock 不重施——其装载呈现维持 v1.11 注册口径"G1 门控判据装载即呈现"）。中央让位页保持已构建不挂载（宿主中央 RWStudioView3D 唯一所有三维）；三区可见性目标登记改绑 Dock 本体（左＝插件主 Dock、右/底＝同级 Dock——区域开关语义作用面从内容 Widget 升格为 Dock；Top/Central 仍契约无操作——顶栏随主 Dock、中央归宿主）；布局记忆三区键不变（PM-14 同组同键跨形态）；§4.4"状态行无隐藏入口"在多 Dock 形态的等价保障＝宿主状态栏本身恒可见（强于 v1.11 的 Dock 内钉底——任何 Dock 开关均不影响状态投影；v1.11 钉底修复的语义等价迁移逐项对照见 §13 UI-T18 落位登记注）。
...
| UI-T17 | 会话入口处理器补全＋宿主菜单融合（O-43 裁决承接） | UI-T11、UI-T12、UI-T16 | 宿主 File/Tools 菜单可见项目命令（使能态随命令可用性快照刷新）；关闭项目走 §5.4 统一确认对话框（草稿/任务两轴决议）＋Draining 轮询有界收口；保存项目＝saveAll(Manual) 真实落盘链路；Plugins 菜单仅留基类显示开关；harness 行为回归零变化；ird_gates 命中集增量登记；留痕 traceability/builds/wp10-t17/ | O-43 ②承接；菜单只路由命令板（§4.2）；saveAs/applyDraft/undo/redo 维持 §11.4 阶段 A 占位（真实接线归 workflow/project 命令网关装配任务）；Dock 拆分与状态行迁移归 UI-T18（O-43 ③） |
| UI-T18 | Dock 拓扑拆分＋状态行迁移宿主状态栏（O-43 裁决③承接） | UI-T16、UI-T17 | 插件呈多 Dock 拓扑（主 Dock＝命令条＋项目导航；"IRD 属性与诊断"右 Dock；"IRD 任务和状态"底 Dock）且宿主中央 RWStudioView3D 保持宿主所有；宿主状态栏唯一（插件零 QStatusBar——PM-11 永久文本＋瞬态消息经双观测钩子直投宿主）；布局记忆三区键跨会话有效；harness 自有状态栏等价迁移（呈现行为逐项等价）；ird_gates 命中集零新增；留痕 traceability/builds/wp10-t18/ | O-43 ③承接（高风险重构单独立任务）；IWorkbenchContent 契约面变更随落位登记（§10.1 v1.14 增量注⑦⑧）；v1.11 状态行钉底修复的语义等价迁移逐项对照登记；宿主 GUI 冒烟随验收段补录 |
...
>
> **UI-T16 落位登记注（v1.10，2026-09-23）**：①**目标落位**＝`sdurws_ird_ui_plugin` MODULE（集成树专属——独立冒烟模式无框架目标 sdurws，以 `if(TARGET sdurws)` 判别不注册；冒烟模式零错误结论不含本目标，如实登记）；插件源集＝`ui/plugin/UiPlugin.hpp/.cpp`（IrdWorkbenchHostPlugin：Q_OBJECT＋Q_INTERFACES＋Q_PLUGIN_METADATA(IID "dk.sdu.mip.Robwork.RobWorkStudioPlugin/0.1" FILE plugin.json)）＋`app/PortAdapters.*`＋`app/DiagnosticsAssembly.*`（**诊断栈装配自 HarnessMain 抽出为两宿主共用单一序列**——DiagnosticsStack/assembleDiagnostics，语义逐行自 UI-T15 搬移，harness 行为零变化）；链接面＝ui＋project（DTB §4.5 新登记 SUB 行）＋框架 sdurws＋Qt 三件套；AUTOMOC 仅插件目标显式 ON（集成树 RobWorkStudioSetup 全局 AUTOMOC=ON 对既有目标本就生效且零 Q_OBJECT 无 moc 产物——构建形态零变化；冒烟树默认 OFF，per-target 显式 ON 使两模式口径一致）；②**壳拆分落位**＝§10.1 v1.10 增量注逐项（WorkbenchContent.hpp 契约面＋WorkbenchShell 收敛 chrome＋两段装配 build/activate＋嵌入式旗标半区记忆＋§4.4 折叠随嵌入式形态关闭＋会话入口覆写面＋三维让位页 ird_view3d_host_yield＋命令状态/标题观察钩子）；harness 行为回归零变化实证＝UI-WB-1/2/3＋UI-SES-1＋PM-10/PM-14＋UX-11 既有 GUI 套件全通过＋harness 首页态/--new/--open 打开协议/PM-07 双开降级交互复测留痕（traceability/builds/wp10-t16/harness-*.png/.log）；③**测试落位**＝sdurws_ird_ui_gui_test 增 WorkbenchContentGuiTest.cpp 六用例（UI-SPLIT 具名族：两段装配契约/装配校验拒绝/嵌入式旗标半区边界＋跨实例恢复/损坏回退 Dev 诊断/会话入口覆写路由/三维让位页零交互）；④**门禁**＝恰增 SUB×2（插件登记面）＋R3×2（ui 例外源集文件级延伸——WorkbenchContent.cpp/_p.hpp，wp10-t03 §1 请求＋wp10-t13 先例同型），登记提交件 traceability/wp10-t16-gate-registrations.md；⑤**宿主交互验证**＝框架 RobWorkStudio.exe Plugins→Load plugin 手动装载冒烟，留痕 traceability/builds/wp10-t16/host-*.png/.log（**勘误（v1.11）**：本注初版所列"五区嵌入/PM-11 投影/布局记忆/恢复默认布局"表述强于该批留痕实证——宿主形态集成缺陷，验收 attempt 1 判 acceptance 3 不成立（B-1）；经返工修复与证据全量重录后方成立，根因链与修复见返工登记注——动态加载边界登记维持不变：仅开发期验证通道，SA-01 静态白名单与 WP-24-T03 正式装配口径不变）。

> **UI-T16 返工登记（v1.11，2026-09-24）**：验收 attempt 1 verdict=fail（阻断 B-1/B-2，记录 traceability/acceptance/UI-T16-20260924.md @ acc/UI-T16/1）返工承接——①**B-1 根因链**＝嵌入式宿主形态缺陷：框架对插件 Dock 可见性的两步处理均在插件 initialize() 返回**之后**执行——⑴ Plugins→Load plugin 对话框路径硬编码 visible=false（RobWorkStudio.cpp `loadPlugin()`→`setupPlugin(pathname, filename, 0, 1)`→addPlugin 尾段 `setVisible(PluginVisible_<名> 缺省实参)`）；⑵ addPlugin 尾段 `restoreState(QtMainWindowState)`——Qt 对主窗口状态 blob 未登记的 Dock 按隐藏处理（blob 为装载前会话保存，本 Dock 必不在其中）——五区面板/状态行随 Dock 整体不可见（Plugins 菜单区域开关勾选态是内容装配面内存态，故与实际显示自相矛盾；打开/新建对话框以隐藏 Dock 体为父，自动化粘贴确认不可靠——G2/G3 未过的同链缺陷）；②**修复**＝插件侧装载呈现自证（UiPlugin initialize 末尾零等待单发定时器→`reassertEmbeddedPresentation()`：`setFloating(false)`＋`show()`——事件循环回归后第一拍执行，队列语义保证严格晚于⑴⑵；**单发**不与用户后续手动开关竞争〔Plugins 菜单基类开关/Dock 关闭钮可再隐藏，插件不夺回〕，区域级可见性记忆仍归内容装配面 §4.5——SA-02 框架零修改）；G1 装载门控增**第二判据**＝控制台行"[ird-ui-plugin] 工作台 Dock 已呈现（宿主主窗口嵌入式——装载呈现自证完成）"＋Dev 日志"装载呈现自证完成"；initialize 收尾行同步改述"工作台装配完成（…装载呈现自证在事件循环稍后执行）"（不提前声称已呈现——attempt 1 教训）；**共存形态收口**＝呈现后 resizeDocks 把 Dock 宽度收束到宿主客户宽约 2/5（下限 420 px、上限 1024 px；受内容最小宽度钳制时事实落 Dev 日志——本机实测钳制宽 1054 px）——装载序列中 addDockWidget 先按停靠区整幅宽给位、重显即恢复整幅宽会挤压宿主中央 RWStudioView3D 为零（attempt 2 首录截图实证），收束后两能力同帧共存；**状态行钉底**＝Dock 体改外层纵排（五区栅格持 stretch 先行吸收竖向短缺＋状态行恒高钉底）——宽度收束至内容最小宽时右栏文本换行加高曾把栅格末行（状态行）整体挤出视口（attempt 2 二录截图实证：Dock 底缘空白条带、PM-11 投影不可见），纵排拆分后 PM-11 投影恒可见（§4.4"状态行无隐藏入口"的嵌入式保障）；装载几何事实一次性落 Dev 日志（[geometry] 行——排障面）；③**宿主交互证据全量重录**＝同一操作序列（G1 装载门控〔两判据〕→命令面板→打开协议 G2 锁心跳刷新→新建协议 G3 目标目录落盘→左栏开关→跨会话 G4 布局记忆→恢复默认布局→PM-11 状态投影），客观门控全过＋逐帧截图，traceability/builds/wp10-t16/（attempt 1 缺陷证据与收尾更正登记原样保留）；④**装载环境前置登记**（S-3/F-320）＝宿主动态装载本插件的 PATH 前置清单：框架 exe 目录（`build/RobWorkStudio/bin/Release`——sdurw_*.dll/sdurws.dll/boost/Qt 随构建就位，进程自身目录默认在 DLL 搜索路径）＋vcpkg `installed/x64-windows/bin`（boost_filesystem-vc143-mt-x64-1_92.dll 等——插件 DLL 直接导入表面，objdump 实扫）＋Qt bin（Qt6Core/Gui/Widgets/OpenGLWidgets）＋Python 目录（python313.dll——sdurw 链传递依赖，attempt 1 异机装载三连败实测缺口，本机 `D:\software\Miniconda3`）；**装载门控客观判据**＝控制台 `[ird-ui-plugin]` 前缀行（诊断栈就绪→工作台装配完成→工作台 Dock 已呈现）＋CWD 下 `ird-ui-plugin-logs/dev-diagnostics.log` 生成——二者皆无＝装载未发生（QPluginLoader 静默失败形态，异机复现先按本清单补 PATH 再判定）；⑤**B-2 更正**＝v1.10 收尾提交（6441c136）验证段"独立复跑实录"实为既有日志字节级拷贝（-r2 两件，且内容与提交信息矛盾）——失实载体随返工删除，复跑结论一律以带 ctest"Start testing"时间戳头的全新捕获为准（防再现记忆点：复用旧日志必须显式标注"同轮留存副本"）。需求语义零变化。

> **UI-T17 立项登记（v1.12，2026-09-24）**：O-43 裁决（DTB §4.2，所有者 2026-09-24 融合方案审核指令）承接——宿主菜单融合＋会话入口处理器补全（§2.11 WP-10-T17，≙UI-T17）：①**会话入口覆写面扩展**＝WorkbenchContentDeps 增 `saveProjectHandler`（draft.save）/`closeProjectHandler`（workbench.closeProject）两字段——§11.5"触发时机编排归装配层"既有模式同型（覆写只换处理器，描述符/谓词/登记序零变化；未注入＝§7.1 阶段 A 占位，harness 形态不变）；project.saveAs/draft.apply/project.undo/project.redo 维持阶段 A 占位（真实接线归 workflow 向导编排/project 命令网关装配任务——§14.1 归属不变）；②**插件侧真实编排**＝关闭项目走 §5.4 统一确认对话框（beginClose→对话框呈现→resolveCloseDialog；草稿/任务两轴决议的呈现面由装配层以 QDialog 承载——机制归控制器、呈现归装配层，与 §5 注释分工一致）＋Draining 轮询驱动（UI 线程 QTimer 周期 pollDrain；ForceConfirmDue→强制结束确认呈现→resolveForceCloseDialog；GivenUp/ClosedNow 收口）＋saveAllDraftsManual 接线（DraftController 装配：串行落盘执行器＋postToUiThread Marshal＋onSessionDirtyChanged→reportSessionDirty 生产者接线＋打开成功 bindSession/上下文清空 unbindSession——**保存链路真实**）；③**登记的诚实边界**＝草稿恢复编排（restoreOnOpen/恢复横幅呈现）、定时保存心跳、草稿清单真实适配（现 DraftQueryPortStub 恒空）与分支锚端口（SessionPortBundle/C-3 冻结面无分支查询——DraftSessionBinding.branchId 以缺省值承载且当前不可达：本任务无挂接模块，saveAll 收集零脏模块，store 写半区 save 翻译真实但 tryLoad/discard 的分支锚不可达）随完整草稿链路装配任务接续；④**宿主菜单融合**＝setupMenu 经 getRobWorkStudio()->menuBar() 定位宿主 File/Tools 菜单：File 内在 Preferences 分隔符前按位插入"工业机器人项目"子菜单（新建/打开/保存草稿/项目另存为/关闭项目＋最近项目子菜单——按位插入而非尾插：updateLastFiles 每次 open 后移除重加最近文件条目，尾插会被重排到子菜单之后）；Tools 追加命令面板；自建"视图"菜单承载三区开关＋恢复默认布局；Plugins 菜单仅留基类显示开关；使能态全部经既有 setCommandStateObserver→refreshHostMenuActions 消费命令可用性快照（本插件零判定）；⑤**不变项**＝装载方式（O-38 ①开发期通道）、生命周期正交边界（O-38 ③——open()/close() 保持观测留痕）、框架零修改（SA-02）、Plugins→Load plugin 通道；Dock 拓扑拆分与状态行迁移宿主 statusBar 归 UI-T18（O-43 ③，已立项未实施）。需求语义零变化。

> **UI-T17 落位登记注（v1.13，2026-09-24）**：本行已交付（分支 wp10-t17，待验收）——①**覆写面扩展落位**＝WorkbenchContentDeps 增 `saveProjectHandler`（draft.save）/`closeProjectHandler`（workbench.closeProject）两字段＋登记循环接线（§11.5 既有模式同型——覆写只换处理器，描述符/谓词/登记序零变化；未注入＝阶段 A 占位，harness 形态不变）；②**插件侧真实编排落位**＝关闭项目 `beginClose→presentCloseDialog（草稿/任务两轴 QDialog 呈现：无草稿呈现"无未应用修改"、无任务呈现"无后台任务"——零虚构；保存决议仅可写会话可选）→resolveCloseDialog→startDrainWatch/pollDrainOnce（UI 线程 QTimer 200 ms 周期驱动 §5.6 四级防线：ForceConfirmDue→强制结束确认呈现→resolveForceCloseDialog；GivenUp/ClosedNow 收口；logic_error 状态违约就地提示＋Dev 留痕不吞错）`；`saveAllDraftsManual` 接线＝`saveDraftsNow()`（DraftController saveAll(Manual)），`forceAbandonAll` 接线＝显式 Dev 留痕空兜底（无 execution 引擎装配＝零任务可放弃）；打开成功 `bindSession`（BundleCapturingStoreFactory 捕获包装——StorePortAdapter 双面适配器 dynamic_cast 取草稿写半区端口；切换流先 unbind 再绑）＋上下文清空 `unbindSession`（§8.6 表处置）；③**宿主菜单融合落位**＝`setupMenu` 仅留基类 Plugins 显示开关；`registerHostMenus` 经 `getRobWorkStudio()->menuBar()` 定位 File/Tools：File 内以 Preferences 前分隔符为锚 `insertMenu` "工业机器人项目"子菜单（新建/打开/保存草稿/项目另存为/关闭项目＋最近项目子菜单——aboutToShow 现取重建、失效项保留并禁用〔PM-10 失效≠删除〕）；Tools 追加命令面板；自建"视图"菜单（三区开关＋恢复默认布局，插入 Plugins 之前）；全部命令动作使能态经既有 `setCommandStateObserver→refreshHostMenuActions` 消费命令可用性快照（本插件零判定）；④**装配层增量**＝`app::SerialTaskExecutor`（§3.4 落盘线程开发期宿主——单工作线程串行队列、stop 有界排空）＋`StorePortAdapter` 增实现 `IUiDraftStorePort`（C-5 写半区真实翻译：save 分支由文档自带、tryLoad/discard 分支锚缺省值不可达——立项登记注③诚实边界维持）；⑤**测试落位**＝`WorkbenchContentGuiTest` 增 `SaveCloseEntryOverrideRoutesShellEntries_UI_SPLIT`（覆写路由＋项目作用域门控——无项目禁用/有项目可用并路由到点）；⑥**验证结果**＝集成模式六目标（ui/plugin/app/三测试）构建零错误；模型 154/154、契约 22/22、GUI 35/35 全过（gtest XML 留痕 traceability/builds/wp10-t17/）；ui 叶子目录 ctest `^ird$` 2/2、`^ird_gui$` 1/1 通过；独立冒烟模式全树构建零错误（build_smoke_t17，配置/构建日志同目录）；ird_gates 命中集归一化比对**零新增**（R3 唯一文件清单与 wp10-t16 基线 17 文件逐一一致、SUB/LIB/T 码零变化——本任务零 CMake/链接面变更；日志同目录）；**未执行如实登记**＝宿主 GUI 冒烟（RobWorkStudio.exe Plugins→Load plugin 交互序列——菜单位置/关闭确认流/保存链路的真机留痕）随验收段补录。需求语义零变化：PM-03/PM-04/PM-10/UX-09 承接语义按 §5/§7/§11.5 原文落位。

> **UI-T17 落位登记注增补（v1.13-r2，2026-09-24，实施派发会话承接收尾）**：流水线以 base=ac5d107（UI-T17 入队提交）派发实施段；因分支实现提交（c2c1e0a4）先于入队提交存在，实施会话以**合并 base 入分支**方式对齐派发基线（不改写已推送历史；merge 提交 656b7f30——契约文件两侧逐字节一致、产品文件零冲突），并对全部验证以**全新捕获（-r2 后缀）重录**（B-2 防再现纪律）。增补登记：①**证据位置更正**＝batch 1 将留痕置于仓库根 `traceability/builds/wp10-t17/`（allowedFiles 之外），r2 已整体迁入规范路径 `RobWork/doc/industrial-robot-design/traceability/builds/wp10-t17/`；batch 1 登记所称"配置/构建日志同目录"中 4 件日志实际未随提交入库（untracked），r2 已重跑并入库（smoke-configure-r2/smoke-build-r2——冒烟树实测需 `-DCMAKE_TOOLCHAIN_FILE` 绝对路径＋`-DCMAKE_PREFIX_PATH`/`Qt6_DIR` 指向真实 Qt 前缀，AGENTS §4.1 口径的 Qt 补充项）。②**ird_gates 比对升级为同引擎 detached worktree 基线全量归一化比对**＝head 94 条 ↔ base（ac5d107）94 条唯一命中（LIB×10/R3×22/R4×2/SUB×56/T1×4）逐条一致、双向差集为 0（gate-hitset-head/base/diff-r2 三件同目录；归一化规则＝GBK 解码＋行内多命中拆分＋扫描根剥离＋排序去重）——较 batch 1 的 R3 文件清单口径更强的零新增证据。③**测试与门禁 r2 复核全绿**＝模型 154/154＋契约 22/22＋GUI 35/35（gtest XML 三件＋ird-test-report 三件＋控制台日志）；ctest `^ird$` 2/2、`^ird_gui$` 1/1（ctest-ui-ird/gui.log）；validate-task PASS（validate-task-r2.log）；集成六目标构建零错误（integration-build-r2.log）。④**宿主 GUI 冒烟裁决**＝r2 构建了全自动取证面（`--ini-file` 的 `[Plugins/IRD]` 正斜杠节名自动装载＋键盘驱动＋逐帧截图脚本 host-smoke-automation.ps1）并实测启动至装载门控，确认所有者桌面正被其他活跃会话占用（probe 截图留证）后**终止执行**，所有者当场裁决**随验收段补录**（AskUserQuestion 应答 2026-09-24）；移交手册 HOST-SMOKE-HANDOVER.md 登记实测增补的 PATH 前置（`build/RobWork/bin/Release`——sdurw_pathoptimization.dll 等 RobWork 模块 DLL 所在目录，缺失＝0xC0000135，S-3/F-320 清单的实测修正）与执行手册；本项与 v1.13 原登记口径一致（未执行如实登记），交付面零变化。需求语义零变化。

> **UI-T17 落位登记注增补（v1.13-r3，2026-09-24，验收 attempt 1 fail 返工〔attempts.fix=1〕）**：返工输入＝不可变验收记录 `traceability/acceptance/UI-T17-20260924.md` @ acc/UI-T17/1（evidence 提交 c216d6c8）——阻断唯一项 **B-1**（宿主 GUI 冒烟的关闭确认流/Draining 收口/保存链路半区无宿主实证；实现代码面验收零整改要求）＋建议级 F-324~F-327。增补登记：①**F-326 修复**＝`plugin/UiPlugin.hpp` 与 `app/PortAdapters.hpp` 两文件头注释对齐 UI-T17 增量（@brief 任务口径 UI-T16→"UI-T16 立项＋UI-T17 增量"；背景说明补保存/关闭编排＋草稿链装配＋宿主菜单融合；PortAdapters 头"所有权在 HarnessMain/harness 未装配 DraftController"更新为"两宿主共用〔harness＝HarnessMain；插件宿主＝UiPlugin::initialize/assembleDraftChain〕＋落盘线程纪律自 UI-T17 起由插件宿主经 SerialTaskExecutor 真实兑现"）——纯注释零行为变更，类/函数级注释本就完备（attempt 1 §4.7 已核），AGENTS §2.5"注释随代码更新"消账。②**F-327 裁决＝延期登记**（未在本返工私裁）＝StorePortAdapter C-5 写半区翻译面＋SerialTaskExecutor 直测需给测试目标编入 `app/PortAdapters.cpp` 并增链 `sdurws_ird_project`——与本契约 acceptance 3 登记的"本任务零 CMake/链接面变更"直接抵触，且验收记录自身推荐"随草稿链路完整装配任务补齐"，返工不扩大登记面。③**F-326 后全量重验（-r3 全新捕获，B-2 纪律）**＝集成模式全量构建零错误（integration-build-r3.log）；独立冒烟模式新树 build_smoke_t17-r3 配置/构建零错误（smoke-configure/build-r3.log——vcpkg toolchain＋Qt 前缀同 r2 口径）；模型 154/154＋契约 22/22（gtest XML＋ird-test-report＋控制台 -r3 三件套，`--ird_report` 显式落点防同名互覆）；**GUI 34/35**——`ParamTablePanelGuiTest.BatchPasteImpactDetail_UI_T08_ACC1` 1 失败＝桌面剪贴板被进程持续锁定的环境干扰（三重独立证据：PowerShell Set-Clipboard 失败、clip.exe 拒绝访问、用例内"粘贴 0 行：接受 0 项、拒绝 0 项"）——UI-T08 既有用例、本返工产品面（纯注释）零触碰、r2 同二进制 35/35，所有者裁决"不管它，如实登记"（AskUserQuestion 应答 2026-09-24）；ctest `^ird$` 2/2、`^ird_gui$` 34/35（同前述用例）；ird_gates 同引擎 detached worktree 基线（ac5d107）比对＝head 55 ↔ base 55、**双向差集 0**（gate-hitset-head/base/diff-r3 三件；两日志各 119 条命中行与 r2 计数逐一相同；归一化粒度与 r2〔94〕/验收者〔151〕不同不影响差集结论——attempt 1 验收记录 §4.6 既有口径）；validate-task PASS（validate-task-r3.log）。④**宿主 GUI 冒烟（B-1 唯一阻断面）仍未执行——如实登记**：闭环取证工具已就位（host-smoke-r3-driver.ps1——原子动作制＋每步截图视觉审读后决定键鼠＋原生文件夹对话框鼠标点击『选择文件夹』＋退出后读 dev 日志，逐条落地 B-1 返工输入③⑤⑥与 `--rwsplugin` 装载通道①②）；所有者授权调度时裁决"**稍后指定时间**"（AskUserQuestion 应答 2026-09-24）——待所有者指定空闲窗口执行取证、入库后随全量证据发起验收请求 attempt 2（新记录路径 -r2 命名＋evidence 分支 acc/UI-T17/2）。需求语义零变化。

> **UI-T17 落位登记注增补（v1.13-r4，2026-09-24，宿主 GUI 冒烟执行登记〔所有者人工操作〕）**：宿主 GUI 冒烟（B-1 唯一阻断面）已由**所有者本人于 2026-09-24 14:32~14:44 在空闲桌面人工执行**——实施侧闭环驱动（host-smoke-r3-driver.ps1/-r4 补件）实测合成键鼠对 Qt 弹出菜单不可靠（子菜单点击漂移，attempt 1 五版脚本同族＝F-325 取证管线脆弱性再次实证，驱动尝试帧 host-smoke-r4-01/02/03 在案），所有者接手人工操作并裁决补拍"不用"。增补登记：①**执行序与所有者确证**＝新建项目（预建空目录 projtarget-r4＋显示名"冒烟项目r4"）→保存草稿→关闭项目取消/确认两路径→重开与再开尝试；所有者验证结论原话"我已经手动验证了，都正确可以通过验收"（2026-09-24 会话消息；时点帧 host-smoke-r4-04）。②**磁盘事实**＝项目库真实落盘（project.json projectId prj-4a387672…／createdAtUtc 2026-09-24T06:32:35.376Z＋HEAD rev-577cee08〔M0/main〕＋内容寻址对象＋project-init 修订 command/manifest）＋锁文件 pid=0000010956〔宿主进程〕心跳至 06:44:44Z〔宿主退出时刻〕——host-smoke-r4-store-metadata/store-listing/lock 三件在案。③**dev 日志**（辅证，进程退出后采样尾段落盘——F-325 口径）＝`draining entered: in-flight references=0`（关闭编排进入 Draining 的直接留痕）＋`switch rejected (candidate open failed): code=lock-held-by-other`（项目在本会话保持打开期间再次发起打开同一项目，S2 切换流候选验证按"候选验证成功才切"保守拒绝〔UiSessionController §5.4 S2 路径〕——该拒绝的产品语义属性〔同一项目重开的正确保守拒绝 vs 语义面待完善〕实施侧不裁定，登记事实交验收段与所有者裁量）——host-smoke-r4-dev-diagnostics-lastrun.log。④**诚实缺口（不粉饰）**＝关闭确认对话框/保存草稿状态行（"草稿已保存（0 个模块）"）/Draining 状态行（"正在关闭项目……"→"项目已关闭。"）的过程视觉帧未捕获——第一轮 12 分钟连拍先于操作过期、第二轮连拍期间所有者已裁决不重走；菜单位置半区（『工业机器人项目』位于 Reload 与 Preferences 之间）在 e917734a 二进制上刷新实证（帧 01/02/02b）。⑤**冒烟前预构建**＝e917734a 增量重建三目标零错误（integration-build-r4-prebuild.log——冒烟二进制与送验对象一致）。B-1 三行为（关闭确认流/Draining 收口/保存链路）的宿主执行以上述磁盘＋dev 日志＋锁 pid/心跳＋所有者确证为登记事实，视觉缺口随全部证据一并交验收段 attempt 2 独立裁定（证据索引 host-smoke-r4-README.md）。需求语义零变化。

> **UI-T18 落位登记注（v1.14，2026-09-24）**：本行已交付（分支 wp10-t18，待验收）——O-43 裁决③承接（DTB §2.11 WP-10-T18 行：高风险重构单独立任务；立项依据＝O-43 登记注＋本行 §13 行，立项/落位同轮登记）。①**契约面变更**＝IWorkbenchContent 移除 `statusBarWidget()`（QStatusBar\* 出口），增双状态观测钩子 `setStatusTextObserver`（PM-11 永久文本——formatProjectStatusText 唯一权威现取现投）/`setStatusMessageObserver`（瞬态消息——showMessage 语义原样移交，文本＋超时 ms 零加工）；内容层 6 处瞬态消息点统一收敛 showStatusFeedback 私有路由（判空收口唯一）；本层零状态栏 Widget（§10.1 v1.14 增量注⑦）；②**多 Dock 拓扑**＝插件本体 Dock＝主 Dock（Left 区：命令条＋项目导航纵排）＋新建"IRD 属性与诊断"Dock（Right 区）＋"IRD 任务和状态"Dock（Bottom 区）——两同级 Dock 由插件创建、装载呈现自证拍 addDockWidget 入宿主（与主 Dock 同受 restoreState 隐藏根因链支配，一并重显＋尺寸初值）；重显后按内容装配面模型位重施右/底区域旗标（PM-14——用户跨会话隐藏不被装载重显夺回；主 Dock 不重施，装载呈现维持 v1.11"G1 门控判据装载即呈现"注册口径）；中央让位页保持已构建不挂载（宿主中央三维唯一所有）；三区可见性目标改绑 Dock 本体（区域开关作用面升格）；布局记忆三区键零变化（PM-14 跨会话）（§10.1 v1.14 增量注⑧）；③**harness 等价迁移**＝WorkbenchShell 自有 QStatusBar（永久位 QLabel objectName 同名 ird_status_project_text＋stretch 1＋showMessage 转发）接双观测钩子——呈现行为与原实现逐项等价（harness GUI 套件全量回归通过为证）；④**v1.11 钉底修复语义等价迁移对照**＝v1.11 三要素逐项——〔要素 1"状态行无隐藏入口/PM-11 恒可见"〕多 Dock 形态由宿主状态栏本身恒可见承载（强于原 Dock 内钉底——任何 Dock 开关均不影响状态投影）；〔要素 2"外层纵排防栅格末行挤出"〕根因（Dock 视口竖向短缺把末行挤出视口）随状态行退出 Dock 而消解，承载结构不再需要；〔要素 3"装载几何事实落 Dev 日志（排障面）"〕[geometry] 行改载主/右/底三 Dock 尺寸与可见性＋宿主状态栏可见性（排障面对位迁移）；⑤**测试落位**＝TwoPhaseAssemblyContract 用例改载状态出口观测断言（PM-11 投影随上下文刷新经理观察者＋瞬态消息经未知命令反馈经理观察者）；⑥**验证结果**＝集成模式六目标（ui/plugin/app/三测试——契约 verify[0] 命令面）构建零错误；模型 154/154＋契约 22/22＋GUI 35/35 全过；ctest ird 2/2＋ird_gui 1/1；独立冒烟全树零错误（build-smoke-t18，含 vcpkg toolchain 配置）；ird_gates 命中集归一化比对基线（feb09471）/头部各 63 条唯一命中逐一一致（零新增——本任务零 CMake/链接面变更；归一化脚本与比对件 gate-hitset-＊-r3.txt 同目录）；**未执行如实登记**＝宿主 GUI 冒烟（多 Dock 拓扑＋宿主状态栏投影的真机操作序列）随验收段补录。需求语义零变化：PM-11/PM-14/§4.1/§4.4 承接语义按原文落位（形态承载面变更随 §10.1 v1.14 登记）。

> **UI-T18 落位登记注增补（v1.14-r2，2026-09-24，返工登记〔验收 attempt 1 fail——阻断 B-1/B-2；记录 traceability/acceptance/UI-T18-20260924.md @ acc/UI-T18/1，attempts.fix=1〕）**：返工面＝两阻断修复＋建议级 F-332/F-333/F-334 顺带消账，代码行为零变化（改动面＝纯注释/文件头/死包含/留痕删除）。①**B-1 范围越界修复**＝仓库根 `traceability/builds/wp10-t18/` 3 件 r2 代 gtest XML（allowedFiles 外的过期副本，F-331 同源）`git rm` 删除——纯留痕残留，零产品影响；未以任何方式扩大契约 allowedFiles。②**B-2 宿主 GUI 冒烟补录（已执行）**＝所有者 AskUserQuestion 授予桌面窗口后，实施侧以**截图→目视定位→真实坐标点击闭环**驱动（F-325 教训对策，本轮零菜单点击漂移），`--rwsplugin` 直载＋F-320 PATH 五项前置，两轮装载全序真机核验：多 Dock 拓扑（主 Dock『IRD 工作台』＋右 Dock『IRD 属性与诊断』＋底 Dock『IRD 任务和状态』）装载自证门控三行齐；宿主状态栏唯一＋PM-11"未打开项目"投影实拍；瞬态消息"已恢复默认布局"经宿主状态栏实拍（showStatusFeedback 通路的真机抽样）；PM-14 旗标重施＝隐藏右/底 Dock→WM_CLOSE 优雅退出→重载→**保持隐藏**（帧 07 vs 08 对比）＋菜单勾选态与模型位同步。截图 9 帧＋双轮 console/dev 日志＋驱动工具（smoke-tools.ps1/launch-run.ps1）入库 `builds/wp10-t18/`（索引 host-smoke-r4-README.md）。**诚实缺口（不粉饰）**＝F1（help.contents）合成键盘未触达快捷键、瞬态未呈现（未确证路径如实登记）；6 处瞬态消息点真机抽样 1 处（命令面板中文检索＝F-325 已登记 IME 盲驱动不可靠面）；Dev 日志存活期仅首行（F-322 家族），[geometry] 行未固化——以截图帧与控制台门控行为替代判据。③**建议级消账**＝F-332（§16.7 v1.14 行⑦"命中 43 条"→归一化 63/63 双向差集 0、⑥"集成五目标"→六目标——同注⑥ d7e4a24 修正的平行补齐）；F-333（UiPlugin.hpp @brief/设计依据、UiPlugin.cpp 设计依据、WorkbenchContent.hpp 设计依据随 UI-T18 增量更新——F-326 同族复发面闭环；尺寸钳制注释"280/160 px 以上"与代码下界 qBound(300,…,480)/qBound(190,…,340) 表意对齐）；F-334（WorkbenchContent_p.hpp 死包含 `<QStatusBar>` 删除——statusBarWidget() 出口移除后的字面一致性收口）。④**返工后全量重验 -r4 全新捕获**＝集成模式全量构建零错误（integration-build-r4.log BUILD_EXIT=0）＋verify 六目标（verify1-targets-r4.log）＋独立冒烟全树零错误（smoke-build-r4.log SMOKE_BUILD_EXIT=0）＋模型 154/154＋契约 22/22＋GUI 35/35（gtest-*-r4.xml＋ird-test-report-*-r4.json decisive=true）＋ctest ird 2/2＋ird_gui 1/1＋ird_gates 63↔63 双向差集 0（gate-hitset-*-r4.txt，与 -r3 逐位一致——注释级改动零命中面变化）＋validate-task PASS。需求语义零变化。
...
| v1.10 | 2026-09-23 | WP-10-T16（≙UI-T16）工作台宿主插件落位登记（O-38 裁决承接）：①`sdurws_ird_ui_plugin` MODULE 集成树专属落位（RobWorkStudioPlugin 派生薄插件＋Q_PLUGIN_METADATA；零计算逻辑——界面语义全经内容装配面；AUTOMOC 仅该目标显式 ON，集成树全局 AUTOMOC 对既有目标本就生效且零 Q_OBJECT 无产物＝构建形态零变化；独立冒烟模式无框架目标不注册该目标——如实登记）；②壳拆「内容装配层＋顶层窗口宿主层」：WorkbenchContent.hpp 公共契约面（WorkbenchHostKind/WorkbenchContentDeps/IWorkbenchContent 两段装配＋五区出口＋旗标半区布局记忆＋会话入口覆写＋观察钩子）＋WorkbenchShell 收敛为顶层窗口 chrome＋§10.1 v1.10 增量注＋§3.3 增列；harness 行为回归零变化（既有 GUI 套件＋CLI 三态复测全通过）；③LayoutMemory 增 loadFlags/storeFlags 旗标半区（嵌入式宿主同组同键、跨形态共享用户级设置——PM-14；§4.4 折叠随形态关闭）；④插件菜单承载五区开关＋会话入口（§11.5 覆写→真实打开协议）＋三维让位页（O-38 ③让位形态）；⑤测试：gui_test 增 WorkbenchContentGuiTest 六用例（UI-SPLIT 族）＋harness 三态/PM-07 双开交互复测留痕；⑥门禁：恰增 SUB×2（DTB §4.5 新登记行）＋R3×2（ui 例外源集文件级延伸——t03 请求/t13 先例同型），提交件 traceability/wp10-t16-gate-registrations.md，其余各码零变化。需求语义零变化（UX-09/PM-07/PM-10/PM-11/PM-14 承接语义按 §4/§5/§7/§11.5 原文落位） |
| v1.11 | 2026-09-24 | WP-10-T16（≙UI-T16）返工登记（验收 attempt 1 fail——阻断 B-1/B-2，记录 traceability/acceptance/UI-T16-20260924.md @ acc/UI-T16/1）：①宿主装载呈现缺陷修复——根因＝框架 loadPlugin 对话框路径硬编码 visible=false（setupPlugin(pathname, filename, 0, 1)）＋addPlugin 尾段 restoreState(QtMainWindowState) 对状态 blob 未登记 Dock 按隐藏处理（两步均在插件 initialize 之后——SA-02 框架零修改），插件侧 initialize 末尾零等待单发重申嵌入式呈现（reassertEmbeddedPresentation：setFloating(false)＋show()；单发不与用户手动开关竞争，区域级记忆仍归内容装配面）；G1 装载门控增第二判据（"工作台 Dock 已呈现"控制台行＋Dev 日志）；②宿主交互证据全量重录（G1~G4 客观门控全过＋逐帧截图，builds/wp10-t16/）；③装载环境前置登记（S-3/F-320——PATH 需求清单＋装载门控客观判据，见 §13 返工登记注④）；④B-2 失实复跑载体（-r2 字节级拷贝）删除更正，复跑结论一律以带 ctest 时间戳头的全新捕获为准（S-2/F-319 消账）。需求语义零变化 |
| v1.12 | 2026-09-24 | UI-T17 立项登记（O-43 裁决承接，DTB v0.19 §4.2/§2.11 同日登记）：§13 新增 UI-T17 行＋立项登记注——宿主菜单融合（File"工业机器人项目"子菜单按位插入＋最近项目/Tools 命令面板/自建视图菜单/Plugins 仅留基类显示开关——菜单只路由命令板，使能态随命令可用性快照）＋会话入口覆写面扩展（WorkbenchContentDeps 增 saveProjectHandler/closeProjectHandler——§11.5 同型；saveAs/applyDraft/undo/redo 维持阶段 A 占位，归属 workflow/project 不变）＋插件侧真实编排（§5.4 统一确认对话框呈现＋Draining 轮询驱动＋saveAllDraftsManual 接线〔DraftController 装配——保存链路真实；恢复/autosave/草稿清单适配/分支锚随完整草稿链路任务接续，登记为诚实边界〕）。Dock 拆分与状态行迁移归 UI-T18（O-43 ③，已立项未实施）。需求语义零变化 |
| v1.15 | 2026-09-26 | **WP-24-T03 首版装配落位登记**（owner 指示 2026-09-26"每完成一个插件进行装配"提前启动——原排期阶段 E；分支 wp24-t03 待验收；首版范围＝ui 侧装配契约落地＋建模插件真宿主挂位，收口余量见④）：①**装配契约头落位（P-MDL-8 消账）**——IPluginUiRegistrar.hpp〔§10.9 六形状＋createPluginUiRegistrar 工厂＋实现 src/PluginUiRegistrar.cpp：白名单→重复→描述符→Ok 校验序、失败隔离 §11.3、白名单构造期固定〕＋IPluginUiModule.hpp〔§11.2 三方法冻结签名；project::CommandEnvelope/IWorkbenchShell 前向声明——ui 头零 project include 纪律＋IWorkbenchShell→UiPorts→AboutDialog 包含链断环〕；PluginAssemblyReport 自 AboutDialog.hpp 迁移（v1.2 预注兑现，形状零变化源/二进制兼容）。②**宿主插件装配序列**——ui/plugin/DomainAssembly.*（白名单八 token＋建模 registrar 登记报告＋面板工厂消费＋命令提交出口绑定〔宿主状态栏瞬态消息"已受理域命令 …（域执行面随装配收口任务接线）"——首版可见反馈面，不虚构执行成功〕）；ui_plugin 链接 sdurws_ird_modeling_plugin（ui→modeling 装配边——DTB §4.5 同款 SUB；消费面仅 modeling 装配门面 assembly/…/ModelingPluginAssembly.hpp〔modeling 侧新增，plugin 目标 PUBLIC 包含面〕）；建模 Dock 入宿主 Left 区（ird_modeling_dock）；UiText 增键族"域命令标题"10 键（cmd.modeling.*.title 中文工程用语——resolveText 唯一出口，面板按钮键名→中文）。③**建模侧切换**——ModelingUiModule 同形→继承 ui::IPluginUiModule（三方法 override，签名/语义零变化）；新增装配 API（bindCommandSubmit/bindTextResolver/seedTemplateSession/createPanel）＋面板 setCommandTitleResolver 呈现缝。④**首版范围诚实边界（收口余量）**——域命令入 content 注册表（§7.2 冲突规则）／只读投影入 StageStatusModel 汇聚／中央区 CentralAreaHost 挂位〔首版＝宿主侧 Left Dock 承载〕——随装配收口任务（阶段 E）兑现。**T03b 已落位（2026-09-26 同日增量）**：①draft.apply 域侧挂钩——WorkbenchContentDeps 增 `applyDraftHandler` 覆写（同 new/open/save/close 五覆写模式）＋宿主 orchestrateApplyDraft（权威分支表首条 tip 锚定会话→模块 buildDraftCommand→`ProjectCommandService::submit`→回执投影回写 onCommandResult→Committed 后会话锚前移＋根身份回填〔objectRefs 按 robot-design token 过滤〕）；StorePortAdapter 增 projectStore() 访问器（装配层特权面）；②确认交互桥——HostCommandInteraction（project::ICommandInteraction 宿主实现：Confirmable 集三要素模态确认→凭据 local-owner＋UTC 留痕，否决＝整体拒绝；§7.7 非重入形态的同步变体，收口迁 CommandInteractionBridge Marshal）；③就绪真判定——建模模块装配真实 ModelReadinessChecker（policy makeJointLimitEvaluator＋草稿名称桩），编辑/锚定/应用后重算（策略未装载时 L11 如实 Blocking，不伪造可行）。**仍余（阶段 E）**：策略装载与 runtime 名称适配（就绪 L11/L 行程全量）、会话事件全同步、About 接线。⑤**验证**——集成 ui/modeling 五测试目标全绿（154/154＋22/22＋265/265＋17/17）；真宿主 GUI 冒烟＝--rwsplugin 直载、建模 Dock 呈现＋十条命令按钮中文工程用语（UiText 生效实证）＋无错误弹窗，截图 traceability/builds/wp24-t03-first/modeling-in-host-smoke-02.png；ird_gates 引擎直跑基线..branch 归一化比对＝净新增恰 1 处逻辑命中（SUB：ui->modeling——ui→project 同族装配边，登记册回填归 WP-01-T03）。需求语义零变化（详见 §3.3 两头落位行＋§10.9/§11 落位登记） |
| v1.14-r2 | 2026-09-24 | WP-10-T18（≙UI-T18）返工登记（验收 attempt 1 fail——阻断 B-1/B-2，记录 traceability/acceptance/UI-T18-20260924.md @ acc/UI-T18/1；attempts.fix=1；代码行为零变化）：①B-1 范围越界修复（仓库根 traceability/ 3 件 r2 代 XML 删除——纯留痕残留，未扩大契约 allowedFiles）；②B-2 宿主 GUI 冒烟补录执行（所有者授予窗口，实施侧截图→目视定位→坐标点击闭环驱动＋--rwsplugin 直载＋F-320 PATH 五项前置；两轮装载真机核验：多 Dock 拓扑＋宿主状态栏唯一＋PM-11"未打开项目"投影＋瞬态消息"已恢复默认布局"实拍＋PM-14 隐藏右/底→优雅退出→重载保持隐藏；截图 9 帧＋双轮 console/dev 日志＋驱动工具入库；诚实缺口＝F1 瞬态未确证〔合成键盘未触达〕＋瞬态抽样 1/6＋Dev 日志采样限制〔F-322 家族〕如实登记——host-smoke-r4-README.md）；③建议级消账＝F-332（§16.7 v1.14 行⑦"43 条"→归一化 63/63、⑥"五目标"→六目标）＋F-333（UiPlugin.hpp/.cpp、WorkbenchContent.hpp 三文件头随 UI-T18 增量＋钳制注释 280/160 与代码下界 300/190 表意对齐）＋F-334（WorkbenchContent_p.hpp 死包含 QStatusBar 删除）；④返工后全量重验 -r4 全新捕获＝双模式构建零错误＋154/22/35＋ctest ird 2/2＋ird_gui 1/1＋ird_gates 63↔63 双向差集 0（与 r3 逐位一致）＋validate-task PASS（详见 §13 UI-T18 落位登记注增补 v1.14-r2） |
| v1.14 | 2026-09-24 | WP-10-T18（≙UI-T18）Dock 拓扑拆分＋状态行迁移宿主状态栏落位登记（O-43 ③承接；分支 wp10-t18 待验收）：①契约面变更（statusBarWidget() 出口移除→双状态观测钩子 setStatusTextObserver/setStatusMessageObserver——内容层 6 处瞬态消息点收敛 showStatusFeedback 唯一路由；§10.1 v1.14 增量注⑦）；②多 Dock 拓扑（主 Dock＝命令条＋项目导航；"IRD 属性与诊断"右 Dock＋"IRD 任务和状态"底 Dock 装载自证拍入宿主＋尺寸初值钳制；中央让位页不挂载；三区可见性目标改绑 Dock 本体；布局记忆三区键零变化；§10.1 v1.14 增量注⑧）；③harness 自有状态栏等价迁移（呈现行为逐项等价——GUI 套件全过为证）；④v1.11 钉底修复语义等价迁移对照（宿主状态栏恒可见强于 Dock 内钉底）；⑤测试（TwoPhaseAssemblyContract 改载状态出口观测断言；模型 154/154＋契约 22/22＋GUI 35/35；ctest ird 2/2＋ird_gui 1/1）；⑥双模式构建零错误（集成六目标——ui/plugin/app/三测试，契约 verify[0] 命令面＋冒烟全树 build_smoke_t18）；⑦ird_gates 命中集归一化比对基线（feb09471）63/63 双向差集 0 零新增；⑧宿主 GUI 冒烟未执行如实登记——随验收段补录。留痕 traceability/builds/wp10-t18/。需求语义零变化（详见 §13 UI-T18 落位登记注） |
| v1.13-r4 | 2026-09-24 | UI-T17 宿主 GUI 冒烟执行登记（所有者人工操作 14:32~14:44；实施侧合成驱动对 Qt 弹出菜单不可靠〔F-325 再实证，驱动尝试帧在案〕）：新建项目（projtarget-r4＋"冒烟项目r4"）→保存草稿→关闭取消/确认→重开被 S2 候选验证拒绝（lock-held-by-other）全序执行；磁盘落盘（project.json/HEAD/内容寻址对象/修订＋锁 pid·心跳至宿主退出）＋dev 日志 `draining entered: in-flight references=0`＋所有者确证"我已经手动验证了，都正确可以通过验收"；诚实缺口＝对话框/状态行过程视觉帧未捕获（连拍错过＋所有者裁决"不用"不重走）如实登记；冒烟前预构建零错误（e917734a 二进制一致）；B-1 裁定权交验收段 attempt 2（详见 §13 UI-T17 落位登记注增补 v1.13-r4＋host-smoke-r4-README.md） |
| v1.13-r3 | 2026-09-24 | UI-T17 返工登记（验收 attempt 1 fail——阻断 B-1 唯一项，记录 UI-T17-20260924.md @ acc/UI-T17/1；attempts.fix=1）：①F-326 修复（UiPlugin.hpp/PortAdapters.hpp 文件头注释对齐 UI-T17——纯注释零行为）；②F-327 延期登记（直测需测试目标增链 project＝抵触本任务"零 CMake/链接面变更"，随草稿链路完整装配任务补齐——验收记录推荐口径）；③F-326 后全量重验 -r3 全新捕获＝双模式构建零错误＋模型 154/154＋契约 22/22＋GUI 34/35（1 失败＝桌面剪贴板被进程锁定的环境干扰〔Set-Clipboard/clip.exe/粘贴 0 行三重证据〕，UI-T08 既有用例零触碰，所有者裁决如实登记）＋ctest ird 2/2＋ird_gui 34/35＋ird_gates 55↔55 双向差集 0＋validate-task PASS；④宿主 GUI 冒烟＝闭环取证工具就位（host-smoke-r3-driver.ps1），所有者裁决"稍后指定时间"——仍未执行如实登记，取证入库后发起验收请求 attempt 2（详见 §13 UI-T17 落位登记注增补 v1.13-r3） |
| v1.13-r2 | 2026-09-24 | UI-T17 落位登记增补（实施派发会话承接收尾；分支 wp10-t17，merge base=656b7f30）：①证据位置更正（batch 1 留痕误置仓库根 traceability/——allowedFiles 外，r2 迁入 doc 规范路径；batch 1 未入库的 4 件日志经 r2 重跑补齐）；②ird_gates 同引擎 detached worktree 基线（ac5d107）全量归一化比对＝94↔94 逐条一致双向差集 0（hit-set/diff 三件留痕）；③r2 复核全绿＝154/22/35＋ctest ird 2/2＋ird_gui 1/1＋validate-task PASS＋双模式构建零错误（全部全新捕获 -r2 后缀）；④宿主 GUI 冒烟＝自动化取证面就绪（自动装载 ini＋键盘驱动截图脚本＋移交手册），实测确认桌面被占用后所有者裁决随验收段补录（交付面零变化——详见 §13 UI-T17 落位登记注增补） |
| v1.13 | 2026-09-24 | WP-10-T17（≙UI-T17）会话入口处理器补全＋宿主菜单融合落位登记（O-43 裁决承接；分支 wp10-t17 待验收）：①WorkbenchContentDeps 覆写面扩展（saveProjectHandler/closeProjectHandler——§11.5 同型，harness 形态不变）；②插件侧真实编排（§5.4 统一确认对话框两轴呈现＋Draining QTimer 轮询驱动四级防线＋saveAllDraftsManual 接线〔DraftController 装配：SerialTaskExecutor 串行落盘＋Marshal 回投＋会话脏生产者接线＋bindSession/unbindSession 编排〕＋forceAbandonAll 显式留痕空兜底）；③宿主菜单融合（File 内 Preferences 锚 insertMenu"工业机器人项目"子菜单＋最近项目现取重建/Tools 命令面板/自建视图菜单/Plugins 仅留基类开关——使能态全经命令可用性快照，插件零判定）；④装配层增量（app::SerialTaskExecutor＋StorePortAdapter 增 IUiDraftStorePort 双面）；⑤测试（GUI 增覆写路由与门控用例；模型 154/154＋契约 22/22＋GUI 35/35；ctest ird 2/2＋ird_gui 1/1）；⑥双模式构建零错误（集成六目标＋冒烟全树）；⑦ird_gates 归一化比对零新增（R3 基线 17 文件逐一一致）；⑧宿主 GUI 冒烟未执行如实登记——随验收段补录。留痕 traceability/builds/wp10-t17/。需求语义零变化（详见 §13 UI-T17 落位登记注） |
