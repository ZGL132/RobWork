...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.5（EX-T03 实现落位登记——§15.4 变更记录行＋§7 协议编排落位说明（Controller/Scheduler 两契约头与实现、TaskCapability 运行时限字段、协作窗起点语义、DrainPolicy 排空编排语义、两处头内归置登记）；契约语义零变更；v0.4＝EX-T02 实现落位登记；v0.3＝EX-T01 构建落位登记；v0.2＝全链一致性审计消账；v0.1＝首版草案） |
| 日期 | 2026-09-18 |
