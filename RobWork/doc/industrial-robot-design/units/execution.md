...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.12（EX-T10 文档与门禁同步——本文与实现零偏差登记＋§15.3 P-EX-1～10 状态逐项复核〔P-EX-5/P-EX-7/P-EX-10 消账〕＋DETAILED-DESIGN execution 行状态更新申请与 README 指向复核＋ird_gates/CI 建议提交登记；契约语义零变更，登记见 §15.4；v0.11＝EX-T09 契约测试套件落地登记——§15.4 变更记录行；契约语义零变更；v0.10＝EX-T08 实现落位登记；v0.9＝EX-T07 实现落位登记；v0.8＝EX-T06 实现落位登记；v0.7＝EX-T05 实现落位登记；v0.6＝EX-T04 实现落位登记；v0.5＝EX-T03 实现落位登记；v0.4＝EX-T02 实现落位登记；v0.3＝EX-T01 构建落位登记；v0.2＝全链一致性审计消账；v0.1＝首版草案。卡头版本行 v0.5→v0.11 一次性追平——v0.6~v0.10 落位期仅更新了 §15.4 未同步卡头〔F-175 家族同型微瑕，本行消账〕） |
| 日期 | 2026-09-19 |
...
| 构建落位 | `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/execution/`（**已随 EX-T01 落位〔2026-09-18〕**：目标 `sdurws_ird_execution` 升级 STATIC〔C++17、PUBLIC 链 core/evidence/project——ARCH §3.5 三条登记编译链接边，§3.4 原文；diagnostics 边按注入形态不落链接——P-EX-8 裁决；零 Qt 含 Core——D-01；零 runtime/policy 编译边——P-EX-3 处置：经 §3.3 最小接口注入消费、policy 经快照 PolicyRef 值传递；src/ 空起步锚点翻译单元 `Execution.cpp`——STATIC 库最小翻译单元先例形态，CORE-T01/POL-T01/DIAG-T02/PRJ-T01 同款，无接口预建〕，别名 `RWS::ird::execution`；测试目标 `sdurws_ird_execution_test`/`sdurws_ird_execution_contract_test` 已按 DTB §5.5 注册〔gtest vcpkg 接入；落位期最小构建冒烟用例＝EX-BLD-1 红线扫描＋构建图边界契约，业务用例随 EX-T02~T09〕；配置期红线守卫就位。公共头保留位 `include/sdurws/ird/execution/README.md` 指向本文 §12——原 §9 指向已于 2026-09-10 版修正，EX-T01 复核确认零偏差，"不参与编译/不表示已实现"声明保留；§3.1 契约头 **Errors/TaskTypes/StateMachine 三件已随 EX-T02 落地〔2026-09-18〕**（§4 三级身份＋§5 状态机＋§5.5 能力声明承载＋§3.4 错误面），**Controller/Scheduler 两件已随 EX-T03 落地〔2026-09-18〕**（§7.1 取消与终止协议编排＋§7.5 DrainPolicy 排空编排——§15.4 v0.5），**RunRegistry/Ports 两件已随 EX-T04 落地〔2026-09-18〕**（§9.1/§10.5 登记记录与两段式接纳＋§9.2 九步编排＋§3.3 注入接口三件——§15.4 v0.6；余件随 EX-T05+），ArchivePhase 枚举随其消费字段落 TaskTypes.hpp（§15.4 v0.4 归置登记）。**契约测试套件已随 EX-T09 落地〔2026-09-19〕**（§11 矩阵补齐 EX-ARC-1~4/EX-RCV-1/EX-RES-1 缺口用例；两测试目标按 T-1 允许形态链接 `sdurws_ird_testkit`〔RecordListener/TempDir/DeterministicEnv/Fault/TestProcessRunner/AssertMacros 消费〕并改自有 main 安装 RecordListener——ird-test-report.json 与 gtest XML 并存；faultPointId 命名登记与替身边界声明落 `test/ContractSuiteFacilities.hpp`——§15.4 v0.11）。见单元 `execution/CMakeLists.txt` 与上级 `industrialrobot/CMakeLists.txt`） |
...
| # | 问题 | 依据 | 影响 | 建议 | 需要裁决者 |
| --- | --- | --- | --- | --- | --- |
| P-EX-1 | 消费的 core/evidence/project/runtime/policy/testkit 契约均 v0.1 Draft 未冻结 | 各卡文档头 | EX-T02 起返工 | 各卡冻结时出 diff，本文按影响面增量修订留痕。**〔EX-T10 状态同步 2026-09-19〕仍 open（冻结未发生）**：消费卡实测现版 core v0.11（Draft-Structured）/evidence v1.3/project v0.18/runtime v0.15/policy v0.14/testkit v0.9——均未冻结（各卡状态行"不自行宣布 Accepted"）；EX-T02～T09 各落位期按现状签名消费并以 static_assert/用例钉住（§15.4 v0.4～v0.11），本任务复跑两套件 149/149＋38/38 复证消费面零漂移（traceability/builds/wp08-t10/）；冻结 diff 后按影响面增量修订路径不变 | 各卡所有者＋本文所有者 |
| P-EX-2 | ARCHITECTURE v0.11 Draft 待评审 | 架构文档头 | §3/§5/§9 可能同步 | 评审后增量修订（A9+ 处置）。**〔EX-T10 状态同步 2026-09-19〕仍 open（评审未完成）**：ARCHITECTURE 实测 v0.12（2026-09-10 全链一致性审计事实修正——附录引用补登/PM-08-S1 补列/O-32 登记，git 实测未触及本文锚点 §3/§5～§7/§9），零返工面；评审 A9+ 结论产出后按影响面增量修订 | 架构所有者 |
| P-EX-3 | 任务指令述"execution 可依赖 core、evidence、runtime、policy、project、diagnostics"，ARCH §3.5 实际登记四条边（runtime/policy 不在表内） | 任务约束§三 vs ARCH §3.5（SA-10 表外边＝构建失败） | 编译期依赖形态 | 维持本文 §3.3 注入消费（D-17）；如需直连请架构修订 §3.5 补登 execution→runtime 边。**〔EX-T10 状态同步 2026-09-19〕仍 open（架构未补登边）**：注入形态已全量兑现——IExecutionModelService/ICompileCacheJudge（EX-T04/T06/T08 落位）＋INameResolverAdapter（EX-T04）＋ICancelSignal（EX-T03），零编译依赖零表外边（ird_gates 命中集合与 wp08-t09 验收基线一致——GATES-IDENTICAL-18，builds/wp08-t10/）；D-17 平滑直连路径不变 | 架构所有者 |
| P-EX-4 | 任务指令示例列出 11 态（含 Created/CancelRequested/Rejected），上游（PM-03/ARCH §4.3/core 词表）定义九态 | REQUIREMENTS §17 PM-03、ARCH §4.3、core.md §4.7 | 状态机词表 | 按上游九态执行（§5.1 注：提交拒绝在状态机之外、Canceling 承载取消请求相）；如需三态显式化走需求变更。**〔EX-T10 状态同步 2026-09-19〕仍 open（需求侧未确认）**：九态已实现（EX-T02——§5 逐转移表 T1～T14，矩阵外 InvalidState fail-fast，§15.4 v0.4）；REQUIREMENTS v1.16 无相关变更；裁决归属不变 | 需求所有者（登记确认） |
| P-EX-5 | P-PR-4（project §15.3）：归档调用线程与 RunRegistry 对接 | project.md §15.3 | 契约测试线程模型 | 本文 §9.5 已冻结（调度线程调用＋writer 互斥串行化＋完成事件幂等无上限＋abandon 三时机）；请 project 侧确认消账。**〔EX-T10 状态同步 2026-09-19〕已消账（登记消账依据与日期）**：对端 project.md §15.3 P-PR-4〔PRJ-T16 状态同步 2026-09-17〕已消账并明文"对端 P-EX-5 消账由其卡承接"——本文 §9.5 冻结答复（调度线程调用＋经 project §9.8 writer 互斥串行化＋begin 白名单前置校验＋writeBatch≠finalize＝唯一完整标志＋abandon 无永久等待＋完成事件幂等无上限）与 project 侧逐项核对一致、其确认零增量需求；实现侧已按同语义落地（EX-T04 §9.5 网关＋EX-T09 EX-ARC 契约用例，§15.4 v0.6/v0.11） | project 详设所有者 |
| P-EX-6 | 任务中断持久化判据＝归档预留目录（无独立 journal）：Queued 任务崩溃即消失（不呈现"已中断"）；Preparing 起可观测 | PM-08/NFR-REL-03、project §4.1 results 行 | 崩溃恢复呈现范围 | 维持 D-12/D-13（磁盘痕迹唯一经 project；无第二账本）；若需求"排队任务也须恢复呈现"走需求变更。**〔EX-T10 状态同步 2026-09-19〕仍 open（需求所有者＋project 详设所有者未裁决）**：D-12/D-13 已实现承载（EX-T03 DrainCoordinator——排队保留是设计决定不阻塞 drained；EX-T09 EX-RCV-1 真进程用例锁定恢复判据——预留批次字节保留＋T14 重建 Interrupted，§15.4 v0.5/v0.11）；裁决归属不变 | 需求所有者＋project 详设所有者 |
| P-EX-7 | 评估器运行期能力声明（暂停/检查点粒度）的承载位置：evidence EvaluatorDescriptor 未含执行能力字段 | TASK-01、evidence §9.2 | 能力声明数据源 | execution 侧注册表扩展声明（§5.5），登记为 evidence 交接（其卡增量修订时决定是否上收 descriptor）。**〔EX-T10 状态同步 2026-09-19〕已消账（登记消账依据与日期）**：承载方向两侧登记一致——evidence.md v1.1（EV-T10 落位登记 2026-09-12）明文"P-EX-7 处置：EvaluatorDescriptor 七字段不含任何执行期能力字段，该面归 execution 侧注册表扩展声明承载，两侧均不私改对端口径"；本文 §5.5 EvaluatorRuntimeCapabilities 已随 EX-T02 落地（§15.4 v0.4）——evidence 详设所有者的承载决定已在其卡登记；后续如上收 descriptor 走 evidence 卡增量修订 | evidence 详设所有者 |
| P-EX-8 | EX-\* 稳定码与 IExecutionDiagnosticsSink 形态待 diagnostics 冻结 | diagnostics 未产出、ARCH §3.5 边已登记 | 码表/适配器返工 | diagnostics.md 起草时收编 §3.4 建议清单；sink 与 project §5.0 形态统一。**〔EX-T10 状态同步 2026-09-19〕码值部分已消账（§15.4 v0.2——diagnostics.md §4.6 收编 18 项）；sink 名称/归属统一维持登记不消账（裁决申请已提交、未裁决）**：project.md §15.3 P-PR-6〔PRJ-T16 2026-09-17〕申请二＝traceability/wp04-t16-adjudication-requests.md，diagnostics.md §9.7 IDiagnosticSink＋DiagnosticsSinkImpl 已交付且与本文 §3.3 签名逐字一致，本项同案建议同批裁决（该申请依据 #4 在案）；裁决后本文按影响面增量修订收编统一接口与链接形态；注入式先行＋空实现占位（EX-T09 NullExecutionDiagnosticsSink，§15.4 v0.11）不受裁决结果阻塞 | diagnostics 详设所有者 |
| P-EX-9 | 旧版本检查点不迁移（Incompatible 拒绝） | CON-04 保守方向 | 升级后长任务须重跑 | 维持保守（宁可重算不可错续）；如需迁移工具走需求/设计变更。**〔EX-T10 状态同步 2026-09-19〕仍 open（需求所有者未确认）**：保守方向已实现承载（EX-T08——Incompatible 拒绝＋零迁移代码路径＋写出无条件打当前戳，§15.4 v0.10 P-EX-9 处置）；裁决归属不变 | 需求所有者（登记确认） |
| P-EX-10 | DETAILED-DESIGN.md execution 行状态"待产出"需随本文更新；execution README 任务卡指向 §9 需修正 §12 | §1.2 实测 | 索引一致性 | 随 EX-T01/EX-T10 消账（文档同步任务）。**〔EX-T10 状态同步 2026-09-19〕已消账（登记消账依据与日期）**：①README 半区——§9→§12 修正于 2026-09-10 版完成，EX-T01 复核零偏差（§15.4 v0.3），本任务再复核零偏差；②DETAILED-DESIGN 半区——execution 行实测已登记"Draft / 已有任务卡"（2026-09-10 首批索引同步落位，与本文 §1.2 预登记目标状态、卡头 Draft、unit-status.json status=draft/designCompletion=written 三方一致；2026-09-19 复核零偏差——traceability/builds/wp08-t10/README-audit.md §6）；③实现进度反映（unit-status.json implementationStatus 等）为跨单元索引治理面——状态更新申请已提交详设目录维护者（traceability/wp08-t10-detailed-design-status-request.md），不私改共享索引 | 详设目录维护者（本文所有者执行） |
...
| v0.4 | 2026-09-18 | EX-T02（≙WP-08-T03）实现落位登记：§3.1 契约头 Errors/TaskTypes/StateMachine 三件与同名实现落位——§4 三级身份（TaskId 按 §4.1 "core 未定义此类型"原文由 execution 以 core §4.1 同纪律自持实现、tag "tsk-"；RunId/AttemptId/TaskIdentity 只消费 core 公共头零重定义；bindRun＝§4.3 派发登记绑定 AttemptId=1）、§5 逐转移表 T1~T14＋Running→Canceling（单一事实源驱动 §5.2 矩阵查询与查表推进；矩阵外 InvalidState fail-fast、T3 重复取消幂等 ack、T10 能力门控 EX-CAPABILITY-UNSUPPORTED 结构化反馈、T13 显式 ForceTerminated＋EX-FORCE-TERMINATED、T14 恢复期工厂指派 EX-TASK-INTERRUPTED）＋事件发布接缝 ITaskEventSink（TaskStatusChanged 每次转移）、§5.5 EvaluatorRuntimeCapabilities 注册表扩展声明（P-EX-7 处置：evidence EvaluatorDescriptor 字段面零改动）、§5.6 terminalOutcome 终态映射承接（core.md §10.3 交接）、§3.4 错误面（19 值＋toToken；ContextClosed/InvalidState 按 v0.2 对齐说明为 fail-fast token）。**一处头内归置登记（本文与实现的一处已登记偏差）**：ArchivePhase 枚举按 §3.1 组成表登记于 RunRegistry.hpp 行，实现落 TaskTypes.hpp——其唯一消费字段 TaskRecord::archivePhase 在 TaskTypes.hpp 且 RunRegistry.hpp（EX-T04）反向依赖 TaskId，落 TaskTypes.hpp 防头循环；值域与语义零变更，EX-T04 直接复用零重定义。测试：`sdurws_ird_execution_test` 31 用例（EX-SUB-1/EX-SM-1~7/强制终止矩阵/四态取消出口/恢复期指派——纯逻辑替身；2 s/10 s 时序、worker 进程、归档磁盘断言归 EX-T03/T04/T06/T09 对应用例）＋`sdurws_ird_execution_contract_test` 9 用例（四词表＋五元组类型 execution 面零重定义、TaskId 归属正反核对）；本文契约语义零变更。证据：traceability/builds/wp08-t02/（双模式构建零错误＋31/31＋9/9＋ird_gates 零新增命中＋validate-task PASS＋故障探针 6 项红/复跑全绿） |
| v0.5 | 2026-09-18 | EX-T03（≙WP-08-T04）实现落位登记：§3.1 契约头 Controller/Scheduler 两件与同名实现落位——**§7.1 协作式取消协议编排**（TaskController：取消命令通道〔任意线程入队＋poll 命令优先段＝"取消命令插队于派发之前"的编排表达；生效延迟上界＝poll 周期，2 s 保证归 EX-T05 调度周期配置〕、协作窗 10 s 收敛〔**窗起点语义冻结为"进入 Canceling 时刻"**——ARCH §4.4 条 1 先于条 2 的次序即先入 Canceling 后等收敛，2 s 生效延迟由协议承担而非批次〕、超时强杀序列〔terminateForce 恰一次→T13→Failed(EX-FORCE-TERMINATED)，KILL_ON_JOB_CLOSE 语义锚定于 IWorkerHandle::terminateForce 契约注——真进程兑现归 EX-T06 JobScope〕、运行超时 EX-WKR-5 同卡死路径〔EX-WORKER-HUNG 判定诊断 cause 携带 evaluationTimeout 标记区分心跳失联；判定诊断经 appendDiagnostic 先于终结标记写入——因果序〕、无 worker 任务〔Queued/Preparing/Paused〕取消两步连发直达 Canceled、requestForceTerminate 终态幂等 no-op/无 worker 任务取消等效〔§7.4"强杀归任务操作"的矩阵内表达〕、releaseResources 保留窗守卫〔D-07〕；显式驱动模型：poll() 由调度线程周期调用〔EX-T05 接入〕，时序度量全部经可注入 ClockFn——ManualClock 虚拟推进断言、不 sleep，testkit §6.5/§6.6）、**§7.5 DrainPolicy 排空编排**（DrainCoordinator：等待排空路径〔shutdown(CancelQueuedAndWait) 排队取消/KeepQueuedTerminate 排队保留＋在途等待自然终态〕＋协作取消路径〔requestCancelAll 批量入口——§7.4 PM-03"协作取消"分支〕＋drained() 无阻塞快照查询〔判定＝无在途运行〔Preparing/Running/Canceling〕；排队保留是设计决定非未决工作，不阻塞 drained——P-EX-6 同源〕＋超阈值 abandonAll(ForceTerminated) 兜底〔§7.5"L5 关闭控制器强制 abandonAll"的执行侧承载：自动触发经 poll 恰一次＋手动入口暴露 L5；排队任务不在兜底处置面〕；abandonThreshold 默认 15 s＝10 s 协议窗＋5 s 余量、0＝禁用——**实现参数非上游值**）。**既有头增量两处**：TaskCapability 增 `evaluationTimeout` 字段（§11 EX-WKR-5 行"能力声明字段"明文的承载位——nullopt＝不启用监视、0 视为未声明；登记为 §5.5 三件套之外的执行期声明扩展偏差）；TaskStateMachine 增 `appendDiagnostic`（§4.2 diagnostics 行"追加"可变性的编排层入口——转移类诊断仍在转移内写）。**头内归置登记两处（本文与实现的一处边界澄清＋一处接缝扩展）**：①§10.2 ITaskController 完整接口（requestPause/requestResume）随 EX-T05 调度器组合落位——暂停确认＝检查点写出并确认（§7.2）依赖 EX-T06 通道与 EX-T08 检查点协调器，本任务卡行产物为"取消与终止协议"故 Controller.hpp v0.5 落 CancelAck/StatusAck/ICancelSignal＋TaskController 取消/终止编排；②IWorkerHandle（§10.3 单 worker 视口）与 IDispatchGate（§7.1 步 1 停派发回调）两编排接缝随 Controller.hpp 登记（§3.1 组成行未单列——签名面被 TaskController 公共方法引用故入公共头；EX-T06 supervisor/EX-T05 调度器适配）。P-EX-2 处置：协议语义逐条锚定 ARCH §4.4 明文，2 s/10 s 以 constexpr 公开常量冻结（static_assert 钉值，测试探针实证编译期拦截篡改）、无运行期修改点，评审 A9+ 变更按影响面增量同步（影响面集中 §7）；P-EX-1 处置：新增产品源仅消费 core 公共头现状签名（Evaluation/DiagData/Identity），冻结 diff 后按影响面增量同步。测试：`sdurws_ird_execution_test` 增 14 用例（EX-SM-1/3/4、EX-WKR-4/5、超时强杀＋无永久等待、DrainPolicy 两路径＋兜底、ICancelSignal 接缝、releaseResources 保留窗——ManualClock 虚拟推进不 sleep；EX-SM-4 检查点磁盘断言为取消编排零磁盘动作的验证，写出协议归 EX-T08；累计 45 用例）＋`sdurws_ird_execution_contract_test` 9 用例保持全绿（消费面零扩展）；本文契约语义零变更（本行为落位登记＋两处增量偏差登记）。证据：traceability/builds/wp08-t03/（双模式构建零错误＋45/45＋9/9＋ird_gates 零新增命中〔与 base 命中集合 diff 为空〕＋validate-task PASS＋故障探针：static_assert 编译期拦截＋运行期 2 项红/还原复跑全绿） |
| v0.6 | 2026-09-18 | EX-T04（≙WP-08-T05）实现落位登记：§3.1 契约头 **RunRegistry/Ports 两件**与实现 `src/RunRegistry.cpp`/`src/Admission.cpp` 落位——**§9.1 RunRegistration 登记记录全字段**（identity 五元组＝接纳唯一核对基准、runDir＝归档位置取登记原值不重新推导〔A8〕、allowedResultKinds 五值词表、supersededAttempts 迟到判定数据源、resourceContext 引用持有义务〔快照/会话/存储上下文三件——A7〕；**增量两字段**：mode 与 archivePhase 镜像——步 9 envelope 组装需要 mode、§10.5 archivePhase(RunId) 查询需要阶段镜像，随本行登记）、**§9.2 九步接纳**（步 1~7＝classifyArrival 纯查表一次性判定：未知 Run→UnknownRun〔绝不写入任何项目 results/〕、五元组逐字段失配→IdentityMismatch、陈旧 attempt→StaleAttempt、第二道绑定核对→BindingMismatch、类型越集→KindNotAllowed、已终结〔Canceled/Failed/ForceTerminated〕拒绝 FinalEnvelope→AlreadyTerminated〔TASK-02 执行侧落点——不构造不归档〕；**步 2/步 4 次序细则冻结**：attemptId 三分支〔＝现役通过／∈superseded 出 StaleAttempt／其余出 IdentityMismatch〕使两行语义并存不互吞；步 8＝名称反解经 INameResolverAdapter 以**登记** nameMapIdentity 绑定〔绝不用当前映射——CON-06〕，先反解后接纳、任一名称不可解即拒绝；步 9＝aggregateVerdict→validateCombination 快照绑定重载→ResultEnvelope::make 单点复用零复制〔acceptance 3；make 失败→T9 markTerminated(Failed)＋诊断〕；**实现口径两处**：①重投递"九步重新执行"的承接读法＝步 1~7 真实重执行＋载荷指纹一致即短路 DuplicateIgnored〔同 canonical 输入是确定性纯计算，跳过重算无观测差异；"不重写"由零归档端口调用最强保证〕，②T8 按文字序在组装通过后即落〔noteAdmissionCompleted 镜像〕，finalize 的 D-14 比对若被指纹表为空的跨进程窗口 bypass 检出冲突，判定面出具 ContentConflict 而任务镜像保持 Completed〔冲突语义归属归档内容——§5.6 两轴正交〕）、**§9.3 迟到处置**（FinalEnvelope 指纹幂等〔完成事件重复投递无次数上限——P-PR-4 冻结义务之二〕/批次经真实 project writeBatch 幂等与冲突〔D-13/D-14 全链路〕、有界自动重试默认 2 次退避 1 s→4 s〔D-11——实现参数〕、重试耗尽 abandon(Failed)＋EX-ARCHIVE-FAILED＋任务终态不变〔Completed＋ArchiveFailed 正交——EX-ARC-5〕、冲突不重试〔数据冲突非瞬态〕、丢弃计数 rejectedArrivalCount 观测面）、**§9.5 归档协作**（IRunArchiveGateway 网关＝写面端口＋A7 重取两动作的最小消费口；预留会话复用/补建〔noteArchiveSession 回写〕、begin ContextClosed→重新取得写权限→成功照常/失败 EX-ARCHIVE-AUTHORITY-LOST〔A7 防御两分支〕、finalize 成功 ResultArchived 事件恰一次〔总线未装配＝不发布＋一次性开发留痕——core D-09〕；P-PR-4 单侧冻结兑现：全部归档端口调用仅调度线程、不另持项目写锁——与 PRJ-T14 的 §5.6/§9.8 冻结文本互为对端起点，偏差走二次对齐）。**归置/词表增量登记三处**：①§10.5 AdmissionDecision.RejectReason 表尾追加四值 NameUnresolved/PayloadUndecodable/ConstructionFailed/ContentConflict〔九步步 8/9 与重投递冲突的失败面——前六值序不动〕；②§10.5 AdmissionOutcome 实现成形〔decision＋envelope〔Admitted 非空——SA-13 唯一权威产出〕＋taskStateAfter/archivePhaseAfter 两轴投影〕；③Ports.hpp 增 IEvaluationOutputDecoder 注入接缝〔payloadCanon 的" EvaluationOutput canonical"字节级契约归 EX-T06 通道协议，execution 不私造第二编码、也不在域外解析域载荷——L5/通道解码器适配；§13 L5 交接清单"三注入适配器"扩为四件，影响面登记于此〕；证据清单（EvidenceManifest）组装口径＝到达条目集＋登记绑定面（快照/切片/Profile 三元组属登记事实，RunEvaluationMaterials.manifestProfile 自派发期评估器注册清单查得）。P-EX-5/P-PR-4 处置：§9.5 单侧冻结文本兑现＋网关线程纪律注释；P-EX-3 处置：runtime 反解经 INameResolverAdapter 注入、零编译依赖零表外边（SA-10）；P-EX-1 处置：消费 evidence（Evaluator/Envelope/Verdict/Snapshot/Evidence）与 project（ArchivePort/ProjectStore/QueryPort/StoreTypes）公共头现状签名，冻结 diff 后按影响面增量同步。CheckpointBatch/Progress/PartialDataOnTerminate 三类到达的登记核对面在接纳编排放行，其持久化分属 EX-T08/自有通道/envelope 随终结路径（PA-1 不伪造第二持久化路径——本任务边界登记）。测试：`sdurws_ird_execution_test` 增 25 用例（EX-REG-1/2/3/6/7/8＋EX-ORT-1＋步 5/6/7 各格＋名称反解门＋解码失败＋组装失败 T9＋有界重试/耗尽 abandon＋A7 两分支＋幂等三连投递事件恰一次——归档写面用真实 ProjectStore＋故障注入转发器、退避 DelayFn 虚拟推进不 sleep；累计 70 用例）＋`sdurws_ird_execution_contract_test` 增 3 用例（EX-REG-4 两项目切换归档 A 原 runDir/不进 B/listRuns D-13 可见性/manifest 五元组＝登记原五元组、EX-REG-8 双接收窗口真实端口 D-14 冲突全链路、envelope 组装复用 evidence 校验器〔接纳产物全字段等于 evidence API 直接组装产物——单点复用不复制的结构性证明〕；累计 12 用例）。证据：traceability/builds/wp08-t04/（双模式构建零错误＋70/70＋12/12 两模式 XML＋ird_gates 零新增命中〔与 wp08-t03 命中集合 diff 为空〕＋validate-task PASS＋故障探针 2 项红/还原复跑全绿） |
| v0.7 | 2026-09-18 | EX-T05（≙WP-08-T06）实现落位登记：§3.1 契约头 **EventBus 一件（新增）**＋Scheduler/StateMachine 两件（扩展）与实现 `src/EventBus.cpp`（新增）落位——**§10.4/§6.2 DomainEventBusImpl**（core IDomainEventBus 的 execution 侧进程内参考实现：专用投递线程〔每总线一条；构造启动/析构 drain on close——已入队事件投递完再退出〕、publish 线程安全即返〔锁内仅簿记〕、同发布者 FIFO＝全局单队列按入队序〔调度线程发布的 TaskStatusChanged 序＝状态机转移序；跨发布者同样保持全局到达序，语义强于契约〕、RAII 订阅句柄 DeliverySubscription〔析构即退订/幂等〕、退订不与投递死锁〔锁结构保证：投递线程每帧锁内取帧＋订阅表快照→锁外逐订阅方回调——回调期间零内部锁〕、waitForIdle 确定性同步面〔队列空∧在投归零——"断言不 sleep"的替代设施〕、事件不持久化 core D-09；进度不入事件）、**§6.1~§6.3/§10.1 ITaskScheduler/ResourceBudget/SubmitResult 契约面＋TaskScheduler 调度本体**（提交验证 V1~V4：V1 形式＝ISubmissionGuard 上下文注入半区〔§3.1 组成行兑现——"属当前存储上下文"是存储侧知识经 L5 注入、PA-1 不私判〕＋快照三元组/冻结〔snapshotId 非保留值〕/CON-06 策略与名称映射非空/mode 值域/评估器注册与契约版本〔evidence IProducerRegistryView 查询面〕→EX-TASK-REJECTED；V2 内容＝修订闭包含性〔evidence IRevisionClosureSource 同源形态注入——objectClosure 每条 (oid,cv) 属锚定修订〕→EX-SNAPSHOT-STALE〔CON-01/EX-SUB-2〕；V3 权限＝Quick/Verified 消费 store.writable()→EX-STORE-READ-ONLY〔PM-07 只读拒绝启动——写权限判定归 project 不私判〕、Preview 跳过；V4 预算＝等待队列容量→EX-TASK-REJECTED〔§6.1 提交期预算检查仅拒绝对列容量溢出〕；**校验面缺失 fail-closed**：协作指针空→对应校验按失败处置拒绝提交＋开发诊断，不静默放行；ISubmissionGuard 可空＝可选增强）、**§4.3 受理段**（互斥临界区内验证→TaskId::generate→TaskRecord{Queued}→状态机 T1 发布 Queued 事件→入等待队列——acceptance 1"五元组分配在提交受理时完成"的落点；capability 受理时经 EvaluatorRuntimeCapabilities 推导〔注册期早于一切提交、与派发时推导等效——不可变纪律更早成立〕）、**§6.1 队列治理**（双优先级 Interactive 全序先于 Background＋同级 FIFO 提交序号单调〔D-04 排序键〕；不抢占——出队只在额度空位；maxConcurrentTasks 0＝自动 min(4,核/2) 下限 1；maxTasksPerProject 同项目**正式**任务上限〔Preview 不计——不写 results/ 不占正式额度〕；**队头阻塞语义**：受限队头挡住整个队列〔严格序可预测性优先于吞吐——实现冻结，§6.1"超出排队"未定义跳过语义〕）、**§6.3 内联门槛**（谓词注入〔可预测 <1 s——ARCH §4.1；"可预测"是评估器/装配侧知识 execution 不私猜〕＋IInlineRunExecutor 执行体接缝＋Preview 三者齐备→锁外执行〔锁只保护簿记不覆盖计算——NFR-PERF-01 结构表达；锁外段机器无并发写者＝tick 线程自身〕→T5→T8/T9 同一转移表；执行体异常逸出→重锁后 T9 兜底不带病穿透；cause 非法→fail-fast）、**§6.1 进度节流**（reportProgress 任意线程入口→主锁内按每任务 100 ms 窗过滤〔≤10 Hz＝实现参数 D-07 非上游值〕→tick 段 a 经 updateProgress 应用；窗内帧丢弃；未知/终态任务帧 fail-fast）、**tick 显式驱动**（段 a 进度应用→关闭期 DrainCoordinator.poll 排空复合拍／非关闭期 controller.poll＋单拍单出派发；**调度线程模型＝Controller/DrainCoordinator 同款阶段 A 显式驱动**：不内建真实线程、tick 即调度域推进原语、主锁域＝调度串行域——§6.2"调度线程（每调度器 1 条）"的生产形态由 L5 固定周期在其自有线程调用 tick 兑现）、shutdown 组合 DrainCoordinator〔§10.1 注兑现：关闭后 submit→ExecutionError(ContextClosed)； drained() 无永久等待轮询面〕。**既有头增量两处**：StateMachine 增 updateProgress（§4.2 progress 行流式更新的受控写面——终态拒绝 fail-fast、非事件通道）；Scheduler.hpp 头注由 EX-T03 单主题扩为两主题（DrainPolicy 排空＋调度器本体）。**归置/边界登记四处（本文与实现的已登记边界）**：①§6.3 内联门槛"仍登记 RunRegistry"的**登记段**依赖派发装配面（RunEvaluationMaterials/归档网关/runDir 布局——EX-T04 登记面），随 EX-T06 派发链就绪统一放开；本任务内联走链完整支持 Preview〔表 1：Preview 不登记不归档——零登记语义自洽〕，非 Preview 任务满足谓词也排队（登记完整性优先）；②Ports.hpp 的 IExecutionModelService/ICompileCacheJudge 未随本任务落位（§3.1 归置行原注"随 EX-T05/T08 落位"）——其真实消费是 Preparing 派发编排（EX-T06 worker 链），本任务不预建无消费者的接口（防 stub），归置修正为随 EX-T06/T08 落位；③capability 受理时推导替代派发时（语义等效，见受理段）；④§10.1"Preview 任务的 submit 要求 capability.archiveless 显式声明"未实现校验——TaskCapability 无 archiveless 字段（§4.2 字段表/evidence descriptor 均未定义该概念，P-EX-7 同源），待词表落位后补校验。P-EX-1 处置：EventBus 事件家族与身份类型消费 core v0.1 Draft 现状签名（Events.hpp/Identity.hpp——DomainEvent 四类家族/总线接口/TaskIdentity/TaskState 零重定义零私改），冻结 diff 后按影响面增量同步（影响面：EventBus 两件＋Scheduler/StateMachine 消费点）。测试：`sdurws_ird_execution_test` 增 19 用例（EX-SUB-1 并发唯一身份/同任务事件 FIFO＋EX-SUB-2 过期快照两变体＋V1 四变体＋PM-07 真实只读存储＋V4 容量溢出＋D-04 优先级/不抢占两用例＋同项目上限/Preview 不计/队头阻塞＋D-07 节流三面＋UX-10 透传＋NFR-PERF-01 非阻塞结构断言〔future 有界等待〕＋§7.5 关闭排空/幂等＋tasksByProject/预算可调——累计 89 用例）＋`sdurws_ird_execution_contract_test` 增 7 用例（同发布者 FIFO/九态流＋ResultArchived 家族逐帧全等/并发零丢失/RAII 退订幂等/退订不与投递死锁〔future 有界等待〕/订阅序投递/waitForIdle——累计 19 用例）。证据：traceability/builds/wp08-t05/（双模式构建零错误＋89/89＋19/19 两模式 XML＋ird_gates 零新增命中〔与 wp08-t04 命中集合排序 diff 为空〕＋validate-task PASS） |
| v0.8 | 2026-09-18 | EX-T06（≙WP-08-T07）实现落位登记：§3.1 契约头 **ChannelProtocol/WorkerSupervisor 两件（新增）**＋Ports 一件（扩展）与实现 `src/ChannelProtocol.cpp`/`src/WorkerSupervisor.cpp`/`src/win32/{ChannelPair,JobScope,ProcessLauncher}.{hpp,cpp}`（新增）/`worker/main.cpp`（新增）落位——**§6.5 IRDCHN/1 通道协议**（帧自定界：magic 7B＋protoVersion u16＋msgType u8＋flags u8＋seq u64＋payloadLen u32＋TaskIdentityBlock 168B 定长域〔四 Id128 规范文本各 36B＋attempt 规范文本 NUL 补齐 24B——§6.5"定长域拼接"的编码实现〕＋载荷；FrameAssembler 字节流增量装配〔vector＋消费位、协议错误闩〕；FrameSequencer seq 序控＝EX-CHN-1 判定本体〔首帧定基线、乱序按 seq 重排窗口〔有界 256 帧实现参数〕、重复/回退丢弃＋开发诊断非致命、窗口溢出/缺口超时〔10 s 实现参数〕→断裂协议错误闩；注入时钟〕；分帧续传 fragmentMessage＋MessageReassembler〔DispatchRequest/FinalOutput 恒分片布局〔16B 分片头 totalLen/fragIndex/fragCount〕、空消息至少 1 片、不可分片类型透传——NFR-PERF-03 不整体装载〕；九类消息载荷编解码〔解码校验失败＝nullopt 协议错误面不抛〕；manifestDigestHex/computeContentIdentity/contentIdentityMatches〔CON-05 同源 SHA-256——握手比对值与 worker 侧身份核对的共用函数〕）、**§6.4/§10.3 worker 监督**（WorkerId/HeartbeatPolicy〔D-06 默认 5 s×3 实现参数非需求值——acceptance 1 明文〕/WorkerStatus/WorkerAssignment〔＋manifestDigest 字段增量——§6.4 握手比对值载体，同 EX-T03 evaluationTimeout 先例〕/WorkerLaunchResult/WorkerEvent＋IWorkerEventSink〔通道消息→结构化事件唯一出口——监督器不理解状态机不接触接纳不写盘〔PA-1〕，"通道消息→结构化事件"处切分进程/任务语义〕/classifyWorkerExitCode〔§6.4 退出码约定集表纯函数化：0/10/11/12/20/21＋约定集外含 0xC0000005 异常码族一律 Crashed＋监督方强杀优先于退出码表〕/WorkerSupervisor 本体〔launch 池取或新建＋握手协议推进〔Hello 版本/manifest 比对→HelloAck 携心跳间隔→DispatchRequest 分帧派发〕＋poll 显式驱动〔每 worker 一条读线程〔§6.2——OVERLAPPED 有界等待读＋进程句柄双源 WaitForMultipleObjects，不无限阻塞 EX-WKR-3 检测基础〕→收件箱→seq 序控→重组→按类型出事件→失联判定〔连续 hungWindow 无心跳且管道无数据〕→自动强杀→退出分类→池纪律〔正常退回 Idle 池、异常 Dead 永不回池——结构保证〕→空闲超时回收〔120 s 实现参数，Shutdown 协作退出〕〕＋requestCooperativeCancel/terminateForce〔TerminateJobObject 恰一次〕＋status/list 并发只读＋HandleAdapter〔IWorkerHandle 适配——EX-T03 TaskController 绑定面兑现〕；**§6.4/§6.6 Windows 设施**（私有实现 src/win32/——R-2 纪律：命名管道对〔CreateNamedPipeW BYTE 模式＋客户端端点主进程预开＋仅两份管道端副本标可继承经命令行传递＋启动后立即关闭继承副本——§6.6 防泄漏〕、作业对象 RAII〔KILL_ON_JOB_CLOSE——每 worker 独立 Job Scope 不存在孤儿 worker〕、启动器〔CREATE_SUSPENDED|CREATE_NO_WINDOW＋挂起→Job assign→Resume 时序消除竞态＋IProcessOps 故障注入接缝——§3.5〕）、**worker/main.cpp 宿主**（握手→派发〔握手期保留非 HelloAck 帧——主进程背靠背写 HelloAck＋DispatchRequest 是常态〕→物化身份核对〔重算摘要不等拒绝执行——§6.5/runtime §9.2 同源纪律〕→临时目录 %TEMP%\ird-worker-<pid>-<run>-<attempt>〔每尝试独立 §6.6〕→DispatchAccept→阶段 A 脚本替身评估器〔progress/batches/checkpoint/final/crash〔0xC0000005 真异常码〕/heartbeat-off/misorder/duplicate/gap/hang/write-temp/error/deverror/exit 动作集——§3.4"阶段 A 仅测试替身"〕→线程收尾 stopThreads〔std::thread joinable 析构 std::terminate 会把正常退出污染成 0xC0000409 崩溃分类——心跳停止＋CancelIoEx 解阻塞＋join〕→临时目录 best-effort 清理〔失败→开发诊断经通道 ErrorReport——EX-ARC-3 边界〕→约定集退出码；WIN32_EXECUTABLE 无控制台——诊断一律经通道 §6.4 边界规则）。**既有头增量一处**：Ports.hpp 落位 IExecutionModelService/ModelPrepareResult/MaterializedDispatch/PrepareOptions（§3.3 原文形态——P-EX-3 处置件，EX-T05 归置登记"随 EX-T06/T08 落位"的兑现；值传递零编译依赖；PrepareOptions 最小承载 preferCompileCache，字段面随 runtime 适配器装配定形）。**实现登记四处（本文与实现的已登记边界）**：①§10.3 launch 后置注"握手完成"的实现读法＝握手为**异步协议由 poll 驱动达成**，launch 的 ok 面＝进程启动＋Job Scope 绑定＋通道建立（同步握手会阻塞调度线程于外部进程——§10.8 无永久等待精神；失配拒绝经 HandshakeRejected 事件由编排方 T7 Failed）；②worker 目标链接形态＝共享源（ChannelProtocol/Errors/TaskTypes 三翻译单元）直接编入 worker＋链接 core/evidence 两条白名单边（ird_gates 以单元级边为粒度，同单元产品目标互链 execution->execution 不在 §3.5 词表且白名单登记文件不在任务 allowedFiles——协议单点实现不变〔同一批源〕，§3.4"链接 sdurws_ird_execution"以编译面等效兑现）；③ErrorReport 载荷增 dev 通道形态（dev=true＝开发级自由文本——§6.4"诊断一律经通道回传"的 worker 侧出口，编排方按空稳定码分流 reportDev；码值仍归 StableCodeRegistry 不私定）；④§3.1 ChannelProtocol 组成行增 FrameSequencer/MessageReassembler/身份核对 helper（序控语义是 §6.5"乱序与重复"行的契约面——接收侧判定本体需公共可测；manifest 摘要函数为握手契约的两侧同源计算件）。P-EX-3 处置：物化经 IExecutionModelService 值传递消费、worker 侧身份核对经 computeContentIdentity 重算（零编译依赖零表外边——ird_gates 零新增命中实证；如架构补登边可平滑直连 D-17）；P-EX-1 处置：消费 core/evidence/project 公共头现状签名（Digest/Identity/DiagData/ProjectStore——零私改零重定义），冻结 diff 后按影响面增量同步；R-3 处置：§6.6 各行逐项以真进程用例实证（句柄继承副本防泄漏/强杀时序/退出码/管道有界等待/KILL_ON_JOB_CLOSE——不超诺声明见各头注释）。测试：`sdurws_ird_execution_test` 增 24 用例（帧编解码/流装配/seq 序控 EX-CHN-1 判定本体〔ManualClock〕/分帧续传/九类载荷/握手摘要/心跳策略钉值/退出码分类表——纯逻辑零进程零管道；累计 113 用例）＋`sdurws_ird_execution_contract_test` 增 8 用例（EX-WKR-1 正常完成九步接纳＋握手失配拒绝派发、EX-WKR-2 崩溃只失败当前任务〔真进程 0xC0000005〕、EX-WKR-3 失联强杀〔ManualClock 判定＋真实 TerminateJobObject〕、EX-CHN-1 真进程缺口断裂、流式分批回传、临时目录隔离与清理、R-3 句柄副本防泄漏——worker 进程为真实进程，8 用例稳定复跑 3 次 8/8 后记录；累计 27 用例；LinkageContractTest 白名单随 worker 目标注册同步扩一项——单元内部引用）。证据：traceability/builds/wp08-t06/（双模式构建零错误＋113/113＋27/27 两模式 XML＋ird_gates 零新增命中〔编码归一排序比对 new-in-t06=0〕＋validate-task PASS） |
| v0.9 | 2026-09-18 | EX-T07（≙WP-08-T08）实现落位登记：**§6.1 内存预算行/§6.6 内存采样行/ARCH §4.6 资源治理的执行侧基础形态**（D-05 三段：65% 节流阈值〔实现参数非需求阈值，§15.1 既有登记〕→停派发/降并行→70% 诊断〔NFR-PERF-04 上游值不改动〕）与实现 `src/win32/MemProbe.{hpp,cpp}`（新增）落位——**§6.6 内存采样行三探针**（querySystemMemory＝GlobalMemoryStatusEx、queryCurrentProcessWorkingSetBytes＝GetProcessMemoryInfo→WorkingSetSize〔PSAPI_VERSION=2 即 kernel32 K32 入口，不引 psapi.lib〕、queryJobPeakCommittedBytes＝QueryInformationJobObject→PeakJobMemoryUsed〔作业创建以来提交内存峰值，单调不回落——与 §6.1"合计峰值 ≤ 物理内存 70%"预算行对齐的保守口径；不超诺声明见头注，kernel32 调用隔离 src/win32/——§3.2/§3.5〕）。**归置登记一处（本文与实现的落位选择，Scheduler.hpp 组成行先例同 EX-T03 DrainCoordinator）**：汇总＋节流决策＋资源不足诊断四件落位 Scheduler 公共头（§3.1 十二模块清单不变）——MemoryReading（主进程＋全部 worker＋物理总量三元读数与占比〔§6.1 汇总口径〕）、IMemorySampler（采样接缝——EX-RES-2"内存采样 fake"的注入面＋§6.2"资源监控线程〔采样〕…决策仍由调度线程执行"中采样/决策可分线程的结构前提）、ResourceController（三段判定＝当前读数纯函数＋层级边沿记忆，确定性〔同读数同预算同上层级→同决策〕；恢复＝随读数回落即时重判、无迟滞带〔防抖依赖采样间隔低通——规模化形态归 WP-23-T06 不提前实现，acceptance 3〕；进入 Exhausted 边沿恰一次出具 EX-RESOURCE-INSUFFICIENT＋经 §3.3 IExecutionDiagnosticsSink 注入外报〔sink 可空＝合法装配，治理动作不依赖上报〕；比较型三要素＝comparison{actual=当前占比, expected=上限, unit="1"〔core Units Dimensionless 冻结 token〕}，SourcedValue 来源 DerivedReadOnly/methodTag memprobe、subject 恒空不伪造 ObjectId；采样失败＝保持最近层级不猜值＋reportDev("execution/resource") 每失败 episode 边沿一次；Config::sampleInterval 默认 1000 ms＝§6.2"周期内存采样"的显式驱动参数化——evaluate 逐拍被调用、真实采样按间隔节流、间隔内复用最近决策）、SupervisorMemorySampler（生产采样件＝MemProbe 三探针＋监督器作业内存聚合；任一半区失败整体失败 fail-to-no-data——半读数会把"worker 超限"误判为"主进程无压力"）、TaskScheduler 集成三处（setResourceController 注入〔可空＝无资源闸合法装配〕；setResourceBudget 转发预算——maxMemoryRatio/throttleRatio 的单一用户入口仍是 ITaskScheduler，EX-T05 登记的"ResourceController 消费"归属兑现；tick 段 c 出队前资源闸——pauseDispatch 本拍不出队，闸只挡出队段不触碰任务状态机，"已排队/运行任务不失败"〔§6.1 括注〕由结构保证〔排队保持 Queued、在途自然运行，D-04 不抢占〕）。**既有头增量一处（§10.3 具体类治理/观测面，同 workerCount 先例）**：WorkerSupervisor 增 aggregateJobMemoryBytes（全部活 worker 作业提交内存峰值求和——§6.6 JobMemory 半区监督器侧聚合；单条查询失败计 0 尽力求和、"半读数"防混淆由采样器整体失败通道承担；仅调度线程）与 reclaimIdleWorkers（§6.1"降低并行度（回收空闲 worker）"池侧执行面，与空闲超时回收同触发语义、区别仅触发源；**按 Idle 记录进程存亡分流**：阶段 A worker 宿主每任务终结即退出〔约定码 0〕→回池记录进程已不在→直接出清池槽〔join→关资源→双表 erase，与 poll 尾部销毁同序——对死进程走 Shutdown 协议永远等不到第二次退出确认，记录会滞留〕；§6.4"进程保活"模型的存活空闲 worker＝Draining＋Shutdown 协作退出同 checkIdleRecycle 路径；Boot/Running/Draining/Dead 不在处置面——在途是"并行度"本身，其新增由调度侧资源闸停派发承载，两侧合为"停派发＋降并行"完整执行面）。**越界声明（acceptance 3）**：规模化验收（大任务分批/流式/检查点下的峰值 ≤70% 基准）归 WP-23-T06，本任务零提前实现；maxWorkers 字段仍无编译期消费者（池规模治理归编排装配 EX-T09/L5——EX-T05 登记的延续，如实陈述）。P-EX-2 处置：三段治理语义逐条锚定 ARCH §4.6/本文 §6.1 明文（ARCH v0.11 Draft 待评审——评审 A9+ 变更按影响面增量同步，影响面集中 Scheduler/WorkerSupervisor 两头）；P-EX-1 处置：消费 core 公共头现状签名（DiagData/Units/Provenance/Identity——DiagnosticRecord/ComparativeFields/ComparativeValue/SourcedValue/UnitToken/ValueProvenance 零重定义零私改）＋evidence/project 消费面零扩展，冻结 diff 后按影响面增量同步。测试：`sdurws_ird_execution_test` 增 9 用例（EX-RES-2 主场景〔fake 采样推至 66%→71%：派发暂停观测点＋诊断恰一次＋事件流全程零 Failed/Interrupted＋回落自动恢复的对照闭环〕、D-05 三段阈值边界 64.9/65.0/69.9/70.0%＝≥语义、耗尽边沿恰一次＋比较型三要素全字段、采样失败保持＋reportDev 边沿、采样间隔节流与重放、预算转发、MemProbe 真实探针冒烟两例、SupervisorSampler 真实半区自洽——累计 122 用例）＋`sdurws_ird_execution_contract_test` 增 1 用例（真进程：worker 跑完 final 回池 Idle→aggregateJobMemoryBytes 非零〔§6.6 worker 半区"覆盖全部工作进程"真值面〕→reclaimIdleWorkers 出清池槽归零＋二次回收幂等——累计 28 用例）。证据：traceability/builds/wp08-t07/（双模式构建零错误＋122/122＋28/28 两模式 XML＋ird_gates 零新增命中〔与 wp08-t06 命中集合排序 diff 为空〕＋validate-task PASS） |
| v0.10 | 2026-09-19 | EX-T08（≙WP-08-T09）实现落位登记：**§8 检查点、缓存与恢复**的存储治理两件与实现 `src/Checkpoint.cpp`/`src/CacheCoordinator.cpp`（均新增）落位——**§8.1/§10.6 检查点协调器**（Checkpoint.hpp 契约面＋CheckpointCoordinator 本体：CheckpointRecord 字段表逐行承载〔payloadDigest＝复合摘要经 core ContentDigester 单点——computePayloadDigest 以 relPath 字典序编码"relPath‖0x00‖文件摘要"，写/读两侧同一函数杜绝两侧编码漂移；verifyCheckpointFiles 逐文件核对＋复合终验给出损坏文件定位〕；写出三段＝批次校验〔请求内预填载荷引用/复合摘要一律被实际批次重算覆盖——防"声明与载荷不一致"伪造面〕→归档端口 begin→writeBatch→finalize〔§8.1 写出位置规则：manifest 发布归 project，本单元零直接磁盘写；环境类失败→abandon 结束责任＋结构化 ArchiveFailed＋EX-ARCHIVE-FAILED，批次残留保留——"失败不破坏已提交检查点"由只增语义兑现；同 (run,seq) 已发布再写＝协议错误 fail-fast〕→会话索引登记〔记录＋登记 archiveDir 原值——A8 恢复装载不重新推导〕；恢复调度＝磁盘**只读**装载→完整性校验〔损坏→EX-CHECKPOINT-CORRUPT＋会话内废弃标记——§8.1"只读登记废弃标记"，零删盘〕→**evidence judgeCheckpointCompatibility 判定单点调用**〔Incompatible→EX-CHECKPOINT-INCOMPATIBLE＋reasons 原样透传——零复制零改写〕→resumable=false 拒绝〔仅供诊断〕；**全部拒绝路径零磁盘写副作用**＝EX-CKP-1/3"原文件保留"的结构保证；listCompatible 复用 evidence::CheckpointSummary〔零第二套摘要结构——§10.6 返回类型原文的归置增量：CheckpointCandidate{id, summary} 配对补主键关联，登记于本行〕并与 restore 同一判定单点，主键字典序确定性；discard＝内存标记，阶段 A 不做 GC〔§10.6 注〕；**信封版本/记录 schema 版本写出时无条件打当前戳**〔§8.1 所有权列"execution（信封）"——请求值不采信，本实现写出的永远是当前版本，P-EX-9"无迁移"的写出面；旧版本记录只能来自历史存储装载，其判定拒绝归 evidence 支持区间条件〕）、**§8.2/§10.7 执行缓存协调器**（CacheCoordinator.hpp 契约面＋ExecutionCacheCoordinator 本体：**ICompileCacheJudge 注入消费随本任务落位**〔§3.3 原文形态——CompileCacheKeyView＝runtime CompileCacheKey 的复合键面值传递、CompileCacheVerdict 三态词表承载；分量指纹表不跨边界〔runtime 侧判定器输入细节，无指纹键 judge 按复合键给保守结论＝runtime CacheKey.hpp 行为契约〕；判定器缺席→模型面按 Incompatible 保守拒绝＋一次性开发留痕——校验面缺失 fail-closed，EX-T05 同口径〕；**结果缓存查找＝按 sliceId 取候选集→真实 evidence::judgeCacheHit 逐候选判定**〔FullHit→ShortPath 短路径＋LRU 刷新；其次 DiagnosticOnly；Incompatible reasons 原样透传——候选集结构是判定语义可观测的前提：精确复合键查表会把 mode-mismatch 吞成 miss，实现口径登记于本行〕；**storeResult 双门槛**＝登记镜像 currentState==Completed ∧ archivePhase==Archived〔§8.2 写入行原文——仅 finalize 产物入缓存，CON-04/TASK-02 执行侧落点；拒绝为设计正常路径走开发通道不发稳定码——缓存是优化非正确性要件〕；**OPT-06 键绑定**＝ResultCacheKey 复合编址〔sliceId＋模式＋契约版本＋Profile——§8.2 编址原文〕＋inputBaselineId 随条目携带〔记录面核对——evidence CachedResultSummary 注原文"不参与 FullHit 判定"〕；**D-10 同键在途合并**＝noteDispatchStarted/noteRunFinished 编排维护面〔RunRegistry noteXxx 先例〕＋lookup WaitForInFlightRun〔共享首个运行完成事件；storeResult/noteRunFinished 解除——失败路径等待者不悬挂〕；**LRU 分账淘汰**＝模型字节预算/结果字节预算/诊断条目数上限三账户独立〔2 GiB 默认＝§8.2 实现参数；三级字典序 lastHit→cachedAt→键序平局裁决——确定性〕；storeModelCache/storePartialDiagnostic〔内容冲突保留既有＋开发诊断——D-14 同精神；部分批次仅入诊断账户，查找给 DiagnosticOnly 且 guidance 恒 DispatchNormal——永不正式命中〕；stats 互斥保护并发只读〔§10.8〕）。**命中≠当前**（CON-04）的结构面＝CacheLookup 无当前性语义字段〔当前性归 evidence computeCurrentness 另判〕。**实现登记两处（本文与实现的已登记边界）**：①CheckpointId 复用 TaskTypes.hpp §4.1 已落位定义〔其头注明文"EX-T08 落地时 include 本头，不重定义"——本头零重定义，字段名 sequence〕；②检查点写入口的消费形态＝构造注入 IResultArchivePort 引用〔§3.2 消费清单行〕，测试以 §11"归档端口 fake"形态承接（ArchiveSessionRef 构造归 project 内部——替身以默认空句柄自持，磁盘断言为真实文件系统断言）；project 侧 checkpoints/ 白名单写入口〔project §4.1 行〕落地前，真存储上下文的检查点写通道不可端到端联调——本任务验证面为协调器契约＋归档端口接口消费，跨单元白名单扩展归 project 侧任务（如实登记，不私改 project）。P-EX-9 处置（acceptance 2）：Incompatible 拒绝＋无迁移代码路径（结构性）＋写出打当前戳（语义性）——宁重算不可错续；迁移工具如需走需求/设计变更。P-EX-3 处置（acceptance 4）：判定逻辑零复制——结果缓存唯一判定调用点＝evidence::judgeCacheHit、模型缓存唯一判定调用点＝注入 judge（两处均无本地复判；契约测试以"协调器输出 ≡ evidence 直接调用输出逐字段相等"结构性证明）；runtime 零编译依赖（产品/测试目标均无 execution→runtime 边——ird_gates 命中集合与 base diff 为空实证；契约测试**不**链接 runtime 以免制造表外边观测面，真适配器归 L5 装配与 EX-T09 按 DTB §4.5 登记后引入）。P-EX-1 处置：消费 core（Digest/Identity/Evaluation/DiagData）、evidence（Compatibility/Slice）、project（ArchivePort/StoreTypes）公共头现状签名，零重定义零私改，冻结 diff 后按影响面增量同步。测试：`sdurws_ird_execution_test` 增 21 用例（EX-CKP-1 篡改一字节→Corrupt＋目录逐字节不变＋损坏自动废弃、缺文件同 Corrupt、EX-CKP-2 契约版本失配→Incompatible＋['contract-mismatch']＋同版本重试恢复成功对照、slice-mismatch、EX-CKP-3 resumable=false→拒绝＋磁盘断言保留＋未标废弃、listCompatible 过滤/排序/失配零候选/显式废弃、写出发布＋载荷面重算覆盖伪造预填＋manifest 面、复合摘要交序确定性、信封版本打戳、四违约 fail-fast、三段环境失败结构化＋临时窗口重写、EX-CCH-2 双门槛五面构造＋唯一正例命中对照、部分批次仅 DiagnosticOnly、EX-CCH-1 mode-mismatch 不派发伪装＋inputBaselineId 绑定＋命中无当前性字段、D-10 同键合并全闭环〔完成解除 FullHit/失败解除回正常派发/异键隔离〕、模型缓存三态透传＋Miss 不调判定器＋判定器缺席 fail-closed＋同键异载荷冲突保留既有＋预算/上限淘汰＋统计——累计 143 用例）＋`sdurws_ird_execution_contract_test` 增 3 用例（EX-CKP-2 restore ≡ evidence 直接判定逐字段相等〔零复制的结构性证明〕、EX-CKP-3 接线面＋真实目录字节断言、EX-CCH-1 lookup ≡ evidence 直接判定＋Quick↔Verified 双向不升降级均 DispatchNormal——累计 31 用例）。证据：traceability/builds/wp08-t08/（双模式构建零错误＋143/143＋31/31 两模式 XML＋ird_gates 零新增命中〔与 wp08-t07 命中集合排序 diff 为空〕＋validate-task PASS） |
| v0.11 | 2026-09-19 | EX-T09（≙WP-08-T10）契约测试套件落地登记：**§11 矩阵补齐**——缺口用例落位：EX-ARC-1（`contract_test/ArchiveFaultContractTest` DiskFullBoundedRetryThenAbandonKeepsCompletedAndSkipsCache——testkit FaultInterceptor 按 `execution/archive/write-batch` 故障点驱动归档端口 fake 注入 disk-full：有界重试恰 3 次写调用＋退避 1 s→4 s 虚拟推进＋abandon(Failed)＋EX-ARCHIVE-FAILED＋任务保持 Completed＋**缓存跳过登记**〔ExecutionCacheCoordinator.storeResult 双门槛——正常归档运行对照闭环〕＋FaultLog 命中观测）、EX-ARC-2（WriteRejected 瞬态/持续两形态同路径——StoreError 透传至重试决策）、EX-ARC-3（`contract_test/WorkerProcessContractTest` TempDirCleanupFailureKeepsTerminalStateAndReportsDev——**真进程 OS 注入**：父进程持 worker 临时目录无共享删除位句柄→RemoveDirectory 必然失败→deverror 开发诊断〔通道 execution/worker-temp〕→退出码 0→T8 Completed 不受影响＋已提交检查点字节不变＋目录残留在盘）、EX-ARC-4 两形态（LateCompletionArchivesAfterShutdownThenContextCloses——shutdown 后迟到完成照常归档至原 runDir、drained() 后 requestClose 才闭合、closed() 时序＝finalize 先于 Closed；ThresholdExceededForcesAbandonBoundedDrain——ManualClock 虚拟推进越阈自动 abandonAllForced 兜底、显式终态、drained() 有界达成、零 sleep）、EX-RCV-1（`contract_test/RecoveryProcessContractTest`＋RecoveryChild——**TestProcessRunner 真进程**：子进程扮演主进程建库→归档预留〔无 manifest〕→标记停靠→kill→重启开库〔D-02 崩溃接管〕→listRuns 空〔不伪装完整结果〕→预留批次字节保留〔现场可续〕→T14 重建 Interrupted＋EX-TASK-INTERRUPTED〔工厂自带状态标注诊断〕；testkit §10.1 触发登记兑现——EX-WKR-2 真进程面已由 EX-T06 期 WorkerProcessContractTest 登记，本任务验收证据并列引用）、EX-RES-1（DoubleDispatchSameTaskRejectedSingleActiveWorker——真 worker 运行中注入同任务第二次派发：状态机 T2 矩阵外抛 InvalidState＋登记表 run 重复登记抛 ExecutionError 两道登记互斥拒绝＋监督器观测面该 run 恰一个 Running worker）。**testkit 设施接入（acceptance 1/2）**：两测试目标按 T-1 允许形态链接 `sdurws_ird_testkit`、链接面 GTest::gtest_main→GTest::gtest＋自有 main（`test/TestMainReport.cpp`；契约目标 `ContractTestMain.cpp` 先于 gtest 拦截子进程模式再安装 RecordListener——PRJ-T15 同款双形态协议）；ird-test-report.json 与 gtest XML 并存（testkit §7.2/§7.3），夹具经 bindFixtureContext 携带 ReproRecord（DeterministicEnv 确定性上下文随每条 TestRecord）。**设施与替身边界声明在案（acceptance 2）**：`test/ContractSuiteFacilities.hpp`（两目标共享——契约目标私有包含根）落 §11 设施列全项声明（ScriptedEvaluator＝worker 内置脚本替身评估器＋ScriptedOutputDecoder 接纳侧承载的双端形态、FakeWorkerMain＝worker exe 内置脚本宿主不另造第二 worker、FaultInterceptor 接缝、ManualClock、TempDir/DeterministicEnv——R-7/自审 A-9 边界声明）＋**faultPointId 命名登记**（`execution/process-launcher/create`、`execution/channel/send-frame`、`execution/channel/recv-frame`、`execution/archive/{begin,write-batch,finalize,abandon}`——testkit §6.4/§10.2 交接义务的登记面）＋设施自证用例 6 例（`test/ContractSuiteFacilitiesTest`：登记表内容与命名约定、P-EX-8 空占位、码值镜像 18 项口径、ManualClock 虚拟推进、脚本 FIFO＋耗尽 fail-closed、TempDir/DeterministicEnv 消费）。**P-EX-8 处置（acceptance 4）**：`NullExecutionDiagnosticsSink` 空实现占位（project §3.2 同模式——sink 名称/归属统一待裁决不私定，不预建 diagnostics 适配器）；kRegisteredExStableCodes＝diagnostics.md §4.6 已收编 18 项的测试侧只读镜像＋isRegisteredExStableCode 自检闸（CollectingSink.hasReportCode 内嵌——清单外码值出现在断言里即测试自身失败），零私定新码值。**AT 载体（acceptance 5）**：AT-10/11/13/34 经 IRD_TEST_INFO 登记（新用例全量；既有用例的追溯由各落位任务以用例名承载——ird-test-report.json 记录面自本任务起生效）；时钟敏感用例一律 ManualClock 虚拟推进、零 sleep 断言。**LinkageContractTest 增量修订两处**：UnitEdgesRegisteredOnly 白名单增 `sdurws_ird_testkit`；NoTestkitEdge 由落位期"全文零 testkit"收窄为"testkit 引用只落在测试目标链接语句行"（产品目标/worker 零 testkit 边的 T-1 钉子保持）。**ird_gates 登记两处**：新增命中恰 2（SUB：两测试目标→sdurws_ird_testkit——本卡 §13 testkit 交接行＋契约 acceptance 明文的消费边；F-173/F-115 同型先例，转登 **F-221**，实例行增补归 WP-01-T03）；比对基线回补说明见 diff 文件（wp08-t08 归一化清单 GBK 吞字缺 testdata_lint 行——F-220 同型处理）。既有 174 用例零改动零回归（WorkerProcessContractTest 仅文件头声明追加＋登记输入构造器抽取复刻，行为面零变更）。本文契约语义零变更（本行为落位登记＋卡头版本行 v0.5→v0.11 追平——F-175 家族同型微瑕消账）。证据：traceability/builds/wp08-t09/（双模式构建零错误＋149/149＋38/38 两模式 XML＋ird-test-report.json×4〔两目标×两模式〕＋ird_gates 命中集合 diff〔新增恰 2、移除 0〕＋validate-task PASS；真进程契约套件连跑 3 次 38/38 稳定） |
| v0.12 | 2026-09-19 | EX-T10 文档与门禁同步（卡内治理任务，DTB §2.9 未列——按 DTB §5.4 增量修订登记；契约 tasks/foundation/EX-T10.json；**纯文档任务零代码变更**——execution/include、src、CMakeLists、worker 零触碰，DoD §5.2-1 构建项按纯文档任务免除）：①**本文与实现零偏差登记（acceptance 1——零偏差显式登记）**——§3.1 组成表 12 契约头＋README＋worker/main.cpp 与磁盘 include/sdurws/ird/execution/ 逐名对应零缺零增；src/ 13 翻译单元＋win32/ 4 对与 §15.4 v0.3～v0.11 各落位登记累加一致（1＋3＋2＋2＋1＋2＋2；§3.1/§3.5 src 列表为落位前预览口径，逐任务落位以 §15.4 为权威——本句为该口径的显式登记，非偏差）；test/ 10＋contract_test/ 10 个 .cpp（含两 main 与 RecoveryChild）＋替身设施头与 EX-T09 登记一致；CMake 链接面与 §3.2 一致（产品目标 PUBLIC 链 core/evidence/project、diagnostics 注入不落链接——P-EX-8、worker 链 core/evidence、测试目标 T-1 形态链 testkit、零 Qt）；两套件复跑 149/149＋38/38 与 v0.11 基数逐数一致零回归；ird_gates 命中集合与 wp08-t09 验收基线排序 diff 为空（GATES-IDENTICAL-18——归一化含 F-220 同型 GBK 吞字修复，builds/wp08-t10/）；本文契约语义零变更、零 DTB §5.4 偏差登记项；核对证据 traceability/builds/wp08-t10/README-audit.md；②**§15.3 待裁决项状态更新（acceptance 2）**——P-EX-1～10 逐项附〔EX-T10 状态同步 2026-09-19〕标注（五栏本体不动、仅在建议栏追加状态段）：**P-EX-5 消账**（对端 project.md P-PR-4 已消账——PRJ-T16 2026-09-17"对端 P-EX-5 消账由其卡承接"，两侧冻结答复逐项一致）；**P-EX-7 消账**（evidence.md v1.1 EV-T10 登记与本文 §5.5 承载方向两侧一致）；**P-EX-10 消账**（依据见④）；P-EX-8 码值已消账、sink 统一维持登记（裁决申请已提交未裁决——wp04-t16-adjudication-requests.md 申请二）；P-EX-1/2/3/4/6/9 维持登记（事实更新：消费卡现版 core v0.11/evidence v1.3/project v0.18/runtime v0.15/policy v0.14/testkit v0.9 均未冻结；ARCHITECTURE v0.12 审计修正未触及本文锚点；九态/中断判据/检查点不迁移均已按登记语义实现承载），十项裁决归属均不变更、不私裁；③**DETAILED-DESIGN.md execution 行状态更新申请提交＋README 指向复核（acceptance 3）**——execution 行"Draft / 已有任务卡"复核一致（与 §1.2 预登记目标、卡头 Draft、unit-status.json 三方一致——2026-09-10 首批索引同步已落位，本任务不私改共享索引行）；实现进度反映的状态更新申请件 traceability/wp08-t10-detailed-design-status-request.md（详设目录维护者裁决）；README 指向 §12 复核零偏差（§15.4 v0.1 修正项＋v0.3 复核项再确认，README 追加本任务复核行）；④**ird_gates/CI 建议提交登记（acceptance 4）**——traceability/wp08-t10-gate-registrations.md：UT-BUILD 七例（test/BuildRedLineTest.cpp ExecutionBuild.*）CI 执行面扩展建议登记 WP-01/WP-24（CI run-tests 正则实测仍钉 `^sdurws_ird_core_test$`——与 POL-T12/EV-T12/RT-T13 登记的同一缺口互为佐证，2026-09-19 复测）；R-4 例外清单＝零申请声明；本任务零新增命中（GATES-IDENTICAL-18）；validate-docs（契约 verify）PASS＋validate-task PASS 留痕 builds/wp08-t10/ |
