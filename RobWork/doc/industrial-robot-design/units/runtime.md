...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.15（RT-T13 文档与门禁同步：README 指向核对零偏差＋§15.3 待裁决项最新状态逐条＋R-4 例外清单提交 WP-01-T01＋UT-BUILD 并入 CI 建议登记 WP-01/WP-24；v0.14＝RT-T12 返工登记〔testdata 行尾钉死〔B-1/F-052〕＋F-053 注释消账〕；v0.13＝RT-T12 实现落位登记＋RT-RES-3 实现修正；v0.12＝RT-T11 实现落位登记；v0.11＝RT-T10 实现落位登记；v0.10＝RT-T09 实现落位登记；v0.9＝RT-T08 实现落位登记；v0.8＝RT-T07 实现落位登记；v0.7＝RT-T06 实现落位登记；v0.6＝RT-T05 实现落位登记；v0.5＝RT-T04 实现落位登记；v0.4＝RT-T03 实现落位登记；v0.3＝全链一致性审计消账〔含卡头版本对齐更正〕；v0.2＝FOUNDATION-CR-01 契约审查修正；v0.1＝首版草案） |
| 日期 | 2026-09-12 |
...
| `Adapter.hpp` | WorkCellConstView、DynamicWorkCellConstView、DeviceView、RobWorkBaselineVersion、IRobWorkAdapterFactory（RobWorkBaselineVersion 自 v0.10 起随 RT-T09 落位——快照身份块记录；DeviceView/IRobWorkAdapterFactory 仍随编译链任务 RT-T11） | §8.2～§8.5 |
| `Resource.hpp` | ResourceRef/ResourceManifest、ResourceReadError、IRuntimeResourceProvider（值类型 ResourceState/ResourceRef 自 v0.4 起随 RT-T03 落位；ResourceBytes/ResourceReadError/IRuntimeResourceProvider 自 v0.10 起随 RT-T09 落位——Compiler.hpp CompileRequest.resources 字段依赖，io 侧实现仍归阶段 B） | §8.6 |
| `Snapshot.hpp` | RuntimeSnapshot、SnapshotIdentity、IRuntimeSnapshotFactory、MaterializedSnapshotCodec（worker 物化） | §9.1～§9.3 |
| `CacheKey.hpp` | CompileCacheKey 及其分量、CompileCacheCompatibility、judgeCompileCacheCompatibility、IRuntimeCompileCacheKey（自 v0.11 起随 RT-T10 落位——实现层增补分量指纹表 CompileCacheComponentFingerprint 与具体生成器 RuntimeCompileCacheKeyBuilder/codecVersionsFromCodecHeaders，判定表行序与 reasons 稳定词汇见 §15.4 v0.11） | §9.4 |
...
// ★ RT-T11 增量（v0.12）：JointDescription/LinkDescription/ToolDescription/
//   SceneObjectDescription 各增 core::ObjectId objectId 字段——S5 装配
//   Canonical 部件 objectId∈objectRefs 的输入面（§4.3.3/§4.3.4），见 v0.12 登记）
struct JointDescription {          // 关节（权威参数化侧；DH 权威时由 modeling 先转为显式表示）
    core::ObjectId objectId;       // 对象稳定身份（RT-T11 增量——∈objectRefs，S5/builder 复核 CM-0）
...
struct LinkDescription {           // 连杆
    core::ObjectId objectId;       // 对象稳定身份（RT-T11 增量）
...
struct ToolDescription { core::ObjectId objectId; std::string localName; std::optional<GeometryRef> geometry;
...
struct SceneObjectDescription { core::ObjectId objectId; std::string localName; rw::math::Transform3D<double> worldPose;
...
② 反向：T_base_world = inverse(T_world_base)（Rᵀ=R；t' = -Rᵀt：Rᵀt = (0,0,-2.0)，t' = (0,0,+2.0)）
   p_base' = T_base_world · p_world = (0.3, 0.2, 0.5) m                      ✔ 与输入逐位一致
   （〔v0.7 修正〕原文 "t' = (0,0,-2.0)" 为转写笔误：代入 Rᵀ=diag(1,−1,−1)、t=(0,0,2)
   得 −Rᵀt=(0,0,+2)；且本行闭环断言唯一要求 +2——若 t'=−2 则 p_base'=(0.3,0.2,−3.5)
   与"逐位一致"矛盾。RT-T06 实现与用例按修正值执行）
...
struct CompileRequest {
    core::ProjectId project;         // ★ RT-T11 增量（v0.12）：目标修订所属项目——
                                     //   S5 header.project 唯一来源（§4.3.1 必填）
...
| NFR-MNT-07/R-4（前缀操作唯一例外） | §7（例外清单已于 RT-T13 提交 WP-01-T01——提交件 traceability/wp06-t13-gate-registrations.md §1） | RT-NM-\*＋静态门禁 |
...
| # | 问题 | 依据 | 影响 | 建议 | 需要裁决者 |
| --- | --- | --- | --- | --- | --- |
| P-RT-1 | 本文消费的 core 契约以其 v0.1（Draft）为基线；冻结版可能调整签名 | core.md 文档头状态行 | RT-T02 起返工 | core 冻结时出 diff 清单，本文按影响面增量修订并留痕。**〔RT-T13 状态同步 2026-09-12〕仍 open**：core.md 现为 v0.11（Draft-Structured，仍未冻结——实测卡头）；runtime 消费面按 v0.1 基线签名实现并经 RT-T02~T12 全量用例钉住（集成 261 例，builds/wp06-t12/），未遇签名漂移；冻结时出 diff 清单的义务不变 | core 详设所有者＋本文所有者 |
| P-RT-2 | ARCHITECTURE v0.11 Draft 待评审 | 架构文档头 | 评审结论可能要求同步（尤其 §7.2⑥/§7.3/§3.5） | 评审后按影响面增量修订。**〔RT-T13 状态同步 2026-09-12〕仍 open**：ARCHITECTURE 现为 v0.12（2026-09-10 全链一致性审计事实修正，仍 Draft 待评审——实测卡头）；其 v0.12 变更记录明示"不改任何架构语义/决策"，对本卡 §3/§7.2⑥/§7.3 锚点零影响、零返工面；评审后按影响面增量修订的义务不变 | 架构所有者 |
| P-RT-3 | ARCH §2.3 L2 行"L1 的 RobWork 数学/运动学类型"与 §5.1/§7.3 要求 runtime 构造 WC/DWC（rw::models/rwsim）的措辞差异 | ARCH §2.3 vs §5.1/§7.3 | runtime 的合法 L1 依赖集需权威确认 | 按 §7.3 明文将 rw kinematics/models＋rwsim dynamics 列为 runtime 依赖（零 Qt 基线库）；请架构评审确认 §2.3 措辞覆盖。**〔RT-T13 状态同步 2026-09-12〕仍 open（架构所有者确认缺位）＋实现侧事实消账**：§7.3 明文依赖集已随 RT-T01/07/08/09/11 在两模式产品链接运转（sdurw_kinematics/sdurw_models/sdursim；ird_gates 框架白名单 sdurw* 允许形态、零 IRD-GATE-LIB 命中——builds/wp06-t12/）；ARCH §2.3 L2 行措辞 v0.12 未变；确认请求维持有效 | 架构所有者 |
| P-RT-4 | 安装预设轴向（倒挂绕 X、壁装绕 Y）为本文选择，MDL-22 未指明 | MDL-22（"预设 180°/90°"无轴向） | 预设矩阵与 modeling 编辑器/用户预期的对齐 | 维持本文定义（§6.2）；modeling 详设起草时交叉核对后冻结，分歧时以建模侧用户口径为准并回改本文。**〔RT-T06 冻结留痕 2026-09-12〕runtime 侧已按 §6.2 冻结定义实现（倒挂＝R_x(π)、壁装＝R_y(π/2)，元素精确 {0,±1}）——实现锚点＝`BaseWorldTransform.hpp` 的 `installationPresetRotation()`（冻结矩阵唯一权威产出点，证据 traceability/builds/wp06-t06/）；modeling 卡（WP-13-T01/T11）交叉核对以该锚点为对照基准，分歧时以建模侧用户口径为准并回改本文＋该函数单点（契约 note/DOC-T06：O-16 登记类不阻塞编译）。**〔RT-T13 状态同步 2026-09-12〕状态不变**：runtime 侧冻结维持（对照锚点同上）；modeling 交叉核对（WP-13-T01/T11，阶段 B）未启动，本同步无新增事实 | modeling 详设所有者＋需求侧确认 |
| P-RT-5 | IRobotDesignReader 注入形态 vs modeling 直接解码其 canonical 编码 | 本文 §4.2（D-02） | 阶段 B 集成形态 | 维持注入 reader（解耦 schema）；modeling 详设起草时交叉核对，若其主张直解再评估。**〔RT-T13 状态同步 2026-09-12〕仍 open**：runtime 侧注入形态已落地——IRobotDesignReader 随 RT-T03 落位于 Description.hpp（S2 消费，RT-T11）、Description 四子结构 ObjectId 契约面随 §15.4 v0.12② 冻结；modeling 详设（WP-13-T01/T11）未起草，交叉核对请求维持 | modeling 详设所有者＋本文所有者 |
| P-RT-6 | ResourceBytes 生命周期已由 io §8.6 / P-IO-2 裁决 | 本文 §8.6；io §8.6 | io 实现与跨边界使用 | 已关闭（设计级）：provider 持有稳定缓冲至析构；调用方同步消费、不跨调用长期持有；Recorded 每次重读重算；实现测试待执行。**〔RT-T13 状态同步 2026-09-12〕已关闭（设计级）维持**：runtime 侧消费面已实现——provider 契约（Resource.hpp，§15.4 v0.10①）与 S4 摘要复核＋S10 前复查随 RT-T11 落地（RT-RES-1/2 行为用例通过，builds/wp06-t12/）；"实现测试待执行"所指 io 侧 provider 实现仍归阶段 B，不影响本关闭结论 | io 详设所有者（2026-09-10 回接） |
| P-RT-7 | 耦合矩阵"病态"阈值（条件数 ≤1×10⁸）为本文设计默认，MDL-21/DYN-04 只说"病态给诊断并阻止" | MDL-21（M-6/M-12） | 阻止边界 | 维持 1×10⁸ 设计默认并随诊断输出实际条件数；建议后续并入 EngineeringPolicySet（工程策略默认类，附录 D 类别说明）——归 policy 时走其登记。**〔RT-T13 状态同步 2026-09-12〕仍 open（归属裁决）＋实现侧事实消账**：1×10⁸ 默认已随 RT-T03 落地（DescriptionValidator 的 kConditionNumberLimit 常量单点，诊断随输出实际 κ；RT-CPL-1 用例通过）；并入 policy EngineeringPolicySet 的建议未启动（policy 卡 P-POL-8/O-12 亦未决），归 policy 时走其登记的路径不变 | 需求所有者＋policy 详设所有者 |
| P-RT-8 | project.md §5.3.6 IModelCompilePort 与本文编译入口的适配差异（CompileResult 无快照句柄、无取消参数）已按兼容处理（§10.2） | project.md P-PR-7 点名 runtime 交叉核对 | 阶段 B 命令路径落地 | 请 project 侧确认无增量需求（如命令路径也要快照句柄——本文可提供但需其 CommandResult 扩字段）。**〔RT-T13 状态同步 2026-09-12〕仍 open**：runtime 侧契约面（CompileRequest/CompileOutcome/ICanonicalModelCompiler，§10.0）已随 RT-T09 冻结并实现 compile() 整体入口（RT-T11）；project 侧确认仍缺位（project 详设所有者未答复），确认请求维持 | project 详设所有者 |
| P-RT-9 | evidence.md §1.2/P-EV-6 登记"project.md 磁盘版本不完整"已过时（2026-09-10 实测完整 §1～§15） | 本文 §1.2 实测 | evidence 侧 P-EV-6 状态应消账 | 请 evidence 详设所有者按磁盘现状更新其登记；本文不受影响（已按完整版承接其 §5.3.6/§10.7）。**〔RT-T13 状态同步 2026-09-12〕仍 open**：磁盘复核（本任务实测）——evidence.md（v0.1）§15.3 P-EV-6 仍按"project.md 不完整"登记未消账；请 evidence 详设所有者更新的请求维持有效；本文不受影响（已按完整版承接） | evidence 详设所有者 |
...
| v0.7 | 2026-09-12 | RT-T06 实现落位登记（DTB §5.4，BaseWorldTransform.hpp/.cpp 随 §12 RT-T06 交付——§6 规则面：安装预设→矩阵、Custom EAA→R、T_world_base 正解/矩阵层校验/预设一致性、正反解〔点变换/重力投影/FK 组合〕、S9 一致性判定；含一处卡文笔误修正＋P-RT-4 冻结留痕＋两处实现层选型登记）：①**§6.5② 卡文笔误修正（事实修正）**——数值例中间式 "t' = -Rᵀt = (0,0,-2.0)" 的数值为转写笔误：代入 Rᵀ=diag(1,−1,−1)、t=(0,0,2) 得 −Rᵀt=(0,0,**+2**)，且同例闭环断言（p_base'=(0.3,0.2,0.5) 与输入逐位一致）唯一要求 +2（若 t'=−2 则回代得 (0.3,0.2,−3.5)，自相矛盾）；§6.5② 已按正确值修订，实现与用例按修正值执行（RT-BW-1 全部其他数值①③④与原卡逐字一致）。②**P-RT-4 冻结留痕（契约 acceptance 3）**——runtime 侧按 §6.2 冻结定义实现：倒挂＝R_x(π)、壁装＝R_y(π/2)，预设矩阵元素精确 {0,±1}（不经三角函数，编码无舍入）；`installationPresetRotation()` 为冻结矩阵唯一权威产出点，即 modeling 卡（WP-13-T01/T11）交叉核对的对照锚点（§15.3 P-RT-4 行已同步留痕；契约 note/DOC-T06 放行口径：O-16 登记类不阻塞编译，分歧时以建模侧用户口径为准回改本文＋该函数单点）。③Custom 预设的 EAA→R 换算（§6.2"R 由 modeling 换算"的注入通道落点）：Description.base 携带的权威编辑表示为 customEaa 旋转矢量，编译链经 `rotationFromCustomEaa()`（Rodrigues 逐元素实现）确定性换算——矩阵入 CanonicalModel、编辑表示不入身份（§4.3.2），换算落 runtime 与 §4.2 单位纪律一致（reader 产出 SI 值）。④一致性检查分工（§4.3.2/S5）：CanonicalModelBuilder 保留其"Custom 而 R≈I"构造拒绝面（RT-T04 既有），本任务交付全量规则函数 `checkWorldBaseTransform()`（有限性/正交性 1e-12/反射 det>0）与 `checkPresetConsistency()`（Inverted/Wall 矩阵匹配＋Custom 非恒等，容差与 builder 同源）——builder 未扩大接入（不重定义 RT-T04 已验收语义），S5 全量接线随编译器任务 RT-T11。⑤S9 一致性检查以纯函数 `checkBaseMountConsistency()` 交付（候选 vs 权威 T 逐元素 1e-9 比较＋≈T·T 二次叠加形态探测——§6.6"含 ≈T·T 形态"；BaseWorldInconsistent 码值装配归 RT-T11，PA-1）。⑥冒烟 header-only 纪律延续：本模块全部矩阵运算逐元素实现（零 rw 外联符号——multiply/operator*/identity/EAA::toRotation3D 均为框架 .cpp 外联；唯一使用的 rw 方法＝Rotation3D::inverse() 原地转置，头内 inline）。实现证据见 traceability/builds/wp06-t06/（双模式 172/172；失败能力探针：禁用反射拦截注入即红〔恰好 RT-BW-6 反射用例〕、精确还原复绿） |
| v0.8 | 2026-09-12 | RT-T07 实现落位登记（DTB §5.4，Adapter.hpp＋src/WorkCellCompiler.hpp/.cpp 随 §12 RT-T07 交付——§5.2 S6＋§8.2/§8.3/§8.4 适配层；含一处分阶段说明＋三处实现层选型登记）：①**Adapter.hpp 分阶段落位**——§3.1 模块表列五实体，本任务按首个消费者原则落位 WorkCellConstView（RT-T07 消费）与 translateRobWorkError（§8.4 签名面）；DynamicWorkCellConstView 随 RT-T08（S7 构造 DWC 后才有视图语义）、DeviceView/RobWorkBaselineVersion/IRobWorkAdapterFactory 随快照/编译链任务（RT-T09/T11——基线版本记录随快照身份块，§8.4），与 v0.4 同例。②**两模式编译分工**——S6 必须构造真实 WorkCell/SerialDevice/Joint（基线非模板类，符号实现于 sdurw_kinematics/sdurw_models 的 .cpp——§8.1 P-RT-3 登记的 L1 依赖正是本任务设计依据面），src/WorkCellCompiler.cpp 与 test/WorkCellCompilerTest.cpp 均按 TARGET sdurw_kinematics 条件增列（仅集成模式）；冒烟模式下 Adapter.hpp 不被任何 TU include，172/172 证明既有面零回归——§5.1 冒烟口径"目标注册＋include 路径"不受影响。③**任意轴关节映射（实现层选型）**——基线 Revolute/PrismaticJoint 固定绕局部 Z 旋转，编译器将权威（axis/zeroOffset）编码为关节静态变换 T_static＝T_parent_joint·R_axis(offset)·R_align（R_align·Z＝axis，极轴分支确定性构造：cosθ＞1−1e-12 取 I、＜−(1−1e-12) 取 R_x(π)、一般分支 EAA Rodrigues 复用 rotationFromCustomEaa 单点），并在子连杆 FixedFrame 放补偿旋转 R_alignᵀ——由共轭恒等式 R_align·Rz(q_rw)·R_alignᵀ＝R_axis(q_rw)，链上连杆系（含法兰）精确等于权威 FK（q_auth＝zeroOffset＋q_rw）；关节 Frame 本身带固定尾巴 R_align·Rz(q_rw)，轴线/原点与权威逐元素一致（绕轴旋转不动轴与原点——RT-EQ-1 断言面）。④**连续关节表达（实现层选型）**——基线无独立 continuous 类型，Continuous 编译为 RevoluteJoint＋显式 ±inf bounds（显式语义表达，与构造默认 ±DBL_MAX 可区分；MDL-12 工作范围不入 WC）。⑤**RT-AD-2 显式设值的落位口径**——Provided 字段逐字/按零位偏置映射覆写（bounds 平移 −offset）；NotProvided 限速/限加速度显式 +inf、限位显式 ±inf（基线默认＝±DBL_MAX/1，与显式值可区分——"无默认残留"由值域可判定性保证）。⑥**Fixed 关节的 zeroOffset 不消费**——无轴关节无 q 语义（合法性边界归 S3 口径，本层不扩大拒绝面）。⑦S9 防御性自检随编译器内置（§12 行"RT-BW-4〔S9 部分〕"）：BaseMount 读回值经 checkBaseMountConsistency 复核，不一致即 BaseWorldInconsistent fail-fast（实现缺陷类，不得放行）；几何资源（visual/collision）的 WC 挂接随资源 provider（io 阶段 B）接入任务落位——§12 行交付范围为 Frame 树/Device/Joint/限位，已列分阶段。实现证据见 traceability/builds/wp06-t07/（集成 188/188、冒烟 172/172；失败能力探针×2：禁用连杆尾巴补偿→恰好 RT-EQ-1 变红、禁用 maxAcceleration 显式设值→恰好 RT-AD-2 变红，均精确还原复绿） |
| v0.9 | 2026-09-12 | RT-T08 实现落位登记（DTB §5.4，src/DynamicWorkCellCompiler.hpp/.cpp 随 §12 RT-T08 交付——§5.2 S7：DWC 构造＋能力门控＋§8.5 关联校验；含 Adapter.hpp 一实体落位＋R-2 前置实测结论三条＋四处实现层选型登记）：①**Adapter.hpp 分阶段落位（续 v0.8①）**——DynamicWorkCellConstView 随本任务落位（§8.2/§8.3 只读包装纪律的 DWC 面：dynamicWorkCell()/findBody()/bodyCount() 全 const 查询；空 Ptr 构造→RobWorkError fail-fast）；DeviceView/RobWorkBaselineVersion/IRobWorkAdapterFactory 仍随 RT-T09/T11。②**R-2 前置实测结论（契约 acceptance 3，源码头文件实测）**：(a) rwsim DWC 构造器齐全（WorkCell::Ptr＋Body/Device/Constraint/Controller 列表整体构造、一次注册进 StateStructure）；(b) **RigidBody 强制要求 Object 基帧为 MovableFrame**（RigidBody.cpp 构造 RW_THROW 实测），而 S6 产物连杆帧为 FixedFrame——故 S7 为每个移动连杆在 WC 树内挂接 Body 承载 MovableFrame（名＝映射 Body 条目，§7.1"IRB6700.link_2.body"），承载帧挂连杆帧之下恰为 RigidDevice 的 RigidLink 向上回溯父关节（jointIdx 定位）的结构前提；(c) **rwsim 摩擦 API 为 material-pair 载体、无逐关节摩擦一等职**（MaterialDataMap::addFrictionData/FrictionData{type=Coulomb|Custom, parameters 自由名值} 实测）——偏置摩擦模型 {fv,fc,bias}（MDL-16）按 §8.8 默认"无补丁方案"承载（见④），不触发补丁流程、不作能力降级（能力位仍由 S5 按内容派生，runtime 只承载事实）。③**门控与降级落位（§5.2 S7/§5.6/§9.1）**——S7 重走连杆物性四态（mass/centerOfMass/inertia 均 Provided）得缺失清单，与 model.capabilities().hasDynamicWorkCell（S5 派生，PA-1）防御性互核（不一致→DwcCompileFailed 内部违约 fail-fast）；门控为假→SkippedNoPhysics＋缺失对象清单（链序）＋恰一条警告级 RT-CAPABILITY-MISSING（subject＝首个缺失连杆，§9.6）正常返回（零副作用、不触碰 WC）；门控为真才构造。④**摩擦承载与 Body 装配（实现层选型，R-2(c) 的落地面）**——有物性关节在 DWC 的 MaterialDataMap 注册材质（id＝该关节映射全名——名称唯一源仍是映射，R-4 合规）并在 (mat,mat) 对登记 Custom 摩擦数据（typeName 冻结 "ird-joint-friction"，参数名冻结 fv/fc/bias，dynamics 阶段按此约定检索）；Body 装配：基座连杆＝FixedBody（承载 FixedFrame 挂 BaseFrame 下，名＝映射 Body 条目）、移动连杆＝RigidBody（承载 MovableFrame，入 RigidDevice 对象序列）、**有物性工具＝FixedBody 锚其既有 Tcp 帧**（§7.1 Body 范围仅覆盖 link，工具体复用 Tcp 条目名——避免映射规则外拼名〔R-4 例外清单在 NameMap 单点〕；其"随法兰运动"的引擎语义归阶段 B 集成，如需工具体 MovableFrame 载体须先扩映射 Body 范围——契约面变更，届时另行走 DTB §5.4）；Body 覆盖校验（§8.5 行 2）按"全连杆＋有物性工具"计数逐名核对。⑤**销毁顺序承载（§8.5 行 4，RT-AD-3）**——DynamicWorkCellCompileOutcome 成员声明序〔…dynamicWorkCell, workCell〕即 §8.5 快照声明序的实现面：析构逆序使 WC 成员先出作用域、WC 对象因 rwsim DWC 自持 WC 引用存活至 DWC 释放（"DWC 先于 WC 释放"由引用计数观测钉住）；**RT-T09 的 RuntimeSnapshot 私有成员必须保持同一相对声明序**；MSVC 工具链无 ASAN，RT-AD-3 以 shared_ptr 控制块计数做泄漏/UAF 等价观测（weak 过期时序断言），登记为该用例的证据形态。⑥**scopedFullName 提升（小幅重构）**——S6 的命名定位封装由 WorkCellCompiler.cpp 匿名空间提升为 WorkCellCompiler.hpp 单元内共享声明（S7 的 Body 承载帧/连杆帧/基座帧/Tcp 名同一命名唯一来源，消除重复实现）；S6 行为与既有 16 例零变化。⑦**§8.2"DWC 重力"子项消账**——DWC 重力显式设为 model.world().gravityWorld（基线构造默认 −9.82 与 canonical 默认 −9.81 数值可区分，读回即设值证据）；MotorForceLimits 等无 canonical 对应字段不设值（不发明数据）。⑧**两模式编译分工（续 v0.8②）**——S7 构造真实 rwsim 对象（DynamicWorkCell/RigidBody/RigidDevice 为 sdurwsim 非模板外联符号），src/DynamicWorkCellCompiler.cpp 与 test/DynamicWorkCellCompilerTest.cpp 均按 TARGET sdurw_kinematics 条件增列；冒烟 172/172 零回归。实现证据见 traceability/builds/wp06-t08/（集成 199/199＝既有 188＋新增 11〔RT-CPX-1/2、RT-AD-3、§5.6 正交、§8.5 关联校验、R-2 摩擦承载、DWC 视图〕、冒烟 172/172；失败能力探针：禁用能力门控→恰好 RT-CPX-2 两例变红、精确还原复绿；ird_gates 命中集与 wp06-t07 基线逐项相同＝既有登记 IRD-GATE-SUB/T1 两项、零新增） |
| v0.10 | 2026-09-12 | RT-T09 实现落位登记（DTB §5.4，Snapshot.hpp/.cpp＋Compiler.hpp 随 §12 RT-T09 交付——§9.1～§9.3/§9.5：RuntimeSnapshot 全字段表＋§8.3 IRuntimeModelView 实现＋§8.5 声明序持有＋快照身份＋工厂 create/materialize＋§9.5 MaterializedSnapshotCodec；含一处分阶段落位＋三处实现层选型登记＋CR-05 值供给样例）：①**Compiler.hpp 分阶段落位（首个消费者原则，§3.1 行实体）**——§10.0 冻结的契约面（CompileOptions/CompileRequest/CompileStatus/CompileOutcome/ICanonicalModelCompiler＋GeometryDetail 单值枚举）由本任务先行冻结（IRuntimeSnapshotFactory::create 按值消费上述实体，无契约面则工厂无法成文）；十段链产品实现（事务/取消/资源复查的 compile() 整体入口）仍归 RT-T11 增量交付——契约面只增不改。同批按字段依赖落位：Adapter.hpp 增 **RobWorkBaselineVersion**（v0.8① 预留——基线记录随快照身份块，§8.4；取值＝vendored 基线锚 31b81845＋工具链摘要，WP-24-T01 冻结精确基线串后仅替换 capture() 常量单点，O-20 同源待确认）；BaseWorldTransform.hpp 增 **kBaseWorldRuleVersion＝1**（§9.4 缓存键分量 baseWorldRuleVersion 的单一权威常量）；Resource.hpp 增 **ResourceBytes/ResourceReadError/IRuntimeResourceProvider**（§8.6 原文——CompileRequest.resources 字段类型依赖，io 实现仍归阶段 B）。②**快照身份与缓存键（实现层选型）**——snapshotIdentity 身份域＝§9.1"入"列全集按表行序编码（RT-Codec 家族同规则：大端/长度前缀；**含 revisionSeq**——快照层"来源定位"与模型层 §4.3.1"不入"的区分是字段表原文两处各自成立）＋排除 diagnostics/createdAtUtc/createdFrom/snapshotIdentity 自身（编码域结构层排除）；workCellCompileIdentity 按 §9.1 公式单点实现于 snapshotidentity::computeWorkCellCompileIdentity（baseWorldRuleVersion 不作参数、恒取 kBaseWorldRuleVersion 单点；compileOptions 取 DWC 无关子集——D-04 分层），RT-T10（CacheKey.hpp）复用该函数避免第二套键编码。③**worker 物化（§9.5 实现层选型）**——载荷"对象字节"承载＝CanonicalModel 的 IRDCANO 全字段编码（worker 无 reader，编译后物化而非字节重编译——§4.1/D-01"可经 RT-Codec 序列化供 worker 物化"；rtcodec::parse 的 builder 全量复核＋身份复核＝篡改/半传输的模型层防线）；**编译器版本面（compilerContractVersion/compilerVersion）随载荷值传递**——快照身份含 compilerVersion（§9.1"入"），worker 不持编译器实例，值传递保证重建快照 snapshotIdentity 与主进程产物逐字节相等（RT-SNAP-2 断言）；载荷"资源字节"随资源几何挂接（v0.8⑦）与 S4 复查（RT-T11）落位，本版不含。④**工厂编排与门禁（§5.2 S10/§9.6 组合表）**——create 经 ICanonicalModelCompiler::buildCanonicalModel 分段入口（§10.0 原文"测试替身注入点"）取得模型后编排 S6→S7→S8→S10：段边界轮询取消令牌（取消→Cancelled＋RT-CANCELLED 非 error 诊断）；UnknownObject/ContextReleased 按 §3.4 总纲保持 fail-fast 异常轨（不发稳定码——RT-SNAP-3 的拒绝观测面）；std::bad_alloc→ResourceBudget（§5.5）；S8 交叉校验对 S6+S7 写入 WC 的全量帧名＋设备名执行（WORLD 根帧不入映射故排除）；发布门禁执行 §9.1"dynamicWorkCellState 与 capability 一致（不一致构造拒绝）"——Compiled⟹hasDynamicWorkCell、Skipped 带缺失清单⟹能力位 false；DwcSnapshotState 的两成因（物性缺失跳过〔清单非空〕/未请求 capabilityLevel〔清单空〕）随 §9.4 分层键语义登记。⑤**CR-05 值供给样例（跨单元红线消账）**——RuntimeSnapshot 暴露 modelIdentity/nameMapIdentity/robworkBaselineVersion（＋compilerContractVersion）只读取值入口＝evidence 侧唯一供给面；样例＝固定种子 rich 夹具＋(1u,"cr05-sample-1.0")＋RobWorkBaselineVersion::capture()，断言快照暴露值＝模型/映射/采集点计算值（值传递无重编码）＋toCanonical 往返＋样例确定性——EV-T04 侧以同一样例断言其 Environment 条目 `runtime.model-identity`/`runtime.robwork-baseline` 编码值与本测试输出逐字节一致（evidence 不重算 CanonicalModel）。⑥**两模式编译分工（续 v0.8②/v0.9⑧）**——工厂/快照消费 S6/S7 真实框架类，src/Snapshot.cpp 与 test/SnapshotTest.cpp（新增 14 例：RT-SNAP-1～4、RT-CPX-4 取消/bad_alloc、CR-05 值供给、materialize 往返＋D-13 拒绝、身份确定性＋键公式、失败诊断/违约轨、DWC 两成因）均按 TARGET sdurw_kinematics 条件增列；Snapshot.hpp 在冒烟模式不被任何 TU include，冒烟 172/172 零回归；RT-SNAP-1/4 的 TSAN/ASAN 以等价评审观测承载（并发压力＋逐线程 State 写入互扰检查／weak_ptr 过期时序），与 RT-AD-3 证据形态同款登记。实现证据见 traceability/builds/wp06-t09/（集成 213/213＝既有 199＋新增 14、冒烟 172/172；失败能力探针：禁用 D-13 modelIdentity 核对→恰好 WorkerIdentityMismatchRejected_D_13 变红、精确还原复绿 213/213；ird_gates 命中集与 wp06-t08 基线逐项相同＝既有登记 IRD-GATE-SUB/T1 两项、零新增） |
| v0.11 | 2026-09-12 | RT-T10 实现落位登记（DTB §5.4，CacheKey.hpp/.cpp 随 §12 RT-T10 交付——§9.4：CompileCacheKey 分层键＋IRuntimeCompileCacheKey＋judgeCompileCacheCompatibility＋CompileCacheCompatibility；含一处实现层增补＋四处实现层选型登记＋NameMap 一常量单点增设）：①**分量指纹表（实现层增补，§9.4 草图两字段之上）**——judge 的"分量级 reasons"（RT-CACHE-1"reasons 精确"、RT-CACHE-3"contract-changed"）需逐分量可比数据，两个复合摘要无法还原差异出自哪个分量；CompileCacheKey 增补 **components**（8 条 WC 层分量指纹 CompileCacheComponentFingerprint：model/contract/compiler/baseline/namemap-rule/base-world-rule/codec/wc-options，声明序＝键编码域分量序；model 分量直接取 modelIdentity——其本身已是摘要，不再二次摘要）；该表只增不改草图两字段的名字/类型/语义，judge 对无指纹键仍按复合键判定（reasons 退化为保守 workcell-key-changed）。②**DWC 键链式派生（实现层选型）**——dynamicWorkCellKey＝SHA-256(workCellKey‖requestDynamicWorkCell)：capabilityLevel 是 WC 无关子集之外唯一增量分量，链式派生结构性保证"任一 WC 分量变→DWC 键必变"（RT-CACHE-1 判据前提），避免全量分量第二遍编码；缓存侧 SkippedNoPhysics 落账 nullopt 的规则属 execution 存储职责（§9.4 首句"存储、命中执行、淘汰归 execution"——acceptance 2 边界），头注释声明、不设存储 API。③**具体生成器构造入参（实现层选型）**——RuntimeCompileCacheKeyBuilder 构造期固定编译器/环境侧分量（契约版本 ≥1／实现版本/基线版本非空的 fail-fast 校验前移到构造期——§10.0 buildKey"无（不抛）"的成立前提），默认值＝各单点（RobWorkBaselineVersion::capture()〔§8.4 采集单点〕/kNameMapRuleVersion/codecVersionsFromCodecHeaders()〔各编码头常量组装——§9.1"消费方不得另写字面量"〕）；值注入形态与 worker 物化"编译器版本面随载荷值传递"（v0.10③）同构；baseWorldRuleVersion 不经手——恒在 computeWorkCellCompileIdentity 内部取 kBaseWorldRuleVersion 单点。④**reasons 稳定词汇（一经交付不得改语义）**——WC 层逐分量：model-changed/contract-changed/compiler-changed/baseline-changed/namemap-rule-changed/base-world-rule-changed/codec-changed/wc-options-changed（按编码域分量序）；DWC 层：dwc-missing-in-cached（RT-CACHE-2）/dwc-not-requested（请求不要求而缓存有——保守不报完整命中）/dwc-key-mismatch；门与兜底：key-invalid（未 finalize/伪造部分键）/workcell-key-changed；FullReuse 时 reasons 为空；WorkCellOnlyReuse 有向（请求侧为 DWC 重编处置方）。⑤**未 finalize 键的判定面表示**——buildKey 对无效模型（contentIdentity 全零）不抛（§10.0 属性表），返回零键＋DWC nullopt＋空指纹；judge 第 1 步有效性门对"任一侧 workCellKey 全零或在场 DWC 键全零"恒 Incompatible(key-invalid)——部分/失败产物在判定面被结构性拦截，无占位合法键伪装命中面（CON-04，acceptance 3）。⑥**NameMap.hpp 增 kNameMapRuleVersion＝1（常量单点增设，v0.5③ 登记值的常量化、v0.10 kBaseWorldRuleVersion 先例）**——m_ruleVersion 成员默认与 buildRuntimeNameMap 赋值改取该常量（值不变、消除两处字面量），CacheKey 键分量默认同源；待产品确认冻结后仅替换此单点。⑦**两模式编译分工（续 v0.8②/v0.9⑧/v0.10⑥）**——CacheKey.cpp 消费 src/Snapshot.cpp 的 snapshotidentity::computeWorkCellCompileIdentity（v0.10② 公式单点——不写第二套 WC 键编码），与 test/CacheKeyTest.cpp（新增 7 例：RT-CACHE-1 分层与确定性＋逐分量扰动精确 reasons＋指纹集合钉住〔策略/种子/线程无条目——CON-06〕、RT-CACHE-2 WorkCellOnlyReuse 非 FullReuse、RT-CACHE-3 旧契约版本拒绝、RT-CACHE-4 命中≠当前性〔只断言 runtime 侧〕、未 finalize/伪造部分键恒不命中）均按 TARGET sdurw_kinematics 条件增列；CacheKey.hpp 在冒烟模式不被任何 TU include，冒烟 172/172 零回归；WC 键与公式单点同参数输出逐字节相等有行为钉住（BuildsTwoLayerKeys 用例）。实现证据见 traceability/builds/wp06-t10/（集成 220/220＝既有 213＋新增 7、冒烟 172/172；失败能力探针：判定表 RT-CACHE-2 行 verdict 注入为 FullReuse→恰好 2 例变红〔WorkCellOnlyReuseNotFullReuse_RT_CACHE_2／ComponentPerturbationPreciseReasons_RT_CACHE_1 能力级别分支〕、精确还原复绿 220/220；ird_gates 命中集与 wp06-t09 基线逐项相同＝既有登记 IRD-GATE-SUB/T1 两项、零新增；validate-task 三查 PASS） |
| v0.12 | 2026-09-12 | RT-T11 实现落位登记（DTB §5.4，Compiler.hpp 增量＋src/CompilerImpl.hpp/.cpp＋src/SnapshotAssembler.hpp 提升随 §12 RT-T11 交付——§5.1～§5.7 十段链 compile() 整体事务＋buildCanonicalModel 分段入口＋§5.3 事务状态机＋D-11 协作取消＋§5.4 资源摘要复查；含两处契约面增量＋三处实现层选型登记＋S8 警告转译落位＋一处重构）：①**CompileRequest 增 project 字段（契约面增量——§10.0 只增不改，上方代码块已同步）**——§4.3.1 CanonicalModelHeader.project 是"空 id→构造拒绝"的必填身份字段（来源定位三元组之一），而原请求面与 §3.3 RevisionSummary 最小投影均无 project 承载，S5 装配没有合法来源；本字段是 S5 header.project 的唯一权威来源（调用方＝L5 装配/命令侧经 project 查询上下文取值），全零＝未提供→S0 前置校验 InputInvalid（可恢复——补值后重编译；行为证据 MissingProjectContextRejected 用例）。②**Description 四子结构增 ObjectId 字段（契约面增量——§4.2 节选已同步、§3.1 行已注）**——§4.3.3/§4.3.4 规定 CanonicalJoint/CanonicalLink/CanonicalTool/CanonicalSceneObject 的 objectId"∈ header.objectRefs；全模型唯一"，而 JointDescription/LinkDescription/ToolDescription/SceneObjectDescription 原文本无承载字段（对象身份在 modeling schema 内、reader 解析时可得，映射语义归其实现）；字段语义与 Canonical 侧一一对应（∈objectRefs＋全模型唯一由 S5/builder 复核 CM-0），不入任何编码（Description 无 RT-Codec 编码）。S2 只解析 robot-design 对象（§5.2 S2 行原文），robotObjectId 由 objectTypeToken 命中 kRobotDesignObjectType（Sources.hpp 常量单点，v0.12 增设）定位——恰一语义（零个→InputInvalid／多个→StructureInvalid，实现层登记）。③**S10 前资源摘要复查（§5.4 交付核心）**——S4 逐资源读取＋摘要复核（ResourceMissing/ResourceChanged/ResourceBudget 三码分立＋resourceId 定位，RT-RES-1）并记录已消费清单；compile() 在 S9 通过后、S10 装配前对 Recorded 资源逐项复读比对，任一不符（含复读失败——实现层登记：S4 时点存在、S10 前不可得本质是内容变化）→ResourceChanged 整体失败（RT-RES-2 行为证据：替换编程下 provider 恰 2 次调用＝复查时点在发布前）；Solidified 免复查（同替换编程下 Published 且恰 1 次调用）；Recorded 资源随 S4 产出 RT-RESOURCE-RECORDED 警告级诊断（§8.6"不代阻断"，建议码待 diagnostics 收编——PA-1，收编后仅替换码常量单点）。④**S1～S5 装配（§5.2 前五段产品实现）**——S1 锚定（tryRevision 未命中→UnknownObject 违约轨）；S2 全量闭包校验（objectRefs 每条 objectInRevision——CM-0 产品执行点，RT-CONT-1）＋字节摘要复核（登记 digest≠实得→StructureInvalid，CM-0 字节面）＋reader 解析（格式版本固定传 1——实现层登记：ObjectRefEntry 无格式版本字段，协商随 modeling 阶段 B/P-RT-5）；S3 经 DescriptionValidator（Error 全量拼入 detail 不短路、Warning 随模型发布）；S5 装配接线 §6 规则全量（resolveWorldBaseTransform 唯一产生入口＋checkWorldBaseTransform＋checkPresetConsistency——v0.7④ 预留的本任务接线点）＋builder 不变量（builtFrom＝robot-design 对象字节 SHA-256〔"reader 输出的身份"落点，实现层登记〕；deviceName 承载 robotLocalName 原值——消歧后事实名以映射/S6 产物为准；zeroOffset＝0——§4.2 显式表示无零位偏置承载、已折叠进限位/原点；material 显示属性不映射——§4.3.6）。⑤**取消与超时（D-11/TASK-01）**——每段边界（S1/S2/S3/S5 前＋S6/S7/S8/S10 前）与 S4/复查逐资源循环轮询 ICompileCancelToken（无资源夹具轮询密度恰 8 次，行为钉住）；取消→RollingBack→Cancelled＋非 error 诊断（UX-03）、可重入重试；**编译器无任何内部超时机制**（无时钟读取/无期限检查——acceptance 2 静态事实＋行为面用例）；bad_alloc→ResourceBudget（§5.5）。⑥**SnapshotAssembler 提升重构（行为零变化）**——S8+S10 装配尾段与终态工具（cancelRequested/cancelledOutcome/failedOutcome/toDiagnostic）由 Snapshot.cpp 提升至 src/SnapshotAssembler.hpp（v0.9⑥ scopedFullName 先例：产品编译器与工厂共用单一发布逻辑，消除第二套实现）；同批落位 v0.5⑥ 的 S8 警告转译（translateNameMapNotices：RuntimeNameNotice→RT-NAME-DISAMBIGUATED 警告级诊断，建议码待 diagnostics 收编；assemble 增 extraWarnings 合并通道——默认空＝工厂既有调用零变化，三编排路径均传入）。⑦**状态机转移覆盖口径**——S6 之后各段的失败在产品输入面为"实现缺陷类"不可达（§5.3 原文：S9 不可放行/理论不触发），其转移由 RT-T07/T08 单元级失败用例（RT-CPX-1 的 S7 层原子性）＋CompileTransaction 单元级用例（rollback 幂等＋发布后无失败路径）共同承载；产品可达失败面（S0/S2/S3/S4/S5）各有具名用例钉住。⑧**两模式编译分工（续 v0.8②…）**——CompilerImpl.cpp 消费 S6/S7 真实框架类，与 test/CompilerTest.cpp（新增 24 例）均按 TARGET sdurw_kinematics 条件增列；Compiler.hpp 的 CompileStage 枚举（§3.1 本头行"编译链阶段枚举"落位）与 project 字段为纯值契约面，两模式均可用，产品实现冒烟不被任何 TU include。实现证据见 traceability/builds/wp06-t11/（集成 244/244＝既有 220＋新增 24、冒烟 172/172 零回归；失败能力探针：禁用 S10 前 recheckConsumedResources→恰好 RT-RES-2 变红、精确还原复绿 244/244；ird_gates 命中集与 wp06-t10 基线逐项相同＝既有登记 IRD-GATE-SUB/T1 两项、零新增；validate-task PASS） |
| v0.13 | 2026-09-12 | RT-T12 实现落位登记（DTB §5.4，runtime/test/ 的 §11 设施与契约套件＋testdata 数据集随 §12 RT-T12 交付；含一处产品实现修正＋两处偏差登记＋一处观测点口径）：①**§11 设施规范落位**——test/RuntimeTestDoubles.hpp 收拢 §11 设施清单原文命名的替身（ScriptedObjectSource/ScriptedClosureSource〔内存对象库 ScriptedObjectLibrary〕/FakeResourceProvider〔可编程缺失/变化/预算〕/ScriptedReader〔Description 直构〕/CancelToggle〔原子取消注入〕）＋ContractHarness 确定性装配（id/摘要固定种子派生——跨进程字节一致的夹具前提）；RT-T02～T11 各 TU 私有同型替身保留原状（行为零变化，收拢面供契约套件与后续跨单元契约测试消费）。②**契约套件（test/ContractSuiteTest.cpp，集成模式）**——§11 矩阵的组合面缺口用例：RT-ID-1 跨进程身份稳定（父进程编译＋子进程经 gtest 过滤器再入同夹具，模型/快照/映射身份与 WC 帧名集逐字节相等——ARC-03/AT-16 载体）、RT-ID-2（1e-15 浮点差：容差判等 vs 身份位模式不等 vs 缓存 Incompatible+model-changed）、RT-ID-3（同内容重编译身份不变＋判定 FullReuse 无失效类 reasons——显示名非 runtime 输入面的类型层事实随用例注释留痕）、RT-BW-1（§6.5 数值例经快照视图 worldToBase/baseToWorld/gravityBase，容差经档案 rt-runtime）、RT-BW-3（默认地面恒等变换编译面）、RT-BW-4（消费端二次叠加自检拦截＋偏差比较字段）、RT-BW-5（kinematics/trajectory/dynamics/policy 四消费替身按 §6.4 各自读取路径互等——AT-37 载体）、RT-CAP-1/2（混合缺失能力位＋subject 定位首个缺失连杆；替身消费方按声明处置＋公共头零 EngineeringStatus 扫描）、RT-RES-3（路径不作身份）、RT-EQ-1/RT-NM-1 数据集载体（见④）。③**RT-RES-3 产品实现修正（findings F-049）**——RT-T04 落位的 Codec.cpp 身份域编码与 RT-T09 落位的 Snapshot.cpp 快照身份域编码把 ResourceRef.sourcePathHint 计入了身份字节，与 §8.6 规则总表"路径不作身份：路径变化而内容不变→身份不变"直接矛盾（且与 Snapshot.cpp 既有注释自相矛盾）；RT-T12 契约套件 RT-RES-3 用例检出后修正：身份域只编码 resourceId+contentDigest+state+accessVersion，全字段域保留 pathHint 供往返（parse 只接受全字段编码——往返/物化不受影响；全字段格式零变化）。④**数据集交付（acceptance 3）**——testdata/golden/rt-fk-equation/1.0.0/（analytic-case：平面 3R 闭式 FK 四构型〔零值/近零/正负抵消/一般位〕，generate/make_fk_equation.mjs 闭式解独立于生产实现，三方对照＝数据集期望↔测试内手写 FK↔RobWork Device FK，附录 D 第 4/5 项 1e-9）＋testdata/golden/rt-namemap-roundtrip/1.0.0/（contract-fixture：平面 2R 全量 9 条目〔身份作用域＋BaseMount/BaseFrame/Flange 派生〕，AT-18 双向往返精确等值）＋testdata/tolerance/rt-runtime/v1.0.0.json（**偏差登记：§11 设施行助记名"rt"不满足 testkit.md §4.3.1 profileId 句法 [a-z0-9-]{3,64}（≥3 字符），以 rt-runtime 承接**，basis 声明附录 D 第 4/5/12 项——第 12 项身份精确等值无数值条目，经 IRD_EXPECT_IDENTICAL 承载）；lint 经 testkit GoldenDataset/ToleranceProfile 装载全链（GoldenDataLintTest.cpp，两模式编译进测试目标——"随 CI 跑通"）＋独立工具 sdurws_ird_testdata_lint 实跑零违规（字典 192 ID）。⑤**RT-STUB-0 边界声明在案（acceptance 2）**——runtime/test/README.md §1 全文声明（替身只覆盖注入接口；RobWork/rwsim 本体与被测编译器/工厂不做替身；手写 FK 仅测试对照工具；未执行不得标注通过）＋RuntimeTestDoubles.hpp 文件头同款；§3 给出 §11 矩阵→测试文件落位全表（RT-T02～T12 累积）。⑥**RT-CAP-1 观测点口径**——"每缺失一条警告"的已实现落点＝S7 门控降级事件恰一条 RT-CAPABILITY-MISSING（subject＝链序首个物性缺失连杆，§9.6）；摩擦/几何/工具缺失的能力事实由能力位承载（§9.6 派生规则），不产额外警告/error。⑦**测试目标链接 testkit（治理登记项 F-051）**——数据集装载/档案/IRD 断言经 units/testkit.md §10.2 交接表 runtime 行消费，sdurws_ird_runtime_test 增链 sdurws_ird_testkit（产品目标链接面不变）；ird_gates_whitelist.cmake T-1 段已声明"sdurws_ird_<unit>_test → sdurws_ird_testkit"为允许形态，但引擎 SUB 判定未豁免该边→产生第 3 处命中（既有 F-019 两项之外）；引擎/白名单一致性处置归 WP-01 门禁所有者（cmake/ 不在本任务 allowedFiles）。实现证据见 traceability/builds/wp06-t12/（集成 261 例＝既有 244＋新增 17：260 通过＋1 例 worker 占位按设计跳过〔仅父用例派生子进程时执行〕、冒烟 176/176＝既有 172＋新增 4 lint 用例；ird_gates＝F-019 既有 2 项＋F-051 新登记 1 项；validate-task PASS） |
| v0.14 | 2026-09-12 | RT-T12 返工落位登记（DTB §5.4；attempt 1 验收阻断 B-1＝findings F-052 的修复——数据资产检出形态防护，零公共接口/零产品代码行为变化）：①**testdata 行尾钉死**——新增 `industrialrobot/testdata/.gitattributes`（`* text eol=lf`）。背景：黄金数据集与容差资产按 manifest sizeBytes＋SHA-256 字节级申报（testkit.md §4.5），申报基准形态为 LF；仓库 core.autocrlf=true 且上层 .gitattributes 仅 ppm/pgm/stl/stlb——fresh checkout（git worktree add／fresh clone／Windows CI 默认）曾把 json/mjs LF→CRLF，检出字节数膨胀"每行一个 CR"，装载即抛 dataset-invalid（attempt 1 验收实测：集成 5 败／冒烟 3 败）。规则取 `text eol=lf` 双向钉死而非整目录 `-text`：检出方向强制 LF（fresh 检出恒为申报形态），入库方向 CRLF→LF 归一化（工作树行尾腐坏不污染 blob——`-text` 对此不设防，返工实测其 renormalize 曾把滞留 CRLF 形态的既有资产吸入 index）；附带把 requirements-ids.json 与 tolerance/kin-fk/v1.0.0.json 在主仓工作树滞留的 CRLF 形态归一为 LF（blob 零变化、JSON 语义不变）。自证：`git add --renormalize` 实证 index 与 HEAD 逐字节零变化；fresh detached worktree 检出修复后提交——11 件资产检出字节与 manifest/blob 申报全部一致，且双模式测试在该检出形态下全绿（对照：同法检出未修复基线 ff4b943c 复现集成 5 败＋冒烟 3 败——B-1 因果钉死）；证据见 traceability/builds/wp06-t12/ rework 系列留痕。②**F-053（验收 S-1）顺手消账**——ContractSuiteTest.cpp `loadRtProfile` 与 GoldenDataLintTest.cpp 档案 lint 用例注释中 rt→rt-runtime 路径残留更正（注释与实参一致，零行为影响）。③**F-054 标注纪律落地**——本轮留痕提交与验收请求显式区分构建树性质（集成＝主树存量增量树＋全新 worktree 自证树两类口径；冒烟＝独立从零目录） |
| v0.15 | 2026-09-12 | RT-T13 文档与门禁同步（DTB §5.4，文档任务零代码变更）：①**README 指向核对零偏差（acceptance 1）**——`runtime/include/sdurws/ird/runtime/README.md` 任务卡指向维持 §12（§9→§12 修正复审通过）；实现状态段同步至 RT-T12 落位后现状（公共头九→十二模块逐头与 include/ 目录实证比对、登记版本段改 v0.4~v0.14＝RT-T03~T12、验证留痕段改 builds/wp06-t01~t12 与最新双模式口径集成 261 例〔260 过＋1 设计跳过〕/冒烟 176/176），消除"RT-T09 起任务未落地"过期表述——核对证据 traceability/builds/wp06-t13/README-audit.md。②**§15.3 待裁决项最新状态逐条（acceptance 2）**——P-RT-1～9 逐条附〔RT-T13 状态同步〕事实标注：P-RT-1/2 登记上游卡现状实测（core.md 现 v0.11 未冻结、ARCHITECTURE 现 v0.12 待评审，均零返工面）；P-RT-3/5/6/7 实现侧事实消账（L1 依赖集两模式运转且门禁零命中、注入 reader＋ObjectId 契约面落地、provider 消费面 RT-RES-1/2 通过、1×10⁸ 默认随 RT-T03 落地）；P-RT-4/8/9 状态不变项如实标注——九项裁决归属均不变更、不私裁（架构/对端所有者辖域项维持 open，沿 CORE-T10 文档任务纪律）。③**R-4 例外清单提交（acceptance 3）**——名称解析器实现文件清单（`runtime/include/sdurws/ird/runtime/NameMap.hpp`＋`runtime/src/NameMap.cpp`，前缀拼装单点 joinScopeLocal）按 DTB §4.5 预登记行"清单随 WP-06-T13 提交转生效"提交：WP-01-T01（机器面回填 IRD_R4_EXCEPTION_FILES）／WP-01-T03（§4.5 登记册同步），提交件 traceability/wp06-t13-gate-registrations.md §1（例外范围＝文件级非单元级；引擎 4c 步 runtime 硬编码跳过的过渡态转登记册驱动属门禁所有者辖域，本提交不代行——cmake/ 不在本任务 allowedFiles）。④**UT-BUILD 并入 CI 建议登记（acceptance 4）**——runtime UT-BUILD 红线用例组（test/BuildRedLineTest.cpp 4 例）已随 sdurws_ird_runtime_test 常驻 gate-all.ps1（WP-01-T02 本地门禁"全部 _test/_contract_test 一键执行"面）与 ird_gates 构建期等效扫描；CI 模板测试阶段 run-tests 正则当前钉 `^sdurws_ird_core_test$`（WP-01-T02 交付时点唯一已建测试目标的历史口径）——登记 WP-01（CI 执行面扩至全部单元 _test/_contract_test）／WP-24（WP-24-T01 基线登记时复核覆盖），提交件同上 §2；沿 CORE-T10 先例口径：既有承载如实消账、真实缺口如实登记，非新裁决（scripts/ci 不在本任务 allowedFiles）。⑤本卡版本 v0.14→v0.15；validate-docs（契约 verify）/validate-task/ird_gates 与双模式构建测试留痕见 traceability/builds/wp06-t13/ |
