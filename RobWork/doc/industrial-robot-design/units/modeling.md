...
| v0.19 | 2026-09-26 | WP-13-T15 增量落位登记——**开发验证 harness `sdurws_ird_modeling_app`**（owner 指示 2026-09-26："现在需要每开发完成一个插件，就必须能够通过 VS 编译手动验证界面的功能"；DTB §5.1 新增"插件 GUI 手动验证通道"行＋§5.2 DoD 第 5 条，v0.20 同批）：①**动因与定位**——面板已于 T15 交付（verdict=pass）但装配点（registrar 调用）归 WP-24-T03、P-MDL-8 期间面板在宿主中不可见，"插件完成即须可 GUI 手动验证"缺执行载体；harness＝ui 单元 `sdurws_ird_ui_app`（UI-T15）同款先例的 modeling 侧载体，**开发工具非产品交付路径**（装配层落位时按同一契约替换，插件面零改动）。②**落位物**——`app/HarnessMain.cpp`（最小装配序列：真实 generic-6r 草稿〔RobotDesignTemplateFactory::createDraft，Ground 预设〕＋`ModelingPanelWidget` 五区面板＋编辑目标提供器注入〔现取零缓存——ACC5〕＋命令提交出口〔控制台回显，不进 project 管线——L-3 完整语义待装配，诚实边界写明于窗口状态行〕＋可写切换工具条〔L-7〕）；`CMakeLists.txt` 增 `_app` 目标（C++17、/utf-8、仅链接 `sdurws_ird_modeling_plugin`——PUBLIC 面携带 modeling＋ui＋Qt，零新跨单元边；POST_BUILD 运行时 DLL 复制＋qt.conf Qt 前缀钉扎，ui_app 同款坑位修复；冒烟分支 boost 头注入随 `_app` 纳入——T15 既有 Readiness 链处置的第三消费目标）；仓库根 `run-modeling-app.ps1` 启动脚本（PATH 前缀＋运行目录日志）。③**真值边界**（防误读验证结论）：真实部分＝L-1 树/属性联动、L-2 编辑流（真实域裁决）、L-7 只读、命令目录呈现、脏标记；演示部分＝就绪条条目手填（真实判定需 AssertionSuite 装配——policy 评估器＋runtime 名称上下文 L5 注入面，harness 不复制，链接面保持 modeling＋ui 同源）；V-30 GUI 主流程的完整人工点验步骤登记于 §10.2 V-30 行，装配后按同一场景在宿主执行。④**构建图契约测试白名单增 `_app`**（`BuildGraphContractTest`：本单元四目标→五目标——产品/插件/两测试/harness app；类注登记增量出处）。⑤**门禁**：ird_gates 引擎直跑 base（1b3a7371，临时 worktree 比对后删除）..branch 归一化比对——净新增恰 1 处逻辑命中 `IRD-GATE-R1: 业务域目标互链 sdurws_ird_modeling_app→sdurws_ird_modeling_plugin`（同单元引用；与 T15 插件链接面两处既有命中同性质＝卡面明文链接面的引擎解析结果，登记册回填归 WP-01-T03 治理面）。⑥**验证留痕**（traceability/builds/wp13-t15-app/）：集成 Release 构建零错误＋`sdurws_ird_modeling_test` 265/265＋`sdurws_ird_modeling_contract_test` 17/17；GUI 冒烟＝harness 启动（run-modeling-app.ps1）进程存活响应＋五区面板/结构树（关节1~6/法兰/基座）/命令目录十条/状态栏截图留痕（modeling-app-smoke-01.png）。⑦**验证留痕（独立冒烟侧·时序实录）**：首次冒烟 configure 被同树并行会话的 WP-14-T07 进行中改动阻断（requirements/CMakeLists.txt 引用其尚未落盘的 `test/TemplateArrayTest.cpp`——与本任务改动面零交集，非本任务缺陷）；该文件落盘后 configure 通过，**本单元三目标（test/contract_test/app）在冒烟树编译零错误**：`sdurws_ird_modeling_test` 215/215＋`sdurws_ird_modeling_contract_test` 8/8（与 T16 登记的冒烟口径一致）＋`sdurws_ird_modeling_app.exe` 产出（build_smoke_t15-app/modeling/Release/）；全树冒烟构建仍失败于 T07 在制品自身的编译错误（TemplateArrayTest.cpp:186——T07 会话进行中未完成件，非本任务面），全树零错误结论归 T07 送验时点复核；harness stderr 观测到既有 T15 代码路径 Qt 警告 `QFormLayout::takeRow: Invalid row 0`（启动期一次，功能无碍——建议级观测，归 T15 呈现面所有者）。文档头版本升 v0.19。 |
| v0.20 | 2026-09-26 | **WP-24-T03 首版装配——P-MDL-8 消账＋装配门面落位**（分支 wp24-t03 待验收；owner 指示 2026-09-26"每完成一个插件进行装配，及时验证效果"提前启动——原排期阶段 E；ui.md v1.15 同批；版本号承接 wp13-t15-app v0.19 先推序）：①**继承切换**——ModelingUiModule 自"逐方法同形实现"切换为继承 ui::IPluginUiModule（ui 侧两头 IPluginUiRegistrar/IPluginUiModule 已随首版装配落位；三方法 override，签名/语义零变化——R-MDL-1 增量同步预告的兑现；§14.6 P-MDL-8 处置注同步翻转）。②**装配 API**——模块新增 bindCommandSubmit（提交出口绑定，面板创建前后皆可）／bindTextResolver（titleKey→工程用语，宿主接 ui::resolveText）／seedTemplateSession（generic-6r 真实草稿种子；readiness 不预置＝DataInsufficient 缺省行，不伪造）／createPanel（§10.9 工厂半区——会话提供器/出口/文案一次接线＋attachPanel＋**首刷**〔树/属性区即刻呈现种子草稿——首版验收反馈缺陷修复：原实现漏首刷致树空〕）；面板新增 setCommandTitleResolver（解析失败回退键名原文，不空洞不虚构）。③**装配门面**——assembly/sdurws/ird/modeling/ModelingPluginAssembly.hpp/.cpp（plugin 目标新增 PUBLIC 包含面 assembly/——宿主装配层（ui 插件 DomainAssembly）唯一消费入口，零建模私有头依赖（R-2）； ModelingPluginAssembly{module+descriptor+bind*+seed} 单点结构，friend 装配点回填 m_impl）。④**真宿主实证**——ui 插件直载后建模 Dock 入宿主 Left 区＋十条命令按钮 UiText 中文工程用语（cmd.modeling.*.title 键族随 ui.md v1.15 登记）＋L-1/L-2 编辑流可交互＋无错误弹窗；截图 traceability/builds/wp24-t03-first/modeling-in-host-smoke-02.png。⑤**收口余量（登记于 ui.md v1.15④）**——域命令入 content 注册表/StageStatusModel 汇聚/中央区 CentralAreaHost 挂位/draft.apply 域侧挂钩/AssertionSuite 真判定注入/会话事件同步——随装配收口任务兑现，插件面零改动预期。 |
