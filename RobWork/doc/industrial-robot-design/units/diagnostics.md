...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.13（§8.2 StoreError→诊断映射表 PRJ-LOCK-HELD 行 subject 格对齐 O-41 所有者裁决——原"subject=∅（项目级）"与 §9.2 工厂用户级 subject 边界相悖〔F-290 根因〕，改为 subject=项目对象 ProjectId 发射侧必填；纯文档一格修订，登记见 §14.4；v0.12（DIAG-T11 文档与门禁同步——本文与实现零偏差登记＋README 指向核对零偏差＋§14.3 P-DIAG-1~9 状态逐条同步＋P-PR-6/P-EX-8 消账确认提交〔不私改对端契约〕＋§1.2 卡头一处措辞更正，登记见 §14.4；v0.11＝DIAG-T10 实现落位登记——测试替身与契约套件〔ScriptedDiagSource＋DT-\* 用例体收口〕落地＋实现口径登记见 §14.4；v0.10＝DIAG-T09 实现落位登记——诊断生命周期与容量〔目录清理策略/宿主留痕形状〕落地＋实现口径登记见 §14.4；v0.9＝DIAG-T08 实现落位登记；v0.8＝DIAG-T07 实现落位登记；v0.7＝DIAG-T06 实现落位登记；v0.6＝DIAG-T05 实现落位登记；v0.5＝DIAG-T04 实现落位登记；v0.4＝DIAG-T03 实现落位登记；v0.3＝DIAG-T02 构建落位登记；v0.2＝全链一致性审计消账；v0.1＝首版草案；各版变更记录见 §14.4） |
| 日期 | 2026-09-15 |
...
| 构建落位 | `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/diagnostics/`（**已随 DIAG-T02 落位〔2026-09-14〕**：目标 `sdurws_ird_diagnostics` 升级 STATIC〔C++17、PUBLIC 链 core、零 Qt 含 Core——D-01；src/ 空起步锚点翻译单元〕，别名 `RWS::ird::diagnostics`，测试目标 `sdurws_ird_diagnostics_test`/`sdurws_ird_diagnostics_contract_test` 已按 dtb §5.5 注册〔gtest〕，配置期红线守卫就位；公共头保留位 `include/sdurws/ird/diagnostics/README.md` 继续指向本文 §11——10 个契约头随 DIAG-T03+ 落地。见单元 `diagnostics/CMakeLists.txt` 与上级 `industrialrobot/CMakeLists.txt`。**已随 DIAG-T03 落位〔2026-09-14〕**：公共头 `Errors.hpp`（§9.0 错误类型）与 `DiagCodes.hpp`（§4.3 词表＋§4.5 CodeDescriptor/StableCodeRegistry＋§4.6 内置码表 87 码）落地，实现 `src/Errors.cpp`/`src/DiagCodes.cpp`；`_test` 增列 `test/StableCodeRegistryTest.cpp`（DT-REG-1~5＋P-DIAG-1/3/9 处置钉住 17 用例）；§4.6 压缩串的逐码展开登记见 §4.6 表后注记）。**已随 DIAG-T04 落位〔2026-09-14〕**：公共头 `Catalog.hpp`（§4.2 信封 DiagnosticEntry/DiagContext＋§6.4 DedupKey/ScopeKind/OrderKey＋§4.4 actionKindToken＋§9.7 IDiagnosticSink/DiagProjectionItem/DiagQuery/IDiagObserver/ISubscription/DiagCatalog＋P-DIAG-5 兼容形态 IDevLogSink/DiagnosticsSinkImpl）与 `Factory.hpp`（§9.2 IDiagnosticFactory 接口＋DiagnosticsFactory 实现＋ErrorCodeTranslator 命名视图＋registerStageATranslations）落地，实现 `src/Catalog.cpp`/`src/Factory.cpp`/`src/ParamSchema.cpp`（paramSchema 解析单点自 DiagCodes.cpp 迁入，行为逐字保持）；`_test` 增列 `test/CatalogFactoryTest.cpp`（DT-DIAG-1/2、DT-DUP-1/2、DT-CAT-1/2＋DT-REG-3/4 工厂半区＋CR-08/P-DIAG-1/5 处置钉住 19 用例）；实现口径登记见 §14.4 v0.5）。**已随 DIAG-T05 落位〔2026-09-14〕**：公共头 `Confirmable.hpp`（§5.2 FindingRecord 字段表 15 字段全量＋§5.3 FindingBinding 绑定四元组＋§5.4 FindingState 服务端状态与 finding_reason 原因 token 词表＋§9.3 FindingId（P-DIAG-4 自持解析）/ConfirmOutcome/IConfirmableFindingService 五方法签名逐字＋ConfirmableService 实现＋IConfirmationEnvironment 绑定复核环境探针）落地，实现 `src/Confirmable.cpp`（findingRecordDigest 唯一定义点——findingDigest canonical 编码登记于其文件头；CR-02：SHA-256 只经 core ContentDigester）；`_test` 增列 `test/ConfirmableTest.cpp`（DT-CFM-1~5 服务端半区＋P-DIAG-4/6、CR-02 处置钉住 13 用例），`_contract_test` 增列 `test/ConfirmableProjectStubTest.cpp`（DT-CFM-6~10 project §6.7 桩联动 6 用例——P-DIAG-8 处置：桩验证不私改对端链接）；实现口径登记见 §14.4 v0.6）。**已随 DIAG-T06 落位〔2026-09-14〕**：公共头 `Aggregation.hpp`（§6.3 CauseLink 原因链遍历〔rootOf/chainOf——严格契约：断链/环/未知起点 Usage〕＋§6.4 AggregationRule/AggregationScope/AggregatedEntry〔成员明细 entryId/subject/localName 全保留〕＋§9.4 IDiagnosticAggregator 两方法签名逐字＋DiagnosticAggregator 无状态实现）落地，实现 `src/Aggregation.cpp`（容差链遍历/三类全序分组键/阈值折叠/orderKey 稳定排序）；`_test` 增列 `test/AggregationTest.cpp`（DT-AGG-1~5、DT-CHAIN-1、DT-DUP-1 计数半区＋P-DIAG-1 处置钉住 11 用例，两模式 65/65＋9/9）；实现口径登记四项见 §14.4 v0.7）。**已随 DIAG-T07 落位〔2026-09-14〕**：公共头 `Logging.hpp`（§7.1/§7.2 LogTier/LogLevel/CorrelationIds/LogRecord〔4 KiB 截断/token 边界〕＋§7.5 WorkerLogBatch 载荷〔workerId/任务五元组/序号/final 收尾/records——P-EX-8：只定载荷语义不定帧格式〕＋§9.6 LogSinkConfig/ILogger 四方法签名逐字＋ILogFileOps 文件接缝〔DT-LOG-3 故障注入面，testkit D-10 形态〕＋FileLogFileOps 默认实现＋LoggingPipeline〔ILogger＋IDevLogSink＋replayWorkerBatch〕）落地，实现 `src/Logging.cpp`（五步管线/节流采样/轮转/队列溢出丢弃/worker 乱序缓冲与断裂补条）；`_test` 增列 `test/LoggingTest.cpp`（DT-LOG-1~5、节流采样/轮转/队列溢出/截断行为面＋入口违约面＋P-DIAG-7/P-EX-8 处置钉住 12 用例，两模式 77/77＋9/9）；实现口径登记见 §14.4 v0.8）。**已随 DIAG-T08 落位〔2026-09-15〕**：公共头 `Redaction.hpp`（§7.7 脱敏规则表全量承载＋§9.5 IRedactionService 四方法签名逐字〔redact/redactPath/safeSummary/setPolicy〕＋PathPolicy 四值〔Keep/Hash/RootOnly/Strip 枚举冻结〕＋RedactionPolicy〔默认 RootOnly/32〕＋RedactionService 实现〔stats/policy/applyRules 虚缝为实现类扩展——seal() 先例〕）与 `CrashReport.hpp`（§7.6 崩溃诊断文件：CrashReportInput 六段内容载体〔core::ProjectId/TaskIdentity 内嵌——P-DIAG-1〕＋CrashReportWriter 写出设施〔noexcept＋失败降级 stderr＋一条 DIAG-LOG-WRITE-FAILED 不递归；OS 级崩溃捕获与应用壳装配归 WP-24-T02——不越界实现 dump 机制〕）落地，实现 `src/Redaction.cpp`/`src/CrashReport.cpp`；接线三面（§14.4 v0.9 登记）：LoggingPipeline::attachRedactionService＋devTailSnapshot（§7.6 环形缓冲 512 行快照读取面）、DiagCatalog::attachRedactionService（exportSafeSummary 输出前强制再脱敏——双保险 DT-SEC-4）、DiagnosticsFactory::attachRedactionService（§4.2 redactedContextSnapshot 构造期产出＋translate 异常消息脱敏整备收口）；`_test` 增列 `test/RedactionTest.cpp`（DT-SEC-1~4、DT-LIFE-4 磁盘不足两轨注入、AT-11 崩溃文件写出且脱敏＋P-DIAG-1 处置钉住 21 用例，两模式 98/98＋9/9——既有 77＋9 零回归）；实现口径登记见 §14.4 v0.9）。**已随 DIAG-T09 落位〔2026-09-15〕**：公共头 `Catalog.hpp` 扩展（§6.2 目录容量护栏 `CatalogCapacityConfig`〔P-DIAG-7 工程默认：enabled＋maxEntries=10,000 条〕＋`DiagCatalog` 五方法 `configureCapacity/capacityConfig/markConsumed/setTaskActive/attachDevLogSink`＋`kCatalogInternalChannel`〔"diag/catalog"〕），实现 `src/Catalog.cpp` 扩容（分级淘汰〔最旧已消费 Info 先行〕/引用计数快照链保护/软溢出仍追加/`DIAG-CATALOG-OVERFLOW` 溢出登记——无新增翻译单元，`Catalog.*` 扩展即 §11 DIAG-T09 行的落位形态）；`_test` 增列 `test/CatalogLifecycleTest.cpp`（DT-LIFE-1/2/3＋P-DIAG-7 处置钉住 9 用例），`_contract_test` 增列 `test/CatalogHostTraceStubTest.cpp`（DT-LIFE-5＋宿主留痕形状冻结 5 用例——P-DIAG-8 同款桩验证纪律：envelope/CommandRecord 对接桩编码器定义在测试侧），两模式 107/107＋14/14——既有 98＋9 零回归；实现口径登记见 §14.4 v0.10）。**已随 DIAG-T10 落位〔2026-09-15〕**：`_contract_test` 增列 `test/ScriptedDiagSource.hpp`（仅头替身——产出逻辑内联于头文件；卡头原"hpp/.cpp 对"系措辞 slips，与磁盘实测及 §14.4 v0.11 权威登记不符，已按 DTB §5.4 于本版更正）与 `test/ScriptedDiagSourceTest.cpp`（§11 DIAG-T10 行产物"按序列产出预设条目/finding/日志行"——统一序列步进回放〔条目/finding/日志行三形态交错〕＋替身边界机检，6 用例＝ScriptedReplay 回放 3＋ScriptedDoubleBoundary 边界机检 3；替身边界声明三面在案——test/README.md §1 全文＋替身头文件头＋机检两面，EV-REG-3 同模式）与 `test/DiagContractCheckTest.cpp`（testkit `checkDiagnosticRecord`/`checkComparativeFields` 消费 6 用例——§10 头注承接，双面对账：工厂产出与 testkit 独立判定同判）；`_contract_test` 链接 `sdurws_ird_testkit`（testkit.md §2.4 允许依赖形态 `{被测产品目标, sdurws_ird_testkit, gtest}`——**产品目标 sdurws_ird_diagnostics 不链 testkit**，LinkageContractTest 白名单沿用面＋CMake 配置期守卫＋替身符号机检三面钉住）；`_test` 增列 `test/CollaborationQueryTest.cpp`（DT-COLLAB-1＋DT-AT 观测点映射 AT-01/10/11/13/19/30/34 七用例——§10 矩阵收口的最后两个缺口组）；test/README.md 为单元测试文档入口（§1 替身边界声明全文/§2 替身清单/§3 §10 矩阵→文件落位全表 DIAG-T02～T10 累积），两模式 115/115＋26/26——既有 107＋14 零回归；实现口径登记见 §14.4 v0.11） |
...
| 构建骨架 | 存在：`industrialrobot/CMakeLists.txt`＋20 单元目录＋`patches/`（23 文件）；diagnostics 目录**自 DIAG-T02 落位〔2026-09-14〕**为 `CMakeLists.txt`（STATIC＋两测试目标＋配置期守卫）＋`src/Diagnostics.cpp`（空起步锚点）＋`test/`（DT-BUILD 红线用例＋落位期契约用例）＋`include/.../README.md` 保留位；**自 DIAG-T03 落位〔2026-09-14〕**增列：公共头 `include/sdurws/ird/diagnostics/{Errors,DiagCodes}.hpp`（§9.0 错误类型＋§4.3/§4.5/§4.6/§9.1 词表/描述符/注册表/内置码表 87 码）、实现 `src/{Errors,DiagCodes}.cpp`、测试 `test/StableCodeRegistryTest.cpp`（DT-REG-1~5 等 17 用例） | `sdurws_ird_diagnostics` 已为真实 STATIC 库；`_test`/`_contract_test` 目标已随 DIAG-T02 按 dtb §5.5 登记（其余目标仍按任务卡逐个登记、不预建空目标）；DIAG-T03 后目标/边集合不变（仅增列源文件） |
...
**逐码展开登记（DIAG-T03 落地，2026-09-14——把上表压缩串的机械展开结果显式化；码值集合与逐字收编清单零改动，仅登记 category/severity 的逐码落值依据，实现＝`DiagCodes.cpp` 内置表逐码注释）**：分类/严重串按 R1~R4 展开——R1 行内串按码序逐位对齐（"×n"＝连续 n 码同值）；R2 段长合计与码数不符（PRJ 9 对 10、POLICY 22 对 23、EX 14 对 18、EVI 严重 6 对 7）＝压缩丢位，丢位码以 §4.3"典型来源码"点名锚点／§4.4 默认严重矩阵／收编卡语义登记补齐；R3 严重列省略号＝延续首值，行内显式标注（"(Warning)"/"(Info)"/备注"四个开发级标 Dev"）为逐码覆盖值；R4 分类串中的非词表 token（POLICY-INFO-DEFAULT-APPLIED 与 RPT-SECTION-NOT-APPLICABLE 的"信息"、DIAG-FINDING-\* 的"可确认类辅助"）按 §4.3 词表落位（infeasibility-proof＝§4.3 Info 行"有效结论/告知"举例与 §4.4"结论呈现（RPT-05）"锚点；confirmable＝SA-15 确认流自省码），**不新增词表值**（P-DIAG-3"不私裁"）。全部消歧点（逐码可复核）：PRJ-FORMAT-LEGACY 补 format-or-version（§4.3 点名）；RT-RESOURCE-BUDGET=security-or-redaction（§4.6 串逐位优先；§4.3 资源缺失行亦点名 BUDGET 的矛盾登记）；RT-NAME-CONFLICT/RT-BASE-WORLD-INCONSISTENT=internal/Error（串严重"Error…"显式登记优先于 §4.4 internal 默认 Dev）；POLICY-APPLICABILITY-INVALID 补 input-invalid（policy §5 校验表同段）；EX-STORE-READ-ONLY=permission-or-lock/Warning（§4.3 权限类点名，压缩段丢位）；EX-CHECKPOINT-\*=format-or-version（"版本×2"段位）；EX-TASK-INTERRUPTED=interrupted/Info（§4.4 默认）；EVI-CACHE-INCOMPATIBLE 严重 Error（§4.4 format-or-version 默认；Dev 会跨用户级/开发级分界——§4.5 禁令排除）。逐码 paramSchema 除 PRJ-LOCK-HELD（§9.2 调用示例明示 `["pid","host"]`）外均为 `"[]"`（无参数显式声明——各单元产码路径落地时按需增量登记并升 registryVersion，不私造参数名）；retryable 按 §4.4 分类动作族机械映射（含重试语义的动作族→UserRetry，none/review-proof/report-bug/inspect 族→Never；AutoRetry 阶段 A 无动作族不使用）。
...
| --- | --- | --- | --- | --- |
| lock-held-by-other | PRJ-LOCK-HELD | 权限/Warning | subject=项目对象（ProjectId，发射侧必填——O-41 裁决 2026-09-22 对齐：本表原"∅（项目级）"与 §9.2 工厂用户级 subject 边界相悖，系 F-290 根因）；非比较型 | paramSchema{pid,host}；只读打开提示（PM-07） |
...
| # | 问题 | 依据 | 影响 | 建议 | 需要裁决者 |
| --- | --- | --- | --- | --- | --- |
| P-DIAG-1 | 本文消费的 core 契约以其 v0.1（Draft）为基线；冻结版可能调整签名（DiagData/Id128/词表） | core.md 文档头状态行 | DIAG-T03 起返工 | core 冻结时出 diff 清单，本文按影响面增量修订并留痕。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（冻结未发生）**：core.md 实测 v0.11（Draft-Structured，未冻结）；本文消费面由两测试目标两模式实跑 115＋26 例复证零漂移（builds/wp09-t11/），DIAG-T03~T08 各落位登记的 core v0.1 基线 static_assert 编译期钉住在案（§14.4 v0.4~v0.9）——冻结 diff 后增量同步路径不变 | core 详设所有者 |
| P-DIAG-2 | ARCHITECTURE v0.11 Draft 待评审；本文以其为基线 | 架构文档头 | 评审结论可能要求同步（§7.8/SA-15/依赖表） | 评审后按影响面增量修订。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（评审未完成）**：ARCHITECTURE 实测 v0.12（"全链一致性审计事实修正"，仍 Draft 待评审）——其 v0.11→v0.12 变更实测仅 §1.4 附录引用补登、§10.2 分期子项补列与 O-32 登记，未触及本文锚点（§3.1/§3.5/§7.8 SA-12/§7.1 SA-15），零返工面；评审结论产出后按影响面增量修订 | 架构所有者 |
| P-DIAG-3 | DiagnosticCategory（15）/DiagnosticSeverity（4）/actionKind 动作族为本文实现承载词表：上游无需求级枚举（ERR-01 只定字段；NFR-REL-05 只分两级日志），但 UX-10/PM-15 分组呈现与 §7 分流需要机器可读分类 | ERR-01、NFR-REL-05、UX-10、PM-15 | 下游（ui/reporting）呈现分组依赖该词表；若需求侧未来定义正式枚举需对齐 | 维持实现承载＋§4.4 逐行上游锚点（不新增工程状态——呈现映射只引用既有轴值）；需求侧如需冻结走需求变更。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（需求侧未定义正式枚举）**：词表实现承载已落地（DIAG-T03——分类 15/严重 4/actionKind 3，§14.4 v0.4；"不新增轴值"由用例钉住）；ui 详设已产出并消费该词表（ui.md 公共依赖面登记 DiagnosticCategory/DiagnosticSeverity），无对齐缺口；reporting 详设已产出（码表只读访问——§12.2 交接行） | 需求所有者＋ui/reporting 详设 |
| P-DIAG-4 | FindingId 的类型归属：core Id128 体系现冻结 7 tag（obj/prj/brn/rev/run/evt/att），无 `fnd-`；本文按同构自定义（D-05） | core.md §4.1/§5.1 | 跨单元规范文本解析无统一入口；core 若收编则签名变更 | 建议 core 下次修订收编 `fnd-` tag（影响面小：新增独立类型）；收编前本文自持解析（句法与 core 严格一致）。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（core 未收编）**：core.md v0.11 全文实测零 `fnd-`（未收编）；本文自持解析已落地且句法与 core Id128 严格一致（DIAG-T05，§14.4 v0.6，用例钉住）；与 P-RPT-3（`rpt-`）同案收编路径不变 | core 详设所有者 |
| P-DIAG-5 | project §5.0 IDiagnosticsSink 与 execution §3.3 IExecutionDiagnosticsSink 的接口统一：两者为消费方单侧定义（diagnostics 产出前占位），本文 §9.7 DiagnosticsSinkImpl 承诺实现二者语义（report/reportDev），但接口名/归属（diagnostics 定义统一接口 vs 各消费方自持＋L5 适配）未裁决 | project P-PR-6、execution P-EX-8 | 三处接口漂移风险；装配层适配代码量 | 建议方向：diagnostics 冻结 `IDiagnosticSink`（§9.7）为标准接口，project/execution 详设修订时将其本地 sink 别名/继承到标准接口（一次性小改）；过渡期 L5 适配器桥接（本文已承诺实现兼容）。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（sink 统一裁决未发生；governance-log 实测 open）＋P-PR-6/P-EX-8 消账确认（本文侧登记，契约 acceptance 3）**：本文侧标准接口 IDiagnosticSink＋DiagnosticsSinkImpl 兼容形态已交付（DIAG-T04，§14.4 v0.5——report/reportDev 与 project §5.0/execution §3.3 同形签名逐字一致）；对端实测 project.md/execution.md 均 v0.2 且 P-PR-6/P-EX-8 仍登记 open（其 v0.2 变更记录明言：码值部分消账、sink 名称/归属统一"不因卡存在自动消账"）——**消账路径＝project/execution 详设修订联动（各自卡内增量修订时收编统一接口与链接形态），本文不私改对端契约，本确认随 DIAG-T11 提交登记** | project/execution 详设所有者＋架构所有者 |
| P-DIAG-6 | ConfirmableFinding 服务端扩展态（Invalidated/Expired）与 `expiresAtUtc` 字段：SA-15/MDL-06④ 未定义超时语义；project §5.3.4 明言"无超时自动确认" | SA-15、project §5.3.4 | 若需求侧未来引入确认超时（如安全审计要求），本文状态机已预留 | 维持默认无限期（仅失效条件驱动）；expiresAtUtc 字段保留但默认 nullopt；启用＝需求变更＋码表/文案更新。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（需求侧未引入确认超时）**：默认无限期＋expiresAtUtc 默认 nullopt 已落地（DIAG-T05——armExpiration 为实现类原语，§14.4 v0.6 口径④）；ui 详设已按"findings 默认 expiresAt 空"对齐（ui.md 确认对话超时行点名本条），无对齐缺口 | 需求所有者 |
| P-DIAG-7 | 日志文件轮转参数（默认 5×2 MiB/文件、flush 窗口 N=256/T=2 s）与目录容量（10,000）为本文工程默认；上游未定义 | NFR-REL-05 无参数；性能归 WP-23 | 长会话磁盘/内存占用 | 维持可配默认；WP-23 性能验收时校准；参数不属需求语义（实现配置）。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（WP-23 未启动，阶段 D）**：工程默认全部落地并用例钉住（DIAG-T07 轮转 5×2 MiB/flush 窗 N=256/T=2s/采样 N=100/节流窗 10s/队列 4096；DIAG-T09 目录容量 10,000——DtPdiag7 默认值用例，§14.4 v0.8/v0.10）；WP-23 校准面不变 | 构建负责人（WP-23 消费时确认） |
| P-DIAG-8 | dtb §1.1 指出"diagnostics 虽为 L3，但被 project/execution/io/ui 依赖，其任务卡与 StableCodeRegistry 必须先于四者实现"——execution.md 已产出但其目标暂未链接 diagnostics（其 §3.4：边经注入实现）；本文按"入边存在、链接随消费方任务启用"理解 | ARCH §3.5、execution §3.4、dtb §1.1 | 契约测试（DT-COLLAB/DT-CFM 桩联动）需 mock 还是链接执行 | 本文测试以桩+契约夹具验证；execution 链接启用（EX-T01 后按 P-EX-8）时补双库集成冒烟。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（execution 链接启用未发生；governance-log 实测 open，不消账）**：桩+契约夹具验证路线已随 DIAG-T02/T05/T09/T10 落地并全绿（test/README.md §3 矩阵落位全表）；DIAG-T10 的 testkit 契约谓词消费面新增 1 处 IRD-GATE-SUB（contract_test→testkit 允许形态）已如实登记（§14.4 v0.11；wp09-t11 复证 ird_gates 8 命中与 wp09-t10 基线逐码一致零新增）；双库集成冒烟仍按 P-EX-8 联动另行登记 | execution 详设所有者＋构建负责人 |
| P-DIAG-9 | 用户级文案本地化策略：本文冻结"文案键"体系（titleKey/detailKey），文案值与语言解析归 ui/文案资源；多语言（中英）是否 R1 范围上游未定义 | UX-02、NFR-MNT-03 | 键体系是持久化契约（进码表），值策略影响 ui/交付 | 维持键/值分离（键本文冻结、值归 ui）；R1 语言范围走需求侧确认（不影响本文设施）。**〔DIAG-T11 状态同步 2026-09-15〕仍 open（需求侧未确认 R1 语言范围）**：键/值分离已全表合规落地（DIAG-T03 注册期验证链——文案键冲突报 NFR-MNT-03 语义码，§14.4 v0.4）；ui 详设已产出并承接交接（diag.\<code-lower\>.title/detail 键约定、值归 ui 文案资源——ui.md 文案键行点名本条交接），交接面闭合、裁决归属不变 | 需求所有者＋ui 详设 |
...
| v0.4 | 2026-09-14 | DIAG-T03 实现落位登记（≙WP-09-T03，契约 tasks/foundation/DIAG-T03.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Errors.hpp`（§9.0 错误类型 11 值全表＋DiagnosticsError，token 逐值承载）与 `DiagCodes.hpp`（§4.3 词表 15/4/3 值＋§4.5 CodeDescriptor 16 字段/注册期验证链＋§9.1 IDiagnosticRegistry 五方法签名逐字＋StableCodeRegistry＋§4.6 内置码表 87 码全量收编）落地，实现 `src/Errors.cpp`/`src/DiagCodes.cpp`，`_test` 增列 `StableCodeRegistryTest.cpp`（DT-REG-1~5＋P-DIAG-1/3/9 处置钉住 17 用例，两模式 22/22）；§4.6 表后新增"逐码展开登记"注记（压缩串机械展开规则 R1~R4＋全部消歧点显式化——码值集合零改动）；实现口径登记两项：①§9.1"运行期"判据＝实现类 `seal()` 装配原语（§9.1 未定义判据，evidence EV-T09 实现口径登记先例；IDiagnosticRegistry 五方法签名不扩——非接口偏差），②注册期验证链检查序固定（键唯一性 DuplicateCode 先于键形约定 Usage——文案键冲突报 NFR-MNT-03 语义码）；paramSchema 逐码落值＝PRJ-LOCK-HELD `["pid","host"]`（§9.2 示例明示）其余 `"[]"`（不私造参数名——增量登记随各单元产码路径并升 registryVersion）；P-DIAG-1/P-DIAG-3/P-DIAG-9 处置按契约 acceptance 3/4 执行（core::DiagCode v0.1 基线编译期钉住、词表 15/4 不新增轴值、键/值分离全表合规）；DT-REG-3/4/5 的工厂构造/translate 路径随 DIAG-T04（§11 任务分工——T03 产物＝Errors.*/DiagCodes.*），本任务自证注册表侧判定原语与同一错误码面（用例注释逐处锚定）；验证留痕 traceability/builds/wp09-t03/（ird-test-report.json：双模式 25/25，ird_gates 7 命中＝wp09-t02 基线零新增，探针失败能力自证） |
| v0.5 | 2026-09-14 | DIAG-T04 实现落位登记（≙WP-09-T04 部分，契约 tasks/foundation/DIAG-T04.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Catalog.hpp`（§4.1 双层模型的信封 DiagnosticEntry 14 字段/DiagContext（§9.2 示例明示的 params 字段补入 §4.2 表）＋§6.4 DedupKey/ScopeKind/OrderKey＋§4.4 actionKindToken（矩阵行首动作机器锚点）＋§9.7 IDiagnosticSink 四方法原文/DiagProjectionItem/DiagQuery/IDiagObserver/ISubscription/DiagCatalog＋IDevLogSink/DiagnosticsSinkImpl（P-DIAG-5 兼容形态））与 `Factory.hpp`（§9.2 IDiagnosticFactory 接口签名逐字＋DiagnosticsFactory 实现＋ErrorCodeTranslator 命名视图＋registerStageATranslations）落地，实现 `src/Catalog.cpp`/`src/Factory.cpp`/`src/ParamSchema.cpp`，`_test` 增列 `CatalogFactoryTest.cpp`（DT-DIAG-1/2、DT-DUP-1/2、DT-CAT-1/2＋DT-REG-3/4 工厂半区＋CR-08/P-DIAG-1/5 处置钉住 19 用例，两模式 41/41＋3/3）。实现口径登记六项（§9.2/§9.7 未定义判据，DTB §5.4）：①**工厂与转译器一体**——§9.2 将创建与转译合并于同一接口/同一契约表，ErrorCodeTranslator 为 DiagnosticsFactory 的别名视图（转译注册表即工厂组成，D-14；不设第二实现体——NFR-MNT-04），L5 注入同一对象充任两角色；②**subject 边界严格口径**——用户级码（severity≠Dev）缺或非法（全零）subject 一律 SubjectMissing（ERR-01"稳定诊断项以 subjectObjectId 绑定对象"＋core.md §4.8"稳定诊断项必须携带合法 subjectObjectId"＋DT-DIAG-2 权威链严格执行；§9.2 调用示例 PRJ-LOCK-HELD subject={} 与 §8.2/§8.5 表注 subject=∅ 记法与该权威链不符——该等映射落地任务时须为主对象绑定或修订表注，需所有者届时确认）；③**去重键派生**——DedupKey.scopeKind 由工厂从 DiagContext 推导：task 存在→Task（scopeId＝五元组规范串）否则 subject→Object（scopeId＝subject 规范串）否则 None；Case/Batch 为 T06/T07 保留值不预建；④**translate 查找序**——精确 typeid(E)→（静态派生）runtime_error 登记→exception 登记→DIAG-REGISTRY-UNKNOWN-CODE 兜底条目（Dev 级恒不抛；消息截断 512 字节＝§7.6 口径，脱敏整备随 T08）；root 非空→causedBy 链接＋subject 继承，rootless 转译用户级目标码按②拒绝（§9.2 未定义基类派生判据——登记）；⑤**§8.10 上下文必填的机械执行面**——阶段 A 仅执行明文锚定的 execution 域码⇒context.task 必填（ownerUnit=="execution" 且五元组须 isValid）；评估/命令路径的逐码清单未由本文给规范性枚举，不预判，随各单元产码路径增量登记；⑥**阶段 A 转译清单**＝仅 §8.3 明文的 std::exception→RT-ROBWORK-ERROR（RT 族）；POLICY/EVI 直接产码无异常转译行（§8.4/§8.7），PRJ/EX 逐值映射归域装配（§8.8"映射数据归域"）——acceptance 2"PRJ/RT/POLICY/EVI/EX 映射就位"以此口径落地（设施＋注册协议＋RT 具体映射＋审计面 registeredTranslations()），不预建无消费者映射行（NFR-MNT-04）。接线登记（后续任务）：emittedAtUtc 时钟注入 IClock 已就位；threadTag 采集点随各单元产码路径；workerId 标注随 §7.5 回传路径（T07）；redactedContextSnapshot 经 IRedactionService 产出（§4.2——T08 接线）；目录容量/清理策略（§6.2）随 T09；exportSafeSummary 的 §9.5 safeSummary 脱敏双保险随 T08；订阅回调当前为调用方线程同步派发（§9.8 异步化随 T07 日志线程）。P-DIAG-1/P-DIAG-5/CR-08 处置按契约 acceptance 2/3/4 执行（信封内嵌 core::DiagnosticRecord v0.1 基线 static_assert 钉住；IDiagnosticSink 为统一标准接口＋report/reportDev 同形签名与 project §5.0/execution §3.3 逐字一致、最终统一待 P-PR-6/P-EX-8 不私改对端；转译只经已登记类型映射、禁字符串匹配、不跨边界抛对方异常类型）。验证留痕 traceability/builds/wp09-t04/（ird-test-report.json：双模式 41/41＋3/3，ird_gates 7 命中＝wp09-t03 基线零新增且零 diagnostics 命中，validate-task PASS，探针失败能力自证——subject 边界与去重折叠两探针真实变红后逐字还原） |
| v0.6 | 2026-09-14 | DIAG-T05 实现落位登记（≙WP-09-T03/T04 设施面，契约 tasks/foundation/DIAG-T05.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Confirmable.hpp`（§5.2 FindingRecord 字段表 15 字段全量承载＋§5.3 FindingBinding 绑定四元组＋§5.4 FindingState 服务端四值状态与 finding_reason 原因 token 词表〔namespace 常量，§5.3/§5.4/§9.3 明文 token 冻结〕＋§9.3 FindingId〔P-DIAG-4：diagnostics 层类型，自持解析，句法与 core Id128 严格一致（fnd-＋32 小写 hex＝36 字符）；不 include core detail（R-2）、不私改 core〕＋ConfirmOutcome 四值＋IConfirmableFindingService 五方法签名逐字＋ConfirmableService 实现＋IConfirmationEnvironment 绑定复核环境探针）与实现 `src/Confirmable.cpp`（findingRecordDigest 唯一定义点——findingDigest canonical 编码"IRDDREC1"＋u32le 长度前缀逐字段登记于其文件头；CR-02/acceptance 4：SHA-256 只经 core ContentDigester，不私设第二哈希路径）；`_test` 增列 `ConfirmableTest.cpp`（DT-CFM-1~5 服务端半区＋P-DIAG-4/6、CR-02 处置钉住 13 用例），`_contract_test` 增列 `ConfirmableProjectStubTest.cpp`（DT-CFM-6~10 project §6.7 桩联动 6 用例——P-DIAG-8 处置：桩验证不私改对端链接；两模式 54/54＋9/9）。实现口径登记八项（§5.3/§9.3 未定义判据，DTB §5.4）：①**绑定复核环境探针**——§9.3 submitConfirmation 后置要求"复核绑定四元组（当前策略/命令/修订）"但签名（契约原文）只携带凭据，服务端"当前实际值"来源未定义：实现为构造注入 IConfirmationEnvironment（currentTipRevision/currentPolicyContentId/currentCommandDigest 三查询；L5 由 project 桥接实现；实现类构造约定不在五方法契约上——StableCodeRegistry::seal() 同先例，不扩接口签名）；②**复核定性归类**——复核序＝§5.3 四元组序（findingDigest 重算自检→commandDigest→policyContentId〔绑定有值才复核；绑定 nullopt＝非策略来源类跳过，其必填义务归调用方——§5.2 条件必填，服务端无"策略来源类"判据字段不私造启发式〕→baseRevisionId），首个不符成员定性 rejectionReason：baseRevisionId→revision-changed／policyContentId→policy-changed／其余→binding-mismatch（DT-CFM-4/5 观测点）；编译前复核半区＝project 编排（§9.3 契约表"tryFind＋自行比对"），findingRecordDigest 公共定义点供其复算；③**invalidate 状态域**——Pending/Confirmed→Invalidated（Confirmed 失效＝编译前复核失配的登记面，DT-CFM-4 前置"已确认 finding→Invalidated(revision-changed)"明文；确认事实不逆转回 Pending、复核结论须登记——§5.4 图注 Confirmed"编译前复核绑定"的落地面），Invalidated/Expired 再失效→InvalidState（不可逆）；Confirmed 失效后 coreProjection 落回 Pending（凭据失效按未确认处置——§5.3，core 投影不伪造放行）；④**Expired 态确认提交返回 InvalidState**——ConfirmOutcome 无 Expired 值（§9.3 注释原文四值），"结果枚举优先"口径下终态一律 InvalidState（登记）；⑤**reasonToken 透传**——submitRejection/invalidate 的 reasonToken 不做词表白名单强制校验（§9.3 未定义校验错误，私扩＝接口偏差；token 透传登记），空 token→Usage（§9.0"空参数等"）；Expired 态 rejectionReason＝nullopt（§5.4 词表未定义过期原因 token——不私造，状态本身表意）；⑥**confirmTextKey 派生**——"diag.<code-lower>.confirm"（P-DIAG-9 titleKey/detailKey 同一命名规则的 confirm 槽位；值归 ui 文案资源）；optionKeys/evidenceRefs/snapshotId/inputSliceId 在 create 契约签名（§9.3 逐字）无对应参数——默认空/无值，调用方经返回的记录拷贝按需补填（服务端原件仅状态字段可变，§5.2 表头）；⑦**callbackToken 一次性**——进程内单调分配（0＝保留值），任何终态转移后清零作废（§5.2"同一令牌仅一次提交有效"/§5.6"回调令牌作废"；提交/拒绝时 token 不匹配→InvalidState〔结果枚举优先〕或异常〔void 方法〕）；⑧**DT-CFM-8 装配面断言**——类型层 static_assert（Confirmable.hpp：IDiagnosticSink/IDevLogSink 与 IConfirmableFindingService 互不可转换、core::ConfirmableFinding 不得隐式窄化为 core::DiagnosticRecord 经 report 通道回传——worker 装配清单只含诊断通道）＋运行期桩半区（契约测试：worker 通道帧均为普通诊断、服务端 pendingFor 恒空）。接线登记（后续任务）：转移 Dev 日志经注入 IDevLogSink（§5.4"每次状态转移写入开发级日志"的承载；devLog 可空——DIAG-T07 logger 落地前静默跳过，落地后装配必注入）；finding 等待期不占用 diagnostics 持久资源（§5.3——目录/清理归 T09 会话态范围）；会话级记录自动清理（§5.4 终态清理）随 T09 目录清理周期。P-DIAG-1/P-DIAG-4/P-DIAG-6/CR-02 处置按契约 acceptance 3/4 执行（FindingId 自持解析句法钉住不私改 core；expiresAtUtc 默认 nullopt 无限期——project §5.3.4"无超时自动确认"，显式设置唯一起点＝实现类原语 armExpiration〔不在虚接口，seal() 先例〕；失效/过期不进 core 枚举——coreProjection 三态投影钉住；findingDigest 只经 core ContentDigester）。验证留痕 traceability/builds/wp09-t05/（ird-test-report.json：双模式 54/54＋9/9，ird_gates 7 命中＝wp09-t04 基线逐码一致零新增且零 diagnostics 命中，validate-task PASS，探针失败能力自证——tip 比对短路〔DT-CFM-4 变红、桩半区独立保持绿＝两层复核独立性自证〕与惰性过期短路〔DT-CFM-3 变红〕两探针真实变红后逐字还原，探针零残留） |
| v0.7 | 2026-09-14 | DIAG-T06 实现落位登记（≙WP-09-T03 范围设施面，契约 tasks/foundation/DIAG-T06.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Aggregation.hpp`（§6.3 CauseLink 命名空间〔rootOf/chainOf——严格契约：断链/环/未知起点/空身份一律 Usage，不返回半截链〕＋§6.4 AggregationRule 三值〔Auto/CauseRoot/CodeAndScope〕/AggregationScope〔rule＋minGroupSize 阈值 ≥1〕/AggregatedEntry〔code/causeRootEntryId/severity/category/scopeKind/occurrences/orderKey/members——MemberDetail 内嵌 core::ObjectId〕＋§9.4 IDiagnosticAggregator 两方法签名逐字＋DiagnosticAggregator 无状态纯函数实现）落地，实现 `src/Aggregation.cpp`；`_test` 增列 `test/AggregationTest.cpp`（DT-AGG-1~5、DT-CHAIN-1、DT-DUP-1 计数半区＋P-DIAG-1 处置钉住 11 用例；两模式 65/65＋9/9——既有 54＋9 零回归）。实现口径登记四项（§6.4/§9.4 未定义判据，DTB §5.4）：①**聚合键的成员归属单组化**——条目有 causedBy→"同根因"组（键＝沿单父链的**集内可达**最深祖先；链在子集边界断裂时键＝断点条目自身，容差不抛——§9.4 前置明示输入可为消费方过滤子集，呈现事实＝集内可见部分，链完整性严格判定归 CauseLink）；无 causedBy→"同 code＋同 scopeKind"组且按 DiagContext.task 五元组分区（跨任务不聚合——§6.4）；每条目恰归一组（确定性）；AggregationRule 可只启用一键（未启用键侧条目单成员私有成组，不跨键混组）；②**根因条目不入"同根因"组成员**（成员＝共享根因的派生条目；根因以 causeRootEntryId 导航呈现且组外自成呈现项——§6.3"根因不被聚合吞没"）；③**共同值投影与计数语义**——severity＝成员最高（级别秩 Error＞Warning＞Info＞Dev，显式映射表避免"枚举声明序＝级别序"误读）；category/scopeKind＝成员共同值、不一致置 nullopt（不私造汇总分类——DT-AGG-4 观测面之一）；occurrences＝成员实际次数总和（每成员计 1，非"≥N"截断；目录侧去重折叠计数归 DiagCatalog——PA-1，聚合器不重计）；视图 orderKey＝成员最早 orderKey（成员键含唯一 entryId 且每条目恰归一组→视图项键互异、输出确定全序——DT-AGG-2）；④**违约面**——aggregate 入口检查：entryId 0/集合内重复/Dev 级条目（§4.3"Dev 不参与用户级聚合"）/minGroupSize 0→Usage fail-fast；occurrenceCount 为纯线性计数查询（不执行去重折叠）；relatedTo/supersedes 不参与聚合判定（§6.4 relatedTo 明文；supersedes 语义由条目本体承载、视图原样呈现两代条目，不预建无消费者查询——NFR-MNT-04）。接线登记：DiagProjectionItem.aggregatedUnder（§9.7 阶段 A 恒空注释）的聚合视图消费随 ui/reporting 落地任务接线（本任务交付纯查询面，不改写既有投影行为）；目录侧自动聚合触发（§6.4 触发行"目录内同根因成员数达到阈值默认 ≥3"）归 DIAG-T09 目录容量/清理面。P-DIAG-1 处置按契约 acceptance 4 执行（MemberDetail.subject 内嵌 core::ObjectId——core.md v0.1 基线 static_assert 编译期钉住＋运行期逐值原样内嵌佐证；core 冻结 diff 后增量同步、不私改 core）。验证留痕 traceability/builds/wp09-t06/（ird-test-report.json：双模式 65/65＋9/9，ird_gates 7 命中＝wp09-t05 基线逐码一致零新增且零 diagnostics 命中，validate-task PASS；违约面 7 类真实抛出路径由用例负路径直接断言） |
| v0.8 | 2026-09-14 | DIAG-T07 实现落位登记（≙WP-09-T04，契约 tasks/foundation/DIAG-T07.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Logging.hpp`（§7.2 LogRecord 七字段＋CorrelationIds 六项〔entryId/findingId/taskId/runId/attemptId/revisionId 与目录/工厂同源类型〕＋§9.6 LogTier/LogLevel 词表、LogSinkConfig、ILogger 四方法签名逐字〔log/logDev/flush(deadline)/configure〕＋§7.5 WorkerLogBatch 载荷〔workerId/task 五元组/seq/final 收尾标记/records〕＋ILogFileOps 四方法接缝〔appendLine/flushFile/rotateFile/fileSize——§10 DT-LOG-3 注入面〕＋FileLogFileOps 默认实现＋LoggingPipeline 实现 ILogger＋IDevLogSink〔§9.7 尾注 reportDev 路由面〕并扩展 replayWorkerBatch）落地，实现 `src/Logging.cpp`（五步管线①截断/②Tier-U 模式过滤/③级别过滤/④节流采样/⑤异步写入＋flush 窗口 N=256/T=2s/Error 即时；轮转世代移位；队列满丢最旧 Debug/Trace；worker 乱序缓冲与断裂补条）；`_test` 增列 `test/LoggingTest.cpp`（DT-LOG-1~5、节流采样/轮转/队列溢出/截断 UTF-8 边界行为面、入口违约面 Usage、P-DIAG-7/P-EX-8 处置钉住 12 用例；两模式 77/77＋9/9——既有 65＋9 零回归）。实现口径登记九项（§7.3~§7.5/§9.6 未定义判据，DTB §5.4）：①**序号＝日志线程写入序**（单一线程串行分配，文件内 seq 严格递增＝§7.4 全局稳定序；队列 FIFO 保证处理序＝入队序）；②**镜像写入**——Tier-U 行同时写 Tier-D 文件（§7.1"Tier-U ⊆ Tier-D"与 DT-LOG-1"Tier-D 全量"的承载），Tier-D 保留原文、Tier-U 用模式过滤呈现；③**Tier-U 内部模式过滤**（§7.3②"额外过滤栈/哈希/内部 token 模式"的本管线内建承载）：0x 前缀地址→[ADDR]、≥32 位连续十六进制→[HASH]、栈帧" at 路径:行号"→" at [FRAME]"——模式清单为工程默认，NFR-SEC-07 全量脱敏（凭据/路径/环境变量/资源内容）随 DIAG-T08 IRedactionService 接线（接线点＝renderRecord 脱敏步骤，登记于 src/Logging.cpp 文件头）；④**节流窗判定用记录自身时间戳**（注入 IClock——ManualClock 确定性可测；窗关闭汇总行在下一同键记录到达时先于其落盘，采样"末条"只在关闭排空时写出——进行中的流无"末条"可定义，§7.4 承诺在关闭路径兑现）；⑤**断裂判定时机**——乱序批次一律缓冲（缺口可能仍在途），仅收尾批（final）或关闭排空时对缺口补 EX-CHANNEL-PROTOCOL-ERROR Dev 行；陈旧/重复批丢弃＋Dev 计数行（幂等重放）；⑥**内部行旁路采样**（自产汇总/补条行不被自身机制吞噬；写失败行保留节流聚合防风暴）；⑦**日志线程顶界兜底**——runLoop noexcept 吞掉一切逃逸异常＋stderr 摘要（§7.6"stderr 摘要＋放弃"同款；日志非权威绝不终止进程——PM-03）；⑧**FileLogFileOps 逐行开-写-关**（fclose 即落盘——崩溃丢失窗口小于 §7.6 的 T；不持常驻句柄——Windows 句柄共享语义下外部读取/轮转不被阻塞）；flushFile 恒真（无持久脏缓冲，幂等面）；⑨**队列满无可丢行时丢弃新行**（§9.6 只定义"丢最旧 Debug/Trace"——全 Error/Warning/Info 队列无可丢项，丢新行并计数登记）。陷阱处置：P-DIAG-7（轮转 5×2 MiB/flush 窗 N=256/T=2s/采样 N=100/节流窗 10s/队列 4096 均为 LogSinkConfig 可配工程默认，默认值由 DtPdiag7 用例钉住——WP-23 性能验收时校准，不作为需求语义）；P-EX-8（WorkerLogBatch 只定载荷语义，Logging.hpp/cpp 零 execution 侧 include、零帧类型词汇——DtPex8 源码扫描钉住；帧类型/通道帧表归 execution §6，§12.2 交接）。接线登记（后续任务）：IRedactionService（DIAG-T08）接入 renderRecord 脱敏步骤；ConfirmableService 的 IDevLogSink 注入自本任务起可装配（v0.6 登记"logger 落地后装配必注入"）；目录订阅回调仍为调用方线程同步派发（§9.7 契约表登记形态，不改写——异步化非本契约 acceptance 面，如需迁移随目录所有任务登记）。验证留痕 traceability/builds/wp09-t07/（ird-test-report.json：双模式 77/77＋9/9，ird_gates 7 命中＝wp09-t06 基线逐码一致零新增且零 diagnostics 命中，validate-task PASS，verify-task PASS；写失败/队列溢出故障注入经 ILogFileOps 接缝自持替身真实触发，开关闸门确定性复现） |
| v0.9 | 2026-09-15 | DIAG-T08 实现落位登记（≙WP-09-T05，契约 tasks/foundation/DIAG-T08.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Redaction.hpp`（§7.7 规则表＋§9.5 IRedactionService 接口四方法签名逐字＋PathPolicy 四值＋RedactionPolicy＋RedactionService 实现〔stats/policy/applyRules 为实现类扩展——seal() 先例，不扩接口〕）与 `CrashReport.hpp`（§7.6 CrashReportInput 六段载体＋CrashReportWriter 写出设施；OS 级崩溃捕获/应用壳装配归 WP-24-T02——不越界实现 dump 机制，契约 acceptance 3 边界）落地，实现 `src/Redaction.cpp`/`src/CrashReport.cpp`；`_test` 增列 `test/RedactionTest.cpp`（DT-SEC-1~4、DT-LIFE-4、AT-11 崩溃文件＋P-DIAG-1 处置钉住 21 用例；两模式 98/98＋9/9——既有 77＋9 零回归）。实现口径登记八项（§7.7/§9.5 未定判据，DTB §5.4）：①**规则执行序**＝凭据键值→认证方案凭证→环境变量引用→用户名路径段→路径候选四策略→令牌形态→User 档内部模式（凭据优先于路径；用户名先于路径策略——Keep 下用户名也不泄露，§7.7 用户名行无策略条件；相对路径不是"本机路径"不处理——R-7）；②**认证方案词表只收 bearer|basic**（不收 digest/ntlm/negotiate——"digest"与摘要语义同词会系统性误伤开发诊断文本"sha256 digest <hex>"，R-7；模式清单"只增不减"安全契约自本头首次交付起算）；③**纯十六进制长串不按令牌处理**（令牌规则须含字母与数字且非纯十六进制——摘要/内容身份形态保留；JWT 三段式经 '.' 入字符集整段捕获）；④**Hash 策略输出**＝[PATH-<16 位小写 hex>]（SHA-256 前 8 字节经 core ContentDigester——P-DIAG-1/CR-02 唯一哈希路径；64 位截断可关联不可逆推；避开 Tier-U ≥32 hex 呈现过滤与规范身份形态）；⑤**RootOnly**＝盘符/UNC 根〔server\share〕/POSIX 根＋一级目录＋"…"＋文件名（§7.7 文本口径"保留盘符＋一级目录"——卡内示例 D:\…\file.stl 与文本不一致按文本实现；仅根级文件名等退化形态不伪造"…"）；⑥**Tier 语义与消费面**——redact(_, Dev)＝NFR-SEC-07 全量且不做内部十六进制遮蔽（管线两 Tier 统一、exportSafeSummary/工厂快照/崩溃文件自由文本用此档：subject/obj- 等规范身份是机器可读锚点）；redact(_, User)＝全量＋内部模式（0x→[ADDR]/≥32hex→[HASH]/栈帧→[FRAME]，与 Logging.cpp 呈现过滤同形——报告面不携带栈/地址/哈希）；safeSummary＝User 档＋UTF-8 边界截断＋[trunc]；⑦**结构化身份字段不过脱敏**（崩溃文件 task/finding/project 规范串——非敏感契约值且多段 '/' 连接的规范串会被令牌规则整体吞掉〔R-7 误伤〕，P-DIAG-1"原样内嵌"优先）；⑧**崩溃写失败码复用 DIAG-LOG-WRITE-FAILED**（§4.6 内置 87 码冻结无 crash 专属码；消息以 [crash-report-write-failed] 前缀区分；stderr 摘要目录经 redactPath 脱敏；不递归诊断）。**接线登记三面**（v0.5/v0.8 预登记的落地）：管线 renderRecord 步骤②挂接 IRedactionService（内部通道 diag/logging、diag/log-merge、diag/redaction 旁路——自产常量文本行再脱敏会形成失败行回环，且 DT-LOG-1~3 既有断言零回归）＋devTailSnapshot 环形缓冲读取面（先于磁盘写捕获——磁盘满时崩溃快照仍完整）；DiagCatalog::exportSafeSummary 每行输出强制再脱敏（DT-SEC-4 双保险）；DiagnosticsFactory 的 redactedContextSnapshot 构造期产出（§4.2——非空 params 快览串 Dev 档脱敏，空 params 恒 nullopt 不伪造）＋translate 异常消息先脱敏后截断（v0.4 口径④收口）。陷阱处置：P-DIAG-1（崩溃文件输入以 core::ProjectId/TaskIdentity 内嵌＋FindingId 自持——测试侧 static_assert×3 编译期钉住＋运行期 canonical 逐值断言；Hash 策略 SHA-256 只经 core ContentDigester——CR-02 同源；core 冻结 diff 后增量同步，不私改 core）。验证留痕 traceability/builds/wp09-t08/（ird-test-report.json：双模式 98/98＋9/9，ird_gates 7 命中＝wp09-t07 基线逐码一致零新增且零 diagnostics 命中，validate-task PASS，verify-task PASS；DT-SEC-3 经 applyRules 虚缝、DT-LIFE-4 经 ILogFileOps 两轨〔false/抛异常〕真实触发降级链） |
| v0.10 | 2026-09-15 | DIAG-T09 实现落位登记（≙WP-09-T04 范围，契约 tasks/foundation/DIAG-T09.json）：§1.2 卡头"构建落位"字段实测刷新——公共头 `Catalog.hpp` 扩展（§6.2 `CatalogCapacityConfig`〔enabled＋maxEntries=10,000——P-DIAG-7 工程默认登记于结构体注释〕＋`DiagCatalog` 五方法 `configureCapacity`〔0 容量 Usage〕/`capacityConfig`〔观测面〕/`markConsumed`〔消费确认〕/`setTaskActive`〔活动任务保护登记〕/`attachDevLogSink`〔溢出事实出口〕＋`kCatalogInternalChannel`="diag/catalog"〔对齐 diag/redaction 命名族〕），实现 `src/Catalog.cpp` 扩容（§6.2 分级淘汰＋`DIAG-CATALOG-OVERFLOW` 溢出登记——无新增翻译单元）；`_test` 增列 `test/CatalogLifecycleTest.cpp`（DT-LIFE-1 容量与分级淘汰·软溢出·链保护·缩容、DT-LIFE-2 失败/取消/中断保留、DT-LIFE-3 归档失败报告＋关闭有界〔真实 LoggingPipeline＋FileLogFileOps 临时目录半区〕＋P-DIAG-7 默认值钉住 9 用例），`_contract_test` 增列 `test/CatalogHostTraceStubTest.cpp`（DT-LIFE-5 字节不变·摘要比对·supersedes 更新＋宿主留痕形状冻结〔envelope/CommandRecord 对接桩〕5 用例；两模式 107/107＋14/14——既有 98＋9 零回归）。实现口径登记六项（§6.2/§9.7 未定义判据，DTB §5.4）：①**"已消费"判据＝markConsumed 显式确认**（目录无法感知 ui/reporting 呈现时机——消费方拉取投影/导出后显式登记；advisory 语义：未知/已淘汰 id 容忍无操作〔淘汰只移除已消费条目，迟到确认是良性竞态，fail-fast 会把竞态升级为消费方崩溃〕，entryId==0 Usage；Warning/Error 条目确认同样登记——登记"已消费事实"与"是否可淘汰"分离）；②**活动任务保护＝集合登记而非状态判定**（setTaskActive 只承载保护登记，任务状态权威归 execution——PA-1；注销不立即淘汰且终结任务诊断照常保留——DT-LIFE-2/NFR-REL-03；淘汰只在 append 容量压力时评估——§6.2"超限淘汰"的压力驱动语义）；③**淘汰算法＝引用计数快照＋多遍收敛**（每遍构建全目录 causedBy/relatedTo/supersedes 被指条目计数——被指者不淘汰：append"链接指向已存在条目"的不变式不得被淘汰制造悬挂引用〔§6.3〕；淘汰序＝入目录序〔最旧先行〕；引用者与被引用者同批可淘汰时被引用者下一遍解除保护——正确性优先于遍数；applyEvictions 保序一次性重建三张索引表，避免逐条中段删除 O(n²)；复杂度每遍 O(n·L)，仅容量压力路径执行——护栏不进热区，WP-23 校准面）；④**软溢出＝护栏让位于证据完整性**（无可淘汰候选〔Warning/Error/活动关联/未消费/链保护〕时新条目仍追加，规模可暂超上限；淘汰腾位与保留保护两种溢出均登记 DIAG-CATALOG-OVERFLOW Dev 诊断〔§6.2"开发诊断登记溢出事实"〕——通道 kCatalogInternalChannel、消息首 token＝稳定码＋capacity/evicted/retention-protected/size 观测数值；出口未挂接＝静默〔装配前合法降态，同 RedactionService failureSink 口径〕；锁内捕获锁外派发〔出口可能入队 I/O〕）；⑤**配置语义＝快照切换＋缩容延迟生效**（configureCapacity 下一条 append 起生效；缩容不立即淘汰——主动收缩可能淘汰未消费条目，宁暂超限不越"已消费"判据，由下一次压力一次腾位到位；enabled=false 关闭护栏无溢出登记；v0.7"目录侧自动聚合触发"的关联面澄清：§6.4 聚合触发仍为消费方显式调用〔"不在创建时自动聚合"〕，本任务未预建目录→聚合器回调面——§9.7 无此消费面，如需随 ui/reporting 消费任务登记，NFR-MNT-04）；⑥**宿主留痕形状＝对接数据形状冻结**（acceptance 2——诊断随宿主持久化的编码归宿主所有者：envelope canonical 归 evidence、CommandRecord 归 project；本任务零宿主编码设施新增，契约测试的编码器定义在测试侧即结构性证明；持久化载荷＝core::DiagnosticRecord 纯值〔§4.2 序列化行〕——entryId 不入形状〔两会话异 id 同字节反例钉住〕、绑定＝宿主对象身份〔envelope task/CommandRecord revision〕、Dev 码宿主桩拒绝〔historical=true 才允许〕；Superseded 投影不传播为记录修改——重读逐值/逐字节/SHA-256 摘要〔core ContentDigester〕三重不变）。陷阱处置：P-DIAG-7（容量默认 10,000 条＝可配工程默认——DtPdiag7 默认值逐项断言＋可配置面〔整值替换/关闭护栏〕自证；WP-23 校准不构成需求偏差）。验证留痕 traceability/builds/wp09-t09/（ird-test-report.json：双模式 107/107＋14/14，ird_gates 7 命中＝wp09-t08 基线逐码一致零新增且零 diagnostics 命中，validate-task PASS，verify-task PASS；DT-LIFE-3 关闭时限以 steady_clock 墙钟有界断言，溢出登记经真实管线落盘 Tier-D 且 Tier-U 零命中） |

| v0.11 | 2026-09-15 | DIAG-T10 实现落位登记（≙WP-09-T06，契约 tasks/foundation/DIAG-T10.json）：§1.2 卡头"构建落位"字段实测刷新——§10 验证矩阵收口三面落地：①**ScriptedDiagSource 替身**（§11 行产物"按序列产出预设条目/finding/日志行"；`test/ScriptedDiagSource.hpp`——统一序列步进回放〔条目/finding/日志行三形态交错，回放游标单调、脚本耗尽＝有界正常终态〕＋回放进度/产出观测面；替身只负责按序列供给预设数据，三个回放分派全部走**真实产品路径**〔DiagnosticsFactory::create→DiagCatalog::append／ConfirmableService::create／LoggingPipeline::log〕——EV-REG-3 模式第 3 条"被测产品对象不做替身"；替身边界声明三面在案＝test/README.md §1 全文＋替身头文件头＋ScriptedDoubleBoundary 机检两面〔产品源码面零替身符号递归扫描＋README/头文件声明锚点检查〕——acceptance 2）；②**testkit 契约谓词消费**（`test/DiagContractCheckTest.cpp` 6 用例——checkDiagnosticRecord 双面同判〔工厂正例双过／Dev 记录 allowTransient 选项门控／未经工厂的占位默认记录被独立拦下且逐字段指明〕＋checkComparativeFields 量纲契约〔同量纲双过／混量纲拒绝且指明 unit〕；`_contract_test` 链接 `sdurws_ird_testkit`＝testkit.md §2.4 允许依赖形态，LinkageContractTest 的单元边白名单沿用面扩入 sdurws_ird_testkit 并在用例 DOC 注释登记依据——acceptance 3/P-DIAG-8 处置）；③**DT 矩阵最后两组**（`test/CollaborationQueryTest.cpp` 8 用例——DT-COLLAB-1 命令/任务路径诊断经 DiagQuery 的 revision/task 两轴过滤各自命中且互不混＋DT-AT 观测点映射七用例〔AT-01 比较型三要素＋确认留痕形状／AT-10 EX-REGISTRY-\* 码形状＋原因链可达根因／AT-11 崩溃文件六段载体＋脱敏写出／AT-13 PRJ-RECOVERY-\* 呈现形状／AT-19 码表单点两次查询逐字段一致／AT-30 params 承载失效原因清单形状／AT-34 取消 Info 非错误分类形状——"本文只提供观测点，不代验 AT"边界在各用例 DOC 注释逐处钉住〕——acceptance 1 收口）。**ird_gates 命中 7→8 如实登记**：新增 1 处 IRD-GATE-SUB＝sdurws_ird_diagnostics_contract_test→sdurws_ird_testkit（本任务 acceptance 3 明文要求的契约谓词消费面——testkit.md §2.4 允许依赖形态 `{被测产品目标, sdurws_ird_testkit, gtest}`；与既有基线中 evidence/policy/runtime 五处测试目标直链 testkit 同款同源，产品目标仍零 testkit 边〔T-1 面由 LinkageContractTest＋配置期守卫＋替身符号机检三面钉住〕；DTB §4.5 例外登记册维护归 WP-01-T03、登记义务转登验收裁决——本契约 allowedFiles 不含 DTB/门禁白名单）；P-DIAG-8 处置其余面按契约原文执行：execution 链接启用前不私改其构建、双库集成冒烟按 P-EX-8 联动另行登记（governance-log P-DIAG-8 状态仍 open，本任务不消账）。实现口径登记三项（§10/§11 未定义判据，DTB §5.4）：①**替身仅覆盖"可控诊断源"一个测试设施位**——脚本数据是按 §4.2/§5/§7.2 契约形状构造的测试输入，不构成真实业务错误/worker 崩溃/磁盘故障等场景的正确性证明、不进任何持久形态（README §1 五条全文）；②**脚本负例不进替身**——脚本载荷违反产品契约时异常从真实工厂/服务/管线原样传播（替身不捕获不吞错），负例验证归各行为套件自持、替身只承载正例契约形态数据；③**目标分工按 §10 头注**——替身基座与 testkit 谓词消费落位 `_contract_test`（跨单元契约面），DT-COLLAB-1/DT-AT 落位 `_test`（单元内行为），两目标两模式构建。验证留痕 traceability/builds/wp09-t10/（ird-test-report.json：双模式 115/115＋26/26——既有 107＋14 零回归；validate-task PASS、verify-task PASS；集成全量构建 exit 0 零 error；ird_gates 命中清单见 gates-ird-gates-target.log） |

| v0.12 | 2026-09-15 | DIAG-T11 文档与门禁同步（≙§11 末行〔无 dtb 编号——卡内治理任务，DTB §5.4 增量修订登记〕，契约 tasks/foundation/DIAG-T11.json；**纯文档任务零代码变更**——diagnostics/src、include/*.hpp、CMakeLists 零触碰）：①**本文与实现零偏差登记（acceptance 1）**——§11 DIAG-T02~T10 各产物与 §14.4 v0.3~v0.11 各登记逐项复核：include/ 10 文件（9 契约头＋README.md）与 §3.1 模块表逐行对应、src/ 11 翻译单元＋2 私有头与各卡登记一致（累加 1＋2＋3＋1＋1＋1＋2）、test/ 14 测试 .cpp＋仅头替身＋README.md 与各卡登记一致（累加 2＋1＋1＋2＋1＋1＋1＋2＋3）；双模式实跑复证 §14.4 v0.11 基数逐数一致（115/115＋26/26，21＋6 套件逐套件同构）；ird_gates 8 命中与 wp09-t10 验收基线逐码一致零新增（grep IRD-GATE 排序 diff＝GATES-IDENTICAL-8）；本版登记一处卡内措辞 slips 的更正（§1.2 卡头"hpp/.cpp 对"→仅头替身——非实现偏差：§14.4 v0.11 权威登记与磁盘实现一致，见④）；核对证据 traceability/builds/wp09-t11/README-audit.md（§2~§5 清单/计数/状态比对实录）；②**README 指向核对零偏差（acceptance 2）**——`diagnostics/include/sdurws/ird/diagnostics/README.md` 任务卡指向维持 §11（§9→§11 修正复审通过，无回退无第二指向）；实现状态段同步至 DIAG-T10 落位现状（九契约头逐头内容锚点＋§14.4 版本链＋builds/wp09-t02~t11 留痕路径＋../test/README.md 落位表指引），消除 2026-09-10"本文不表示库已实现或测试已通过"过期前瞻句（EV-T12 同款处置，改为"实现与测试通过状态以 §14.4 与留痕为准"的如实指向）——审计证据 README-audit.md §1/§5；③**§14.3 状态更新（acceptance 3）**——P-DIAG-1~9 逐条附〔DIAG-T11 状态同步〕事实标注：P-DIAG-1/2 登记上游现状实测（core.md 现 v0.11 未冻结、ARCHITECTURE 现 v0.12 事实修正未触及本文锚点，均零返工面）；P-DIAG-3/4/6/7/9 实现侧事实同步（词表/自持解析/无限期默认/工程默认/键值分离均已落地并用例钉住，ui.md 承接面闭合）；P-DIAG-5 附 **P-PR-6/P-EX-8 消账确认**（对端 project.md/execution.md 实测均 v0.2 且两项仍登记 open——其 v0.2 变更记录明言"码值部分消账、sink 名称/归属统一不因卡存在自动消账"；消账路径＝project/execution 详设修订联动〔各自卡内增量修订时收编统一接口与链接形态〕，本文侧 IDiagnosticSink＋DiagnosticsSinkImpl 已交付〔v0.5〕，不私改对端契约）；P-DIAG-8 维持 open 不消账（execution 链接启用未发生，与 §14.4 v0.11 口径一致）；九项裁决归属均不变更、不私裁（沿 CORE-T10/RT-T13/EV-T12 文档任务纪律）；④**卡头同步**——文档版本行 v0.11→v0.12、§1.2 卡头 T10 段措辞更正（"test/ScriptedDiagSource.hpp/.cpp 对"→"test/ScriptedDiagSource.hpp 仅头替身＋test/ScriptedDiagSourceTest.cpp"——磁盘实测与 §14.4 v0.11 登记均为仅头替身〔产出逻辑内联于头文件〕，git 全历史无 ScriptedDiagSource.cpp，DIAG-T10 实现提交 a71e42ee 仅增 hpp＋Test.cpp 两文件——DTB §5.4 登记不静默）；validate-docs（契约 verify）/validate-task/verify-task PASS、双模式构建零 error 与复证实跑留痕见 traceability/builds/wp09-t11/（ird-test-report.json：regressionReRun 两模式 115/115＋26/26，零新增用例——纯文档任务） |
| v0.13 | 2026-09-22 | O-41 所有者裁决联动对齐（纯文档一格修订——零代码变更）：§8.2 project StoreError→诊断映射表 lock-held-by-other 行 subject 格由"∅（项目级）"改为"项目对象（ProjectId，发射侧必填）"——本格与本文 §9.2 IDiagnosticFactory 对用户级码的 subject-missing 校验相悖（§4.2 合法示例〔PRJ-LOCK-HELD 带 subject〕同证），WP-10-T15 补验收 B-1 根因链实证该矛盾（findings F-290）：project 发射侧此前按本表产出空 subject 记录，首个经工厂消费的下游（UI-T15 ProjectDiagnosticsBridge）即触发契约异常。裁决方向＝发射侧补齐（units/project.md v0.19 承载），本文仅对齐映射表消除自相矛盾；码表 §4.6 权威零变更 |
