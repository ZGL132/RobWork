...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.9（2026-09-27，WP-15-T09 落位登记——§14.6；v0.8＝2026-09-27 WP-15-T08 落位登记；v0.7＝2026-09-27 WP-15-T07 落位登记；v0.6＝2026-09-26 WP-15-T06 落位登记；v0.5＝2026-09-26 WP-15-T05 落位登记；v0.4＝2026-09-26 WP-15-T04 落位登记；v0.3＝2026-09-26 WP-15-T03 落位登记；v0.2＝2026-09-26 WP-15-T02 落位登记；v0.1 首版草案 2026-09-22，WP-15-T01 交付物） |
| 日期 | 2026-09-27（v0.9）；2026-09-27（v0.8）；2026-09-27（v0.7）；2026-09-26（v0.6）；2026-09-26（v0.5）；2026-09-26（v0.4）；2026-09-26（v0.3）；2026-09-26（v0.2）；2026-09-22（v0.1） |
...
| 构建落位 | `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/kinematics/`（**WP-15-T02 起为真实目标**：`sdurws_ird_kinematics` STATIC（C++17、零 Qt、PUBLIC 链 core/diagnostics/runtime/policy/evidence/execution 六条 §3.2 登记边；PRIVATE 链 Eigen3——O-40，见 §14.6 v0.3）＋`_test`/`_contract_test` 两测试目标＋配置期红线守卫；T02 公共面＝Errors/DiagCodes 两头，T03 公共面＝KinTypes/Fk/Evaluators 三头，T04 公共面＝Ik/SolutionSet/Bounds 三头＋KinTypes 表尾追加，T05 公共面＝Evidence.hpp 新增＋Evaluators 表尾追加，T06 公共面＝Sampling/Coverage 两头新增＋Evaluators 表尾追加，T07 公共面＝Collision.hpp 新增（实现 TU 集成模式专属）＋Ik/Evaluators/Evidence/Sampling 表尾追加，T08 公共面＝Commands.hpp 新增（两模式编译）；`_plugin` 随 WP-15-T12，见 §3.2、§11） |
...
| `Collision.hpp` | ④端口碰撞接入（KIN-05/AT-19）：`IKinCollisionSceneSource`（快照事实注入端口）＋`assembleCollisionScene`（请求方场景装配）＋`PolicyCollisionSessionAdapter`（真实 policy 会话适配）＋`makeCollisionSession`（接线三态）。实现 TU 集成模式专属（policy POL-T06 同款 gating） | T07 |
| `Commands.hpp` | `IKinematicsCommandHandler`（KIN-14 设默认门面） | T08 |
| `Export.hpp` | 结果导出值行（KIN-08/§7.4）：解行/过滤诊断行/区域覆盖行值模型＋JSON/CSV 文本编码＋导出文件头来源声明（写出经 io 通道——纯数据整理面，AT-04 零修订结构保证） | T09 |
| `Recompute.hpp` | 批量复算请求值与构造（KIN-08/§7.4）：按原 slice 重新提交 execution 新 run 的域侧请求构造（提交动作归 execution；不复用会话残留、当前性重新判定——CON-02/05） | T09 |
...
| 评估键 | 依赖声明 | 模式 |
| --- | --- | --- |
| `kin.pose-metrics`（KIN-01；实现键＝`kin-pose-metrics`——evidence 冻结评估键词形闸门 `[a-z][a-z0-9-]{1,63}` 不接受点（DependencyKey 才允许），kebab 化登记随 §14.6 v0.3；T04~T07 各键随其落位任务同样处理） | `model.robot-design`(Object)、`tcp`(Object→tool-definition)、`policy.resolved`(Policy)、`namemap`(NameMap)、`config.ik`(Configuration) | Preview/Quick/Verified |
| `kin.task-point-ik`（KIN-02；实现键＝`kin-task-point-ik`——kebab 化随 §14.6 v0.4，同 pose-metrics 行口径） | `model.robot-design`(Object)、`tcp`(Object→tool-definition)、`policy.resolved`(Policy)、`namemap`(NameMap)、`config.ik`(Configuration)、`req.points`(Object) | Preview/Quick/Verified |
| `kin.task-points-batch`（KIN-03；实现键＝`kin-task-points-batch`——kebab 化随 §14.6 v0.5，同 pose-metrics 行口径） | 上述＋`req.conditions`(Object)（caseSubset 分批） | Quick/Verified |
...
| `KIN-NEAR-SINGULAR` | warning | 条件数恶化/可操作度低（NearSingular/Singular；阈值读 policy） | T03（码面已随 T02 登记）；产生面随 policy 阈值消费面落位（T02 CMake 分工：policy 边消费归 T04/T07——§14.6 v0.3） |
...
| `KIN-POINT-REF-DANGLING` | error | 任务点引用悬空（appliesTo 显式清单引用点集外对象→该工作项产 InputInvalid 素材——V-12） | T05（行随消费任务表尾追加——§14.6 v0.5） |
| `KIN-ROOT-BYTES-ILLEGAL` | error | 设默认命令门面的根对象字节补丁失败（基线根不可解码/补丁产物编码失败——数据侧，零提交零修订） | T08（行随消费任务表尾追加——§14.6 v0.8） |
...
| WP-15-T03 | FK 与指标评估器（`kin.pose-metrics`＋IFkEvaluator）＋解析黄金 UT（AT-03/NFR-COR-01）——原行"＋Bounds 解析素材"按 §3.3 布局表归 T04（T04 契约 note 已登记该落点偏差，随 v0.3 消歧） | T02、WP-06-T09（DeviceView） | ③（已完成——WP-15-T03） |
| WP-15-T04 | 多初值 IK＋去重排序（`kin.task-point-ik`＋IIkSolver＋SolutionSet；搜索未果记录；两提交）——已完成（WP-15-T04：`Ik.hpp/.cpp`＋`Bounds.hpp/.cpp`＋`SolutionSet.hpp/.cpp`＋KinTypes 表尾追加＋评估器族追加，§14.6 v0.4） | T03、WP-07-T07 | ④（已完成——WP-15-T04） |
| WP-15-T05 | 批量任务点验证（`kin.task-points-batch`＋EvidenceBuilder＋检查点 watermark）——已完成（`Evidence.hpp/.cpp` 新增＋Evaluators.hpp/.cpp 表尾追加，§14.6 v0.5） | T04、WP-08 | ⑤（已完成——WP-15-T05） |
| WP-15-T06 | 区域覆盖率评估（`kin.region-coverage`＋IWorkspaceSampler＋sampleSetIdentity 实现＋零样本/降级）——已完成（WP-15-T06：`Sampling.hpp/.cpp`＋`Coverage.hpp` 新增＋`Evaluators.hpp/.cpp` 表尾追加，§14.6 v0.6） | T04 | ⑤ |
| WP-15-T07 | 碰撞证据接入（④端口会话组装＋KIN-05 语义＋三入口一致断言）——已完成（WP-15-T07：`Collision.hpp/.cpp` 新增＋`Ik.hpp/.cpp`（碰撞判定三态＋结局标记）＋`Evaluators.hpp/.cpp`/`Evidence.hpp/.cpp`（碰撞证据行＋诊断产码面）＋`Sampling.hpp` 注释接线随附同步，§14.6 v0.7） | T04、WP-07-T07 | ⑤ |
| WP-15-T08 | 会话姿态与设默认（L-K4/L-K9＋Commands 门面）——已完成（WP-15-T08：`Commands.hpp/.cpp` 新增（两模式编译——KinSessionPose＋IKinematicsCommandHandler/KinematicsCommandHandler＋①/②端口投影注入面 IKinProjectCommandGateway＋RootDesignPatcher 补丁缝＋载荷框架编码 encodeKinApplyRobotDesignPayload），DiagCodes 表尾追加行 17，§14.6 v0.8） | T03、WP-04-T10、WP-10 | ⑥ |
| WP-15-T09 | 结果筛选/导出/批量复算（SolutionSet 视图消费面＋导出值行）——已完成（WP-15-T09：`Export.hpp/.cpp`＋`Recompute.hpp/.cpp` 新增＋`SolutionSet.hpp/.cpp` 规范筛选谓词工厂增列＋测试三文件，§14.6 v0.9） | T05 | ⑥ |
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| v0.4 | 2026-09-26 | WP-15-T04 落位登记（DTB §5.4，多初值 IK 与解集去重排序＝`Ik.hpp/.cpp`＋`Bounds.hpp/.cpp`＋`SolutionSet.hpp/.cpp`＋`KinTypes.hpp` 表尾追加＋`Evaluators.hpp/.cpp` 表尾追加，验证留痕 traceability/builds/wp15-t04/）：①**KinTypes.hpp 表尾追加**（T03 批声明不重排）：`IkOutcomeKind` 五类结局枚举（§5.4 表行号 1~5；枚举无任务级不可行值——D-KIN-7）、`KinematicSolution`/`CollisionStatus`/`FilteredSolutionRecord`/`IkSearchRecord`/`IkSolutionSetStatistics`/`IkSolutionSet`/`IkTargetRef`/`IkRequestIdentity` 值模型（§6.1）、`configurationSignature`（magic `IRDSIG01` 全精度定宽——I-KIN-3 记录键）；`IkRequestIdentity` 增列 `referenceQ`（§6.3 D-KIN-4"身份含之"——卡面 §6.1 要点列未穷尽字段，随本行登记）。②**Bounds.hpp/.cpp**（§3.3 布局表 T04 行兑现，v0.3⑦消歧的落位）：解析工作半径上界＝Σ逐关节 origin 平移模长＋Σ移动关节行程贡献 max(\|qmin\|,\|qmax\|)＋TCP 偏置模长（三角形不等式——对全部关节角取值成立，仅约束位置半径不含姿态，前提写入 proof.preconditions）；证明素材经 `makeAnalyticBoundMaterial` 组装（claimToken=`kin.reach-beyond-link-sum`、coverageClaim=kCoverageClaimAllAlternatives、producer=kin-task-point-ik＋契约版本、snapshotId/sliceId 取请求身份＝expectedSliceId 绑定）；仅素材不裁定。③**Ik.hpp/.cpp**：`IIkSolver`/`IkSolver` 无状态实现＋`IkRequest`/`IkOutcome`。实现口径（随本行登记，黄金锁定 T13）：阻尼最小二乘 Δq＝Jᵀ(JJᵀ＋λ²I₆)⁻¹e、λ=1e-3（LLT 正规方程）；FK 复算唯一委托 FkEvaluator（计算面单一实现点）；取消探针每 16 次迭代＋初值边界（kIkCancellationProbeInterval——§8.3 粒度落值）；初值策略——ReferenceQ={referenceQ}、SeededRandom=splitmix64(seed·2³²+k) 逐轴均匀、JointGrid=评价区间对角均分 t_k=k/(count−1)（count=1 取中点；不做自由度组合展开），无工作范围 continuous 轴采样名义区间 [−π,π]；硬过滤顺序固定①残差复验（FK 复算）→②限位→③碰撞，记录取**首个命中阶段**（顺序即语义）；去重＝`deduplicateSolutions` 唯一实现点（逐轴阈值成对比较、无跨周取模、输入序先到先留＝组内代表最小 sourceInitIndex；去重合并不产生过滤记录）；稳定排序＝`sortSolutions` 唯一实现点（§6.3 四键——求解器管线与解集视图共享同语义）；结局判定落值——零可行解时按**阶段①幸存数**区分结局 2/3（有收敛候选全被②③过滤＝3、零收敛＝2）、有可行解且存在 Collision 原因记录＝4（PartialCollision）、否则 1；结局 5 先于迭代（静态界限检查）。`IkRequest` 携带评价区间字段 `intervals`（§5.3"关节 bounds 来自快照"——值经 `evaluationIntervals(view)` 从同一快照提取，调用方义务"与 modelView 同快照"随注）。碰撞会话句柄＝`IKinCollisionSession` 最小消费端口（`Ik.hpp`——O-37 宿主注入先例同款：本单元定义自有最小接口、L5/T07 以适配器注入真实④端口会话；R-POL-2 不变——碰撞实现唯一归 policy，测试替身先行 §10.1 口径）。调用方错误 fail-fast 轨（§9.1）：非法目标/容差→KIN-TARGET-ILLEGAL 语义锚、TCP 缺失→KIN-NO-TCP、referenceQ/初值/区间违例同轨——**KinematicsErrorCode 不追加枚举值**（§9.2 IIkSolver 无 @错误 行域错误名可收编、solve() 签名无 Expected 通道故无值面生产者——§9.6"不预建无消费者条目"纪律；Errors.hpp 零改动）。④**SolutionSet.hpp/.cpp**：`IKinematicSolutionSet`/`KinematicSolutionSet` 视图（§6.2——sorted/filtered/statistics/worstBy；构造时完成一次稳定排序、构造后不可变）；`sortSolutions`/`deduplicateSolutions` 自由函数＝排序/去重唯一实现点（NFR-MNT-04）。worstBy 三度量（裕量最小/条件数最大/残差最大）取 sorted 序第一个最差值（稳定语义）。⑤**Evaluators 表尾追加**：`kin.task-point-ik` 评估器族——`makeTaskPointIkDescriptor`（§4.3 行六条 Required 依赖＝pose-metrics 五条＋req.points；随 §4.3 行标注实现键 `kin-task-point-ik` kebab 形态，v0.3 同款偏差）、`TaskPointIkEvaluator`/`TaskPointIkEvaluatorFactory`（宿主注入、create() 无参）。错误分轨（evidence §9.3"域自选"落值）：装配期 fail-fast（目标 NaN/容差非法/referenceQ 违例/SeededRandom seed=0〔I-KIN-4〕/tcpKey 不命中）；评估期结构化诊断——KIN-NO-TCP（工具侧缺失两分口径同 T03）、KIN-RESIDUAL-EXCEEDED（比较型，按实际越限维度逐条携带 m/rad 对比——§9.6 行 4 requiresComparison 生产面）、KIN-JOINT-LIMIT-VIOLATED（行 5）、KIN-SEARCH-EXHAUSTED（行 10，附 searchRecord）；KIN-COLLISION-FILTERED 产码面归 T07（§9.6 任务列分工）。结局映射：1/4→payload（`encodeTaskPointIkPayloadCanonical`，magic `IRDIK01`＋codec u32 v1，§5.6 绑定六要素内嵌——snapshotId/sliceId/configDigest/mode/seed/referenceQ＋pointOid/conditionId＋task 五元组（4×16B＋attempt u64）＋evaluationKey/contractVersion；要求值分离——解记录只载达成值）、2/3→output.searchRecord（evidence::SearchExhaustedRecord，filteredSolutions.signature→solutionRef、reason 三值映射）＋诊断、5→output.proof、cancelled→零素材（取消不是结局）。模式敏感性：mode 入身份字节（Quick/Preview 载荷以 mode 承载 screening-only——§8.4，效力门禁归汇总/包络层）。评估键/契约版本常量唯一书写点随 Ik.hpp（证明素材 producer 绑定共用）。⑥**测试**：KinIk 18 新用例（V-05 收敛、V-04 双构型保留〔平面 3R 双分支闭式参考——2R 全位姿逆解唯一故 V-04 载体改用 threeLinkModel，随本行登记〕、V-06 搜索未果、V-07 扩初值翻转、V-08 碰撞单解过滤、V-09 全碰撞过滤不出不可行、V-03 非法 fail-fast、V-10 前半素材＋evidence validateProof 零问题自证、I-KIN-3 两向语义、逐轴阈值语义、取消零素材、逐位一致、referenceQ 入身份、视图四键排序/worstBy/filtered、FilterOrderLimitBeforeCollision、InitialValueStrategiesDeterministic、DedupMergesWithinPerAxisTolerance）＋KinTaskPointIkEval 9 契约用例（descriptor 六条依赖/键词形闸门/宿主注入工厂/装配期 fail-fast/结局映射三轨/取消/确定性与模式敏感）——**45/45＋20/20 两模式通过并留痕**（gtest XML＋ird-test-report 双份；ctest 2/2；集成构建零错误；冒烟配置/构建通过；validate-task PASS、validate-docs PASS〔20 units/12 trace/189 task files〕；ird_gates 引擎直跑 base..head 归一化比对**零新增零消除**——附件 traceability/builds/wp15-t04/ird-gates-base-head-compare.md，三套计数 base/head 均为直方图 145 行、去重条目 69、引擎自计 69）。两提交拆分：a7580ddd「求解」＋本提交「过滤去重排序」（卡 §11 规模列）。无语义偏差；P-KIN-7 处置沿 v0.3 口径（policy 会话以 IKinCollisionSession 最小端口＋替身先行，真实④端口组装归 T07）。 |
| v0.5 | 2026-09-26 | WP-15-T05 落位登记（DTB §5.4，批量任务点验证＝`Evidence.hpp/.cpp` 新增＋`Evaluators.hpp/.cpp` 表尾追加＋`DiagCodes.hpp/.cpp` 行 16 表尾追加，验证留痕 traceability/builds/wp15-t05/）：①**Evidence.hpp/.cpp（§3.3 布局表 Evidence.hpp 行 T05 半区）**：批量通道值模型——`BatchRequirementLevel`（Must/Should 两值词表镜像）、`BatchAppliesToScope`（三值词表镜像）、`BatchDemands`、`BatchTaskPoint`/`BatchCondition`（宿主解析投影值——req 对象 schema 不进本单元，R-1；O-37 宿主注入同款纪律）、`BatchItemStatus` 七态、`BatchDemandCheck`（实际/要求/单位四类对比行）、`BatchWorkItemRecord`、`BatchCaseCompletion`、`BatchComputation`、`IBatchCheckpointSink`（批 watermark 最小消费端口——P-KIN-7 处置：检查点写出设施唯一归 execution，本端口只投影"第 N 批已完成"事实，L5 适配）、`EvidenceContext`＋`IKinematicEvidenceBuilder`/`KinematicEvidenceBuilder`（§8.2 表五行产出唯一组装点）＋`encodeBatchPayloadCanonical`（magic `IRDBP01`＋codec u32 v1，§5.6 绑定六要素内嵌）；批量通道键/契约版本常量唯一书写点在 Evidence.hpp（评估键 `kin-task-points-batch` kebab 词形、契约版本 1、载荷 token `kin.task-points-batch.v1` 点形、黄金默认批大小 `kBatchDefaultBatchSize`=256——D-KIN-6）。②**评估器（Evaluators.hpp/.cpp 表尾追加）**：`makeTaskPointsBatchDescriptor`（§4.3 行七条 Required 依赖＝task-point-ik 六条＋req.conditions；supportedModes={Quick,Verified}；stateless=true＋threadSafety=FullyThreadSafe——§9.2 头注）、`TaskPointsBatchEvaluator`/`TaskPointsBatchEvaluatorFactory` 同构宿主注入工厂、`taskPointsBatchCapability()`（§8.3/§12 交接——execution::TaskCapability：supportsPause=false 如实声明、checkpointGranularity=Batch、forceTerminateCost=Cheap；经 execution::EvaluatorRuntimeCapabilities 注册归 L5——P-EX-7 形态）、`runBatchComputation` 自由函数（计算库可直调面——展开/全序/分批/求解/取消/检查点/进度/完成矩阵，NFR-MNT-01）。执行口径（随本行登记）：展开＝启用点集×caseSubset（去重 caseSubset 规范化字典序），appliesTo **仅 Stations 清单外不产项**，停用点/停用工况/appliesTo=None→NotApplicable 显式标记（ERR-01），悬空引用→InputInvalid 工作项＋KIN-POINT-REF-DANGLING（§9.6 行 16——V-12），重复项按集合语义去重；全序＝(pointOid,conditionId) 字典序（§8.4）；预终结项不入批次（终态展开期即定）；分批＝每批 ≤maxBatch（默认 256）——批间协作取消查询（每批至少一次，V-22 本单元侧；ARCH §4.4 的 2 s 停止派发界由批间查询点＋项起点探针承载）、批内并行分片＝全序连续区间（threadCount>1 时 std::thread，各线程只写全序槽位＝合并按全序归并；异常按分片序重抛首错——确定性失败面）、批完成→IBatchCheckpointSink 批 watermark＋reportProgress(批粒度，phase="solve-batch")；取消→未派发可解项如实 NotRun（无伪完成）＋incomplete 标记；完整性自检＝工作项总数＝Σ批项数（已求解＋预终结）＋NotRun 数＋caseSubset 每项有终态标记（评估器与组装器双侧复核，违例 logic_error fail-fast）。要求值对比（§7.1 demandsChecked）：残差两行恒产出（位置 m/姿态 rad，要求＝点容差）、碰撞/裕量行仅声明的要求产出——碰撞 evaluated=false=证据缺失绝不解读为满足（KIN-05）、裕量实际值＝解归一化裕量（D-KIN-6）×评价区间半行程的绝对距离、单位取 arg-min 关节天然单位（rad|m）；要求值合并口径＝碰撞取点∨工况、裕量取可选值较大者。TCP 侧缺失（工具未配置/悬空）＝批量级零素材＋KIN-NO-TCP（T03/T04 两分口径同源；tcpKey 不命中仍装配期 fail-fast）。③**组装器口径（随本行登记）**：逐项证据行 itemId=`kin.task-point-outcome`（表 4 运动学行"每任务点 IK 收敛状态与残差"批量落位行；artifactDigest＝批量载荷整体摘要，行以 digest＋subject=pointOid＋caseScope={conditionId} 溯源——EvidenceItem 无数值字段，值面在载荷；同 itemId 逐项重复为批量通道设计形态，evidence 完备性核对不在此消费）状态映射四计算终态→Satisfied、NotApplicable→NotApplicable＋原因、InputInvalid→Invalid＋KIN-POINT-REF-DANGLING 原因诊断、NotRun→Missing（漏验素材交 evidence 覆盖矩阵②级）；搜索未果聚合＝预算/初值数求和、逐解过滤按全序拼接→output.searchRecord＋`kin.search-exhausted` 汇总行；证明素材＝全序首个 BoundExceeded 项→output.proof，**producer 重绑为批量评估键＋契约版本**（对外交付产生者——validateProof 第③查查证对象；素材内容与绑定身份保持原值）；verdictInputs＝仅 BoundExceeded 项产生违例（itemId=点规范文本；Must→mustViolations、Should→shouldViolations；搜索未果不产生违例——C5/C8）；诊断面收敛＝KIN-RESULT-INCOMPLETE（批量级一条）＋KIN-POINT-REF-DANGLING（逐悬空项）两码，搜索未果素材经记录通道交付不出逐项诊断。④**偏差登记**：§7.1/契约 acceptance 1 的 per-item 状态五值清单增补两值——`BoundExceeded`（结局 5 证明素材的承载态——若归入 NoConvergence/AllFiltered 会把工程不可行素材污染进数据不足，违反三态素材完备语义）与 `NotRun`（§7.1/§9.2 @post 明文要求的取消终态，五值清单未列）——余五值逐字保留；DiagCodesTest 封闭断言 15→16 码随附同步（kSection96FullTable/IsError/CategoryRows 三表＋行序逐位核对）；随本任务触及 DiagCodes.hpp/DiagCodesTest.cpp，**F-380 处置兑现**——三处散文计数「4 error＋11 warning」纠正为真值（T02 面 6 error＋9 warning；T05 增行 16 error 码后全表 7 error＋9 warning）。⑤**测试**：KinTaskPointsBatch 5 新用例（真实求解器三反例结局映射〔V-05/V-06/结局 5 批量侧〕、要求值对比素材四行〔KIN-05 证据缺失口径〕、恒真取消端到端 KIN-RESULT-INCOMPLETE＋NotRun 行、载荷头 §5.6 绑定逐字段解析、None 工况 NotApplicable 完成矩阵口径）＋KinTaskPointsBatchEval 14 新契约用例（descriptor 七条依赖/double 模式/FullyThreadSafe、能力声明三件套、展开三态素材〔appliesTo 过滤/NotApplicable/去重〕、悬空引用 InputInvalid＋行 16 诊断、per-item 状态独立四脚本结局、完成矩阵素材、分批取消 NotRun＋watermark 停推、组装形状＋verdictInputs REQ-06 口径、producer 重绑、批取消探针粒度＋批粒度进度＋watermark 序列、默认批大小 256 两批实例化、注入序无关全序＋payload 逐位一致、线程数 1↔4 等价〔替身线程 id 断言，10 轮无竞态〕、分片连续区间）——**50/50＋34/34 两模式通过并留痕**（gtest XML＋ird-test-report 双份；ctest 2/2；集成构建零错误；冒烟配置/构建通过；validate-task PASS、validate-docs PASS〔20 units/12 trace/189 task files〕；ird_gates 引擎直跑 base..head 归一化比对**零新增零消除**——附件 traceability/builds/wp15-t05/ird-gates-base-head-compare.md，三套计数 base/head 均为直方图 145 行、去重条目 69、引擎自计 69）。knownPitfalls：O-37 沿宿主注入裁决口径（投影值/求解器/检查点通道经工厂闭包注入，create() 无参）；P-KIN-7 沿最小端口＋替身先行（IBatchCheckpointSink＋ScriptedSolver，真实检查点通道/④端口会话组装归 L5/T07）。无其他语义偏差。 |
| v0.6 | 2026-09-26 | WP-15-T06 落位登记（DTB §5.4，区域采样与覆盖率＝`Sampling.hpp/.cpp`＋`Coverage.hpp` 新增＋`Evaluators.hpp/.cpp` 表尾追加，验证留痕 traceability/builds/wp15-t06/）：①**Sampling.hpp/.cpp（§3.3 布局表 Sampling.hpp 行 T06）**：采样通道值模型——`RegionSamplingBudget`（seed/threadCount；seed 入 sampleSetIdentity、threadCount 执行参数不入身份——§8.4）、`RegionBox`/`PositionSamplingDefinition`（Grid counts/Random count——GridBySpacing 已被 requirements 计划构建期规范化为计数，I-REQ-1/D-REQ-2 投影只收计数）/`OrientationSamplingDefinition`（方向数×roll 数，≥1——requirements 字段表）/`SamplingPlan`（宿主解析投影——R-1 禁 include；regionObjectId＋planContentIdentity＋Box{B}＋位置/姿态定义＋BatchDemands 碰撞要求面＋mirrorOf 镜像派生溯源元数据）、`SampleKind`/`SampleState` 五值/`SampleRecord`（sampleIndex 全局序键——分母完整性核查键）/`SampleSet`/`SampleResultRecord`/`SampleResultSet`/`PlanIdentityCheck`/`RegionCoverageComputation`；确定性生成黄金（D-KIN-6，登记随本行、黄金锁定 T13，修改走设计变更）：Grid 体心规则 coord＝min＋(k＋0.5)·size/counts（枚举序 x 最慢 z 最快，index＝(ix·counts1＋iy)·counts2＋iz）、Random＝splitmix64(seed·2³²＋k) 逐轴连续派生（Ik.cpp 初值生成同构）、斐波那契螺旋方向集（z_j＝1−(2j＋1)/N、φ_j＝j·黄金角 2.39996322972865332223 rad 字面锁定）、roll 体心均分 −π＋(r＋0.5)·2π/M、位姿组装 R＝R_dir(d)·Rz(roll)（R_dir＝ẑ→d 最小旋转 Rodrigues 直写，退化 ±ẑ 取恒等/绕 x 半周——黄金锁定）；`generateSampleSet`（多计划全局 sampleIndex 连续——计划注入序×计划内序，计划内先全部位置样本后全部位姿样本 (pi·D＋j)·R＋r 序）/`computeCoverage` 唯一实现点（results↔samples 双射核查＋守恒式自检违例 logic_error）＋`sampleSetIdentity`（evidence §4.1.4 公式实现——SHA-256 over (planContentIdentity ‖ 预算/种子 canonical)；canonical 字节布局＝"IRDSSID"7B〔F-395 纠正——原登记「"IRDSSID1"8B」系把字面量尾数字符误计入 magic 长度，实现实写 magic+7，随 v0.8 批注订正〕＋u32 codec1＋u32 len＋planContentIdentity.toCanonical() ASCII＋u64 seed（LE），登记随本行供 evidence 冻结对齐——O-38/P-KIN-3 冻结前对账面＝快照 SamplingPlanRef 逐字节比对，布局漂移即对账失败显性化，设计上安全；身份对计划参数计算不枚举样本）。两模式可链接纪律：Rotation3D::operator*/默认构造为 rw 外联符号冒烟不可解析——R_dir·Rz 手写三阶矩阵积规避（Ik.hpp identityTransform3D 同款）。②**Coverage.hpp（§3.3 布局表 Coverage.hpp 行 T06）**：`CoverageTotals`（planned/reached/unreachable/dataInsufficient/notRun/notApplicable 六计数——R8 分母恒计划样本总数）/`CoverageResult`（位置存在性/姿态全局双口径＋positionDefined/orientationDefined/downgraded/incomplete 四标记）——**无比率/百分比字段**（"绝不输出 0%/100%"的结构保证——KIN-04 R8；比率推导归报告/汇总层投影）＋`encodeRegionCoveragePayloadCanonical` 声明（实现落 Sampling.cpp；magic `IRDCV01`＋codec u32 v1，标记块 u8×4＋绑定块 T05 同构〔mode u8〕＋计划块{oid 16B＋pid 32B＋重算身份 32B＋matched u8＋分母 u64×2}＋覆盖计数块（双口径六计数）＋逐样本状态表{sampleIndex u64＋oid 16B＋kind u8＋state u8＋outcomeKind presence u8[+u8]＋collisionNotEvaluated u8＋reason 长度前缀}；样本几何不入载荷——样本由 (plan,budget,seed) 确定性再生，evidence §4.1.4 同源）。③**评估器（Evaluators.hpp/.cpp 表尾追加）**：`makeRegionCoverageDescriptor`（§4.3 行九条依赖＝model.robot-design/tcp/req.regions/req.sampling-plans/req.conditions/policy.resolved/namemap/config.ik 八条 Required＋`collision-models` 一条 **Conditional**——条件依赖语义＝碰撞启用状态只读自 policy、策略未启用碰撞时该键不进切片（V13-01），resolutionNote 登记条件语义；supportedModes={Quick,Verified}；stateless=true＋FullyThreadSafe）、`WorkspaceSampler`（§9.2 IWorkspaceSampler 三方法实现）/`WorkspaceSamplerFactory` 宿主注入工厂（create() 无参——O-37）、`runRegionCoverageComputation` 直调面（NFR-MNT-01）、`regionCoverageCapability()`（checkpointGranularity=**Sample**——§8.3"批 watermark（batch 覆盖任务）/样本 watermark（coverage）"原文落值；supportsPause=false、Cheap）。评估主流程（§7.2 覆盖率图落位）：身份对账先于生成（逐计划重算 sampleSetIdentity 与 request.snapshot.samplingPlans 的 SamplingPlanRef 比对——身份＋分母两值；任一不一致→KIN-SAMPLE-IDENTITY-MISMATCH（§9.6 行 13）＋**零素材**输出＝无覆盖率输出，V-15；O-38 冻结前以快照对账为准、绝不沿用未冻结/不一致样本集）→确定性采样→零样本轨（任一计划乘积=0→KIN-COVERAGE-ZERO-SAMPLES（§9.6 行 14）＋整体降级 DataInsufficient；聚合分母存在不恢复该计划覆盖结论——随本行登记）→逐样本评估（样本批分批/协作取消 NotRun 如实保留/IBatchCheckpointSink 样本批 watermark/进度 phase="solve-sample"/并行分片＝样本全序连续区间 T05 同构；IK 接线——位置样本＝位置存在性 IK、姿态无约束落值＝姿态残差容差 π rad〔角度型残差量程 [0,π]，收敛判据逐项故收敛⇔位置入容差——黄金锁定 T13〕、位姿样本＝完整位姿 IK〔查询容差〕；碰撞接线——会话在场随求解硬过滤③、碰撞要求在场而缺会话→该样本 DataInsufficient（KIN-05 语义承接：绝不视为无碰撞；要求合并口径＝计划级∨任一启用工况级，随本行登记；KIN-COLLISION-UNAVAILABLE 产码面归 T07——§9.6 任务列分工，本通道以样本状态＋原因文本承载素材）、无要求且无会话→collisionNotEvaluated 检查不在范围非降级）→computeCoverage→素材组装（payload＋逐计划证据行 itemId=`kin.region-coverage` subject=regionObjectId artifactDigest=载荷摘要＋诊断面〔零样本/不完整 KIN-RESULT-INCOMPLETE〕）。**样本状态映射登记**（§7.2 五值词表落值）：Reached⇐结局 1/4（存在性凭据）、Unreachable⇐结局 5（解析界限确定性证明——唯一确定性不可达路径，C5/C8 搜索类失败绝不入此态）、DataInsufficient⇐结局 2/3 或缺检测器、NotRun⇐取消未派发（partial 不产正式覆盖率——重跑同一样本集）、NotApplicable 词表完备性保留（采样计划无停用语义，生成面不产——覆盖缺口随本行显式登记，F-146 家族口径）。装配期 fail-fast 面：空视图/Box 非退化（I-REQ-6）/姿态计数 0/seed=0（I-KIN-4——采样种子是身份决定输入）/referenceQ 维度/容差阈值/计数 0/tcpKey 不命中。④**测试**：KinRegionCoverage 12 新用例（ACC1 固定计数 100/60 双口径黄金＋存在性/全局口径分离实证〔位置 60/100、位姿 60/200——D=2 方向反例〕、ACC2 数据不足保留分母整体降级〔50/50 守恒〕＋零样本端到端诊断/素材/无比率、ACC3 sampleSetIdentity canonical 字节测试侧独立复算＋对账三变体〔篡改身份/未冻结/篡改分母〕＋matched 对照、ACC4 生成黄金〔Grid 体心/斐波那契螺旋/roll 均分独立参考复算＋Random 同 seed 同异 seed 异〕＋双射核查〔缺项/重复 logic_error〕＋取消 partial NotRun＋watermark 1/20＋重跑同集逐位对齐、ACC5 逐请求接线断言〔位置 π 容差/位姿查询容差/目标位置＝格心〕＋缺检测器短路零求解/无要求对照不降级＋镜像独立计数分母相加）＋KinRegionCoverageEval 9 新契约用例（descriptor 九条依赖含 Conditional＋词形/宿主注入工厂 create 无参/装配期 fail-fast 六反例/无工具零素材轨/对账契约轨零素材/载荷绑定块逐字段解析/确定性 payload 逐位一致/独立入口 generateSamples＋computeCoverage/能力声明 Sample 粒度三件套）——**62/62＋43/43 两模式通过并留痕**（gtest XML＋ird-test-report 双份〔模型 62＝T05 50＋新 12；契约 43＝T05 34＋新 9〕；ctest 2/2；集成构建零错误；冒烟配置/构建通过——冒烟配置注入 CMAKE_PREFIX_PATH=Qt6 msvc2022_64（testkit sdurws_ird_testkit_qt 消费面）；validate-task PASS、validate-docs PASS〔20 units/12 trace/189 task files〕；ird_gates 引擎直跑 base..head 归一化比对**零新增零消除**——附件 traceability/builds/wp15-t06/ird-gates-base-head-compare.md，三套计数 base/head 均为直方图 145 行、去重条目 69、引擎自计 69）。knownPitfalls：O-37 沿宿主注入裁决口径（计划/工况投影值/求解器/检查点/碰撞会话经工厂闭包注入，create() 无参）；O-38/P-KIN-3 沿"冻结前以快照 SamplingPlanRef 对账为准、不一致即 DataInsufficient"（canonical 布局随本行登记供 evidence 冻结对齐）；P-KIN-7 沿最小端口＋替身先行（IBatchCheckpointSink 复用＋ScriptedSolver，真实检查点通道/④端口会话组装归 L5/T07）。**F-394 处置兑现**——随本版触及卡面执行三处散文纠正：v0.3 ⑧句『零新增命中』→『＋1 项 sanctioned 命中（Eigen PRIVATE——O-40）』（F-387 消账）、v0.4 ⑥句『KinIk 15 新用例』→『18 新用例』并补列三用例名（F-390 消账）、v0.5 ⑤句『12 新契约用例』→『14 新契约用例』并拆分枚举（F-393 消账）。无其他语义偏差。 |
| v0.3 | 2026-09-26 | WP-15-T03 落位登记（DTB §5.4，FK 与位姿指标＝`KinTypes.hpp`＋`Fk.hpp/.cpp`＋`Evaluators.hpp/.cpp`，验证留痕 traceability/builds/wp15-t03/）：①**KinTypes.hpp**：`TcpRef`（§5.2 输入行"tool-definition oid＋tcpKey"的值面；解析规则登记——tcpKey 空串＝canonical TCP、非空须与工具 localName 精确相等否则 FrameUnresolved——canonical 面单 TCP 与 KIN-14 默认 TCP 单值同源，多 TCP 工具属 modeling 语义扩展）＋`IKinRuntimeView` 最小消费接口（§14.4 条 6 承载——T03 最小面＝`model()`＋`worldToBase()` 两成员，语义与 runtime::IRuntimeModelView 对应访问器逐字一致、不新增 runtime 语义；gravityBase 随首个重力消费任务表尾追加——位姿指标无重力相关量，§4.5 约束在消费出现时生效；虚接口扩展走表尾追加纪律）。②**Fk.hpp/.cpp**：`PoseMetrics` 全字段（§5.2 输出行）＋`IFkEvaluator`＋无状态实现 `FkEvaluator`＋canonical 载荷编码 `encodePoseMetricsCanonical`（magic `IRDPM01`＋codec v1，定宽小端＋f64 IEEE754 位模式——确定性"同字节输出"的承载面）。**D-KIN-2 实现口径**（随本行登记，黄金锁定 T13）：基础雅可比 6×n 行优先（平移行 m、转动行无量纲、零关节加权）；奇异值＝Eigen JacobiSVD 降序（6×n 取 min(6,n) 个）；条件数＝σmax/σmin、σmin=0 或商非有限→+∞＋`conditionNumberIsFinite=false`（不静默截断）；可操作度 w＝√det(J·Jᵀ) 经 SVD 恒等式实现＝∏σᵢ（n≥6）、n<6 行秩不足 det≡0→w=0（数学等价、免 Gram 行列式舍入负号）；**关节裕量归一化**（D-KIN-6 设计默认，随黄金锁定）＝margin_i＝min(q_i−qmin, qmax−q_i)/(行程/2)（有界关节中点 1/限位 0/超限为负——超限素材交调用方），continuous 按工作范围同式（MDL-12）、无工作范围＝+∞。坐标纪律：tcpInWorld 只经 runtime::composeWorldBaseTcp（M-11/AT-37——本单元零第二套基座变换代数）；权威 q 直消费不叠加 zeroOffset（§5.1）。**两模式可链接纪律**：rw 值类型仅头内 inline 面，PoseMetrics 恒等初值经 runtime detail::identityTransform3D()（Transform3D 默认构造引 Rotation3D::identity 外联符号——冒烟不可链接，runtime 同款规避）。③**Evaluators.hpp/.cpp**：`kin.pose-metrics` 评估器形态（acceptance 4）——`makePoseMetricsDescriptor()`（§4.3 行五条 Required 依赖声明＋全三模式＋stateless=true＋profile{kin,1,零值}——域不申报身份随 evidence §9.5/R-3）＋`PoseMetricsEvaluatorFactory`（宿主注入工厂——O-37 裁决形态落点：构造捕获 (视图,查询) 闭包、create() 无参签名不变、视图非 owning 随请求生命周期）＋`PoseMetricsEvaluator`（委托 FkEvaluator 单一计算点）。**错误分轨登记**（evidence §9.3"域自选"落值）：装配期 fail-fast（std::invalid_argument——空视图/q 维度/q 非有限/tcpKey 不命中＝调用方错误轨，NFR-COR-03）；评估期结构化诊断（KIN-NO-DEVICE/KIN-NO-TCP error 级——经 kinematicsDiagCode 映射取注册码禁拼码，零 payload 零证据——FK 失败≠工程不可行）；取消豁免（§9.2 @取消 行——<1s 内联不轮询 context）。评估键词形偏差：卡面 `kin.pose-metrics` 含点而 evidence 冻结闸门 isValidEvaluationKey 无点（EV-T04 落位；evidence 自家替身先例 kin-batch-ik 同 kebab）——实现键 `kin-pose-metrics`，§4.3 行随本版标注，T04~T07 各键随落位任务同样处理；域载荷 kindToken 不受该闸门约束保留点形 `kin.pose-metrics.v1`。④**Errors 映射两行**（§9.6 分批纪律兑现）：NoDevice→KIN-NO-DEVICE、NoTcp→KIN-NO-TCP（Errors.cpp switch 表尾登记＋Errors.hpp 阶段注同步＋ErrorsTest 映射用例随附同步——T02 预登记"随消费任务登记"的兑现）；IllegalQ/FrameUnresolved 维持 nullopt（无 §9.6 同义码行）。KIN-NEAR-SINGULAR 产生面随 policy 阈值消费面落位（T02 CMake 分工 policy 边消费归 T04/T07）——§9.6 行任务列随本版标注；NoDevice 不可达性论证：CanonicalModelBuilder 禁空链→真实模型不可达，守卫为注入视图面契约防线（§10.2 V-03 的 KIN-NO-DEVICE 分支由该守卫＋评估器映射路径承载）。⑤**O-40 兑现**：Eigen 经 vcpkg 经典模式引入，**实测安装版本 5.0.1**（installed/x64-windows/share/eigen3）；PRIVATE 仅 sdurws_ird_kinematics（消费面收敛 src/Fk.cpp 单 TU，公共头零 Eigen 类型）；引入前集成构建零错误。⑥**O-37 实施前裁决记录**：契约 acceptance 4 括注"evidence §9 增补未登记即注入契约未冻结，任务转 blocked"——实测 O-37 已于 2026-09-22 裁决 closed（governance-log＋DTB §4.2 双登记：宿主注入形态）而 evidence.md §9 文字增补未落卡且无排期任务（evidence 卡最新 v1.3/2026-09-13）；实施会话停止请求编排者裁决，**2026-09-26 编排者裁决放行**（理由链：裁决 closed 即形态已定、卡 P-KIN-2/R-KIN-1 先行路径使 T03 自持、契约编译面已按宿主注入放行；随附义务 a/b/c——本行即义务 a 的登记，义务 b＝findings.json F-383 转登 evidence §9 增补义务无落点无排期，义务 c＝实现严格走自有最小接口＋测试替身宿主注入、零 evidence 侧未落卡类型依赖——已满足）。⑦**§11 T03 行消歧**：原行"＋Bounds 解析素材"按 §3.3 布局表归 T04（T04 契约 note 已登记的落点偏差随本版落卡——Bounds 被五类结局第 5 类消费）。⑧测试：新增 KinFk 15 用例（解析黄金先行批——二连杆零位/π/2 位姿态与雅可比解析、转动＋移动正交列奇异值全解析 σ={√(L²+1),1}、六轴独立参考实现对照〔位姿/雅可比逐元素＋Σσ²=‖J‖_F²＋∏σ=w 恒等式——附录 D 第 9 项独立参考类别〕、dof=0 退化面、裕量归一化、V-02 倒挂安装基座—世界、权威 q 不叠加 zeroOffset 字节级、超限位两分语义、IllegalQ 两分支 params、NoTcp 两分支、确定性字节）＋KinPoseMetricsEval 6 用例（descriptor §4.3 行逐项/键词形闸门/宿主注入工厂与摘要重算/装配期 fail-fast 四反例/NoTcp 结构化素材/跨调用跨模式确定性）；ErrorsTest 映射用例随附同步——**27/27＋11/11 两模式通过并留痕**（gtest XML＋ird-test-report 双份＋构建/运行日志 traceability/builds/wp15-t03/）；ird_gates base..head 归一化比对＋1 项 sanctioned 命中（Eigen PRIVATE——O-40；附件同目录）。⑨F-380 处置说明：其触发条件（下次触及 DiagCodes.hpp/DiagCodesTest.cpp）本任务未发生——T03 零修改该两面，处置留待下次触及会话。无语义偏差；validate-docs PASS（留痕同目录）。 |
...
| v0.7 | 2026-09-27 | WP-15-T07 落位登记（DTB §5.4，接入 policy 碰撞证据并落实 KIN-05 语义＝`Collision.hpp/.cpp` 新增＋`Ik.hpp/.cpp`、`Evaluators.hpp/.cpp`、`Evidence.hpp/.cpp`、`Sampling.hpp` 表尾追加/接线，验证留痕 traceability/builds/wp15-t07/）：①**Collision.hpp/.cpp 新增（§3.3 布局表 Collision.hpp 行 T07；实现 TU 集成模式专属——policy POL-T06 同款 gating `TARGET sdurw_kinematics`，冒烟模式无 TU include Collision.hpp）**：④端口碰撞接入三件——`IKinCollisionSceneSource`（O-37 宿主注入形态的碰撞侧落点：快照 WC 共享只读别名〔CR-04 别名构造——保活编译产物，policy §6.1 只读生命周期第一层〕＋workCellCompileIdentity〔sceneContentIdentity 值传递——policy 不重算〕＋⑥端口名称解析转发〔tryObjectId/tryRuntimeName/nameMapIdentity——只转发 runtime 解析结果，R-4 零拼拆〕）；`assembleCollisionScene`（请求方装配唯一执行点——policy.md §6.1"谁装配"行语义：primaryDevice＝链 robotObjectId；对象清单＝链连杆（RobotLink，hasCollisionGeometry＝collision 引用在场）→工具（Tool，geometry 引用在场）→场景对象（EnvironmentObject，几何必有）三段投影、对象一律 ObjectId；相邻对＝链序相邻＋工具安装对 (链末连杆, tool_i)；错误分轨＝空 WC 别名 fail-fast〔宿主适配器契约违约〕、链空 NoDevice Expected〔环境事实→证据缺失轨〕）；`PolicyCollisionSessionAdapter`（真实 policy 会话→T04 端口适配：每查询直穿会话、零缓存零本地副本〔AT-19/V-25——运行断言＋静态扫描双防线〕；查询仅 SingleState 单构型、无阈值/模式参数〔R-POL-5/D-9〕；仅 Collision 种类发现构成碰撞〔SafetyMarginViolation＝间距不足非碰撞——policy §7.4〕；三态映射＝Completed＋Applicable→Evaluated、EmptyScope/CollisionDisabledByPolicy/Canceled→EvidenceMissing〔非终态/不适用绝不解读为无碰撞——KIN-05〕、Failed→FacilityFailed〔会话诊断 cause 入 statusDetail——该解 DataInsufficient 素材、不中断整批——§8.1 取消/失败行〕；调用上下文恒存活恒无取消——单构型查询原子，取消经 IkRequest.cancellationProbe 在求解循环承载）；`makeCollisionSession` 接线入口（可用性三态：PolicyDisabled〔唯一开关读取源＝policy.collision.enabled——V13-01〕/DetectorUnavailable〔评估器空指针/装配 NoDevice/createSession PolicyError——原因素材入 detail〕/SessionReady；policyContentIdentity 随三态回填——CON-06 观测面）。②**Ik.hpp 表尾追加**：`IkCollisionEvaluationState` 三值＋`IkCollisionVerdict.state/statusDetail`（追加成员带默认值——T03/T04 既有替身聚合初始化零破坏）＋`IkOutcome.collisionEvidenceMissing`（会话在场而构型评价未完成的结局级标记；与 collisionNotEvaluated〔会话不在场〕分工，两者同为 KIN-COLLISION-UNAVAILABLE 素材——两臂同码口径登记，不新增码避免 §9.6 语义漂移）；Ik.cpp 硬过滤③三态轨（Evaluated 参与过滤/放行；缺失态解保留 collisionStatus.evaluated=false 素材、不中断其余候选）。③**诊断产码面落位（§9.6 行 8/9——T07 产码义务收口）**：KIN-COLLISION-FILTERED＝逐 Collision 过滤记录一条（warning，构型级不下结论——C8）；KIN-COLLISION-UNAVAILABLE＝"要求在场＋未评价"聚合诊断（单点评估器＝设施臂〔collisionEvidenceMissing〕；批量组装器＝CollisionFree 检查在场∧collisionNotEvaluated 计数恰一条；覆盖评估器＝样本 collisionNotEvaluated∧原因文本计数恰一条〔"不在范围"样本有标记无原因不入计数——标记面≠缺陷面〕）。④**证据明细交付（§8.2 行 1/acceptance 4）**：`kKinBatchCollisionRowId="kin.collision-verdict"` 证据行（Evidence.hpp 表尾追加）——评价在场→Satisfied＋行内容规范字节摘要（`encodeCollisionVerdictRowCanonical` 公开面：三元组 {subjectPair(ObjectId×ObjectId), configurationRef＝构型签名〔I-KIN-3 记录键〕, verdict} 逐条；条目序＝最佳解在前＋Collision 过滤记录全序在后；缺失态不出条目——KIN-05 不伪造已检）、要求在场而未评价→Missing 行（DataInsufficient 碰撞臂素材）、无要求且未评价→不出行（V13-01 查询面口径）；策略版本入身份（CON-06）＝policy.resolved 依赖键（四 descriptor 在册）＋wiring/session 的 policyContentIdentity 观测面＋快照 policyRef（切片面）——策略变更→切片失效。⑤**测试**：KinCollisionPort 6 新用例（真实 WorkCell＋RobWorkCollisionEvaluator 解析算例——1 DOF 串链＋双 0.2 m 立方体、q=0 相交/q=π 间隙 0.8 m：装配三段投影/相邻对/CR-04 身份逐字段、接线禁用臂＋检测器臂、真实会话判定〔对象对 ObjectId 明细含环境端〕＋同输入重复接线等价会话身份〔AT-19〕＋CON-06 观测面、计数后端 5 适配器查询＝5 底层查询无二次缓存运行断言〔V-25〕、设施异常→FacilityFailed＋原因素材、禁用被误调用→EvidenceMissing fail-safe）＋KinCollisionConsistency 4 新契约用例（R-POL-2 静态扫描零 proximity 痕迹＋消费单点〔policy/CollisionEvaluator.hpp include 与 createSession 仅限 Collision.hpp/.cpp〕、V13-01 结构性检查〔IkRequest/TaskPointIkQuery/BatchQuery/RegionCoverageQuery 无 collisionEnabled/enableCollision 成员——void_t 探测；BatchDemands.collisionFreeRequired 系要求值非开关的语义澄清〕、policy.resolved 四 descriptor 在册＋collision-models Conditional 回归钉住、碰撞两码常量在册）＋KinTaskPointIkEval 2 新（FILTERED 逐记录产码〔恒碰撞脚本→结局 3 确定性形态；"一构型碰撞另一构型有效"双分支素材面由 CollisionFilteredKeepsValidConfiguration_WP15T04_ACC2 求解器级承载——其已经本任务三态轨回归〕、评价缺失 UNAVAILABLE）＋KinTaskPointsBatchEval 1 新（碰撞行 Satisfied/Missing/不出行三态＋UNAVAILABLE 聚合恰 1＋同输入行摘要逐位一致）＋KinRegionCoverage 1 新（设施臂样本 DataInsufficient＋UNAVAILABLE＋无要求对照不产码）——**69/69＋50/50 集成、63/63＋50/50 冒烟两模式通过并留痕**（gtest XML＋ird-test-report 双份〔模型 69＝T06 62＋新 6 CollisionPort＋新 1 覆盖设施臂；契约 50＝T06 43＋新 4 Consistency＋新 2 IK＋新 1 批量〕；ctest 2/2；集成/冒烟构建零错误；validate-docs PASS；ird_gates 引擎直跑 base..head 归一化比对零新增零消除——附件 traceability/builds/wp15-t07/ird-gates-base-head-compare.md，三套计数 base/head 均为直方图 145 行、去重条目 69、引擎自计 69）。knownPitfalls：O-37 沿宿主注入裁决口径（场景事实经 IKinCollisionSceneSource 端口注入，WC 别名构造归宿主/测试适配器）；**P-KIN-7 碰撞半区消账**（真实④端口会话组装随本任务按 POL-T07 落位的 policy 冻结签名消费——T04 预留的"最小端口＋替身先行"过渡形态由 PolicyCollisionSessionAdapter 接替，端口类型保留为域侧消费面）。无其他语义偏差。 |
| v0.8 | 2026-09-27 | WP-15-T08 落位登记（DTB §5.4，会话姿态承载与设默认命令门面＝`Commands.hpp/.cpp` 新增＋DiagCodes 表尾追加行 17＋Sampling.hpp 督办纠正，验证留痕 traceability/builds/wp15-t08/）：①**Commands.hpp/.cpp 新增（§3.3 布局表 Commands.hpp 行 T08；两模式编译——纯 std＋core/runtime 头内模板面）**：a) `KinSessionPose`（L-K4 会话姿态写入口——`setJointConfiguration`/`resetToHome`/`clear`/`isSet`/`jointConfiguration`；结构性零修订零失效零身份耦合〔§4.5/D-KIN-4：类型不携带任何端口/事件面，非任何请求身份字段——V-18 的类型面根基〕；非线程安全仅 UI 线程〔ARCH §7.7〕；非有限分量 fail-fast〔NFR-COR-03〕；复位 Home 值由调用方供给〔ui 从命名位姿集保留键 homeConfiguration 读取——MDL-17/V-27 建模侧同则，本单元不解析位姿集对象〕）。b) `IKinematicsCommandHandler`/`KinematicsCommandHandler`（KIN-14 设默认门面——§9.2 两方法原文签名；执行序＝补丁缝装配核查〔空缝 TCP 路径 fail-fast——装配违约〕→fetchBaseline〔闭包无根→KIN-NO-DEVICE 零提交〕→根身份在场防御→补丁 nullopt→KIN-ROOT-BYTES-ILLEGAL 零提交→组装信封经网关提交回显**原样透传**〔两态不拆包重组——PA-1〕）。c) `setProjectDefaultDevice` 语义登记（**实现口径登记**：R1 数据模型每分支恰一个 robot-design 根＝项目设备本体〔runtime 编译唯一 SerialDevice〕，故"设默认设备"＝经命令重写根对象的**设备指定修订**〔槽字节＝基线根字节原样——零内容变更零引用触碰，KIN-14"经命令产生新修订、不破坏既有引用"字面兑现；robotOid≠根身份＝调用方契约违约 fail-fast〕；多设备目标选择属 R2/MDL-18 对端，R2 交付时随其增量修订本节）。d) `IKinProjectCommandGateway`（①/②端口投影注入面——R-1 禁互链下不 include project/modeling 头，宿主适配器转译真 ProjectCommandService〔O-37 宿主注入纪律同款〕；fetchBaseline 基线事实〔branch/tip/根 oid/根字节透传〕＋submit 信封提交；`CommandSubmission` 两态回显〔Committed/NotCommitted＋诊断透传——细分理由词表不复制，①端口权威语义经宿主装配层直供 ui，NFR-MNT-03〕）。e) `RootDesignPatcher` 补丁缝（RobotDesign canonical 编解码**唯一权威保持 modeling**〔NFR-MNT-03/04——本单元零第二套对象编解码器〕，宿主以 modeling 公共 RobotDesignCodec 实现 decode→写 defaultTcp→encode；闭包存在性校验归 modeling assertDefaultTcp——D-KIN-5"断言归 modeling"）。f) `encodeKinApplyRobotDesignPayload` 载荷框架编码（modeling 载荷 v2 文档化布局兼容：magic "IRDMCP2" 7 字节＋版本 2＋Apply＋单根槽〔allocateNew=0＋oid 规范文本＋token＋根字节透传〕＋removals 空段尾段）＋三常量（`kFacadedModelingCommand`="apply-robot-design"/`kModelingCommandPayloadVersion`=2/`kRobotDesignObjectTypeToken`="robot-design"——第二书写点，契约测试金标钉桩）。②**O-35 处置登记（不适用性——acceptance 4）**：kinematics 无自有领域命令（D-KIN-5）——不设 project commandType，O-35 命令 token 无点形态裁决对本单元无直接约束面；门面提交的是 modeling `apply-robot-design` 无点 token（服从 2026-09-22 已裁决形态）；§9.8 `kinematics.*` 七条仅为 ui CommandId 词表登记（ui 命名空间约定，落位随 WP-15-T12 插件），非 project 命令语法——契约测试 NoOwnDomainCommandSchema 机械面（公共头命令常量封闭集恰门面一常量＋零带引号 "kinematics.*" 字面）。③**P-KIN-5 知会登记**：KIN-14 命令归宿 modeling（处理器/断言/inverse 归 modeling 卡 §9.3；kinematics 组装＋提交——D-KIN-5），本行即对 modeling 所有者的知会登记，登记面数据侧随验收交付（modeling 卡侧增登义务归其所有者）。④**P-KIN-7 处置（命令半区）**：载荷框架字节与三常量以 modeling 卡 v0.15 现行文本（encodeCommandPayload v2 布局/kCommandPayloadVersion=2）为基线，漂移即契约测试独立解码器/金标失败显性化——增量同步义务在案（对端冻结后 diff 同步）。⑤**§9.6 表尾追加行 17**：`KIN-ROOT-BYTES-ILLEGAL`（error——设默认门面根对象字节补丁失败：format-or-version/UserRetry、非比较型非确认型；DiagCodes.hpp/.cpp 表尾追加＋DiagCodesTest 封闭断言扩 17 行〔8 error＋9 warning〕随附同步——清单 16→17）。⑥**F-394/F-400 督办兑现（Sampling.hpp 两站点纠正）**：F-395＝sampleSetIdentity canonical 布局登记由「"IRDSSID1"（8 字节 ASCII magic）」改为 7 字节口径「"IRDSSID"＋u32 codecVersion」〔与 Sampling.cpp 实写 magic+7 对齐〕；F-396＝generateDirections 的 N=1 退化方向注释由 +Z 改 **+X**〔z₀=0→方向 (1,0,0)，与实现公式一致〕。⑦**测试**：KinSessionPose 5 新用例（写读往返与副本语义/复位 Home/非有限 fail-fast/会话操作零网关流量〔V-18 修订计数面〕/会话姿态不入请求身份〔显式输入身份前后逐字段相等＋referenceQ 变化对照——D-KIN-4〕）＋KinCommands 8 新（TCP 组装信封逐项〔token/版本/分支/tip/补丁透传/金标载荷〕＋提交回显、框架编码确定性、补丁失败零修订零提交恰一条行 17 码、无设备根零提交 KIN-NO-DEVICE 两轨、设备指定修订槽字节＝基线原样、外来对象身份 fail-fast、补丁缝装配违约仅 TCP 路径、只读拒绝诊断透传〔L-K11/PA-1——门面不自判可写性〕）＋KinCommandsContract 3 新（O-35 无点 token 同串/语法钉桩、载荷框架独立解码器严格回解〔magic/版本/模式/槽字段/removals 尾段/无尾随字节〕、无自有命令 schema 扫描）——**82/82＋53/53 集成、76/76＋53/53 冒烟两模式通过并留痕**（gtest XML＋ird-test-report 双份；base..head XML 差值＝模型＋13/契约＋3，与 WP15T08 具名用例数一致——base 冒烟 63/50 以临时 base worktree〔1b166a7f，用毕即删〕实测；ctest -L "^ird$" 2/2；集成/冒烟构建零错误；ird_gates 引擎直跑 base..head 归一化比对零新增零消除——附件 traceability/builds/wp15-t08/ird-gates-base-head-compare.md，三套计数 base/head 均为直方图 145 行、归一化去重条目 76、引擎自计 69）。knownPitfalls：O-35 按裁决登记不适用性（②）；P-KIN-5 知会随本行登记（③）；P-KIN-7 命令半区按现行对端文本消费＋金标钉桩（④）。覆盖缺口登记：`CommandSubmission` 仅承载两态回显，①端口细分理由（Rejected 六值/Aborted 三值）不经本单元透传结构表达——真 ui 呈现面消费宿主直供的 CommandResult（§9.2 门面"结果回显"的两态契约口径），该裁剪为 NFR-MNT-03 单一权威取舍，无需求级验收缺口。无其他语义偏差。 |

| v0.9 | 2026-09-27 | WP-15-T09 落位登记（DTB §5.4，结果筛选/导出/批量复算＝`Export.hpp/.cpp`＋`Recompute.hpp/.cpp` 新增＋`SolutionSet.hpp/.cpp` 表尾增列＋测试三文件，验证留痕 traceability/builds/wp15-t09/）：①**SolutionSet 消费面增列（acceptance 1，NFR-MNT-04）**：`usableSolutionPredicate`/`collisionDiagnosticPredicate` 两规范谓词工厂（§6.2"可用解/含碰撞诊断解"的比较集中实现——UI/报告不得各自书写 inCollision/evaluated 比较式）。**语义登记**：可用解＝collisionStatus.evaluated==true 且 inCollision==false（碰撞证据完备且无碰撞——KIN-05 保守读法：KinTypes CollisionStatus 契约"调用方不得把 evaluated=false 且 inCollision=false 当作可行凭据"，未评价解可用性未证不入可用集，呈现侧以"碰撞未评价"提示承载、仍在 sorted() 序中）；含碰撞诊断解＝inCollision==true（生产端不变式上求解器直接产出的视图恒空集——碰撞解移入 filteredRecords；谓词仍集中在此，供"解∪过滤记录"合并诊断呈现与语义演进复用同一比较）。既有 T04 声明零改动（§6.2 四操作接口不变）。②**Export.hpp/.cpp 新增（acceptance 2，KIN-08/AT-04/§7.4）**：`KinExportSource`（archived-run/session-memory 两分）＋`KinExportHeader`（来源/快照/模式——runId 两分纪律违例 invalid_argument fail-fast）＋三值行模型（`KinSolutionExportRow`——rank/snapshotId/pointOid/conditionId/q/指标/碰撞标记/签名逐行携带；`KinFilteredSolutionExportRow`——原因 token〔residual-recheck/joint-limit/collision 三值映射，导出面唯一书写〕＋碰撞对象对 canonical 文本列〔竖线分隔，R-4 无名称拼接〕；`KinCoverageExportRow`——(regionOid, position/pose) 分组六计数 tally，regionOid 首现序×position 前 pose 后、零样本口径不产行〔分母信息归 CoverageResult 聚合面——不在导出面复制覆盖判定语义〕；samples/results 双射违约 logic_error fail-fast）＋`buildKinExportPackage`＋`encodeKinExportJson`/`encodeKinExportCsv`（字段/列定序固定；'#'注释头声明 source/run-id/snapshot-id/mode；浮点＝std::to_chars 最短往返〔确定性唯一表示〕；JSON 转义 RFC 8259/CSV 转义 RFC 4180；非有限浮点 JSON 侧以字符串词形承载）。**写出归 io 通道由插件侧触发**（§7.4/§2.3 不拥有——本交付为纯数据整理面，CSV/JSON 写出器集成随 io 消费面验证）；AT-04"导出不产生修订"为结构保证（类型面零项目/命令/归档依赖）。③**Recompute.hpp/.cpp 新增（acceptance 3，KIN-08/§7.4/CON-02/05）**：`KinRecomputeRequest`（sliceId＝原切片语义锚〔新 run 输入切片由执行管线自当前快照重推导——CON-05 内容寻址：工程未变重推导同 sliceId、已变更新 sliceId，当前性由 evidence 条目级重新判定，请求不携带当前性结论〕＋snapshotId＝调用方提供的当前绑定＋evaluationKey〔同键复算〕＋subjectOid 可选对象引用〔单点复算携带/整闭包为 nullopt〕＋mode/seed/referenceQ 自原身份逐字段携带〔D-KIN-4——构造签名无会话形参，会话残留结构性不可入〕；类型面无 runId/attempt/结果载荷字段——"execution 新 run"结构性保证）＋`buildKinRecomputeRequest`（空键/全零快照锚 invalid_argument fail-fast——§9.1）。提交动作与触发方归 execution/插件编排侧（本单元只保证重提交请求构造正确性 UT——契约 acceptance 3 原文口径）。④**测试**：KinSolutionSetView 5 新用例（可用解谓词正面凭据选择〔KIN-05 保守面——证据缺失解不入可用集〕、诊断谓词选择＋生产端形态恒空面、排序面复用与已排序幂等〔视图排序≡sortSolutions 唯一实现点逐位——V-04 排序面复用断言〕、筛选幂等＋空谓词全保留契约、4 线程×200 轮并发只读全等〔§9.4〕）＋KinExport 8 新（解行身份/引用携带、诊断行原因 token 全表＋对象对文本、覆盖行分组 tally、双射违约 fail-fast×2、文件头两分纪律 fail-fast×2＋合法对照、JSON 来源声明＋确定性＋AT-04 纯面、CSV 注释头/三段表/最短往返词形/RFC 4180 转义、会话内存来源无 run-id 行/字段）＋KinRecompute 4 新（原任务定义逐字段携带、当前快照直传与未变更对照＋subjectOid、确定性＋零残留〔换 referenceQ/换评估键对照〕、空键/全零锚 fail-fast）——**99/99＋53/53 集成、93/93＋53/53 冒烟两模式通过并留痕**（gtest XML＋ird-test-report 双份；base..head XML 差值＝模型＋17〔82→99〕/契约＋0，与 WP15T09 具名用例数一致——base 侧对照取 wp15-t08 留痕 XML〔其任务 head 即本任务 base 0d115514，T08 收尾后 kinematics 零改动〕；冒烟 93＝集成 99−6，差为集成专属 CollisionPortTest 既有 gating 口径）；ctest -L "^ird$" 2/2；集成/冒烟构建零错误（冒烟仅 CommandsTest.cpp 两处 C4834 既有警告——T08 遗留非本任务引入）；ird_gates 引擎直跑 base..head 归一化比对零新增零消除——附件 traceability/builds/wp15-t09/ird-gates-base-head-compare.md（三套计数 base/head 均为直方图 145 行、归一化去重条目 76、引擎自计 69）。validate-task PASS、validate-docs PASS（20 units/12 trace/189 task files）。knownPitfalls：interUnit=false——契约无 knownPitfalls 登记，无未决项。覆盖缺口登记（F-146/F-386/F-389 家族口径）：a) CSV/JSON **写出器**集成（io AtomicFile 消费面）与 kinematics.export-results 命令接线不在本任务——随 io 消费面/WP-15-T12 插件验证（契约 acceptance 2 原文口径）；b) 导出文件头刻意不设生成时间戳/用户等环境字段（确定性优先——NFR-COR-01/02、§3.4 禁读时钟），环境溯源字段如报告层需要归 reporting 域增列；c) 覆盖行不承载 downgraded/incomplete 聚合结论（判定归 evidence 汇总——CoverageResult 聚合面权威，行面只整理事实计数）。无其他语义偏差。 |
