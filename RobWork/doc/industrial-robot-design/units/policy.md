...
//   表尾补登建议值（随实现落位增量登记，P-PR-6 同模式——码值权威归 diagnostics
//     StableCodeRegistry，PA-1）：POLICY-OBJECT-MISSING（POL-T05——④端口存储侧
//     条件：对象缺失/版本未指定不可编址取数）、POLICY-PORT-ASSEMBLY-INCOMPLETE
//     （POL-T05——④端口装配契约违约，Errors.hpp 表尾追加码 PortAssemblyIncomplete
//     的补登建议码）、POLICY-CLL-QUERY-INVALID（POL-T07——评估期查询契约违约，
//     Errors.hpp 表尾追加码 QueryInvalid 的补登建议码；评估期其余 CLL 码——
//     DETECTOR-UNAVAILABLE/-CONTEXT-EXPIRED/-EVALUATION-FAILED/-GEOMETRY-MISSING
//     ——走评估输出诊断轨〔Failed＋DiagnosticRecord，非异常枚举〕，POL-T10
//     Diagnostics.hpp 码表已收编——CollisionQuery.cpp/JointLimits.cpp
//     单点字面量已迁移，v0.11）、
//     POLICY-JNT-TABLE-INVALID/-ENGINEERING-RANGE-INVALID 与 POLICY-CLL-NAME-
//     UNRESOLVED/-EVALUATION-FAILED 评估期轨（POL-T08——关节限位评估输出诊断，
//     迁移通道同上，已收编 v0.11）、POLICY-THRESHOLD-REQUIRED-MISSING（POL-T10
//     ——§5.2 行 5 系值，registryCode 既有而本清单原未列，随码表落位补登）
//   码表实现落位（POL-T10，v0.11）：全表 29 值（清单 23＋表尾补登 6）唯一
//     字面量点＝Diagnostics.hpp policyDiagCode(PolicyDiagCode)（inline
//     constexpr switch 表）；registeredCodes() 返回全表（供注册表核对）；
//     五消费 TU 的历史单点字面量已收敛为编译期码表引用别名（NFR-MNT-03）
...
测试目标 `sdurws_ird_policy_test`（单元内）与 `sdurws_ird_policy_contract_test`（跨单元）。**阶段 A 矩阵用例已全部实现并通过并留痕（POL-T03～T11 逐任务交付；用例→文件落位映射见 `policy/test/README.md` §3，验证留痕见各任务 `traceability/builds/wp07-tnn/`）；任何未执行测试不得标注通过**。测试设施：testkit 断言/夹具（`IRD_TEST_INFO`/`IRD_EXPECT_CLOSE|AT_MOST|IDENTICAL`/`checkDiagnosticRecord`/`checkComparativeFields`/`checkSetEquivalent`/`checkStableOrder`/`DeterministicEnv`）；替身（header-only，`policy/test/sdurws/ird/policy/testdouble/`）：`ScriptedCollisionEvaluator`（按脚本返回预设 findings/Failed/Canceled/异常）、`StubPolicyProvider`——**替身只验证契约与消费方逻辑，其输出不构成任何碰撞算法正确性证明**（边界声明随测试文档留痕，同 evidence EV-REG-3 精神；POL-TD-1 声明全文见 `policy/test/README.md` §1）。真实碰撞数值正确性经内置后端对构造场景（已知相交/分离几何）的解析算例验证（analytic-case 类黄金数据集，WP-02 协同——POL-T11 交付 `testdata/golden/pol-collision-analytic`）。
...
| P-POL-5 | **已消账（2026-09-26，WP-24-T01 基线首版登记）**：NFR-DEP-05 冻结版本基线已产出（`ird/share/baseline.md` v1.0）——backendId=`rw.proximity.builtin-rw`、backendVersion=**`rw-31b8184`**（框架核心锁定 commit 短哈希；原"暂取 RobWork 构建版本 RW_VERSION"临时口径删除——构建宏为配置期动态产物，不满足复现要素跨构建可比）、toleranceModel 登记串尾占位替换为冻结标注。代码单点＝CollisionEvaluator.hpp `kFrozenBuiltinBackendVersion`（字面量唯一处），两处装配调用（Compatibility.cpp／RobWorkCollisionEvaluator.cpp）同常量同源；RobWorkConfig.hpp 包含已随之删除。原登记：碰撞后端版本取值依赖 NFR-DEP-05 冻结版本基线（未产出）；内置 ProximityStrategyRW 版本串暂取 RobWork 构建版本 | NFR-DEP-05、§8.1 | 复现块与兼容判定的版本口径 | ~~WP-24 冻结基线时锁定 `CollisionBackendDescriptor` 取值并回填本文~~（已完成）；后续**变更走设计变更评审**（全体切片身份影响） | 版本基线所有者（WP-24） |
| P-POL-6 | **已消账（2026-09-14，POL-T12 就地翻转；同源登记 P-EV-6 已于 2026-09-13 随 EV-T12 先行翻转）**。原登记（2026-09-10 编写时点，已过时）：project.md 磁盘不完整（§8～§15 缺失）：确认绑定细节、命令编排、升级器不可查证 | 本文 §1.2 实测（与 P-EV-6 同源）；**〔POL-T12 状态同步 2026-09-14〕磁盘复核**：project.md 磁盘完整——§1～§15 章标题全在（实测 1386 行，`__PART3__` 零命中），确认绑定（§6）/命令编排/升级器/任务拆分（§12）均可查证 | 原影响面（策略编辑命令适配器与确认绑定实现细节不可查证）已消除；残余＝§10.4/§13 交接项（行程上限比较型结果消费、策略字节 schema、编辑命令处理器适配器宿主〔P-POL-9〕）的联合复核仍按 phase-one-readiness §2 登记，不因正文齐备自动通过 | 联合复核执行后按增量修订同步；不阻塞本文其余设计 | project 详设所有者 |
...
| P-POL-9 | **已定稿关闭（2026-09-10，development-task-breakdown §4.4 定稿行；2026-09-14 POL-T12 卡内同步翻转——governance-log 册载 closed 在案）**。原登记：策略编辑命令处理器适配器的宿主未定（policy 为 L2 不能实现 project 的 ICommandHandler；UX-08 主 WP-07） | ARCH §7.1/§7.2①、project.md §6.5；定稿依据＝DTB §4.4（"适配器宿主＝ui 单元承载〔L5 应用壳装配期装配注入；policy 仅供纯函数解析/校验/编码〕——WP-10-T07 落地"）＋§2 WP-10-T07/WP-01 行 | （已消除）原策略编辑上线路径不定风险 | 定稿承接：ui 卡产出时冻结该交接（WP-10-T07 实现统一工程策略入口界面与策略编辑适配器） | 构建约定所有者＋ui 详设所有者 |
| P-POL-10 | **会话构建期 Group 目标不可展开（POL-T06 实施发现，2026-09-13 登记）**：§7.2 要求"Role/Group 展开为对象集后比对"与"与场景清单求交"，但组定义数据不在 (policy, scene, names) 任一会话输入内——RawPolicyInput v1/EngineeringPolicySet/PolicyCodec 均无组定义字段（v0.5 ⑥ 登记"组定义数据由装配侧持有"），createSession 签名（§9.3 冻结）亦无组数据参数 | §7.2、§4.3、v0.5 ④⑥；POL-T06 场景校验实测 | 携带 Group 规则的策略可发布（解析期 groupDefined 通过）但会话构建必失败——组规则在 v1 管线内不可用；成员级冲突复核（v0.5 ④"归会话构建同码复核"）对 ×Group 不可执行 | POL-T06 按保守口径执行：Group 目标 → POLICY-CLL-SCENE-INVALID 保守拒绝（不猜测 ARC-04、不静默收窄必检集）；数据通道二选一待裁决——(a) schema v1.1 次版本追加组定义字段（组名＋成员集，入内容身份——§7.2"组名与成员进入策略身份"），或 (b) 维持装配侧持有并由所有者确认"v1 管线不产出 Group 规则"的口径 | 需求所有者＋本文所有者（schema 变更走 §4.6 版本策略） |
| P-POL-11 | **内置后端距离能力缺口（POL-T07 实施发现，2026-09-13 登记）**：§6.2/§7.4/§6.6 规定最小距离输出（minDistances/measuredClearance）与间距检查（safetyClearance 比较），但 §6.5 冻结的内置后端 ProximityStrategyRW 为二值碰撞策略（实测无距离/容差查询路径）；sdurw_proximity 内无任何具体 DistanceStrategy 实现——距离策略（PQP/FCL/Bullet）位于 sdurw_proximitystrategies，属 §12"不含外部碰撞策略（Yaobi/PQP 等）"排除项且不在 R-5 登记基线（DTB §4.5：math/kinematics/models/proximity 四库） | §6.2/§6.4/§6.5/§6.6/§7.4 对照 RobWork/src 实测（ProximityStrategyRW 单继承 CollisionStrategy；rw/proximity 无距离实现；RW_USE_PQP=ON 但属 rwlibs 排除面）；POL-T07 评估半区实测 | 内置后端下：safetyClearance＞0（发布门强制有值）与 requestMinDistance 的评估不可机械执行——间距检查语义在当前后端下不产出 | POL-T07 按保守口径执行：需要距离的评估 → Failed＋POLICY-CLL-DETECTOR-UNAVAILABLE（§6.3"检测器不可用"既有触发器——KIN-05：缺检测器≠无碰撞，消费方判 DataInsufficient；不静默降级为"仅碰撞"输出、不伪造实测值、不自行实现距离算法〔R-POL-1 精神〕）；m=0（§7.4 合法退化形态）纯碰撞评估不受影响；评估实现以能力探测（后端是否同时实现 DistanceStrategy——RobWork 多接口策略惯用法）承载，WP-24 冻结 NFR-DEP-05 后端基线时换装距离能力后端即自适应、评估语义零变化；裁决项：(a) R-5/后端基线引入距离能力（如 ProximityStrategyPQP——需 §12 排除项与 DTB §4.5 登记册同步修订），或 (b) 维持二值碰撞基线并由需求所有者确认间距检查在阶段 A 的口径 | WP-24（版本基线所有者）＋需求所有者＋本文所有者 |
...
| v0.3 | 2026-09-13 | POL-T02（≙WP-07-T02，分支 wp07-t02）实现落地——`Errors.hpp/.cpp`＋`PolicySet.hpp`（§4 数据模型全量；测试 sdurws_ird_policy_test 增 PolicySetTest.cpp，30 用例双模式通过）。实现细节登记（DTB §5.4，语义零偏差）：①PolicyErrorCode 初表 14 值＝§5.2 错误类 13 码（表序）＋表尾追加 PolicyObjectInvalid（发布对象工厂即时校验——evidence SnapshotIncomplete 同模式；建议码 POLICY-POLICY-OBJECT-INVALID 为补登建议值，P-PR-6 同模式）；§9.6 其余家族（CLL/JNT/VERSION-INCOMPATIBLE 等）留待对应任务表尾追加；②新增 PolicyThresholdDomain 校验域枚举（实现件非字段数据：承载 §4.4"单位由字段位置固定"的构造时校验参数——私有元数据＋domain() 访问器，不入 §5.3 编码）；③PolicyThreshold 采私有构造＋make() 唯一入口＋成员私有无 setter（整体赋值合法——子模型 optional 槽位装配所需，赋值源仅限已校验实例；runtime CanonicalModel 同款封装式不可变）；EngineeringPolicySet 全字段 const（§4.6"无任何修改途径"的最强形态——赋值编译期删除，后续 PolicyParseResult 持有方需以就地构造/emplace 承载，登记消费约束）；④kPolicySchemaVersionCurrent/kNumericContractAnchorDefault/kDefaultFiniteRotationTravelLimit 先行落位 PolicySet.hpp（PolicyInput.hpp 于 POL-T03 落地后复用为唯一事实源——§5.3 PolicySchema 常量的归属不变）；⑤发布门（EngineeringPolicySet::make）执行 §4.5"只产 Valid"＋§5.2 行 5 必填阈值缺省拒绝＋§5.1④ 空域启用拒绝＋阈值域＝字段位置绑定；规则集语义（重复/冲突/循环/对象存在性）未在本层重复实现——权威归解析管线④⑤（POL-T04，PA-1）；⑥O-11/O-18 保守口径执行：JointThresholds 无惯量比/耦合阈值字段（SFINAE 静态钉住），追加通道锚＝schema 代 1（裁决归 selection 卡 WP-19-T01/需求所有者，未私裁） |
| v0.4 | 2026-09-13 | POL-T03（≙WP-07-T03，分支 wp07-t03）实现落地——`PolicyInput.hpp/.cpp`（§3.1 产物行：RawPolicyInput、PolicySchema 常量、PolicyCodec encode/decode/encodeSemanticProjection/contentIdentity；测试 sdurws_ird_policy_test 增 PolicyInputTest.cpp 23 用例，53/53 双模式通过）。实现细节登记（DTB §5.4，语义零偏差）：①**双形态编码**（CR-02 排除字段登记的机械落点）：formFull(0x00)＝对象字节（完整编码 RawPolicyInput——语义闭包原始形态＋显示单位＋policyObject/origin/兼容注记；往返载体，decode 唯一接受）；formSemanticProjection(0x01)＝语义闭包投影（仅 {schemaVersion, collision, jointThresholds, applicability, numericContractAnchor}——CR-02 排除字段 policyObject/origin/校验状态/诊断/兼容注记结构性不存在；contentIdentity 唯一摘要输入，非往返载体——decode 拒绝，evidence baseline-projection 同款先例）；帧格式＝magic "IRDPOL1"(7B)＋形态(1B)＋schemaVersion(4B)＋载荷长度(4B)＋载荷——大端、长度前缀、presence 字节（缺失≠空值≠零）、无填充（§5.3 逐行）；②**编码规范序**（POL-ID-2 机制，codec 自足排序不依赖调用方）：集合字段升序去重（枚举值序/Id128 字节序）；PairRule 无序对按 ScopeTarget 规范键 (kind, 载荷) 摆放（{a,b}≡{b,a}）；规则列表按 (first, second, level, reason) 字典序——列表承载序不进入身份；③**阈值来源标注不入身份**（实现口径，§4.1"来源标注不影响"延伸）：语义投影中 PolicyThreshold 只编码 siValue——PolicyValueOrigin（阈值来源标注）与 PolicyThresholdDomain（字段位置域元数据）均不入编码；依据＝评估行为仅消费数值、数值域由字段位置隐含（v0.3 ②同款）、同值不同来源的评估结果语义相同（CON-05 失效判据正确方向）；PolicyOrigin（对象创建来源）按 CR-02 排除字面执行——测试 ThresholdValueOriginNotInIdentity/ExcludedFieldsPinned_CR_02 钉住；④**RawPolicyInput 保留原文**（§5.2 行 3"原文保留于 RawPolicyInput"）：阈值槽位以 RawThresholdInput{原始值 double（可 NaN/±Inf 驻留）＋显示单位 token（字符串直载，词表核对归解析②）}承载，全部 optional——encode 对非有限值拒绝（§5.3 编码入口＋CR-02 浮点纪律；无效策略不产生修订〔SA-15〕，要求持久化即调用方契约违约 fail-fast ThresholdNonFinite）；显示单位只在 full 形态承载、语义投影无单位槽位（POL-ID-3）；5. **错误码面**：PolicyErrorCode 表尾追加 EncodingInvalid（policy/encoding-invalid，建议码 POLICY-ENCODING-INVALID 补登——§9.2"字节损坏在解码期报错"落点；decode 帧/载荷/枚举/位模式/presence 违约与空 reason 全走此码；版本代违约例外走 SchemaVersionFuture/Unknown），14→15 值，PolicySetTest 全表锚定同步；6. **ODR 修复（既有头缺陷，POL-T03 触发）**：PolicySet.hpp detail::requireScopeTargetWellFormed/requirePairRuleWellFormed 头内定义补 inline 标注——POL-T03 引入第二产品翻译单元（PolicyInput.cpp）后 MSVC LNK2005 多重定义；inline 语义零变化（ODR 合规），两测试 TU 与产品 TU 链接正常；7. **CR-02 摘要单点**：contentIdentity＝全单元唯一摘要调用点（core::ContentDigester 栈上局部实例每线程各持），摘要输入恒为同次调用产出的投影字节（身份与投影不可独立漂移）；浮点原语 canonicalF64/parseCanonicalF64 为 policy 侧唯一浮点进出通道（evidence/runtime 同款双端纪律——CR-02 只约束摘要算法唯一，编码原语按单元本地实现）；mutation 探针自证（contentIdentity 摘要输入截断→DigestViaCoreContentDigesterOnly_CR_02 如期变红→复原复绿） |
| v0.5 | 2026-09-13 | POL-T04（≙WP-07-T04，分支 wp07-t04）实现落地——`PolicyParsing.hpp/.cpp`（§3.1 产物行：resolvePolicy/IPolicyValidator/IPolicyValidationContext；测试 sdurws_ird_policy_test 增 PolicyParsingTest.cpp 8 套件 44 用例，97/97 双模式通过）。实现细节登记（DTB §5.4，语义零偏差；验收对照 §5.1/§5.2 原文）：①**七段管线与发射序**：契约违约复检（fail-fast 不入诊断轨）→版本门→①token→②阈值→③默认告知→④规则→⑤范围/对象→⑥身份（PolicyCodec::contentIdentity——CR-02 发布路径唯一调用）→⑦发布（EngineeringPolicySet::make）；诊断发射序＝段序、段内按承载序（NFR-COR-02）；错误级判定以管线内部 hasError 标志承载（core DiagnosticRecord 无严重级字段——§5.2"码→级"为固定映射：PolicyErrorCode 全表 Error、POLICY-INFO-* Info）；②**"未知字段"落点**（§5.2 行 1）：强类型 RawPolicyInput 的字段集即 schema，未知键在类型层不可表达——POLICY-SCHEMA-UNKNOWN-FIELD 的活性检测面＝**角色 token 词表核对**（五值词表外的拼写错误会静默收窄必检集，语义同"防拼写错误降级为默认值"），字节层等价拒绝由 PolicyCodec::decode 精确耗尽契约承担（EncodingInvalid，v0.4 ⑤）；③**版本门即返回**（§5.2 行 2）：未来/未知代命中后不再解释字段——"不前向猜测解析"为行 2 处置语义，非首错短路（行 2④ 不短路约束针对可共存的输入异常）；诊断为比较型（实际/期望代，单位 1）＋升级指引（PM-06）；④**冲突/重复的展开等价＝解析期可检子集**（§5.2 行 7/8"经 Role/Group 展开的等价对"）：对端覆盖等价＝同 kind 同载荷（Object 同 ID/Role 同 token/Group 同名，精确等值）＋Object×Role 经 context.objectRole 命中同角色类；**Object/Role×Group 的成员级交集需场景清单（解析期上下文无组成员查询）→ 归会话构建同码复核**（§7.2 首框"与场景清单求交"＋POL-SCOPE-2——两层防御，解析期不虚报可检能力）；重复判定键＝对端覆盖等价＋同级别（同清单内）；⑤**空域不填默认**（§5.1④）：enabledDomains 空集且 enabled=true → POLICY-APPLICABILITY-INVALID——空集与"显式清空"不可区分，§5.1④ 为解析期权威语义（与 v0.3 ⑤发布门口径一致）；§4.3 字段表"{Self,Environment,Tool} 默认解析"归模板/装配侧预填，非解析管线职责；⑥**RULE-CYCLE 活性面**（行 9）："组引用未定义组"经 context.groupDefined 核对；"组引用组/循环"在扁平组 schema 层不可表达（组定义数据由装配侧持有，RawPolicyInput v1 无组定义字段）——本检查为纵深防御的活性落点；⑦**信息级诊断**仅 POLICY-INFO-DEFAULT-APPLIED（§9.6 唯一已登记 INFO 码，§5.2 行 12）：行程上限未提供→填 4π（kDefaultFiniteRotationTravelLimit 唯一数值源）＋origin=DefaultAppendixD；继承标记在 RawPolicyInput v1 不可表达（project 修订引用语义，非解析期可判定）、兼容注记缺失无已登记码面——均不产出（不发明码）；⑧**sourceVersion 恒 nullopt**：对象字节内容版本归 project 编址（§4.1 两层身份），PolicyParseResult.sourceVersion 由④端口 Provider（POL-T05）按 (policyObject, ContentVersion) 记忆化键填充（PA-1）；⑨**消费约束兑现**（v0.3 ③登记）：PolicyParseResult.policy 以 emplace/聚合初始化承载（EngineeringPolicySet 全 const——optional 赋值算子不可用）；⑩阈值检查序（②）：非有限先于单位换算（行 3 分类保真——NaN 经 core convert 会被误分类为单位错误），单位换算唯一经 core convert（SA-12，CoreError 原文转发行 6），域窗唯一经 PolicyThreshold::make（v0.3 单点，PolicyError 转译比较型诊断、异码重抛不吞错）；验收：POL-PARSE-1~6 全量反例通过（不短路——总反例 7 诊断跨五检查面按段序逐位断言）、acceptance 2 全面（默认仅 4π/nullopt 不适用/RequiredMissing 拒绝/默认与显式同身份），留痕 traceability/builds/wp07-t04/ |
| v0.6 | 2026-09-13 | POL-T05（≙WP-07-T05，分支 wp07-t05）实现落地——`Contexts.hpp`＋`PolicyPort.hpp/.cpp`（§3.1 产物行：三个最小注入接口、PolicyResolutionRequest/PolicyResolution、CollisionBackendDescriptor、IPolicyProvider＋PolicyProvider 实现与 (policyObject, ContentVersion) 记忆化；测试 sdurws_ird_policy_test 增 PolicyPortTest.cpp 3 套件 15 用例，112/112 双模式通过）。实现细节登记（DTB §5.4，语义零偏差；验收对照 §9.1/§3.3/§9.7 原文）：①**错误矩阵六项**（§9.1 错误类型行逐项落点）：全零 policyObject/全零 expectedVersion＝调用方契约违约 fail-fast 抛 PolicyError(PolicyObjectInvalid)（与解析管线复检同款）；expectedVersion 未指定＝不可编址取数（CON-05——端口唯一服务形态，§10.1 调用方先经 project ②端口取得 (oid,cv)）→ 空 policy＋诊断不抛；存储无字节 → POLICY-OBJECT-MISSING 诊断（subject 绑定请求对象）；decode 抛 PolicyError（EncodingInvalid/SchemaVersionFuture/Unknown）→ 转译同码面诊断、what() 全文进 cause（不吞错——"POLICY-SCHEMA-*／存储侧诊断转发"的字节层落点，POL-PARSE-1 处置语义的端口级延续）；内容非法 → 管线全量诊断原样转发（端口诊断集==管线直调诊断集）；②**记忆化**：键＝(policyObject, ContentVersion)（内容编址 CON-05——键即完整输入代理）；成功与失败都入表（键下结果为进程内纯函数值——worker 物化发生在端口服务前，§3.3）；命中直接返回缓存值（§9.1 后置"逐字段相等"平凡成立）；并发未命中允许重复重算、入表先入为准（结果一致）；无淘汰（键空间有界、键下结果恒定）；③**装配口径**（vs §6.5 装配线示意，登记差异）：构造参数＝(bytesSource, validationContext, evaluator, backend)——nameContext **不进**构造参数（本端口三方法契约面均不消费名称映射，createSession(policy, scene, names) 由调用方显式传名 §9.3；持有不消费的接口指针违反最小装配面；§9.1 前置行"nameContext 就绪"指宿主装配环境状态）；validationContext 为 §9.7 已登记注入面（解析④⑤步闭包查询依赖）；evaluator 为 shared_ptr 可空（装配分步——注入前 collisionEvaluator() 抛 PortAssemblyIncomplete）；backend 描述符值注入（推荐 evaluator->backend() 同源——sessionIdentity 组成一致）；④**Errors.hpp 表尾追加 PortAssemblyIncomplete**（15→16 值；§9.1 前置行 fail-fast 载体；token policy/port-assembly-incomplete；建议码 POLICY-PORT-ASSEMBLY-INCOMPLETE 补登——P-PR-6 同模式）；**§9.6 清单表尾补登 POLICY-OBJECT-MISSING**（④端口存储侧条件建议码——码值权威归 diagnostics，PA-1）；PolicySetTest 两张全表锚定用例同步 16 值；⑤**CollisionBackendDescriptor 落位 PolicyPort.hpp**（§3.1 组成表原文归属——非 CollisionQuery.hpp）：三字段 {backendId, backendVersion, toleranceModel}（§8.1 版本要素行取值；toleranceModel＝后端数值分辨率登记非工程阈值 R-7；P-POL-5 取值锁定留待 WP-24）；⑥**ICollisionEvaluator 仅前置声明**（完整定义归 CollisionEvaluator.hpp/POL-T06——④端口接口在 T06 前冻结，业务单元可先面向本端口编程；碰撞半区正向实例同一性断言归 POL-SHARE-1/POL-T11）；⑦CR-04/P-POL-9 处置约束执行（任务契约 acceptance 3）：Contexts.hpp 仅定义三个最小注入接口、零适配器/工厂（适配器归 L5/ui 装配——P-POL-9 宿主＝ui，WP-10-T07 定稿）；CR-04 映射（IPolicyNameContext↔IRuntimeNameResolver：Expected→nullopt；IPolicyBytesSource↔IProjectQueryPort::tryObject 一比一）写入头契约注释；零 runtime/project 编译依赖（BuildRedLineTest NoCrossUnitInclude 白名单 {core, policy} 对新头自动扫描通过）；验收：acceptance 1（注入接口＋端口契约 12 用例）、acceptance 2（POL-ID-1 缓存一致 3 用例）通过，留痕 traceability/builds/wp07-t05/ |
| v0.7 | 2026-09-13 | POL-T06（≙WP-07-T06，分支 wp07-t06）实现落地——`CollisionEvaluator.hpp`＋`src/CollisionEvaluator.cpp`＋`src/RobWorkCollisionEvaluator.cpp`（§3.1 产物行：SceneObjectRole/SceneObjectEntry/CollisionScene、ScopePairStatus/ScopedCollisionPair/ResolvedCollisionScope 作用域展开产物、CollisionEvaluationSession 构建期半区、ICollisionEvaluator＋RobWorkCollisionEvaluator＋makeRobWorkCollisionEvaluator 唯一构造入口；测试 sdurws_ird_policy_test 增 CollisionSessionTest.cpp 5 套件 21 用例，集成 133/133、冒烟 112/112 通过——新测试 TU 消费 rw 非模板类〔WorkCell/FakeDevice/ProximitySetup〕仅集成模式编译，runtime RT-T07 同款 CMake 分工；新公共头零 rw include——rw 类型仅前向声明，冒烟模式无 TU include）。实现细节登记（DTB §5.4，语义零偏差；验收对照 §6.1/§6.4/§7.1/§7.2/§9.3 原文）：①**会话三步构建**（§6.4）：场景校验序固定——发布复检（validationState/双身份）→sceneContentIdentity 非空/workcell 在场/objectId 唯一→primaryDevice 两级解析（名称上下文 nullopt 原样呈现＋findDevice 设备存在性——§7.5 名称不可解析同码）→规则引用 ⊆ objects（Object 清单核对＋Role 词表防御复检＋Group 保守拒绝见③）；展开＝§7.1 矩阵逐行转写（classifyDomainPair：连杆×连杆→Self、连杆×工具/负载→Tool、工具/负载×工件→Tool〔工件归环境侧〕、机器人侧×环境对象/连杆×工件→Environment、环境侧对象间→Scene、工具×负载矩阵未列不入）＋域门控（域禁用 voids 该域全部对含必检——§7.1 不适用条件列；Scene 域仅显式必检进入）＋状态四分（Mandatory/InScopeDefault/ExcludedByRule/ExcludedByAdjacencyDefault——相邻默认过滤仅 Self/Tool 域适用，Environment 行只允许逐对显式排除）；**冲突同码复核**（§7.2 两层防御第二层）：展开集排除∩必检≠∅→POLICY-RULE-CONFLICT（解析期可检子集〔v0.5 ④〕之后的场景装配事实复核——两层事实源不一致在此拦截，POL-SCOPE-2 用例钉住）；检测器初始化＝ProximitySetup 仅由策略规则生成（排除对→EXCLUDE_RULE、必检对→INCLUDE_RULE〔基线过滤层纵深防御〕、域默认必检对无规则；useExcludeStaticPairs=false 显式关闭 RobWork 默认过滤、useIncludeAll=true 显式固定——R-POL-3/POL-EVAL-8 构建期前提）；sessionIdentity＝magic "IRDSESS1"＋form 代＋三内容身份＋后端三串长度前缀（SHA-256 唯一经 core::ContentDigester——PolicyCodec::contentIdentity 的"策略内容身份唯一计算点"登记不变，会话身份为 §4.1 既定另一身份概念）。②**Errors.hpp 表尾追加 SceneInvalid/NameUnresolved**（16→18 值；§9.6 CLL 家族正式落位非补登；token policy/cll-scene-invalid・policy/cll-name-unresolved——§6.1 场景校验四项＋§9.3"场景校验失败→PolicyError"的 fail-fast 载体）；PolicySetTest 两张全表锚定用例同步 18 值。③**Group 目标会话构建期保守拒绝**（POLICY-CLL-SCENE-INVALID，P-POL-10 登记）：组成员数据不在 (policy, scene, names) 任一会话输入内（v0.5 ⑥）——不可核对即拒绝，静默跳过违反"过滤不得隐藏必检"、猜测违反 ARC-04；数据通道二选一待所有者裁决（P-POL-10）。④**展开产物规范序**：ResolvedCollisionScope.pairs 恒按 (objectA, objectB) 字节字典序——场景清单承载序不进入结果序，同语义场景展开逐字节一致（NFR-COR-02；POL-SHARE-1 等价会话基础）。⑤**TU 切分（R-4 门禁误报的结构性隔断，评审对照点）**：§9.3 冻结命名（RobWorkCollisionEvaluator/makeRobWorkCollisionEvaluator，含"RobWork"子串）定义于 src/RobWorkCollisionEvaluator.cpp——**零字符串字面量 TU**（源码零 ASCII 引号）；字面量（会话域错误消息＋复现要素冻结值 detail::makeBuiltinBackendDescriptor 单点）留在 CollisionEvaluator.cpp（不含 RobWork 标识符）。动机：IRD-GATE-R4 启发式以"跨引号对的 RobWork 子串"疑似名称拼接/剥离，冻结命名与字面量同 TU 时跨行误配命中（实测同 TU 形态 6 命中→切分后 5 命中＝base 既有）；本单元无任何名称拼接/剥离行为（P-POL-8 措辞登记），例外登记册（ird_gates_whitelist.cmake）归 WP-01、policy 例外待 O-12 裁决——裁决前不私改门禁、以 TU 切分隔断误报面；裁决后如需可回并。⑥**复现要素取值**：backendId="rw.proximity.builtin-rw"（§6.5 冻结）；backendVersion=RW_VERSION（RobWork 构建版本——P-POL-5"暂取构建版本"口径，经 RobWorkConfig.hpp 于零字面量 TU 取值）；toleranceModel=登记串（复现要素非工程阈值——R-7）。⑦**构建期/评估期边界**：evaluate 与查询类型（§6.2/§6.3）不在本任务声明（§12 POL-T07 行产物）——会话仅含构建期访问器（policy/sessionIdentity/sceneIdentity/nameMapIdentity/backendDescriptor/scope/proximitySetup/collisionEnabled），无占位无空实现；strategy 由会话共享持有保活（§6.4"内置后端实例"——检测器两半生命周期绑定）；collisionEnabled=false 会话可构建（§9.3 前置行只要求已发布＋场景校验）、作用域为空（禁用被误调用的评估期应答归 POL-T07）。验收：acceptance 1（POL-SCOPE-1/2 构建期：域区分 3 用例＋过滤留痕/必检相邻存活/解析期冲突回归/场景展开冲突复核 4 用例）、acceptance 2（CR-04 映射：场景校验守卫 8 用例＋会话身份确定性 3 用例＋唯一入口/复现要素 1 用例＋角色词表 1 用例；替身先行——TestNameContext/TestValidationContext/FakeDevice，POL-TD-1 边界声明随测试文件头）通过；门禁：validate-task PASS；ird_gates 5 命中＝base 既有（POL-T04 验收记录逐码相同：SUB×4＋T1×1，无一引用 policy 面——GATE_DELTA_ZERO）；留痕 traceability/builds/wp07-t06/ |
| v0.8 | 2026-09-13 | POL-T07（≙WP-07-T07，分支 wp07-t07）实现落地——`CollisionQuery.hpp`＋`src/CollisionQuery.cpp`（§3.1/§12 POL-T07 产物行：查询与输出类型＋RobWork 适配评估——SingleState/PathSequence/SampleSet、取消、异常捕获；测试 sdurws_ird_policy_test 增 CollisionEvaluationTest.cpp 2 套件 17 用例，集成 150/150、冒烟 112/112 通过——真实后端解析算例装置〔1 DOF RevoluteJoint 串链＋0.2 m 立方体 RigidObject 几何，q=0 相交/q≈π 间隙 0.8 m 解析已知答案〕＋脚本化碰撞/距离后端与上下文替身，POL-TD-1 边界声明随文件头）。实现细节登记（DTB §5.4）：①**查询与输出类型**（§6.2 逐字段原文）：六成员查询载体（**无阈值/模式成员**——R-POL-5/D-9 结构防覆盖，测试以六成员结构化绑定钉住：新增成员即编译失败）；十字段发现＋PairCoverageRecord/SampleDistanceSummary/十一字段评估结果（§8.3"无工程结论字段"——POL-EVAL-1 以结构化绑定复核成员数）；全部类型逐字段 operator==（POL-EVAL-7 断言基础）；头含 rw/math/Q＋rw/kinematics/State（值成员需完整类型——runtime Snapshot.hpp 同款 include 纪律，冒烟模式无 TU include）；②**评估半区装配**（buildEvaluationHalf——构造末段调用）：主链设备固化→作用域对象固化（名称解析→findFrame 定位〔§7.5 断链→NameUnresolved〕→几何注册：声明有几何的对象在编译产物 Object 清单按基帧匹配注册进注入后端＋hasModel 幂等核对；声明与产物不一致→评估期 coverage 缺口＋POLICY-CLL-GEOMETRY-MISSING 告知性诊断——不伪装已检、不 fail-fast，§7.5 几何缺失行）→作用域对固化（级别解析：必检规则 authored 级别/域默认 Must——§6.2 level 语义）→距离能力探测（见⑤）；③**evaluate 执行序**（§6.3/§6.6 固定序）：查询契约校验（kind↔样本数＋构型维度↔设备自由度——违约 PolicyError(QueryInvalid) fail-fast）→迟到调用拒绝（alive()=false→Failed＋POLICY-CLL-CONTEXT-EXPIRED——POL-LATE-1）→适用性（禁用→Completed＋CollisionDisabledByPolicy；空作用域→Completed＋EmptyScope——均非"无碰撞"，coverage 表达）→距离能力核对（见④）→样本循环（基准态克隆〔缺省＝WorkCell 默认态〕→setQ→逐对二值碰撞；非碰撞对距离查询〔m＞0 或 requestMinDistance 时〕；**§7.4 边界值决策表逐行**——d＞m/d＝m 清晰〔边界含于安全侧 D-08〕、0≤d＜m MarginViolation（measuredClearance=d）、后端相交＝Collision（不查距离〔二分互斥的机械证明〕、penetrationDepth 显式空不伪造）、d 为 NaN/±Inf→评估级 Failed＋POLICY-CLL-EVALUATION-FAILED（原文 %.17g 保留、不入 findings）、m＝0 退化仅碰撞；stopAtFirstFinding 首发现即断＋coverage 如实；样本边界 cancellationRequested→Canceled＋部分 findings＋零错误诊断〔UX-03〕；框架异常捕获〔rw::common::Exception/std::exception/...三轨〕→评估级 Failed＋诊断，不吞不崩）→稳定排序（findings 按 (sampleIndex, 对象对字典序, kind)、minDistances 按样本序、appliedFilters 按对象对字典序——生成序与键序同向＋stable_sort 契约兜底）→Completed/finalized=true；④**距离能力口径（P-POL-11 新登记，§15.3）**：内置后端 ProximityStrategyRW 为二值碰撞策略（实测无距离/容差查询路径；sdurw_proximity 无具体 DistanceStrategy——距离策略属 §12"不含外部碰撞策略"排除项且不在 R-5 登记基线）——需要距离的评估→Failed＋POLICY-CLL-DETECTOR-UNAVAILABLE（§6.3"检测器不可用"既有触发器，KIN-05 缺检测器≠无碰撞——不静默降级、不伪造、不自行实现距离算法）；m=0 纯碰撞评估不受影响；能力探测对 WP-24 后端基线冻结自适应（换装距离能力后端零语义变化）；⑤**并发只读的执行侧机制**：RobWork 检测器查询面非线程安全（ProximityStrategyRW 统计计数器在 doInCollision 内非原子累加——实测源码），会话构造签名表尾追加 backendQueryMutex（缺省自建——POL-T06 直构调用源兼容）＋RobWorkCollisionEvaluator 持评估器级共享互斥——同一后端实例的全部访问（构建期注册＋评估期查询）串行化，§9.3"并发只读可重入"组合成立（POL-CONC-1 四线程执行证明；锁不进入输出——§6.4 evaluate 纯度）；碰撞与距离分持两只 ProximityStrategyData（缓存槽单所有者）；⑥**Errors.hpp 表尾追加 QueryInvalid**（18→19 值；policy/cll-query-invalid；建议码 POLICY-CLL-QUERY-INVALID 补登——§9.3 错误类型行"调用方违约（样本数/kind 不符）→PolicyError"评估半区载体）；评估期其余 CLL 码走评估输出诊断轨（DiagnosticRecord 单点字面量——POL-T10 落位后迁入码表，§9.6 补登注记同步）；PolicySetTest 两张全表锚定 19 值；⑦**R4 字面量纪律延续（v0.7 ⑤同动机）**：评估 TU 初版诊断文案含框架品牌子串触发 IRD-GATE-R4 跨引号对启发式 1 命中（6 命中＝base 5＋新增 1）——文案改写归零（终态 5 命中＝base 既有）；⑧**POL-T06 夹具适配**：CollisionSessionTest 场景增补对象 Frame 登记（addObjectFrames）——评估半区装配要求作用域对象解析名可定位编译产物 Frame（§7.5 会话契约收紧的设计内后果；POL-T06 用例语义不变）。验收：acceptance 1（POL-EVAL-1~9/POL-EXC-1/POL-CONC-1 状态机/稳定排序/边界值决策表——17 用例逐条执行）、acceptance 2（API 无阈值/模式参数——D-9 结构断言＋POL-EVAL-8② 模式标签不变性）通过；门禁：validate-task PASS；ird_gates 5 命中＝base 既有（与 POL-T06 验收基线逐码一致——GATE_DELTA_ZERO；初版 R4 新增命中已任务内修复）；留痕 traceability/builds/wp07-t07/ |
| v0.9 | 2026-09-14 | POL-T08（≙WP-07-T08，分支 wp07-t08）实现落地——`JointLimits.hpp/.cpp`（§12 POL-T08 产物行：§7.4/§9.4 关节限位与行程阈值评估——JointLimitSpec/JointLimitQuery/JointLimitFinding/JointMarginRecord/JointLimitEvaluation/IJointLimitEvaluator＋唯一产品实现 JointLimitEvaluator 与 makeJointLimitEvaluator；测试 sdurws_ird_policy_test 增 JointLimitsTest.cpp 1 套件 16 用例，集成 166/166、冒烟 112/112 通过——评估本体纯标量零框架调用，rw include 面仅经 JointLimits.hpp→CollisionQuery.hpp 的 CollisionEvaluationStatus 词表复用链传导〔§9.4"复用状态词表"原文〕，故 src/test TU 集成模式专属 gating〔POL-T06/T07 同款；冒烟模式无任何 TU include 新头〕）。实现细节登记（DTB §5.4，语义零偏差；验收对照 §7.4/§9.4 原文）：①**§9.4 冻结面零截断**：JointLimitSpec 五字段/JointLimitFinding 七字段（含比较型三要素 actualValue/thresholdValue——UX-03/ERR-01）/JointLimitEvaluation 五成员/IJointLimitEvaluator::evaluate 签名逐字落地；各 kind 的比较口径实现冻结——IntervalInvalid: actual=qMin/expected=qMax〔rad〕、EngineeringRangeInvalid: actual=first/expected=second〔rad〕、TravelLimitExceeded: actual=T=|qmax−qmin|/expected=L〔rad〕、NearLimit: actual=r/expected=阈值〔无量纲〕、LimitViolated: actual=q/expected=被越过的限位端〔rad〕；级别固定口径：NearLimit→Should（§7.4"Should 级默认"）、其余四类→Must；②**JointLimitQuery/JointMarginRecord 实现形态**（§9.4 仅冻结 evaluate 签名与 Spec/Finding/Result，查询与裕量记录成员清单为实现落位）：Query＝{joints, configurations（逐构型 rad，内层与 joints 等长同序）}——无阈值/开关成员（KIN-13/R-POL-5 同源口径：阈值唯一来自 evaluate 传入的 EngineeringPolicySet）；MarginRecord＝{定位三元组, sampleIndex, marginToNearestLimit, nearLimitRatioValue}——裕量四态经 core SourcedValue 承载（§9.6"NotApplicable 输出经 SourcedValue::notApplicable 语义"原文）：有限限位且有效→Provided（derived-readonly 溯源）、continuous/未声明限位→NotApplicable 显式标记、被跳过非法关节→整个记录缺席（"跳过该关节（不伪造裕量）"原文）；③**notApplicableChecks 实现增量**（acceptance 3 执行载体，§9.4 五成员清单的唯一扩展项）：JointCheckNotApplicable{Check∈{NearLimitRatio, ConditionNumberWarning, TravelLimit}, cause}——nearLimitRatio/conditionNumberWarning 未设置（P-POL-2/O-10：nullopt＝无冻结默认，不发明数值）与 travelLimitCheckEnabled=false 各输出显式标记；"无该类发现"由此不可被读作"检查通过"；④**执行序七步**（固定——确定性 NFR-COR-02）：policy 发布态复检（PolicyObjectInvalid fail-fast——与会话侧同码）→查询契约校验（PolicyError(QueryInvalid) fail-fast：空表/空构型/全零或重复身份/单侧限位/continuous 缺必填范围/维度不符）→构型位置有限性（任一非有限→Failed＋POLICY-CLL-EVALUATION-FAILED、原文 %.17g 保留、finalized=false——§7.5 不伪造不入 findings）→名称解析（规范序；nullopt→Failed＋POLICY-CLL-NAME-UNRESOLVED——ARC-04 不猜测）→不适用标记预填（Check 枚举序）→逐关节检查（规范序＝jointObject 字典序，输出与装配顺序无关）→稳定排序＋终态化；⑤**D-13 诊断级不阻断（acceptance 2）**：区间非法（qmin≥qmax——NaN 端点经"含 NaN 的＜比较恒 false"自然落入）与工程范围非法（min≥max/端点非有限——MDL-12 有限性）均检出为 JointLimitFinding＋伴随稳定诊断（POLICY-JNT-TABLE-INVALID/POLICY-JNT-ENGINEERING-RANGE-INVALID，比较型三要素齐备、非有限端点经 SourcedValue Invalid 态保留原文）并**跳过该关节**，评估状态仍 Completed＋finalized=true——硬断言执行权归 modeling/project 处理器，policy 只供给比较事实（不代替放行/阻止）；⑥**"必填"与"有限性"两分**（JointLimitSpec 注释契约的落位口径）：continuous 缺 engineeringRange＝装配契约违约 fail-fast（QueryInvalid）；已提供但非法＝诊断级发现（§9.4 边界段"工程工作范围有限性……检出为诊断级发现"原文——非有限端点 fail-fast 将越权接管处理器断言权，故不取 fail-fast）；同口径：qMin/qMax 非有限无独立断言（设计仅要求 qmin＜qmax——§7.3 硬约束行），NaN 端点自然落入区间非法、±Inf 端点产生 T=Inf 的行程超限事实（设计一致）；未声明限位的有限关节（§9.4"同时有/无"合法形态）＝全部限位类检查不适用、裕量 NotApplicable；⑦**行程/近限位/违例边界语义**（§7.4 逐行）：T＞L 才超限（T＝L 不超限——D-08 边界含于合规侧）、r＜阈值才警告（r＝阈值不警告）、q ∈ [qmin,qmax] 闭区间端点不违例；越限样本裕量负有符号保号（违例深度事实）；NearLimit 仅在区间内评估（margin≥0）；每关节每 kind 至多一条发现（§9.4 稳定排序键"对象字典序×kind"的结构推论），LimitViolated/NearLimit 的 actualValue 取首个命中样本（样本序升序——确定性约定），逐样本明细由 margins 承载；margins 排序＝(sampleIndex, jointObject)（实现冻结口径）；⑧**诊断码单点**：POLICY-JNT-TABLE-INVALID/-ENGINEERING-RANGE-INVALID＋POLICY-CLL-NAME-UNRESOLVED/-EVALUATION-FAILED 四值为 JointLimits.cpp 单点字面量（评估输出诊断轨非异常枚举——POL-T10 落位后迁码表；§9.6 补登注记同步）；PolicyErrorCode 枚举零变化（表尾追加通道未动用）；⑨验收：acceptance 1（POL-JNT-1/2：4π 边界/6π 超限三要素逐位精确/continuous 豁免/6π 显式阈值多圈反例〔AT-01〕/近限位严格小于/闭区间违例/裕量公式与排序——16 用例逐条执行）、acceptance 2（IntervalInvalid/EngineeringRangeInvalid 诊断级不阻断两用例）、acceptance 3（双阈值 nullopt→显式 NotApplicable 标记＋开关关闭标记——消账归需求所有者）通过；门禁：validate-task PASS；ird_gates 5 命中＝base 既有（与 POL-T06/T07 验收基线逐码一致——GATE_DELTA_ZERO，gate-all.log 零 JointLimits 相关条目）；留痕 traceability/builds/wp07-t08/ |

| v0.10 | 2026-09-14 | POL-T09（≙WP-07-T09，分支 wp07-t09）实现落地——`Compatibility.hpp/.cpp`（§3.1 组成表五实体逐字承载：PolicyConsumerRequirements/PolicyIncompatibilityReason/PolicyCompatibility/checkPolicyCompatibility/IPolicyCompatibilityChecker；§12 POL-T09 产物行；测试 sdurws_ird_policy_test 增 CompatibilityTest.cpp 5 套件 15 用例，集成 181/181、冒烟 112/112 通过——backend 真值经唯一装配线 makeRobWorkCollisionEvaluator().backend() 取得，测试 TU 消费框架库仅集成模式编译〔与 src/Compatibility.cpp 同因同 gating〕；头内类型面零框架依赖，冒烟测试面保持 112 用例）。实现细节登记（DTB §5.4，语义与 §8.1/§8.2/§9.5 零偏差；细节口径见下）：①**检查序与全量收集**：七项固定序＝枚举声明序（policy-invalid 防御臂→schema-future→schema-obsolete→collision-required-disabled→backend-mismatch→numeric-contract-mismatch→mode-not-applicable），全部执行、不首错短路——同失配必得同一清单（NFR-COR-02；evidence CacheHitReasons C-1 同款纪律）；verdict＝reasons 非空即 Incompatible；reasons 与 diagnostics 每因一诊、下标一一对应。②**比较型三要素口径（UX-03 数值判定原文）**：schema 两臂为数值比较——comparison 携带实际/期望代（无量纲单位 "1"，Provided 态，DerivedReadOnly＋policy-compat 来源标记）；后端描述符与数值契约锚为**字符串**比较——core ComparativeValue 仅承载数值＋已注册单位，字符串侧以 cause 原文逐字承载（ERR-01 原因要素）、不伪造数值、不滥用 SourcedValue 四态（NotApplicable/Invalid 语义均不符）——本口径为 UX-03"比较型校验（限位、容差、裕量、预算等数值判定）"的机械执行，POL-COMPAT-1 三要素观测点由 schema 臂三要素断言＋字符串臂 cause 逐字断言共同覆盖。③**policy-invalid 防御臂**：EngineeringPolicySet::make 发布门只产 Valid 实例（§4.5）——公共 API 下该臂类型层不可达，无运行期用例（设计事实登记非覆盖缺口）；保留以兑现 §8.1 reason 词表并为未来非门构造路径提供稳定码面；token 由词表全表用例钉住（七值 kebab 逐一精确）。④**backend-mismatch 核对对象（§8.1"缺省=任意已注册后端"的实现语义）**：当前进程唯一注册后端＝运行构建的内置后端（§6.5"内置 ProximityStrategyRW 为默认且唯一注册后端"——ARC-05 不引入第二碰撞算法；§8.1 取值来源列"policy（构建期冻结）"），复现要素经字面量单点 detail::makeBuiltinBackendDescriptor＋版本宏 RW_VERSION 注入（P-POL-5/O-20 保守口径——**本单元不私定最终取值**，NFR-DEP-05 基线冻结后由 WP-24-T01 锁定回填，判定逻辑零变化）；O20Ppol5 用例以"要求侧携带装配线真值→兼容／版本串变异→失配"机械证明判定与装配线同宏同单点（无第二取值源）；诊断 cause 逐字携带双侧三串（id/version/tolerance）。⑤**TU 与门禁纪律（§15.4 v0.7 ⑤同源）**：src/Compatibility.cpp 持有诊断文案字面量并含框架配置头包含（版本宏为纯版本串、尖括号包含不在双引号对内——IRD-GATE-R4 启发式扫描面零新增；本 TU 自身文本无任何双引号对携带的框架品牌子串）；ird_gates 实测 5 命中＝base 既有（SUB×4＋T1×1，与 POL-T06/T07/T08 验收基线逐码一致——GATE_DELTA_ZERO）。⑥**POLICY-VERSION-INCOMPATIBLE 单点字面量**（src/Compatibility.cpp TU 顶部；POL-T10 Diagnostics.hpp 落位后迁入码表——JointLimits.cpp/CollisionQuery.cpp 同款迁移通道；码值权威归 diagnostics StableCodeRegistry，PA-1）。⑦**POL-COMPAT-2 本侧面**（CON-05/06）：阈值变化策略＝新内容身份（切片键素材——身份↔阈值绑定归 POL-T03 POL-ID-4，本套件消费"身份不同"素材事实）；兼容判定本身不按身份拒绝复用（越权核对＝替 evidence 行使缓存判定权，PA-1；拒复用由 evidence judgeCacheHit 在切片层执行——跨单元契约测试联动归其侧），判定仅供身份＋原因素材（stale 请求的原因清单全量＋诊断）。验收：acceptance 1（POL-COMPAT-1/2 版本/后端/锚失配全量 reason——15 用例：词表全表/正例零误报/schema 双臂三要素/闭区间边界/碰撞开关双态/后端三字段变异＋精确命中/锚失配＋精确命中/模式三态/全量五臂按序收集/确定性/接口委托）、acceptance 2（O-20/P-POL-5 保守口径——同源正例＋失配反例＋冻结标识断言）通过；门禁：validate-task PASS；ird_gates 5 命中＝base 既有（GATE_DELTA_ZERO）；留痕 traceability/builds/wp07-t09/ |
| v0.11 | 2026-09-14 | POL-T10（≙WP-07-T10，分支 wp07-t10）实现落地——`Diagnostics.hpp/.cpp`（§3.1 组成表三实体：PolicyDiagCode 建议码全表枚举＋policyDiagCode/policyDiagCodes 码表＋IPolicyDiagnostics＋PolicyDiagnostics 产品实现＋makeComparative 辅助组〔makeComparativeValue/makeNotApplicableValue/makeComparative〕；§12 POL-T10 产物行；测试 sdurws_ird_policy_test 增 DiagnosticsTest.cpp 4 套件 19 用例，集成 200/200、冒烟 131/131 通过——DiagnosticsTest 纯 core＋testkit 依赖双模式编译）。实现细节登记（DTB §5.4，语义与 §9.6 零偏差）：①**码表全集 29 值**＝§9.6 清单 23＋表尾补登 6（OBJECT-MISSING/PORT-ASSEMBLY-INCOMPLETE/CLL-QUERY-INVALID 三值清单已登记＋POLICY-POLICY-OBJECT-INVALID/POLICY-ENCODING-INVALID 两值随 v0.3/v0.4 registryCode 落位入表＋POLICY-THRESHOLD-REQUIRED-MISSING 本任务补登——§5.2 行 5 系值，registryCode 既有而 §9.6 清单原未列，P-PR-6 同模式随码表落位登记）；唯一字面量点＝policyDiagCode 的 inline constexpr switch 表（定义于公共头——迁移 TU 编译期别名与 Errors.cpp 委托所需的可见性；NFR-MNT-03 单一权威不因载体 .hpp/.cpp 而变化）；全表无 default 全枚举 switch＋全表数组＋测试全表锚定三重防漂移；②**评估期字面量迁移兑现**（§9.6 迁移通道注记）：CollisionQuery.cpp（CLL 四值）与 JointLimits.cpp（JNT 两值＋CLL-NAME-UNRESOLVED/CLL-EVALUATION-FAILED）单点字面量按 v0.8 ⑥/v0.9 ⑧登记迁入码表，另将同模式的 PolicyParsing.cpp（POLICY-INFO-DEFAULT-APPLIED）/PolicyPort.cpp（POLICY-OBJECT-MISSING）/Compatibility.cpp（POLICY-VERSION-INCOMPATIBLE，v0.10 ⑥）一并收敛——各 TU 常量改为 `constexpr std::string_view kXxx = policyDiagCode(PolicyDiagCode::Xxx)` 编译期码表引用别名（使用点零改动、字符串零变化——既有全量回归 200/200 钉住）；③**make() 四步核对**（§9.6"构造遵循 core DiagData 不变量 C-1~C-3"）：码表成员核对（表外码→PolicyError(PolicyObjectInvalid) fail-fast——"工厂即时校验拒绝非法实例"语义，radUnit() 内部不变量违约同款先例；ARC-04 不发明码）→C-3 必填串前置核对（空串→PolicyError——core 异常类型不向 policy 调用方泄漏，Errors.hpp 转译总纲）→比较型单位有效性核对（UX-03"单位必填"——无效 UnitToken 句柄〔0xFFFF〕拦截在构造入口；数值侧四态不限：Provided/Invalid 原文保留/NotApplicable 显式不适用均为 §9.6 认可语义，语义核对归消费侧）→core::DiagnosticRecord::make 委托（表内码必过 C-3 句法半场；CoreError 防御性转译不吞错）；subject 为 optional 不强制——ERR-01"稳定诊断绑定对象"完整性核对归消费侧（testkit allowTransient 选项＋diagnostics/reporting 边界，core DiagData.hpp 同口径）；④**makeComparative 辅助组**：有限值→Provided＋DerivedReadOnly＋methodTag（DYN-06 事实记录）；非有限→Invalid＋%.17g 原文保留（§7.5/NFR-COR-03——JointLimits/Compatibility 既有文案口径收敛为构造单点）；NotApplicable 保量纲标注（不适用的是数值不是量纲）；⑤**Errors.cpp registryCode 委托**：19 值建议码字面量表改为经 diagCodeOf 映射（全枚举无 default——新增异常码未登记映射编译期告警）委托码表，字符串行为零变化（PolicySetTest 两张全表锚定用例同步复证）；⑥**testkit 链接落位**：sdurws_ird_policy_test 自本任务起链接 sdurws_ird_testkit（acceptance 1"checkDiagnosticRecord 族（testkit）"字面要求——契约校验器真实消费非手工等价断言；CompatibilityTest 等既有套件的手工等价断言保持不动）。⑦**门禁增量登记（如实，不私改）**：ird_gates 实测 6 命中＝base 既有 5（SUB×4＋T1×1，与 POL-T06~T09 基线逐码一致）＋本任务新增 1（SUB：policy_test 直链 testkit）——白名单数据面 cmake/ird_gates_whitelist.cmake T-1/T-2 段注释明文"允许形态：sdurws_ird_<unit>_test → { 同单元产品目标, sdurws_ird_testkit, gtest 系 }"，而引擎 cmake/ird_gates.cmake 将 testkit 分类为 product 形态且形态一无 testkit 豁免——**数据面与引擎实现矛盾**（base 既有 5 命中中 4 个 SUB 同根源：evidence/runtime 测试直链 testkit 同被命中）；两文件均在 WP-01 辖区且不在本任务 allowedFiles，按 v0.7 ⑤"裁决前不私改门禁"纪律保留命中、登记转所有者/WP-01 裁决（引擎补 testkit 豁免分支〔与白名单注释对齐〕或 DTB §4.5 例外登记二选一，裁决权不在实施者）；gate-all 16/17（唯一 FAIL 即上述命中；集成/冒烟 12 项测试发现与执行全 PASS）。⑧**P-PR-6/P-EX-8 处置**（任务契约 knownPitfalls）：本单元只产出建议码（码表 29 值全部为"建议"——正式码值以 diagnostics StableCodeRegistry 注册为准，PA-1 不越权注册）；不实现任何 sink、不裁决 sink 统一（§10.6 文案与注册归 diagnostics）。验收：acceptance 1（checkDiagnosticRecord 族（testkit）码表与构造不变量用例——19 用例逐条执行：码表 5＋构造不变量 9＋辅助 4＋多态 1）通过；门禁：validate-task PASS；ird_gates 6 命中＝base 5＋新增 1（见⑦登记）；留痕 traceability/builds/wp07-t10/ |
| v0.12 | 2026-09-14 | POL-T11（≙WP-07-T11，分支 wp07-t11）实现落地——替身与契约套件（§12 POL-T11 产物行）：①**规范替身**（§11 测试设施行，header-only `test/sdurws/ird/policy/testdouble/PolicyTestDoubles.hpp`——testdoubles 命名空间，runtime/evidence 先例同款）：ScriptedCollisionEvaluator（ICollisionEvaluator 测试实现——backend() 脚本描述符〔backendId 带 test. 前缀＋toleranceModel 标注 not-a-real-backend〕；createSession 以脚本化后端构建**真实会话**——作用域展开/冲突复核/状态机/稳定排序全部走产品代码、几何应答脚本化，"预设 findings/Failed/Canceled/异常"经真实评估逻辑自然产生）＋StubPolicyProvider（IPolicyProvider 测试实现——预设 PolicyResolution、脚本抛出轨、评估器注入槽〔注入前 collisionEvaluator() 抛 PortAssemblyIncomplete＝装配分步契约的替身侧镜像〕、预设描述符）＋四个注入面替身（ScriptedNameContext/ScriptedValidationContext/ScriptedBytesSource/ScriptedCallContext——§3.3/§9.7 只读面逐一适配）＋脚本化后端对（ScriptedCollisionBackend/ScriptedDistanceBackend——rw 策略接口注入缝）；与 POL-T07 TU 内同型类职责面不同（整评估器/端口替身 vs 单后端脚本），既有验收面零改动；②**POL-TD-1 替身边界声明两面**（acceptance 2）：文书面＝test/README.md（§1 声明全文＋§2 替身清单＋§3 §11 矩阵→测试文件落位映射全表——EV-REG-3 同款三段结构）；机检面＝PolicyTestDoubleBoundaryTest（双模式 3 用例：产品源码 src/＋include/ 替身符号零命中扫描＋替身头声明在案＋README 声明在案）；行为面＝ScriptedEvaluatorProducesAllFourScriptTracks（真实会话对脚本输入的处置断言）；③**契约套件目标 sdurws_ird_policy_contract_test 注册**（§3.4"跨单元契约面"——此前 POL-T01 仅注册 _test）：双模式文件 PolicyCodecFixtureContractTest（5 用例）＋PolicyTestDoubleBoundaryTest（3 用例）；集成专属文件（TARGET sdurw_kinematics gating）PolicyContractDoublesTest（7 用例）＋PolicyProofMaterialContractTest（4 用例）＋CollisionAnalyticDatasetTest（4 用例）；链接面＝policy＋testkit＋gtest（T-1 允许形态）；④**数据集交付**（acceptance 3，testdata/golden/pol-* 按 testkit manifest schema 登记）：pol-policy-codec-fixture/1.0.0（contract-fixture 类——RawPolicyInput/PolicyCodec 契约：canonical＋variant 双变体〔规则乱序/无序对互换/域乱序/安全间距 10mm 等值表述〕、full/投影双形态编码 hex＋contentIdentity 独立摘要对照；expected 由 generate/make_pol_policy_fixture.mjs 按 §5.3 布局规则独立重实现产出〔node:crypto SHA-256——与 core ContentDigester 同算法标准实现〕——双实现互证任一侧漂移即显性失败；POL-ID-1/2/3 数据集载体＋CON-05 身份可独立复算）；pol-collision-analytic/1.0.0（analytic-case 类——轴向对齐立方体对已知相交/分离/近零分离/正负镜像六工况＋倒挂安装判别例；referenceSource=hand-built 独立；edgeCases 三布尔全 true＋sampleRefs；producer.solverConfig 记录策略阈值快照〔§4.3.2 工程策略默认类别消费：行程 4π 经 policy 对象 origin=DefaultAppendixD 断言，测试不旁路策略私设阈值；safetyClearance=0 dataset-declared——§7.4 m=0 退化，内置后端无距离能力 P-POL-11〕）；容差档案 pol-codec（零宽容差＝字节级等值档案化表达）＋pol-analytic（dataset-declared absolute 1e-9 m——闭式舍入界推导登记）；**lint**：testdata_lint 全量扫描 5 数据集版本 0 违规；**随 CI 跑通**＝ctest 双模式 policy 叶 2/2（gate-all 集成/冒烟各 7 注册全 PASS）；⑤**§11 矩阵收尾**：POL-SHARE-1（三入口替身消费者经同一 provider——评估器实例地址全等＋会话身份相等＋输出逐字段相等＋findings checkSetEquivalent〔SetMatchTraits<CollisionFinding> 显式特化：静态函数经参数静态类型调用、无动态分发，派生类型不可见——特化须落在真实 testkit 命名空间〕）＋POL-AT-1（替身 project 处理器走查 §10.4 全链：resolvePolicy→JointLimitEvaluator→比较型三要素 actual=6π/threshold=4π/rad→ConfirmableFinding 四元组绑定〔policyObject/policyVersion/policyContentId/jointObject〕→未确认阻止→确认放行摘要含 policyContentId→多圈反例：阈值 6π 新身份复检放行〔D-08 闭区间〕）＋POL-AT-2（AT-19：三入口对象 ID 对/判定/原因码完全一致——显式排除面 appliedFilters ruleOrigin 逐字一致＋域默认面 level=Must）＋POL-AT-3（AT-27：m↔mm 等值重解析身份不变且评估输出不变〔间距面经脚本距离会话承载〕；替身分析配置对象修改输出不变——配置无覆盖通道的结构事实，KIN-13）＋POL-AT-4（倒挂消费：inverted q=0 无碰撞〔若二次旋转即翻转误报的判别例〕＋q=π 对象对与手算一致＋"policy 禁止二次旋转"契约注释在案断言——analytic-case 数据集对照载体）；POL-EVAL-3 契约面（PolicyProofMaterialContractTest：finding→evidence CollisionPair 映射〔objectIdA/B←objectA/B、inCollision←kind==Collision；sampleIndex 由映射记录承载〕＋validateProof 校验清单五段预检零问题＋反例面〔判定为否/A==B/缺必经性依据〕＋源码级字段锚〔两侧头成员名在案——任一侧字段漂移即显性失败〕）；⑥**POL-EVAL-3 的 evidence 对齐依赖面声明**（DTB §5.4 登记解释）：_contract_test 对 evidence 公共头（Evidence.hpp 值类型）**头级消费**（R-2 允许形态"只允许 include 其他单元的公共头"——§3.4 明文设计本目标承担"与 evidence 证明字段形状的对齐断言"），**零链接边**（不 link sdurws_ird_evidence——T-1 白名单合规；ird_gates 实证无 policy_contract_test→evidence 命中）；validateProof 实现（evidence 产品库符号）不调用——"可被 validateProof 采信"落为清单级预检＋源码锚双机制，采信真值判定归 evidence 侧既有 EV-VER 覆盖；§3 依赖表"policy→evidence 文档级对齐（零编译依赖）"的产品链接面口径不变；⑦**测试与门禁**：集成 _test 200/200＋_contract 23/23；冒烟 _test 131/131＋_contract 8/8（既有回归零破坏）；validate-task PASS；ird_gates 7 命中＝base 既有 6（POL-T10 基线逐码一致）＋本任务新增 1（SUB：policy_contract_test 直链 testkit——F-072 同族，白名单数据面注释允许而引擎 SUB 形态未豁免；处置权 WP-01 所有者，不私改）；gate-all 18/19（唯一 FAIL 即上述命中；集成/冒烟各 7 注册全 PASS）；CMake：policy/CMakeLists.txt 增 _contract_test 目标（两模式文件＋集成 gating 文件＋policy/test include 根〔替身头〕＋evidence 公共头 include 根〔仅集成 gating 内〕）。验收锚：acceptance 1（§11 矩阵逐条——既有 POL-T03~T10 交付面＋本任务 POL-SHARE-1/POL-AT-1~4/POL-EVAL-3 契约面，AT-01/AT-19/AT-37 观测点载体齐备）、acceptance 2（POL-TD-1 两面在案）、acceptance 3（pol-* 数据集 lint 通过＋随 CI 跑通）通过；留痕 traceability/builds/wp07-t11/ |
| v0.13 | 2026-09-14 | POL-T12（≙WP-07-T12，分支 wp07-t12）文档与门禁同步（§12 任务行产物；DTB §5.4，纯文档任务零代码变更，构建按 DoD 第 1 条免除，双模式既有测试全绿复证留痕 traceability/builds/wp07-t12/）：①**README 指向核对零偏差（acceptance 1）**——`policy/include/sdurws/ird/policy/README.md` 任务卡指向维持 §12（§9→§12 修正复审维持——v0.1 变更记录行在案）；实现状态段同步至 POL-T11 落位后现状（公共头十一模块逐头与 include/ 目录实证比对；过期表述"本目录当前不含公共头"消除；T02~T05/T07~T11 落位记录补全——原增量记录止于 POL-T06；落位登记指向 §15.4 v0.3~v0.12、验证留痕指向 builds/wp07-t01~t11 与最新双模式口径集成 223 例/冒烟 139 例）——核对证据 traceability/builds/wp07-t12/README-audit.md；②**§15.3 待裁决项状态更新（acceptance 2）**——P-POL-1 复核维持已关闭（2026-09-10 CR-04 随 runtime 卡产出消账在案；governance-log 册载 closed 一致，零动作）；P-POL-6 stale 就地翻转＝已消账（project.md 磁盘复核完整：§1～§15 章标题全在、实测 1386 行、`__PART3__` 零命中——原登记"§8～§15 缺失"过时；残余 §10.4/§13 交接项联合复核按 phase-one-readiness §2 维持，不因正文齐备自动通过）；governance-log.md P-POL-6 行（stale→closed）与 §4 汇总行同步——同源双登记中本卡仅处置 P-POL-6 侧，登记中发现的 P-EV-6 行滞后（EV-T12 已翻转 evidence 卡内状态而本册状态列未同步）按 governance-log §4 维护规则③以卡为准回改本册行（消账留痕＝evidence.md v1.3 EV-T12——本句为 v0.14 勘误：原行"如实留册载所有者处置，不越权代改"与实际动作矛盾，见 F-096；该登记册改动于 POL-T12 返工时按 B-1 路径 b 拆出治理提交 governance/pol-t12-registry）；其余 P-POL-2/3/4/5/7/8/10/11 维持 open（裁决归属不变更、不私裁）；③**UT-BUILD 并入 CI 建议登记（acceptance 3，§12 任务行产物原文）**——沿 CORE-T10/EV-T12/RT-T13 先例口径：policy UT-BUILD 四例（test/BuildRedLineTest.cpp：SourceTreeReachable_POL_BUILD/NoQtInclude_POL_BUILD_NFR_MNT_01/NoCrossUnitInclude_POL_BUILD_R1_R2/BaselineLibsPinned_POL_BUILD_R5）已随 sdurws_ird_policy_test 常驻 gate-all.ps1 本地门禁（WP-01-T02"全部 _test/_contract_test 一键执行"面）与 ird_gates 构建期等效扫描；CI 模板测试阶段 run-tests 正则钉 `^sdurws_ird_core_test$` 的执行面缺口如实登记 WP-01（CI 执行面扩至全部单元 _test/_contract_test 目标）／WP-24（WP-24-T01 基线登记时复核覆盖），提交件 traceability/wp07-t12-gate-registrations.md——scripts/ 与 ci/ 不在本任务 allowedFiles，不代行辖域改动、非新裁决；④**F-089/F-091 散文计数消账**（traceability/findings.json 两 resolution 指名"随 policy.md 下次增量修订（POL-T12）"执行；本句编号为 v0.14 勘误——原行误标〔F-088/F-090〕，见 F-095）——v0.11 行 DiagnosticsTest 套件分布修正为实测"4 套件 19 用例（码表 5＋构造不变量 9＋辅助 4＋多态 1）"〔F-089〕；v0.12 行契约测试用例数修正为实测"PolicyCodecFixtureContractTest（5 用例）/PolicyContractDoublesTest（7 用例）"〔F-091；集成 23＝7＋5＋4＋4＋3、冒烟 8＝5＋3；ird-test-report.json 与提交正文已入库历史按留痕不可变纪律不追溯改写〕；⑤**卡头版本链补全**——文档版本行由滞留的 v0.2 补全至 v0.13 完整链（v0.3~v0.12 各实现落位登记版本回填——evidence 卡 F-057 同型消账）；**本文与实现零偏差登记**＝v0.3~v0.12 各落位登记在案，本卡未发现未登记偏差 |
| v0.14 | 2026-09-14 | POL-T12 返工（attempt 3 验收 fail→重新送验；分支 wp07-t12 追加提交不改写历史；返工输入＝traceability/acceptance/POL-T12-20260914.md @ acc/POL-T12/3 @ 67dfb62e）：①**B-1 载体归位（唯一阻断项消账，findings F-094 blocking）**——acceptance 2 的登记册侧落点改动（governance-log.md：P-POL-6/P-EV-6 行翻转、P-POL-9 备注更新、§4 汇总行、§5 一致性核对行、v1.1 变更记录）自任务分支拆出，改道治理提交 governance/pol-t12-registry @ 94fe223c09dfd325a1bd52ab612769f1958a8081（已推 origin，补丁与原改动 diff 比对逐行等同；机制依据＝governance-log §4 维护规则①"任务提交或紧随治理提交同步翻转本表"），任务分支以追加还原提交将 governance-log.md 恢复至 base 44595aa 内容——送验 diff 自此不含该文件（不 force-push、不修改契约：路径 a 契约增补需所有者授权，所有者二选一裁决询问未获答复，本处置为验收记录路径 b 保守变体、待所有者合入段确认，可否决或改判），处置全文见 traceability/builds/wp07-t12/rework-b1-note.md；②**F-096 消账**——v0.13② P-EV-6 表述勘误（见该行勘误注：原"如实留册载所有者处置，不越权代改"更正为与实际动作一致"按维护规则③以卡为准回改本册行"）；③**F-095 消账**——v0.13④ 与卡头版本行引用编号勘误〔F-088〕→〔F-089〕、〔F-090〕→〔F-091〕（散文修正内容本身正确且经 attempt 3 验收 §4.8 复现吻合；findings.json F-089/F-091 翻转 fixed 并补注勘误）；④**零代码变更**——attempt 3 验收者亲手复现的构建/测试/门禁证据（集成 0 error＋200/23、冒烟 131/8、双探针即红、ird_gates 7 命中＝POL-T11 基线逐码一致零新增）对新 head 继续有效（复用依据 rework-b1-note.md 在案）；validate-docs/validate-task 重跑留痕 rework-validate-docs.log／rework-validate-task.log |
| v0.15 | 2026-09-26 | **WP-24-T01 冻结版本基线回填**（分支 wp24-t01；owner 指示 2026-09-26 随 WP-24-T03 首版装配提前启动——原排期阶段 E；本卡增量＝P-POL-5 消账＋代码回填，语义零变化）：①**基线文档产出**——`ird/share/baseline.md` v1.0（NFR-DEP-05 全字段：框架三子树锁定 commit〔core/RWSim＝31b818452913… 2026-07-01 初始可运行版本；RWS 框架源码同基线，伴随登记行变更 db31b6ab 仅 rwslibs/CMakeLists.txt ＋7 行子目录注册〕＋SA-02 零源码修改声明〔PATCHES.md 空表佐证〕＋Qt 6.11.1/MSVC v143〔VS 17.12.0〕/生成器＋碰撞后端冻结取值＋构建选项＋已用 API 清单〔库级链接语句快照＋类级面指针；WIP 单元框架面标注待阶段 E 复核刷新〕＋第三方许可证清单）；②**backendVersion 冻结值＝`rw-31b8184`**（框架核心锁定 commit 短哈希；原 RW_VERSION 构建宏为配置期动态产物——日期＋分支名，跨构建不可比，不满足复现要素判定前提）；③**代码单点回填**——CollisionEvaluator.hpp 新增 `inline constexpr char kFrozenBuiltinBackendVersion[]`（backendVersion 冻结值唯一字面量，紧邻 detail::makeBuiltinBackendDescriptor 声明；同基线变更纪律绑定）；两处装配调用切换：Compatibility.cpp currentRegisteredBackend／RobWorkCollisionEvaluator.cpp 构造器——同常量同源（§9.1 复现要素同源机械保证不变），`#include <RobWorkConfig.hpp>` 双双删除（兼容判定 TU 不再依赖框架配置头；集成模式 gating 维持不变——冒烟使能归后续任务，本任务不扩面）；RobWorkCollisionEvaluator.cpp 保持零字符串字面量纪律（常量经头标识符取得，<string> 显式补入）；toleranceModel 登记串尾占位"(…待 WP-24 基线冻结回填)"替换为冻结标注"；④**§15.3 P-POL-5 就地消账**（本表行更新——处置列已完成部分划线保留原文）。⑤**验证**：集成模式 policy 三目标构建零错误，`sdurws_ird_policy_test` 200/200＋`sdurws_ird_policy_contract_test` 23/23（CompatibilityTest backend-mismatch 臂／CollisionSessionTest 复现要素非空断言全过——测试零修改，原断言本就只钉"非空＋同源"不钉具体串）；独立冒烟 `sdurws_ird_policy_test` 131/131（gating 面不变）；ird_gates 引擎直跑 base（1b3a7371）..branch 归一化比对 diff＝空（零新增零消失，路径归一化后 63 逻辑命中集逐条一致）；**诚实边界**：R-4 零字面量 TU 注释仍含"RobWork"叙述词——注释不在扫描域（引号对面），与既有口径一致。 |
