...
| v0.13 | 2026-09-14 | POL-T12（≙WP-07-T12，分支 wp07-t12）文档与门禁同步（§12 任务行产物；DTB §5.4，纯文档任务零代码变更，构建按 DoD 第 1 条免除，双模式既有测试全绿复证留痕 traceability/builds/wp07-t12/）：①**README 指向核对零偏差（acceptance 1）**——`policy/include/sdurws/ird/policy/README.md` 任务卡指向维持 §12（§9→§12 修正复审维持——v0.1 变更记录行在案）；实现状态段同步至 POL-T11 落位后现状（公共头十一模块逐头与 include/ 目录实证比对；过期表述"本目录当前不含公共头"消除；T02~T05/T07~T11 落位记录补全——原增量记录止于 POL-T06；落位登记指向 §15.4 v0.3~v0.12、验证留痕指向 builds/wp07-t01~t11 与最新双模式口径集成 223 例/冒烟 139 例）——核对证据 traceability/builds/wp07-t12/README-audit.md；②**§15.3 待裁决项状态更新（acceptance 2）**——P-POL-1 复核维持已关闭（2026-09-10 CR-04 随 runtime 卡产出消账在案；governance-log 册载 closed 一致，零动作）；P-POL-6 stale 就地翻转＝已消账（project.md 磁盘复核完整：§1～§15 章标题全在、实测 1386 行、`__PART3__` 零命中——原登记"§8～§15 缺失"过时；残余 §10.4/§13 交接项联合复核按 phase-one-readiness §2 维持，不因正文齐备自动通过）；governance-log.md P-POL-6 行（stale→closed）与 §4 汇总行同步——同源双登记中本卡仅处置 P-POL-6 侧，登记中发现的 P-EV-6 行滞后（EV-T12 已翻转 evidence 卡内状态而本册状态列未同步）如实留册载所有者处置，不越权代改；其余 P-POL-2/3/4/5/7/8/10/11 维持 open（裁决归属不变更、不私裁）；③**UT-BUILD 并入 CI 建议登记（acceptance 3，§12 任务行产物原文）**——沿 CORE-T10/EV-T12/RT-T13 先例口径：policy UT-BUILD 四例（test/BuildRedLineTest.cpp：SourceTreeReachable_POL_BUILD/NoQtInclude_POL_BUILD_NFR_MNT_01/NoCrossUnitInclude_POL_BUILD_R1_R2/BaselineLibsPinned_POL_BUILD_R5）已随 sdurws_ird_policy_test 常驻 gate-all.ps1 本地门禁（WP-01-T02"全部 _test/_contract_test 一键执行"面）与 ird_gates 构建期等效扫描；CI 模板测试阶段 run-tests 正则钉 `^sdurws_ird_core_test$` 的执行面缺口如实登记 WP-01（CI 执行面扩至全部单元 _test/_contract_test 目标）／WP-24（WP-24-T01 基线登记时复核覆盖），提交件 traceability/wp07-t12-gate-registrations.md——scripts/ 与 ci/ 不在本任务 allowedFiles，不代行辖域改动、非新裁决；④**F-088/F-090 散文计数消账**（traceability/findings.json 两 resolution 指名"随 policy.md 下次增量修订（POL-T12）"执行）——v0.11 行 DiagnosticsTest 套件分布修正为实测"4 套件 19 用例（码表 5＋构造不变量 9＋辅助 4＋多态 1）"〔F-088〕；v0.12 行契约测试用例数修正为实测"PolicyCodecFixtureContractTest（5 用例）/PolicyContractDoublesTest（7 用例）"〔F-090；集成 23＝7＋5＋4＋4＋3、冒烟 8＝5＋3；ird-test-report.json 与提交正文已入库历史按留痕不可变纪律不追溯改写〕；⑤**卡头版本链补全**——文档版本行由滞留的 v0.2 补全至 v0.13 完整链（v0.3~v0.12 各实现落位登记版本回填——evidence 卡 F-057 同型消账）；**本文与实现零偏差登记**＝v0.3~v0.12 各落位登记在案，本卡未发现未登记偏差 |
