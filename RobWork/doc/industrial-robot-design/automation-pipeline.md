...
### 0.4 单会话执行模式（v1.7：所有者指令禁用子智能体）

2026-09-10 所有者下达"禁止使用子智能体"指令：本环境的实施段与验收段**不再派发子代理**，由 tick 会话**顺序扮演**实施者/验收者角色——逐字执行附录 B/C 的纪律与检查清单（角色脚本不变，承载者从"全新子代理"变为"同一会话切换角色"）。配套调整与诚实声明：

- **独立性降级**：角色分离从"模型级全新上下文"降级为"同会话角色切换"，验收独立性弱于 §7 原声明——必须在验收记录开头声明本降级及指令依据；对抗式清单（4.1～4.11）、复现构建、逐条证据表**不得缩减**，以清单刚性弥补独立性损失。
- **续租纪律保持**：进入实施/验收角色前各 renew 一次（长任务防租约误杀）；角色完成、写 state 前再次 renew 确认持有者身份（对应 v1.6"阻塞等待返回后再次 renew"的会话内等价物）。
- **实施者上下文隔离尽力而为**：编排者阶段读过的信息（state/守卫输出）无法从会话中抹除，实施仍须从契约全读开始、按阅读阶梯执行；验收阶段须以产物（headSha 冻结对象＋证据文件）为唯一裁决输入，不采信实施阶段的自述。
- 指令解除（所有者重新允许子代理）＝PIPE 增量修订回 v1.6 执行模型，禁止静默切回。

## 1. 角色映射

| 协议角色 | 流水线承载 | 独立性保证 |
| --- | --- | --- |
| 实施者 | v1.7 §0.4：tick 会话内进入实施者角色（原为全新子代理），仅输入：任务契约路径、分支名、base SHA；返工时另含 `lastFailureRecord` | 首次实施不读验收历史；返工仅读取该不可变失败记录，中断续作先按 §5 对账协议盘点；同会话降级声明见 §0.4 |
| 验收者 | v1.7 §0.4：tick 会话内进入验收者角色（原为另一个全新子代理），仅输入：契约路径、headSha（冻结送验对象）、验收记录输出路径 | **只给产物不给叙述**——以 headSha 冻结对象与证据文件为唯一裁决输入，不采信实施阶段自述；验收在**独立 worktree**（detached @ headSha）进行（ACC §3）；独立性降级在验收记录声明 |
...
6. 实施段（§0.4 单会话模式）：renew -Token 续租后，本会话逐字按附录 B 模板进入实施者
   角色（占位符：契约路径=队首条目 contractPath、分支=队首条目 branch、base=刚冻结 SHA；
   返工时附 lastFailureRecord）；实施完成后再次 renew，成功才处理完成事件：
   headSha=git rev-parse <branch>；清 run、attempts.implement=0、
   lastFailureRecord=null、phase=awaiting_acceptance、写回 state。
7. 验收段（§0.4 单会话模式）：attempts.accept+1；renew 续租；本会话逐字按附录 C 模板
   进入验收者角色（输入=契约路径/headSha/attempt 号/记录路径；以产物为唯一裁决输入，
