...
理由：单元卡是需求＋架构在该单元的消化产物，契约又是卡的编译产物（designRefs 带锚点、acceptance 逐条化）——重读全量等于重做已评审的推导，且挤占留给代码与证据的注意力；两例外（跨切/治理任务）的阅读范围由契约 designRefs 显式列出。
...
1. 工作树不干净，或当前分支不是 `redesign-main`（且非本流水线在管分支）→ 暂停报告。**豁免（v1.2.1，防自锁）**：流水线自持路径不视为"不干净"——`traceability/pipeline/**`（状态心跳写回）、`traceability/acceptance/**`（验收记录）、以及纯构建产物目录（`build*/`、`*-smoke/` 等未跟踪构建树）；豁免判定只看这些路径时视为干净，**除此之外**的任何未提交改动仍然触发暂停；
