...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.3（Draft；11 单元详设与编码准入同步；v0.2 重写为**全量任务卡结构**——v0.1 为 WP 登记汇总结构，其 WP 登记、构建约定与实测登记已被本版收编取代，见 §7 变更记录） |
| 日期 | 2026-09-10 |
| 状态 | **`Draft`** |
| 文档代号 | DTB |
...
第一批 11 单元详设已编写；本次“第一阶段”指设计批次，不替代 §1.2 的产品阶段 A～E。可开始 M0 的基础构建与测试设施编码；不允许跳过前置同时实施全部 11 单元，也不宣布 M0～M3 完成。
...
各卡任务拆分比历史 WP 更细（例如 DIAG-T05/06/09、EX-T10、UI-T11～T14、RPT-T01～T16）。完整映射按 [任务索引](traceability/phase-one-task-index.json) 的原始任务行回查，不能按数字后缀推定一一对应。新增任务未生成执行契约前保持 planned；详设编写完成不冒充评审 Accepted。
...
### 2.11 WP-10 ui（单元卡待产出；L3 界面支撑——Widgets 唯一例外 ARCH §3.2）
...
### 2.13 WP-12 reporting（单元卡待产出；B 级先行、C 级补全）
...
| 项 | 约定 |
| --- | --- |
| 分支 | 短生命周期分支 `wp<nn>-t<kk>`（如 `wp03-t02`）；DoD 达成后合入 main；文档修订可直接 main 小步提交 |
...
一次实现会话的输入固定为：①本文该任务行（含 DoD/禁止项）；②其"输入"列指向的文档章节（任务卡行/上游条款）；③其前置任务的产出清单（公共头/目标/数据集）。会话循环：读输入 → 实现 → 双模式构建 → `ird_gates` → 用例执行留痕 → 文档同步 → 提交。任何一步失败停在原地登记，不带病前进；发现上游缺陷按 §5.4 登记，不私自裁决。所有持久状态落文档与代码，不依赖会话间记忆。
...
AI 只能领取 `doc/industrial-robot-design/tasks/` 下（含 foundation/ 子目录）状态为 `ready` 的单份执行契约；索引 JSON 不是执行契约。单元任务行与 canonical 的前置必须同时满足。任务必须先通过 `scripts/industrialrobot/validate-task.ps1`，完成后运行 `verify-task.ps1`，并同步 `traceability/` 中的状态和映射。需求或架构语义发生变化时，任务必须转为 `blocked`，不得在代码中自行解释或修改上游文档。
