...
| WP-24-T03 | 实现应用壳装配与静态白名单〔v0.21 拆段：**T03a 首版装配已于 2026-09-26 提前落位**（owner 指示"每完成一个插件进行装配"——IPluginUiRegistrar/IPluginUiModule 头＋registrar 白名单装配＋建模插件真宿主挂位（宿主侧 Dock 承载）＋UiText 域命令键族；收口余量＝域命令入 content 注册表/StageStatusModel 汇聚/中央区挂位/draft.apply 域侧挂钩/AssertionSuite 注入/会话事件同步/About 清单接线，归 T03b 收口）；分支 wp24-t03 待验收〕 | 横切（L5） | M3（平台单元就绪）、WP-10-T03 | ARCH §2.4（SA-01）、§5.4；NFR-SEC-04 | 应用壳（RobWorkStudioApp 壳＋白名单静态装配＋About 插件清单 UX-14） | NFR-SEC-04、SA-01、UX-14（协作） | 白名单在装配期固定、无运行时动态加载入口；About 清单与白名单一致用例通过 | 不提供动态加载/卸载入口 | M |
...
| O-14 | P-EV-9：必验工况标记的权威 schema（负载工况对象 enabled/mandatory 字段）归 requirements，其卡未产出 | RequiredCaseSet 解析口径 | evidence.md §15.3 | requirements 卡（WP-14-T01，冻结后回接 evidence §4.1.3） | **已消账（2026-09-22，WP-14-T01）**：units/requirements.md v0.1 §6.2 冻结 enabled/mandatory schema（mandatory≡(level==Must)＋I-REQ-9 唯一解析）＋§12 交接；evidence.md §4.1.3 权威锚点与 §15.3 状态回接随同批治理提交 |
...
| O-16 | P-RT-4：安装预设轴向（倒挂＝R_x(π)、壁装＝R_y(π/2)）为 runtime.md 单侧选择，MDL-22 未指明 | 预设矩阵与建模编辑器/用户预期对齐 | runtime.md §15.3 P-RT-4 | modeling 卡（WP-13-T11 交叉核对冻结） | 已裁决（2026-09-22，WP-13-T01 同批）：units/modeling.md v0.1 §7.7 采纳 runtime 侧锚点（installationPresetRotation() 唯一权威产出点，倒挂 R_x(π)／壁装 R_y(π/2)，modeling 只存参数不存矩阵）；WP-13-T11 实现交叉核对留痕待做 |
...
| O-35 | 命令 commandType 语法矛盾：project.md §4.4.4 冻结语法 `^[a-z0-9-]{3,64}`（无点）与 §6.5 内置示例 project.create-branch（含点）冲突（P-PR-9）；modeling/requirements 两卡已按无点形态设计（apply-robot-design/apply-tool-definition/apply-scene-objects/apply-named-poses/apply-drivetrain-design/apply-requirement-set/apply-requirement-import） | 阶段 B 三单元命令契约的最终 token 形态 | modeling.md §9.3、requirements.md §9.1 vs project.md §4.4.4/§6.5 | project 卡所有者（订正 §6.5 示例） | 已裁决（2026-09-22，治理会话奉所有者批次授权）：**采用无点形态**＝服从现行冻结语法（点分仅系 project.md 内部矛盾示例，选无点非新决策）；project.md §6.5 示例订正登记为 project 卡增量义务；两卡命令 token 即最终形态 |
| O-36 | 子条目 ObjectId 与 objectRefs 复核范围：runtime.md v0.12② 注记"Description 四子结构 objectId ∈ objectRefs"，但 IRobotDesignReader 契约为单根字节输入（runtime.md §4.2）——modeling/requirements 两卡据此采用"根＋集合对象"分解，关节/需求条目子 ObjectId 为模型内标识不入 objectRefs（P-MDL-1/P-REQ-2） | runtime S5/CM-0 复核实现口径；两卡对象分解合法性 | runtime.md §4.2/§4.3 vs modeling.md §4.2/§14.3、requirements.md §4.1 | runtime 卡所有者 | 已裁决（2026-09-22，治理会话奉所有者批次授权）：**objectRefs/CM-0 复核范围＝真实存储对象**（各单元根/集合/资源对象；reader 单根字节契约为准），子条目 ObjectId 属模型内标识（诊断 subject／名称映射锚）；runtime.md §4.3 注记收窄登记为 runtime 卡增量义务（P-RT-5 reader 注入形态同批确认＝modeling.md §9.2 采纳注入、直解否决） |
| O-37 | RuntimeSnapshot 进入评估器的注入点未定义：evidence §9 IEvaluationContext/EvaluationRequest 无运行时模型访问面，kinematics 评估器获取只读模型视图的机制待定（P-KIN-2） | WP-15-T03~T07 评估器接线 | evidence.md §9.3 vs kinematics.md §4.2/§14.3 | evidence＋execution 卡所有者 | 已裁决（2026-09-22，治理会话奉所有者批次授权）：**宿主注入形态**——评估宿主（execution/L5）构建绑定请求 RuntimeSnapshot 的只读模型视图、经评估器工厂闭包注入（registry.create() 无参签名不变；视图随请求生命周期）；evidence.md §9 增补登记归 evidence 卡下次增量修订；冻结前 WP-15 契约 knownPitfalls 携带"注入契约未冻结即 blocked"触发 |
| O-38 | 平台界面（ui 工作台壳）与 RobWorkStudio 主窗口的融合形态：壳现为独立顶层 QMainWindow（harness `sdurws_ird_ui_app`），框架侧 `rws::RobWorkStudioPlugin`（QDockWidget 派生＋initialize/open/close/setupMenu＋`Plugins→Load plugin` 动态装载）尚未被产品使用——开发期能否以插件形态把平台/业务功能逐块嵌入宿主主窗口操作验证（SA-01 静态白名单是否排除开发期动态加载） | 嵌入形态不定＝平台能力只能在独立 harness 验证（三维视图/停靠/框架菜单共存等宿主体验验不到），各业务插件（WP-13-T15 起）落位时才有首次真实集成——形态偏差返工风险整体后置 | 所有者会话裁决（2026-09-23，融合方案评估：RobWorkStudioPlugin 机制与 WorkbenchShell 现状逐项核对） | —（所有者已裁决） | 已裁决（2026-09-23，所有者）：①**开发期宿主插件验证通道设立**——ui 单元交付 `sdurws_ird_ui_plugin`（RobWorkStudioPlugin 派生薄插件、零计算逻辑），开发期经框架 `Plugins→Load plugin` 动态加载做界面级操作验证；动态加载**仅限开发期验证通道**，产品交付路径零新增动态加载入口（SA-01/NFR-SEC-04 不变，正式静态装配仍归 WP-24-T03）；②**壳拆"内容装配层＋顶层窗口宿主层"两层**——harness 与插件共用同一内容装配面（五区交互/布局记忆/命令门控语义不变），IWorkbenchShell 契约面增量修订随实现落位登记（ui.md §10.1 增量注）；禁止把顶层 QMainWindow 嵌套进宿主 Dock（双菜单/双状态栏/双 Dock 管理反模式）；③**宿主模式中央区与框架 RWStudioView3D 共存的最小接入归 WP-10-T16**，完整三维交互仍归 WP-10-T05 阶段 B；④任务载体＝§2.11 WP-10-T16（≙UI-T16），ird_gates 新增命中随任务按 §4.5 模式登记 |
| O-38 | sampleSetIdentity canonical 格式：公式在 evidence §4.1.4（SHA-256 over planContentIdentity‖预算/种子 canonical），生成算法归 kinematics、冻结凭据归 evidence——预算/种子 canonical 编码字节须三方一致（P-KIN-3） | 覆盖率评估的身份对账；误判 DataInsufficient 风险 | evidence.md §4.1.4、requirements.md §5.2、kinematics.md §7.2 | evidence 卡所有者（格式冻结）＋kinematics（实现） | 登记（evidence 卡增量义务，沿 requiredCaseSetId I-4"前缀＋规范串摘要"同款模式）；冻结前 kinematics 以快照 SamplingPlanRef 对账为准、不一致即 DataInsufficient（设计上安全） |
...
| O-43 | 平台界面与 RobWorkStudio 主窗口的深度融合形态（O-38 的产品化延伸）：插件命令现挂宿主 Plugins 菜单、五区整体装于单一 Dock、插件 Dock 体底部内嵌第二 QStatusBar（PM-11 投影行）——与"项目命令融入宿主 File/Tools/视图菜单＋功能分片多 Dock＋宿主状态栏唯一"的目标形态存在差距（融合方案审核会话逐条核对） | 融合深化＝UI 呈现层宿主接入面变更（§4.1 五区拓扑/§7.6 宿主菜单半区/§10.1 WorkbenchHostKind/statusBarWidget 契约面）；需求语义与单元边界零变化 | 所有者指令（2026-09-24，融合方案审核） | —（所有者已指令） | 已裁决（2026-09-24，所有者）：**按序三步承接**——①治理先行（本行＋§2.11 WP-10-T17/WP-10-T18 行登记）；②**WP-10-T17**＝会话入口处理器补全（draft.save/workbench.closeProject 装配层覆写面扩展——§11.5 既有模式同型；关闭统一确认对话框呈现＋Draining 轮询驱动＋saveAllDraftsManual 接线〔DraftController 装配，保存链路真实；恢复/autosave/分支锚随完整草稿链路任务接续——登记于任务卡〕）＋宿主菜单融合（File 内按位插入"工业机器人项目"子菜单＋最近项目、Tools 挂命令面板、自建"视图"菜单承载区域开关与恢复布局；Plugins 菜单仅留基类显示开关）；③**WP-10-T18**（已立项未实施，高风险重构单独立任务）＝Dock 拓扑拆分＋状态行迁移宿主 statusBar。**不变项**：装载方式（动态加载仅限开发期通道——SA-01 不变，正式静态装配仍归 WP-24-T03）；生命周期边界（宿主 WorkCell 回调保持正交观测——O-38 ③；项目引用 WorkCell 加载宿主三维视图归阶段 B 端口化任务）；框架零修改（SA-02） |
...
| 开发期宿主验证通道（v0.17，O-38） | ui 宿主插件 `sdurws_ird_ui_plugin` 开发期经框架 RobWorkStudio `Plugins→Load plugin` 动态加载做界面级操作验证——动态加载**仅限开发期**，产品交付路径零新增动态加载入口（SA-01 静态白名单不变，正式装配归 WP-24-T03）；插件目标属 ird_gates plugin 分类面（Qt 链接合法），其 Q_OBJECT/插件元数据所需 AUTOMOC 仅该目标开启（ui 库本体与既有目标维持关闭） |
| 插件 GUI 手动验证通道（v0.20，owner 指示 2026-09-26） | **每个 `_plugin` 交付任务完成时，必须可经 VS 集成编译（`cmake --build build --config Release`）后启动 GUI 人工点验界面功能**。默认形态＝单元自持开发验证 harness `sdurws_ird_<unit>_app`（`_app` 后缀＝ird_gates 分类面 Qt 合法形态；先例 ui `sdurws_ird_ui_app`〔UI-T15〕、modeling `sdurws_ird_modeling_app`〔WP-13-T15 增量〕）——装配任务（WP-24-T03）前的过渡通道，真实域路径＋最小注入（编辑目标提供器/命令提交回显），不虚构提交语义；harness 源码置于 `app/` 子目录（产品扫描域零触碰），不改变产品交付路径与装配契约。验证步骤与截图留痕入 `traceability/builds/<task>/`。WP-24-T03 落位后，面板在真实宿主内的手动验证按同一 DoD 条目执行 |
...
5. **插件 GUI 手动验证**（v0.20 增补，owner 指示 2026-09-26；仅适用 `_plugin` 交付任务）：按 §5.1"插件 GUI 手动验证通道"行完成 harness（或装配后宿主）启动的人工点验——集成模式构建零错误后可执行文件直接启动、面板可见、核心交互（选中联动/编辑流/命令目录）可操作；步骤与截图留痕入 `traceability/builds/<task>/`。无法执行时（如环境受限）必须在送验记录中如实登记原因，不得标注通过。
