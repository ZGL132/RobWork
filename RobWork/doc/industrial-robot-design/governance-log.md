...
| O-31 | ui.md 依赖声明三方矛盾（§2.1.2 C 表 vs §3.1 vs ARCH §3.5） | 架构所有者＋ui 卡 | **closed** | 已裁决（2026-09-19：确认 ui 侧最小注入接口模式——不增边＋ui 自有端口/值投影＋L5 适配；ui.md v0.4 落地，UI-T03~T14 整链放行；DTB §4 O-31 行同日登记） |
...
| O-34 | CI 常驻激活前置（框架路径补丁登记＋入口脚本命名裁决） | 构建约定所有者＋所有者 | open | 补登——DTB §4.2 已于 WP-01-T02 登记（v0.14），集中视图漏同步；权威本体以 DTB §4.2 O-34 行为准 |
| O-35 | 命令 commandType 语法矛盾（点/无点形态） | project 卡所有者 | **closed** | 已裁决（2026-09-22：采用无点形态——服从现行冻结语法；DTB §4.2 O-35 行） |
| O-36 | 子条目 ObjectId 与 objectRefs 复核范围 | runtime 卡所有者 | **closed** | 已裁决（2026-09-22：objectRefs/CM-0 复核范围＝真实存储对象；DTB §4.2 O-36 行） |
| O-37 | RuntimeSnapshot 进入评估器的注入点 | evidence＋execution 卡所有者 | **closed** | 已裁决（2026-09-22：宿主注入形态——评估宿主构建只读模型视图经工厂闭包注入；DTB §4.2 O-37 行） |
| O-38 | 平台界面与 RobWorkStudio 主窗口融合形态（开发期宿主插件验证通道） | 所有者 | **closed** | 已裁决（2026-09-23，所有者融合方案评估）：开发期 `sdurws_ird_ui_plugin` 动态加载验证通道（仅限开发期，产品交付路径零新增动态加载入口——SA-01/NFR-SEC-04 不变，正式装配归 WP-24-T03）＋壳拆内容装配层/顶层窗口宿主层两层（harness 与插件共用）＋宿主模式 RWStudioView3D 共存最小接入；任务载体＝WP-10-T16（≙UI-T16，队列头插队）；DTB §4.2 O-38 行＋v0.17 变更记录＋ui.md v1.9 §13 UI-T16 行同日登记 |
