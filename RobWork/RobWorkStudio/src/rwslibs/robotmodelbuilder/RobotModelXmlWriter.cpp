...
bool isLegacyAutoDrawable (const DrawableSpec& drawable)
{
    const QString name = QString::fromStdString (drawable.name);
    return (QRegularExpression ("^Joint\\d+Housing$").match (name).hasMatch () &&
            drawable.shape == "Cylinder" && drawable.filePath.empty ()) ||
           (QRegularExpression ("^Link\\d+To\\d+$").match (name).hasMatch () &&
            drawable.autoLinkGeometry);
}

bool isAutoGeneratedDrawable (const DrawableSpec& drawable)
{
    return drawable.autoGenerated || isLegacyAutoDrawable (drawable);
}

bool hasUserDrawableForFrame (const RobotModelSpec& spec, const std::string& refFrame)
{
    return std::any_of (spec.drawables.begin (), spec.drawables.end (),
                        [&refFrame] (const DrawableSpec& drawable) {
                            return !isAutoGeneratedDrawable (drawable) &&
...
void appendLinks (RobotModelSpec& spec)
{
    ...
    if (n < 2)
        return;
    // Let automatic links carry the primary arm silhouette.  Long shoulder
    // and forearm segments stay fuller than wrist segments, avoiding a box
...
RobotModelSpec RobotModelXmlWriter::makeDefaultSixAxisModel (const QString& saveDirectory)
{
    ...
    // Desktop industrial-arm envelope.  These values deliberately remain
    // vendor-neutral while providing a credible tabletop reach and a compact
    // wrist for the generated pick-and-place workcell below.
    ...
    // The vendor STL set is intentionally not copied into every project.  The
    // automatic links form the visible arm, leaving only the fixed details
    // that cannot be inferred from inter-joint distances.
    const auto appendPrimitive = [&spec] (const std::string& name, const std::string& frame,
        ...
        // The default workcell must not be visual-only: every primitive used
        // to represent a fixed arm or gripper body gets the matching explicit
        // collision model.  Automatic link collision bodies are added below
        // after their poses have been derived from the joints.
        CollisionModelSpec collision;
        collision.name       = "Collision" + name;
        collision.refFrame   = frame;
        collision.shape      = shape;
        collision.dimensions = dimensions;
        collision.radius     = radius;
        collision.length     = length;
        collision.pos        = position;
        collision.rpyDeg     = rpy;
        spec.collisionModels.push_back (collision);
    };
    const std::array< double, 3 > armGray = {{0.48, 0.51, 0.54}};
    ...
    for (const DrawableSpec& drawable : spec.drawables) {
        if (!drawable.autoLinkGeometry)
            continue;
        CollisionModelSpec collision;
        collision.name       = "Collision" + drawable.name;
    ...
    const double posMin[DefaultJointCount] = {-180, -270, -180, -270, -180, -180};
    const double posMax[DefaultJointCount] = {180, 90, 180, 90, 180, 180};
    const double vel[DefaultJointCount]    = {60, 60, 60, 60, 60, 60};
    const double acc[DefaultJointCount]    = {120, 120, 120, 120, 120, 120};
    for (int i = 0; i < DefaultJointCount; ++i) {
    ...
    PoseSpec ready;
    ready.name = "Ready";
    ready.q    = {0.0, -90.0, 0.0, -90.0, 0.0, 0.0};
    ...
    const auto appendSceneFrame = [&spec] (const std::string& name, const std::string& parent,
                                           ...
                                           const std::array< double, 3 >& position,
                                           const std::array< double, 3 >& rpy = {{0, 0, 0}}) {
        FrameSpec frame;
        frame.name      = name;
        frame.refFrame  = parent;
    ...
    const auto appendSceneBox = [&spec] (const std::string& name, const std::string& frame,
    ...
    appendSceneFrame ("PickTarget", "WorkTable", SceneFrameType::Normal, false,
                      {{0.250, -0.230, 0.125}}, {{180, 0, 0}});
    appendSceneFrame ("PlaceApproach", "WorkTable", SceneFrameType::Normal, false,
                      {{0.250, 0.230, 0.250}}, {{180, 0, 0}});
    appendSceneFrame ("PlaceTarget", "WorkTable", SceneFrameType::Normal, false,
                      {{0.250, 0.230, 0.125}}, {{180, 0, 0}});
    appendSceneFrame ("InspectionTarget", "WorkTable", SceneFrameType::Normal, false,
                      {{0.050, 0.310, 0.180}}, {{180, 0, 0}});

    const std::array< double, 3 > tableGray = {{0.36, 0.33, 0.29}};
    appendSceneBox ("TableTop", "WorkTable", {{1.400, 0.900, 0.060}},
                    {{0, 0, 0}}, tableGray);
    appendSceneBox ("WorkTableLegFrontLeft", "WorkTable", {{0.070, 0.070, 0.680}},
                    {{0.620, -0.370, -0.370}}, tableGray);
    appendSceneBox ("WorkTableLegFrontRight", "WorkTable", {{0.070, 0.070, 0.680}},
                    {{0.620, 0.370, -0.370}}, tableGray);
    appendSceneBox ("WorkTableLegBackLeft", "WorkTable", {{0.070, 0.070, 0.680}},
                    {{-0.620, -0.370, -0.370}}, tableGray);
    appendSceneBox ("WorkTableLegBackRight", "WorkTable", {{0.070, 0.070, 0.680}},
                    {{-0.620, 0.370, -0.370}}, tableGray);
    appendSceneBox ("PickBinFloor", "PickBin", {{0.220, 0.180, 0.020}},
...
QString RobotModelXmlWriter::makeSerialDeviceXml (const RobotModelSpec& spec)
{
    ...
    // 3. 动态配置末端工具坐标系 (TCP)。Only an explicit ToolFrame actually
    // named TCP replaces the canonical fallback.  A differently named
    // ToolFrame (for example Joint6 during an editable conversion) must not
    // make the device lose its TCP contract.
    const bool hasExplicitCanonicalTcp = std::any_of(
        spec.transformJoints.begin(), spec.transformJoints.end(),
        [] (const JointTransformSpec& joint) {
            return isToolFrameType (joint.type) && joint.name == "TCP";
        });
    const std::vector< size_t > movable = movableJointIndices (spec);
    if (!hasExplicitCanonicalTcp) {
        QString tcpRef = "Base";
        if (!movable.empty ())
                                                 ...
                                                 double /*paddingBeforeFirst*/)
{
    spec.drawables.erase (
        std::remove_if (spec.drawables.begin (), spec.drawables.end (),
                        [] (const DrawableSpec& d) { return isAutoGeneratedDrawable (d); }),
        spec.drawables.end ());
