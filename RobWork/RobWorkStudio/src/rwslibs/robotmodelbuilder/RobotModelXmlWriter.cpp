...
bool isMeshCollisionShape (const CollisionModelSpec& collision)
{
...
bool isSimplifiedLinkDrawable (const DrawableSpec& drawable)
{
...
bool collisionSuppressedBySimplifiedDrawable (const RobotModelSpec& spec,
                                              const CollisionModelSpec& collision)
{
    if (!isMeshCollisionShape (collision))
        return false;
    return std::any_of (spec.drawables.begin (), spec.drawables.end (),
                        [&collision] (const DrawableSpec& drawable) {
                            return drawable.refFrame == collision.refFrame &&
                                   isSimplifiedLinkDrawable (drawable);
                        });
}
...
RobotModelSpec RobotModelXmlWriter::makeDefaultSixAxisModel (const QString& saveDirectory)
{
    ...
    // Use the UR5 dimensions bundled in the RobWork reference workcell.  New
    // projects remain generic and editable, but start from a credible compact
    // six-axis industrial-arm envelope instead of arbitrary link lengths.
    ...
    const double pos[DefaultJointCount][3]    = {
        {0, 0, 0.0892}, {0, 0, 0}, {-0.425, 0, 0},
        {-0.39243, 0, 0}, {0, 0, 0.109}, {0, 0, 0.093}};
    ...
    const std::array< double, 3 > urGray = {{0.50, 0.52, 0.54}};
    const std::array< double, 3 > urDark = {{0.18, 0.20, 0.22}};
    appendPrimitive ("BasePedestal", "Base", "Cylinder", {{0.1, 0.1, 0.1}},
                     0.120, 0.070, {{0, 0, 0.035}}, {{0, 0, 0}}, urDark);
    appendPrimitive ("ShoulderHousing", "Joint1", "Cylinder", {{0.1, 0.1, 0.1}},
                     0.110, 0.120, {{0, 0, 0.035}}, {{0, 0, 0}}, urGray);
    appendPrimitive ("ToolFlange", "Joint6", "Cylinder", {{0.1, 0.1, 0.1}},
                     0.052, 0.070, {{0, 0, 0.050}}, {{0, 0, 0}}, urDark);
    appendLinks (spec);
...
bool RobotModelXmlWriter::validate (const RobotModelSpec& spec, QStringList& errors)
{
    ...
    for (const CollisionModelSpec& collision : spec.collisionModels) {
        if (!collision.enabled || collisionSuppressedBySimplifiedDrawable (spec, collision))
            continue;
