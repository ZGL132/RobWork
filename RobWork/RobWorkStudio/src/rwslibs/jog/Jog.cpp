...
formatUnitDescriptions (const std::string& angles, const std::string& distances,
                        ...
                        const std::map< std::string, double >& distanceUnitConverters)
{
    ...
    // Angles
    for (std::map< std::string, double >::const_iterator it = angleUnitConverters.begin ();
         it != angleUnitConverters.end ();
         ++it)
        if (QString::compare (
                QString (angles.c_str ()), QString (it->first.c_str ()), Qt::CaseInsensitive) == 0)
            descs.first = it->first;

    // Distances
    for (std::map< std::string, double >::const_iterator it = distanceUnitConverters.begin ();
         it != distanceUnitConverters.end ();
         ++it)
        if (QString::compare (QString (distances.c_str ()),
                              QString (it->first.c_str ()),
...
void makeSliderUnitDataJoint (const std::pair< std::string, std::string > descs,
                              const std::map< std::string, double >& angleUnitConverters,
                              const std::map< std::string, double >& distanceUnitConverters,
                              Joint* const joint, std::vector< std::string >& desc,
...
/*
 * Inputs are the desired angular/translational unit descriptions, a map of descriptions/converters
 * and a device pointer. First the descriptions are corrected (capital first letter) and then the
 * unit data is created. If a device is input, a vector of angular <descriptions, converters> are
 * returned for each joint. Else the first three DOF designate the translational part and the last
 * three DOF the angular part.
 */
std::pair< std::vector< std::string >, std::vector< double > >
makeSliderUnitData (const std::string& angles, const std::string& distances,
                    const std::map< std::string, double >& angleUnitConverters,
                    const std::map< std::string, double >& distanceUnitConverters,
                    const rw::models::Device::Ptr selectedDevice = 0)
{
    const std::pair< std::string, std::string > descs =
        formatUnitDescriptions (angles, distances, angleUnitConverters, distanceUnitConverters);
    ...
    if (selectedDevice) {    // Device
        ...
        rw::models::JointDevice::Ptr jointDevice =
            selectedDevice.cast< rw::models::JointDevice > ();
        if (jointDevice) {
            // Get joints
            const std::vector< rw::models::Joint* >& joints = jointDevice->getJoints ();
            // Iterate through
            for (std::vector< rw::models::Joint* >::const_iterator it = joints.begin ();
                 it != joints.end ();
                 ++it) {
                makeSliderUnitDataJoint (
                    descs, angleUnitConverters, distanceUnitConverters, *it, desc, conv);
            }
        }
        else {
...
std::pair< std::vector< std::string >, std::vector< double > >
makeSliderUnitDataFull (const std::string& angles, const std::string& distances,
                        const std::map< std::string, double >& angleUnitConverters,
                        const std::map< std::string, double >& distanceUnitConverters,
...
/*
 * Update a unit combo box index by searching (case insensitive) for the current unit description
 * set.
 */
void updateUnitCB (QComboBox* cmbUnits, const std::string& desc)
{
    int index = cmbUnits->findText (QObject::tr (desc.c_str ()),
...
Jog::Jog () :
    RobWorkStudioPlugin ("Jog", getIcon ()), _workcell (0), _selectedDevice (0),
    _jointSliderWidget (0), _jointSliderWidgetFull (0), _selectedFrame (0), _cartesianTab (0),
    _cartesianDeviceTab (0), _updating (false), _cartesianBounds (Q::zero (6), Q::zero (6)),
    _rwsSettings (0)
{
    ...
    _angleUnitConverters.insert (
        std::pair< std::string, double > ("Grads", rw::math::Rad2Deg * 4.0 / 3.6));
    _angleUnitConverters.insert (std::pair< std::string, double > ("Turns", 0.5 / rw::math::Pi));

    for (std::map< std::string, double >::const_iterator it = _angleUnitConverters.begin ();
         it != _angleUnitConverters.end ();
         ++it)
    ...
    _distanceUnitConverters.insert (
        std::pair< std::string, double > ("Inches", rw::math::Meter2Inch));

    for (std::map< std::string, double >::const_iterator it = _distanceUnitConverters.begin ();
         it != _distanceUnitConverters.end ();
         ++it)
    ...
    connect (
        _cmbDistanceUnit, SIGNAL (currentIndexChanged (int)), this, SLOT (cmbUnitChanged (int)));
...
void Jog::open (WorkCell* workcell)
{
    ...
    _workcell->workCellChangedEvent ().add (
        boost::bind (&Jog::workcellChangedListener, this, boost::arg< 1 > ()), this);
    ...
    if (workcell) {
        ...
        for (FrameVector::iterator it = frames.begin (); it != frames.end (); ++it) {
            ...
            if (frame) {
                ...
                _cmbDevices->addItem (frame->getName ().c_str (),
                                      QVariant ((int) (_items.size () - 1)));
            }
        }
    }
    else {
    ...
    _rwsSettings = getRobWorkStudio ()->getPropertyMap ().getPtr< rw::core::PropertyMap > (
        "RobWorkStudioSettings");
    if (_rwsSettings) {
...
void Jog::cmbChanged (int index)
{
    ...
    connect (_tabWidget, SIGNAL (currentChanged (int)), this, SLOT (tabChanged (int)));
    updateUnit (_cmbAngleUnit->currentText ().toStdString (),
                _cmbDistanceUnit->currentText ().toStdString ());
}
...
void Jog::updateUnit (const std::string& angles, const std::string& distances)
{
    ...
    const std::pair< std::vector< std::string >, std::vector< double > > sliderUnitDataCartesian =
        makeSliderUnitData (angles, distances, _angleUnitConverters, _distanceUnitConverters, 0);
    if (_jointSliderWidget) {
        const std::pair< std::vector< std::string >, std::vector< double > > sliderUnitDataJoint =
            makeSliderUnitData (
                angles, distances, _angleUnitConverters, _distanceUnitConverters, _selectedDevice);
        _jointSliderWidget->setUnits (sliderUnitDataJoint.second, sliderUnitDataJoint.first);
    }
    if (_jointSliderWidgetFull) {
        const std::pair< std::vector< std::string >, std::vector< double > > sliderUnitDataJoint =
            makeSliderUnitDataFull (angles,
                                    distances,
                                    _angleUnitConverters,
                                    _distanceUnitConverters,
                                    _selectedDevice.cast< ParallelDevice > ());
    ...
    if (_cartesianDeviceTab) {
        _cartesianDeviceTab->setUnits (sliderUnitDataCartesian.second,
                                       sliderUnitDataCartesian.first);
    }
    if (_rwsSettings) {
...
void Jog::constructTabs (Device::Ptr device)
{
    ...
    connect (_jointSliderWidget,
             ...
             this,
             SLOT (deviceConfigChanged (const rw::math::Q&)));
    ...
    if (sdevice != NULL) {
        ...
        _jointSliderWidgetFull->setup (
            titles, pdevice->getAllBounds (), pdevice->getFullQ (_state), false);
        ...
        connect (_jointSliderWidgetFull,
                 SIGNAL (valueChanged (const rw::math::Q&)),
                 this,
                 SLOT (deviceConfigChangedFull (const rw::math::Q&)));
...
{
    // we need to call the update slot, but since this is possibly from a non qt thread, we need to
    // separate it through Qt::queue
