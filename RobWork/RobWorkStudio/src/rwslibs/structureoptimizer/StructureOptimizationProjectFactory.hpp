...
class StructureOptimizationProjectFactory
{
  public:
    ...
    /** Creates a legacy-compatible project and attaches an explicitly imported canonical shadow. */
    static bool create(const RobotModelSpec& spec, const KinematicImportRequest& importRequest,
                       StructureOptimizationProblem& problem, std::string* error = nullptr);
};
