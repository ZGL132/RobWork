    ...
    const CancellationToken& cancellation) const
{
    ...
    // Bound the number of targets actually generated by this orientation
    // mode. PointAtTarget emits one target per roll for each cell; the other
    // modes emit at most one target per cell. Check multiplication before it
    // can wrap and bypass the safety limit.
    std::size_t targetsPerCell = 1;
    if (region.orientationMode == RequirementExecutionOrientationMode::PointAtTarget) {
        if (region.rollSamples <= 0)
            targetsPerCell = 0;
        else
            targetsPerCell = static_cast<std::size_t>(region.rollSamples);
    }
    const bool multiplicationOverflows =
        targetsPerCell != 0 &&
        result.cells.size() > std::numeric_limits<std::size_t>::max() / targetsPerCell;
    const std::size_t compositeTargets = multiplicationOverflows
        ? std::numeric_limits<std::size_t>::max()
        : result.cells.size() * targetsPerCell;
