 ...
 * @brief  requirements 产品面源码红线运行期扫描用例组（ReqBuildRedLine）——
 *         零 Qt（R-3/NFR-MNT-01）与 include 面限于五条登记边（R-2/卡
 *         §3.2；任务契约 WP-14-T02 acceptance 1/5 的运行期自证面）。
 *
 * 设计依据：
 *   - units/requirements.md §3.1（计算库 L2 等效零 Qt）、§3.2（五条登记边
 *     ＋"不得 include 其他单元私有头"——R-2）、§14.5 自审清单（零越权）
 *   - 需求 NFR-MNT-01（计算内核零 Qt）
 *   - 先例：modeling/test/BuildRedLineTest.cpp（运行期扫描与 CMakeLists
 *     配置期守卫互为两道防线——配置期守卫在本单元 CMakeLists.txt 末尾，
 *     本文件是源码面那一道）
 *   - 任务契约 tasks/foundation/WP-14-T02.json acceptance 1（零 Qt）/
 *     acceptance 4（配置期红线守卫随文件自持——本用例组是其中源码面
 *     半区）
 *
 * 扫描域＝产品面 include/**＋src/**（test/**、contract_test/** 为测试面
 * 不在此列——与 ird_gates 第 4 步"产品面"同口径）；扫描经编译定义
 * IRD_REQUIREMENTS_UNIT_ROOT 注入的源码树根定位（modeling/io 同款）。
 */
...
std::vector<fs::path> collectProductFaceFiles()
{
    ...
    for (const auto* sub : {"include", "src"}) {
        ...
        if (!fs::exists(base)) {
            ADD_FAILURE() << "requirements 产品面目录缺失: " << sub;
            continue;
        }
        std::error_code iec;
...
/// requirements 产品面允许 include 的他单元清单＝五条登记边（卡 §3.2 边表；
/// 白名单 "requirements->…" 五行同源——runtime 暂不登记（T02 零引用），
/// 新增表外单元 include 在此暴露）。
 ...
 */
TEST(ReqBuildRedLine, ProductFaceHasZeroQtIncludes_WP14T02_ACC1)
{
...
/**
 * include 面限于五条登记边（acceptance 4——卡 §3.2；R-2 跨单元只经公共
 * 头）：产品面 #include <sdurws/ird/<unit>/…> 的 <unit> 只允许 requirements
 * 自身＋五条登记边单元；其他单元（业务域互链＝R-1 面；表外平台单元＝
 * SUB 面；runtime 暂不登记——T02 零引用）一经出现即越界。对登记边单元
 * 还核对公共头可达性语义的镜像：对端头必须落在对方 include/ 下（ird_gates
 * 第 4a 步同判据的单元内复核——私有头不可达）。
 */
TEST(ReqBuildRedLine, IncludeFaceRestrictedToFiveRegisteredEdges_WP14T02_ACC4)
{
    ...
    for (const auto& file : collectProductFaceFiles()) {
        ...
        for (std::sregex_iterator it(text.begin(), text.end(), kIrdInclude), end;
            ...
            EXPECT_TRUE(allowed)
                ...
                << "（五条登记边外——R-1/R-2，卡 §3.2）: " << file.string();
            if (allowed) {
 ...
 * 语义归 runtime（ARC-04/NFR-MNT-07）——requirements 产品面字符串字面量
...
TEST(ReqBuildRedLine, ProductFaceHasNoRobWorkStringLiterals_WP14T02_ACC4)
{
 ...
 * 面失真——文件为空/占位时先行暴露）。
 */
TEST(ReqBuildRedLine, ProductFaceFilesNonEmpty_WP14T02_ACC1)
{
    IRD_TEST_INFO(std::vector<std::string>{"NFR-MNT-01"},
                  std::vector<std::string>{});
