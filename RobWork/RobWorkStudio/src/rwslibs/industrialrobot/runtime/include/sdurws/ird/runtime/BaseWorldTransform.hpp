...
// =====================================================================
// 基座—世界变换规则版本（§9.4 缓存键分量 baseWorldRuleVersion 的常量承载
// ——自 v0.10 起随 RT-T09 落位：快照 workCellCompileIdentity 与编译缓存键
// 的确定性分量，"规则变化＝名称/变换可能变＝新键"）。
// =====================================================================

/**
 * @brief §6 规则版本常量（§9.4 分量表"基座—世界变换规则版本"行的原文承载）。
 *
 * 当前值 1＝§6 规则表的初始版本（含 P-RT-4 已冻结的预设轴向——RT-T06）。
 * 规则修改（预设矩阵、一致性容差、正反解定义等）＝本常量递增＝全体 WC 层
 * 编译缓存键与快照身份变化；升版走设计变更评审（与 nameMapRuleVersion=1
 * 同口径）。单一权威＝本常量；消费方（RT-T09 快照身份、RT-T10 缓存键）
 * 只经这里取值，不得各持副本（§6.3"不新增第二套基座变换规则"的版本面）。
 */
inline constexpr std::uint32_t kBaseWorldRuleVersion = 1;
