...
{
    // 全表 19 值逐项核对（顺序＝枚举声明顺序；字符串＝§5.2 建议码 kebab 派生；
    ...
    // 校验：SceneInvalid/NameUnresolved）；表尾 1 值为评估期查询违约码
    // （POL-T07——QueryInvalid，§9.3 错误类型行评估半区载体）。
    const std::pair<PolicyErrorCode, const char*> table[] = {
        ...
        {PolicyErrorCode::QueryInvalid, "policy/cll-query-invalid"},
    };
...
{
    // 全表 19 值逐项核对（§9.6 清单 13 值＋表尾补登 3 值：POLICY-POLICY-
    ...
    // ＋表尾补登 1 值（POL-T07）：POLICY-CLL-QUERY-INVALID——评估期查询
    // 违约码，单元卡 v0.8 登记）。
    const std::pair<PolicyErrorCode, const char*> table[] = {
        {PolicyErrorCode::SchemaUnknownField, "POLICY-SCHEMA-UNKNOWN-FIELD"},
        {PolicyErrorCode::SchemaVersionFuture, "POLICY-SCHEMA-VERSION-FUTURE"},
