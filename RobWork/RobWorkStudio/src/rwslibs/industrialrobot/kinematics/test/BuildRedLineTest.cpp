 ...
 * @brief  kinematics 产品面源码红线运行期扫描用例组（KinBuildRedLine）——
 *         零 Qt（R-3/NFR-MNT-01）与 include 面限于六条登记边（R-2/卡
 *         §3.2；任务契约 WP-15-T02 acceptance 1/5 的运行期自证面）。
 *
 * 设计依据：
 *   - units/kinematics.md §3.1（计算库 L2 等效零 Qt）、§3.2（六条登记边
 *     ＋"不得 include 其他单元私有头"——R-2）、§14.5 自审清单（零越权）
 *   - 需求 NFR-MNT-01（计算内核零 Qt）
 *   - 先例：requirements/test/BuildRedLineTest.cpp（WP-14-T02 同款——
 *     运行期扫描与 CMakeLists 配置期守卫互为两道防线；modeling 更早先例）
 *   - 任务契约 tasks/foundation/WP-15-T02.json acceptance 1（零 Qt）/
 *     acceptance 5（架构红线自查——R-1/R-2 机器证据与 ird_gates 互证）
 *
 * 扫描域＝产品面 include/**＋src/**（test/**、contract_test/** 为测试面
 * 不在此列——与 ird_gates 第 4 步"产品面"同口径）；扫描经编译定义
 * IRD_KINEMATICS_UNIT_ROOT 注入的源码树根定位（modeling/io/requirements
 * 同款）。
 */
...
/// kinematics 产品面允许 include 的他单元清单＝六条登记边（卡 §3.2 边表；
/// 白名单 "kinematics->…" 六行同源——表外单元 include 在此暴露）。
 ...
 */
TEST(KinBuildRedLine, ProductFaceHasZeroQtIncludes_WP15T02_ACC1)
{
...
/**
 * include 面限于六条登记边（acceptance 5——卡 §3.2；R-2 跨单元只经公共
 * 头）：产品面 #include <sdurws/ird/<unit>/…> 的 <unit> 只允许 kinematics
 * 自身＋六条登记边单元；其他单元（业务域互链＝R-1 面；表外平台单元＝
 * SUB 面）一经出现即越界。对登记边单元还核对公共头可达性语义的镜像：
 * 对端头必须落在对方 include/ 下（ird_gates 第 4a 步同判据的单元内复核
 * ——私有头不可达）。
 */
TEST(KinBuildRedLine, IncludeFaceRestrictedToSixRegisteredEdges_WP15T02_ACC5)
{
 ...
 * 语义归 runtime（ARC-04/NFR-MNT-07）——kinematics 产品面字符串字面量
...
TEST(KinBuildRedLine, ProductFaceHasNoRobWorkStringLiterals_WP15T02_ACC5)
{
 ...
 * 面失真——文件为空/占位时先行暴露）。
 */
TEST(KinBuildRedLine, ProductFaceFilesNonEmpty_WP15T02_ACC1)
{
    IRD_TEST_INFO(std::vector<std::string>{"NFR-MNT-01"},
                  std::vector<std::string>{});
