 ...
 *   - DataInsufficient ⇐ 结局 2/3（搜索未果→DataInsufficient 素材）、
 *     碰撞要求在场但缺检测器（KIN-05：绝不视为无碰撞——本单元以
 *     "要求＋无会话"判定）或碰撞要求在场而构型评价未完成（WP-15-T07
 *     设施臂——"要求＋评价缺失"同素材面；稳定诊断码 KIN-COLLISION-
 *     UNAVAILABLE 的产码面已随 T07 落位，§9.6 任务列 T07 行）；
