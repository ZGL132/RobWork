 ...
 *
 * 设计依据：
 *   - units/kinematics.md §9.6（KIN- 稳定诊断码登记表——15 码/级别/语义/
 ...
 * 逐码给出级别/语义）。本头把 §9.6 **全表 15 行**的登记值物化为
 * CodeDescriptor（逐字段可追溯到卡面与 diagnostics 词表），装配期由 L5
 * 经 registerKinematicsCodes 注册——本实现不改动 diagnostics 单元任何
 * 文件，不私定任何卡面之外的码值。
 *
 * 阶段纪律（acceptance 4 括号内三个短语的执行口径）：
 *   1. "全表 15 个稳定码清单登记"——本头工厂清单恰含 §9.6 全表 15 行
...
// =====================================================================
// KIN-* 稳定诊断码描述符清单（§9.6 全表 15 行的物化；阶段纪律见文件头注）
// =====================================================================

/**
 * @brief 产出 kinematics §9.6 全表 15 码的稳定码描述符全集（契约
 *        acceptance 4"全表 15 个 KIN- 稳定码清单登记"的执行面）。
 *
 ...
 *   - severity＝§9.6 表"级别"列逐行原值（4 error＋11 warning——注册表
...
/**
 * @brief 将 §9.6 全表 15 码注册进稳定码注册表（§9.6 表头"装配期注册，
 *        ownerUnit=kinematics"的执行面——L5 装配清单的调用入口）。
 *
