 ...
 * @brief  modeling 跨单元契约测试（落位期）——构建图边界契约（任务契约
 *         WP-13-T02 acceptance 2/3 的具名自证面）。
 *
 * 设计依据：
...
{
    const auto cmakeFile = unitRoot() / "modeling" / "CMakeLists.txt";
    EXPECT_TRUE(fs::exists(cmakeFile)) << "modeling/CMakeLists.txt 不存在";
    std::ifstream in(cmakeFile, std::ios::binary);
    EXPECT_TRUE(static_cast<bool>(in)) << "无法读取 modeling/CMakeLists.txt";
...
/// 会误报）。目标名由单词边界分隔，逐字符扫描稳定实现（reporting 同款）。
...
/// 六条登记边单元（卡 §3.2 边表序——与白名单 "modeling->…" 六行同源）。
 ...
 * 链接成功自证（构建日志＝执行证据）。
 */
TEST(MdlBuildGraph, SixRegisteredEdgesPresent_WP13T02_ACC3)
{
    ...
    for (const char* u : kEdgeUnits) {
        EXPECT_NE(refs.find(std::string("sdurws_ird_") + u), refs.end())
            << "modeling→" << u << " 登记边缺失（modeling.md §3.2 边表/ARCH "
               "§3.5 许可方向实例化）";
    }
}
...
TEST(MdlBuildGraph, NoBusinessUnitOrExtraPlatformEdge_WP13T02_ACC2)
{
    ...
    const auto refs = collectTargetRefs(readCMakeLists());
    // R-1 面：业务域单元（modeling 自身除外）。
    for (const char* business :
         {"requirements", "kinematics", "trajectory", "dynamics",
...
/**
 * testkit 仅落在测试目标链接语句（T-1 具名自证——reporting 同款收窄
 * 重述）：testkit 不随产品分发（testkit.md §2.4 T-1）——产品目标
 * sdurws_ird_modeling 的链接面出现 testkit 即违约；测试目标
 * （_test/_contract_test）的报告设施消费是允许形态。
 */
TEST(MdlBuildGraph, NoTestkitEdgeOnProductTarget_WP13T02_ACC2)
{
    IRD_TEST_INFO(std::vector<std::string>{"ARC-02"},
                  std::vector<std::string>{});
...
/**
 * include 面与构建图同界（acceptance 2/3——源码面半区）：include/＋src/
 * 的 #include 指令行出现的单元段只允许 modeling 自身＋六条登记边单元
 * （与 BuildRedLineTest 的六边白名单同判据；此处以"集合封闭"口径复核
 * ——两个独立实现的同界断言互为防线）。业务用例级跨单元契约（fake 对
 * 端等）随 T03+ 在测试面建立，不改产品编译边。
 */
TEST(MdlBuildGraph, IncludeFaceMatchesRegisteredEdges_WP13T02_ACC3)
{
    IRD_TEST_INFO(std::vector<std::string>{"ARC-02"},
                  std::vector<std::string>{});

    const auto included = collectIncludedUnits();
    std::set<std::string> allowed = {"modeling"};
    for (const char* u : kEdgeUnits) {
        allowed.insert(u);
    }
