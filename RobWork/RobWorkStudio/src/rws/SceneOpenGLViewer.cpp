...
void SceneOpenGLViewer::init ()
{
    ...
    _pmap->getValue ()
        ...
        ->changedEvent ()
        .add (std::bind (&SceneOpenGLViewer::propertyChangedListener, this, _1), this);
...
SceneOpenGLViewer::SceneOpenGLViewer (QWidget* parent) :
    QOpenGLWidget (parent), _scene (ownedPtr (new SceneOpenGL ())), _viewLogo ("RobWork"),
...
void SceneOpenGLViewer::updateViewSize (int width, int height)
{
    const double scale = devicePixelRatioF ();
    _width            = std::max (1, static_cast< int > (std::round (width * scale)));
    _height           = std::max (1, static_cast< int > (std::round (height * scale)));
