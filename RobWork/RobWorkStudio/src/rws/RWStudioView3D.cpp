...
void RWStudioView3D::setupActions ()
{
    ...
    // wire action
    _showWireAction =
        new QAction (QIcon (":/images/wireframe.png"), tr ("&Wire"), this);    // owned
    _showWireAction->setCheckable (true);
    connect (_showWireAction, SIGNAL (triggered ()), this, SLOT (setDrawTypeSlot ()));

    // outline action
    _showOutlineAction =
        new QAction (QIcon (":/images/outline.png"), tr ("&Outline"), this);    // owned
    ...
    connect (
        _checkForCollision, SIGNAL (triggered (bool)), this, SLOT (setCheckForCollision (bool)));

    _saveBufferToFileAction = new QAction (tr ("Save view..."), this);    // owned
    connect (
        _saveBufferToFileAction, SIGNAL (triggered ()), this, SLOT (saveBufferToFileDialog ()));

    // view perspective
    _setPerspectiveViewAction = new QAction (
        QIcon (":/images/default_view_100.png"), tr ("&Perspective view"), this);    // owned
    connect (_setPerspectiveViewAction, SIGNAL (triggered ()), this, SLOT (setCheckAction ()));

    _setOrthographicViewAction = new QAction (
        QIcon (":/images/default_view_100.png"), tr ("&Orthographic view"), this);    // owned
    ...
    _frontViewAction = new QAction (
        QIcon (":/images/front_view_100.png"), tr ("Front (Ctrl + F)"), this);    // owned
    connect (_frontViewAction, SIGNAL (triggered ()), this, SLOT (setCheckAction ()));

    _rightViewAction = new QAction (
        QIcon (":/images/right_view_100.png"), tr ("Right (Ctrl + R)"), this);    // owned
    connect (_rightViewAction, SIGNAL (triggered ()), this, SLOT (setCheckAction ()));

    _topViewAction =
        new QAction (QIcon (":/images/top_view_100.png"), tr ("Top (Ctrl + T)"), this);    // owned
    connect (_topViewAction, SIGNAL (triggered ()), this, SLOT (setCheckAction ()));

    _rearViewAction = new QAction (
        QIcon (":/images/rear_view_100.png"), tr ("Rear (Ctrl + E)"), this);    // owned
    connect (_rearViewAction, SIGNAL (triggered ()), this, SLOT (setCheckAction ()));

    _leftViewAction = new QAction (
        QIcon (":/images/left_view_100.png"), tr ("Left (Ctrl + L)"), this);    // owned
    connect (_leftViewAction, SIGNAL (triggered ()), this, SLOT (setCheckAction ()));

    _bottomViewAction = new QAction (
        QIcon (":/images/bottom_view_100.png"), tr ("Bottom  (Ctrl + B)"), this);    // owned
    ...
    // Setup Zoom buttons:
    _zoomInAction =
        new QAction (QIcon (":/images/zoom_in.png"), tr ("Zoom in (Ctrl + +)"), this);    // owned
    connect (_zoomInAction, SIGNAL (triggered ()), this, SLOT (zoomInSlot ()));

    _zoomOutAction =
        new QAction (QIcon (":/images/zoom_out.png"), tr ("Zoom out (Ctrl + -)"), this);    // owned
    connect (_zoomOutAction, SIGNAL (triggered ()), this, SLOT (zoomOutSlot ()));

    // outline action
    _zoomAutoAction = new QAction (
        QIcon (":/images/zoom_auto.png"), tr ("Autozoom (Ctrl + *)"), this);    // owned
...
RWStudioView3D::RWStudioView3D (RobWorkStudio* rwStudio, QWidget* parent) :
    QWidget (parent), _view (NULL), _wcscene (NULL), _wc (NULL), _rws (rwStudio),
    _viewLogo ("RobWork")
{
...
void RWStudioView3D::setWorkCell (rw::models::WorkCell::Ptr wc)
{
    ...
    if (!_wcscene->getWorldNode ()->hasChild ("FloorGrid")) {
        ...
        _floorDrawable = _wcscene->addLines ("FloorGrid",
                                             Line::makeGrid (10, 10, 0.5, 0.5),
                                             _wc->getWorldFrame (),
                                             DrawableNode::Virtual);
        _floorDrawable->setColor (Vector3D<> (0.8f, 0.8f, 0.8f));
    }
    // look for all cameras in the scene

    std::vector< Frame* > frames =
        Kinematics::findAllFrames (_wc->getWorldFrame (), _wc->getDefaultState ());
    for (Frame* frame : frames) {
        if (frame->getPropertyMap ().has ("Camera")) {
            ...
            if (!_wcscene->findDrawable (camName, frame)) {
                SensorCameraView view = makeCameraView (frame->getName () + "Cam",
                                                        fovy,
                                                        (double) width,
                                                        (double) height,
                                                        0.001,
...
void RWStudioView3D::keyPressEvent (QKeyEvent* e)
{
    ...
    // Rotate view commands:
        ...
        Rotation3D<> rot =
            EAA<> (Tview.R () * Vector3D<>::x (), -ROTATE_VIEW_STEP_DEG * Deg2Rad).toRotation3D ();
        ...
        rw::math::Transform3D<> Tview = _view->getTransform ();
        Rotation3D<> rot =
            EAA<> (Vector3D<>::z (), -ROTATE_VIEW_STEP_DEG * Deg2Rad).toRotation3D ();
        rw::math::Transform3D<> newTview = Transform3D<> (rot * Tview.P (), rot * Tview.R ());
        _view->setTransform (newTview);
        _view->updateView ();
    }
    ...
    // change camera view according to the keyboard inputs
        ...
        if (camNr == 0) {
            ...
            if (camNr - 1 < int (_sensorCameraViews.size ())) {
                ...
                Log::infoLog () << "Camera view \""
                                << _sensorCameraViews[camNr - 1].first._view->_name
                                << "\" selected!\n";
                _view->updateView ();
            }
            else {
...
{
    // if collision detection is enabled then run it now, and highlight any frames that are
    // overlapping
...
void RWStudioView3D::setMaskCheckAction ()
{
    ...
    if (obj == _physicalMaskEnabled) {
        _rws->getPropertyMap ()
            .get< PropertyMap > ("SceneViewer")
            .set< bool > ("ShowPhysicalModels", _physicalMaskEnabled->isChecked ());
    }
    else if (obj == _virtualMaskEnabled) {
        _rws->getPropertyMap ()
            .get< PropertyMap > ("SceneViewer")
            .set< bool > ("ShowVirtualModels", _virtualMaskEnabled->isChecked ());
    }
    else if (obj == _drawableMaskEnabled) {
        _rws->getPropertyMap ()
            .get< PropertyMap > ("SceneViewer")
            .set< bool > ("ShowDrawableModels", _drawableMaskEnabled->isChecked ());
    }
    else if (obj == _collisionMaskEnabled) {
        _rws->getPropertyMap ()
            .get< PropertyMap > ("SceneViewer")
            .set< bool > ("ShowCollisionModels", _collisionMaskEnabled->isChecked ());
    }
...
void RWStudioView3D::setCheckAction ()
{
    ...
    if (obj == _axometricViewAction) {
        _view->setTransform (Transform3D<>::makeLookAt (Vector3D<> (v2c.norm2 () / sqrt (3.0),
                                                                    v2c.norm2 () / sqrt (3.0),
                                                                    v2c.norm2 () / sqrt (3.0)),
                                                        center,
                                                        Vector3D<>::z ()));
        ...
        _view->setTransform (
            Transform3D<>::makeLookAt (Vector3D<> (v2c.norm2 (), 0, 0), center, Vector3D<>::z ()));
    }
        ...
        _view->setTransform (
            Transform3D<>::makeLookAt (Vector3D<> (0, 0, v2c.norm2 ()), center, -Vector3D<>::x ()));
    }
        ...
        if (nrView < 10) {
            ...
            if (dialog->exec () == QDialog::Accepted) {
                ...
                if (frame != NULL) {
                    ...
                    SensorCameraView view = makeCameraView (name,
                                                            map.get< double > ("Fovy"),
                    ...
                    // set params
                    RenderCameraFrustum::Ptr camFrustum = ownedPtr (new RenderCameraFrustum ());
                    camFrustum->setPerspective (
                        view._width / view._height, view._fovy, view._near, view._far);
        ...
        _view->getViewCamera ()->setProjectionMatrix (
            ProjectionMatrix::makeOrtho (-1.0, 1.0, -1.0, 1.0, 0.1, 30));
    }
    else if (obj == _setPerspectiveViewAction) {
        int x, y, w, h;
        _view->getViewCamera ()->getViewport (x, y, w, h);
        _view->getViewCamera ()->setProjectionMatrix (
            ProjectionMatrix::makePerspective (45, w / ((double) h), 0.1, 30));
    }
    else {
        for (size_t i = 0; i < _customViews.size (); i++) {
...
RWStudioView3D::SensorCameraView RWStudioView3D::makeCameraView (const std::string& name,
                                                                 double fovy, double w, double h,
                                                                 double n, double f, Frame* frame)
{
    ...
    view._view->_viewCamera->setPerspective (
        view._fovy, view._width, view._height, view._near, view._far);
...
{
    rw::math::Transform3D<> Tview = _view->getTransform ();
    rw::math::Transform3D<> newTview =
        Transform3D<> (1.0 / ZOOM_STEP_MULT * Tview.P (), Tview.R ());
    _view->setTransform (newTview);
    _view->updateView ();
}
...
void RWStudioView3D::saveBufferToFileDialog ()
{
    ...
    QString filename = QFileDialog::getSaveFileName (
        ...
        "Save Image",
        lastDir.c_str (),
        "Images (*.png *.bmp *.jpg *.jpeg *.gif *.ppm *.tiff *.xpm *.xbm)");
