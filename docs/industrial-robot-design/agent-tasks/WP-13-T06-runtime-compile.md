...
- **允许创建/修改/删除的文件：**创建 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/modeling/include/sdurws/ird/modeling/ModelingCompileRequest.hpp`；`modeling/src/ModelingCompileRequest.cpp`；`modeling/test/RuntimeCompileTest.cpp`；`modeling/evidence/WP-13/T06/`；`modeling/CMakeLists.txt`（仅追加本任务文件；WP-06 端口契约三例入 `sdurws_ird_modeling_contract_test`）。禁止删除任何文件
...
- **精确验证命令**（仓库根；含 WP-06 端口契约测试）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`grep -rn "rw::\|<rw/" modeling/include/sdurws/ird/modeling/ modeling/src/ModelingCompileRequest.cpp` 零命中（RobWork 指针只存在于 WP-06 builder 内）；`grep -rn "+ *\".\" *\|substr.*prefix\|removePrefix" modeling/src/` 零命中（无名称拼接/剥离）；`CompiledCandidateArtifact` 不得出现于本模块（symbol-registry 裁决 5）
- **证据工件：**`modeling/evidence/WP-13/T06/`——failpoint 注入日志（双侧）、名称映射双向表、交叉校验清单、编译日志与修订计数
- **提交格式：**`WP-13-T06: compile runtime artifacts`
