...
- **允许创建/修改/删除的文件：**（基目录 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/kinematics/`）
  ...
  - 创建：`test/FkTest.cpp`；`testdata/kinematics/fk-golden/`（Zero/Home/边界/固定种子姿态解析对照夹具，登记进 WP-02 manifest）；`out/test-evidence/wp-15/<run-id>/`
...
- **证据工件：** `out/test-evidence/wp-15/<run-id>/fk-analytic-report.md`（姿态清单、解析值、最大误差、种子）＋测试日志（命令、commit、配置、manifest 哈希）；独立验证者复核解析对照。
- **提交格式：** `WP-15-T01: 基于规范模型的正运动学与解析夹具`
