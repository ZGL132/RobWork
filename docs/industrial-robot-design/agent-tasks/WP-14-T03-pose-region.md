...
- **精确验证命令**（仓库根）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "coverage|jacobian|samplePose" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/requirements/src/PoseRegion.cpp; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 零命中（覆盖率定义与子空间构造不归本卡）；上限 `1000000` 常量仅出现一处（冻结常量）
