...
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "csv|fopen|QFile" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/selection/src/CatalogView.cpp; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 零命中（无文件 IO，一律经 WP-11）；`rg -n "IRD-SEL-" RobWork/RobWorkStudio/src/rwslibs/industrialrobot -g '*.?pp'; if ($LASTEXITCODE -ne 0) { throw '预期命中缺失或扫描命令执行失败' }` 命中仅在本模块
