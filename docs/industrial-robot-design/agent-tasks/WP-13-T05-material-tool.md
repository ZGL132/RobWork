...
- **精确验证命令**（仓库根）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`grep -rn "d */ *2\|0.5 *\* *d" modeling/src/MaterialInertiaEstimator.cpp` 命中处唯一（单一 r=d/2 转换点）；`grep -rn "geometry.*copy\|cloneGeometry" modeling/src/ToolEnvironmentEditor.cpp` 零命中（TCP 不复制几何，MDL-13）；`grep -rn "ofstream\|QFile" modeling/src/MaterialInertiaEstimator.cpp modeling/src/ToolEnvironmentEditor.cpp` 零命中
- **证据工件：**`modeling/evidence/WP-13/T05/`——三类截面解析算例对照表（公式值 vs 实现值）、口径转换记录、覆盖前后来源快照、诊断样本
- **提交格式：**`WP-13-T05: add material and tool model`
