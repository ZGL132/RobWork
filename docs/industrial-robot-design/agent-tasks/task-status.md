...
| WP-01-T04 | Ready | - | 独立验证者/治理协调（ZCode 治理会话，2026-08-31 门禁判定） | 2026-08-31 | 前置 WP-01-T03 `Done`（c161d6d），卡内前置工件四入口脚本＋check-boundaries.ps1＋test.log 均随实现提交/治理证据在库；DOCUMENT-BASELINE §4 九项门禁逐条判定通过：①需求 v0.8 `Accepted`；②testing-contract（CTR-TST-001/002）`Accepted`（IRD-D14），无直接关联 ADR；③前置 Done＋工件在；④允许/禁止文件明确（新增 gitlab-ci yml＋package.ps1，禁改 industrialrobot/、旧插件与 T01/T03 既有脚本签名）；⑤CI 步骤顺序、模型/GUI 分 job、缓存白名单与安装 manifest 排除项由卡唯一冻结；⑥验证命令完整（run-tests/check-boundaries/package 三脚本＋原生 ctest 回退），package.ps1 为本卡自有资产、RED（t04-fail-blocks）以既有入口可先行执行；⑦NFR-DEP-01/02 追踪链路经 T03 验证确认闭合；⑧文档门禁双 PowerShell（5.1 与 pwsh 7）退出码 0（2026-08-31 复跑，128 需求/19 验收测试/25 契约/0 追踪缺口），代码基线随 c161d6d 快进与文档基线 main HEAD 一致；⑨无遗留跨模块语义决定；"Runner 无 Windows x64 执行机"等属卡内停止/升级条件（实施期风险），不构成入口门禁阻塞 |
