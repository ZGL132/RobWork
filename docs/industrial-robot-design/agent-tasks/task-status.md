...
| WP-01-T03 | Ready | - | 独立验证者/治理协调（ZCode 治理会话，2026-08-31 门禁判定） | 2026-08-31 | 前置 WP-01-T02 `Done`（2443d99＋结构修订 2287b55），卡内两工件真实存在（可配置构建的 out/build/industrial-robot 含 sdurws_ird_core 与 core_test CTest 注册——本次验证已实测配置/构建/CTest 1/1，industrialrobot/CMakeLists.txt 选项集在库）；DOCUMENT-BASELINE §4 九项门禁逐条判定通过：①需求 v0.8 `Accepted`；②testing-contract／public-interfaces 均 `Accepted`（IRD-D14），无直接关联 ADR；③前置 Done＋工件在；④允许/禁止文件明确（三入口脚本新增、common.ps1 仅追加不改既有签名、禁改 industrialrobot/ 与旧插件）；⑤参数/行为契约由卡与 WP-01 计划 §5.1 唯一冻结；⑥验证命令完整且 RED 断言给出（入口缺失失败）；⑦追踪闭合（NFR-MNT-01、NFR-DEP-01 链路 T04 评审通过）；⑧文档门禁退出码 0（2026-08-31 复跑）；⑨无遗留跨模块语义决定；环境：VS 2022 17.12＋CMake 4.3.1＋vswhere 可用 |
