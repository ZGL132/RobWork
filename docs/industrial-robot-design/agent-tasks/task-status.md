...
| WP-01-T01 | Blocked | 契约/ADR 签署待用户裁决（DOCUMENT-BASELINE §4 门禁 2；AGENTS §3.1） | 独立验证者/治理协调（ZCode 治理会话，2026-08-30 门禁判定） | 2026-08-30 | 前置 WP-00-T04 已 Done 且卡内要求的两份证据工件（t03-gate-and-fixtures.md、t04-independent-review.md）真实存在；阻塞原因：卡引用架构契约 public-interfaces.md／testing-contract.md（CTR-API-001、CTR-TST-001、CTR-TST-002）及全部 ADR 均为 Proposed（IRD-D10-20260829 联合评审通过待签署），DOCUMENT-BASELINE §4 门禁 2（引用契约与 ADR 均为 Accepted）不满足；按 AGENTS §3.1 本任务非 WP-00 治理/自举任务，治理上下文不得自行授予豁免；解除条件：用户签署全部契约与 ADR 升 Accepted，或明确指定本任务豁免（任务 ID＋契约 ID＋允许范围＋有效期）；解除后由治理上下文按九项门禁复判转 Ready |
