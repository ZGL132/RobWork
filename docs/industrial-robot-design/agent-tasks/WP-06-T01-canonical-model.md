...
- **允许创建/修改/删除的文件：**根 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/runtime/` 下 `include/sdurws/ird/runtime/CanonicalKinematicModel.hpp`、`src/CanonicalModel.cpp`、`src/RuntimeJson.cpp`（canonical 序列化部分）、`test/CanonicalModelTest.cpp`、`testdata/runtime/{dh,explicit,urdf}/`；修改 `plugins/../runtime/CMakeLists.txt`（即 runtime 模块根 `industrialrobot/runtime/CMakeLists.txt`，用于登记 `sdurws_ird_runtime` 目标与测试源）
...
- **修改后接口：**`CanonicalModelCompiler::compile(const RobotDesign& robot, const CompileContext& context) -> expected<CanonicalKinematicModel, CompileError>`（模块私有；`CompileContext{projectId, revisionId, tools[], environments[]}` 由调用方 WP-13/WP-20 从项目修订装配，RobotDesign 不内嵌修订与工具/环境——persistence-schema §2.4 裁决）；`CanonicalKinematicModel` 字段为 WP-06 计划 §4 冻结集（projectId/revisionId/robotDesignId/baseFrame/links[]/joints[]/tools[]/environments[]/sourceFormat（枚举 `StandardDH | ExplicitJoint | UrdfImport`）/algorithmVersion；Joint 无 `zero` 字段——偏置全部折叠进 OriginPose，`home` 由 `RobotDesign.homePosition` 逐轴映射）；`RuntimeJson` 输出确定性 canonical JSON 供 T03 双编译与 WP-08 缓存键消费
