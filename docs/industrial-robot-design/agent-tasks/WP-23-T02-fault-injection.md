...
- 产出：注入保存、进程、缓存、检查点、外部资源和报告渲染故障。 以及可审计测试和结构化证据。
...
- 命令：`powershell -NoProfile -ExecutionPolicy Bypass -File .\RobWork\scripts\industrial-robot\run-tests.ps1 -Configuration Debug -Regex '^sdurws_ird_fault_injection_test$'`；脚本尚未创建时先执行 WP-01 交付的同名入口。
- 证据：模块测试日志、契约测试结果、黄金数据或人工复核报告，包含 commit、配置、种子和输入快照身份。
- 提交：`WP-23-T02: 故障注入与恢复`
