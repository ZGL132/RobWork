...
- **精确验证命令：**（仓库根、VS x64 环境；GUI 相关断言单独启动、一次一个并设 `QT_QPA_PLATFORM=windows`）
  ...
  - 回退：`cmake --build out\build\industrial-robot --config Debug --target sdurws_ird_fault_injection_test`
  - 回退：`ctest --test-dir out\build\industrial-robot -C Debug -R "^sdurws_ird_fault_injection_test$"`
...
- **证据工件：** `out/test-evidence/wp-23/<run-id>/t02-fault-injection.log`：八类矩阵逐条注入/恢复日志、三演练场景记录与恢复统计（批次不重复证明）、A-GATE-04/05 复验结果、命令原文与 commit。
