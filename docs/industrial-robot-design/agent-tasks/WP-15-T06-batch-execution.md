...
- **证据工件：** `out/test-evidence/wp-15/<run-id>/batch-execution-report.md`（身份校验矩阵、迟到回调/取消/检查点日志、固定种子复现对照、`KinematicsSettings` 进缓存键证明）＋测试日志（命令、commit、配置、manifest 哈希）；独立验证者复核身份与确定性。
- **提交格式：** `WP-15-T06: 集成批量验证执行`
