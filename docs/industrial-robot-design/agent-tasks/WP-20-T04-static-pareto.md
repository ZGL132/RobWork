...
- **证据工件：**`plugins/optimization/evidence/WP-20/T04/`——静态 Pareto 黄金集（含支配/并列/容差边界）、指标来源对照、`comparisonTolerance` 取值评审（优化工程师签署）、测试日志
- **提交格式：**`WP-20-T04: implement static pareto`
