...
- 产出：实现双 PowerShell 兼容、VS x64、Qt windows 平台和单进程 GUI 规则。 以及可审计测试和结构化证据。
...
- 命令：`powershell -NoProfile -ExecutionPolicy Bypass -File .\RobWork\scripts\industrial-robot\run-tests.ps1 -Configuration Debug -Regex '^sdurws_ird_test_entry_test$'`；脚本尚未创建时先执行 WP-01 交付的同名入口。
