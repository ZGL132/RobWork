...
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "set[A-Z]" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/selection/src/CatalogView.cpp; if ($LASTEXITCODE -gt 1) { throw '扫描命令执行失败' }` 对 `CatalogVersion` 无变更方法；历史结果文件在测试前后 `git status` 零变化（payload 不回写）；无就地刷新入口
