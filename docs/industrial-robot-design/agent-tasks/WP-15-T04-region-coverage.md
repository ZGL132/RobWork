...
- **证据工件：** `evidence/WP-15/coverage-matrix.md`（分母/边界/两锚点/Partial 禁 Verified 结果矩阵、种子）＋测试日志（命令、commit、配置、manifest 哈希）；独立验证者复核覆盖率口径。
- **提交格式：** `WP-15-T04: implement region coverage`
