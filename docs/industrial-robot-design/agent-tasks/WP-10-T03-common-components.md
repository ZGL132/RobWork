...
- **允许创建/修改/删除的文件：**创建 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/ui/include/sdurws/ird/ui/StageNavigationView.hpp`、`DiagnosticPanel.hpp`、`EngineeringTableView.hpp`；`ui/src/DiagnosticPanel.cpp`、`EngineeringTableView.cpp`；`ui/test/CommonComponentsTest.cpp`；`ui/testdata/components/`；`ui/evidence/WP-10/T03/`；`ui/CMakeLists.txt`（仅追加本任务文件到 `sdurws_ird_ui`/`sdurws_ird_ui_widget_test`）。禁止删除任何文件
...
- **精确验证命令**（仓库根；GUI 测试须 Visual Studio x64 环境，`QT_QPA_PLATFORM=windows`，一次只启动一个 GUI 测试可执行文件）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`grep -rn "evaluate\|collision\|ik(" ui/src/DiagnosticPanel.cpp ui/src/EngineeringTableView.cpp` 零命中（无业务算法）；`grep -rn "QStringLiteral.*失败\|错误：" ui/src/DiagnosticPanel.cpp` 零命中（无硬编码文案）
- **证据工件：**`ui/evidence/WP-10/T03/`——组件状态截图、批量粘贴矩阵（合法/非法行组合）、诊断映射对照表、评审者签署
- **提交格式：**`WP-10-T03: implement common engineering components`
