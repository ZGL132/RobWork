...
- **基线 commit：**代码 `94fb910e8d4b1e2bb84d569cbca4aa623cbd2844`；语义源 `module-design/session-ui.md` v0.4 §8.3～§8.9
...
- **修改后接口：**`StageNavigationView`（每阶段状态＋问题数＋下一步建议，消费 `StageStatusModel` 八值）；`DiagnosticPanel`（右侧与底部共用，经 `IDiagnosticCatalog.lookup` 渲染，禁止本地文案表）；`EngineeringTableView`（批量粘贴、SI 单位显示、逐行诊断、分页接口 `setPage/window`）；三组件状态、按钮和空态遵循 session-ui v0.4 §8.3～§8.9。
