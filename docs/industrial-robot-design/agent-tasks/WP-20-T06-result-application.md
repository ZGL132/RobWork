...
- **精确验证命令**（仓库根、VS x64；三形式，仅用登记目标）：
  ...
  - 预期退出码 0
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "createRevision|writeRevision|new ProjectRevision" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/optimization/candidate/src; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 零直接修订写入（只经 WP-04 命令）；`rg -n "OptimizationStudy\b" RobWork/RobWorkStudio/src/rwslibs/industrialrobot; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 禁名零命中；无第二套结果仓库
