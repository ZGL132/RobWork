...
- **前置任务及必需工件：** WP-01-T04；工件：`RobWork/scripts/industrial-robot/check-boundaries.ps1`（可扩展）、`out\build\industrial-robot` 配置日志（含生成器/架构/选项证据）与 `gitlab-ci/industrial-robot-windows.yml`。
- **允许创建/修改/删除的文件：**
...
- **实施步骤：**
  ...
  2. 采集基线：RobWork/RobWorkStudio/RobWorkSim 锁定 commit（94fb910e8d4b1e2bb84d569cbca4aa623cbd2844 及仓库实际状态）、Qt 版本、MSVC 与 Windows SDK 版本、碰撞检测后端及版本、第三方组件与许可证。
...
- **证据工件：** `industrial-robot-baseline.json`（提交物）、`out/logs/industrial-robot/<timestamp>/dependency-audit.log`（两条 RED 断言前后结果）、版本采集记录（MSVC/SDK/Qt/碰撞后端命令输出）、独立评审签署（评审者非采集者）。
