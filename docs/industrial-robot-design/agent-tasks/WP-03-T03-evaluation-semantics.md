...
- **基线 commit：**代码 `94fb910e8d4b1e2bb84d569cbca4aa623cbd2844`；文档：requirements v0.7、检查点 `IRD-D2-20260829`、architecture/evaluation-semantics.md（CTR-DOM-004）§1～§4 为唯一权威、core-domain v0.3。
- **前置任务及必需工件：**WP-03-T01（`Diagnostic` 错误通道与值类型）；WP-02-T02（参数化测试框架）；WP-01-T03（测试入口）。
- **允许创建/修改/删除的文件**（模块根同 WP-03-T01）：创建 `include/sdurws/ird/core/EvaluationSemantics.hpp`、`test/SemanticsTest.cpp`、`evidence/WP-03/`；修改 `CMakeLists.txt`、`DomainValidation.hpp/.cpp`（组合校验段）；删除：无。
...
- **diff 和禁止项检查：**diff 仅命中允许清单；枚举值域与 §1 逐字一致；全仓只有一处正式可行谓词定义；`EvaluationEnvelope` 等禁止名称零命中（grep 校验）。
- **证据工件：**`evidence/WP-03/T03/`：60 组合矩阵日志、谓词正反例（含警告类别边界与证据等级差一档）、profile 唯一性记录。
- **提交格式：**`WP-03-T03: evaluation semantics`。
