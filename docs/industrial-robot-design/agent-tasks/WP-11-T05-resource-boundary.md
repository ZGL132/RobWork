...
- **精确验证命令**（仓库根）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`grep -rn "setNetworkAccessible\|QNetworkAccessManager\|CommandLine" io/src/` 零命中；`grep -rn "DOCTYPE\|ExternalEntity" io/src/` 命中处仅限禁用配置；`grep -rn "objects/" io/src/` 零命中
- **证据工件：**`io/evidence/WP-11/T05/`——恶意样本清单与哈希、预算消耗、路径规范化结果、逐行诊断 JSON、命令日志
- **提交格式：**`WP-11-T05: enforce URDF mesh JSON boundaries`
