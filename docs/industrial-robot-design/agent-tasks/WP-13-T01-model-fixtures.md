...
- **允许创建/修改/删除的文件：**创建 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/modeling/test/ModelFixtureTest.cpp`；`modeling/testdata/modeling/` 下 `dh/`、`explicit/`、`urdf/`、`axes/`、`branches/`、`materials/`、`failpoints/` 七类夹具＋夹具哈希清单；`modeling/evidence/WP-13/T01/`；`modeling/CMakeLists.txt`（登记 `sdurws_ird_modeling`、`sdurws_ird_modeling_test`、`sdurws_ird_modeling_contract_test` 骨架）。禁止删除任何文件
...
- **精确验证命令**（仓库根；夹具往返先过 Schema 校验）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；夹具哈希清单与文件一一对应（`git status` 无未登记新夹具）；`grep -rn "RobotDesignEditor\|UrdfImportAdapter" modeling/test/ModelFixtureTest.cpp` 零命中（夹具独立于实现）
- **证据工件：**`modeling/evidence/WP-13/T01/`——夹具清单与哈希、失败断言输出、Schema 校验日志
- **提交格式：**`WP-13-T01: add modeling fixtures`
