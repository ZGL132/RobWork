...
- **diff 和禁止项检查：**`git diff --name-only` 仅含 `evaluation/drivetrain/` 新建文件与 `industrialrobot/CMakeLists.txt` 一处接入；`rg -n "IRD-DTM-" RobWork/RobWorkStudio/src/rwslibs/industrialrobot -g '*.?pp'; if ($LASTEXITCODE -ne 0) { throw '预期命中缺失或扫描命令执行失败' }` 命中仅在 drivetrain 目录（诊断码唯一归属）；源码无 `QtWidgets` include
