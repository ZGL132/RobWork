...
- 产出：从需求生成 CSV 并验证 124 行、唯一 ID、发布和阶段字段。 以及可审计测试和结构化证据。
...
- 命令：`powershell -NoProfile -ExecutionPolicy Bypass -File .\RobWork\scripts\industrial-robot\run-tests.ps1 -Configuration Debug -Regex '^sdurws_ird_generate_trace_test$'`；脚本尚未创建时先执行 WP-01 交付的同名入口。
