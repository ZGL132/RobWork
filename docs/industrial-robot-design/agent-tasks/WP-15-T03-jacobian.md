...
- **允许创建/修改/删除的文件：**（基目录 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/kinematics/`）
  ...
  - 创建：`test/JacobianTest.cpp`、`testdata/kinematics/jacobian/`（解析对照夹具，登记 WP-02 manifest）、`out/test-evidence/wp-15/<run-id>/`（本卡工件）；不删除文件。
...
- **证据工件：** `out/test-evidence/wp-15/<run-id>/jacobian-analytic-report.md`（`J_norm`/`L*`/条件数解析对照、最大误差、回退案例）＋测试日志（命令、commit、配置、manifest 哈希）；独立验证者复核解析对照与 §15.3 尺度一致性。
- **提交格式：** `WP-15-T03: 实现 Jacobian 与可操作度`

  - 新增 Jacobian 计算与 L* 退化处理
  - 新增解析对照测试
  - 新增运行证据记录
