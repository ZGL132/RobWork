...
- **正常/边界/失败测试：**
  ...
  - 边界（2026-08-31 二次治理裁决，恢复原预期）：Given `IRD_BUILD_UI=OFF`（`WITH_RWS=ON`），When 配置构建，Then UI 被跳过、其余 9 个模块目标与测试正常生成，无半目标。Given `WITH_RWS=OFF`，When 配置构建，Then `sdurws_ird_core` 等仍可构建且无 UI 目标（无半目标）。结构依据：同日治理授权在顶层 `RobWork/CMakeLists.txt` 于 `if(WITH_RWS)` 门控之外追加 `add_subdirectory(RobWorkStudio/src/rwslibs/industrialrobot)`，使 IRD 模块独立于 Studio 构建（§2.1：core 不依赖 RobWorkStudio）；`industrialrobot/CMakeLists.txt` 以 GLOBAL 属性守卫防止 WITH_RWS=ON 时经 rwslibs 与顶层双重注册。
