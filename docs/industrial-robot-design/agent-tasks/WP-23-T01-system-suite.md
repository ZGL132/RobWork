...
- **精确验证命令：**（仓库根、VS x64 环境；GUI 相关 AT 单独启动、一次一个并设 `QT_QPA_PLATFORM=windows`）
  ...
  - 回退：`cmake --build out\build\industrial-robot --config Debug --target sdurws_ird_system_suite_test`
  - 回退：`ctest --test-dir out\build\industrial-robot -C Debug -R "^sdurws_ird_system_suite_test$"`
...
- **证据工件：** `out/test-evidence/wp-23/<run-id>/t01-at-matrix.log`：AT-01～19 执行矩阵（每条含覆盖需求/入口/数据集/判据/结果/seed/threadCount/输入哈希/命令原文与 commit）。
