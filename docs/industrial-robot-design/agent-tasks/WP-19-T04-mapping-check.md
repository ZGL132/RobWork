...
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -ni "eta|inertia|i \* i|ratio \*" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/selection/src; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 无本地映射/惯量公式（静态扫描零命中，ADR-004）；`DynamicResult` 字段只读（无 setter 调用）
