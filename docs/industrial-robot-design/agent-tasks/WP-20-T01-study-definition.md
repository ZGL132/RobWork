...
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "OptimizationStudy\b" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/optimization; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 无单独 `OptimizationStudy` 类型（禁止名，SYM 裁决 #9）；无反射式字段写入（注册表外路径全部拒绝）；`joint/` 目录零新增
