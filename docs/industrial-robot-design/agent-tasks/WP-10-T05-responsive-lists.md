...
- **基线 commit：**代码 `94fb910e8d4b1e2bb84d569cbca4aa623cbd2844`；语义源 `module-design/session-ui.md` v0.3、`architecture/testing-contract.md` §2
...
- **允许创建/修改/删除的文件：**创建 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/ui/include/sdurws/ird/ui/VirtualResultModel.hpp`；`ui/src/VirtualResultModel.cpp`；`ui/test/ResponsiveListsTest.cpp`；`ui/testdata/perf/`（5,000 任务/100,000 摘要/10,000 候选生成器，不含手工巨文件）；`ui/evidence/WP-10/T05/`；`ui/CMakeLists.txt`（仅追加本任务文件）。禁止删除任何文件
...
- **正常/边界/失败测试：**
  ...
  - 边界：Given 导航/选择/筛选/编辑操作序列，When 测量，Then P95 ≤ 200 ms；超过 1 s 的工作全部转后台；主线程连续阻塞 ≤ 2 s
...
- **精确验证命令**（仓库根；GUI 测试须 Visual Studio x64 环境，`QT_QPA_PLATFORM=windows`，一次只启动一个 GUI 测试可执行文件；性能数据集与统计口径引用 `benchmark-manifest.json`）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`grep -rn "evaluate\|readFile\|QFile" ui/src/VirtualResultModel.cpp` 零命中（主线程无评估/IO）；`git status` 确认 testdata 生成器不产生入库巨文件
- **证据工件：**`ui/evidence/WP-10/T05/`——规模与实例化行数对照、P50/P95 表、主线程阻塞窗口日志、后台转移计数、环境（CPU/内存/Qt 版本）记录与评审者签署
- **提交格式：**`WP-10-T05: implement virtualized responsive lists`
