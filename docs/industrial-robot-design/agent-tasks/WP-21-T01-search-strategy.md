...
- 产出：冻结外层结构/传动探索、内层 Quick/Verified/器件匹配策略和预算。 以及可审计测试和结构化证据。
...
- 命令：`powershell -NoProfile -ExecutionPolicy Bypass -File .\RobWork\scripts\industrial-robot\run-tests.ps1 -Configuration Debug -Regex '^sdurws_ird_search_strategy_test$'`；脚本尚未创建时先执行 WP-01 交付的同名入口。
