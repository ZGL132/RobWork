...
- **允许创建/修改/删除的文件：**（前缀 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/optimization/`）创建 `joint/include/sdurws/ird/opt/JointSearchOrchestrator.hpp`、`joint/src/JointSearchOrchestrator.cpp`、`test/JointSearchTest.cpp`、`testdata/optimization/` 下本任务夹具；修改本模块 CMakeLists（登记 joint 源与目标）；写 `evidence/WP-21/`。不删除任何文件。
...
- **证据工件：** `evidence/WP-21/t01-search-strategy.log`：命令原文、commit、种子、线程数、外层样本身份、保留池快照、同种子两轮复现对照表。
