...
- **精确验证命令**（仓库根、VS x64；三形式，仅用登记目标）：
  ...
  - 预期退出码 0
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "PreliminaryDesign|ExternallyValidated" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/dynamics/src/DynamicsEvaluator.cpp; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 无自动提升路径（提升仅由用户确认/独立对照输入触发）；七码与 dynamics.md §4 矩阵逐字一致；无诊断文案硬编码（经 WP-09 目录）
