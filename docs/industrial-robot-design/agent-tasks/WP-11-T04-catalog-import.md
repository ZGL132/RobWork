...
- **允许创建/修改/删除的文件：**创建 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/io/include/sdurws/ird/io/CatalogPackageReader.hpp`；`io/src/CatalogPackageReader.cpp`；`io/test/CatalogImportTest.cpp`；`io/test/IoContractFixture.cpp`（追加本端口三例）；`io/testdata/io/catalog/`（合法包＋六类损坏包样本）；`io/evidence/WP-11/T04/`；`io/CMakeLists.txt`（仅追加本任务文件）。禁止删除任何文件
...
- **精确验证命令**（仓库根）：
  ...
  ```
- **diff 和禁止项检查：**diff 仅含允许清单；`grep -rn "rank\|score\|select\|filter" io/src/CatalogPackageReader.cpp` 零命中（无筛选语义）；`grep -rn "revision\|commit" io/src/CatalogPackageReader.cpp` 零命中（不写修订）
- **证据工件：**`io/evidence/WP-11/T04/`——manifest/哈希清单、跨表引用图、六类失败包诊断样本、重复导入一致性记录
- **提交格式：**`WP-11-T04: implement catalog package validation`
