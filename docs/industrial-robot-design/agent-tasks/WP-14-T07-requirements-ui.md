...
- **允许创建/修改/删除的文件：**创建 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/requirements/gui/RequirementsPlugin.hpp`、`gui/RequirementsPlugin.cpp`、`gui/panels/`；`requirements/test/RequirementsGuiTest.cpp`；`requirements/evidence/WP-14/T07/`；`requirements/CMakeLists.txt`（登记 `sdurws_ird_requirements_plugin`、`sdurws_ird_requirements_gui_test`）。禁止删除任何文件
...
- **证据工件：**`requirements/evidence/WP-14/T07/`——GUI 回归录屏/截图（批量编辑/筛选/单位显示/错误定位/就绪摘要）、AT-02/AT-03 断言输出、评审者签署
- **提交格式：**`WP-14-T07: add requirements ui`
