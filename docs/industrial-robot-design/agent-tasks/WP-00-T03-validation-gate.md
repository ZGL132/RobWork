...
- **前置任务及必需工件：** WP-00-T02；工件：`docs/industrial-robot-design/requirement-traceability.csv`（124 行、与生成器输出逐字节一致）与 `docs/industrial-robot-design/evidence/wp-00/t02-generation-log.md`。
- **允许创建/修改/删除的文件：**
  ...
  - 创建：`docs/industrial-robot-design/evidence/wp-00/t03-gate-and-fixtures.md`
...
- **修改前接口：** `validate-development-docs.ps1` 无参数、路径全部由 `$PSScriptRoot` 派生；成功行 `124 requirements, 19 acceptance tests, <派生> contracts, <派生> symbols, <派生> ADRs, 0 trace gaps`；失败经 `Write-Error` 累积后 `exit 1`。
...
- **实施步骤：**
  ...
  2. 实现基础门禁：必需路径、`benchmark-manifest.json` 字段、124/19 计数、CSV 字段/行/ID/release/phase 检查。
...
- **diff 和禁止项检查：** `git diff --name-only` 仅含 `validate-development-docs.ps1`、`fixtures/wp-00/`、`evidence/wp-00/t03-gate-and-fixtures.md`；`requirements.md`、`requirement-traceability.csv`、`generate-traceability.ps1` 零变化；夹具为最小副本，不含完整正式文档拷贝。
- **证据工件：** `docs/industrial-robot-design/evidence/wp-00/t03-gate-and-fixtures.md`：门禁成功行原文、8 夹具各自的退出码/诊断摘录/前后正式目录哈希、验证顺序清单。
