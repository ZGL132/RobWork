...
- 产出：实现并验证需求、表格、追踪和工作包完整性门禁。 以及可审计测试和结构化证据。
...
- 命令：`powershell -NoProfile -ExecutionPolicy Bypass -File .\RobWork\scripts\industrial-robot\run-tests.ps1 -Configuration Debug -Regex '^sdurws_ird_validation_gate_test$'`；脚本尚未创建时先执行 WP-01 交付的同名入口。
