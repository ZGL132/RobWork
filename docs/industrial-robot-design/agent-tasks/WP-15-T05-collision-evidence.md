...
- **证据工件：** `evidence/WP-15/collision-evidence-report.md`（共享评估器一致性对照、缺检测器案例、措辞记录）＋测试日志（命令、commit、配置、manifest 哈希）；独立验证者复核与 WP-07 结论一致性。
- **提交格式：** `WP-15-T05: add collision evidence`
