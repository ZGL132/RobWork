...
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "extrapolat|slope \* \(x -" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/selection/src/CurveInterpolation.cpp; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 无域外外推路径；样条/高阶插值关键词零命中（只允许分段线性）；需求侧字段在 `TemperatureDerating.cpp` 零写
