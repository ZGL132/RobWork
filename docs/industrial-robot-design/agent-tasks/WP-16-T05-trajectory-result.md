...
- **允许创建/修改/删除的文件：**（基目录 `RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/trajectory/`）
  ...
  - 创建：`src/TrajectoryJson.cpp`、`test/TrajectoryResultTest.cpp`、`out/test-evidence/wp-16/<run-id>/`（本卡工件）
...
- **证据工件：** `out/test-evidence/wp-16/<run-id>/trajectory-result-report.md`（Schema v1 字段清单、`ResolvedIkBranchSequence` 记录案例、JSON 往返哈希对照、AT-04 预览/修订对照）＋测试日志（命令、commit、配置、manifest 哈希）；独立验证者复核 payload 完整性与候选引用不复制。
