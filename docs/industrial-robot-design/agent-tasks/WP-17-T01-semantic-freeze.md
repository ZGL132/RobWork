...
- **精确验证命令**（仓库根、VS x64；三形式，仅用登记目标）：
  ...
  - 预期退出码 0
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "static_friction|DynamicsResult\b" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/dynamics; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 零命中（无独立静摩擦、禁名）；无 T02～T05 计算体文件混入；常量与 dynamics.md §5.1 逐字一致
