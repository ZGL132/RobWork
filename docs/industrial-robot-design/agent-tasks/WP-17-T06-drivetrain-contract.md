...
- **精确验证命令**（仓库根、VS x64；三形式）：
  ...
  - 预期两者退出码均 0
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "DriveTrain|efficiency|\bratio\b" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/plugins/dynamics; if ($LASTEXITCODE -eq 0) { throw '检测到禁止实现' } elseif ($LASTEXITCODE -ne 1) { throw '扫描命令执行失败' }` 零命中（映射归 WP-18）；`DynamicsResult` 禁名零命中；未改动 WP-18 目录任何文件
