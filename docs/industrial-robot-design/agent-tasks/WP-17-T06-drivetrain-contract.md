...
- **允许创建/修改/删除的文件**（模块根同 WP-17-T01）：
  - 创建：`test/DynamicResultContractTest.cpp`（公共头稳定化用例）、`evidence/WP-17/T06/`
...
- **证据工件：**`evidence/WP-17/T06/`——候选无关性联合验收记录（动力学/驱动工程师＋独立测试双评审签署）、映射消费接口评审纪要、公共头稳定化记录、两侧测试日志（commit/配置）
- **提交格式：**`WP-17-T06: stabilize dynamics contract for drivetrain`
