...
- **Task ID / 需求 ID / ADR / 阶段：** WP-22-T02；UX-01、UX-03、UX-06、AT-05；ADR-004、ADR-005；阶段 E / R1＋R2。契约：`architecture/evaluation-semantics.md` §4～§5、`architecture/execution-model.md` §1；模块详设 `module-design/workflow-integration.md` v0.4 §4、§10.3～§10.4，`module-design/session-ui.md` v0.4 §8.4。
...
- **修改后接口：** `NextStepAdvisor` 按 §4 规则表求值；驾驶舱把只读端口按阶段×评估入口聚合为 `StageStatusModel` 八值（输入未完整/可计算/计算中/结果有效/需要重算/证据不足/计算失败/工程不可行），每阶段同时投影问题数和简短下一步；任务面板复用 WP-10 的 `TaskState` 组件，不暴露内部身份值。
- **实施步骤：**
  ...
  4. 实现展示义务渲染：不可行按 `gaps` 列具体缺口并汇总问题数；`DataInsufficient/Partial/NotEvaluated` 显式展示不与不可行混排；`Superseded`＝“需要重算”；`Historical` 保留工程名称；Quick 不显示为正式通过。
