...
- **diff 和禁止项检查：**diff 仅含允许清单；静态扫描（效率/反射惯量/映射关键词）在本包外零命中；`rg -n "class .*MappingEvaluator" RobWork/RobWorkStudio/src/rwslibs/industrialrobot` 仅一处声明；无 Qt Widgets、无 RobWork 运行时对象
