...
- **diff 和禁止项检查：**diff 仅含允许清单；`regen" src/EnergySplitter.cpp|regen" src/EnergySplitter.cpp` 确认 W+ 只读自 `DynamicResult`（无本地功率积分重复实现）；分项字段名与 §9.3 口径一一对应，无"驱动器电能=0"式空值占位
