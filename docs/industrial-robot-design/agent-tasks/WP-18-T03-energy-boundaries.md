...
- **diff 和禁止项检查：**diff 仅含允许清单；`rg -n "W_plus|regen" RobWork/RobWorkStudio/src/rwslibs/industrialrobot/evaluation/drivetrain/src/EnergySplitter.cpp; if ($LASTEXITCODE -gt 1) { throw '扫描命令执行失败' }` 确认 W+ 只读自 `DynamicResult`（无本地功率积分重复实现）；分项字段名与 §9.3 口径一一对应，无"驱动器电能=0"式空值占位
