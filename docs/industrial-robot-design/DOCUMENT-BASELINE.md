...
| 当前架构检查点 | `IRD-D14-20260830`（D0～D14；契约检查点 `IRD-D10-20260829` 联合评审结论通过，2026-08-30 用户签署，契约与 ADR 状态 `Accepted`） |
| 基线日期 | 2026-08-29 |
...
| 架构 | `architecture/*.md` | `Accepted`（IRD-D10-20260829 联合评审通过；2026-08-30 签署生效；25 契约、76 符号、ADR-001～005） |
...
| `IRD-D14-20260830` | 2026-08-30 | D14 契约签署与实施启动解锁：用户指令"签署契约与 ADR"，评审记录 §5 签署栏完成四行签署（产品/需求所有者、契约所有者代表、消费模块代表由用户一次性授权，独立验证者由治理会话依据 WP-00-T01～T04 独立复跑证据签署）；§3 追认清单 7 项全部追认、无否决项；contract-registry、symbol-registry、ADR-001～005 与各契约文件头部状态整体升 `Accepted`（不改契约正文）；WP-00 治理任务 T01～T04 全部 `Done`（T04 实现提交 553c9b5），WP-01-T01 九项入口门禁复判通过转 `Ready` | 契约与 ADR 可作为实施依据，DOCUMENT-BASELINE §4 门禁 2 解除；模块方案与 Schema 仍为 `Proposed`，各自接受流程不变 |
