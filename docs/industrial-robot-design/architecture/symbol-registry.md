...
| `SYM-UI-001` | `SessionState` | 会话状态值对象 | `sessionId、projectRef、selectedObjectId、cameraPose、visibility、colorMode、filter、jogPose、playbackState、previewRef`；会话内可变，更新不产生 revision | WP-10 | public-interfaces §7 | 各插件私有会话/显示状态 | `Proposed` |
| `SYM-UI-002` | `EditDraft` | 编辑草稿值对象 | `draftId、baseRevisionRef、patches[]、validationDiagnostics[]、dirty、savedAt`；存 WP-04 drafts，不进入 `EvaluatorInputSlice` | WP-10 | public-interfaces §7 | 插件内私有草稿结构 | `Proposed` |
| `SYM-UI-003` | `StageStatusModel` | 阶段状态值对象 | 八值阶段状态（输入未完成/可计算/计算中/结果有效/需要重算/证据不足/计算失败/工程不可行）；与 `ResultCurrentness` 映射，不与 `TaskState` 混用 | WP-10 | public-interfaces §7 | 各插件私有阶段状态映射 | `Proposed` |
| `SYM-UI-004` | `SelectionModel` | 会话状态值对象 | `selectedObjectIds[]`（有序 objectId）、`primaryObjectId`；只写 `SessionState` 对应字段，不反写领域数据 | WP-10 | public-interfaces §7 | 场景/列表各自私有选择缓存 | `Proposed` |
| `SYM-TRJ-002` | `ResolvedIkBranchSequence` | 结果值对象 | 每路点实际采用 IK 解（q、分支标识、残差、候选 ID）；引用 `KinematicResult` 候选，不复制完整 payload | WP-16 | CTR-API-003 | 在轨迹内复制候选 payload | `Proposed` |
| `SYM-TRJ-003` | `IkBranchPolicy` | 策略值对象 | 自动分支策略、用户显式锁定和连续性阈值；经 `AnalysisConfiguration` 进入输入切片 | WP-16 | CTR-API-003 | 散落在采样/规划代码内的分支开关 | `Proposed` |
| `SYM-DTM-001` | `MotorSideOperatingPoint` | 计算值对象 | 每轴电机侧转矩/转速/功率序列、峰值/RMS、反射惯量与惯量比、能量分项和工作制统计；单位 N·m/rad/s/W | WP-18 | CTR-DOM-003 | 关节侧与电机侧量纲混排 | `Proposed` |
