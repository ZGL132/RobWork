...
本表是契约索引，不替代契约正文。契约 ID 一经发布不得复用；契约语义只能在“权威位置”修改，消费者文档只能引用。D1 完成“登记和消歧”，D2 完成核心语义，D3 完成字段、Schema、接口和机器校验；未达到目标门禁的条目保持 `Accepted`。
...
- D10（`IRD-D10-20260829`）：语义闭合修复（任务级循环依赖、诊断注册表 117 码裁决一致、跨模块符号补登记、WP-04/WP-12 口径统一、验证命令可执行化）后完成契约联合评审，全部契约与 ADR 转为 `Accepted`；评审记录见 [review/2026-08-29-contract-review.md](review/2026-08-29-contract-review.md)。此后语义变更必须走契约变更流程并提升版本。
