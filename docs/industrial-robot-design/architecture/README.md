...
> 当前检查点：`IRD-D2-20260829`（D1 登记消歧 + D2 五大核心契约正文完成；等待消费者评审签署）  
> 目录状态：`Proposed`。完成所有者/消费者签署与 D3 Schema/接口机器验证后，方可逐份转为 `Accepted`。
...
- 所有契约在联合签署前保持 `Proposed`，工作包不得据此跳过实施入口门禁。
...
公共契约必须由对应所有者提出变更，附 ADR、消费者影响清单和契约测试；未完成评审前保持 `Proposed`，不得由业务 WP 私自修改。消费者发现缺口时应提出变更，不得在模块方案或实现中创建第二套定义。
