...
- 负责 WP：WP-20/WP-21；阶段/发布：阶段 B 子集 / R1；阶段 D 全量 / R2
- 架构契约：`architecture/execution-model.md`、`architecture/public-interfaces.md`
- 任务卡：对应 WP 的全部 agent-tasks/WP-20-T*.md

## 1. 目标与非目标

- 目标：静态 OPT-B 子集、联合搜索、Quick/Verified、Hard/Soft、缓存、Pareto 和应用修订。
- 非目标：不修改需求语义、不复制其他模块的公共接口、不把私有实现细节升级为跨模块契约。

## 2. 代码与构建

- 拥有目录：`industrialrobot/plugins/optimization/`；公共类型和接口只能位于该模块的 include 子目录；测试与夹具位于对应 test/testdata 子目录。
- CMake target：`industrialrobot_plugins_optimization`；实现只能依赖本方案列出的架构契约和 WP 前置 target。
- 禁止依赖：Qt Widgets（纯 UI 模块除外）、当前界面选择、其他 WP 私有头、未登记第三方库和跨模块文件写入。

## 3. 数据与接口

- 核心对象：OptimizationStudyDefinition、DesignCandidate、ParetoSet、Checkpoint。输入必须携带 projectId/branchId/revisionId 或明确的快照身份；浮点量使用 SI 单位且必须有限。
...
- 任务卡按 WP-20/WP-21 索引执行；每张卡一个 worktree、一个分支和一个可评审提交，公共接口只能由所有者 WP 修改。
