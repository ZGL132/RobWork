...
类型为硬约束、工程约束、偏好。安全要求和需求中的必须项固定为硬约束，不能降级或关闭；工程约束可调整，偏好只用于查看和次级排序，不改变可行性。页面不提供偏好权重，也不计算加权总分。
...
Pareto 图设置：横轴、纵轴、颜色、点大小、筛选、仅显示正式复核。点的悬停卡片显示候选名称、四个主要指标和状态；点击后同步三维预览和候选详情。
...
| 设为当前方案 | 正式复核通过、审计通过且结果有效 | 打开采用确认 |
