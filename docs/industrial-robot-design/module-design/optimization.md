...
- 负责 WP：WP-20（阶段 B / R1，OPT-B）＋WP-21（阶段 D / R2，全量）；任务卡：`agent-tasks/WP-20-T01～T08`、`WP-21-T01～T07`
...
默认目标为节拍、结构质量、器件成本；静态模式自动改为当前可计算的指标，并说明“完整目标需要关节优化”。选择五项及以上时显示“目标较多，结果可能较难比较”，但不阻止运行。
...
| 候选 | 可行性 | 尺寸包络 | 结构质量 | 节拍 | 正机械功 | 器件质量 | 器件成本 | 最小关节裕量 | 最小驱动裕量 | 状态 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
