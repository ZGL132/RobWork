...
- 负责 WP：WP-13；阶段/发布：阶段 B / R1
- 架构契约：`architecture/domain-model.md`、`architecture/persistence-schema.md`、`architecture/public-interfaces.md`
- 任务卡：对应 WP 的全部 agent-tasks/WP-13-T*.md

## 1. 目标与非目标

- 目标：SE(3) 为唯一真值、URDF/DH 仅输入或投影、任意轴、物性和单机械臂主链。
- 非目标：不修改需求语义、不复制其他模块的公共接口、不把私有实现细节升级为跨模块契约。

## 2. 代码与构建

- 拥有目录：`industrialrobot/plugins/modeling/`；公共类型和接口只能位于该模块的 include 子目录；测试与夹具位于对应 test/testdata 子目录。
- CMake target：`industrialrobot_plugins_modeling`；实现只能依赖本方案列出的架构契约和 WP 前置 target。
- 禁止依赖：Qt Widgets（纯 UI 模块除外）、当前界面选择、其他 WP 私有头、未登记第三方库和跨模块文件写入。

## 3. 数据与接口

- 核心对象：RobotDesignEditor、URDF/DH 导入、CompiledCandidateArtifact。输入必须携带 projectId/branchId/revisionId 或明确的快照身份；浮点量使用 SI 单位且必须有限。
...
- 任务卡按 WP-13 索引执行；每张卡一个 worktree、一个分支和一个可评审提交，公共接口只能由所有者 WP 修改。
