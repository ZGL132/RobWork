...
> 目录状态：全部 23 篇方案已按 `IRD-D2-20260829` 冻结契约重写为 v0.3（平台 10 篇＋业务 12 篇＋testkit），并于 `IRD-D10-20260829` 完成语义闭合修复与联合评审（结论：通过，`Proposed` 待签署）。此后文件变更走契约/详设变更流程；实现启动按总纲依赖顺序与任务状态账本（`agent-tasks/task-status.md`）。
