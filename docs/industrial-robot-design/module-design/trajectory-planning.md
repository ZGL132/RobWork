...
本节定义任务序列到可验证轨迹的工程界面。任务序列只读使用需求阶段数据；需要修改工位或顺序时跳转需求页，不在轨迹页维护第二份任务。
...
│ [规划] [重规划] [取消]   │                                   │               │
├──────────────────────────┴───────────────────────────────────┴───────────────┤
│ 轨迹段 │ q │ 速度 │ 加速度 │ 碰撞 │ 分支 │ 诊断 │ ◀ 播放 ▶             │
└──────────────────────────────────────────────────────────────────────────────┘
```

左侧任务序列显示序号、名称、工艺和就绪状态；不可直接新增、删除或改序。选择任务时三维视图定位目标，轨迹段表筛选到相关段。

### 8.2 轨迹段

| 序号 | 来源 | 起点/终点 | 类型 | 状态 | 开始时间 | 持续时间 | 最小裕量 | 碰撞 | 限位 | 原因 |
| ---: | --- | --- | --- | --- | ---: | ---: | ---: | --- | --- | --- |

首批界面只显示已交付类型：关节运动、笛卡尔直线、停留。未交付的圆弧和跃度能力不显示按钮、空列或占位页。

当前段属性：来源任务、段类型、起点、终点、开始时间、持续时间、分支、最小关节裕量、碰撞结论、限位结论、状态和原因。规划结果字段只读。

### 8.3 规划设置与操作

| 分组 | 字段 |
| --- | --- |
| 常用 | 速度比例、加速度比例、平滑方式、碰撞策略、分支策略 |
| 高级 | 规划预算、随机种子、采样细度、连续性设置、时间参数设置 |

公共工程设置是唯一策略入口；当前页只显示本次使用摘要和“打开工程设置”。

| 操作 | 启用条件 | 状态规则 |
| --- | --- | --- |
| 规划 | 任务与运动学结果有效 | 启动完整规划；运行中禁用 |
| 重规划 | 已有结果或设置已变化 | 保留旧结果供查看，新结果完成后替换当前结果 |
| 取消 | 存在运行任务 | 请求后显示“正在取消” |
| 定位问题 | 存在失败或警告段 | 选中首个问题段并在三维视图高亮 |
| 前往需求 | 任务定义需要修改 | 跳转并选中来源工位 |
| 前往运动学 | 分支或可达性需要修改 | 跳转并选中对应目标 |

### 8.4 曲线与播放

曲线标签页：关节位置、速度、加速度。横轴统一为时间，纵轴显示关节名称和单位。图例可隐藏单个关节，支持框选缩放和“适合窗口”。

```text
时间 [ 00:02.350 / 00:08.420 ]  [|◀] [◀] [播放/暂停] [▶] [▶|]  速度 [100%]
```

- 拖动时间游标时，三维视图、当前轨迹段、曲线竖线和碰撞表同步。
- 播放、暂停、逐段和速度调整只改变会话显示，不修改轨迹结果。
- 播放期间切换阶段会暂停；返回后从当前时间继续。
- 结果过期时仍允许播放，但顶部持续显示“旧结果，仅供查看”。

### 8.5 碰撞、限位与分支

碰撞表：

| 时间 | 轨迹段 | 对象 A | 对象 B | 距离/穿透 | 结论 | 建议 |
| ---: | --- | --- | --- | ---: | --- | --- |

限位表：

| 时间 | 轨迹段 | 关节 | 项目 | 实际值 | 限值 | 超出量 | 建议 |
| ---: | --- | --- | --- | --- | --- | --- | --- |

分支表：

| 路点 | 任务 | 分支 | 与前一点变化 | 连续性 | 状态 |
| ---: | --- | --- | ---: | --- | --- |

选择任一问题行都同步定位三维姿态和时间游标。没有距离查询能力时显示“距离不可用”，不得推断为安全。

### 8.6 页面状态

| 场景 | 提示 | 主要操作 |
| --- | --- | --- |
| 无任务 | 请先完成可达的任务定义 | 前往需求 |
| 运动学过期 | 运动学结果需要重新计算 | 前往运动学 |
| 尚未规划 | 输入已就绪，可生成轨迹 | 规划 |
| 部分段失败 | 已保留完成段，请处理首个失败段 | 定位问题 |
| 轨迹过期 | 上游方案已变化，旧结果仅供查看 | 重规划 |
| 规划失败 | 未生成可用轨迹，输入与设置已保留 | 重试 |

部分失败时，完成段保持可见但不得计算正式节拍或进入动力学。导出入口只在对应导出能力已经交付且结果有效时显示。
