...
- 负责 WP：WP-25；阶段/发布：阶段 E / R2
- 架构契约：`architecture/testing-contract.md`、`architecture/persistence-schema.md`
- 任务卡：对应 WP 的全部 agent-tasks/WP-25-T*.md

## 1. 目标与非目标

- 目标：真实机械臂固定试点、对照测量、用户测试、缺陷关闭和交付材料。
- 非目标：不修改需求语义、不复制其他模块的公共接口、不把私有实现细节升级为跨模块契约。

## 2. 代码与构建

- 拥有目录：`docs/industrial-robot-design/pilot/`、样例项目、报告模板、用户手册与培训材料（与 WP-25 声明一致）；本模块为非代码模块，不拥有 include/test 目录。
- CMake target：无（试点协议、签署记录与报告为文档工件，验证方式为人工复核与签署，不以空 CTest 目标替代——见 testing-contract.md §4）。
- 禁止依赖：Qt Widgets（纯 UI 模块除外）、当前界面选择、其他 WP 私有头、未登记第三方库和跨模块文件写入。

## 3. 数据与接口

- 核心对象：PilotProtocol、UserStudyReport、DefectRegister。输入必须携带 projectId/branchId/revisionId 或明确的快照身份；浮点量使用 SI 单位且必须有限。
...
- 任务卡按 WP-25 索引执行；每张卡一个 worktree、一个分支和一个可评审提交，公共接口只能由所有者 WP 修改。
